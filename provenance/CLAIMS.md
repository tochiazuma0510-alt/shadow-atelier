...
| # | 主張 | 証拠 | 状態 | 日付 |
|---|---|---|---|---|
| C-1 | K⁽ⁿ⁾(n=3..12)で \|Gₙ\| = 4n³(奇)/4(n/2)³(偶)、K_ord = lcm(n,2)、K⁽ⁿ⁾=K⁽²ⁿ⁾(n=3,5,7,9,11) | GAP(`week1-kn-spotcheck.g`/`suite-wp1.g`)+**独立照合器 `crosscheck/check.mjs` の自前再計算が一致**(2026-07-18 WP2 統合・LEDGER 参照) | **cross-checked**(2026-07-18 昇格) | 2026-07-18 |
| C-2 | 宇宙 {3..16,18,36} で C-1 の数値事実+doubling(13,15)+**Prop 3.5 の包含が全 256 順序対で marked factor map と一致**+N₅ control(GT(N₅) = {0,1,3,4}・T(c)=c^{2m+1}) | WP1 ALL PASSED(GAP)+**WP2 統合で N₅・数値事実・包含(branch 5 対の reduction として)が照合器と一致** | **cross-checked**(2026-07-18 昇格) | 2026-07-18 |
