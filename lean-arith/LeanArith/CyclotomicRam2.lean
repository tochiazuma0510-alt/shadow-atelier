...
`(4 * n)`-cyclotomic extension of `ℚ` is `2 ^ 1 * (2 - 1) = 2`.
-/
...
/-- In a `(4 * n)`-cyclotomic extension of `ℚ`, with `n` odd, the global
ramification index of the ideal `(2)` is `2`. -/
theorem cyclotomic_ramificationIdxIn_two
    (n : ℕ) (hn : Odd n)
    (K : Type*) [Field K] [NumberField K]
    [IsCyclotomicExtension {4 * n} ℚ K] :
    (Ideal.span {(2 : ℤ)}).ramificationIdxIn (NumberField.RingOfIntegers K) = 2 := by
...
theorem cyclotomic_ramificationIdx_two
    ...
    [P.IsPrime] [P.LiesOver (Ideal.span {(2 : ℤ)})] :
    ramificationIdx P ℤ = 2 := by
  have hfactor : 4 * n = 2 ^ (1 + 1) * n := by norm_num
  simpa using
