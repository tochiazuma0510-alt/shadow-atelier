...
と置くと、求めるものは有限集合系の逆極限 \(\varprojlim_KY_K(g)\) の元である。各 (Y_K(g)\ne\varnothing\) に加えて遷移の整合性または ML 条件が要る。その遷移全射を示す局所条件が (REL-VANISH)、(GEN-COVER)、charming の同時実現である。従って逆極限へ移っても同じ三前件に戻る。
...
> **命題.** 有限部分族 \(\mathcal F\subset\mathcal I_M\) が cofinal と証明されていないなら、全 (K\in\mathcal F\) で (R_{K,M}\) が全射であっても、(0.1) の全称結論は従わない。

**証明.** Cor 5.4 の量化には \(\mathcal F\) 外の refinement が残る。`MONO-CNF-139` により、そこで像が真部分集合となる (K\) が一つあれば、その下の refinement は全て非全射であり、有限な成功例からその可能性を除けない。逆に cofinality があれば同命題と COMPACT で閉じる。∎
...
1. 位数 \(\le B\) の有限群 (E\) と、固定した (PB_3/M\) への標識整合な全射 (E\twoheadrightarrow PB_3/M\) を重複なく列挙する。
2. (PB_3\twoheadrightarrow E\) の kernel (L\subset M\) について (B_3\)-安定性を証明書化する。
