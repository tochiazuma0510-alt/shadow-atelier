# R07 compact separated leading-onto lift v376

Author: Sol / 2026-08-30

Status: **REJECTED DRAFT; superseded by v377.**  The abstract theorem in
Sections 1--3 is sound, but this draft omits the physical equality between
the task395 closure and the leading image of `B_C`, and its final dependency
sentence is off by one step.  Nothing in this draft is promoted.  The abstract
...
If \(f:M\to N\) is continuous and \(\Lambda\)-linear, then \(f(M)\),
...
3. the actual occurrence image fills the entire target,
   ...
   containing one chosen defect.

Under these three facts, (5.2) identifies the reduction of
\(\operatorname {im}B_C\) with all of
...
2. construct (5.3) and prove its isomorphism, then compute full leading
   onto (4.9), or instead prove the direct all-rung nonemptiness criterion;
...
Only after items 1--4 are established for the same physical owner does the
linear tail cease to be an issue and (6.1) become the remaining analytic
Newton step.  Nothing here changes the current fake/Ihara status.
...
`R07_COMPACT_SEPARATED_LEADING_ONTO_LIFT_V376_AUDIT_READY`
