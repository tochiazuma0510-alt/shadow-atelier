# R07 reachable-class one-defect theorem with intrinsic saturation v382

Author: Sol / 2026-08-30

Status: corrected final-audit paper theorem replacing rejected v380 and v381.  The compact
...
### Theorem 6.2 (SECTION-FREE REACHABLE NEWTON RECURSION)

Assume Theorem 5.1.  Suppose that the full filtered Fox package (6.3) and
the intrinsic saturation vanishing (6.5) hold for every pair requested by
the recursion.  Then one may construct a Cauchy product of actual
registered-lane corrections whose limiting residual is zero on the relative
pro-\(3\) lane.  No finite-free target cover, based right lift, or global
section is required.

#### Proof

Put \(w_0=\widehat w_0\) and
\(z_d=\Phi_{\rm lane}(w_d)\).  At \(d=0\), and inductively whenever

\[
 z_d\in\widehat J^dL_{\rm reach},
\tag{6.6}
\]

the strict equality (5.3) supplies a choice
...
V369 Theorem 3.1 materializes this one requested value by
...
Set \(w_{d+1}=w_dc_d(t_d)\).  Equations (6.2), (6.4), and (6.7)
give

\[
 z_{d+1}
 =z_d+B_C(t_d)+\epsilon_d(w_d,t_d)
 \in\widehat J^{d+1}L_{\rm reach}.
\tag{6.9}
\]

This proves (6.6) by induction.  V369 Lemma 1.2 makes the correction
product Cauchy, so \(w_d\) converges to some
\(w_\infty\in\widehat w_0\widehat{\mathscr Q}_C\).  Continuity of the
enriched residual gives
\(\Phi_{\rm lane}(w_\infty)\in\widehat J^dL_{\rm reach}\) for every
\(d\).  Separation then gives
\(\Phi_{\rm lane}(w_\infty)=0\). \(\square\)

This is the exact remaining nonlinear condition; it is not replaced by
ambient depth gain alone.
...
COMPATIBLE RELATIVE PRO-3 LIFT:                       CONDITIONAL THEOREM / ACTUAL OPEN
MIXED-PRIME / PERFECT-CORE / SETTLEMENT:              OPEN
FAKE / IHARA WITNESS:                                 NOT DECLARED
~~~

\(\mathtt{R07\_REACHABLE\_CLASS\_ONE\_DEFECT\_WITH\_SATURATION\_V382\_FINAL\_AUDIT}\)
