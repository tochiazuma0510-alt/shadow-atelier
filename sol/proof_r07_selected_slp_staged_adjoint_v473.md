# R07: staged adjoint evaluation of the selected-SLP DAG (v473)

Author: Sol / 2026-09-03

Status: candidate resource repair for the Task601 leaf evaluator after GHA
run `33723160379`.  It changes only the evaluation schedule of the same
authenticated noncommutative DAG.  It does not prune a source edge, identify
actor paths by a finite quotient, or assert an actual selected payload,
fresh residual, grade decision, A0, fake, or Ihara conclusion.
`verified=false`.
...
exact words, and let (E_{m reached}) count constructor edges incident to
nonzero accumulated states.  The staged work is charged to
...
bound; the theorem does not assert that (5.1) is uniformly small.

The producer and checker must report, per stage, the processed node count,
nonzero accumulated-state count, number of exact interned paths, maximum path
length, maximum simultaneously live entries, final leaf count, wall time,
RSS, and peak RSS.  Explicit caps on accumulated states, interned words,
path length, wall time, RSS, and durable bytes fail as
`UNKNOWN_RESOURCE`.  A partial leaf map is never a payload.
...
STAGED EXACT-PATH ADJOINT:              PAPER-CLOSED
...
`R07_SELECTED_SLP_STAGED_ADJOINT_V473_CANDIDATE`
