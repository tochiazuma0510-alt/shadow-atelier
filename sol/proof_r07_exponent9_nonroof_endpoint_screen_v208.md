...
all in (mathbf Z/9mathbf Z).  In particular,
...
The 729-state bound closes only the translating-word orbit.  It does not by
itself supply a complete presentation of the first-successor kernel
(H_1), nor does it replace v200's complete Schreier or normal-relator
input.  The v188 compressed successor construction must retain enough
word-bearing ancestry to provide that input.  Likewise, non-roof status in
Theorem 3.1 does not predict whether the actual endpoint or its repair dual
is nonzero.
