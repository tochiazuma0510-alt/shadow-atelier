#!/usr/bin/env python3
"""Helper-nonshared v12a checker for SELFTEST_BOOTSTRAP only.
...
validators; production and resume entry points do not exist in v12a.
"""
...
class Sources:
    ...
    def authenticate(self, meter: CheckerMeter | None = None) -> None:
        for key, (relative, size, digest) in SOURCE_PINS.items():
            path = ROOT / relative
            raw, _identity = read_owner_bytes(path, size, digest, meter)
            self.raw[key] = raw

    def load(self, key: str) -> Any:
        if key in self.modules:
            return self.modules[key]
        name = "_d972_v12a_checker_" + key
...
def checker_q0_parent_letter_digest(owners: dict[str, Any]) -> str:
    ...
    return sha_obj([parents, letters])
...
def _checker_process_case(runtime: dict[str, Any], dual: Sparse, workers: int,
    ...
    try:
        ...
        return {"workers": workers, "epoch": 1, "fault": fault,
                "atomic_discard": True,
                "fault_reason": type(exc).__name__}
    finally:
...
def checker_process_selftest(runtime: dict[str, Any],
    ...
    for workers in (2, 4):
        ...
        for fault in ("timeout", "death", "partial"):
            runs.append(_checker_process_case(runtime, dual, workers, fault))
    parent, child = socket.socketpair(socket.AF_UNIX, socket.SOCK_STREAM)
    context = multiprocessing.get_context("fork")
    deadline = time.monotonic() + 0.05
    process = context.Process(target=_checker_no_drain, args=(child, deadline),
                              name="r07-v12a-checker-blocked-send")
    process.start(); child.close()
    rejected = False
    try:
    ...
    return {"first_dual": public_sparse(dual),
            ...
            "cumulative_accounting": {"completed_additive": cumulative,
                ...
                "fault_owner_count": 6,
                "composition": "v290-additive; host telemetry excluded"},
...
def _checker_validate_triangular_owner_frame(
    ...
    checker_validator_event(trace_runtime, "validate_triangular_owner_frame",
                            "triangular", "old.columns")
    require(type(frame) is dict and set(frame) == {
            "schema", "columns", "P_rows_sha256", "self_digest"} and
            frame.get("schema") ==
            "d972-r07-history-free-positive-fast-resume/v12a/triangular-physical-frame" and
    ...
    for expected_id, record in enumerate(frame["columns"], 1):
        ...
        require(type(ancestry) is list and
                ...
                "checker physical triangular ancestry")
        try:
        ...
        require(pivot and pivot not in pivot_set,
                "checker physical triangular pivot identity")
    ...
    for record, pivot in zip(frame["columns"], pivots):
        ...
        require(product and min(product) == pivot and product[pivot] == 1 and
                ...
                "checker physical triangular P equation")
...
def validate_selftest_v12a(runtime: dict[str, Any], receipt: dict[str, Any]) -> None:
    ...
    require(type(receipt.get("triangular_owner_frame")) is dict,
            "selftest triangular physical owner transport")
    ...
    require(type(cumulative) is dict and
            ...
            cumulative.get("fault_owner_count") == 6 and
            cumulative.get("composition") ==
                "v290-additive/max; host telemetry excluded" and
...
def validate_selftest_envelope(receipt: dict[str, Any],
    ...
    p0, p0_raw, _p0_identity = open_physical(
        p0_path, 16 * 1024 * 1024, meter=meter)
    ...
    for key, path in expected_paths.items():
        ...
        raw, _identity = read_owner_bytes(ROOT / path, row["bytes"],
                                          row["sha256"], meter)
        require(len(raw) == row["bytes"], "SELFTEST P0 source bytes:" + key)
    ...
    require(type(p0_reference) is dict and
            ...
            p0_reference.get("physical_sha256") == sha_bytes(p0_raw),
    ...
    return {"selftest": True, "production_invocations": 0,
            ...
            "p0_physical_sha256": sha_bytes(p0_raw)}
...
def validate_p0_checker(path: Path,
    ...
    paths = {"producer_v12a":
                "search/d972_r07_history_free_positive_fast_resume_v12a.py",
        "checker_v12a":
                "crosscheck/check_d972_r07_history_free_positive_fast_resume_v12a.py",
        "fixture_v12a": FIXTURE_PATH}
    ...
    for key, relative in paths.items():
        ...
        source_raw, source_identity = read_owner_bytes(
            ROOT / relative, row["bytes"], row["sha256"], meter)
    ...
    return value, {"path": str(path.relative_to(ROOT)).replace("\\", "/"),
        ...
        "self_digest_sha256": claimed, "physical": physical}
...
def checker_triangular_mutation_ledger(runtime: dict[str, Any],
    ...
    """
    fixture = load_fixture_bounded()
    baseline = clone(receipt["triangular_owner_frame"])
    baseline_revalidated = False
    ...
    for ordinal, name in enumerate(names):
        ...
        _before, after, reason = checker_mutation_trace(
            ...
            lambda candidate: _checker_validate_triangular_owner_frame(
                candidate, trace_runtime), baseline_value=baseline,
...
def checker_boundary_mutation_ledger(runtime: dict[str, Any],
    ...
    outcome = independent_boundary_outcome(runtime, dual, workers, 1)
...
def checker_transport_mutation_ledger(runtime: dict[str, Any],
    ...
    fixture = load_fixture_bounded(); baseline = clone(receipt)
...
def checker_phase_mutation_ledger(runtime: dict[str, Any],
    ...
    """Independent phase-owner gate checks; no detached producer transcript."""
    fixture = load_fixture_bounded()
    ...
    def validate(value: dict[str, Any]) -> None:
        checker_validator_event(runtime, "phase_owner.validate", "preselection",
                                "phase-owner-frame")
        validate_seal(value)
        require(set(value) == set(baseline) and
                ...
                value.get("claims") == FALSE_CLAIMS, "phase gate")
    actions = {
    ...
    def validate_positive_gate(name: str, value: dict[str, Any]) -> None:
        checker_validator_event(runtime, "phase_owner." + name,
...
def validate_owned_checker_paths(manifest: Path, receipt: Path,
    ...
    expected_manifest = ROOT / "ci/in/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12a.manifest.v1.json"
    ...
    for label, target, required in (("manifest", manifest, True),
        ...
        if required:
            try:
                ...
                raise CheckStop("v12a " + label + " unavailable") from exc
            require(stat.S_ISREG(item.st_mode) and item.st_nlink == 1,
                    "v12a checker " + label + " owner")
        elif os.path.lexists(str(target)):
            item = os.lstat(target)
            require(stat.S_ISREG(item.st_mode) and item.st_nlink == 1,
                    "v12a checker " + label + " output identity")
    for relative in OUTPUT_SIBLINGS:
        target = ROOT / relative
        if relative == R_OUTPUT_PATH:
            require(os.path.lexists(str(target)), "v12a receipt missing")
        else:
            require(not os.path.lexists(str(target)),
                    "stale v12a checker transport:" + relative)


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description="v12a SELFTEST_BOOTSTRAP checker")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    require(manifest_path.resolve() ==
            (ROOT / "ci/in/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12a.manifest.v1.json").resolve(),
            "v12a P0 manifest required")
    checker_meter.check("p0_before_heavy")
    p0, p0_identity = validate_p0_checker(manifest_path, checker_meter)
    checker_p0_public = {
    ...
    sources = Sources(); sources.authenticate(checker_meter)
    ...
    validate_selftest_v12a(runtime, receipt)
    ...
    with tempfile.TemporaryDirectory(prefix="r07-v12a-checker-selftest-") as directory:
        root = Path(directory)
    ...
    verdict_body = {"schema": VERDICT_SCHEMA, "status": "CANDIDATE_ONLY",
        ...
        "receipt_physical": receipt_projection, "receipt_bytes": len(receipt_raw),
