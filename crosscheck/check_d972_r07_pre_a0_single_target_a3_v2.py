...
P0_PATH = "ci/in/d972_r07_pre_a0_single_target_a3_v2.prereg.v1.json"
P0_BYTES = 13748
P0_SHA256 = "4a7f966d803dfdf7977dc73ab2db9b7f54d0d932ec2a10e410e034b5b6b151af"
P0_SELF_SHA256 = "f633498efef1e8fb2adf02a474dcca05b87de83fe5655e1e805f8e10f1f916a4"
SCHEMA = "d972-r07-pre-a0-single-target-a3/v2"
...
def authenticate_task198(p0, raw_by_path):
    ...
    require(manifest["receipt"] == {
        "basename": authority["accepted_receipt_basename"],
        "bytes": authority["receipt"]["bytes"],
        "self_digest_sha256": receipt_seal,
        "sha256": authority["receipt"]["sha256"]} and
            manifest["task198_source_identities"] == authority["source_identities"],
            "task198 raw/manifest/source binding")
    ...
    return rows_out, evaluator_out, meta
...
def make_projection(full_abi, rows, targets, task198_meta, p0):
    occurrences = []
...
def evaluator_exercise(contract, rows, g760, central, targets):
    ...
    return {"contract_sha256": digest_obj(contract),
...
def mutation_fixture(rows, p0, task198_meta, g760, interface, consumer,
                     central, areas, targets, base_ref):
    return {"authority": {
                "manifest_contract": copy.deepcopy(task198_meta["manifest_contract"]),
            ...
            "base": copy.deepcopy(base_ref), "central": copy.deepcopy(central),
            "areas": copy.deepcopy(areas), "area_target": copy.deepcopy(targets),
            "interface": copy.deepcopy(interface),
            "consumer": copy.deepcopy(consumer),
            "flags": {flag: False for flag in FALSE_FLAGS}}


def _seal_matches(value):
    if type(value) is not dict or type(value.get("self_digest_sha256")) is not str:
        return False
    body = dict(value)
...
def cheap_validate(value, reference, p0):
    expected_manifest = p0["authority"]["task198"]["manifest_contract"]
    manifest = value["authority"]["manifest_contract"]
    if (manifest != expected_manifest or
            manifest["producer"]["member"]["sha256"] !=
                value["authority"]["receipt_sha256"] or
            manifest["checker"]["member"] != manifest["producer"]["member"]):
        raise NarrowReject("task198 raw/manifest binding")
    for actual, expected in zip(value["ledger"], make_ledger()):
    ...
    if value["ledger"] != make_ledger():
        raise NarrowReject("task198 ledger roster")
    if word_digest(value["g760"]) != EXPECTED_G760_SHA256:
        raise NarrowReject("g760 digest")
    if value["base"].get("mode") != "PRE_A0_COMPUTATIONAL_BASE_ONLY":
        raise NarrowReject("computational-base mode")
    if value["base"].get("task192_consumed") is not False:
        raise NarrowReject("task192 binding")
    for block in ("H1", "H2", "P"):
        if (value["central"].get("blocks", {}).get(block) !=
                reference["central"]["blocks"][block] or
                value["central"]["blocks"][block]["signed_rows"] !=
                p0["central_rows"][block]["rows"]):
            raise NarrowReject(block + "_central_row")
    if (value["area_target"] != reference["area_target"] or
            any(row.get("bar_epsilon_1") != reference["area_target"]
                for row in value["areas"].get("rows", []))):
        raise NarrowReject("projected area target")
    if (not _seal_matches(value["interface"]) or
            not _seal_matches(value["consumer"]) or
            value["consumer"].get("bar_epsilon_1") !=
                value["interface"].get("target_blocks")):
...
def run_mutations(rows, p0, task198_meta, g760, interface, consumer,
                  central, areas, targets, base_ref, meter):
    roster = p0["mutation_roster"]
    reference = mutation_fixture(rows, p0, task198_meta, g760, interface,
                                 consumer, central, areas, targets, base_ref)
    require(cheap_validate(reference, reference, p0) is True,
            "checker untouched cheap baseline")
    records = []
    for name in roster:
        mutant = copy.deepcopy(reference)
        if name == "task198_raw_manifest_binding": mutant["authority"]["manifest_contract"]["producer"]["member"]["sha256"] = "0" * 64; owner = "authority"
        ...
        elif name == "H1_central_row": mutant["central"]["blocks"]["H1"]["signed_rows"][0][0] = 3; owner = "central"
        elif name == "H2_central_row": mutant["central"]["blocks"]["H2"]["signed_rows"][0][0] = 6; owner = "central"
        elif name == "P_central_row": mutant["central"]["blocks"]["P"]["signed_rows"][0][3] = 3; owner = "central"
        elif name == "projected_area_target": mutant["areas"]["rows"][0]["bar_epsilon_1"]["H1"] = []; owner = "areas"
        elif name == "ABI_seal_target": mutant["consumer"]["bar_epsilon_1"]["H1"] = []; owner = "consumer"
        ...
        observed = None
        try:
            cheap_validate(mutant, reference, p0)
        ...
        records.append({"name": name, "changed_field": owner,
                        ...
                        "first_gate": observed, "rejected": True})
...
def load_engine(rel, size, sha, tag_name, meter):
    ...
    spec = importlib.util.spec_from_file_location("_d363_checker_" + tag_name, path)
    ...
    require(digest_bytes(raw) == sha, "pre-import source drift:" + tag_name)
    post = read_bytes(rel, size, sha, meter, "post-import:" + tag_name,
                      authority=False)
    require(digest_bytes(post) == sha, "post-import source drift:" + tag_name)
    del raw, post
...
def read_receipt(path_text, expected_sha, meter):
    ...
    require(len(raw) == info.st_size and digest_bytes(raw) == expected_sha,
...
def atomic_publish(target, raw):
    prefix = "." + target.name + ".tmp."
    temporary = target.parent / (prefix + str(os.getpid()) + "." +
                                 str(time.time_ns()))
    descriptor = -1
    directory = -1
    renamed = False
    try:
        ...
        result = renameat2(-100, os.fsencode(temporary), -100,
        ...
        if renamed:
            try:
                os.unlink(target)
                rollback = os.open(target.parent,
                                   os.O_RDONLY | getattr(os, "O_DIRECTORY", 0))
                try:
                    os.fsync(rollback)
                finally:
...
def verdict_document(receipt_path, receipt_raw, terminal, accepted, p0,
    ...
    value = {"schema": VERDICT_SCHEMA, "status": terminal,
             ...
             "receipt_sha256": digest_bytes(receipt_raw) if receipt_raw else None,
...
def emergency_unknown(receipt_path, receipt_raw, terminal, exc, meter, p0, pins):
    reason = str(exc)[:512]
    ...
    return verdict_document(receipt_path, receipt_raw, terminal, False, p0,
                            pins, reason=detail)


def reconstruct_context(p0, pins, rows, evaluator_contract, task198_meta, meter):
    ...
    evaluator_trace = evaluator_exercise(evaluator_contract, rows, g760,
                                         central, targets)
    interface, consumer, seal_trace = make_projection(
        full_abi, rows, targets, task198_meta, p0)
    base_ref = base_reference(full_abi, g760)
    areas = area_canary(t226, g760, rows, targets, meter)
    mutations = run_mutations(rows, p0, task198_meta, g760, interface,
                              consumer, central, areas, targets, base_ref, meter)
...
def check_certificate(receipt_path, receipt_raw, receipt, p0, context, meter):
    ...
    require(resource_meter.get("caps") == CAPS and
            ...
            resource_meter.get("used", {}).get("dynamic_imports") == 2,
...
def main(argv=None):
    ...
    try:
        ...
        rows, evaluator_contract, task198_meta = authenticate_task198(p0, raws)
        raws.clear()
        del raws
        meter.sample_rss("checker task198 raw released before production receipt DOM")
        context = reconstruct_context(p0, pins, rows, evaluator_contract,
                                      task198_meta, meter)
        del evaluator_contract
        receipt_path, receipt_raw, receipt = read_receipt(
        ...
        output = verdict_document(
            receipt_path, receipt_raw, terminal, accepted, p0, pins,
    ...
    try:
        ...
        target = prepare_output(args.verdict)
        unknown = emergency_unknown(receipt_path, receipt_raw, terminal, failure,
                                    meter, p0, pins)
