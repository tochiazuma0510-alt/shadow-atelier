...
SCHEMA = "d972-r07-joint-slice-kernel-general/v8"
SELF = SCHEMA + "/selftest"
FIXTURE = "search/certs/d972_r07_joint_slice_kernel_general_selftest_v8_20260828.json"
FIXTURE_SEAL = "literal-static-fixture-v8"
PASS = "R07_JOINT_SLICE_KERNEL_GENERAL_V8_CHECKER_SELFTEST_PASS"
...
def mutation_seal(value):
    body=dict(value);body.pop("mutation_fixture_seal",None);return digest(body)
...
def replay(case, receipt):
    ...
    if is_member:
        ...
        require(bool(duals) and receipt["dual"] is not None, "dual replay")
        dual = receipt["dual"]
...
def checker_mutate(case, receipt, owner):
    """Build a mutation, prove its seal, then isolate the semantic oracle."""
    ...
    result = {
        ...
        "semantic_oracle_reached": False,
        "semantic_rejection_seen": False,
        "rejected": False,
        "rejection_stage": "mutation_construction",
        "rejection_reason": None,
    }
    require(owner in OWNERS, "unknown mutation owner")
    if owner in fixture_owners:
        ...
        require(digest(mutated_case) != digest(case), "mutation canonical unchanged")
        result["canonical_changed"] = True
        mutated_case["mutation_fixture_seal"] = mutation_seal(mutated_case)
        require(type(mutated_case["mutation_fixture_seal"]) is str and mutated_case["mutation_fixture_seal"] == mutation_seal(mutated_case), "mutation seal")
        result["reseal_passed"] = True
        result["semantic_oracle"] = "independent_terminal"
        result["semantic_oracle_reached"] = True
        try:
            independent_terminal(mutated_case)
        except (RuntimeError, ValueError, KeyError, TypeError, IndexError) as exc:
            result["semantic_rejection_seen"] = True
            result["rejected"] = True
            result["rejection_stage"] = "independent_terminal"
            result["rejection_reason"] = str(exc)
            return result
        result["rejection_stage"] = "independent_terminal_returned"
        result["rejection_reason"] = "semantic oracle accepted mutation"
        return result
    if owner == "seed_index": mutated_receipt["rows"][1]["seed_index"] = 99
        ...
        mutated_receipt["left_kernel_basis"][0] = [1, 1]
        mutated_receipt["left_kernel_basis"][1] = [1, 1]
    ...
    require(digest(mutated_receipt) != digest(receipt), "mutation canonical unchanged")
    body = dict(mutated_receipt)
    body.pop("self_digest_sha256", None)
    mutated_receipt["self_digest_sha256"] = digest(body)
    require(type(mutated_receipt["self_digest_sha256"]) is str and mutated_receipt["self_digest_sha256"] == digest(body), "receipt seal")
    result["canonical_changed"] = True
    result["reseal_passed"] = True
    result["semantic_oracle"] = "replay"
    result["semantic_oracle_reached"] = True
    try:
        ...
        result["semantic_rejection_seen"] = True
        result["rejected"] = True
        result["rejection_stage"] = "replay"
        result["rejection_reason"] = str(exc)
        return result
    result["rejection_stage"] = "replay_returned"
    result["rejection_reason"] = "semantic oracle accepted mutation"
    return result

def validate_fixture(f):
    require(f.get("schema") == SELF and type(f.get("fixture_seal")) is str and f.get("fixture_seal") == FIXTURE_SEAL, "fixture seal/schema")
    require(f.get("modulus") == 3 and len(f.get("cases", [])) == 5, "five cases")
...
def fixture_preflight(value):
    """Fail closed on every literal owner binding before any case replay."""
    shapes = {
...
def run(fixture, receipt):
    f = validate_fixture(load(fixture))
    f = fixture_preflight(f)
    ...
    require(p.get("wrong_seal_rejected") is True, "producer wrong-seal canary")
    ...
    require(all(x.get("rejected") is True for x in controls), "producer mutation controls")
    receipt_case_index = {"seed_index": 0, "parent": 0, "row_theta": 0, "left_kernel": 0, "Hd1": 1, "member_ancestry": 0, "dual": 1, "terminal": 1}
    independent_controls = []
    for owner in OWNERS:
        case_index = receipt_case_index.get(owner, 1)
        verdict = checker_mutate(f["cases"][case_index], p["cases"][case_index], owner)
        require(verdict.get("owner") == owner, "independent mutation owner")
        require(verdict.get("canonical_changed") is True, "independent canonical change")
        require(verdict.get("reseal_passed") is True, "independent reseal")
        require(verdict.get("semantic_oracle_reached") is True, "independent semantic oracle")
        require(verdict.get("semantic_rejection_seen") is True, "independent semantic rejection")
        require(verdict.get("rejected") is True, "independent mutation verdict")
        independent_controls.append(verdict)
    require(len(independent_controls) == len(OWNERS), "independent mutation count")
    return {"schema": SCHEMA + "/checker-verdict/v8", "accepted": True, "independent": True, "terminal": "SELFTEST_COMPLETE", "cases": results, "mutation_attempted": len(independent_controls), "mutation_rejected": sum(x["rejected"] for x in independent_controls), "wrong_seal_rejected": seal_rejected, "producer_mutation_controls_checked": True, "independent_mutation_controls": independent_controls, "producer_imported": False}
...
def main(argv=None):
    ...
    try:
        ...
        verdict = {"schema": SCHEMA + "/checker-verdict/v8", "accepted": False, "independent": True, "status": "UNKNOWN_INPUT", "terminal": "UNKNOWN_INPUT", "reason": str(exc), "producer_imported": False}
    ...
    if verdict.get("accepted"):
        ...
        print("R07_JOINT_SLICE_KERNEL_GENERAL_V8_CHECKER_TERMINAL " + verdict["terminal"])
