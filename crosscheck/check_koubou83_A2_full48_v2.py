#!/usr/bin/env python3
"""WO-157-2: full-48 A2 checker v2 with mandatory W-1 semantics.
...
Version 2 preserves the v1 predicates and adds a fail-closed, noncommutative
B3 fixture for regulation W-1 plus a machine-generated ad_convention block.
"""
...
CHECKER = "crosscheck/check_koubou83_A2_full48_v2.py"
VERDICT = "crosscheck/verdicts/koubou83_A2_full48_crosscheck_v2_20260823.json"
...
def paper_product(*raw_factors):
    """Evaluate a paper product via W-1: paper A*B maps to raw B*A."""
    z=[]
    for factor in reversed(raw_factors): z=red(z,factor)
    return z

def braid_comparison(lhs,rhs):
    """Compare same-permutation B3 words by their exact PB3 normal form."""
    difference=red(inv(lhs),rhs)
    f2_word,c_power=pb(difference)
    return {"lhs_raw":lhs,"rhs_raw":rhs,"lhs_inverse_times_rhs_raw":difference,
            "difference_pb3_f2_word":f2_word,"difference_c_power":c_power,
            "equal":not f2_word and c_power==0}

def w1_semantic_gate():
    """Mandatory W-1 positive identity, negative canary, and noncentrality."""
    sigma2=[2]; x=[1,1]; y=[2,2]
    # Paper f1=y*x^-1.  W-1 reverses factors in the raw/GAP-order word.
    f1=paper_product(y,inv(x))
    former_raw_order_error=red(y,inv(x))

    correct_lhs=paper_product(inv(f1),sigma2,f1)
    correct_rhs=paper_product(x,sigma2,inv(x))
    correct=braid_comparison(correct_lhs,correct_rhs)

    former_lhs=paper_product(inv(former_raw_order_error),sigma2,former_raw_order_error)
    former=braid_comparison(former_lhs,correct_rhs)
    noncentral=braid_comparison(correct_rhs,sigma2)

    gate={
      "gate_id":"W-1/noncommutative-B3/nu-1/v1",
      "word_convention_id":WORD_CONVENTION_ID,
      "fixture":{"group":"B3=<sigma1,sigma2 | sigma1*sigma2*sigma1=sigma2*sigma1*sigma2>",
                 "raw_word_alphabet":{"1":"sigma1","-1":"sigma1^-1","2":"sigma2","-2":"sigma2^-1"},
                 "sigma2_raw":sigma2,"x_equals_sigma1_squared_raw":x,
                 "y_equals_sigma2_squared_raw":y,"noncommutative_required":True},
      "paper_statement":{"parameter":"nu=1","f1":"y*x^-1",
                         "identity":"f1^-1*sigma2*f1 = x*sigma2*x^-1"},
      "paper_f1_raw_gap_order":f1,
      "former_gap_raw_order_error":former_raw_order_error,
      "correct_order_evaluation":correct,
      "former_raw_order_evaluation":former,
      "noncentral_fixture_evaluation":{"paper_ad_x_sigma2_vs_sigma2":noncentral,
                                       "assertion_result":not noncentral["equal"]},
      "positive_identity_pass":correct["equal"],
      "former_error_rejected":not former["equal"],
      "noncentral_fixture_pass":not noncentral["equal"],
    }
    gate["overall_pass"]=gate["positive_identity_pass"] and gate["former_error_rejected"] and gate["noncentral_fixture_pass"]
    assert gate["overall_pass"]
    return gate

def make_ad_convention(checker_source_sha256,w1_gate):
    """Generate the convention pin; no source digest is embedded in source."""
    block={
      "paper_ad_x(u)":"x*u*x^-1",
      "gap_power_convention":"u^x=x^-1*u*x",
      "implementation_ad_label":"paper_ad_x",
      "implementation_ad_evaluator":"paper_product(x,u,x^-1)",
      "implementation_raw_label_interpretation":{
        "matches_adx":"raw x*u*x^-1 = GAP u^(x^-1) = paper Ad(x^-1)",
        "matches_adx2":"raw x^2*u*x^-2 = GAP u^(x^-2) = paper Ad(x^-2)"
      },
      "nu_to_action_class":[
        {"nu":0,"paper_action":"identity","raw_gap_action":"u","implementation_action_class":"action_is_id"},
        {"nu":1,"paper_action":"Ad(x)","raw_gap_action":"u^x = x^-1*u*x","implementation_action_class":"matches_adx2"},
        {"nu":2,"paper_action":"Ad(x)^2","raw_gap_action":"u^(x^2) = x^-2*u*x^2","implementation_action_class":"matches_adx"}
      ],
      "action_class_scope":"order-3 action on P/Phi(P); labels are those emitted by koubou83_c3lift_check_v1_1.g",
      "noncentral_fixture_assertion_result":w1_gate["noncentral_fixture_pass"],
      "noncentral_fixture_id":w1_gate["gate_id"],
      "word_convention_id":WORD_CONVENTION_ID,
      "action_convention_id":ACTION_CONVENTION_ID,
      "checker_source_sha256":checker_source_sha256,
    }
    required=("paper_ad_x(u)","gap_power_convention","implementation_ad_label",
              "nu_to_action_class","noncentral_fixture_assertion_result",
              "word_convention_id","action_convention_id","checker_source_sha256")
    assert all(k in block for k in required) and len(block["nu_to_action_class"])==3
    assert block["noncentral_fixture_assertion_result"] is True
    return block
...
def main():
    checker_source_sha256=sha(CHECKER)
    w1_gate=w1_semantic_gate()
    ad_convention=make_ad_convention(checker_source_sha256,w1_gate)
