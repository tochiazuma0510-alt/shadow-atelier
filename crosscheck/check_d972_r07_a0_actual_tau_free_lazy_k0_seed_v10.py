#!/usr/bin/env python3
"""Independent Task529 checker for the exact-schema lazy K=0 successor."""
...
MARKER = "R07_A0_ACTUAL_TAU_FREE_LAZY_K0_SEED_V10_CHECKER"
...
old = load(OLD_CHECKER, "task529_checker_v7")
...
def validate_counters(value: Any) -> dict[str, int]:
    need(isinstance(value, dict) and set(value) == set(COUNTER_KEYS) and
         all(type(value[key]) is int and value[key] >= 0 for key in COUNTER_KEYS),
         "counter shape/type")
    return value
...
def validate_new_source(source: dict[str, Any]) -> dict[str, Any]:
    ...
    need(source["record_version"] == 6 and
         source["direct_scalar"] == source["scalar"] in (1, 2),
    ...
    if source["kind"] == "action":
        ...
        need(source["K"] == 0, "selected K!=0 rejected")
        need(source["formula_scalar"] == source["scalar"], "formula scalar field")
...
def read_checkpoint(cert: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("bytes", "accepted_count", "rank"):
        exact_int(durable.get(key), "durable state integer:" + key)
    for key in ("accepted_count", "physical_rank", "round"):
        exact_int(cert.get(key), "result integer:" + key)
    validate_counters(cert.get("counters"))
    validate_counters(cert.get("attempt_counters"))
    validate_progress(cert.get("selector_progress"))
    raw = path.read_bytes()
...
def setup():
    v1 = low.load(low.V1, "task529_check_state_v1")
    v4 = v1.load(v1.V4, "task529_check_state_v4")
    m = v4.load_v1()
    v12 = m.load(m.V12, "task529_check_v12")
    p435 = m.load(m.P435, "task529_check_p435")
    p179 = m.load(m.P179, "task529_check_p179")
...
def replay_new(P: dict[str, Any], m: Any, p179: Any,
    ...
    for record in new_records:
        ...
        need(authenticated_round > authenticated_previous_round,
             "independent authenticated round chain")
        dual, remainder, _ = state
...
def check(cert: dict[str, Any]) -> None:
    ...
    exact_int(cert.get("legacy_accepted_count"), "result legacy count integer")
    need(cert.get("claims") == expected_claims and cert.get("pins") == PIN_BINDING and
         cert["legacy_accepted_count"] == LEGACY_ACCEPTED, "claims/pins/legacy count")
    state_cp = read_checkpoint(cert)
    accepted = cert["accepted_sources"]
    validate_new_round_chain(accepted[LEGACY_ACCEPTED:], cert["round"])
...
def self_test() -> dict[str, Any]:
    ...
    def mutated(path: tuple[Any, ...], value: Any) -> dict[str, Any]:
        result = json.loads(canonical(sample))
        target: Any = result
        for key in path[:-1]:
            target = target[key]
        target[path[-1]] = value
    ...
    for label, mutate in (("K!=0", {"K": 1}),
        ...
        candidate = dict(sample); candidate.update(mutate)
        try:
            validate_new_source(candidate)
        except RuntimeError:
            rejected.append(label)
    need(len(rejected) == 4, "checker mutation fixture")
    exact_integer_rejections = []
    for label, path, value in (
        ...
        try:
            replay_new({}, None, None, [{} for _ in range(LEGACY_ACCEPTED)] +
                       [mutated(path, value)], (None, None, None))
        except RuntimeError:
            exact_integer_rejections.append(label)
    need(len(exact_integer_rejections) == 6,
         "live replay exact integer mutation fixture")
    round_chain_rejections = []
    round_cases = (
        ("first_1", [mutated(("round",), 1)]),
        ("first_73", [mutated(("round",), 73)]),
        ("duplicate", [sample, mutated(("round",), 74)]),
        ("decreasing", [mutated(("round",), 75), sample]),
        ("noninteger", [mutated(("round",), 74.0)]),
    )
    for label, records in round_cases:
        try:
            replay_new({}, None, None, [{} for _ in range(LEGACY_ACCEPTED)] + records,
                       (None, None, None))
        except RuntimeError:
            round_chain_rejections.append(label)
    need(len(round_chain_rejections) == 5,
         "live replay strict round mutation fixture")
    return {"status": "PASS", "actual_imported_checker": OLD_CHECKER[2],
            "v7_self_test": base_result, "mutation_rejections": rejected,
            "exact_integer_mutation_rejections": exact_integer_rejections,
            "round_chain_mutation_rejections": round_chain_rejections,
            "mutation_path": "replay_new",
