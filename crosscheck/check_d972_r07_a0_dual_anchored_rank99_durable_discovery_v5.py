#!/usr/bin/env python3
"""Independent Task494 checker for the v424 actual-owner continuation."""
...
MARKER = "R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V5_CHECKER"
...
def compiled_formula_scalar(formula: dict[str, Any], blobs: tuple[bytes, ...]) -> int:
    """Evaluate the compiled selector ABI without importing the producer."""
...
def self_test() -> dict[str, Any]:
    ...
    # Regression for the v4 compiled/raw ABI split, kept independent from the
    # producer module so the checker remains a separate implementation.
    ...
    need(compiled_coordinate_rejected and compiled_constant_substitution_rejected and
         compiled_merged_rejected, "selftest:compiled_shape_rejections")
    ...
    class RawIdentityModel:
        ...
        def formula_scalar(self, formula: dict[str, Any], ids: Any) -> int:
            need("constant" in formula and "K" not in formula,
                 "selftest:raw_identity_lane")
            raw_calls.append(formula)
            return 0

    raw_p179 = type("RawP179", (), {"AllSevenModel": staticmethod(
        lambda runtime: RawIdentityModel())})
    raw_P = {"g760": [], "runtime": type("Runtime", (), {
             "old": object(), "e3": type("E3", (), {"identity": object()}),
             "e4": type("E4", (), {"identity": object()})})(),
             "p176": {"packed_joint_blob": lambda identity, label: b""}, "dual": {},
             "pres": {"relators": [[1]]}, "v12": type("V12", (), {
                 "v3": type("V3", (), {"exp_pair": staticmethod(lambda word: (18, 0))}),
                 "aggregate": staticmethod(lambda value: value),
                 "seed_v12": staticmethod(lambda *args: {"row": 1})})(),
             "model": object(), "owner": object(), "q": object()}
    raw_v4 = type("RawV4", (), {"b": type("RawB", (), {
        "pair": staticmethod(lambda dual, row: 0)})})()
    formula_bundle(raw_v4, raw_P, None, raw_p179, {})
    need(len(raw_calls) == 1, "selftest:raw_identity_call")

    selector_formula = {"K": 1, "merged": {(0, b"a"): 1},
                        "required_coordinates": [0]}
    selector_sf = type("SelectorSF", (), {
        "rt": object(), "canonical": staticmethod(lambda coordinate, target: object()),
        "ensure_kernel_prefix": staticmethod(lambda coordinate, count: [object()]),
        "kernel_candidate": staticmethod(lambda fibre, eta: {
            "source_word": [1], "coordinate_blobs": [b"a", b"b"]})})()
    selector_p179 = type("SelectorP179", (), {
        "coordinate_blobs": staticmethod(lambda rt, delta: (b"a", b"b"))})()
    ...
    selector_literal({}, selector_p179, selector_sf, RejectRawEvaluator(), [selector_formula],
                     selector_record, "a" * 64)
    c99 = frozen_c99()
    ...
    return {"status": "PASS", "compiled_scalar_cases": True,
