...
PRODUCER_PIN = ("search/d972_r07_history_free_positive_fast_resume_v13.py",
                147409,
...
_PROGRESS_LAST = _PROGRESS_STARTED - 60.0


def checker_progress(phase: str) -> None:
    """Rate-limited stdout telemetry; never part of receipt/verdict semantics."""
    global _PROGRESS_LAST
    now = time.monotonic()
    if now - _PROGRESS_LAST >= 60.0:
        _PROGRESS_LAST = now
        print("A0_PROGRESS side=checker phase=" + str(phase) +
              " elapsed_seconds=" + str(int(now - _PROGRESS_STARTED)), flush=True)
...
def decode_task176_owners(sources: Sources) -> dict[str, Any]:
    checker_progress("task176_decode")
    receipt = sources.json("task176_receipt")
    ...
    decoded["q0_marked_permutations"] = tuple(
        bytes(value - 1 for value in row) for row in q3_marks)
    return decoded
...
def reconstruct_task176_selected(runtime: dict[str, Any], section: dict[str, Any]) -> None:
    checker_progress("selected_task176_replay")
...
def reconstruct_k0_selected_fibre(runtime: dict[str, Any], section: dict[str, Any],
    ...
    for qid in range(1, count + 1):
        if (qid & 4095) == 1:
            checker_progress("K0_single_coordinate_stream")
...
def validate_correction_provenance(runtime: dict[str, Any],
    ...
    require(section.get("membership_bound") is True and
            ...
            section.get("gamma_full_state_factor_widths") == [40] + [154] * 31 and
...
def validate_common(runtime: dict[str, Any], receipt: dict[str, Any],
    ...
    checker_progress("COMMON_envelope")
    ...
    require(receipt.get("schema") == SCHEMA and
            ...
            receipt.get("selftest") is None and
    ...
    for selected_row in sorted(selected, key=lambda row: row["symbol"]):
        checker_progress("COMMON_selected_support")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    checker_progress("checker_start")
    ...
    if terminal == SELFTEST_TERMINAL:
        raise CheckStop("production-only researcher override forbids SELFTEST")
    elif terminal == COMMON:
        checker_progress("COMMON_sources")
        ...
        derived = validate_common(runtime, receipt, include_selftest=False,
