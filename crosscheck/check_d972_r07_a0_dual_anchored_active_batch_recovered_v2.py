#!/usr/bin/env python3
"""Independent Task451 recovered-v2 dual-anchored closed-batch replay."""
from __future__ import annotations
import argparse,dis,hashlib,importlib.util,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];SCHEMA="d972-r07-a0-dual-anchored-active-batch/v1";CP_SCHEMA=SCHEMA+"/checkpoint";MARKER="R07_A0_DUAL_ANCHORED_ACTIVE_BATCH_RECOVERED_V2_CHECKER"
V3=("search/d972_r07_a0_actual_tau_free_rank_ladder_v3.py",12215,"0140447110cfa568a77300bd7d43d51c622597704b7b91ed5209c63168c9ef37");V6=("crosscheck/check_d972_r07_a0_actual_tau_free_rank_ladder_v6.py",3590,"e902468fca7ead498e78c06496ccea596c10a1904e571f5d6b709962458b1739");FROZEN=("search/certs/d972_r07_a0_actual_tau_free_rank51_checkpoint_v1.json",10934,"a83959e4c9fcfa79093c712e82164d47c31b78c9fc00b512f7adac9413c481f4");BINDING=hashlib.sha256((SCHEMA+V3[2]+FROZEN[2]).encode()).hexdigest()
def need(x,m):
...
v3=load(V3,"task451_check_v3");v6=load(V6,"task451_check_v6");IC=v6.c.c.c;I2=IC.c;IP=v6.c;GATES=v6.c.c
def frozen():return v6.frozen_state()
def checkpoint(cert):
...
def formulas(P,p179,raw):
    model=model179(p179,P);out=[];coords=set();dual=P["dual"];n1=dual.get(b"N\x01",0);n2=dual.get(b"N\x02",0);ids=[P["p176"]["packed_joint_blob"](P["runtime"].e3.identity,"task451 checker identity") for _ in range(5)]+[P["p176"]["packed_joint_blob"](P["runtime"].e4.identity,"task451 checker identity") for _ in range(5)]
...
def replay_prefix(P,m,p179,prefix,args):
    ...
    need(len(P["phys"].order)==51,"rank51");return state,sf
    ...
    for expected,batch in enumerate(batches,1):
        ...
        for record in batch["rows"]:
            ...
            if record["kind"]=="action":
                ...
                model,compiled_formulas,adj=compiled;need(record["adjoint_digest"]==adj["adjoint_digest"],"adjoint");selector(P,m,p179,sf,model,compiled_formulas,record);exponent(P,p179,record);row=P["v12"].aggregate(P["v12"].replay_atom(record["seed_index"],record["delta_word"],P["runtime"],P["model"],P["pres"],P["owner"],P["p176"],P["q"]));src={"family":"DIRECT_CORRECTION","seed_index":record["seed_index"],"delta_word":record["delta_word"],"source_digest":record["row_digest"]}
...
def _binding_regression_gate():
    """Exercise the repaired name binding through Python bytecode metadata."""
    names=set(replay.__code__.co_varnames)
    need("formulas" not in names,"formula global shadowed")
    need(callable(globals().get("formulas")) and globals()["formulas"].__name__=="formulas","formula builder unavailable")
    need(any(i.opname=="LOAD_GLOBAL" and i.argval=="formulas" for i in dis.get_instructions(replay)),"replay does not call global formula builder")
    return {"global_formula_builder_callable":True,"replay_local_shadow_absent":True,"replay_global_formula_load":True}

def self_test():
    binding=_binding_regression_gate();anchor="a"*64;row={"kind":"correction","selector_cursor":[1,0,"00",0],"anchor_scalar":1,"pre_rank":51,"post_rank":52,"pivot":"aa","exact_exponent_pair":[72,0]};batch={"batch":1,"anchor_rank":51,"anchor_dual_digest":anchor,"anchor_remainder_digest":"b"*64,"rows":[row],"row_count":1,"post_rank":52,"post_remainder_digest":"c"*64,"post_dual_digest":"d"*64,"closed":True};rejected=[]
    def toy(x):
    ...
    need(len(rejected)==10,"mutations");return {"status":"PASS","mutation_rejections":rejected,"toy_batch_rows":1,"anchor_dual_reused":True,"frozen_prefix_semantic_replay":True,"max_rises_exact":64,"binding_regression":binding}
def main(argv=None):
