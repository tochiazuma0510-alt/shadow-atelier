#!/usr/bin/env python3
"""Task367 checker-side independent rows 1--7 authority trace.
...
SCHEMA = "d972-r07-a4-actual-owner-trace/v4"
FIXTURE_REL = "search/certs/d972_r07_a4_actual_owner_trace_authority_fixture_v4_20260829.json"
FIXTURE_BYTES = 8_457
FIXTURE_SHA = "9bf92d19c8ebcabb8cb6a342508af60689e3c689979b065a7733052b499172b3"
FIXTURE_SELF = "f7d929846f069139bcfe148d07b072849b1f92d2dd9c782aa4a387b4d3467663"
...
LARGEST_INTENDED_PEAK = BASELINE_PEAK_BYTES + 2 * (RECEIPT_BYTES + 8 + MANIFEST_BYTES) + 6 * (RECEIPT_BYTES + 8 + MANIFEST_BYTES) + 35_000_000
...
class Counter:
    ...
    def public(self) -> dict[str, Any]: return {"caps": dict(self.CAPS), "counts": dict(self.counts), "peak_live_bytes": self.peak_seen, "revalidated_bytes": self.revalidated_bytes, "one_meter": True, "logical_open_account": "logical owner opens (files and retained transaction-parent directories) plus retained-fd revalidation passes; traversal-component OS opens are not included"}
...
def canon_meter(value: Any, counter: Counter, bound: int, owner: str) -> bytes:
    ...
    counter.reserve("canonical_bytes", bound); previous = counter.reserved["canonical_bytes"]; peak_active = False; retained_active = False; complete = False
    try:
        counter.reserve_peak(bound); peak_active = True; raw = canonical(value)
        if len(raw) > bound: raise CheckerInputStop("checker:canonical:overflow")
        counter.charge("canonical_bytes", len(raw))
        counter.release_peak(bound); peak_active = False; counter.retain_peak(owner, len(raw)); retained_active = True
        ...
        if peak_active: counter.release_peak(bound)
        if retained_active and not complete: counter.release_retained(owner)
def digest_bytes(raw: bytes | bytearray) -> str:
    h = hashlib.sha256(); h.update(memoryview(raw)); return h.hexdigest()
def digest_object(value: Any, counter: Counter, bound: int) -> str:
    token = "transient:digest:" + str(id(value)); raw = canon_meter(value, counter, bound, token)
    try: return digest_bytes(raw)
    finally: del raw; counter.release_retained(token)
...
class AuthenticatedOwner:
    ...
    def revalidate_all(self, expected: dict[str, dict[str, Any]]) -> dict[str, Any]:
        self.counter.reserve("opens", 1); self.counter.charge("opens", 1)
        answer: dict[str, Any] = {}
        for key, fd in self.handles.items():
            ...
            self.counter.revalidated_bytes += read
...
def parse_object(raw: bytes | bytearray, label: str, counter: Counter, exact: bool, owner: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if exact:
            token = "transient:parse:" + label; canonical_raw = canon_meter(value, counter, size, token)
            try:
                if canonical_raw != raw: raise CheckerInputStop("checker:json:noncanonical:" + label)
            finally: del canonical_raw; counter.release_retained(token)
        return value
...
def stream_rows(rows: list[Any], chunks: list[Any], counter: Counter) -> tuple[str, bool]:
    ...
    for pos, row in enumerate(rows, 1):
        typed_row(row, pos); token = "transient:row:" + str(pos); piece = canon_meter(row, counter, 1_000_000, token)
        try:
            if pos != 1: whole.update(b",")
            whole.update(piece)
            if n != 0: ch.update(b",")
            ch.update(piece); n += 1
            if row.get("word") == []: has_empty_word = True
        finally:
            del piece; counter.release_retained(token)
...
def load_fixture(owner: AuthenticatedOwner, counter: Counter, argument: str) -> dict[str, Any]:
    ...
    if type(seal) is not str or seal != FIXTURE_SELF or seal != digest_object(body, counter, 1_000_000) or fixture.get("schema") != SCHEMA + "/authority-fixture/v4": raise CheckerInputStop("checker:fixture:self_seal")
...
def seal_receipt(value: dict[str, Any], counter: Counter, owner: str) -> SealedReceipt:
    ...
    body.pop("self_digest_sha256", None); body_raw = canon_meter(body, counter, 35_000_000, owner + ":body"); seal = digest_bytes(body_raw); del body_raw; counter.release_retained(owner + ":body"); dom = dict(body); dom["self_digest_sha256"] = seal; raw = canon_meter(dom, counter, 35_000_000, owner); return SealedReceipt(dom, raw, digest_bytes(raw), len(raw), seal)
...
def write_case(path: Path, raw: bytes | bytearray, workspace: Path, counter: Counter, expected_sha256: str | None = None) -> None:
    ...
    try:
        ...
        while pos < size:
            ...
            if source_hash is not None: source_hash.update(chunk[:written])
            pos += written
        ...
        while remain:
            ...
            actual.update(part); remain -= len(part)
...
def execute(fixture: dict[str, Any], counter: Counter, owner: AuthenticatedOwner) -> dict[str, Any]:
    ...
    expected_counts = {"opened_bytes": INTENDED_OPENED_BYTES, "temporary_bytes": INTENDED_TEMPORARY_BYTES, "dom_bytes": INTENDED_DOM_BYTES, "opens": METERED_LOGICAL_OPENS, "writes": INTENDED_WRITES, "events": INTENDED_EVENTS, "mutations": len(MUTATIONS)}
    if any(counter.counts[key] != value for key, value in expected_counts.items()) or counter.revalidated_bytes != INTENDED_REVALIDATED_BYTES or any(counter.reserved.values()) or counter.live_peak != BASELINE_PEAK_BYTES or counter.peak_seen != LARGEST_INTENDED_PEAK: raise CheckerInputStop("checker:meter:final_account")
...
def output_seal(value: dict[str, Any], counter: Counter, token: str) -> tuple[bytes, str]:
    body = dict(value); body.pop("self_digest_sha256", None); body_raw = canon_meter(body, counter, 35_000_000, token + ":body"); seal = digest_bytes(body_raw); del body_raw; counter.release_retained(token + ":body"); body["self_digest_sha256"] = seal; raw = canon_meter(body, counter, 35_000_000, token); return raw, digest_bytes(raw)
def write_output(path: Path, value: dict[str, Any], counter: Counter) -> None:
    ...
    try:
        parent_fd, leaf = parent_nofollow(target); counter.charge("opens"); parent_open_reserved = False
    except Exception:
        if parent_open_reserved: counter.release("opens", 1)
        raise
    stage = None; stage_created = None; stage_fd = -1; data_fd = -1; published = False; target_was_absent = False; raw: bytes | None = None; view: memoryview | None = None; token = "output:final_raw"
    try:
        before = os.fstat(parent_fd); registered_before = lstat_nofollow(target.parent); directory_key = lambda st: (st.st_dev, st.st_ino, st.st_mode, st.st_nlink)
        if not stat.S_ISDIR(before.st_mode) or directory_key(before) != directory_key(registered_before): raise CheckerInputStop("checker:output:parent_identity")
        ...
        for _ in range(64):
            candidate = next(tempfile._get_candidate_names())
            try: os.mkdir(candidate, 0o700, dir_fd=parent_fd); stage = candidate; stage_created = candidate; break
            except FileExistsError: continue
        ...
        try:
            ...
            raise
        counter.reserve("opens", 1); data_open_reserved = True
        try:
            data_fd = os.open("staged.json", os.O_RDWR | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o600, dir_fd=stage_fd); counter.charge("opens"); data_open_reserved = False
        except Exception:
            if data_open_reserved: counter.release("opens", 1)
            raise
        view = memoryview(raw); pos = 0
        while pos < len(raw):
            count = os.write(data_fd, view[pos:pos + 1_048_576])
            if count <= 0: raise CheckerInputStop("checker:output:short_write")
            pos += count
        os.fsync(data_fd); staged = os.fstat(data_fd)
        if not stat.S_ISREG(staged.st_mode) or staged.st_nlink != 1 or staged.st_size != len(raw): raise CheckerInputStop("checker:output:stage_identity")
        if os.lseek(data_fd, 0, os.SEEK_SET) != 0: raise CheckerInputStop("checker:output:rewind")
        staged_hash = hashlib.sha256(); remain = len(raw)
        while remain:
            part = os.read(data_fd, min(1_048_576, remain))
            if not part: raise CheckerInputStop("checker:output:short_read")
            staged_hash.update(part); remain -= len(part)
        if staged_hash.hexdigest() != sha or not same_stats(staged, os.fstat(data_fd)): raise CheckerInputStop("checker:output:stage_sha256")
        ...
        os.fsync(stage_fd); opened = os.fstat(data_fd); pathname = os.stat(leaf, dir_fd=parent_fd, follow_symlinks=False)
        if not stat.S_ISREG(opened.st_mode) or opened.st_nlink != 1 or opened.st_size != len(raw) or not same_stats(opened, pathname): raise CheckerInputStop("checker:output:final_identity")
        os.close(data_fd); data_fd = -1; os.close(stage_fd); stage_fd = -1; os.rmdir(stage, dir_fd=parent_fd)
        try: os.stat(stage, dir_fd=parent_fd, follow_symlinks=False); raise CheckerInputStop("checker:output:stage_present")
        except FileNotFoundError: pass
        stage = None; os.fsync(parent_fd)
        after = os.fstat(parent_fd); registered_after = lstat_nofollow(target.parent)
        if directory_key(before) != directory_key(after) or directory_key(after) != directory_key(registered_after): raise CheckerInputStop("checker:output:parent_identity")
    except Exception as exc:
        cleanup_errors: list[BaseException] = []
        if published:
            try:
                os.unlink(leaf, dir_fd=parent_fd); published = False
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if stage_fd >= 0:
            try: os.unlink("staged.json", dir_fd=stage_fd)
            except FileNotFoundError: pass
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
            try: os.stat("staged.json", dir_fd=stage_fd, follow_symlinks=False); cleanup_errors.append(CheckerInputStop("checker:output:staging_link_present"))
            except FileNotFoundError: pass
            try: os.fsync(stage_fd)
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if data_fd >= 0:
            try: os.close(data_fd); data_fd = -1
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if stage_fd >= 0:
            try: os.close(stage_fd); stage_fd = -1
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if stage is not None:
            try: os.rmdir(stage, dir_fd=parent_fd); stage = None
            except FileNotFoundError: stage = None
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        try: os.fsync(parent_fd)
        except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if target_was_absent and not published:
            try: os.stat(leaf, dir_fd=parent_fd, follow_symlinks=False); cleanup_errors.append(CheckerInputStop("checker:output:rollback_present"))
            except FileNotFoundError: pass
        if stage_created is not None:
            try: os.stat(stage_created, dir_fd=parent_fd, follow_symlinks=False); cleanup_errors.append(CheckerInputStop("checker:output:stage_present"))
            except FileNotFoundError: pass
        try:
            registered_now = lstat_nofollow(target.parent)
            if directory_key(os.fstat(parent_fd)) != directory_key(registered_now): cleanup_errors.append(CheckerInputStop("checker:output:parent_substitution"))
        except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if cleanup_errors: raise CheckerInputStop("checker:output:rollback_failed") from cleanup_errors[0]
        if isinstance(exc, CheckerInputStop): raise
        raise CheckerInputStop("checker:output:atomic_failure") from exc
    finally:
        if view is not None: view.release(); del view
        if raw is not None: del raw
        counter.release_prefix(token)
        if data_fd >= 0:
            try: os.close(data_fd)
            except OSError: pass
        if stage_fd >= 0:
