...
SCHEMA = "d972-r07-history-free-positive-fast-resume/v9"
CHECKPOINT_SCHEMA = SCHEMA + "/checkpoint"
VERDICT_SCHEMA = SCHEMA + "/verdict"
COMMON = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V9_COMMON_WORD"
CHECKER_PREFIX = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V9_CHECKER_TERMINAL"
SELFTEST_TERMINAL = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V9_SELFTEST_PASS"
...
FIXTURE_PATH = "search/certs/d972_r07_history_free_positive_fast_resume_selftest_v9_20260829.json"
...
PRODUCER_PIN = ("search/d972_r07_history_free_positive_fast_resume_v9.py",
                144091,
                "7dd977b009dc3b3b2946efe569d5eb78eaf46490f875af7117471d25e69e5199")
...
TASK176_RECEIPT_DIGEST = "f8f0ce249ff547d3e1235bd4b9760daa2b34f23771bf7da47b48dbd5cbbfae1d"
TASK176_RECOVERY_DIGEST = "f8c6c0faf2588cd58d8a2aec75a2a1f9950ea67769dd913fbd796d018098f581"
...
def validate_task176_authority(sources: Sources) -> dict[str, Any]:
    ...
    recovery = sources.json("task176_recovery")
    ...
    require(recovery.get("schema") ==
            ...
            recovery.get("self_digest_sha256") == TASK176_RECOVERY_DIGEST and
...
def task176_parent_walk(parents: bytes, letters: bytes, identifier: int,
    ...
    while current:
        ...
        require(0 <= parent < current and letter in (0, 1, 2) and
                ((parent == 0 and letter == 0) or parent != 0),
...
def task176_gamma_walk(parents: bytes, records: bytes, words: list[list[int]],
    ...
    while current:
        ...
        require(parent < current and record < len(words),
                "task176 Gamma chronological parent")
        chunks.append([int(letter) for letter in words[record]])
        current = parent
    answer: list[int] = []
...
def reconstruct_task176_selected(runtime: dict[str, Any], section: dict[str, Any]) -> None:
    ...
    require(1 <= gid <= 243 and len(qrecord) == 36 and len(grecord) == 970 and
            len(gparents) == 2 and grecord_parent <= gid and
            type(owners["gamma_words"][grecord_parent]) is list,
            "task176 selected record bounds")
    # The literal byte owners are the authority for the selected state.  The
    # final group replay is deliberately kept as a separate v9 checker
    # operation; producer Q0/Gamma hex is never used as an owner.
    require(section.get("q0_state_hex") == qrecord.hex() and
            type(section.get("gamma_projected_record_hex")) is str and
            section.get("gamma_projected_record_hex") == grecord.hex(),
            "task176 selected literal state binding")
...
def validate_monitor(value: Any) -> None:
    ...
    limits = value["limits"]; counters = value.get("fresh_v9_counters")
...
def validate_checkpoint_transport(receipt: dict[str, Any], receipt_path: Path) -> dict[str, Any]:
    ...
    prior_counters = checkpoint["monitor"]["fresh_v9_counters"]
    final_counters = receipt_monitor["fresh_v9_counters"]
