...
P0_PATH = "ci/in/d972_r07_pre_a0_single_target_a3_v3.prereg.v1.json"
P0_BYTES = 16417
P0_SHA256 = "2660c8e1dce475d19f4d8a40f43626df401d3ca299f34b0f1dd067db896d2ce6"
P0_SELF_SHA256 = "0539e586fd6001ff65965e990461f390fc8868bb6fbbd0ec2a9cd4464f91df0e"
SCHEMA = "d972-r07-pre-a0-single-target-a3/v3"
...
def collect_authority_pins(p0):
    ...
    require(P0_PATH not in paths and
            not any(path.endswith("single_target_a3_v3.py") or
                    path.endswith("single_target_a3_gha_driver_v3.g")
                    for path in paths),
...
def run_raw_authority_mutation(p0, raw_by_path, meter):
    """Mutate the independently read manifest and re-enter full validation."""
    ...
    before = digest_obj({"owner": manifest_owner,
                         "raw_sha256": digest_bytes(raw_by_path[path])})
    ...
    mutant_p0 = copy.deepcopy(p0)
    mutant_p0["authority"]["task198"]["acceptance_manifest"] \
        ["manifest_self_digest_sha256"] = \
        manifest["manifest_self_digest_sha256"]
    mutant_raws = dict(raw_by_path)
    mutant_raws[path] = mutant_raw
    after = digest_obj({
        "owner": mutant_p0["authority"]["task198"]["acceptance_manifest"],
        "raw_sha256": digest_bytes(mutant_raw)})
    require(before != after, "mutation owner unchanged:" + name)
    meter.bump("mutation_work", 1,
               "checker ordinary authority mutation:" + name)
    observed = None
    try:
        authenticate_task198(mutant_p0, mutant_raws)
    ...
    return {"name": name, "changed_field": "task198_manifest_raw_owner",
            ...
            "first_gate": observed, "ordinary_validator":
                "authenticate_task198", "rejected": True}
...
class EvaluatorBudget:
    ...
    def check(self, phase):
        self.meter.check_wall("checker task198 evaluator:" + phase)
...
def mutation_fixture(rows, g760, interface, consumer, central, areas,
    ...
    return {"ledger": copy.deepcopy(rows), "g760": list(g760),
            "base": copy.deepcopy(base_ref), "central": copy.deepcopy(central),
            "areas": copy.deepcopy(areas),
            "target_constructor_owner": copy.deepcopy(target_constructor_owner),
...
def run_mutations(rows, p0, g760, interface, consumer,
    ...
    for name in roster[1:]:
        ...
        if name == "task198_ledger_sign":
            mutant["ledger"][0]["factor_sign"] *= -1; owner = "ledger"
            route = lambda: validate_ledger_owner(mutant["ledger"])
        elif name == "task198_prefix":
            mutant["ledger"][0]["fox_prefix_occurrences"] = [2, 3]; owner = "ledger"
            route = lambda: validate_ledger_owner(mutant["ledger"])
        elif name == "g760_letter_digest":
            mutant["g760"][0] *= -1; owner = "g760"
            route = lambda: validate_g760_owner(mutant["g760"])
        elif name == "computational_base_mode":
            mutant["base"]["mode"] = "MUTATED_FORBIDDEN_MODE"; owner = "base"
            route = lambda: validate_base_owner(mutant["base"])
        elif name == "forbidden_task192_binding":
            mutant["base"]["task192_consumed"] = True; owner = "base"
            route = lambda: validate_base_owner(mutant["base"])
        elif name == "H1_central_row":
            mutant_p0 = copy.deepcopy(p0)
            mutant_p0["central_rows"]["H1"]["rows"][0][0] = 3
            owner = "central"; mutant[owner] = mutant_p0["central_rows"]
            route = lambda: central_replay(rows, mutant_p0)
        elif name == "H2_central_row":
            mutant_p0 = copy.deepcopy(p0)
            mutant_p0["central_rows"]["H2"]["rows"][0][0] = 6
            owner = "central"; mutant[owner] = mutant_p0["central_rows"]
            route = lambda: central_replay(rows, mutant_p0)
        elif name == "P_central_row":
            mutant_p0 = copy.deepcopy(p0)
            mutant_p0["central_rows"]["P"]["rows"][0][3] = 3
            owner = "central"; mutant[owner] = mutant_p0["central_rows"]
            route = lambda: central_replay(rows, mutant_p0)
        elif name == "projected_area_target":
            mutant["target_constructor_owner"]["bar_epsilon_1"]["H1"] = []
            owner = "target_constructor_owner"
            route = lambda: target_from_fox(
                g760, rows, mutant["target_constructor_owner"])
        elif name == "ABI_seal_target":
            mutant["consumer"]["bar_epsilon_1"]["H1"] = []
            seal_fresh(mutant["consumer"], "checker mutated consumer ABI")
            owner = "consumer"
            route = lambda: validate_projection(
                mutant["interface"], mutant["consumer"], p0)
        elif name == "forbidden_conclusion_flag":
            mutant["flags"]["fake"] = True; owner = "flags"
            route = lambda: validate_false_flags(mutant["flags"])
        else:
            raise InputStop("unregistered mutation:" + name)
        before_value = (p0["central_rows"] if name in
                        ("H1_central_row", "H2_central_row", "P_central_row")
                        else reference[owner])
        before, after = digest_obj(before_value), digest_obj(mutant[owner])
        require(before != after, "mutation owner unchanged:" + name)
...
def load_engine(rel, size, sha, tag_name, meter):
    ...
    module_name = "_d363_checker_" + tag_name
...
def main(argv=None):
    ...
    try:
        ...
        with WallDeadline(meter,
                          "checker complete authority physical authentication"):
            pins, raws = authenticate_authority(p0, meter)
        with WallDeadline(meter, "checker task198 complete authority validation"):
        ...
        with WallDeadline(meter, "checker task198 ordinary authority mutation"):
        ...
        with WallDeadline(meter, "verdict sealing and publication"):
