#!/usr/bin/env python3
"""Independent exact adapter-v4 pin successor of task193 checker v2."""
...
_BASE = Path(__file__).resolve().with_name(
    "check_d972_r07_second_frattini_affine_prefix_compiler_v2.py")
_BASE_BYTES = 32933
_BASE_SHA256 = "fdb80b02030a82d4d6cfccc6246c915c14b896e91e126b2b7ba5c22a6dc99ea5"
...
def _swap(source: bytes, old: bytes, new: bytes, count: int, label: str) -> bytes:
    if not old or not new or old == new or source.count(old) != count:
        raise SystemExit("task193 v3 checker " + label + " source cardinality")
    result = source.replace(old, new)
    if result.count(new) != count:
        raise SystemExit("task193 v3 checker " + label + " result cardinality")
    return result
...
if len(_patched) != _BASE_BYTES or hashlib.sha256(
    ...
    raise SystemExit("task193 v3 checker frozen v2 owner drift")
