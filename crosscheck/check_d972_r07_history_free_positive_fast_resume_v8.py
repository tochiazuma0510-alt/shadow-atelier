#!/usr/bin/env python3
"""Helper-nonshared checker for the R07 v8 positive resume.
...
CHECKER_PREFIX = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V8_CHECKER_TERMINAL"
...
FIXTURE_PATH = "search/certs/d972_r07_history_free_positive_fast_resume_selftest_v8_20260829.json"
...
SOURCE_PINS: dict[str, tuple[str, int, str]] = {
    ...
    "fixture": (FIXTURE_PATH, 2784,
                "a96d7e400b5f71a03975b9d223b98fe6cc6c22ef8e17fe59f0eac07f4bc7e641"),
}

# Filled from the frozen producer after its last edit.
PRODUCER_PIN = ("search/d972_r07_history_free_positive_fast_resume_v8.py",
                140467,
                "8499a50d5fc05a5d850de0d4a1ea80d3ddbc0a37c2cfd8b8a28cb3c6f95d5a0e")
...
def validate_correction_provenance(runtime: dict[str, Any],
    ...
    require(section.get("membership_bound") is True and
            ...
            type(section.get("gamma_state_hex")) is str and
            section.get("q0_state_sha256") == sha_bytes(
                bytes.fromhex(section["q0_state_hex"])) and
            section.get("gamma_state_sha256") == sha_bytes(
                bytes.fromhex(section["gamma_state_hex"])),
...
def validate_monitor(value: Any) -> None:
    ...
    limits = value["limits"]; counters = value.get("fresh_v8_counters")
...
def validate_checkpoint_transport(receipt: dict[str, Any], receipt_path: Path) -> dict[str, Any]:
    ...
    checkpoint_name = str(reference.get("path")) if type(reference) is dict else ""
    allowed_checkpoint_name = (checkpoint_name ==
        receipt_path.name + ".checkpoint.json" or
        checkpoint_name.startswith(receipt_path.name + ".checkpoint."))
    ...
    prior_counters = checkpoint["monitor"]["fresh_v8_counters"]
    final_counters = receipt_monitor["fresh_v8_counters"]
