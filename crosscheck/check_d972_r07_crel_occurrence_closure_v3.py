#!/usr/bin/env python3
"""Independent checker for the task395 raw eleven-occurrence closure (v3)."""
...
CHECK_SCHEMA = SCHEMA + "/crosscheck/v3"
...
PREFIX = "CREL_OCCURRENCE_CLOSURE_V3_CHECKER_TERMINAL"
...
TASK382_CHECKER_OWNER: dict[str, Any] | None = None
TASK382_IDENTITY_OWNER: dict[str, Any] | None = None
...
def load_identity_owner(raw: bytes) -> dict[str, Any]:
    """Load only the pinned producer namespace needed for frozen identities."""
    ns: dict[str, Any] = {"__name__": "task382_frozen_producer_identity", "__file__": str(ROOT / TASK382_PRODUCER_PIN[0]), "__package__": None}
    exec(compile(raw, "task382_frozen_producer_identity", "exec"), ns, ns); return ns
...
def restore_checker() -> tuple[Any, Any, Any, Any]:
    producer_raw = read_bytes(ROOT / TASK382_PRODUCER_PIN[0], TASK382_PRODUCER_PIN, "task382.producer")
    # The checker module has no frozen_identities API.  Load the already
    # pinned producer source in a separate, minimal identity namespace and
    # gate its callable/receipt binding before physical restoration.
    identity_owner = load_identity_owner(producer_raw)
    need(ACTIVE_RECEIPT is not None, "task382:identity_receipt_unavailable")
    need(callable(identity_owner.get("frozen_identities")), "task382:producer_identity_callable")
    need(ACTIVE_RECEIPT.get("frozen_owner_identities") == identity_owner["frozen_identities"](), "task382:producer_frozen_owner_identities")
    ...
    global CHECKER_LAYOUT, CHECKER_OWNER, TASK382_CHECKER_OWNER, TASK382_IDENTITY_OWNER
    # The checker restore namespace supplies layout only; its module namespace
    # remains available for checker arithmetic/authority, while the producer
    # namespace is used solely for frozen_identities.
    CHECKER_LAYOUT = frozen.get("CHECKER_BRIDGE_OWNER_LAYOUT")
    CHECKER_OWNER = frozen
    TASK382_CHECKER_OWNER = owner
    TASK382_IDENTITY_OWNER = identity_owner
...
def occurrence_roster(authority: Any, frozen_owner: Any = None, task382_owner: Any = None) -> list[dict[str, Any]]:
    ...
    effective_checker = task382_owner if task382_owner is not None else TASK382_CHECKER_OWNER
    effective_identity = TASK382_IDENTITY_OWNER
    need(isinstance(effective_frozen, dict), "task382:frozen_owner_namespace")
    need(isinstance(effective_checker, dict), "task382:checker_owner_namespace")
    need(isinstance(effective_identity, dict) and callable(effective_identity.get("frozen_identities")), "task382:frozen_identity_owner")
    if ACTIVE_RECEIPT is not None:
        need(ACTIVE_RECEIPT.get("frozen_owner_identities") == effective_identity["frozen_identities"](), "task382:frozen_owner_identities")
