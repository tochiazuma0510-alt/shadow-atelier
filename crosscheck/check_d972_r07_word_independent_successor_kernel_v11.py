#!/usr/bin/env python3
"""Independent bridge-index v11 owner for the frozen v6 A4 checker."""
...
def main() -> None:
    ...
    if len(raw) != SOURCE_BYTES or hashlib.sha256(raw).hexdigest() != SOURCE_SHA256:
        raise SystemExit("v11 checker: frozen v6 source drift")
    for old, new in PATCHES:
        if raw.count(old) != 1:
            raise SystemExit("v11 checker: audited repair site is not unique")
