#!/usr/bin/env python3
"""Independent checker for the dedicated A0/task193 adapter v3.

No producer module is imported.  The pinned A0 checker is loaded and its full
COMMON_WORD replay is repeated before any adapter field is accepted.
"""
...
CHECKER_SCHEMA = SCHEMA + "/checker-verdict/v3"
ATTEST_SCHEMA = SCHEMA + "/attestation"
...
def load_a0_checker() -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        "d972_r07_a0_v18_independent_for_adapter_v3", path)
...
def legacy_sparse_digest(row: list[list[Any]]) -> str:
    ...
    for item in row:
        ...
        try:
            ...
            raise CheckStop("sparse row hex") from exc
...
def replay_a0(receipt: dict[str, Any], verdict: dict[str, Any],
    ...
    need(receipt.get("schema") == A0_SCHEMA and
         ...
         receipt.get("claims") == A0_CLAIMS and receipt.get("selftest") is None and
         "checkpoint" not in receipt and receipt.get("claim_boundary") ==
    ...
    need(type(physical) is dict and physical.get("bytes") == receipt_id["bytes"] and
         physical.get("sha256") == receipt_id["sha256"], "A0 receipt binding")
    ...
    # Required full positive A0 replay; this is deliberately not SELFTEST.
    ...
    c, f, g = receipt.get("correction_word"), receipt.get("corrected_word"), receipt.get("g760")
...
def check_adapter(adapter: dict[str, Any], adapter_id: dict[str, Any],
                  receipt_id: dict[str, Any], verdict_id: dict[str, Any],
                  c: list[int], f: list[int], g: list[int],
                  row: list[list[Any]], replay: dict[str, Any],
                  derived: dict[str, Any], roster: dict[str, Any]) -> None:
    check_seal(adapter, "adapter")
    need(adapter.get("schema") == SCHEMA and
         adapter.get("status") == "ACCEPTED" and
         adapter.get("terminal") == ACCEPTED, "adapter accepted envelope")
    need(adapter.get("claims") == {"adapter": "A0_REPLAY_ONLY", "lift": "NONE",
                                   "fake": "NONE", "Ihara": "NONE"},
         "adapter claims")
    need(adapter.get("a0_receipt") == short(receipt_id) and
         adapter.get("a0_verdict") == short(verdict_id), "adapter A0 identities")
    need(adapter.get("source_roster") == roster and
         adapter.get("a0_derived") == derived and adapter.get("c_exact") == c and
         adapter.get("corrected_word") == f and adapter.get("g760") == g,
         "adapter canonical A0 fields")
    direct = adapter.get("direct_replay")
    need(type(direct) is dict and direct.get("row") == row and
         direct.get("row_sha256") == legacy_sparse_digest(row) and
         direct.get("replay") == replay, "adapter sparse replay")
    need(direct.get("direct_all_seven_replay") is True and
         ...
         "adapter four direct-replay flags")
    provenance = adapter.get("source_provenance")
    need(type(provenance) is dict and provenance.get("a0_producer") == {
         "path": A0_PRODUCER_PIN[0], "bytes": A0_PRODUCER_PIN[1],
         "sha256": A0_PRODUCER_PIN[2]} and provenance.get("a0_checker") == {
         "path": A0_CHECKER_PIN[0], "bytes": A0_CHECKER_PIN[1],
         "sha256": A0_CHECKER_PIN[2]} and provenance.get("adapter") == {
         "path": ADAPTER_PRODUCER_PIN[0], "bytes": ADAPTER_PRODUCER_PIN[1],
         "sha256": ADAPTER_PRODUCER_PIN[2]}, "adapter source pins")


def check_unknown(adapter: dict[str, Any], attestation: dict[str, Any]) -> None:
    check_seal(adapter, "adapter")
    terminal = str(adapter.get("terminal", ""))
    need(adapter.get("schema") == SCHEMA and adapter.get("status") == "UNKNOWN" and
         terminal.startswith(UNKNOWN + ":") and
         adapter.get("claims") == {"adapter": "NONE", "lift": "NONE",
                                    "fake": "NONE", "Ihara": "NONE"},
         "typed adapter UNKNOWN")
    need(attestation.get("schema") == ATTEST_SCHEMA and
         attestation.get("terminal") == terminal and
         attestation.get("line") ==
         "R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V3_TERMINAL " + terminal,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        attestation, _at, _attestation_id = read_physical(args.attestation, 4096)
        if adapter.get("status") == "UNKNOWN":
            check_unknown(adapter, attestation)
            result = seal({"schema": CHECKER_SCHEMA, "status": "UNKNOWN",
                           "terminal": adapter["terminal"],
                           "adapter_receipt": adapter_id,
                           "adapter_source": {"producer": {
                               "path": ADAPTER_PRODUCER_PIN[0],
                               "bytes": ADAPTER_PRODUCER_PIN[1],
                               "sha256": ADAPTER_PRODUCER_PIN[2]},
                               "checker": own_identity()},
                           "claims": {"independent_a0_replay": False,
                                      "accepted_abi": False}})
        else:
            ...
            check_adapter(adapter, adapter_id, receipt_id, verdict_id, c, f, g,
                          row, replay, derived, roster)
            need(adapter_id["path"] == ADAPTER_PRODUCER_PIN[0] or
                 adapter.get("source_provenance", {}).get("adapter") ==
                 {"path": ADAPTER_PRODUCER_PIN[0], "bytes": ADAPTER_PRODUCER_PIN[1],
                  "sha256": ADAPTER_PRODUCER_PIN[2]}, "adapter producer identity")
            line = "R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V3_TERMINAL " + ACCEPTED
            need(attestation.get("schema") == ATTEST_SCHEMA and
                 attestation.get("terminal") == ACCEPTED and
                 attestation.get("line") == line, "accepted adapter attestation")
            result = seal({
                "schema": CHECKER_SCHEMA, "status": "ACCEPTED",
                "terminal": ACCEPTED, "adapter_receipt": adapter_id,
                "a0_receipt": short(receipt_id), "a0_verdict": short(verdict_id),
                "replay": {"c_exact": c, "corrected_word": f, "g760": g,
                           "row_sha256": legacy_sparse_digest(row),
                ...
                "adapter_source": {"producer": {
                    ...
                    "sha256": ADAPTER_PRODUCER_PIN[2]},
                    "checker": own_identity()},
                "claims": {"independent_a0_replay": True,
                           "accepted_abi": True, "task193_values": False},
            })
        write_exclusive(args.output, result)
        print("R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V3_CHECKER terminal=" +
              result["terminal"], flush=True)
        return 0
    except (CheckStop, OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
        print("R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V3_CHECKER_ERROR " +
              str(exc), flush=True)
        return 1
