...
PRODUCER_CODE_PATH = "search/d972_r07_word_independent_successor_kernel_v5.py"
...
def atomic_write(path: Path, raw: bytes) -> None:
    path.parent.mkdir(parents=True, exist_ok=True); fd, temporary = tempfile.mkstemp(prefix=".v5-checker-", dir=str(path.parent))
    try:
...
def selftest_certificate(fixture: dict[str, Any], authority: Authority, normal: dict[str, Any]) -> dict[str, Any]:
    require(fixture.get("schema") == SCHEMA + "/selftest-fixture/v5" and fixture.get("synthetic") is False and
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.selftest:
            ...
            if pstatus in (UNKNOWN_INPUT, UNKNOWN_RESOURCE, HARD_STOP) or \
                ...
                if producer.get("authority") is None:
                    ...
                    print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_TERMINAL " + status,
                          flush=True)
                    return 0
        ...
        if args.selftest:
            ...
            print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_SELFTEST_PASS", flush=True); return 0
        ...
        if pstatus in (UNKNOWN_INPUT, UNKNOWN_RESOURCE, HARD_STOP) or producer.get("terminal") in (UNKNOWN_INPUT, UNKNOWN_RESOURCE, HARD_STOP):
            ...
            print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_TERMINAL " + status, flush=True); return 0
        ...
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_TERMINAL " + PASS, flush=True); return 0
        ...
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_TERMINAL " + UNKNOWN_RESOURCE, flush=True); return 0
        ...
        if output: write_sealed(output, terminal_certificate(UNKNOWN_INPUT, str(exc), meter, checkpoint_arg,
                                                             authority_identity=authority_identity), meter,
                                terminal_transport=True)
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_TERMINAL " + UNKNOWN_INPUT, flush=True); return 0
    except Exception as exc:
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_STOP " +
              type(exc).__name__ + ":" + str(exc), file=sys.stderr, flush=True)
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V5_CHECKER_STOP " + type(exc).__name__, flush=True); return 2
