...
# imports it; the digest binds every peer receipt to the accepted producer v4.
PRODUCER_V4_SOURCE = ROOT / "search/d972_r07_grade2_p1_componentwise_semantic_replay_v4.py"
PRODUCER_V4_SHA = "ff50d0ad50e080a15075bb52365987d9e389bf59e5e39666002b710947287a17"
...
def read_sealed(root: Path, stem: str, digest: str,
    ...
    expected_head = {"body_sha256": digest, "parent_sha256": parent,
                     "schema": STATE_SCHEMA + ".head", "stem": stem}
...
def validate_producer_source(path: Path, expected: str) -> str:
    require(path.is_file(), "producer_v4_source_missing")
    digest = sha(path.read_bytes())
    require(digest == expected, "producer_v4_source_digest")
    return digest
...
def producer_source_digest() -> str:
    ...
    return validate_producer_source(PRODUCER_V4_SOURCE, PRODUCER_V4_SHA)
...
def selftest() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        altered_source.write_bytes(PRODUCER_V4_SOURCE.read_bytes() + b"\n")
        table["producer_source_bytes"] = "REJECT" if expect_reject(
            lambda: validate_producer_source(altered_source, PRODUCER_V4_SHA)) else "ACCEPT"
        table["producer_source_sha"] = "REJECT" if expect_reject(
            lambda: validate_producer_source(PRODUCER_V4_SOURCE, "0" * 64)) else "ACCEPT"
