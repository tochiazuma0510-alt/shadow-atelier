...
def collect_authority_pins(p0):
    ...
    require(P0_PATH not in paths and
            not any(path.endswith("single_target_a3_v4.py") or
                    path.endswith("single_target_a3_gha_driver_v4.g")
                    for path in paths),
...
def main(argv=None):
    ...
    try:
        ...
        with WallDeadline(meter, "complete v4 checker route"):
    ...
    try:
        ...
        with WallDeadline(meter, "complete v4 checker emergency publication"):
            target = prepare_output(args.verdict)
            unknown = emergency_unknown(
                receipt_path, receipt_raw, terminal, failure, meter, p0, pins)
