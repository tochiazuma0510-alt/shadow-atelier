#!/usr/bin/env python3
"""Guarded MEMBER-only checker successor with an independent compact view."""
from __future__ import annotations
import ast
import hashlib
from pathlib import Path

BASE = Path(__file__).with_name("check_d972_r07_zero_base_a5_a6_compiler_v6.py")
BASE_BYTES = 2334
BASE_SHA256 = "a4db1b2b1ad5da1135c8ebcef1898c46fd07df7ebdbfa8778bd36a6098507bc3"
V5_BYTES = 2698
V5_SHA256 = "4dcd1b0540ffce929702bbd4ca6bebce9a53cd9ffb0c2dd4fa902df046897019"
GENERATED_V6_BYTES = 45888
GENERATED_V6_SHA256 = "cf44a9a8397eebf99271a4444bb41bd300fe5cfa60cc00696e9811a1469b52c7"
COMPACT_SOURCE = ("crosscheck/check_d972_r07_a0_compact_pc_invariant_owner_v1.py", 44831,
                  "7c1aea086ce264ad6f51983554a3a371ac481d07a2ec5f5d9a96ee270af6dfcf")
COMPACT_COUNT = 44
COMPACT_SHA256 = "7612682d024b61f873928ad122c9a5d7462c812a6633112f08706cda4412b6c8"
PRODUCER_V2_BYTES = 7707
PRODUCER_V2_SHA256 = "47cc53c0b59cbca0981983373d30604cbffd874cfa01d2d2adef599e505a21d3"


def _sha(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()


def _guarded_body() -> bytes:
    base = BASE.read_bytes()
    if len(base) != BASE_BYTES or _sha(base) != BASE_SHA256:
        raise SystemExit("Task458 v6 checker pin drift")
    tree = ast.parse(base)
    changes = next((ast.literal_eval(n.value) for n in tree.body if isinstance(n, ast.Assign)
                    and isinstance(n.targets[0], ast.Name) and n.targets[0].id == "changes"), None)
    counts = next((ast.literal_eval(n.value) for n in tree.body if isinstance(n, ast.Assign)
                   and isinstance(n.targets[0], ast.Name) and n.targets[0].id == "counts"), None)
    v5 = BASE.with_name("check_d972_r07_zero_base_a5_a6_compiler_v5.py")
    v5raw = v5.read_bytes()
    if len(v5raw) != V5_BYTES or _sha(v5raw) != V5_SHA256:
        raise SystemExit("Task456 v5 checker pin drift")
    v5tree = ast.parse(v5raw)
    v5changes = next((ast.literal_eval(n.value) for n in v5tree.body if isinstance(n, ast.Assign)
                      and isinstance(n.targets[0], ast.Name) and n.targets[0].id == "changes"), None)
    v5counts = next((ast.literal_eval(n.value) for n in v5tree.body if isinstance(n, ast.Assign)
                     and isinstance(n.targets[0], ast.Name) and n.targets[0].id == "counts"), None)
    v4name = next((n.args[0].value for n in ast.walk(v5tree)
                   if isinstance(n, ast.Call) and isinstance(n.func, ast.Attribute)
                   and n.func.attr == "with_name" and n.args and isinstance(n.args[0], ast.Constant)), None)
    if not all(isinstance(z, list) for z in (changes, counts, v5changes, v5counts)) or not isinstance(v4name, str):
        raise SystemExit("Task458 v6 checker transform manifest")
    x = v5.with_name(v4name).read_bytes()
    for (old, new), count in zip(v5changes, v5counts):
        if x.count(old) != count or (new and x.count(new) != 0):
            raise SystemExit("Task456 v5 checker transform cardinality")
        x = x.replace(old, new)
    if len(x) != 45942 or _sha(x) != "82641acb296573cb90fcf8a05048ce089e6b3e0355894f5c9e42fc3fd84d0e00":
        raise SystemExit("Task456 v5 checker generated drift")
    for (old, new), count in zip(changes, counts):
        if x.count(old) != count or (new and x.count(new) != 0):
            raise SystemExit("Task458 v6 checker transform cardinality")
        x = x.replace(old, new)
    if len(x) != GENERATED_V6_BYTES or _sha(x) != GENERATED_V6_SHA256:
        raise SystemExit("Task458 v6 checker generated drift")
    replacements = [
        (b"d972-r07-zero-base-a5-a6-compiler/v6", b"d972-r07-compact-direct-relator-a5-a6-positive-owner/v2"),
        (b"/checker-verdict/v6", b"/checker-verdict/v2"),
        (b"R07_ZERO_BASE_A5_A6_COMPILER_V6", b"R07_COMPACT_DIRECT_RELATOR_A5_A6_POSITIVE_V2"),
        (b"ROWS = 6441", b"ROWS = 44"),
        (b"args.rss_bytes == 8_000_000_000", b"args.rss_bytes == 5_700_000_000"),
        (b"default=8_000_000_000", b"default=5_700_000_000"),
        (b'"search/d972_r07_zero_base_a5_a6_compiler_v6.py", 2342,\n    "32cbc1a8e1faea0d4dc7a88a41a2ad3b535e7b2fd94b73ff286d78001262b96c"',
         b'"search/d972_r07_compact_direct_relator_a5_a6_positive_v2.py", PRODUCER_V2_BYTES,\n    PRODUCER_V2_SHA256'),
        (b"model = CheckerModel(helper, authority, arithmetic, boundary, task193)",
         b"authority = CompactAuthorityProxy(authority)\n    model = CheckerModel(helper, authority, arithmetic, boundary, task193)"),
        (b'    receipt, receipt_id = read_json(args.receipt, "producer:receipt", "ci/out")\n',
         b'    receipt, receipt_id = read_json(args.receipt, "producer:receipt", "ci/out")\n    require(receipt.get("compact_relator_roster") == {"owner": "Task411", "count": COMPACT_COUNT, "sha256": COMPACT_SHA256}, "compact:owner")\n'),
    ]
    for old, new in replacements:
        expected = 2 if old == b"/checker-verdict/v6" else 1
        if x.count(old) != expected or (new and x.count(new) != 0):
            raise SystemExit("compact v2 checker replacement cardinality")
        x = x.replace(old, new)
    marker = b"from __future__ import annotations\n"
    proxy = b'''\nCOMPACT_SOURCE = ("crosscheck/check_d972_r07_a0_compact_pc_invariant_owner_v1.py", 44831,\n                  "7c1aea086ce264ad6f51983554a3a371ac481d07a2ec5f5d9a96ee270af6dfcf")\nCOMPACT_COUNT = 44\nCOMPACT_SHA256 = "7612682d024b61f873928ad122c9a5d7462c812a6633112f08706cda4412b6c8"\n\ndef _compact_rows():\n    path = ROOT / COMPACT_SOURCE[0]\n    raw = path.read_bytes()\n    if len(raw) != COMPACT_SOURCE[1] or hashlib.sha256(raw).hexdigest() != COMPACT_SOURCE[2]:\n        raise Reject("compact:task411_pin")\n    ns = {"__name__": "task411_compact_checker", "__file__": str(path)}\n    exec(compile(raw, COMPACT_SOURCE[0], "exec"), ns, ns)\n    pres = ns["rebuild_presentation"](ns["load_data"](ns["JOINT"]),\n                                       ns["load_data"](ns["Q3"]),\n                                       ns["load_source"]("raw"))\n    if pres.get("compact_relator_count") != COMPACT_COUNT or pres.get("relators_sha256") != COMPACT_SHA256:\n        raise Reject("compact:roster")\n    return tuple({"word": list(word)} for word in pres["relators"])\n\nclass CompactAuthorityProxy:\n    def __init__(self, delegate):\n        self._delegate = delegate\n        self.rows = _compact_rows()\n        self.compact_relator_roster = {"owner": "Task411", "count": COMPACT_COUNT,\n                                       "sha256": COMPACT_SHA256}\n    def __getattr__(self, name):\n        return getattr(self._delegate, name)\n\n'''
    proxy = proxy.replace(b"return tuple({\"word\": list(word)} for word in pres[\"relators\"])" ,
                          b"return tuple(types.MappingProxyType({\"word\": tuple(word)}) for word in pres[\"relators\"])" )
    proxy = proxy.replace(b"    def __getattr__(self, name):", b"    def __setattr__(self, name, value):\n        if name in self.__dict__: raise AttributeError(\"compact proxy is read-only\")\n        if name not in (\"_delegate\", \"rows\", \"compact_relator_roster\"): raise AttributeError(\"compact proxy is read-only\")\n        object.__setattr__(self, name, value)\n    def __getattr__(self, name):")
    if x.count(marker) != 1:
        raise SystemExit("compact v2 checker future marker")
    x = x.replace(marker, marker + proxy, 1)
    return x


_BODY = _guarded_body()
GENERATED_V2_BYTES = 47815
GENERATED_V2_SHA256 = "ee826f1873e045574838e4fd478530edf2ef5986683c7f0ad72cf4958baac262"
if len(_BODY) != GENERATED_V2_BYTES or _sha(_BODY) != GENERATED_V2_SHA256:
    raise SystemExit("compact v2 checker generated body drift")
exec(compile(_BODY, str(Path(__file__).resolve()), "exec"), globals(), globals())
