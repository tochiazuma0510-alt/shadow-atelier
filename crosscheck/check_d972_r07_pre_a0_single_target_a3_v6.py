#!/usr/bin/env python3
"""Independent checker for the R07 pre-A0 projected A3/v6 receipt.
...
RECEIPT_SCHEMA = "d972-r07-pre-a0-single-target-a3/v6/receipt/v1"
VERDICT_SCHEMA = "d972-r07-pre-a0-single-target-a3/v6/verdict/v1"
...
def collect_authority_pins(p0):
    ...
    require(P0_PATH not in paths and
            not any(path.endswith("single_target_a3_v6.py") or
                    path.endswith("single_target_a3_gha_driver_v8.g")
                    for path in paths),
...
def task227_sparse_codec(value, width, label):
    """Translate task226's canonical dict terms to task227's pair codec."""
    require(type(value) is list and width in (4, 10),
            label + ":task226 sparse envelope")
    decoded = {}
    for term in value:
        require(type(term) is dict and set(term) == {"key", "coefficient"},
                label + ":task226 sparse term")
        key, coefficient = term["key"], term["coefficient"]
        require(type(key) is list and len(key) == width and
                all(type(item) is int and 0 <= item < 9 for item in key) and
                type(coefficient) is int and coefficient in (1, 2),
                label + ":task226 sparse typing")
        packed = tuple(key)
        decoded[packed] = (decoded.get(packed, 0) + coefficient) % 3
    decoded = {key: coefficient for key, coefficient in decoded.items()
               if coefficient}
    canonical_task226 = [
        {"key": list(key), "coefficient": coefficient}
        for key, coefficient in sorted(decoded.items())]
    require(value == canonical_task226, label + ":task226 sparse canonical")
    return [[list(key), coefficient]
            for key, coefficient in sorted(decoded.items())]


def task227_u0_codec(value):
    require(type(value) is list and len(value) == 11,
            "task227 u0 codec roster")
    converted = copy.deepcopy(value)
    for index, row in enumerate(converted):
        width = 4 if index < 6 else 10
        require(type(row) is dict and
                set(row) == {"ordinal", "terms", "translated_terms",
                             "source_coefficient_terms"},
                "task227 u0 codec row")
        row["terms"] = task227_sparse_codec(
            row["terms"], width, "task227 u0 terms:" + str(index + 1))
        row["translated_terms"] = task227_sparse_codec(
            row["translated_terms"], width,
            "task227 u0 translated:" + str(index + 1))
        provenance = row["source_coefficient_terms"]
        require(type(provenance) is list and len(provenance) == 2,
                "task227 u0 provenance roster:" + str(index + 1))
        for slot, record in enumerate(provenance):
            require(type(record) is dict and "terms" in record,
                    "task227 u0 provenance term:" + str(index + 1))
            record["terms"] = task227_sparse_codec(
                record["terms"], width,
                "task227 u0 provenance:" + str(index + 1) + ":" +
                str(slot + 1))
    return converted
...
def _consumer_body(interface, p0):
    ...
    for item in interface["occurrences"]:
        ...
        width = row["key_width"]
        for field in ("xi_o", "w_o", "translated", "u0"):
            row[field] = task227_sparse_codec(
                row[field], width,
                "task227 occurrence " + field + ":" + str(row["ordinal"]))
    ...
    return {"schema": p0["consumer_abi"]["schema"],
            ...
            "bar_epsilon_1": {
                block: task227_sparse_codec(
                    interface["target_blocks"][block],
                    10 if block == "P" else 4,
                    "task227 target:" + block)
                for block in ("H1", "H2", "P")},
            "u0": task227_u0_codec(interface["combined_u0"]),
...
def main(argv=None):
    ...
    try:
        ...
        with WallDeadline(meter, "complete v6 checker route"):
    ...
    try:
        ...
        with WallDeadline(meter, "complete v6 checker emergency publication"):
            target = prepare_output(args.verdict)
            ...
            raw = seal_fixed_point(unknown, meter, EMERGENCY_OUTPUT_MAX,
                                   ...
                                   pre_reserved=True)
            atomic_publish(target, raw)
        diagnostic = {"type": type(failure).__name__,
                      "detail": str(failure)[:512]}
        print("D363_CHECKER_UNKNOWN_REASON " +
              canonical(diagnostic).decode("ascii"), file=sys.stderr,
              flush=True)
        print("D363_CHECKER_TERMINAL " + terminal, flush=True)
        return 2
