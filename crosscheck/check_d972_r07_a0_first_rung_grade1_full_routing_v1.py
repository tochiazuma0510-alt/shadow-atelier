...
MARKER = "R07_GRADE1_FULL_ROUTING_REPLAY_V1_PASS"
...
class IndependentOwner:
    ...
    def reduce(self, packed):
        ...
        while cursor < self.packed_width:
            nz = np.flatnonzero(work[cursor:])
            if not len(nz): break
            bi = cursor + int(nz[0]); lead = 4 * bi + int(PACKED_FIRST[int(work[bi])]); pivot = self.lead_to_pivot.get(lead)
...
def selftest():
    ...
    return {"fixture": "PASS", "coefficient_2": "PASS", "packed_echelon": "PASS", "old_lower": "PASS"}
...
def load_source(state: Path):
    ...
    for i, expected_rank in enumerate((1509, 1512, 1512, 1512)):
        ...
        if b.get("phase") != "block" or b.get("character_index") != i or b.get("parent_sha256") != prepare_digest or b.get("rank") != expected_rank: fail("block_semantics")
...
def route(state: Path, candidate: Path, out: Path):
    ...
    target = np.frombuffer(residual, dtype=np.uint8).copy(); remainder, coefficients = grade.reduce(target)
