#!/usr/bin/env python3
"""Independent exact-pin A0-v20 successor of adapter checker v3."""
...
_BASE = Path(__file__).resolve().with_name(
    "check_d972_r07_history_free_task193_compat_adapter_v3.py")
_BASE_BYTES = 16804
_BASE_SHA256 = "f123daeec769aff9254bf913514f0792f20a2f32725aa19bd0020dc84e4c0c6f"
...
def _swap(source: bytes, old: bytes, new: bytes, count: int, label: str) -> bytes:
    if not old or not new or old == new or source.count(old) != count:
        raise SystemExit("adapter v4 checker " + label + " source cardinality")
    result = source.replace(old, new)
    if result.count(new) != count:
        raise SystemExit("adapter v4 checker " + label + " result cardinality")
    return result
...
if len(_patched) != _BASE_BYTES or hashlib.sha256(
    ...
    raise SystemExit("adapter v4 checker frozen v3 owner drift")

_changes = (
    (b'd972-r07-history-free-task193-compat-adapter/v3',
     b'd972-r07-history-free-task193-compat-adapter/v4', 1, "schema"),
    (b'CHECKER_SCHEMA = SCHEMA + "/checker-verdict/v3"',
     b'CHECKER_SCHEMA = SCHEMA + "/checker-verdict/v4"', 1, "checker schema"),
    ...
    (b'd972_r07_a0_v18_independent_for_adapter_v3',
     b'd972_r07_a0_v20_independent_for_adapter_v4', 1, "checker module label"),
    (b'R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V3_TERMINAL',
     b'R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V4_TERMINAL', 2, "terminal prefix"),
    (b'R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V3_CHECKER',
     b'R07_HISTORY_FREE_TASK193_COMPAT_ADAPTER_V4_CHECKER', 2, "checker prefix"),
)
