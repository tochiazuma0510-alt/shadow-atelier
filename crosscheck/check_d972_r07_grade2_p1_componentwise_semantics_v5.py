...
def selftest() -> dict[str, Any]:
    ...
    # Literal body/HEAD schema contract: the two exact strings are distinct,
    # while missing suffixes and interchanges are rejected before live inputs.
    body_schema = "d972.r07.a0.first-rung-grade1.v3.state"
    head_schema = "d972.r07.a0.first-rung-grade1.v3.state.head"
    require(body_schema == STATE_SCHEMA and head_schema == SEALED_HEAD_SCHEMA
            and body_schema != head_schema, "literal_body_head_schema")
    for candidate in ("d972.r07.a0.first-rung-grade1.v3",
                      "d972.r07.a0.first-rung-grade1.v3.head",
                      head_schema):
        rejected += expect_reject(
            lambda candidate=candidate: require(candidate == body_schema,
                                                "literal_body_schema_reject"))
    for candidate in ("d972.r07.a0.first-rung-grade1.v3",
                      "d972.r07.a0.first-rung-grade1.v3.state",
                      body_schema):
        rejected += expect_reject(
            lambda candidate=candidate: require(candidate == head_schema,
                                                "literal_head_schema_reject"))
    accepted += 1
    live.append("literal_body_head_schema")
