#!/usr/bin/env python3
"""Task438 v3: independent checker wrapper fixing the p176/base loader ABI."""
...
MARKER="R07_A0_ACTUAL_B72_FIRST_ACTIVE_V3_CHECKER"
...
def load_v1():
    p=ROOT/V1[0];b=p.read_bytes();need(len(b)==V1[1] and sha(b)==V1[2],"v1 checker pin");s=importlib.util.spec_from_file_location("task438_v1_checker",p);need(s and s.loader,"v1 checker loader");m=importlib.util.module_from_spec(s);s.loader.exec_module(m);return m
...
def toy():
    value=lambda x:("value",x);loader=lambda *x:("load",x);out=adapt({"p176":{"value_from_blob":value},"base":{},"t413":{"load_json":loader}});need(out["p176"].value_from_blob("x")==("value","x"),"p176 attribute adapter");need(out["base"]["load_json"] is loader,"base loader adapter");return True
def main(argv=None):
    ...
    if a.self_test:
        toy();r=m.self_test();print(f"{MARKER}_SELFTEST_PASS {json.dumps(r,sort_keys=True)}");return 0
