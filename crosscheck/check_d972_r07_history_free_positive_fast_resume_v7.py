#!/usr/bin/env python3
"""Helper-nonshared checker for the R07 v7 positive resume.
...
CHECKER_PREFIX = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V7_CHECKER_TERMINAL"
...
FIXTURE_PATH = "search/certs/d972_r07_history_free_positive_fast_resume_selftest_v7_20260829.json"
...
SOURCE_PINS: dict[str, tuple[str, int, str]] = {
    ...
    "fixture": (FIXTURE_PATH, 2783,
                "5f6628d8413ebcc78a87196fa999b4f73122d4bca2c22efc0a9f637e59f483c6"),
}

# Filled from the frozen producer after its last edit.
PRODUCER_PIN = ("search/d972_r07_history_free_positive_fast_resume_v7.py",
                122132,
                "cddef2abe41aaad9b2c0dd81d6fcc67b603bbf70359888fdfd34bf1d9f069ef9")
...
class Sources:
    ...
    def authenticate(self) -> None:
        ...
        for key, (relative, size, digest) in pins.items():
            path = ROOT / relative
            require(path.is_file(), "missing pin:" + relative)
            raw = path.read_bytes()
...
def _serial_group(p176: Any, row: dict[Any, int], block: int) -> Sparse:
    answer: Sparse = {}
    for (component, value), coefficient0 in row.items():
        coefficient = int(coefficient0) % 3
        if coefficient:
            raw = p176.packed_joint_blob(value, "checker group element")
...
def _serial_public(p176: Any, row: dict[Any, int]) -> list[list[Any]]:
    result = []
    for (component, value), coefficient0 in row.items():
        coefficient = int(coefficient0) % 3
        if coefficient:
            result.append([int(component), p176.packed_joint_blob(
...
def build_checker_light(sources: Sources) -> dict[str, Any]:
    ...
    old = sources.load("old"); p176 = sources.load("task176")
    ...
    for label, block, quotient, word in (("H1", 1, e3, h1),
        ...
        row = _serial_group(p176, gradient, block)
    ...
    runtime = {"old": old, "p176": p176, "e3": e3, "e4": e4,
    ...
    light_public = {
        ...
        "pb3_rows_sha256": sha_obj([_serial_public(p176, row)
                                     for row in pb3_group]),
        "pb4_rows_sha256": sha_obj([_serial_public(p176, row)
...
def blob(runtime: dict[str, Any], value: Any) -> bytes:
    return runtime["p176"].packed_joint_blob(value, "checker typed element")
...
class IndependentAllSeven:
    ...
    def occurrence_column(self, delta: Sequence[int], relator: Sequence[int]) -> Sparse:
        ...
        for spec in self.specs:
            ...
            add_scaled(answer, _serial_group(self.rt["p176"], translated,
                                             spec["block"]), 1)
    ...
    def direct_column(self, delta: Sequence[int], relator: Sequence[int]) -> tuple[Sparse,
        ...
        for block, quotient, base_word, corrected_word in words:
            ...
            add_scaled(answer, _serial_group(self.rt["p176"], difference, block), 1)
...
def independent_boundary_outcome(runtime: dict[str, Any], dual: Sparse,
    ...
    for worker_id, (start, stop) in enumerate(intervals):
        ...
        result = {"kind": "RESULT", "epoch": epoch, "worker_id": worker_id,
                  "interval": [start, stop], "attempted": stop - start,
...
def validate_common(runtime: dict[str, Any], receipt: dict[str, Any],
                    *, include_selftest: bool = True) -> dict[str, Any]:
...
def validate_source_transport(receipt: dict[str, Any]) -> dict[str, Any] | None:
...
def validate_monitor(value: Any) -> None:
    ...
    limits = value["limits"]; counters = value.get("fresh_v7_counters")
...
def validate_checkpoint_transport(receipt: dict[str, Any], receipt_path: Path) -> dict[str, Any]:
    ...
    require(type(formal) is dict and
            formal.get("owner") ==
                "old pivot_ancestry arrays plus literal new pivot_expression arrays" and
            formal.get("old_symbol_count") == 2896 and
            type(formal.get("old_ancestry")) is list and
            len(formal["old_ancestry"]) == 2896 and
            type(new_records) is list and
            formal.get("new_expressions") ==
                [record.get("pivot_expression") for record in new_records] and
    ...
    for ordinal, ancestry in enumerate(formal["old_ancestry"], 1):
        require(type(ancestry) is list and ancestry and
                all(type(item) is list and len(item) == 2 and
                    type(item[0]) is int and 1 <= item[0] <= ordinal and
                    item[1] in (1, 2) for item in ancestry) and
                [item[0] for item in ancestry] == sorted(
                    {item[0] for item in ancestry}),
                "resource canonical old ancestry")
    for index, record in enumerate(new_records, 1):
        require(record.get("symbol") == f"n:{index:04d}" and
                ...
                type(record.get("pivot_expression")) is list,
    ...
    formal_solution = checkpoint.get("formal_solution")
    require(sha_obj(public_sparse(target)) == OLD_TARGET_SHA256 and
            type(remainder) is dict and type(formal_solution) is list and
            formal_solution == sorted(formal_solution) and
    ...
    require(type(heavy_complete) is bool and
            ((not heavy_complete and checkpoint.get("heavy_input_sha256") is None) or
    ...
    prior_counters = checkpoint["monitor"]["fresh_v7_counters"]
    final_counters = receipt_monitor["fresh_v7_counters"]
...
def validate_unknown(receipt: dict[str, Any], receipt_path: Path) -> dict[str, Any]:
    ...
    validate_source_transport(receipt)
    sidecar = receipt_path.with_suffix(receipt_path.suffix + ".checkpoint.json")
    if terminal.startswith("UNKNOWN_RESOURCE:"):
        require(receipt.get("source") is not None and sidecar.is_file(),
                "resource physical sidecar required")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if terminal == COMMON:
        ...
        derived = validate_common(runtime, receipt)
        mutations = run_common_mutations(runtime, receipt, receipt_path)
        ...
        unknown_mutations = run_unknown_transport_mutations(receipt, receipt_path)
        verdict = seal({"schema": VERDICT_SCHEMA, "status": "PASS",
