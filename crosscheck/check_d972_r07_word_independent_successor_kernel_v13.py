#!/usr/bin/env python3
"""Independent A4 v13 checker: frozen v6 plus the v12 producer pin and
...
def main() -> None:
    ...
    if len(raw) != SOURCE_BYTES or hashlib.sha256(raw).hexdigest() != SOURCE_SHA256:
        raise SystemExit("v13 checker: frozen v6 source drift")
    for old, new in PATCHES:
        if raw.count(old) != 1:
            raise SystemExit("v13 checker: audited site is not unique")
