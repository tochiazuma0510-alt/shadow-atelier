# $N_\infty$ searcher — **stage 2 述語の仕様(spec v11・自己完結版)**

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 84)。**
...
```text
predicate_spec_id     = "mb/ninfty-stage2-predicate/v11"
...
verifier_contract_digest = 7cee75fdf4190cbaad37c85a920676773c8df9966e4a1307842ac2bb4280294b
...
22. **【必須・K】manifest v6 の FINDING-1 修理**(D-4′ を record ごとの自己束縛へ・「相互照合」撤回・SB-5 の `build_record_present` 明示宣言・SB-6 の I-3d との棲み分け)。**この設計で entry 粒度の substitution 反例が閉じているか。** **build record を持たない entry の宣言義務が抜け穴を塞いでいるか。**
...
```text
exact_freeze_bundle = {
  predicate_spec_id     = "mb/ninfty-stage2-predicate/v11",    predicate_spec_digest,
  verifier_contract_id  = "mb/ninfty-verifier-contract/v6",    verifier_contract_digest,
  dependency_manifest_schema_id = "mb/dependency-manifest/v6",  dependency_manifest_schema_digest
}
...
```text
live_authority_refs[] = [
  { artifact_id: "mb/ninfty-verifier-contract/v6",
    digest_or_receipt_slot: "7cee75fdf4190cbaad37c85a920676773c8df9966e4a1307842ac2bb4280294b",
    anchor: "§3.1 / §3.1.2 witness kinds, §3.4 result-vector, §5.1 two-axis" },
  { artifact_id: "mb/dependency-manifest/v6",
...
| **LA-3** | **自版だけを見る sweep は不十分** — 便 68 F6 が反証した失敗型であり、**裁定 83 に採録された。** |
> **[historical] LA-3**: sweep の対象定義**: 自 artifact の全版 token("spec v5..v10" / `mb/ninfty-stage2-predicate/v*`)と、**他 2 文書の全版 token**("contract v1..v5" / `mb/ninfty-verifier-contract/v*`・"manifest v1..v5" / `mb/dependency-manifest/v*`)。
