# $N_\infty$ searcher — **stage 2 述語の仕様(spec v9・自己完結版)**

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 81)。v8 の erratum。**
...
>
> **手続き的具体化**: `mb/ninfty-verifier-contract/v4` §3.1・§3.1.2(W-2 / W-2′)。**本節と contract v4 は同じ型分けを述べる**【v9 で参照更新】。
...
**二軸 routing(便 66 F13 の発案を採用・contract v3 §5.1 X-1〜X-6 と同期)**:
...
| **X-6** | **[26] の述語は「overall verdict の不一致」ではなく「canonical per-witness result vector $R_A\ne R_B$」**(contract v3 §3.4 R-1)。**vector は `ABSENT` と `FAIL` を区別する**(R-2)。overall verdict の不一致に限定するなら enum 名を `verifier-verdict-mismatch` とすべきだが、**本仕様は vector 比較を採る**(便 66 F4.2)。 |
...
```text
predicate_spec_id     = "mb/ninfty-stage2-predicate/v9"
...
verifier_contract_digest = 703fb47f60e721b2f0f6a79197f4047f723f030367fca9641a841aca6728bd75
...
## 8. Sol への監査依頼(v9 — **erratum の 3 点に限る**)
...
16. **【必須・F】contract v4 / manifest v4 の pin**。**manifest v4 で family を audit flag へ降格し build face を normative に昇格した判断**(manifest v4 §6 注記 N-1 に「三面が旧 I-3c を包含する」証明を付した)を、**spec 側から見て承認できるか。**
...
```text
exact_freeze_bundle = {
  predicate_spec_id     = "mb/ninfty-stage2-predicate/v9",    predicate_spec_digest,
  verifier_contract_id  = "mb/ninfty-verifier-contract/v4",   verifier_contract_digest,
  dependency_manifest_schema_id = "mb/dependency-manifest/v4", dependency_manifest_schema_digest
}
...
- **旧版(v6・v7・v8・contract v1–v3・manifest v1–v3)の audit 結果は、本 bundle の gate を満たさない。**
