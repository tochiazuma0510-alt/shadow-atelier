...
| G6 | §3.4 見積り表 / §0.1 札 | 2 枝 | **3 枝**へ。(N$_\infty$) の期待次元 **2**(= (W) と同じ・(N$_{\rm aff}$) の中の余次元 1 ではなく (N) の中の余次元 1) | 本便 |
...
| **命題 S5-3∞**(副枝 (N$_\infty$) の正規形 (3.3∞) = 多項式 Pell・**v1.2 新設**) | **紙上・単系統・未照合**(Sol 監査へ)。**design count でなく global な同値**であり分離条件を要しない。次元 2 は幾何と係数の二通りの数えが一致(§3.3.5) |
...
| 数え方 | 内訳 | 結果 |
|---|---|---|
| **幾何(stratum)** | $\{(C,P_\infty):P_\infty\ \text{非 W}\}$ は $3+1=4$ 次元。条件は $[\iota(P_\infty)-P_\infty]=[K-2P_\infty]\in J[5]\smallsetminus\{0\}$ で、$J$ は 2 次元・$J[5]$ は有限 ⇒ **余次元 2** | $4-2=\boxed{2}$ |
| **係数(design count)** | 未知数: $f_6$ の $B_0..B_4$(monic・$B_5=0$)$=5$、$a$ の $a_0..a_5=6$、$p$ の $p_0,p_1,p_2=3$ ⇒ 14。gauge: $x$-scaling 1 + $\mu$-scaling 1 ⇒ 12。条件: $a^2-f_6p^2$ の $x^1,\dots,x^{10}$ の係数が消える **10 本** | $12-10=\boxed{2}$ |

**両者が一致する。** ゆえに副枝 (N$_\infty$) は **(N) の中の余次元 1**(次元 2)であり、枝 (W) と**同じ次元**である。

> **【重要】次元が低いことは排除ではない。** 解は 0 次元(rigid)なので、余次元 1 の stratum 上にあってよい。しかも **(N$_\infty$) の組合せ的排除証明書は撤回された**(Rule 1 v1.3 §11 論点 7: 正しい $(0\,\infty)$ 述語 (35.4) の witness が**両 fixture に実在**する)。**「一般には起こらないから後回し」は、この設計書では使えない論法である。**
...
> - **副枝 (N$_\infty$) の 2(v1.2)**: **正規形 (3.3∞) 自体は global な同値(命題 S5-3∞)**で分離条件を要しない。次元 2 は依然 count だが、**幾何(stratum)と係数(design)の二通りの独立な数えが一致**している(§3.3.5 の表)。**この枝の探索器は未設計**であり、走らせられなければ結論は **BRIDGE-UNKNOWN**(§3.3.6)。
...
4. **$P_0$/$P_\infty$ の Weierstrass 性(【GAP-S5a】)を紙上で決められるか。**(**部分的に決着・v1.2 でさらに前進**)$P_\infty$ の枝(W)/(N)は依然として紙上で決まっていない(次元の一般性からは (N) が期待される、という発見的議論のみ)。**$P_0$ については枝 (W) 内で決着**(補題 S5-W: 枝 (W) なら $P_0$ は必ず非 Weierstrass・§3.3.3)。
   ...
   > **副産物**: 副枝 (N$_\infty$) では $[P_0-P_\infty]=[\iota(P_\infty)-P_\infty]=[K-2P_\infty]=2[W-P_\infty]$($W$ は任意の Weierstrass 点・$K\sim2W$)。ゆえに $\operatorname{ord}[P_0-P_\infty]=5$ は **$\operatorname{ord}[W-P_\infty]\in\{5,10\}$** と同値であり、$P_0$ を独立変数として持たない(**$P_\infty$ だけで決まる**)。§3.3.5 の次元 2 はこの縮約の帰結でもある。
