...
| R-3 | 親 manifest 側の whitelist/stop に I-b と同語を反映 | 司令塔(P1+P3) | **一部反映(便 36 条件 4・裁定 37・2026-07-27)**: `docs/manifest_k5_v1.md` **v1.4** の whitelist(§3)は「$\hat c_\mu$ の値・平方類・平方因子・符号の計算」を I-b と同語で逐語追加した。即時 integrity stop 節(「即時 integrity stop」見出し)は「$(N_\infty)$ 枝では $\hat c_\mu$ を含む」と明記したが、whitelist 節と全く同じ「値・平方類・平方因子・符号」の四語を逐語列挙してはいない。**「逐語反映」の十分性判定は Sol 検分待ち**(本行を安易に「閉」と自己判定しない)。 |
...
| **R-7**(v1.3) | §6.3-5: raw の `model_digest` を**凍結 bundle の expected digest に束縛**し、第三 checker で fail-closed に検査(**I-l**) | 実装 | **(N$_\infty$) 副枝で実装済み・全体は部分反映(便 36 F3.2 (3)/F6-2・裁定 37 条件 2・2026-07-27)**。`crosscheck/u-compare-ninf.mjs`(新設・旧 `u-compare-ninf-toy.mjs` を supersede)が二 raw の `expected_model_digest` の相互一致 + 独立再計算 digest との一致を fail-closed に検査(較正では driver が転記した synthetic 期待値を使用・実 K5 では Freeze 2 が注入する設計)。主枝(W/N$_{\rm aff}$)の `crosscheck/u-compare.mjs` にも同機構を追加したが、**K3 較正の凍結済み raw は本便で新設される前の schema のため `expected_model_digest` を持たず**(K3 finite fixture は改変しない規律)、その場合は `NOT_PROVIDED (pre-bridge)` と正直に記録し ACCEPT/STOP 判定に使わない(K3 の既存 ACCEPT は無傷)。実 K5 の Freeze 2 bundle 到達後、両経路とも同じ機構で fail-closed になる。 |
| **R-8**(v1.3) | §2.2 M0 / §6.3-6 / §9.2 **I-m**: 枝ラベルを三値 enumeration とし、**未知ラベルの既定値 fallback を禁止**(fail-closed) | 実装 | **閉(便 36 F3.2 (4)/F6-3・裁定 37 条件 3・2026-07-27)**。`crosscheck/u-extract-pathB-lib.mjs` の `loadModel`(主経路)から `raw.branch_P0 === 'nonWeierstrass' || raw.branchP0 === 'nonWeierstrass' ? 'nonWeierstrass' : 'Weierstrass'` という**無条件 Weierstrass fallback バグ**を除去し、`branchP0` を `{Weierstrass, nonWeierstrass, N_infty}` の三値 enumeration に限定(未知・欠落・`N_infty` はいずれも即 throw、fail-closed)。`loadModelNinf`/`ExtractPathA_Ninf`(GAP)にも `branch === "N_infty"` の逐語一致検査を追加。`crosscheck/check-r5-r8-ninf-fail-closed.mjs` で不正ラベル・欠落ラベル・`N_infty` 誤指定・正常ラベルの 4 ケースすべてが期待どおり fail-closed であることを確認(11/11 PASS、うち R-8 関連 4 件)。 |
...
**現状(2026-07-27・便 36 修理反映後)**: R-4(閉)・R-6(数学 PASS・文言修理済み)・R-8(閉)。R-5 は production 較正済み(K5 実 fixture 未接触は変わらず)。R-7 は (N$_\infty$) 副枝で実装済み・主枝は「機構は配線済み・K3 実データは pre-bridge のため expected digest 未提供」。R-3 は manifest v1.4 で一部反映(逐語性の十分性は Sol 検分待ち)。**R-1・R-2 はなお未着手**(実 commit ID の値記入・全修理後の再 hash・本文書自身の再提出)。

**R-1〜R-3・R-7 が閉じ、かつ本文書の再 hash(R-2)が済むまで、凍結 1 は受理されず、個別モデル探索コマンドは実行しない。**
