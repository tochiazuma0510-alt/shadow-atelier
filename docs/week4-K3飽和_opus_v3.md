# 定理 K3 — 奇数側 dihedral 窓 $K^{(3)}$ の算術飽和(答案 **v3**・便 28 追補反映版)

2026-07-26 起草: Claude(数学者レイヤー・Opus 5)。**司令塔 委嘱 24。便 28(PASS・昇格承認)の非 load-bearing 追補 2 件を反映。裁定 22 に基づく。**
...
検算: `search/week4-k3-v2-repairs.mjs`(43/43)・`search/week4-u-k3.mjs`(16/16)・`search/week4-19a19e.mjs`(7/7)・`search/week4-d2d4-k3.mjs`(13/13)。**v3 で追加した §5.2.3 の縮約は scratchpad 単発検算 10/10(`search/` 未収録・恒久化は司令塔判断 —【要記帳】)。**
...
> **状態(重要)**: **補題 R 自体は証明済み**である — が、それは (5′) を**前件に置いた**からにすぎない。**$R^{\rm cyc}$ が「定理候補・未証明」であることは v2 から変わらない。** 変わったのは**未証明の所在**である:
> $$ \boxed{\ \text{未証明部} = \text{「(4)(5) から (5′) を一般に導く段」}\ } $$
> であって、v2 が書いていた「(6) の $q$ を窓データから導く条件」ではない。2 事例では (5′) を個別に構成した(§2.3(b) と $A_5$ v4 §3.5)。**一般の窓で (4)(5) ⇒ (5′) を示すことが、族の定理へ残された仕事である。**
...
> のときに限る。
> **証明**: $v:=u^{-1}$、$\mathrm{ord}([v]_M)=e$ とする。$\mu_M\subset K$ より $[K(v^{1/M}):K] = \mathrm{ord}([v]_M) = e$。次に $\mathrm{ord}([v]_e)=e$ を示す: $d:=\mathrm{ord}([v]_e)$ とすると $d\mid e$ かつ $v^d\in K^{\times e}$、ゆえに $v^{d\cdot(M/e)}\in K^{\times M}$ で $e = \mathrm{ord}([v]_M)\mid d\cdot(M/e)$。(7.2) より $e\mid d$、よって $d=e$。したがって $[K(v^{1/e}):K]=e$。他方 $\bigl((v^{1/M})^{M/e}\bigr)^e = v$ なので $K(v^{1/e})\subseteq K(v^{1/M})$($\mu_e\subset K$ ゆえ $e$ 乗根の取り方に依らない)。次数が等しいので等号。最後に $\alpha^e = u^{-1}\iff(\alpha^{-1})^e=u$ より $K(v^{1/e}) = K(u^{1/e})$。∎
...
> **`search/` 未収録**(リポジトリ外で走らせた単発)。恒久化と LEDGER 記帳は司令塔判断 —【要記帳】。
>
> **2 例での (6′) の成立**:
> | | $A_5$ 窓 | $K^{(3)}$ 窓 |
> |---|---|---|
...
> **(7.5) の扱い**: 破棄はしない。**(5′)+(6′) から (7.2) の下で (7.5) が従う**(補題 R の 2・3 と注 2)ので、**(7.5) は前件ではなく系**として位置づける。

#### 5.2.5 【v3】射程条件と反証条件の事前登録(便 28 F8 step 1・第三例を選ぶ前に固定)

**結果値を一切含まない。第三例の計算前にこの節を封印する。**

| 種別 | 条件 | 判定後の扱い |
|---|---|---|
| **適用条件(scope-in)** | (0)(1)(2)(3)(4)(5)(5′)(6′) がすべて成立 | $R^{\rm cyc}$ の適用対象。(R6-full) を試験できる |
| **反証条件(falsify)** | 適用条件を満たす窓で、$\mathrm{ord}([u^{-1}]_M)=e$ なのに $\mathrm{Ih}_N$ が**非全射**、または $\mathrm{ord}([u^{-1}]_M)<e$ なのに**全射** | **一例で $R^{\rm cyc}$ を棄却** |
| **射程外(scope-out・棄却ではない)** | (3) が破れる($\mathrm{ord}(X)\ne\lvert\Lambda\rvert$ / 単純推移でない)/ $\Lambda$ が $\Phi(\mathfrak F_0)$-不安定 / $\rho_0$ が**非忠実** / (2) が破れる($\mathfrak F_0$ 非巡回) | **$R^{\rm cyc}$ の反例ではない。** 「射程外」として別札で記録し、$H$ の取り直しか前件の拡張を検討 |
| **$q$-版の反証条件(別立て)** | $\gcd(e,M/e)>1$ の窓で $K(u^{1/e})\ne\mathrm{Fix}(\ker\mathrm{Ih}_N)$ | **v2 の $q$-版の書き方だけを棄却**(安全形 (7.4) は生き残る)。**これが便 28 P4「repeated-primary regime を優先」の検証標的** |
| **縮約の反証条件** | (1)(3) が成立する窓で $\rho_0(\mathfrak F_0)\not\subseteq\tau(\mu_M)$ | **補題 R′ を棄却**(私の証明のどこかが誤り)。同時に (6′) の直接確認へ切替 |
...
> ただし §5.2.3 の縮約により、repeated-primary regime で試されるのは **(6′) ではなく「固定体の $q$-版の書き方」と「(5′) の一般性」**であることに注意する — (6′) 自体は $\gcd$ に依存しない。**この整理は第三例で何が falsify されうるかを鋭くする**(便 28 W3 の運用形)。
...
| **【GAP-Rcyc】** | **$R^{\rm cyc}$ の未証明部 = 「(4)(5) から (5′)(7.3) を一般に導く段」**(§5.2.2)。2 例では個別に構成済み | 中(族の定理への本丸) |
...
| **(v3・`search/` 未収録)** scratchpad 単発 | §5.2.3 の縮約: $\Phi_{0,k}(\bar x)=\bar x$・$\rho_\Lambda(\mathfrak F_0)\subseteq\tau(\mu_6)$・$=\tau(\mu_6[3])$・$\tau$ と可換・**W2 対照($S_6$ の型 $3.3$ の $C_3$ は 20 個)** | **10/10**(【要記帳】) |

---

## 8. ★ 教材(便 27 W1–W7 の受領・v3 で 9–11 を追加)
...
9. **【v3・便 28 W1】全射 $q:\mu_M\twoheadrightarrow\mu_e$ は、位数 $e$ の平行移動部分群への retraction とは限らない。** $M=4,e=2$ では $q(z)=z^2$ が $\{\pm1\}$ を丸ごと殺す。**「商への射」と「部分群の切断」は別物** — 前者を後者のつもりで使うと前件が空回りする。$\gcd(e,M/e)=1$ がちょうどその差を消す条件(§5.2.2 注 2)。
