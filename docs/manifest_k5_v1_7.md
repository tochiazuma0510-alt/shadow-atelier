# 事前登録 manifest — K⁽⁵⁾ 算術飽和キャンペーン v1.7(2026-07-28・司令塔)
...
> **v1.7(2026-07-28・裁定 71・便 60 F5-6)= v1.6 + `Z-norm-seal/v1`(`docs/znorm_seal_final_v1.md`)への typed reference を「結果規則表と結果記録の分離」節に新設。旧正本 v1.6 は上書きしない**(`docs/manifest_k5_v1_6.md` はこの版イベントで一切編集していない)。**既存 `Z20-link-seal/v1`・既存 finite edge ID(`rule1-tb2-root-equality/v1`)・既存 `z20_link_seal_id`/`root_equality_edge_id` 欄は無変更**(便 60 F5 payload 項 6)。本版が変更するのは当該節に追加した typed reference のブロック 1 つのみであり、他の全節は v1.6 から逐語不変。
...
- **(v1.7 新設・裁定 71・便 60 F5-6)full seal(`Z-norm-seal/v1`)への typed reference**: 本節が定める `z20_link_seal_id`/`root_equality_edge_id`(上記 1.)は**無変更**である。以下は**別 ID・別 scope**の seal artifact への参照であり、既存欄を置き換えない。
  ...
  unchanged (この版イベントでも変更しない):
    z20_link_seal_id     = "Z20-link-seal/v1"                # 上記 1. の定義のまま
    finite_edge_id       = "rule1-tb2-root-equality/v1"       # 上記 1. の定義のまま
  ```

  **`root_normalization_level = profinite` の記録欄は、当該 record が `Z-norm-seal/v1` の (R1)(R2)(R3)(TB4 導出 v2.4 §8.6a・便 60 F5 payload 項 4)をすべて満たす場合にだけ許可される。** 本 typed reference の追加だけで、既存 window($K^{(5)}$)の記録が `profinite` へ自動昇格することはない(`docs/znorm_seal_final_v1.md` §3 window inventory・`compatibility_status` の三値 enumeration を参照)。
...
## 出所対応表(v1.7・便 56 差分ゲート用・裁定 68 で機械可読 5 欄へ修理・裁定 71 で v1.7 行を追加)
...
| normative-only | 裁定 71・便 60 F5-6(`sol/sol_reply_60_seal_spec_v2.md` payload 項 6) | 冒頭版履歴(v1.7 ボックス) | — | — |
| adapted | `docs/znorm_seal_final_v1.md`(`Z-norm-seal/v1` final・§1・§9 component 束) | 「結果規則表と結果記録の分離」節・新設 typed reference ブロック | 裁定 71 | seal artifact の path・ID・scope・component 束の path のみを型付けて参照する形へ縮小。個別 hash は転記せず、digest authority を event receipt とする(便 60 F4.1/F4.5 の自己参照禁止を踏襲)。既存 `z20_link_seal_id`/`root_equality_edge_id` 欄は無変更 |
