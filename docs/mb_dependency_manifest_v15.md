# `mb/dependency-manifest/v15` — implementation dependency closure の記録様式

2026-08-01 起草: ep-keeper(EP 専任係・司令塔指示 便98 §6)。**Sol 便98 P98-6.2 が認可した versioned draft。前版を supersede。**

> **[historical]** **【版履歴・本版 = v15 DRAFT】** 起点は **Sol 便98 §6 P98-6.2**(「spec v20・verifier contract v15・dependency manifest v15 の versioned trio を起こすことを認可する」)。**本版は draft/implementation authorization に基づく versioned draft であり、freeze・W-6 closure・EP 発効の認可ではない**(P98-6.2 逐語)。変更は **3 点** — **(Z) §0 header の `governing_spec` / `schema_id` を新版へ**・**(AA) §5.3 に条項 `Y-3c` を新設**(点ごと W-6 の producer / receiver route plane の era 束縛)・**(AB) §9 の `live_authority_refs[]` / `historical_quotation_refs[]` / `sweep_definition` を同期**。**それ以外(交差検査四面・TCB 四欄・D-1〜D-4′・entry の型・role 分類・`Y-3a`/`Y-3b`)は前版と逐語同一。** **前版までの凍結版は byte 不変のまま。**
...
```text
schema_id     = "mb/dependency-manifest/v15"
schema_digest = <64 hex: 本稿 exact blob の sha256 — receipt が記入>
encoding      = UTF-8, LF, no BOM, no normalization
governing_spec = "mb/ninfty-stage2-predicate/v20"
governing_spec_digest = <64 hex: governing spec の digest — receipt が記入>
supersedes    = "mb/dependency-manifest/v14"
supersedes_digest = e892be68e79244c8493e37ec77eb3a1cbdb29ee45a911f73040aadaebbb889af   # 便98 P98-6.2 起点(点ごと W-6 plane の era 束縛)
supersedes_prev1  = df59b25f75e8e48a4607ed39177e5aa15be5a3fd4c738391aec347d8f7c1cb3e
supersedes_prev2  = 5c13e5c3f4101492fd414d48eb10d8b37e7d639162c46a05bb35165d203f22f0
...
## 0.-0.4 前版差分【Sol 便 98・P98-6.2】

> **起点の明示**: 本版の起点は **Sol 便98 §6 P98-6.2**。内部前哨ゲートでも Sol audit FAIL でもなく、**「点ごと W-6 の key/point-map schema と `R1'`/`R2'` を新しい exact era plane として matrix に登録せよ」**という 6 必須条件のうち、manifest が負う部分(era 束縛)への応答である。

| ID | 前版 | 本版 | 出所 |
|---|---|---|---|
| **Z** | §0 header の `schema_id` / `governing_spec` が前版を指す | **新版へ**(hash 順序 manifest → contract → spec の先頭ゆえ、本稿の digest 変更は contract 側 `dependency_manifest_schema_digest` pin の連鎖更新を要する) | 便98 P98-6.2 |
| **AA** | `Y-3b` は governing spec の payload-era matrix へ束縛するが、**点ごと W-6 の producer 出力(`mb/ninfty-w6-point-map/v1`)と二 receiver route が属す plane を持たなかった** | **§5.3 に条項 `Y-3c` を新設**(§8 `covered_clauses` に登録)。新 plane の era 束縛・**凍結 plane の payload として渡すことの禁止**・**宣言先行/採用後行**の順序を normative 化 | 便98 P98-6.2 item 1・item 2・item 6 |
| **AB** | §9 の live/historical refs と `sweep_definition` が前版を指す | **同期** | 便 66 F11 |

> **数学的内容(§1–§8 の他の全条項)・交差検査の四面・TCB 四欄・D-1〜D-4′・`Y-3a`/`Y-3b` は前版と逐語同一。****再監査範囲は §0 header の pin と `Y-3c` の 2 点。**

## 0.-0.5 前版差分【Sol 便 95・W95-2.3 + 司令塔裁定 2026-08-01】

> **起点の明示**: (X) governing spec の直近改版(`[27]` の versioned freeze、便95 W95-2.3・版番号は下表・§0 header 参照)、および (Y) 司令塔裁定 2026-08-01 第 3 項(EP registry generation の bundle receipt が docs trio digest を一切 pin していない穴の閉鎖)。**本稿自身の交差検査四面・TCB 四欄・entry の型・D-1〜D-4′ には一切変更が無い。**
...
| **E-6** | **`build_step_digests[]` は順序保存列である**(`canonicality = order-preserving-seq`・D-2 が順序ごと hash する)。**辞書順への並べ替えを要求してはならない。** **`toolchain_digest` と `build_step_digests[]` も mandatory。** 前者は toolchain 実体の exact blob digest(名前・版文字列ではない)で **null 不可**、後者は build 手順 digest の**順序つき列**。 |
| **E-9′** | **【本版で条件化・B69-4】`build_definition_blob_digest` と `pinned_input_digests[]`(sorted・deduplicated)は **top-level record と `build_record_present = true` の entry で mandatory**。これが **D-3 の preimage の全体**である。**`present = false` の entry では `[branch-contract]` の `forbidden_keys` が ABSENT(key の不在)でなければならない** — 判定の正本は §2.34 `[branch-contract]`(§2.35 の表はその描画)。** |
...
| **D-R2⁗** | **分岐 owner = §2.34 `[branch-contract]`(BC-1)。** **【本版で分岐同期・B70-1】 `[branch-contract]` の `recompute` に従う** — **D-1 と D-2 は全 record**・**D-3 と D-4′ は top-level と `present = true` の entry のみ**。**`present = false` の entry は QD-3 / QD-4 の canonical-empty check だけへ送る。** |
...
canonicality = {
  source_artifact_digests[] : sorted-dedup-set        # 集合表現(順序に意味なし)
  pinned_input_digests[]    : sorted-dedup-set        # 集合表現(順序に意味なし)
  build_step_digests[]      : order-preserving-seq    # 順序保存列(D-2 が順序ごと hash する)
}
# valid_scalar_digest(x) := x が 64 hex の非空文字列
# forbidden_keys は ABSENT(key 不在)でなければならない。null/空文字/0 は違反。
# consumer = { D-R2⁗, I-0″, I-0c″, build_artifact_set, R-6, H-1a″, C-6⁗ }
#   N-2 は補償論法(注記)であって record 判定の consumer ではないため本 literal から外す(便 74 F6.3)。
#   本 literal / BC_USE_MAP の key / executable consumer inventory の三集合は exact equality を要求する。
```
...
```text
[bc-use-map]                          # F12.1: consumer -> 依存 field(reference/causal-use gate の正本)
BC_USE_MAP = {
  D-R2⁗:              [required_keys, forbidden_keys, recompute],
  I-0″:               [required_keys, forbidden_keys, recompute],
  I-0c″:              [required_keys, forbidden_keys, recompute],
  build_artifact_set: [required_keys, forbidden_keys],
  R-6:                [required_keys, forbidden_keys, closure_policy],
  H-1a″:              [required_keys, forbidden_keys, closure_policy],
  C-6⁗:               [required_keys, forbidden_keys, recompute]
}
# 全 consumer は BC-1 の共有ゲート(validate_branch)を先に適用するので、
# required_keys / forbidden_keys には**例外なく**依存する。個別 field はその上乗せ。
# 各 field を変異させたとき「変化すべき consumer」= その field を持つ行、
# 「不変であるべき consumer」= 持たない行。self-audit が宣言的に検査する。
```

### 2.35 `build_record_present` の四象限 fixture 表 — **【rendered-nonnormative】§2.34 `[branch-contract]` block からの描画** {#quadrants}
...
**以後、`build_record_present` の分岐の正本は §2.34 `[branch-contract]` block だけである(【rendered-nonnormative】 本表はその描画)。E-9′・I-0″・I-0c″・build-face 射影は block を参照し、prose で別々に説明しない。**
...
| **QD-2** | `true` | **missing** = `true.required_keys` のいずれかが不在、または scalar digest が invalid、**または list 欄が schema-invalid**(要素が null / 非 64-hex、`canonicality` 違反 — set 表現欄の未 sort / 重複) | **`digest-mismatch` [12]** |
| **QD-3** | `false` | **canonical empty** = `false.forbidden_keys` がすべて ABSENT かつ `false.required_keys` が present **かつ schema-valid**(要素は非 null の 64-hex・`canonicality` に適合) | **bootstrap leaf として受理**・**D-1・D-2 のみ再計算**・**D-3 / D-4′ は免除**・**失う保証は `UNKNOWN`** |
...
implementation_dependency_closure(X) = 最小の集合 D で次を満たすもの:
  ...
  R-6  分岐 owner = [branch-contract]・【chg v4 新設】output-affecting な build-time artifact —
...
| **Y-3c** | **【chg v15 新設・便98 P98-6.2】点ごと W-6 の era plane。** 点ごと branch key schema(`mb/ninfty-w6-branch-key/v1`)・point-map schema(`mb/ninfty-w6-point-map/v1`)・lane A per-point producer・二 receiver route `R1'`/`R2'` は、**`Y-3b` の既存 plane のいずれにも属さない新しい exact era plane**であり、その正本は **governing spec §5.3.4 の `PAYLOAD_ERA_MATRIX` に新設された plane 群**である。**三条**: ① consumer は当該 plane についても **exact 一致**を検査する(「新しい方を許す」は禁止)。② **point-map payload を凍結 plane の payload として凍結 route の core へ渡してはならない** — point-map payload は `point_map_schema_id` を持ち、凍結 era の `native_schema_id` / `predicate_spec_id` 欄を**持たない**。**新 payload を凍結 era の payload と偽装することは明示禁止**(便98 P98-6.2 item 2)。③ **宣言先行・採用後行**: 新 plane は matrix に**先に**宣言され、**freeze receipt が本 trio を束縛するまで live consumer は前 plane 集合のまま**である。この docs 先行は、`Y-3b` 末尾が禁じる「実装だけを進めて matrix を黙って追随させる」違反の**逆向き**であって正しい順序であり、その間の当該 plane の判定は **`PENDING_ADOPTION`** と記録して **PASS とも FAIL とも数えない**(fail-closed)。 |
...
> **【current-unknown】** **⚠ UNKNOWN(一級の結果として記録・**operative であって historical ではない**)**: 上の (2) は **受領側が closure を独立に再導出できること**を前提にする。**受領側が producer の attestation の内部整合しか検査できない場合、build record と R-6 昇格の両方を同時に省いた manifest は、紙上の機構では捕捉されない。****この部分は UNKNOWN であり、EP(external positive control)の検収項目へ送る** — **「H-1a″ の独立再導出が実際に可能か」を EP で確認するまで、本注記は「補償経路が存在する」までしか主張しない。**
...
conformance_record = {
  ...
  covered_clauses = [BA-1, BA-2, BA-3, BA-4, BA-5, BC-1, BC-2, BC-3, CR-1, CR-10, CR-11, CR-2, 
                     ...
                     Y-1, Y-2, Y-3, Y-3a, Y-3b, Y-3c, Y-4, Y-4a, Y-4b, Y-4c, Y-4d]
...
historical_quotation_refs[] = [
  { artifact_id: "mb/dependency-manifest/v1..v14",     note: "版履歴・差分表・自認文のみ" },
  { artifact_id: "mb/ninfty-verifier-contract/v1..v14", note: "差分表・自認文のみ" },
  { artifact_id: "mb/ninfty-stage2-predicate/v5..v19", note: "差分表・自認文のみ" }
]
```
...
sweep_definition = {                      # [sweep-def] 現行有効
  ...
  current_version = "v15",
  historical_upper_bound = "v14",   # = current - 1(script v3 が自動照合)
