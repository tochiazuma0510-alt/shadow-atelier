# Week 3 manifest 追補 v2 — **PSL 七窓(case A/B)の実装宇宙**
...
各窓の
`hexagon_free_certificate`(`h10_fail` / `h11_fail` / `generation_fail` / `shadow_total`)・`gt_count`・per-$m$ の `ha_pass` / `class_coefficient` / `generation_pass_count` / `n_m`・`settled_m` / `settled_count`・`isolated`・`normalizer_order`・`phi_image`

は **封印済み(PSL_v1)**。**ハッシュは `provenance/LEDGER.md`**。本書には値を書かない。
...
| **PU-F14** | $\lvert C_{\mathrm{Aut}(\hat G)}(w)\rvert$ を各窓で**独立計算し証明書へ出力**する(**期待値は本 fixture に書かない — sealed 側 PS-F14 に封印**・開封時に司令塔が突合) | 出力欄の存在(値の事前指定なし)— **裁定済み 2026-07-26: 選択肢 (b) 採用**(U-F9/S-F9 と同型の処置) |
...
>
> **PU-F12 の相方は sealed**: W78 が要求する対のもう一方(**outer 位数 10 の control**)は **S6 の $n_m$ そのもの**なので、**spec に置くと較正の盲検性が壊れる**。⇒ **sealed 側 PS-F12b に封印**し、開封時に司令塔が対で突合する。
>
> **⚠ PU-F14 は部分的な漏れを含む(司令塔の裁定を仰ぐ)**: これは定理 B3′ の仮定($\mathrm{Aut}$ の構成が正しいか、とくに $\mathrm{P\Gamma L}(2,8)$ の実装)を列挙前に潰す**良い fixture** である。一方、**命題 R($n_m = \lvert C_{\hat G}(\hat v_m)\rvert$)を知っている読み手には $n_m$ の封印値が推測できてしまう**。$e$ 自体は PU-F2 のため spec に置かざるを得ないので、漏れの増分は「$\lvert C\rvert = e$ という一行」である。
> **選択肢**: (a) spec のまま(implementer は命題 R を渡されないので実害は小さいと見る)/ (b) sealed へ移し、代わりに「$\lvert C_{\mathrm{Aut}}(w)\rvert$ を出力せよ・期待値は封印」とだけ書く。**私は (b) を推す**(較正力の損失より盲検性を優先)。
> **司令塔裁定(2026-07-26): (b) 採用** — 期待値列(7・8・7・9・11・10・12)は **PS-F14 として sealed**(金庫の封印 payload に追記・spec 射影から除去)。implementer への引き渡し解禁。
...
| # | 内容 | 状態 |
|---|---|---|
| 【GAP-PM1】 | 七窓の per-$m$ staged 分配(`h10_fail`/`h11_fail`/`generation_fail`)の**紙上導出** | $\lvert\mathcal X\rvert\cdot\lvert G\rvert$ からの引き算と (H-a) の解数(= 指定 coset の対合数)までは紙で出るが、**封印値の全項は私の node 実装由来**。GAP との突合が二系統目 |
| 【GAP-PM2】 | `object_count = 2`(case B)は $\mathrm{Aut}(\tilde A) = \mathrm{Inn}(\tilde A)$($\mathrm{PGL}(2,p)$ complete)に依拠 | $q$ 非素の $\mathrm{PGL}(2,q)$ では成り立たない(委嘱 06【GAP-06e】)。**七窓の外へ外挿しない** |
| 【GAP-PM3】 | (3.53) の合成則と $\Psi$ の両立(case A の群同型) | 命題 B 定理 B で「(反)同型」まで進んだが、**実装側の合成表による二系統化は未了**(委嘱 06【GAP-06a】の残り) |
| 【GAP-PM4】 | 定理 M2 の第三型(case C・$\mathrm{Out}(G)\supseteq S_3$) | 本追補の 3 群では起きない。掃引を広げるときは M2 と補題 B1 の拡張が要る |
| 【GAP-PM5】 | 本書の全数値 | **私の node 実装(第三の独立実装)+ Sol の紙上指標計算**の一致(case A 4 窓)。**GAP 実装との突合は未了 ⇒ cross-checked ではない**。**Lean 未接続 ⇒ verified でもない**(W81) |
| 【GAP-PM6】 | 語規約 W-4 の遡及監査 | 既存の全証明書が (H-b′) を paper 積で書いているかを再監査していない(語規約 v2 §7【GAP-W3】) |
