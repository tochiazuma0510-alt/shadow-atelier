...
## 追補 1(2026-07-18・WP2b の差し戻し 6 件への司令塔裁定)

1. **(4.11) の確定式**(2405 p.18・ページ画像で原文照合済み)。shadow (m, g)、g = (r^{2k}, r^{−2k}, r^{ϰ(m)})、h = (h₁,h₂,h₃) ∈ Sₙ³ に対し:
   **x̄^{2m+1} = h · x̄ · h⁻¹ かつ g⁻¹ ȳ^{2m+1} g = h · ȳ · h⁻¹**(成分ごとの Sₙ 内共役)。
   構成: b = (j ↦ (2m+1)j) ∈ Sₙ、h₁ = r^{−2k−m}b、h₂ = b、h₃ = b(m 偶)/ bs(m 奇)。照合器は h を証明書から受け、この 2 等式を検査する(provisional 実装を置換)。
2. **(5.1) の確定式**(2405 p.23・同)。[m,f] ∈ GT(K⁽ⁿ⁾)、3|n。∃g,h ∈ F₂:
   **f K_F₂ = θ(g)⁻¹ g K_F₂**、かつ **f x^m K_F₂ = τ(h)⁻¹ h K_F₂(m ≡ 0 mod 3)/ τ(h)⁻¹ x y h K_F₂(m ≡ −1 mod 3)**。
   ※m ≡ 1 mod 3 のケースが無いのは**空虚**だから: 3|n なら 3|N_ord で、m ≡ 1 mod 3 ⇒ 3|(2m+1) ⇒ 単元条件に反し 𝒳ₙ に入らない。照合器の「ok:null」枠は「該当 m は shadow に現れない」の検査(現れたら FAIL)に置換。
3. **counts の簡略化を承認**: raw 候補の完全再列挙は不要(⑤ の集合一致が最終段を実質カバー)。単調性+最終個数一致で可。承認済みの緩和として verdict に明記。
4. **N₅ の kernel_cert(type=brute)の確定手続き**: BFS で ⟨σ̂₁,σ̂₂⟩(30 元)を列挙しつつ、対応 σ̂ᵢ ↦ σ̂ᵢ^{2m+1} の**井戸定義性**を衝突時に検査(同じ置換に到達する 2 語が右側でも同じ置換に到達するか)。井戸定義+全単射(像の位数 30)なら T は B₃/N₅ を経由し単射 ⇒ settled。数学的根拠: T = δ∘β₅、δ = (S₃ 恒等, t↦t^{2m+1}) ∈ Aut(S₃×C₅)。
5. **reduction の添字規約を凍結**: `image[i]` = source の `shadows[i]` の像の、**target 証明書の shadows[] における添字**。全射性 = image の値域が target の全添字を被覆。
6. **(4.20) の法は n で承認**(ϰ は r の冪指数なので mod n が正しい)。コード内コメントの判断を追認。
