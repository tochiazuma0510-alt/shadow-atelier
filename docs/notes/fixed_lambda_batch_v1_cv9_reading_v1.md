...
**裁定 2173 追補(Astra 実 metadata による集計訂正)**: σ の分布は 1:17/2:15(16/16 ではない)・新三群の実拒否は P 7+6+26 = 39 / C 2+3+14 = 19(P 21 は旧集計)・Python 継承 19+新 2/raw 3 = 全 24・primal 76.402 s + P1 253.602 s = 330.004 s = 候補六相 351.018 s の 94.0 % = producer 全 432.437 s の 76.3 %(工房 express の「P1 相が producer 時間 94 %」は候補六相内の比率であり producer 全体では 76 %)。ω = 2 の 14 件は sr(2) = −1 の literal 指数採用であって −1 対 +2 の算術識別ではない(2150/2151 の同 source/同 P1/同物理行の証明を維持・PB4/Δ-Fox 一致へは拡張しない)。F-flb-1 = 163 F8.89 で共有 TCB 登録(両カーネルの P/C 全 4 source の file/bytes/SHA 明示・旧凍結 source/docstring は上書きしない)。旧 full-prefix replay の rank ≈ 1,890 見積りは thin-anchor 方式へ流用しない。
