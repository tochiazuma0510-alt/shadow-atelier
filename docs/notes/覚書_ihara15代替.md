...
- **【GAP-D0】= 二次情報 2 系統で特定・原本未達(閉鎖ではない — Opus 精度注記で訂正)**: 「G = ∐G_i 自由副有限積 ⇒ G_i ∩ G_j^g = 1(i ≠ j または g ∉ G_j)」= **Ribes–Zalesskii (Springer 2000) Thm 9.1.12**(+隣接 9.5.1・多部構成の可能性 — 項番号の特定は次版課題)。逐語出典: 0712.4244 Thm 2.9(逐語引用・papers/delivered に現物)・1711.01500 p.13 — 2 独立ソース同番号。札: 原本未達(二次確認)・2010 第 2 版の番号異同未確認。
- 補題 D の背骨は「補題 D0 自前証明(主)+ R–Z 9.1.12 番号引用(帯)」の二重装甲で確定。
- 経緯メモ: 前任 scout が旧目標(中心化群)の文脈で 9.1.12 を「別言明」と誤トリアージ → 差し替え発注で同じ番号が正解と判明・自己訂正を報告書に明記(良い挙動)。
