...
# 追記 G(便 99 検収・裁定 412)— **依存の修正 1 件**と射程の明文化

> **追記型**: 追補 E-F・追記 F の本文を**一切改変しない**。以下は**依存表示の修正**と**射程の明文化**である。
> 起草: 数学者(Opus 5)・2026-08-02。入力 = **Sol 便 99 返信 F99-3.7**(`sol/sol_reply_99_math26.md` §3.5)。

## G.1 検収の結果

**PASS。** Sol の判定は次のとおり:

- **補題 PROP41-EVEN-odd** の偶奇計算と **補題 THM44-odd** の CRT 迂回は**循環していない**($m\mapsto m+2nz$ で $n$ を割る素数に対する条件を保ち、$q$ の新しい各素因数について避けるべき剰余類を一つずつ外し、CRT で同時に選び、最後に対応する $k$ を持ち上げる)。
- この補完により、**奇 $q$ の範囲で** GEN-DESC・GEN-COFINAL(整除共終な $S$)・**FIVE-BYPASS**・FAKE-LIFT を **paper-proof candidate** として受理。探索の極小候補を奇素数へ落とす結論もこの射程で正しい。

## G.2 ★ 依存の修正 — ML-ODD の抽象 directedness は **(COF) だけで足りる**

**Sol の指摘(採用)**: 「ML-ODD の抽象 directedness は **(COF) だけで足りる**。**(INT) は roof / intersection の同定に使う**が、directedness 自体の前件ではない。」

### G.2.1 当方の確認(2 行の証明)

$I:=\mathrm{NFI}^{\rm isolated}_{PB_3}(B_3)$ を refinement 順序($N\le K\iff N\subseteq K$)で見る。$N,K\in I$ に対し:

1. $N\cap K\in\mathrm{NFI}_{PB_3}(B_3)$ — **これは自明**(有限指数・$PB_3$ 正規・$B_3$ 安定はいずれも交叉で保たれる)。**isolated 性は要らない。**
2. **(COF)**(2401 Prop 3.14 の系:$\mathrm{NFI}^{\rm isolated}$ は $\mathrm{NFI}$ の中で共終)より、$\widetilde N\in I$ で $\widetilde N\subseteq N\cap K$ なるものが存在する。

⟹ $I$ は**有向**。**(INT) を一度も使っていない。** ∎

### G.2.2 これが効く理由(**依存の格が上がる**)

| | 旧(ihnec §B.3・§7 の (DIR) 行の読み) | ★ 追記 G |
|---|---|---|
| $I$ の有向性の前件 | **(INT)**(= 2401 **Prop 3.15**・**原論文に証明なし**・ihnec §B.2 で自前補完) | ★ **(COF)**(= 2401 **Prop 3.14 の系**・**正典に証明あり**) |
| 「**(INT) が落ちると ML-ODD (iii)$\Rightarrow$(i) は全崩壊**」(ihnec §B.3 の注) | そう書かれていた | ★ **修正**: (INT) が落ちても**有向性は生き残る**((COF) で足りる)。落ちるのは**特定の屋根の isolated 性の同定**(命題 ROOF(3) 等)である |
| (INT) の実際の用途 | 有向性 + 屋根の同定 | ★ **屋根 / 交叉の同定のみ**(命題 ROOF(3): $M=K^{(9)}\cap N_{\rm S4}$ の isolated 性など) |

> ★ **依存鎖からの読者演習の除去**: この修正により、**ML-ODD の主線(有向性)は「原論文に証明がある定理」だけに乗る**ことになった。工房が自前補完した (INT)(ihnec §B.2)は**主線から補助線へ**移る。これは**依存の格が上がる方向の修正**であり、[追補 B](2401 Prop 3.15 = 読者演習)の発見の実務的な帰結でもある。

### G.2.3 本文への反映(**不改変・読み替え表**)

`docs/notes/ihnec_v1.md` の次の記述は、本追記を **effective source** として読み替える(本文は erratum 方式で不改変):

| 箇所 | 旧 | 読み替え |
|---|---|---|
| §B.3 の (INT) 行「用途: ① 補題 ML-3 の適用条件($I$ の**有向性**)② 命題 ROOF(3)」 | ①②両方 | ★ **② のみ**。① は (COF) が担う |
| §B.3 の「**(INT) が落ちると ML-ODD (iii)⟹(i) は全崩壊**」 | 全崩壊 | ★ **有向性は残る**。崩れるのは屋根の isolated 性の同定 |
| §B.3 の「**(COF) が落ちても定理は生き残る**(量化を $\mathrm{NFI}$ 全体に戻せばよい)」 | そのまま有効 | **不変**(ただし有向性の前件が (COF) に移ったので、(COF) を落とす場合は「$\mathrm{NFI}$ 全体で有向」= 自明、を使うことになる旨を追記) |
| §7 の (DIR)/(INT)/(COF) 行 | — | 上記に同じ |

## G.3 THM44 補完の射程の明文化(**奇 $q$ のみ**)

Sol の要求により、追補 E-F が閉じた範囲を**改めて明文化**する:

$$\boxed{\ \textbf{補題 THM44-odd が閉じたのは }q\ \textbf{奇の分岐のみ。偶 }q\ \textbf{の未掲載分岐は未解決のまま。}\ }$$

- **閉じた**: $n,q$ ともに奇で $n\mid q$ の場合の $R_{K^{(q)},K^{(n)}}$ の全射性(§E-F.3.2)。その根の **補題 PROP41-EVEN-odd** も閉じた(§E-F.3.1)。
- **閉じていない**: $q$ 偶(とくに $4\nmid q$)の分岐 = **【IHNEC-GAP-5】**(§E-F.7.2)。正典 2405 Thm 4.4 は $4\mid q$ 分岐のみ証明を掲載し、他は読者演習である。
- ⟹ **「(THM44) を工房が閉じた」と書いてはならない。**「**奇分岐を閉じた**」が正確な形である(§E-F.6 の差替表と同じ)。$\mathrm{Dih}$ 全体(偶を含む)の形を使う文書があれば、**そこは未証明分岐に乗っている**。

## G.4 FIVE-BYPASS の位置づけ(Sol 逐語の採用)

> **「FIVE-BYPASS は seal 回避策ではなく、開封後も独立に有効な整合経路である。」**(F99-3.7)

追記 F の **FB-2**(「$n=5$ の肩代わり」→「(a) GEN-COFINAL の系 / (b) 二窓にまたがる整合性検査 / (c) 戦略の反転」)と**同旨**であり、Sol の表現を**正本の言い方**として採る。すなわち:

- **誤**: 「封印が解けたので FIVE-BYPASS は用済み」。
- **正**: 「封印の有無と独立に、$K^{(15)}$ と $K^{(5)}$ の genuine 判定を結ぶ**整合経路**として有効」。**実用上の優先度**(哨戒の標的)が $K^{(5)}$ 直撃へ移っただけである(FB-3)。

## G.5 この追記が変えないもの

- **定理 GEN-COFINAL・系 CHAIN・系 FAKE-LIFT・系 GEN-COFINAL-(ii)・補題 GEN-DESC・補題 THM44-odd・補題 PROP41-EVEN-odd の言明と証明**(すべて不変)。
- **§E-F.7 の格付け表**(THM44-odd = paper-proof candidate 等)。ただし **Sol 監査済**へ更新してよい(F99-3.7 PASS)。
- **凍結物**: 事前登録予言 P-IHN-1〜7 と検算 digest は**改訂なし**。
- **【IHNEC-GAP-5】は開いたまま**。
