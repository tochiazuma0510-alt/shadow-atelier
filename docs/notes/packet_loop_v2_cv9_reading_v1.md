# fixed root packet loop v2(root 全零終端)増分 CV-9 判読(falsifier 逐語・裁定 2125 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 cea22d227b060942・保存ファイル全体)を逐語転記(2026-09-05)。

**工房裁定(2125)**: CV-9 = **同一対象**(限定 5 条・規約表 11 保持/6 拡大/2 移行/6 新設/弱化ゼロ)。工房格 = **checker PASS(系統完全分離・packet は両側ゼロから独立再構築(cross-side 0.069)・全 3 step を親から zero replay・packet 4 ファイル+manifest と全 3 step の 8 payload+manifest と終端 result.json(176 pairing 値)をバイト一致で相互束縛・両側に負のカナリア・scan に deadline 割込みなし・ただし v15 seed 核 0.95〜0.98 と check_table_transpose 1.00 クローン持ち越し)・cross-checked は限定つき** — (i) 射程 = 固定 44 seed packet に対する rank 1356→1359 の 3 周回のみ・actor origin/orbit/全物理像は未走査・**grade-2 NONMEMBER ではない**(ii) **終端 176 のうち informative は char 0 の 44 本のみ・char 1〜3 は B_a*λ = 0(sha256(0x00×9072) と一致)の構造零 132 本**(iii) rank1354 導出・旧 reduction 884・seed30/34 delta は前提・λ·ρ₂ = 1 は本 run でも未計算(明示 DERIVED・hash 連鎖のみ)・実測は λ·(target 剰余) = 1 と λ ⊥ 全 rank_after 行(3/3 外部再計算一致)(iv) pivot 挿入・正規化・target 更新の算術は 2117 pair の再利用(v) **step 3 は target を動かしていない**(scalar 列 [1,1,0]・target 剰余は step 2 と step 3 でバイト同一)。主張 = 「rank 1356 から seed 35/36/37 由来の pivot 3 本(lead 1419/1420/1421)を追加して rank 1359 とし、その λ が固定 44 seed packet の char 0 の 44 pairing を全て消し(残り 3 character は B_a*λ = 0)、target 剰余は依然非零」という有限事実。GRADE2 NOT_DECIDED・verified=false。**判読規律(追加恒久項目)**: 零 root の character 数を数えて informative/構造零の内訳を必ず書く・各 step の target.scalar 列を並べて 0 の混入を確認。副産物: step 1 で非零は (char0, seed35) 1 本のみ・seed36 は λ_1356 で零 → step 2 で seed36/37 が新たに非零化(Astra 163 F2「未追加の行は旧 λ で零でも次の λ で非零になり得る」の実測例)。
