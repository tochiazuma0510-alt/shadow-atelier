...
| **定理 K3** | $\mathrm{Ih}_{K^{(3)}}$ 全射・$L_3=\mathbb Q(\zeta_{12},\sqrt[3]2)$ | **paper-proof / two-mathematician audit PASS / framework-conditional**(前件 (K1)–(K4);(K3‡) は $A_5$ 由来) | `week4-K3飽和_opus_v3.md`・便 27/28/29;CLAIMS W3-11・裁定 22 |
...
### 6.1 §5(前線)で引用した主張の格付け(**本稿の主張ではない・引用の状態**)
...
| **命題 C-21**($n=21$ で BFC 補題 B-5 の全結論;scope 縮小 = exact (TB4) 不要・$(Z_{84}$-link$)$ 不要) | **paper-proof / framework-conditional / two-mathematician audit PASS** | `c21_draft_v1.md`・裁定 208 起草・**裁定 214 昇格** |
| **A7-fam**(同前件下で A7 が**全奇数 $n\ge3$**)⟹【I23-a】解消 | **同上**(C-21 とは別札) | `c21_draft_v1.md` §7・便 85 F85-1.2・**裁定 214** |
| **定理 CASC**($[u_m]_2\in E_{\gcd\mathcal D}$;歯 $\{3,7\}$ で 0 bit) | **paper-proof candidate**(補題 K1/K3/K4/K5/T2/SQ-$dm$/C′)+ **Sol 射程限定**(F79-3.3: $\gcd(m,21)=1$ の標的のみ) | `i23_cascade_lemma_v1.md`・裁定 153/170 |
| **定理 W-REL**($\nu(H_{2,\alpha,\beta})=H_{3,\alpha,\beta-1}$)+ **定理 J-BLIND** | **paper-proof candidate**(独立 Python 検算 $n=3,5,7,9,11,15$ FAILS=0) | `i24_transport_design_v1.md` §5–§6・裁定 173 |
| **凍結予言 I24-P1**($u_{H_3}=+4$・class 一致) | **HIT**(prediction-first・凍結 `9f6187b` → 実測 16/16) | **裁定 174** |
| **C1**($n=3$ の窓同定) | **完全閉鎖**(機械同定 + 族的機構 W-REL) | 裁定 107(`CLOSED_MATCH`)・裁定 174 |
| **C1′**($n\ge5$ の窓同定・$q=7$ で $[\alpha]$ 3 類) | **開**(唯一の閉鎖路 =【I24-a】$\alpha$ 軌道予想) | `i17_check_v1.md` §5・`i24_transport_design_v1.md` §9.2・§11 |
| **C5**(宇宙の事前登録) | **未登録**(手続き)。$n=21$ 較正 8/8 は**較正であって前件充足ではない**(Sol F85-1.2) | `c21_draft_v1.md` §8・裁定 208-6・`provenance/registered/universe_I1_I3.md` |
...
| **E1-GAP-3** | **$n=12$ の未接続の含意**。定理 K3 + 正典 Thm 5.3 + fiber 積(裁定 101 ④)+ $\mathcal E_{12}=1$(便 75 F6.3(c))から $\mathrm{Ih}_{K^{(12)}}$ 全射が Goursat 一段で出るように見えるが、**その一段は工房のどこにも書かれていない**。裁定 101 ⑤ が UNKNOWN とした算術側残件は便 75 が $n=12$ で閉じているのに、帰結が引き取られていない。副次的確認事項: $L_3$ が $\ker\mathrm{Ih}_{K^{(3)}}$ の固定体であることは (K4)($\Phi$ 単射)に依存する | §5.6 | **未接続**(部品は揃っている・**本稿は主張しない**) |
...
| **E1-GAP-5** | **(S3) の族版が無い**。定理 E1-3 により中間峰は「全奇 $n$ で $\mathrm{ord}(a_n)=n$」に同値だが、これを**族として**証明する道具は無い($n=3$ の個別計算のみ)。A7-fam・C-21 は前件供給の族化であって (S3) 本体の族版ではない | §5.1・§5.5 | **UNKNOWN**(中間峰の本丸) |
| **E1-GAP-6** | **下界 $\mathrm{ord}(a_n)\ne1$ の装置が無い**(E1-GAP-5 のうち、いま最も明確に空いている半分)。CASC・補題 C′・SQ 系が与えるのはすべて**上界** $\mathrm{ord}(a_m)\mid m$ である。下界が出ているのは ①$n=3$(直接計算 $\mathrm{ord}([-4]_6)=3$)②$n=9$(塔関係 (6.3-cls) で $\mathrm{pr}_{18\to6}(a_9)=[-1/4]_6$ の位数 3 — T63-P1・便 75 F4.1)の 2 例のみ。**$n=7$ は素数ゆえ $3\le d\mid7$、$d<7$ なる中間段が無く、②の経路が構造的に使えない。** 「$q=7$ 左枝から $\mathrm{Ih}_{K^{(7)}}$ 全射が従う」という記述は repo 内に**発見できなかった**(全文 grep 済・**出典未発見**) | §5.4 図・§5.5・§5.7 | **UNKNOWN**(**q=7 前線の直下にある空白**) |
...
| §5 | `docs/week4-K3飽和_opus_v3.md` §2.4(定理 K3 の骨格)/ `docs/notes/w2fam_v1.md`(裁定 120)/ `docs/notes/w2arith_v1.md`(裁定 122)/ **`docs/notes/c21_draft_v1.md`**(C-21・A7-fam・C5 fixture・§5.3 の 1 ビット導出)/ **`docs/notes/i23_cascade_lemma_v1.md`**(定理 CASC・補題 K1/K3/K4/C′・梃子率)/ **`docs/notes/i17_check_v1.md`** §4–§5(二枝・C1/C1′/C5 初出)/ **`docs/notes/i24_transport_design_v1.md`**(W-REL・J-BLIND・§9.2 C1′ 残余・【I24-a】)/ `docs/notes/t63_reconnaissance_v1.md`(T63-P1)/ 便 75 §F4.1・§F6.1・§F6.3(裁定 111)/ 便 79 F79-3.3(射程限定)/ 便 85 F85-1.1/1.2(裁定 214)/ 裁定 24・95・101・102・107・153・170・173・174・208・214 / `docs/地図.md` P1・P2 行 |
...
**grep 語**: `GT^odd`・`GT^odd_Dih`・`Aff(`・`4 点セット`・`Goursat`・`framed`・`Fr(G`・`K⁽¹²⁾`・`entanglement`・`E₁₂`・`C1′`・`C5`・`A7-fam`・`CASC`・`q=7`・`ord(a_`。

- **既出**: 4 点セットの内容すべて(裁定 111/130/138/226・CLAIMS 登録済)/ 自然座標(便 75 F6.2(b))/ 極限形(便 75 F6.2(c)・地図 帯 0 領有)/ 同値(便 75 F6.2(d))/ Φ-fam と Out 訂正(裁定 130/138)/ fiber 積と $\mathcal E_{12}=1$(裁定 101・便 75 F6.3)/ 定理 K3 の骨格 / $R^{\rm cyc}_{\rm formal}$(裁定 24)/ CASC の梃子率と Sol の射程限定(裁定 153/170)/ C1′・C5 の内容(i17・i24)。
- **本稿で新しいのは記述であって定理ではない**: ①**証明を全段書き下したこと**(先行文書は明示的に証明を省いていた)②補題 E1-D2(正規化代表と順序が整除順序であること)・E1-S3/S4(分裂系と単数群の極限)の**明示化** — 便 75 が「従って」で通した初等段 ③**E1-1 の効き所を三箇所に分離**したこと(群構造・$\mathrm{Ih}_n$ 準同型性・$\Phi_n$ の codomain)④**(S3) を上界層と下界層に分解**し、CASC が上界側だけを族的に攻めていることを明示したこと(§5.5)⑤**q=7 依存図**と「従わないもの」5 項の明示 ⑥**【E1-GAP-1〜6】の名指し** — とくに **E1-GAP-3**($n=12$ の未接続の含意)と **E1-GAP-6**(下界の装置が無いこと)は本稿の統合作業で見えた ⑦Lean 化候補の 1:1 表と **`E1.levelLift` $=$ I-27 先頭の同一性**の指摘。
- **「初」という語は使わない**(工房外の文献での既知性は未調査)。E1-GAP-6 については repo 全文 grep で該当記述を発見できなかったという事実のみを報告する(**出典未発見**であって「存在しない」の証明ではない)。

### 9.3 申し送り(司令塔へ)

1. **【E1-GAP-6】(下界の装置)を中間峰の第一標的として立てられたい** — 本稿の統合で最も重い発見。**現在の前線 $q=7$ は (S3) の上界層にしか効かない**。左枝が出ても $\mathrm{Ih}_{K^{(7)}}$ 全射は従わず、CASC が全発火しても odd Conj 5.1 は従わない。下界が出ている 2 例($n=3$ の直接計算・$n=9$ の塔経路)はいずれも $n=7$ に転用できない($n=7$ は素数で中間段が無い)。**「q=7 が獲れれば族が刈れる」という地図の読みは、上界層に限れば正しく、中間峰の登頂には半分足りない** — この差を地図 P2 行に書くべきかは司令塔の裁定事項。
2. **【E1-GAP-3】($n=12$)は最優先で裁定されたい** — 部品が全て揃っており、Goursat の一段(数行)だけが欠けている。本稿は委嘱の規律(「新しい数学は作らない」)に従って主張しなかったが、成立すれば **Conj 5.1 の混合位数最小 open 対象($n=12$)が閉じる**。裁定 101 ⑤ の残件が便 75 で閉じたことが引き取られていない、という**台帳の連結漏れ**でもある。
3. **【E1-GAP-1】(framed 圏の定義)は数学ではなく定義の仕事** — 便 77 F77-3.6 が要求してから未起草のまま。E1-4 を「4 点セットの 1 点」として台帳に置く以上、statement が単一の圏論的言明になっていないのは弱い。
4. **地図の P2 行と帯 0/帯 3 の空白欄の更新** — 先行文書(裁定 226)と本稿で E1 の欄は埋まった。ただし地図は「215〜234 未反映」(裁定 239 注記)の状態なので、一括補完の際に併せて処理されたい。
5. **Sol 検分の未実施** — 裁定 226 の工程は「便 87 系で Sol 検分(E1 は中間峰の Sol ゲート対象)」としたが、便 86/87/88 に E1 の検分は無い(grep 済)。**先行文書も本稿も Sol 未監査**である。本稿の Sol 監査では §2–§3 の証明(とくに補題 E1-S3/S4・定理 E1-3 の (b) 稠密性)と【E1-GAP-6】の判定を優先されたい。
6. §7.1 の Lean パック化(I-27 の再定義)。
7. **C5(宇宙登録)と【C21-d】(C1′ schema 化)は依然未着手** — 地図 帯 0 前線の「次 =【C21-d】C1′ schema 化+C5 登録」がそのまま残っている。
