# I8 — 比較橋 (5′) の $n=9$ instance(+ I10・I9 の記載差分)v3

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 117)・**v2 = 裁定 119(算術ゲートの (D) 解消)**・**v3 = 裁定 124(便 76 F1.6 の二修理)**。状態 = candidate。commit していない。**

> **【v2 ⇒ v3 の変更は 2 点のみ】** — 便 76 F1.6(**条件付き PASS**)が指摘した **(i) P1 表の型の取り残し $\tilde\chi\circ\mathrm{Ih}=\chi_{18}$ → $\tilde\chi_{36}\circ\mathrm{Ih}=\chi_{36}$**、**(ii) 残件の記帳し直し**(P1/P2 を universal theorem + $n=9$ instance へ移し、**窓固有凍結は P4・P6・P7・P8-value の 4 件**)。**§2 の (5′_b)@9・§3.4 の和解・§5 の I10・§6 の I9 は不変。**
> **委嘱は (i) のみを指していたが、(ii) も同便 F1.6 の明示要求であり、放置すると「live 本文が旧記帳を主張し続ける」型になるため併せて直した。**
...
> **【v3・便 76 F1.6 (ii) で記帳し直し】** **P1(W2 の instance)と P2((W2)-fam)は数学供給であり、**universal theorem + $n=9$ instance** の側へ移る**(便 76 で (W2)-fam が供給され、P2-b 原典照合も決着した)。**ゆえに「窓固有の凍結」として残るのは P4・P6・P7・P8-value の 4 件**である。**旧記帳「族 3 + 固有 5」は使わない。** P1/P2 の行は**由来の記録**として残すが、**固有残件には数えない。**

| # | 項目 | 何が窓固有か | 現状 |
|---|---|---|---|
| **P1** | **(W2)** の $n=9$ instance | $2M$ 水準の完全列と $\tilde\chi_{36}\circ\mathrm{Ih}=\chi_{36}$【**v3・便 76 F1.6 (i) で型修正**—v2 は $\chi_{18}$ と書いていた・**自認**】 | **universal theorem 側へ移管**(§3.2 注記) |
| **P2** | **(W2)-fam** の形式証明 + 便 29 (6.1) 原文照合 | 便 76 で **(W2)-fam が供給**・**P2-b 原典照合も同便で決着** | **universal theorem 側へ移管**(§3.2 注記) |
...
### 4.1 universal theorem / 族条項が供給する(**窓固有凍結には数えない**)— **3 項**

| # | 項目 | 供給文書案 | 根拠 |
|---|---|---|---|
| **P1** | (W2) の $n=9$ instance | **(W2)-fam(便 76 で供給済)** + $n=9$ instance 化 | **供給済**。$2M$ 水準で $\tilde\chi_{36}\circ\mathrm{Ih}=\chi_{36}$ |
...
### 4.2 $n=9$ 固有の凍結が要る — **4 項**【**v3 で 5 → 4**・便 76 F1.6 (ii)】
...
> **⇒ 記帳(v3)**: **窓固有凍結 = P4・P6・P7・P8-value の 4 件。** P1・P2 は **universal theorem + instance** 側(便 76 で供給)、P3 = (W5) は既に閉、P5 = (E-iv) は **N76-4.1 で普遍 template 採用済**(適用宣言のみ)。
> **⇒ 最短経路**: **4 件はいずれも手続き**(object identity の凍結と value commitment)であり、**数学の未決は残っていない**。**その 4 件を 1 枚で束縛するのが `docs/notes/k9_window_instance_v1.md`(案 α の thin record)である。**
...
3. **窓固有凍結は 4 件**(P4・P6・P7・P8-value)—P1・P2 は universal theorem + instance 側・P3=(W5) は閉・P5=(E-iv) は普遍 template 採用済(【v3 で記帳し直し・便 76 F1.6 (ii)】)。
4. **I10 は family Rule 1 §1.3 の族条項 + 適用宣言 1 行**(量と名前の型分離)。
5. **I9 は記載差分 3 欄のみ**(数学は不変)。
6. **算術ゲートは (D) で解消**($2M$ 水準・$e=n=9$・$K^{(5)}$ 遡及懸念も消滅)。**(W2)-fam は便 76 で供給され、P2-b 原典照合も決着した** — **数学の未決は残っていない**。**残るのは窓固有凍結 4 件(P4・P6・P7・P8-value)の手続きのみ**で、これは thin window-instance record が束縛する。**ただし record 受領前に I8 を閉じたと呼ばない**(N76-4.2 の二段 status)。
...
- **v2 の型の取り残し(自認)**: P1 表が $\tilde\chi\circ\mathrm{Ih}=\chi_{18}$ のままだった(§3.4 は既に $36$ で正しかったので**局所的な型の取り残し**)。**v3 で $\tilde\chi_{36}\circ\mathrm{Ih}=\chi_{36}$ へ修正。**
- **UNKNOWN として残るもの**: **窓固有 object の同一性**(P4・P6・P7)と **value commitment**(P8-value)。**普遍定理はこれらを供給しない**(便 76 F1.6「GLOBAL 八行は既存定理の供給であり、窓固有 object の同一性までは供給しない」)。**位数の一致は必要条件であって object identity ではない。**
