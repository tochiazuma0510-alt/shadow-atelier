...
**状態札: `design only / paper-proof candidate / Sol 未監査 / GAP 実行ゼロ / 実測ゼロ / census 生成ゼロ / 封印非接触 / 発火請求なし`**
...
| # | 内容 |
|---|---|
| **①** ★★★ | **MIRROR-SHADOW-B4 は成立する(紙)**。$(m,f)=(-1,1)$ は 2008 の hexagon 2 本を **$B_3$ 内の恒等式**として満たし(§2.2 で逐語検算)、pentagon は $f=1$ で自明。⟹ **全窓で GT-shadow**、$\ker T^{PB_4}_{-1,1}=\iota(\widetilde N)$。**ただし 1 点だけ PIN 待ち**(§2.4 【PIN-B4-1】) |
...
1. **【PIN-B4-1】を閉じる**(数学者・紙 5 分・機械不要)⟹ 補題 MIRROR-SHADOW-B4 が確定。
2. **【GAP-B4-2】を閉じる**(実装係・$x_{jk}$ 上 6 行)⟹ 補題 FIXED-B4 が無条件化 ⟹ 「現用窓では鏡映線が空」が確定 ⟹ **新窓層の必要性が正式な動機になる**。
3. **R0(240)の timing probe のみ**を便 112 認可後に走らせ、P-B4-4 を判定。**そこで初めて帯を決める**(先に帯を固定して走らせない)。
