# selected cycle materializer v1(E・chord 12 → rank 1386)増分 CV-9 判読(falsifier 逐語・裁定 2143 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 981a0bef53fa4bbd・保存ファイル全体)を逐語転記(2026-09-06)。

**工房裁定(2143)**: CV-9 = **同一対象**(第三実装で Ω 語実体化の全段を生バイトから再現: witness(chord 12・basis [2,3,4,6,11]・係数 (2,0,2,2,2))→ 6 基本閉路の Fox 合成 = raw-chain.bin バイト一致(support 138)・normalizer 語 r_x/r_y/c_x/c_y の 4/4 sha 一致・raw-root 3,338 文字のストリーム sha 一致・SLP 45 ノード全数再現・lower-zero 96,776 trit 全 0・新 target + 1·normalized の sha = 親 rank 1385 の pinned target 剰余・λ の全性質と row_pairings = sha256(0x00×1386)・封 19/19・manifest 27 exact roster・TCB 16+データ pin 3 で mismatch 0)。工房格 = **checker PASS(交差辺なし・新 pair にクローンなし(最大 0.8424)・checker が _seed_* 群を捨てて psl 積/affine/Fox を自前実装 = 改善)・cross-checked は限定 7 条つき** — (i) 射程 = 「rank 1385 → 1386 の 1 pivot(chord 12 の witness 由来・依然 Separator・target 剰余変化)」の有限事実・MEMBER/NONMEMBER いずれでもない (ii) **空虚性 F-cy-1: v547 三因子修理のうち効いたのは 1 因子のみ**(w = (ε_x, ε_y, ω) = (6,0,0) ⟹ repair-y と中心項 repair-central([r_x,r_y]^g)は指数 0 = 空語・ω の二次項も 45 ノード全部で 0・中心項は本番で一度も試されていない)(iii) **紙の齟齬 F-cy-3**: v548 §5 は [r_x,r_y]^ω(w)・v547 (4.2) は [r_x,r_y]^g — 実装は v547・本 run は ω = 0 で区別不能(ω ≠ 0 の witness の前に決着要)(iv) **独立性 F-cy-4a**: read_task712_envelope 1.0000(1,512 token バイト同一)が両系 load-bearing で今回は物理行を作る B 表の復号に効く・_load_words 1.0000・_SeedContext 0.9684・新規 empty_lift/zero_source 0.85 (v) q_a は character 0 のみ非零(2745,0,0,0)・κ は tag 0 のみ台(tag 1〜5 恒等零・aux 8 スロット零)(vi) λ·ρ₂ は DERIVED 前提(2117 規約 17)(vii) B 行列・旧 1385 行・P1 cache 由来の 5,335 lift は二系統一致のみ。軽微: F-cy-2 語長上界は chord 枝で恒真(3338 = 3338)/ F-cy-5 producer は 8,059 行全部の lead 正規化を検査・checker は係数 0 の 2,724 行を skip(checker のみの検査 2 本)/ F-cy-6 producer L768 の normalized_pair == [0,0] は冗長。**前回 2138 からの改善(裁定 2144 で訂正)**: 本走で実行された full selftest は **E checker(Task 966)の 3 件**であって、**oracle checker(Task 968/v2)の full selftest は未実行のまま = 2138 F-sc-3 は開いたまま**(次の実 v2 使用 = Task 972 で一回の義務継続)。「24 塊すべて非零・四 B 非自明・α 1/2・全 4 owner・shared-aux」は **E の raw source** についての実測であり、oracle の F-sc-2(oracle 側の非零係数塊 6/24)を閉じるものではない。GRADE2 NOT_DECIDED・verified=false。
