# seed30 materializer v1 run CV-9 判読(falsifier 逐語・裁定 2105 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 c868b8db6857bb39)を逐語転記(2026-09-05)。

**工房裁定(2105)**: CV-9 = **同一対象**(19 規約一致・限定 4 条)。工房格 = **checker PASS(本周回の差分は両側打ち直し tok 0.12〜0.58・系統は完全分離(producer = d972_* pinned import / checker = check_* import・交差辺なし)・候補全 9 ファイルをバイト一致で相互束縛・ただし v15 seed 核は 0.97〜0.98 クローン持ち越し)・cross-checked は限定つき** — (i) 射程 = seed 30/char 0/空 actor の 1 周回のみ(actor origin 44..32279 未走査)(ii) rank 1354 導出・884 旧 reduction・親行の三角性は前提(親 run checker がバイト比較で確立)(iii) λ の全 1355 行直交性は未スイープ(F1・逆代入中の逐行検査のみ・三角性前提に依存)(iv) q は親保存値への回帰照合。GRADE2 NOT_DECIDED・NONMEMBER ではない・verified=false。**CEGAR 再走の判読規律**: 入力 pin+終端受領証だけでは不足(周回固有 literal 38 箇所で両ファイルが実質新版)→ 増分判読 = ①19 規約表の機械 diff ②import 交差辺 grep ③新差分の類似度 ④F1 閉鎖 ⑤入力 pin/終端受領証。①〜④無変化なら以後⑤のみ。
