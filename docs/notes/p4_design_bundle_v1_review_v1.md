...
> 3. ★ **陰性の含意は札の想定より広い**: 「$M_5,p=11$ で閉」ではなく **CMP-PHI6 により外部標数($p\nmid\lvert G\rvert$)の全窓で「位数 6 以外の 1 次元指標は $V_p(N)$ に現れない」**〔4R M-6〕 ⟹ **この関手上の表現読み(1 次元)の道は当該域で閉じる**(EXHAUST: 1 次元指標に限る・高次元既約成分は OPEN・**旧 (B) ラベルは廃止**〔4R m-2〕)。
