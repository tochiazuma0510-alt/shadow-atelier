...
# 9. 便 101 検収 erratum(裁定 428)— **IF-FIRST の停止規則を S-7′ へ置換**・NW-P8 の gate 化

> **形式**: **追記型。§0–§8 と付録 A は一切改変しない。** 本節は Sol 便 101 返信(`sol/sol_reply_101_math28.md`)の **W101-1.1 / W101-1.2 / P101-1** と**裁定 428** に対する **current erratum** であり、下表の項目については**本節が effective source**(CV-10)。
> 起草: 数学者(Claude / Opus 5)・2026-08-04。$K^{(5)}$ 非接触・封印 payload 非接触・**新しい実測ゼロ**(本節は事前登録の規律だけを扱う)。

## 9.0 効力表(どの主張が本節で置き換わるか)

| # | 対象 | 旧(§8 まで) | ★ 本節後 |
|---|---|---|---|
| **E-1** | §8.7.7 停止規則 **S-7** | 「NW-P2 の等号が破れたら、窓の定義を変えずに**次元の実測値で全予言を書き直す**」 | ★ **撤回**。**S-7′**(§9.1)が IF-FIRST 契約の正本 |
| **E-2** | §8.7.7 予言表 **NW-P8** | 予言表の 1 行(停止規則なし) | ★ **「較正予想」と明示**し、**停止規則 S-8**(§9.3)を付す |
| **E-3** | §8.11 の検算 script | `hs_prop7_dumhex_check.py`(冒頭コメント (3) が旧式) | ★ **v2 が effective source**(§9.5・v1 は記録として不改変) |
| **E-4** | §8.8 格付け表の「Sol 未監査」欄 | 補題 NW-1a/1b・DUM-HEX・DUM-1/p・PREC-1 は Sol 未監査 | ★ **便 101 F101-1.1/1.2/1.4 で PASS**(§9.6) |

**本節が変えないもの**: NW-P1〜NW-P8 の**値**・窓 $\mathbf N,\mathbf N_0$ の定義・補題 NW-1a/1b・定義 DUM-FIN・補題 DUM-HEX・定理 DUM-1/p・系 H8′ の適用(SURJ 自動 + 識別力ゼロ)・停止規則 S-1〜S-6。

---

## 9.1 ★ **S-7′**(IF-FIRST 契約の正本・便 101 W101-1.1 逐語)
...
**工房側の運用形(4 項・上の条文の操作化)**:

1. **停止は即時**。不一致を検出した時点で残りの測定(NW-P3〜P8)へ進まない。**部分結果は破棄せず保存**する(cert に `verdict: PREREGISTRATION_FALSIFIED` と、どの欄がどう外れたかを記録)。
2. **同一 run 内での予言の書き換えを禁止**。実測値を「新しい予言」として cert に書き直す・予言欄を実測欄で上書きする・「$\le$ に緩める」等の操作はすべて禁止。
3. **二分の別検分**: (α) **構成 bug**(pc presentation の生成・class/exponent の設定・語の評価・$\gamma_k$ の取り方)と (β) **数学的に等号が偽**。この二つは**同じ run の中では区別できない**ので、別々の検分として起票する。
4. **続きは別 version の事前登録から**。新登録の冒頭に「**旧予言 NW-P2 の当該欄が外れた**」という事実と旧登録の digest を明記する(CV-10 の effective source chain)。

## 9.2 なぜ旧 S-7 が壊れていたか(起草者の理解 — 自己批判として記録)

旧 S-7 は「**窓の定義を変えずに**」と書くことで「宇宙を後から変えない」規律は守ったつもりだった。**しかし事前登録が守るのは宇宙だけではない — 予言もである。** 予言を実測で書き換えてよいなら、

$$\text{「予言 } \Leftrightarrow \text{ 実測」の照合表は恒等的に PASS になる}$$

ので、IF-FIRST は**反証不能な儀式**に退化する。これは §2 の設計思想(「検出力を事前に見積もる = 測る前に何が出るはずかを固定する」)と正面から衝突する。**Sol の指摘は正しく、旧 S-7 は撤回する。**

★ **不一致の解釈は欄ごとに違う**(停止義務は同じだが、別検分の初動が違う — 起草者の追加):

| NW-P2 の欄 | 紙で証明されているか | 不一致が意味しうること |
|---|---|---|
| $N_{\rm ord}=7$ | ★ **証明済**(補題 NW-1b (4)) | (α) 構成 bug **または** 紙の証明の誤り。**(β) の「数学予言が偽」ではない** |
| $\lvert\mathcal X_{\mathbf N}\rvert=6$ | ★ **証明済**(同上 + $\gcd(2m+1,7)=1$ の勘定) | 同上 |
| $\lvert P\rvert=7^8$ | **上界 $\le7^8$ のみ**(Witt 公式) | (α) bug または (β) **等号が偽**(= $\gamma_5F_2\,F_2^{\,7}$ の外に余分な関係が落ちる)。**両方ありうる** |
| $\lvert[P,P]\rvert=7^6$ | **上界のみ** | 同上 |

⟹ **S-7′ の停止は 4 欄すべてに掛かる**が、停止後の検分は上の 2 群で初動が異なる(上 2 行が外れたら**まず紙を疑う**)。この区別は停止規則を**弱めない**(どちらでも即停止)。

> ⚠ **等号を紙で証明しない選択は意図的**である(§8.7.8): 制限 Burnside / Lazard 対応の一般論を**呼ばずに**機械へ回した。S-7′ はその選択の**代償を正しく払う規律**であり、「上界だけ紙・等号は予言」という格付けと整合する。

## 9.3 ★ NW-P8 は「較正予想」— 停止規則 **S-8**(便 101 W101-1.1 後段)

**格の明示**: **NW-P8 は定理ではなく較正予想である。** 紙で言えているのは補題 NW-1a/1b までで、すなわち

- $\mathbf N$ では $\bar c=1$(中心が窓に入る)⟹ full (3.3)(3.4) の $c$-成分は**恒等的に消える**、
- $\mathbf N_0$ では $\operatorname{ord}(\bar c)=p$ ⟹ $c$-成分は $\langle\bar c\rangle\cong C_p$ の中に**生き残りうる**

の 2 点である。**「実際に生き残って hexagon の判定を変える $m$ が存在する」ことは証明していない**(defect の $c$-成分が $m$ の関数として非零になる、という計算を紙でしていない)。⟹ **【HSP-GAP-5】**(新規・小)として登録する: *$\mathbf N_0$ における (3.3)(3.4) の $c$-成分の $m$-依存性の閉形*。(**番号は grep 済** — 【HSP-GAP-4】は §7.2 の「深さ 5 以上は未計算」で使用中。**再利用しない**。)

> ### 停止規則 S-8(**NW-P8 を gate として走らせる場合**)
> $m$ を $\mathcal X_{\mathbf N}$ 全体にわたって掃いたうえで、$\mathbf N$ と $\mathbf N_0$ の hexagon 判定に**不一致が 1 件も出なかった**場合は `CALIBRATION_FAILED / INTEGRITY_STOP` として停止する。**後から期待値を弱めない。**

**禁止される「弱め方」の具体形**(先回りして塞ぐ):

1. 期待を「不一致が出る**かもしれない**」へ書き換える(NW-P8 を予言から観察へ格下げする)。
2. 掃く $m$ の範囲を後から狭める/広げる($\mathcal X_{\mathbf N}$ の 6 元は事前登録済み)。
3. control 窓を $\mathbf N_0$ 以外へ差し替える(窓対は NW-P1 で凍結)。
4. 「$c$-成分は効かないのが正しい」と**事後に**主張して PASS 扱いにする(それを主張したいなら **S-8 で停止 → 別 version 事前登録**が正路)。

**なぜ停止が正しいか**: NW-P8 は「control 窓が実際に control として働く」ことの検査である。不一致ゼロなら、$\mathbf N_0$ は $\mathbf N$ と**判定を共有する冗長な窓**であり、較正スイート項目 7($c\ne1$ control)は**識別力ゼロ**になる。§8.7.6 で SURJ について起きたのと**同じ型の事故**(識別力ゼロの検査を「通った」と数える)を、今度は事前に止める。

## 9.4 cert に書く停止規則ブロック(**P101-1 (5) の履行形**)

発火条件 2($p=7,e=1$ 構造確認)の cert は、次の 3 規則を**逐語で**含むこと。**期待値をコード生成に使わない**(P101-1 (4))という条項と併せて 1 ブロックにする。
...
"stop_rules": {
  ...
  "S-8":  { "trigger": "X_N 全掃引で N と N_0 の hexagon 判定の不一致が 0 件",
            ...
            "note":    "NW-P8 は較正予想。後から期待値を弱めない" }
},
"prediction_source": { "frozen_at": "<本ノート §8.7.7 の digest>",
                       "codegen_uses_expected_values": false }
```

**認可範囲は P101-1 の 6 条が正本**(窓固定・pc presentation からの測定 4 項・$h_4\ne1$ の直接判定・$\prod_{i=0}^4\rho^i(j(h_4))\ne1$ の直接判定・停止規則の cert 明記・single-lane candidate 扱い)。本節はそのうち**停止規則の中身だけ**を確定する。**shadow 全掃引・$p=11,13$・FV-WALL 解凍は未認可**(P101-1 末)。

## 9.5 検算 script v2(**便 101 W101-1.2 の一行 erratum の履行**)

| 項目 | 値 |
|---|---|
| **新規** | `search/probe/hsp7_v1/hs_prop7_dumhex_check_v2.py` |
| **SHA-256** | `f41af97a92b9a121ba665dc0e270448bfe306236ba83153e419c213ba45dfcff` |
| **旧(不改変)** | `search/probe/hsp7_v1/hs_prop7_dumhex_check.py` = `7b1c7e067a13a24de2200b60d226a8c58fe7ada44567753365827629fad8df5f` |
| **差分** | ★ **冒頭コメント (3) のみ**。`import` 以下の計算コードは v1 と**バイト同一**(機械照合済) |
| **実走** | **14 検査・`FAILS = 0`**。v1 との**標準出力はバイト同一** |

**修正内容**: 旧コメント (3) は $(1+\tau_*+\tau_*^2)$ の像を $(4\alpha-\beta)(v_1{+}v_2{+}v_3)$ と書いていたが、**一般形は $(2\alpha-\beta+2\gamma)$**(精確化 **PREC-1**・便 101 F101-1.4 で PASS)。$(4\alpha-\beta)$ は **(3.10)-locus $\alpha=\gamma$ 上での還元形**であって一般形ではない。$\mathfrak h_4=(1,4,1)$ は locus 上にあるので**両式とも 0 を与え**、v1 の数値結果(検査 11/12/13 を含む)は無傷 — **文書 defect のみ**であった。v2 のコメントはこの関係(一般形 / locus 上の還元形)を明記している。

## 9.6 格付けの更新(§8.8 への差分のみ・便 101 F101 判定の反映)

| 主張 | 便 101 後の格 |
|---|---|
| **補題 NW-1a / NW-1b** | ★ **paper-proof**(F101-1.1 PASS)。**【HSP-GAP-1】は CLOSED(紙)** |
| **定義 DUM-FIN / 補題 DUM-HEX** | ★ **paper-proof**(F101-1.2 PASS) |
| **定理 DUM-1/p** | **paper-proof(NW-P5 相対)**(F101-1.2 PASS・射程は明示 family に限定) |
| **精確化 PREC-1 / R-5** | ★ **PASS**(F101-1.4)。**定理 D4-POWER (a) の結論は不変** |
| SURJ(系 H8′ の適用) | ★ **理由づけの訂正を Sol が全面採択**(F101-1.3 — Sol の旧理由は撤回)。**本窓族で SURJ は識別力ゼロ** |
| NW-P2 の等号・NW-P3〜P8 | **予言のまま**(紙は上界のみ)。**verified は 0 件・cross-checked も 0 件**(CV-9 未実施) |

> ★ **格の境界を再掲**: 上の PASS は **Sol の紙監査**であって Lean の verified ではなく、`hs_prop7_dumhex_check_v2.py` は依然 **single lane**(起草者が書いた 1 系統)である。**cross-checked と書かない。**

## 9.7 司令塔への申し送り(本節から出るもの)

1. ★ **S-7′ / S-8 を発注仕様(条件 2 の起票)へそのまま貼る**(§9.4 のブロック)。**停止規則を cert に書かない起票は差し戻し**(P101-1 (5))。
2. ★ **【HSP-GAP-5】(新規・小・番号 grep 済)**: $\mathbf N_0$ における (3.3)(3.4) の $c$-成分の $m$-依存性の閉形。**NW-P8 を「予想」から「定理」へ上げたければここを埋める** — ただし**発火条件 2 の前に埋める必要はない**(較正としては予想のままで機能する)。
3. **§8.10-4 の申し送り(既存 script の `psi4`/`sigma3` 改名)は未履行のまま**(台帳【CL-10】)。本 v2 は改名対象外(v1 の時点で $\mathfrak h_3/\mathfrak h_4$ 記法)。
4. 便 101 の NOTE(W101-4.1「1210 検査」の provenance 束縛)は EP 側の案件で、本ノートの射程外。
