...
**裁定 2177 追補(Astra errata 2 件)**: (a) 本判読の §8(3)/§7「aux も新 selftest に無い」は誤り — P v2 L3217 の auxiliary-only と C v2 L2476 の first-auxiliary/second-auxiliary は新第二群の実 PASS 経路(本番 aux 未発火と DEPENDENT の本番/新 selftest 未通過は維持)。(b) §4.2/§10 の物理行比較は 6 本抽出であって「冒頭 32 本の全 bytes」ではない — root が旧 k32/新 k64 の rows/000000..000031/physical-normalized.bin を全 32 組・全 bytes/SHA で追加照合し 32/32 一致(工房の「前半 32 本は物理行までバイト同一」はこの root 照合により成立)。a(n) の定義 = 同 λ/同初期 span/同候補順の前置長に対する量。
