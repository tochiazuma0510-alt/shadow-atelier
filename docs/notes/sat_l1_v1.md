...
| `search/probe/wac_v1/sat_l1_probe6.g` | (§10.5 用・実行済) | (A) $\Xi(f_z)=a_1za_1$ の全 $z$ 照合(4 窓)/ (B) $\lvert\Delta(2,3,m)\rvert$、$m=3,4,5$ | (A) 4 窓全 true / (B) $12,24,\mathbf{60}$($m\ge6$ は列挙非停止 = 無限性は古典事実として引用) |
