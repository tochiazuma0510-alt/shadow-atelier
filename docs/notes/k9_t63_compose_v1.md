...
# 【v1.2 追記】GAP-1 を **除外定理の適用確認**へ差し替え — ★ **貫通する(追加補題不要)**

**日付**: 2026-08-12 / **委嘱**: 裁定 919(研究者指摘起点)/ **方式**: additive addendum(**本文・v1.1 追記とも不改変**)

## B.0 ⚠ v1.1 の論法の弱さ(自己申告)

v1.1 A.1 は「**t63 の caveat 表に C1′ が現れない ⟹ 依存なし**」という **不在論法**だった。これは **EXHAUST 規約違反の疑い**がある — 起草者が書かなかったことは、依存しないことの証明ではない。
$$\boxed{\ \textbf{裁定 919 の切替(「管轄問題」→「}\textbf{除外定理の適用確認}\textbf{」)は正しい。以下でやり直す。}\ }$$

## B.1 除外定理(LEDGER 一次確認・逐語)

| # | 行 | 逐語 |
|---|---|---|
| 1 | **L1065** | 「**C1′ は族 gating から外れうる**([α] は **exact 符号のみに効き類と位数に効かない**)」 |
| 2 | **L1140**(便 95 系検収) | 「**C1′ は類/位数 gate から除外可・M2 は不可**」 |
| 3 | **L1166** | ★★ 「**補題 LIFT**: α̃ は $y^n=h$ の整数指数そのもの・**曖昧さ = 一様化元取替と同一** ⟹ 選択肢 A(exact 符号)採用・**C1′ を sel/adm 分割**」 |
| 4 | **L1860** | 「【**C1′-any**】(**十分方向は窓の同定不要**)」「**C1′(7)・C5 は系の適用 gate(G-1/G-2)でのみ要る** — Sol F91-5.3 の定理/gate 分離の族版として固定」 |

★ **除外定理の内容**: $[\alpha]$ の選択は $u$ の **exact 符号**にのみ効き、**類と位数には効かない**。

## B.2 ★★★ 塔側への貫通の証明(**本追記の中核**)

司令塔の問い: 「除外の根拠は **E1 側測定の $u_q$ についての定理**なので、**塔側の対象($a_9$)への適用可否**を明示せよ」。

> ### 命題 **TOWER-α-INV**(candidate・証明つき)
> **(6.3-cls) が構成する $a_9$ の類と位数は $[\alpha]$ の取替で不変**。従って除外定理は塔経路にそのまま適用でき、$\operatorname{ord}(a_9)=9$ の主張に **C1′@$n=9$ は不要**である。
>
> **証明**(2 段・既存資産の接続のみ)
> **第 1 段(曖昧さの同定)**: **補題 LIFT**(L1166)より、$\tilde\alpha$ の曖昧さは **一様化元(uniformizer)の取替と同一**である。
> **第 2 段(t63 §2.2 が既にその不変性を証明している)**: t63 memo §2.2 の逐語 —
> > **uniformizer 非依存の確認**: $s_d\mapsto a's_d(1+\cdots)$ で $u_d\mapsto u_d a'^{-2d}$、$w\mapsto a'w$ ⇒ **(2.3) 不変**。$s_n\mapsto as_n(1+\cdots)$ で $u_n\mapsto u_n a^{-2n}$、$w\mapsto wa^{-n/d}$ ⇒ **不変** ✓。**したがって (T) は class の等式として意味をもつ。**
>
> ⟹ **(T)**(および同値な **(6.3-cls)**)は**上段 $s_n$・下段 $s_d$ の両側**で一様化元取替に対し不変。第 1 段よりこの取替は $[\alpha]$ の取替を**含む** ⟹ **(6.3-cls) は $[\alpha]$ 非依存**。
> さらに $a_9=[u_9^{-1}]_{18}$ は**類**であり、$\operatorname{ord}(a_9)$ は**類の関数** ⟹ **位数も $[\alpha]$ 非依存**。∎

$$\boxed{\ \Longrightarrow\ \textbf{貫通する。}\textbf{追加補題は不要}\textbf{(必要な不変性は t63 §2.2 に既在)}\ }$$

★ **なぜ「ついで」でなく本質か**: t63 §2.2 の不変性検証は「(T) が class の等式として意味をもつ」ことを言うために書かれたもので、**$[\alpha]$ を意識して書かれてはいない**。補題 LIFT が「$\tilde\alpha$ の曖昧さ = 一様化元取替」と同定したことで、**同じ計算がそのまま $[\alpha]$ 不変性の証明になる**。⟹ **二つの既在資産の接続が答えであり、新しい数学は要らない。**

## B.3 三重の裏づけ(独立な 3 経路が同じ結論)

| 経路 | 内容 | 結論 |
|---|---|---|
| **(i) 除外定理の直接適用**(B.2) | 補題 LIFT + t63 §2.2 の一様化元不変性 | $\operatorname{ord}(a_9)$ は $[\alpha]$ 非依存 ✔ |
| **(ii) 十分方向の窓非同定**(L1860【C1′-any】) | 「**十分方向は窓の同定不要**」 | $\operatorname{ord}(a_9)=9$ は**十分方向の主張** ⟹ C1′ 不要 ✔ |
| **(iii) 定理/gate 分離**(L1860・Sol F91-5.3 族版) | 「C1′・C5 は**系の適用 gate(G-1/G-2)でのみ**要る」 | T63-P1 は**定理側**であって系の適用ではない ⟹ C1′ 不要 ✔ |
| (iv) 構成的窓(v1.1 A.1) | HF-2 が $H^{\rm fun}$ 窓間の射として cover を構成 | ★ **補強材料**(不在論法から降格・単独では根拠にしない) |

## B.4 ⚠★ 併せて拾った NAME-COLLIDE 警告(L1860 逐語)

> 補題 LIFT は主定理の前件ではない(Kummer 系の well-posed 化 1 箇所のみ・**§V.2.1 の (S3) と E1 の (S3) は同名異物** ⚠)

★ **私の判読は E1 側**(`E1_gt_odd_dih_canonical_v1.md` §5.1 の (S1)–(S4))を使っており、**FAM-U-ASM §V.2.1 の (S3) とは別物**。RECON v2.1 §1 の対応表も E1 側で統一されている ⟹ **混同なし** ✔
⚠ ただし **今後 (S3) を書くときは必ず「E1 の (S3)」と出所を付ける**(NAME-COLLIDE 規約)。⟹ v1.4.8 queue へ。
★ **補題 LIFT は主定理の前件ではない**ことも記録 — B.2 では LIFT を「曖昧さの同定」にのみ使い、**前件として持ち込んでいない** ✔

## B.5 ⟹ COMPOSE-GAP-1 の最終判定(v1.1 A.6 を**強化**)

$$\boxed{\ \textbf{【COMPOSE-GAP-1】= }\textbf{閉鎖}\ \textbf{— 根拠は「不在」ではなく}\textbf{除外定理の適用が貫通すること}\ \textbf{(命題 TOWER-α-INV)}\ }$$

⚠ **格は不変**: `paper-proof candidate / framework-conditional / 単系統 / Sol 未監査 / Lean 未`。**W3-24 の天井を超えない。**
⚠ **【COMPOSE-GAP-2】(t63 §4 の破れ方 B2 指数ズレ / B3 向きズレ の幾何追検算)は依然 OPEN**(v1.1 A.5 留保 2)。
