# rank1355 root seed scalars v1(CEGAR 2 周回目)増分 CV-9 判読(falsifier 逐語・裁定 2110 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 3eef80429a8e377f・保存ファイル全体)を逐語転記(2026-09-05)。

**工房裁定(2110)**: CV-9 = **同一対象**(限定 3 条・前回 (iii) は F1 閉鎖で消滅)。工房格 = **checker PASS(同一著者系統・本周回の差分は両側打ち直し tok 0.11〜0.39・系統完全分離(交差辺なし・4 系統 pin は 2096/2105 と同一 sha)・候補全 20 ファイルをバイト一致で相互束縛・v15 seed 核と projector は 0.95〜1.00 クローン持ち越し)・cross-checked は限定つき** — (i) 射程 = character 0 の seed scalar 44 個(残り 3 character は q = B*λ = 0 ゆえ 132 個は構造的零・actor origin と orbit は未走査)(ii) rank1354 導出・旧 reduction 884・target-remainder ≡ ρ₂ mod span は前提(本 run は ρ₂ を読まない)(iii) **λ の全 1355 行直交性は両側本番経路でスイープ済(F1 閉鎖・三角性前提に非依存)**(iv) raw seed 行の凍結 pin は actual_pin=False で外れ(falsifier が外部照合で今回のみ閉鎖)。主張 = 「rank1355 に対し走査順最初の非零 root seed scalar が character 0・seed 34 で 1(非零 34→1, 35→2, 36→2・seed 30 は 0 に落ちた)」という有限事実。GRADE2 NOT_DECIDED・verified=false。**次周回以降の判読規律(改訂)**: ①規約表の機械 diff は毎回必須(周回ごとに実行/前提が入れ替わる)・②③は系統 pin の sha が同じなら省略(定数差し替え版であることの diff 1 本)・④は恒久閉鎖(⑤に「old_state_rows_checked + new_pivot_rows_checked = rank」を格上げ)・⑤入力 pin/終端受領証+seed2 行 sha の外部照合 1 行。
