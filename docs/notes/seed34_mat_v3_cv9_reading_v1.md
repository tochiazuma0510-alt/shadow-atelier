# seed34 materializer v3(CEGAR 3 周回目)増分 CV-9 判読(falsifier 逐語・裁定 2117 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 4c67c069dff4bceb・保存ファイル全体)を逐語転記(2026-09-05)。

**工房裁定(2117)**: CV-9 = **同一対象**(限定 4 条)。規約表 diff(seed30 mat v1 → v3)= 16 保持・3 強化(「最初の違反」を両側 176 レコード全走査+rolling 再計算に = 2105 F2 閉鎖/λ の最終全行スイープ = 2105 F1 が materializer 系でも閉鎖/負のカナリア新設 = 2110 R1-3 閉鎖)・**1 弱化(λ·ρ₂ = 1 が実測から前提へ: v3 は ρ₂ を stage せず λ·(親 target 剰余) = 1 のみ実測・cert の lambda_rho2 は accepted-parent-target-derivation 経由)**・1 pin 移行。工房格 = **checker PASS(本周回差分は両側打ち直し cross-side 0.11〜0.58・系統分離・候補全 9 ファイル+manifest をバイト相互束縛・v15 seed 核 0.97〜0.98 クローン持ち越し)・cross-checked は限定つき** — (i) 射程 = 走査順最初の違反(character 0・seed 34・scalar 1)の 1 周回 (ii) rank 1354 導出・884 旧 reduction・seed30 delta は前提 (iii) **λ·ρ₂ = 1 は本 run で未計算**(実測は λ·(親 target 剰余)=1 と λ ⊥ 全 1356 行)(iv) 選択 seed は pin 済み親経由の決定論的導出。主張 = 「rank 1355 に seed 34 由来の pivot 1 本(lead 1418・offer 8060)を追加して rank 1356・target 剰余は依然非零」。GRADE2 NOT_DECIDED・verified=false。**判読規律(再改訂)**: ①規約表 diff は毎回(弱化は①でしか出ない)・②③は「系統 pin 4 本同一 かつ TCB モジュール集合同一」なら省略(今回 checker TCB に 1 本増)・⑤に TCB 集合の同一性 1 行を追加・類似度は autojunk=False 併記(手法注記 F-v3-1)。
