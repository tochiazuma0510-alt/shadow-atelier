...
>
> ### 【註 2 への追記(2026-08-01・裁定 400 承認)— CV-10 流儀・**上の本文は改変していない**】
> ★ **$L$ については、その後に紙上証明が得られている。** 上の「未確認」は**本稿の射程内では正しい**(本稿は $L$ の isolated 性を**使わず**に $\widetilde\chi$ 全射を実測証明書として扱う設計であり、その設計は無傷)。**しかし現況は次のとおり**:
>
> | 窓 | isolated 性の現況 | 出所 |
> |---|---|---|
> | **$L=K^{(3)}\cap N_0$** | ★ **紙上証明 + 相互監査 PASS**(2026-07-25) | `docs/命題_中心持ち上げ_v2.md`(SHA-256 `7463e7281fe9df323ab2670517b31dcaa57befde8b1bced7c6d0d7aeece0b7e8`)**定理 A(ii)**。証明は**補題 F**((3.32) の逐語・p.12 ページ画像照合済)+ **補題 G**(verbal 部分群の多様体論法)。**Sol 便 05 で相互監査 PASS**(W23 は Sol が自己撤回) |
> | **$M_5,N_Q,M_Q,N_2,N_3,M_3,N_A,M_{A_5}$**(残り 8 窓) | **未確認のまま**(不変) | — |
>
> ### ⚠ **$\widetilde\chi$ 全射性の格はこの追記で上がらない**(最重要)
> T-A(4′) は **isolated かつ (AR)** を要求し、**(AR) は framework-conditional** である。⟹ **$L$ の $Q_L=(\mathbf Z/12)^\times$ は実測証明書のまま**でよく、§5.1 の型注記(「$\lvert Q_L\rvert=\varphi(12)$ は実測」)も**そのまま有効**である。**変わったのは「isolated 性が未確認か否か」の 1 点だけ**。
>
> **本追記の適用範囲(黙読み替え防止のため明示)**: 本稿で「$L$ の isolated 性は未確認」と読める箇所は 3 つ — **本註 2**・**§5.1 の v2 型注記**・**§8 の R-4b 行**。**3 箇所すべてに本追記が掛かる**(いずれも本文は改変していない)。**【KE-g】(§9・atlas 各窓の isolated 性の判定)は $L$ について閉じ、残り 8 窓について開いたまま。**
> **pin**: **裁定 400**(2026-08-01 札 D 検収)「kerchi_equality_v2 註 2 の『$L$ の isolated 性は未確認』は **CV-10 流儀の注記追記で修文承認**(黙って書き換えず・定理 A(ii) への誘導を置く・**$\widetilde\chi$ 全射の格は不変**と明記)」。食い違いの申告元 = `docs/notes/roof2_cv9_freeze_v1.md` §6.1(SHA-256 `94ac28568674980558439b1a2dccb36cda2841fababb834eb8c9d1f2a3845915`)。
