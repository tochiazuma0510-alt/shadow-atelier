...
起草: Claude(数学者レイヤー・Opus 5)/ v1 = 2026-07-29 / **v2 = 2026-07-29(便 79 検収・裁定 170 の修理波)/ v2.1 = 2026-07-29(裁定 173 追補: §11 $\chi$-退化窓・予想 KE-P の反証)/ **v2.2 = 2026-07-29(裁定 177・便 80 F80-2 系: §12 KE-P$'$ 撤回・idx162-s1・census・$\chi$-DEG の型修正)**
...
* **状態**: **単系統**(GAP・v5 一発走り)。**独立レーン再構成が別途進行中**につき、`第二反例` ではなく **`第二反例候補`** と呼ぶ。$L$ は cross-checked、idx162-s1 は未達。
* **希少統計の更新**: ~~「$L$ は 25 窓中 1」~~ → **「TYPE-L は v5 の 66 窓中 $\ge2$ 候補($L$ 系統とは別宇宙)」**。ただし v5 の 66 窓と atlas 25 窓は**別の母集団**なので、比率を合算してはならない。
...
**census(`wall_miner_v5_20260729.json` 全 66 行・機械抽出)**: **EQUAL 62 / TYPE-0 3 / TYPE-L 1**。非 EQUAL の全行:

| label | equality_type | $|B_3/N|$ | $|PB_3/N|$ | $N_{\rm ord}$ | $|G_N|$ | $|\ker\widetilde\chi|$ | $|[G_N,G_N]|$ | `settled_all_pass` |
|---|---|---|---|---|---|---|---|---|
...
> **【KE-j/candidate-v1(凍結)】** 条件: 「$|PB_3/N|$ が $2N_{\rm ord}$ を割らない素因子をもつ窓を $\chi$-退化の**候補順位付け** heuristic に使う」。
...
* 【KE-m・新設】**idx162-s1 の独立レーン再構成**(進行中)。二系統一致で初めて「第二反例」と呼ぶ。
