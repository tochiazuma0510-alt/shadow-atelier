...
## 【追補 erratum 続き(裁定 1171)】訂正が要る箇所の完全な名指しと、差し替え後の総括 3 行

裁定 1164 の追補は族 7 の射程だけを訂正した。**本文中で同じ過大表現を運んでいる箇所を全て名指しする**(本文は不改変・引用時は本節を優先):

| 箇所 | 現行の記述 | 訂正 |
|---|---|---|
| §3.1 表 #7 | 「$p
e3$ 係数の全窓」= 定理 OBS-VOID (i) | **自動で盲なのは $p
mid6$ のみ**。$p=2$ は $H^2(C_2,V)$ が残りうる |
| §3.1 末尾 | 「**⟹ 残る路は 1 本**」 | **2 本**: $\{p=3\wedge V|_{\langle	auangle}$ 非自由$\}$ **または** $\{p=2\wedge V|_{\langle	hetaangle}$ 非自由$\}$ |
| §3.2 L107 | 残存 = 「非分裂・非テンソル・$p=3$・非自由」 | 現況は `cofinality_ledger_draft_v1.md` §3 の表(非可換核・$p=2$ 非自由・非半単純・非分裂・多段・$S_4$ 側深化) |
| §3.3 表 最終行 | 「$p
e3$ 係数 ⛔ 定理 OBS-VOID (i)」 | ⛔ は $p
mid6$ のみ。$p=2$ は**走査候補**(実際 ESCAPE-2 = 便 136 で走った) |
| §4 総括 1 行目 | 「$p
e3$ 全係数で不発であることを**定理として**確定した」 | 「$p
mid6$ の全係数で」へ |
| §4 総括 2 行目 | 「探索空間を **1 条件**に圧縮した」 | 「**2 条件**に圧縮した」へ |

### 差し替え後の総括 3 行(引用はこちらを使う)

1. **半決定の到達深度**: A 型の有限証明書(= 分裂屋根で 324)は、**分裂屋根・$K^{(l)}$ 全族・perfect 窓・可換橋・片側自明・$\mathbf F_3$ テンソル型・$p
mid6$ 全係数**で不発であることを**定理として**確定した。加えて $p=3$(ESCAPE-28・1,099,008 行)と $p=2$(ESCAPE-2・2,268 行)の**非自由窓 2 例**でも実測で不発だった — ただしこの 2 例は**測定であって族定理ではない**。
2. **盲の地図**: 7 族の盲目性定理は、**実質 2 機構**である(裁定 1171 の検分)— **機構 A = 相互作用の不在(共通商なし ⟹ 直積)**〔族 1–4〕と **機構 B = 可換核上の $H^2$ 消滅**〔族 5–7〕。探索空間は $\{p=3\wedge	au$ 非自由$\}\cup\{p=2\wedge	heta$ 非自由$\}$ の 2 条件に圧縮されたが、**非可換核・非半単純・非分裂・多段・$S_4$ 側深化には族定理が 1 本も無い**。
3. **計器群**: compact 路(LIFT-AFF / GEN-AFF / MARK / SURJ-LIN / TWIST-2 + GEN-SUB・NORM-TWIST・A-42)により $\lvert Evert\ge10^{14}$ の窓を $d	imes d$ 線形代数で測る汎用資産が確立し、$\mathbf F_3$ と $\mathbf F_2$ の両方で稼働した。★ さらに便 137 で**検出力の事前ゲート $r_t^{m obs}=\mathrm{rank}[A_t\mid C_tZ]-\mathrm{rank}A_t$** を獲得し、判定基準が $\dim H^2$ から $r_t^{m obs}$ へ更新された(単なる $H^2$ 増量は直和分解のため紙で無効)。

### 併せて記録すべき、区切り判断への影響

§3.4 の「区切りの根拠」欄(「残る条件が 1 つまで圧縮された」)は上記により**前提が崩れている**。便 133 で宣言した「非盲全路の涸渇」は裁定 1164 で撤回済み。**区切るか続けるかの判断材料は、裁定 1171 の `cofinality_ledger_draft_v1.md` §4「cofinal まであと何が要るか」を使うこと**。
