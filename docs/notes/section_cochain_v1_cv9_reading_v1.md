# section cochain oracle v1(VIOLATION_CANDIDATE・rank 1385)増分 CV-9 判読(falsifier 逐語・裁定 2138 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 af337891b49048c2・保存ファイル全体)を逐語転記(2026-09-06)。

**工房裁定(2138)**: CV-9 = **同一対象**(紙 v548 §2–5 / v543 §3–4 の宣言・producer・checker v2 の三者が同一の有限対象を計算・falsifier の第三実装が score 653,184 値・f 108,864 値・tree 段全配列・moment 表 27×10・τ/残差/witness の d(3⁵ 全探索で一意解)をバイト単位で再現・output 44 ファイル 5,361,492 B と preserved-input 53 件が run 33975617653 と完全同一・seal 14 JSON 再計算成功)。工房格 = **checker PASS(交差辺なし・新 pair の類似度は最大 0.66 でクローンなし・4 段全再計算+exact roster+全バイト+EOF 超読で比較・非クローン錨の被覆は score/f/tree で 100 %)・cross-checked は限定 8 条つき** — (i) 射程 = 「rank 1385 の現 λ に対し section cochain oracle が VIOLATION_CANDIDATE(τ = 0 の 6 基本閉路合法組合せ・scalar 1)を返した」という有限事実・MEMBER でも NONMEMBER でもない (ii) 継承 TCB に両系統 load-bearing のバイト同一クローン(read_task712_envelope 1.0000・_load_words 1.0000・_SeedContext 0.9684 = transport 表・PSL 順序・B 復号規約は二系統一致で検出されない = F-sc-1)(iii) 空虚性: (tag,character) の非零係数塊は 24 個中 6 個のみ・B₁〜B₃ の随伴は恒等零・score の tag 3/4/5 は恒等零・κ_aux[0:8] = 0(augmentation 項未試験)・COMPLETE_ZERO_CANDIDATE 分岐と aux witness 分岐は本番未走行(F-sc-2)(iv) checker v2 の full selftest は未走行(completion run は serialization canary 15 件のみ・保存 selftest は v1 の受領証 = F-sc-3)(v) checker L188 の integer_carry_division は恒真(F-sc-4)(vi) witness は 36,343 本の failing chord の最小 edge ID 1 本・残差分布 0/1/2 = 18,090/18,083/18,260(非零率 ≈ 2/3・一様乱数と区別不能)= 結果は過剰決定で頑健だが零側の識別力は未検証(F-sc-5)(vii) κ(96,776 座標)と q_a(4×36,288)は第三実装未再計算(二系統一致のみ)(viii) 親 rank 1385 の内部は 2131 を継承。sentinel 修理 v1→v2 は 74 行 diff・A–D 算術/solver/選択規則は無変更・producer 出力の parent.u32 先頭は元から ff ff ff ff(修理は checker 再直列化のみに効く)。物理 pivot 化に要るもの: witness の 6 閉路を v542/v547 R_word で Ω 語として実体化 → 完全 P1 減算で lower-zero → G(v) を物理行として追加(1385 → 1386)。GRADE2 NOT_DECIDED・verified=false。
