...
| **750-クリーク**(唯一のサイズ $\ge3$ 繊維) | 指数 750・`[750,6]`・**5 元**・全 member $\not\le PB_3$・$c$: 1 True + 4 False | **10** | **$\ge8$(= $10-k$, $k\in\{0,1,2\}$)。系 CHARGE-750 の条件下で**ちょうど 8** | $\iota(A)=A$ 強制 + 繊維サイズ 5 > 2(§G.5.1(a)(b)) |
...
3. その残り $C_5$ は $P_5=((C_5\times C_5){:}C_5)$ の**中心そのもの** $Z_5(Q)$ であり、$\lvert Z_5(Q)\rvert=5=\lvert F\rvert$。⟹ **繊維サイズ 5 = 中心荷重の値域サイズ**という機構仮説(candidate・§G.5.1(b) の $\mathrm{Aut}$ 条件が真なら荷重 $\zeta\in\{1,g,g^2,g^3,g^4\}$ と member が 1:1)。**発案札 α の「源 0 = 中心荷重」の実物**であり、L2($c\in N$)が exotic ゼロなのは**層の定義が $\zeta\equiv1$ に凍結して源 0 を丸ごと殺しているから**、という説明が cert 水準で裏づけられた。
