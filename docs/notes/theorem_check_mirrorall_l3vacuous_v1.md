...
| **混在層** | 対の両 member で `c_in_N` が割れる | **5** | **全 5 対が exotic(確定)** | ② の特性中心論法(**$\mathrm{Aut}(B_3)$ 全体で説明不能**)。★ **10 member 全て $N\not\le PB_3$ = 窓ではない** |
| **750-クリーク** | 指数 750・`[750,6]`・5 元繊維 | 10(混在 4 を含む) | **$\ge8$ 対が exotic** | 繊維サイズ 5 > 2 = $\iota$-軌道の上限(G.2) |
| **L0 の残り** | 上記以外 | 146 − (上記) | **UNKNOWN** | 未評価。**「無い」とは言わない** |
