# J3 actual grade-two root scalar batch run 2 CV-9 判読(falsifier 逐語・裁定 2077 で正本化)
著者: falsifier(opus/max・非当事者)。司令塔が scratchpad 報告書(sha256 先頭 16 桁 07371d8c1e852522)を逐語転記(2026-09-05)。

**工房裁定(2077)**: CV-9 = **同一対象**(限定 4 条)。工房格 = **checker PASS(同一著者系統・親検証層と出力照合層は実装独立・F₃ 演算核と seed 生成核は逐語クローン)・cross-checked は限定つき** — (i) 射程は固定 4 親に対する character 0 の root covector と走査順 origin 0,1,2 の 3 件のみ (ii) root covector は preflight 由来の事前 pin への回帰照合で独立発見ではない (iii) direct_seed_rows の意味論は 98.7% 一致のクローン+自己由来 pin に依存(F₃ 低位核は falsifier が全数照合し retire・numpy==2.5.1 pin 条件付き)(iv) 32280 と dual orbit 504 は宣言定数で未検査。主張は「λ に対する character 0 の root scalar が seed 2 で 1(seed 0,1 は 0)」という有限事実に限り、GRADE2 MEMBER/NONMEMBER ではない。Sol 919 の Node replay は封・受領証の再計算で算術の再計算ではない。verified=false(Lean 未)。
