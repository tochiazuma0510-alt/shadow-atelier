...
# 【補遺 v1.2】C1′ 行の追加(裁定 919・数学者検分による)

**日付**: 2026-08-12 / **追記者**: 数学者(Opus 5・後任)/ **方式**: additive addendum(**本文・補遺 v1.1 とも不改変**)
**経緯**: 裁定 916 の合成検分(`k9_t63_compose_v1.md`)で C1′ が争点化 → 研究者指摘で **除外が記録に既在**と判明(裁定 919)。

| 項目 | 札そのものの状態 | ★ **位数 gate としての状態** | 根拠(LEDGER 行) | 正本 |
|---|---|---|---|---|
| **C1′**($n\ge5$ の窓同定・$q=7$ で $[\alpha]$ 3 類) | ✘ **開**($[\alpha]$ 方向・唯一の閉鎖路 =【I24-a】$\alpha$ 軌道予想) | ★ **除外済**(**類・位数の主張には不要**) | **L1065**「C1′ は族 gating から外れうる([α] は **exact 符号のみに効き類と位数に効かない**)」/ **L1140**(便 95 系検収)「**C1′ は類/位数 gate から除外可・M2 は不可**」/ **L1166** 補題 LIFT「α̃ の**曖昧さ = 一様化元取替と同一** ⟹ C1′ を sel/adm 分割」/ **L1860**「【**C1′-any**】十分方向は窓の同定不要」「**C1′(7)・C5 は系の適用 gate(G-1/G-2)でのみ要る**(Sol F91-5.3 の定理/gate 分離の族版)」 | `E1_gt_odd_dih_canonical_v1.md` §522 表(札の状態)/ LEDGER 上記 4 行(除外)/ `k9_t63_compose_v1.md` **v1.2 追記**(塔側への貫通証明) |

> ### ★ 読み方(**混同を防ぐ 3 行**)
> 1. **C1 と C1′ は別札**: **C1 = $n=3$ の窓同定 = 完全閉鎖**(裁定 107 `CLOSED_MATCH` + 174・cert `search/certs/c1_class_check_20260728.json` 実読)/ **C1′ = $n\ge5$**。
> 2. **C1′ が「開」なのは $[\alpha]$ 方向(exact 符号)のみ**。★ **類と位数の主張には効かない**(除外定理)。$\operatorname{ord}(a_9)=9$ は**位数の主張**ゆえ C1′ を要しない。
> 3. ★ **塔経路への貫通**は自明ではないので別途証明した(命題 **TOWER-α-INV**・`k9_t63_compose_v1.md` §B.2): 補題 LIFT が「$\tilde\alpha$ の曖昧さ = 一様化元取替」と同定し、**t63 §2.2 がその一様化元不変性を既に証明済**ゆえ、**追加補題なしで貫通**する。

⚠ **本補遺は記録の転記+1 命題への導線**であり、**判定語の新設・格の変更は行っていない**(索引の状態札を遵守)。
⚠ **NAME-COLLIDE 警告(L1860 逐語)**: 「**FAM-U-ASM §V.2.1 の (S3) と E1 の (S3) は同名異物**」。⟹ **(S3) を書くときは必ず出所(E1 / §V.2.1)を付ける**。
