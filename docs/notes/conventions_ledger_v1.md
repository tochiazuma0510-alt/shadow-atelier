...
### 1.5 CV-10 細則: 正典 pin の「証明本文の有無」欄・3 値(司令塔追記 2026-08-01・裁定 399/406 採択・candidate・Sol ゲート = 便 99・v1.4 編入予定)

- **宣言**: 正典の定理・命題を pin する読解ノート(reading_\*)・依存表は、pin ごとに**「証明本文の有無」欄**を必須とし、値は **3 値**: **あり**(正典に証明本文が掲載)/**読者演習**(言明のみ・証明は演習/省略)/**外部引用**(証明は他文献への引用のみ — 引用先を必ず併記)。
- **動機(事故型)**: 「正典の定理」という格付けが証明本文の実在を含意しない事例が**系統的に 5 例**(2401 Prop 3.15=読者演習・2405 Thm 4.4 奇分岐=読者演習・2008 Thm A.1=**外部引用**[Fresse Thm 6.2.4]・ほか 2405 Prop 4.1 偶/2008 Cor 3.13 = 証明本文なし[3 値の確定は各 reading ノートの pin 欄が正本])。欄がないと依存の梯子の格上げ(未解決予想→記述の穴→外部引用 — 裁定 406 の 2 段格上げが実例)が台帳に現れない。
- **適用**: 新規 reading ノートから必須・既存は発見次第追記(遡及一括は不要・【CL-2】と同流儀)。「あり」以外の pin に荷重を掛ける定理は、依存表にその値を伝播させ、自前補完 or 引用先言明の確認を検討対象として明示する。

### 1.6 CV-11 細則: 封印状態の正本(司令塔追記 2026-08-01・裁定 410・candidate・Sol ゲート = 便 99・v1.4 編入予定)

- **宣言**: 封印の開封状態の正本は**リポジトリ `provenance/seals/` の `*.opened.json` の存在+LEDGER の開封項**とする。金庫 `sealed/` ディレクトリの在庫から状態を推論することを**禁止**(開封プロトコルは「リポジトリへ複写公開・金庫原本は残置」— 残置は未開封を意味しない)。
- **事故**: 裁定 398 が seal_PSL_v1(2026-07-26 開封・7/7 的中・CLAIMS W3-6)を「封印維持」と誤記帳 — 金庫の残置原本から状態を誤読。裁定 410 で決着(三者ハッシュ一致・実質判断は無傷)。
