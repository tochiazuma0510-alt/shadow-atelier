# 命題 RAD-deg — 予想 RAD-2 の次数恒等式の検証(v1)
...
($\Leftarrow$)対偶: $x^p-a=g\cdot h$、$1\le k:=\deg g<p$ とする。分解体で $x^p-a=\prod_{j}(x-\zeta^j\alpha)$($\alpha^p=a$、$\zeta$ は原始 $p$ 乗根)。$g$ の定数項は $\pm$($k$ 個の根の積)$=\pm\zeta^{s}\alpha^{k}=:b\in F$。すると
