...
**裁定 2224(司令塔・2026-09-08)格付け**: 本判読(原本 `scratchpad/fal_cv9_fixed_lambda_batch_v5_report_v1.md` 63,249 B・sha256 83ebf334047e381ca71e32cf7ec9cb7e1cc070ceab2928dd622f2da0c3c77243・工房 sha256sum で pin)を正本として採用。CV-9 = 同一対象・限定 7 条(新設なし)→ **rank 1834/gen 8539 を cross-checked(限定 7 条)で受理し、v4 の 1706 の直系後継として置き換える(合算しない)**・verified=false・grade-2 NOT_DECIDED・A0 actual 0/1 不変。**F-v4-1 は閉鎖**(層数項の欠落として解決: fixed(k, n) = 26.12 + 1.271·k + 44.40·n・producer(k, n) = 32.82 + 12.43·k + 40.31·n・n = 認証する batch 親層数)。**F-v5-1(計画上の重大所見・限定条項ではない)**: 毎 run に親層を 1 つ積み上げると n ≤ 93.7 で producer cap(5,400 s)が飽和し rank ≈ 13,700(目標 48,384 の 28 %)で頭打ち・k を上げても逃げられない(残 46,550 行をどの (k, N) で割っても最終 run が cap の 1.79 倍)→ 層を回転(n を一定に保つ)すれば producer ≈ 1,695 s・k_max ≈ 425 で約 110 run で登り切れる。次 run の P 残差 1 数値(≈ 322 s なら積み上げ・≈ 278 s なら回転)が機構を分ける。
