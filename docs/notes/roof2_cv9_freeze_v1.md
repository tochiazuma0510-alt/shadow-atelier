...
# 追記 H(便 99 検収・裁定 412)— 標的 ENT-1 は**紙で空**と決着

> **追記型**: 本追記は §1–§8 の本文を**一切改変しない**。以下は §7.4 の標的 ENT-1 と §7.3 の「唯一残る型」に対する**上書きでない superseding note** である。
> 起草: 数学者(Opus 5)・2026-08-02。入力 = **Sol 便 99 返信 F99-3.5**(`sol/sol_reply_99_math26.md` L216–232)+ 当方の独立検分。

## H.1 何が決まったか

> ### 定理 NO-ENT(3)(正本 = `docs/notes/no_ent3_v1.md`)
> $N'\trianglelefteq B_3$、$N'\subseteq K^{(3)}$、$[K^{(3)}{:}N']=3$ なら、$1\to C_3\to PB_3/N'\to G_3\to1$ において **$G_3$ の作用は必ず自明**であり、拡大は分裂し、補群は**一意・正規・$B_3$-安定**である。
> ⟹ 命題 ENT-CRIT (b) が成立 ⟹ **この class に本質的 entangled 屋根は存在しない**。
> **格 = paper-proof(two-mathematician: Sol 起草 + 当方検分)・Lean 未**。**射程は index 3・$B_3$-normal・$K^{(3)}$ 内に固定**(拡大解釈禁止・裁定 412)。

要点は「$B_3$-正規性($S_3$-同変性)が $H^2$ の 2 次元をどこまで削るか」ではなく、**$\chi_i$ 作用そのものが $B_3$-正規性と両立しない**ことであった($\mathrm{Aut}(C_3)$ 可換 ⟹ 作用指標は $S_3$-不変 ⟹ 非自明 3 個は $S_3$ の一軌道ゆえ落ちる)。

## H.2 §7.4 の各行への効果(**本文は不変・読み替え表**)

| §7.4 の記述 | 追記 H での状態 |
|---|---|
| 標的 ENT-1(指数 3 の非分裂 $\chi_i$-拡大を探す) | ★ **class が空であることが証明された。起票しない。** |
| 「$B_3$-安定性が 2 次元の $H^2$ をどこまで削るかは **UNKNOWN**。削り切って 0 になる可能性は排除できていない」 | ★ **UNKNOWN 解消(否定側)**。削り切る。ただし削る場所は $H^2$ ではなく**作用**である |
| 「これは『存在する』の主張ではなく『ここを見よ』の設計である」 | **正しかった**(誤りではない)。見た結果、空だった |
| 【文献要請 ROOF2-L1】($\Gamma$-同変拡大の障害理論) | **この class に限れば不要**。ただし $n=5$ 側では依然有用 ⟹ **取り下げではなく射程の縮小**を提案(判断は司令塔) |
| 補題 INF($\dim H^2(G_3,\chi_i)=2$・inflation の像が 1 次元) | **数学として不変**(誤りではない)。ただしその非分裂拡大は **$B_3$-正規窓としては実現しない**ので、$M_{\rm ENT}$ のレシピの入口が閉じる |

## H.3 ENT-1 走査の格の降格

`search/certs/ent1_search_20260801.json`(`lins` による $B_3$ の指数 $1944$ 正規部分群走査)は:

- **旧**: 標的 ENT-1 の探索(見つかれば工房初の entangled 屋根)。
- **新**: ★ **較正 / 実測アンカー**へ降格。「その深さで 1 件・**split**」は定理 NO-ENT(3) と整合する事後確認であって、**非存在証明の役割を持たない**(非存在は紙が与える)。走査の深化・宇宙拡張は**起票しない**。

## H.4 波及(§8.2 の新規性リストへの追記)

§8.2 の項 7(補題 NO-CENTRAL / 補題 INF)に対し、**その先の決着**が付いた:
- 「$G_3$ の非分裂 $\chi_i$-拡大は存在するが、**$B_3$-正規な窓としては現れない**」という**分離**が本追記の内容である。
- 付随して「**正規補群が存在 ⟹ $G_n$ の作用は自明**」($[C,B_0]\subseteq C\cap B_0=1$)という 1 行の観察を記録する。これは命題 ENT-CRIT の裏返しであり、**$n=5$ 側の篩 F-2 の読み(必要条件か判定条件か)にも効く**(`k5_genuine_campaign_v1_addendum_a_k5mod.md` §A.8)。

## H.5 未閉鎖(この追記が閉じていないもの)

- **指数 9・27・… の $B_3$-正規窓**($n=3$)は**未着手**。定理 NO-ENT(3) は指数 3 のみ。
- **一般の奇 $n$ での同型命題**は**未主張**(`no_ent3_v1.md` §9 申し送り 1)。
- 本追記は **$K^{(5)}$ に一切触れていない**($n=3$ のみ)。
