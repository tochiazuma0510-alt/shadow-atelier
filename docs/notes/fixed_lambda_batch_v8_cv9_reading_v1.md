...
**裁定 2279 追補(Astra erratum・工房再計数)**: §2.1 表の行 99「`parent-layout` key 数」の v7 列 11 は誤りで、正は **12**(v8 = 13 は正)。工房が両 artifact の output/parent-layout.json(v7 10,670,185 B・v8 13,026,393 B)を Range 読みして top-level key を計数: v7(10173275037)= 12(schema d972.r07.fixed-lambda-cycle-batch.v7.parent-layout)・v8(10301413308)= 13(schema d972.r07.fixed-lambda-cycle-batch.v8.parent-layout)→ Astra の訂正は正(v7 列を 11 → 12 に読み替え)。C8 の式 acceptance + 2 は v7 で 10 + 2 = 12 と整合。群別 PASS・限定 7 条・rank 2218 の格付けに影響なし。
