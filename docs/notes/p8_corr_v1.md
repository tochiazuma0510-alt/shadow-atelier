...
# 【v1.3 追記】prereg カード **addendum v3.1** — :137 の型エラー訂正(裁定 981)

**日付**: 2026-08-12 / **委嘱**: 裁定 981(falsifier の遡及発見)
**方式**: ★ **addendum 方式**(**v3 は凍結済**〔裁定 977・sha256 頭 16 = `cb392e3dab7be483`〕につき **silent 編集禁止** — 本文・v1.1/v1.2 追記とも**不改変**)

> ## ★★ 訂正の要点
> $$\boxed{\ \textbf{:137「}a_9\ \textbf{の台が }\{3\}\textbf{」は}\textbf{型エラー}\ \textbf{— }a_9\ \textbf{は「台」の言明を持たない}\ }$$
> ★ **これは私が $r$ 仕様(`branchP_and_r_spec_v1.md` §II.1)で摘出した「$a$ と $a_9$ は別類群」の遡及適用**である。**同じ型を P8 カード側で見落としていた**。falsifier の指摘は正しい。

## A. 訂正

| | ✘ **旧(v3 :137 相当)** | ★ **正(v3.1)** |
|---|---|---|
| 記述 | 「$a_9$ の台が $\{3\}$」 | 「★ **$a$**(法 9・$\mathbf Q$ 側)**の素因子台が $\{3\}$**」 |
| 理由 | — | ★ $a_9=[u_9^{-1}]_{18}\in F_9^\times/F_9^{\times18}$($F_9=\mathbf Q(\zeta_{36})$)は **$\mathbf Q$ の素因子分解を持たない**($F_9$ の素イデアル分解しか持たない)⟹ **「素因子台」という言明が型として立たない** |
| radicand | — | ★ **$a$**($L_{9,\mathrm{Aff}}=\mathbf Q(\zeta_9,\sqrt[9]{a})$ の Kummer radicand・$a\in\mathbf Q^\times/(\mathbf Q^\times)^9\cong\bigoplus_p\mathbf Z/9$)⟹ **素因子指数ベクトルを持つ** ⟹ **台が読める** |

## B. ★ NOT 行の追加($r$ 仕様と同一の型宣言)

```
=== P8 CARD v3.1 — 型宣言の追加(NOT 行)===
[1] 測定対象(★ v3.1 で NOT 行を追加)
  ⚠ NOT : a_9(= [u_9^{-1}]_18)を「素因子台」の主語にしないこと
           理由 = 法 18・体 F_9 = Q(zeta_36) ⟹ Q の素因子分解を持たない
  ★ 台の主語になるのは a(法 9・Q^x/(Q^x)^9 ≅ ⊕_p Z/9)のみ
  ★ 位数の関係 : ord(a_9) と ord(a) はどちらも {1,3,9} に落ちるが
                 ★ 類そのものは別(法 9 vs 18・体 Q vs Q(zeta_36))
  ⚠ NOT : ord(a_9) から a の類を推測しないこと(★ r-GAP-1 と同一の禁)

[3] P-K9U-1 行(★ v3.1 で主語を訂正)
  P-K9U-1 : L_{9,Aff} = Q(zeta_9, 3^{1/9})
            ⟺ ★ a(法 9・Q 側)の素因子台 = {3}
            (旧: 「a_9 の台」= 型エラー)
=== END ===
...
| 項目 | 影響 |
|---|---|
| **[2] 判定基準**($\operatorname{ord}(a_9)\in\{1,3,9\}$) | ★ **変更なし**。$\operatorname{ord}(a_9)$ は $a_9$ について well-defined |
| **[5] 出力欄** | ★ **追加**: `a_class`($a$ の素因子指数ベクトル・法 9)— ⟹ **$r$ 仕様の A1 と同一の要求**(★ r-GAP-1: 位数では足りない) |
| **P-K9U-1 の判定** | ★ **主語が $a$ に変わるだけ**で、判定内容は同じ(台 $=\{3\}$) |
| **凍結の有効性** | ⚠ **v3 の凍結(sha `cb392e3dab7be483`)は有効**。本追記は**型宣言の明確化**であって判定基準の変更ではない ⟹ ★ **再凍結は司令塔の判断**(新 sha) |

## D. ★ 教訓

$$\boxed{\ \textbf{同じ型エラー(}a\ \textbf{と }a_9\ \textbf{の混同)を、}r\ \textbf{仕様で摘出しながら P8 カード側で見落としていた}\ }$$
⟹ ★ **新しい型区別を摘出したら、既存の全カード・全ノートに遡及適用する**(=「摘出即遡及」)。⟹ **v1.4.9 の W 行候補**(採番は案 B どおり Sol 突合後)。

**帰属**: 遡及発見 = **falsifier**。型区別の初出 = 私($r$ 仕様 §II.1)。委嘱 = 司令塔(裁定 981)。
**申告**: ⚠ $\operatorname{ord}$・$a\_class$ ともに未計算。走行ゼロ。
