# 事前登録 manifest — K⁽⁵⁾ 算術飽和キャンペーン v1.3(2026-07-27・司令塔)

> v1.2 → v1.3(便 32 P1/P3・裁定 31): 結果規則を total transition table 化(UNKNOWN+UNKNOWN 追加・REFUTED 新設 = Sol 確認対象)・whitelist に Rule 1 I-b と同語の禁止を反映。
...
3. **役割分離**: Model-Builder(A)の出力 **whitelist** — 許可: 明示モデル・Belyi map・分岐 divisor・cusp・uniformizer の式・target triple への exact conjugator(分岐指数 10 や uniformizer 性の証明は許可)。**禁止: λ/t¹⁰ の非零定数項とその同値物(leading coefficient・その valuation/class)、および Rule 1 I-b と同語で「c の平方類・平方因子・符号の計算、λ の (c, μ) 対への分離報告」(v1.3・便 32 P3 — 親子文書の禁止集合を一致させる)・それらを候補選択に使うこと**。A は「u 未計算」を申告し全 transcript 保存。**主根拠 = 凍結済み入出力 schema+役割別 access log・grep は補助検査**(W4)。Extractor(B)は凍結 2+発射錠後にのみ起動・規約変更権なし。
