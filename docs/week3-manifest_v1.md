# Week 3 較正バッテリー canonical manifest v1 — **実装の唯一の正本**
...
**(5) 期待値**(**sealed**):
```json
{ "hexagon_free_certificate": { "candidate_total": 8, "h10_fail": 0,
    "h11_fail": 4, "generation_fail": 0, "shadow_total": 4 },
  "gt_count": 4,
  "explicit_shadows": [ {"m":0,"f":"1"}, {"m":1,"f":"-1"},
                        {"m":2,"f":"-1"}, {"m":3,"f":"1"} ],
  "generation_pass_count": 4 }
```
導出: A = {±1} は中心・自己逆元・θ 不変ゆえ **(H-a) は空虚**(h10_fail = 0)。𝒩_m(f) = f³ = f ゆえ **(H-b′) ⟺ f = E_m**、E_m = (1,−1,−1,1) で各 m にちょうど 1 解(h11_fail = 4)。生成性は u 奇より ⟨±i,±j⟩ = Q₈(generation_fail = 0)。
**(6) reduction**(sealed): 発側なし(塔の粗い端)。**1b・2a・2b の reduction 先**として使われる。
**(7) isolated**(sealed): `true`。根拠 = 4 shadow それぞれの kernel 直接構成(v1 §3.5)+ kernel 証明書(PB₃ 上 8 点)。
...
```json
{ "gt_count": 24,
  "hexagon_free_certificate": { "candidate_total": 432,
      "h10_fail": "UNKNOWN", "h11_fail": "UNKNOWN",
      "generation_fail": "UNKNOWN", "shadow_total": 24,
      "invariant": "h10_fail + h11_fail + generation_fail = 408" },
  "prop_C_formula": 48, "prop_C_break_ratio": 2 }
```
導出: 各 m ∈ 𝒳(8 個)で G₃ 側は [m mod 6, f_G] ∈ GT(K⁽³⁾) の 3 個(2405 Thm 4.3)、Q₈ 側は f_P = E_m の 1 個 ⇒ 8·3·1 = 24。命題 C の公式は \|GT(K⁽³⁾)\|·e·\|[P,P]\| = 12·(8/4)·2 = **48**、実際は 24 = 48/2 ⇒ **公式が破れる初例**、破れの因子は hexagon が [P,P] から奪う自由度 2/1 = 2。
**【GAP-M1】(本書で新設)**: **h10/h11 の排他的分配を私は導出していない**。G₃ 成分の θ 作用を計算していないためである。**実装の観測値として記録**し、次版で紙上導出を追加する。**invariant(和 = 408)だけが fixture である。**
**(6) reduction**(sealed):
```json
[ { "target": "K^(3)", "surjective": true, "image_size": 12, "fibre": 2,
    "kernel_order": 2, "kernel_structure": "C2" },
  { "target": "N_Q", "surjective": true, "image_size": 4, "fibre": 6,
    "kernel_order": 6, "kernel_structure": "UNKNOWN" } ]
```
`R_to_NQ` の繊維 6 は「両対象 isolated ⇒ R は群準同型・全射」から導出した値(24/4)。**ker の群構造は UNKNOWN — 推測を書かない。**
**(7) isolated**(sealed): `true`。根拠 = 2401 Prop 3.15(isolated ∩ isolated)。
...
{ "hexagon_free_certificate": { "candidate_total": 8, "h10_fail": 0,
    "h11_fail": 4, "generation_fail": 0, "shadow_total": 4 },
  "gt_count": 4,
  "explicit_f": "f = w^{ m(m+1)/2 mod 2 }",
  "generation_pass_count": 4 }
```
導出: A = ⟨w⟩ ≅ C₂ 中心・θ(w) = w⁻¹ = w ゆえ (H-a) は空虚。𝒩_m(f) = f、E_m = (1,w,w,1) ⇒ f = E_m が各 m に 1 解。生成性は 2 群の Frattini 論法。m(m+1)/2 mod 2 = (0,1,1,0) が E_m と一致することを確認済み。
**(6) reduction**(sealed): `[ { "target": "N_Q", "surjective": true, "bijective": true, "image_size": 4, "fibre": 1, "kernel_order": 1 } ]`
**(7) isolated**(sealed): `true`。根拠 = **verbal ⇒ 補題 H2**。kernel 証明書は独立確認(PB₃ 上 32 点)。
...
**(5) 期待値**(**sealed**):
```json
{ "hexagon_free_certificate": { "candidate_total": 32, "h10_fail": 16,
    "h11_fail": 8, "generation_fail": 0, "shadow_total": 8 },
  "gt_count": 8, "m_full": true,
  "explicit_shadows": { "m=0": ["1","pq"], "m=1": ["w","wpq"],
                        "m=2": ["w","wpq"], "m=3": ["1","pq"] },
  "generation_pass_count": 8 }
```
導出(定理 H9): f = w^α p^β q^γ に対し **(H-a) ⟺ β = γ**(8 元中 4 元が通る ⇒ 各 m で 4 落ち、計 h10_fail = 16)。𝒩_m(f) = w^α p^{αm} q^{α(1+m)}、E_m = (1, wp, wq, 1) から各 m で残り 4 元中 2 元が通る(h11_fail = 8)。生成性は Frattini(generation_fail = 0)。8 = 32 − 16 − 8 − 0 ✔
**(6) reduction**(sealed):
```json
[ { "target": "N_2", "surjective": true, "image_size": 4, "fibre": 2,
    "kernel_order": 2, "kernel_structure": "C2",
    "kernel_elements": ["[0,1]", "[0,pq]"] },
  { "target": "N_Q", "surjective": true, "image_size": 4, "fibre": 2,
    "kernel_order": 2, "kernel_structure": "C2" } ]
```
**(7) isolated**(sealed): `true`。根拠 = **verbal ⇒ 補題 H2**。
...
**(5) 期待値**(**sealed** — G-06 / P81 の exact witness 形式):
```json
{ "gt_count": "UNKNOWN",
  "gt_count_lower_bound": { "relation": "ge", "value": 4 },
  "known_solution_count": 4,
  "known_solutions": [
    { "i":0, "m":0, "u":1, "f_canonical":"()",            "hexagon":true,
      "generation":true, "settled":true, "automorphism_witness":"()" },
    { "i":1, "m":1, "u":3, "f_canonical":"(1 5)(3 4)",    "hexagon":true,
      "generation":true, "settled":true, "automorphism_witness":"(1 3 5 4)" },
    { "i":2, "m":3, "u":7, "f_canonical":"(1 3 2 5 4)",   "hexagon":true,
      "generation":true, "settled":true, "automorphism_witness":"(1 5 2 4)" },
    { "i":3, "m":4, "u":9, "f_canonical":"()",            "hexagon":true,
      "generation":true, "settled":true, "automorphism_witness":"(1 5)(3 4)" } ],
  "m_full": true,
  "hexagon_free_certificate": { "candidate_total": 240, "h10_fail": "UNKNOWN",
      "h11_fail": "UNKNOWN", "generation_fail": "UNKNOWN",
      "shadow_total": "UNKNOWN" },
  "isolated": "UNKNOWN" }
```
- **`settled[m]` 形式は禁止**(F19): 一つの m に未知の f が複数あり得るので、m を key にすると「その m の全 shadow が settled」と誤読され `isolated = UNKNOWN` と衝突する。**exact な (m, f) を key にする。**
- `gt_count_lower_bound` は **文字列 `">=4"` を禁止**。`{"relation":"ge","value":4}` か `known_solution_count` から導出。
- `automorphism_witness` は α_{m,f}: X ↦ X^u, Y ↦ f⁻¹Y^u f を実現する Aut(A₅) = S₅ の元。
- **`gt_count` を UNKNOWN に置く代償**: 総数は「較正」ではなく「発見」になる。**私は紙上で計算していない**(裁定 P59 — 私が計算すれば私の数が錨になり実装のブラインド性が失われる)。総数を較正に使いたければ司令塔が別途「紙上全列挙」を**別系統として**発注し封印すること。
**(6) reduction**(sealed): 発側なし。**A2 の reduction 先**。
**(7) isolated**(sealed): **`UNKNOWN`**。根拠 = 4 shadow の settled は確認済みだが**全 shadow は未列挙**(【GAP-E11】)。
 **W57(厳守)**: isolated が確定するまで **GT(N_A) を「群」と呼ばない**。
...
{ "gt_count": "UNKNOWN",
  ...
  "hexagon_free_certificate": { "candidate_total": 240, "h10_fail": "UNKNOWN",
      "h11_fail": "UNKNOWN", "generation_fail": "UNKNOWN",
      "shadow_total": "UNKNOWN (= A1 の shadow_total)" },
  "isolated": "UNKNOWN" }
```
**(6) reduction**(sealed) — **G-08 / F20 の全 shadow 集合全単射**:
```json
[ { "target": "N_A",
    "set_surjective": true, "set_bijective": true,
    "fibre": 1, "kernel_order": 1,
    "group_homomorphism": "UNKNOWN",
    "basis": "補題 A2A1 (狩場計画 v4 §5.6)" },
  { "target": "N_5", "surjective": true,
    "basis": "各 m に [m,1](v3 §5.5 / 便 06 F13)" } ]
```
...
```json
{ "gt_count": 48,
  "hexagon_free_certificate": { "candidate_total": 1728,
      "h10_fail": "UNKNOWN", "h11_fail": "UNKNOWN",
      "generation_fail": "UNKNOWN", "shadow_total": 48,
      "invariant": "h10_fail + h11_fail + generation_fail = 1680" },
  "prop_C_formula": 192, "prop_C_break_ratio": 4 }
```
導出: 命題 C の公式は \|GT(K⁽³⁾)\|·e·\|[P₃,P₃]\| = 12·(8/4)·8 = **192**、実際は 48 = 192/4。破れの因子は hexagon が [P,P] から奪う自由度 8/2 = 4(定理 H9 の「各 m に 2 解」から)。**【GAP-M1】と同じ理由で staged 分配は UNKNOWN**、invariant のみ fixture。
**(6) reduction**(sealed):
```json
[ { "target": "K^(3)", "surjective": true, "image_size": 12, "fibre": 4,
    "kernel_order": 4, "kernel_structure": "UNKNOWN" },
  { "target": "N_3", "surjective": true, "image_size": 8, "fibre": 6,
    "kernel_order": 6, "kernel_structure": "UNKNOWN" } ]
```
**(7) isolated**(sealed): `true`。根拠 = 2401 Prop 3.15(isolated ∩ isolated)。
