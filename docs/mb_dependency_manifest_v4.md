# `mb/dependency-manifest/v4` — implementation dependency closure の記録様式

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 81)。v3 を supersede。**
**この文書は `mb/ninfty-stage2-predicate/v9` §4.4 の `dependency_manifest_schema_id` が指す実体である。**
...
```text
schema_id     = "mb/dependency-manifest/v4"
...
supersedes    = "mb/dependency-manifest/v3"
supersedes_digest = 1a8d1f2147178b49d5fe81da625256762a9e9dafd1a963b57d554bcf97c7b7dd   # 監査 FAIL の candidate
```
...
| 共有を許す trusted base と初期値 | §5.2・§5.4(**五欄すべて実値 `[]`**) |
...
| **E-7′** | **E-5・E-6・E-9 の 5 欄が D-1 / D-2 / D-3 すべての preimage を尽くす。** 受領側はこれだけから §2.2 の三 hash を再計算できなければならない。**再計算不能な entry は独立性の証跡として使えない。** |
| **E-8′** | **producer が申告した `source_closure_digest` / `implementation_lineage_digest` / `build_root_id` は参照値でしかない。** 受領側の再計算値と食い違えば `digest-mismatch` [12]。 |
...
| **D-R2′** | **【v4 で D-3 を追加】受領側は preimage から D-1・D-2・D-3 の三つすべてを再計算して照合する。** v3 は D-1・D-2 しか要求していなかった(**自認**)。 |
...
allowed_shared_family[]     = [ { implementation_family_id, role, justification, frozen_at_receipt } ] # audit flag 用
```
| # | 条項 |
|---|---|
...
| # | 条項 |
|---|---|
| **T-1′** | **初期 TCB は四欄とも空である。** 「省略」でも「暗黙の許可」でもなく、**実装着手前の実値宣言**。 |
...
| **I-0c** | **【v4】`build_root_id` の subject code への binding を検査する** — D-3 の再計算値が `dependency_manifest.build_root_id` および該当 entry の値と一致すること。**producer 申告のみの `build_root_id` を受理しない。** |
...
| **I-3c′** | **family 交差**は **audit flag**。非空なら `family_overlap_flag` を receipt に記録するが、**単独では [11] を発しない**(M-3′)。 |
...
dependency_manifest = {
  ...
  build_root_id,                    # 受領側が D-3 で再計算・照合(I-0c)
...
conformance_record = {
  schema_id, schema_digest,
  covered_clauses = [E-1, E-1', E-2, E-2', E-3, E-4, E-5, E-6, E-7', E-8', E-9,
                     D-R1, D-R2', D-R3, D-R4, M-1'..M-5,
                     H-1', H-1a", H-1b, H-1c,
                     H-2, H-2a, H-2b", H-2c, H-2d,
                     R-1..R-6,
                     H-3, H-3a, H-3a', H-3b, H-3c,
                     H-5, H-5a, H-5b, H-5c, H-5d',
                     Y-1..Y-3, Y-4, Y-4a, Y-4b, Y-4c, Y-4d, U-1..U-4,
                     T-1', T-2', T-3, T-4,
                     H-4, I-0', I-0c, I-1, I-2, I-3a, I-3b, I-3d, I-3c', I-5', I-6, I-8,
                     N-1]
  uncovered_clauses = []
}
```
