...
### 1.2 Hall 基底(正本 — `docs/week4-掃引宇宙_v3.md` §1.3 と同一・以後変更しない)
...
$h:=\tau(w)$ に (4.1) を適用する。$[uv,z]=[u,z]^v[v,z]$ と「重み $\ge6$ は $1$」を繰り返し使って
$$ [h,y]=[w,y]^{p^{-1}r_1t_1^{-1}}\,[p^{-1},y]^{r_1t_1^{-1}}\,[r_1,y]=q\,r_2^{-1}\,t_2,\qquad [h,y,y]=[q,y]\,[r_2^{-1},y]=r_3t_3^{-1},\qquad [h,y,y,y]=t_4 $$
($[q,p]\in\gamma_6=1$、$[q,r_1]\in\gamma_7=1$ ゆえ共役は効かない)。これらの積を Hall 順に並べるときの $c_s$ 補正は $\kappa$ の第一引数の $p,q$ 座標 × 第二引数の $w$ 座標だが、第 2 因子以降の $w$ 座標は $0$ なのですべて消える(冪補正 $\delta$ も同じ理由で $0$)。よって
...
| §4.1 補題 4.1(collection 公式) | **紙上証明**(帰納法・全 $m\in\mathbb Z$)+ 機械検算 |
| §4.3 の $\sigma_m$ 表 | **導出ルートは紙上**(補題 4.1)、$\tau(w),\tau(p)$ 経由の 2 元は手計算、**全表は Magnus モデル計算(単系統)**。$\bar{\mathcal N}=1+\bar\sigma+\bar\sigma^2$ は **E19/GAP と cross-checked**(全 200 成分 × 10 個の $m$)。$\bar\sigma$ 自身の第二系統は**ない** |
