# `mb/ninfty-verifier-contract/v3` — divisor equality certificate の検査契約

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 79)。v2 を supersede。**
**この文書は `mb/ninfty-stage2-predicate/v8` §4.4 の `verifier_contract_id` が指す実体である。**
...
```text
contract_id     = "mb/ninfty-verifier-contract/v3"
...
dependency_manifest_schema_id     = "mb/dependency-manifest/v3"
dependency_manifest_schema_digest = 1a8d1f2147178b49d5fe81da625256762a9e9dafd1a963b57d554bcf97c7b7dd
supersedes        = "mb/ninfty-verifier-contract/v2"
supersedes_digest = 1fd36b3eda0da33b2aba5d3d371a24749850b9b05a3f4c4f17ef1725ffe555bd   # 監査 FAIL の candidate
```

> **【hash 順序・便 66 F11】** 非循環な順序は **manifest → contract → spec → receipt**。**本稿は manifest v3 の exact digest を pin し、spec の digest は pin しない**(governing spec は **ID で束縛し digest は receipt 側**)。**spec v8 が本稿の exact digest を pin する。**
...
| **public secondary** | **§5.4 で新設** | **spec 側条文が要る → §8 の erratum 案 A** |
...
| # | 条項 |
|---|---|
| **Y-1** | **`declared_untrusted_inputs[]` は implementation closure の universe から分離される**(manifest v3 §5.3)。**TCB として差し引くのではなく交差検査の対象外。** |
| **Y-2** | **入力の共有は独立性を毀損しない。毀損するのは実装の共有である。** |
| **Y-3** | untrusted input については **A と B で digest 一致を要求する**(P-3.1〜P-3.3)。不一致は [12]。**これは R-3 の前提でもある。** |
| **Y-4** | **入力クラスへの math-helper 混入を禁止**。判定基準は manifest v3 §5.3.2(U-1〜U-4)。**規約を選ぶパラメータ値の共有は許され、禁止されるのはその規約を実現するコードの共有。** |

---

## 7. 二 verifier の独立要件 {#independence}

**`mb/dependency-manifest/v3` を参照する。**

| # | 義務 |
|---|---|
| **C-1** | A と B は **別実装**であり、`implementation_dependency_closure` を**推移的閉包**として提出する(H-1′)。検収は **attestation と受領側 fixpoint 再計算**による(depth の見た目で判定しない)。 |
| **C-2** | 同一性は **content digest** で判定される(H-2)。別名・別 path・薄い wrapper では独立性を主張できない。 |
| **C-3** | 共有してよいのは三つの TCB 欄に列挙された値のみ。**`role = math-helper` を TCB に入れてはならない**(H-3)。 |
| **C-4** | **禁止交差は receipt 受領側が三集合から再計算する**(H-4・I-3a/I-3b/I-3c)。 |
| **C-5** | TCB の拡張は**追加側に挙証責任**があり receipt を要する(H-5)。**三欄は独立**(H-5d)。 |
| **C-6′** | **【B66-1 で強化】** A と B は **lineage preimage**(`source_artifact_digests[]`・`toolchain_digest`・`build_step_digests[]`)を提出する(manifest v3 §2.1)。**`implementation_family_id` は receipt authority が mint する — producer は自分で選べない**(M-1)。**producer 可変の識別子を独立性の根拠にしてはならない**(H-2d)。 |
| **C-7** | **A と B は互いの中間結果を読まない。** 相手の `result_digest` を自分の再計算の**前に**参照することを禁止する。**§5.1 C1 の比較は、両者が独立に確定した $R_A$・$R_B$ に対して受領側が行う。** |
| **C-8** | **初期 TCB は三欄とも空**(manifest v3 §5.4)。ゆえに **A と B は異なる runtime で実装する**。同一 runtime を用いるなら receipt で TCB に追加する。 |

---

## 8. governing spec への erratum 案(**v8**)【裁定 79】{#erratum}

> **【状態】v7 までの要請は発行済み。以下は v8 に必要な 3 件。** 便 66 F10 が **FAIL** とした項目のうち **spec 側でしか直せないもの**である。
...
```text
verifier_contract_id     = "mb/ninfty-verifier-contract/v3"
