# `mb/ninfty-verifier-contract/v6` — divisor equality certificate の検査契約

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 84)。v5 を supersede。**

> **【版履歴】v6 = v5 の修理。** 起点は **Sol 便ではなく内部前哨ゲート(falsifier)**。変更は **3 点** — **(T1) FINDING-3 の修理**: `covered_procedure_checks` にも registry と集合等式を与える(CR-5)・**(T2) FINDING-2 の pin**: governing spec の certificate schema から未定義欄 `verifier_evidence` が除かれ、独立性証跡が SEALED_INTERNAL 並列であることを検査手続きに反映・**(T3) manifest v6 pin・FINDING-4 の配置修理**。**数学的内容・検査手続き・二軸 routing は v5 と逐語同一。**
...
```text
contract_id     = "mb/ninfty-verifier-contract/v6"
contract_digest = <64 hex: 本稿 exact blob の sha256 — receipt が記入>
encoding        = UTF-8, LF, no BOM, no normalization
governing_spec  = "mb/ninfty-stage2-predicate/v11"
governing_spec_digest = <64 hex: governing spec の digest — receipt が記入>
dependency_manifest_schema_id     = "mb/dependency-manifest/v6"
dependency_manifest_schema_digest = f4372600b99850bea4c6545761974e7b4c8189e4e4414f491824ad47fee0c070
supersedes        = "mb/ninfty-verifier-contract/v5"
supersedes_digest = 00831f1397cd51a28c9ef0fe5782827f56d40864aec40ef9ce98174c7962e4bd   # 内部ゲート差戻し (falsifier FINDING-2/3/4)
...
> **【hash 順序・便 66 F11】** 非循環な順序は **manifest → contract → spec → receipt**。**本稿は manifest v5 の exact digest を pin し、spec の digest は pin しない**(governing spec は **ID で束縛し digest は receipt 側**)。**governing spec が本稿の exact digest を pin する。**
...
> **【自認・v6 で移設】** v5 はこの位置(operative 本文)に「v2 が置いていた 1 点優先の例外は spec v7 erratum E1 で解消済み」という歴史記述を置いていた。**operative 行に歴史 token を置くことは LA-2 の配置規約違反**であり、**「記録」の語を含むだけで sweep の除外域と見なされる**穴でもあった(内部ゲート FINDING-4)。**本 blockquote へ移設した。**

---

## 0.15 前版差分【裁定 84・内部前哨ゲート起点】
...
> **v5 でも不変**: **§1 役割・§2 検査対象・§3 検査手続き(W-2 / W-2′ の型分け・§3.4 result vector)・§4 合否・§5.1 二軸 routing(X-1〜X-6)・§5.4 secondary(P-S1〜P-S6)・§6 入力型分離。**
...
> **[historical] Y-1**: `declared_untrusted_inputs[]` は implementation closure の universe から分離される**(manifest v5 §5.3)。
> **[historical] Y-4**: 判定基準は manifest v5 §5.3.2(U-1〜U-4・**Y-4d: build-time artifact は入力クラスに置けない**)。
...
**`mb/dependency-manifest/v6` を参照する。**
...
| **C-4″** | **禁止交差は receipt 受領側が四面から再計算する**(H-4・I-3a binary / I-3b source / **I-3d build**)。**family(I-3c′)は audit flag で、allowlist を差し引かない**(FA-2)。 |
| **C-5″** | TCB の拡張は**追加側に挙証責任**があり receipt を要する(H-5)。**四欄は独立**(H-5d′)。**欄数は四である**(H-5e)。 |
| **C-6‴** | **受領側は D-1・D-2・D-3・D-4 の四つすべてを再計算する**(D-R2″・I-0′・**I-0c′**)。**`subject_build_binding_digest` により `subject_code_digest` が binding の計算に現れる**(SB-1〜SB-3)。**identity binding が保証するのは取り違え防止まで**であり、**実生成関係を主張する場合は §2.5 の `build_attestation` が必須**(SB-4・BA-2)。**output-affecting な toolchain / build step / code generator は closure entry へ必須昇格**(R-6)。**`implementation_family_id` は audit flag であって blocking 判定には使わない**(M-3′)。**producer 可変の識別子を独立性の根拠にしてはならない**(H-2d)。 |
| **C-7** | **A と B は互いの中間結果を読まない。** 相手の `result_digest` を自分の再計算の**前に**参照することを禁止する。**§5.1 C1 の比較は、両者が独立に確定した $R_A$・$R_B$ に対して受領側が行う。** |
| **C-8″** | ゆえに **A と B は異なる runtime で実装し、かつ異なる toolchain / build step で build する**(**共有 toolchain は build face で [11]**)。共有するなら receipt で該当欄に追加する。 |
> **[historical] C-3″**: `allowed_shared_family[]` は audit 専用の acknowledged justification list であり、I-3c′ は差し引かない**(manifest v5 §5.2.1 FA-2)。
> **[historical] C-6‴**: 【chg v5 で manifest v5 へ同期】** A と B は **preimage 6 欄**(`source_artifact_digests[]`・`toolchain_digest`・`build_step_digests[]`・`build_definition_blob_digest`・`pinned_input_digests[]`・**`subject_build_binding_digest`**)を提出する(manifest v5 §2.1 E-5・E-6・E-9・E-10)。
> **[historical] C-8″**: 初期 TCB は四欄とも空**(manifest v5 §5.4 T-1″)。
...
```text
verifier_contract_id     = "mb/ninfty-verifier-contract/v6"
...
conformance_record = {
  ...
  # --- provenance preimage 6 欄(manifest v5 §2.1 E-5・E-6・E-9・E-10)---
  ...
  build_attestation,                 # optional(manifest v5 §2.5)

  # --- closure と入力 ---
  implementation_dependency_closure[],
  declared_untrusted_inputs[],

  # --- TCB 四欄(manifest v5 §5.2・初期値はすべて [])---
  allowed_shared_tcb[], allowed_shared_source_tcb[],
  allowed_shared_build_tcb[], allowed_shared_family[],

  covered_clauses = [C-1′, C-2, C-3″, C-4″, C-5″, C-6‴, C-7, C-8″, CR-1, CR-2, CR-3, CR-4, CR-5, 
                     CR-6, CR-7, LA-1, LA-2, LA-3, P-S1, P-S2, P-S3, P-S4, P-S5, P-S6, R-1, R-2, 
                     R-3, S1, S2, S3, T3, V-0, V-1, V-2, V-3, V3, W-1, W-3, W-4, W-5, W-6, X-1, 
                     X-2, X-3, X-4, X-5, X-6, Y-1, Y-2, Y-3, Y-4]
  covered_procedure_checks = [C1, D-1, D-2, D-3, D-4, P-0.1, P-0.2, P-0.3, P-0.4, P-0.5, P-0.6, 
                              P-0.7, P-0.8, P-1.1, P-1.2, P-1.3, P-1.4, P-1.5, P-3.1, P-3.2, 
                              P-3.3, S1, S2, S3, U-1, U-4, W-1, W-2, W-3, W-4, W-5, W-6]
  uncovered_checks = []
  uncovered_clauses = []
}
```
...
| **CR-5** | **【chg v6 新設・内部ゲート FINDING-3】`procedure_check_registry` の抽出規則(機械的)**: 本稿の本文に現れる **check ラベル** (`D-<digit>`・`U-<digit>`・`P-<digit>.<digit>`・`W-<digit>`・`W-2′`・`S1`・`S2`・`S3`・`C1`)の全体。**受領側は `covered_procedure_checks ∪ uncovered_checks = procedure_check_registry` かつ `∩ = ∅` を exact set equality で照合する。** |
...
```text
live_authority_refs[] = [
  { artifact_id: "mb/ninfty-stage2-predicate/v11",
  ...
  { artifact_id: "mb/dependency-manifest/v5",
    digest_or_receipt_slot: "f4372600b99850bea4c6545761974e7b4c8189e4e4414f491824ad47fee0c070",
    anchor: "§2.1 preimage / §2.4 subject binding / §5.2 TCB / §6 intersections" }
]

historical_quotation_refs[] = [
  { artifact_id: "mb/ninfty-verifier-contract/v4", digest: "703fb47f...(supersedes)" },
  { artifact_id: "mb/ninfty-verifier-contract/v3", digest: "bd4d5064...(supersedes)" },
  { artifact_id: "mb/ninfty-verifier-contract/v2", digest: "1fd36b3e...(supersedes)" },
...
| **LA-3** | **自版だけを見る sweep は不十分**(便 68 F6 が反証した失敗型)。 |
