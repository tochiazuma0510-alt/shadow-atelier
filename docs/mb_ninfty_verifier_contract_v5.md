# `mb/ninfty-verifier-contract/v5` — divisor equality certificate の検査契約
...
```text
contract_id     = "mb/ninfty-verifier-contract/v5"
...
dependency_manifest_schema_id     = "mb/dependency-manifest/v5"
dependency_manifest_schema_digest = 46150ba03ae02c340431fe3a162d2925b7b7a5b8fcf55dca76fd2c6c5e351df8
supersedes        = "mb/ninfty-verifier-contract/v4"
supersedes_digest = 703fb47f60e721b2f0f6a79197f4047f723f030367fca9641a841aca6728bd75   # 監査 FAIL の candidate
...
**v2 が置いていた「1 点優先の例外」(§4.2 の型誤り)は spec v7 erratum E1 で解消済み。** §8 は erratum 案の**記録**を持つが、これは**要請であって本稿の優先を主張するものではない** — **§0 header の governing spec が発行され receipt がその digest を束縛するまで、本稿を operative にしない**(上の fail-closed 条項)。**【v5・F6.2 で同期】**
...
| **V3** | manifest pin = v4 | **manifest v5**(`46150ba0…`)へ。§7 の条項も manifest v5 の新条項(SB / BA / FA)へ同期 | 裁定 83 |
...
## 0.2.1 v3 → v4 差分【裁定 81】(v4 から継承・変更なし)
...
| # | 条項 |
|---|---|
| **Y-1** | **`declared_untrusted_inputs[]` は implementation closure の universe から分離される**(manifest v5 §5.3)。**TCB として差し引くのではなく交差検査の対象外。** |
| **Y-2** | **入力の共有は独立性を毀損しない。毀損するのは実装の共有である。** |
| **Y-3** | untrusted input については **A と B で digest 一致を要求する**(P-3.1〜P-3.3)。不一致は [12]。**これは R-3 の前提でもある。** |
| **Y-4** | **入力クラスへの math-helper 混入を禁止**。判定基準は manifest v5 §5.3.2(U-1〜U-4・**Y-4d: build-time artifact は入力クラスに置けない**)。**規約を選ぶパラメータ値の共有は許され、禁止されるのはその規約を実現するコードの共有。** |

---

## 7. 二 verifier の独立要件 {#independence}

**`mb/dependency-manifest/v5` を参照する。**
...
| **C-3″** | 共有してよいのは四つの TCB 欄に列挙された値のみ。**`role = math-helper` を TCB に入れてはならない**(H-3)。**`allowed_shared_family[]` は audit 専用の acknowledged justification list であり、I-3c′ は差し引かない**(manifest v5 §5.2.1 FA-2)。 |
...
| **C-6‴** | **【v5 で manifest v5 へ同期】** A と B は **preimage 6 欄**(`source_artifact_digests[]`・`toolchain_digest`・`build_step_digests[]`・`build_definition_blob_digest`・`pinned_input_digests[]`・**`subject_build_binding_digest`**)を提出する(manifest v5 §2.1 E-5・E-6・E-9・E-10)。**受領側は D-1・D-2・D-3・D-4 の四つすべてを再計算する**(D-R2″・I-0′・**I-0c′**)。**`subject_build_binding_digest` により `subject_code_digest` が binding の計算に現れる**(SB-1〜SB-3)。**identity binding が保証するのは取り違え防止まで**であり、**実生成関係を主張する場合は §2.5 の `build_attestation` が必須**(SB-4・BA-2)。**output-affecting な toolchain / build step / code generator は closure entry へ必須昇格**(R-6)。**`implementation_family_id` は audit flag であって blocking 判定には使わない**(M-3′)。**producer 可変の識別子を独立性の根拠にしてはならない**(H-2d)。 |
| **C-7** | **A と B は互いの中間結果を読まない。** 相手の `result_digest` を自分の再計算の**前に**参照することを禁止する。**§5.1 C1 の比較は、両者が独立に確定した $R_A$・$R_B$ に対して受領側が行う。** |
| **C-8″** | **初期 TCB は四欄とも空**(manifest v5 §5.4 T-1″)。ゆえに **A と B は異なる runtime で実装し、かつ異なる toolchain / build step で build する**(**共有 toolchain は build face で [11]**)。共有するなら receipt で該当欄に追加する。 |
...
```text
verifier_contract_id     = "mb/ninfty-verifier-contract/v5"
...
## 9. 適合宣言【v5 で同期・F7・F13.3】{#conformance}
...
conformance_record = {
  ...
  covered_clauses = [C-1′, C-2, C-3″, C-4″, C-5″, C-6‴, C-7, C-8″, CR-1, CR-2, CR-3, CR-4, LA-1, 
                     LA-2, LA-3, P-S1, P-S2, P-S3, P-S4, P-S5, P-S6, R-1, R-2, R-3, S1, S2, S3, 
                     V-0, V-1, V-2, V-3, V3, W-1, W-3, W-4, W-5, W-6, X-1, X-2, X-3, X-4, X-5, 
                     X-6, Y-1, Y-2, Y-3, Y-4]
  covered_procedure_checks = [C1, P-0.1, P-0.2, P-0.3, P-0.4, P-0.5, P-0.6, P-1.1, P-1.2, P-1.3, 
                              P-1.4, P-1.5, P-3.1, P-3.2, P-3.3, S1, S2, S3, W-1, W-2, W-2′, W-3, 
                              W-4, W-5, W-6]
  uncovered_clauses = []
}
```
...
| **CR-3** | **range 表記(`C-1..C-5` 型)は禁止**する — **prime / double-prime を落とすため**(便 68 F13.3)。**v4 の `covered_clauses` は range を使い、実際に `C-3′`・`C-4′`・`C-5′`・`C-6″`・`C-8′` を覆っていなかった・自認。** |
...
live_authority_refs[] = [
  ...
  { artifact_id: "mb/dependency-manifest/v5",
    digest_or_receipt_slot: "46150ba03ae02c340431fe3a162d2925b7b7a5b8fcf55dca76fd2c6c5e351df8",
...
| **LA-3** | **sweep の対象定義**: 自 artifact の全版 token("contract v1..v5" / `mb/ninfty-verifier-contract/v*`)と、**他 2 文書の全版 token**("spec v5..v10" / `mb/ninfty-stage2-predicate/v*`・"manifest v1..v5" / `mb/dependency-manifest/v*`)。**自版だけを見る sweep は不十分**(便 68 F6 が反証した失敗型)。 |