...
- **モデル**: 凍結済み transversal-cocycle(12 規則)を Q = Q_L で適用。点数 = |Q_L|·6 = 17496。GAP は G₃×H₃ の置換表現(G₃ は 3 ブロック×3 点 = **9 点**+H₃ の正則表現 27 点 = **36 点**上の直積 — v1.1 訂正: 当初「54 点」と誤記・WP3a 実装が正)で Q_L を作る。node は (G₃ triple [a,e]×3, H₃ (a,b,e)) の整数対で自前実装(§1 の積規約+fixture 自己検査)。
