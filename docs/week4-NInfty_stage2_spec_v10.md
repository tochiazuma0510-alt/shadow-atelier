# $N_\infty$ searcher — **stage 2 述語の仕様(spec v10・自己完結版)**

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 83)。v9 の同期版。**
...
| **H** | §6 pin = contract v4 / manifest v4・§9 exact_freeze_bundle = v9 束 | **contract v5(`00831f13…`)/ manifest v5(`46150ba0…`)へ**・**§9 の三 ID を v10 束へ** | 裁定 83・便 68 F11 |
...
## 0.0.-1 v8 → v9 erratum 差分【裁定 81】(v9 から継承・変更なし)
...
## 0.0.0 v7 → v8 erratum 差分【裁定 80】(v8 から継承・変更なし)
...
## 0.0.1 v6 → v7 erratum 差分【裁定 78】(v7 から継承・変更なし)
...
divisor_equality_certificate = {
  ...
  distinctness_witnesses,                       # §4.2 kind = disjointness【v7 新設】
...
>
> **手続き的具体化**: **verifier contract §3.1・§3.1.2**(W-2 / W-2′)。**本節と verifier contract は同じ型分けを述べる**【v10 で版中立化】。
...
public_envelope = {
  ...
  secondary_reason_codes[],         # canonical 昇順・primary を含まない【v8 新設・§5.4.1】
...
[26] verifier-result-mismatch  # 同一入力に対する A/B verifier result の不一致【v7 改名】
```
...
#### 5.3.3 二軸 routing — semantic 軸と concordance 軸【v8・erratum B】{#two-axis}
...
| **X-5** | **S2 で停止した場合も witness 検証と concordance 比較は実行する。** semantic の後段 reason は発しないが、**concordance の [26] は発する** — これが v7 との違い。 |
...
> **【v9 erratum D で訂正】invariant 1・3・4 は不変。invariant 2 は v9 で更新した**(v8 は「invariant 1–4 はそのまま成立する」と書きながら **invariant 2 が public secondary を禁止したままだった** — **自認**)。**invariant 5 を新設**して secondary の一意性も明示する。

### 5.4.1 `secondary_reason_codes[]`(public)【v8・erratum B】{#secondary}
...
```text
predicate_spec_id     = "mb/ninfty-stage2-predicate/v10"
...
# --- 実装契約(§4.4 が要求する欄の実体)【v8 erratum C・forward reference を消すため先に置く】---
verifier_contract_id     = "mb/ninfty-verifier-contract/v5"
verifier_contract_digest = 00831f1397cd51a28c9ef0fe5782827f56d40864aec40ef9ce98174c7962e4bd
...
# --- schema 群 (ii) external anchor: bound blob = 実体側の artifact 【v8 erratum A】---
...
18. **【必須・H】contract v5 / manifest v5 の pin と §9 の三 ID/digest**。**hash 順序 manifest → contract → spec → receipt** は不変。
...
20. **【推奨】manifest v5 §2.4 の `subject_build_binding_digest`(SB-1〜SB-4)と §2.5 の `build_attestation`(BA-1〜BA-5)の二層分離**を、spec 側の依存として承認できるか。**identity binding は取り違え防止まで・実生成関係は attestation 提出時のみ主張可・未実施なら `UNKNOWN`** という効力宣言でよいか。
...
```text
exact_freeze_bundle = {
  predicate_spec_id     = "mb/ninfty-stage2-predicate/v10",    predicate_spec_digest,
  verifier_contract_id  = "mb/ninfty-verifier-contract/v5",    verifier_contract_digest,
  dependency_manifest_schema_id = "mb/dependency-manifest/v5",  dependency_manifest_schema_digest
}
...
- **旧版(spec v6–v9・contract v1–v4・manifest v1–v4)の audit 結果は、本 bundle の gate を満たさない。**
...
```text
live_authority_refs[] = [
  { artifact_id: "mb/ninfty-verifier-contract/v5",
    digest_or_receipt_slot: "00831f1397cd51a28c9ef0fe5782827f56d40864aec40ef9ce98174c7962e4bd",
...
historical_quotation_refs[] = [
  { artifact_id: "mb/ninfty-stage2-predicate/v5..v9", note: "版履歴・差分表・自認文のみ" },
  { artifact_id: "mb/ninfty-verifier-contract/v1..v4", note: "差分表・自認文のみ" },
  { artifact_id: "mb/dependency-manifest/v1..v4", note: "差分表・自認文のみ" }
]
```
...
| **LA-3** | **sweep の対象定義**: 自 artifact の全版 token("spec v5..v10" / `mb/ninfty-stage2-predicate/v*`)と、**他 2 文書の全版 token**("contract v1..v5" / `mb/ninfty-verifier-contract/v*`・"manifest v1..v5" / `mb/dependency-manifest/v*`)。**自版だけを見る sweep は不十分** — 便 68 F6 が反証した失敗型であり、**裁定 83 に採録された。** |
