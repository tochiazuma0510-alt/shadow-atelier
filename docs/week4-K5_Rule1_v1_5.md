# 凍結 1(Rule 1)候補文書 — $K^{(5)}$ 橋の規約・正規形・抽出手順 v1.5
...
> **v1.4(2026-07-28・裁定 66)= v1.3 + amendment v8(`docs/amendment_5prime_draft.md` 条文案 A)+ TB4 導出 v2.4 の root seals/§7.4 確定案。旧正本 v1.3 は不変・digest 固定**(`docs/week4-K5_Rule1_v1.md` はこの版イベントで一切編集していない)。**版イベント発火許可 = 裁定 66 Part A**(`sol/裁定_66_ben54.md`)。本版が変更するのは **§8.4(bridge evaluation clause の捻れ形化)・§9.2(I-n 新設)・§7.4(quarantine 4 段監査の確定文言)・§1.4(root seals 追加)** のみ。**Part B 由来の whitelist/schema 修理(N_infty 隔離・h 非開示等)はここに混入しない**(裁定 66 F4 — 別ゲート)。凍結 1 の他条項・不変条項(§0.2)・モデル正規形(§2–§6)・§7.1–§7.3・§8.1–§8.3・§8.5 は**逐語不変**。

> **v1.5(2026-07-28・裁定 71・便 60 F5-6)= v1.4 + `Z-norm-seal/v1`(`docs/znorm_seal_final_v1.md`)への typed reference を §1.4.1 に新設。旧正本 v1.4 は上書きしない**(`docs/week4-K5_Rule1_v1_4.md` はこの版イベントで一切編集していない)。**既存 `Z20-link-seal/v1`(4 条)・既存 finite edge ID(`rule1-tb2-root-equality/v1`)は無変更**(便 60 F5 payload 項 6)。本版が変更するのは **§1.4.1 に新設した §1.4.2(full seal typed reference)のみ**であり、他の全節は v1.4 から逐語不変。
...
Z20-link-seal/v1 (scope = level_20):
  ...
  (3) root_equality_edge (named edge, 便 56 P56-2):
        ...
        rhs_object_id       = rule1_root_2M_id     # K5: M=10, 2M=20 (zeta_20^Rule1 の object ID。
                                                    # 本版(v1.4)で単一の field 名へ統一 — alias は残さない。
                                                    # 統一の経緯は本文書末尾の出所対応表を見よ)
        scope               = level_20
        certificate_digest  = zeta_20^TB2 = zeta_20^Rule1 の equality_certificate_digest
  (4) K^(5)/M|20 の全比較は、この同一の bar_iota_id・root_system_tb2_id・root_equality_edge
      (edge_id = "rule1-tb2-root-equality/v1"・rhs_object_id = rule1_root_2M_id・scope = level_20・
      certificate_digest 込み)を参照する(n が 20 の約数でない場合を量化しない)。
```
...
**(Z-norm)/`Z20-link-seal/v1` は A3 を証明しない。** A3 は (TB1)(TB3) と同格の枠組み事実であり【GAP-TB】に残る。本節が固定するのは root 選択の自由度であって、比較の向きではない。

**typed equality(TB4 導出 v2.4 §8.9・便 49 F6.6 — $\ell_i$ と $x=[\gamma_0]$ の同一性)**: Rule 1 §1.1 の底を $U_\lambda=\mathbf P^1_\lambda\smallsetminus\{0,1,\infty\}$ と書き、TB4/$B_{\rm FC}$ の $U_\beta$ と**座標同型 $\beta=\lambda$ により、接基点 $\vec{01}$・標準向き・ループ $\gamma_0$ を保って同一視する**。§7.1 の $\ell_i$ は、この**同じ** $x=[\gamma_0]$ が $\mathrm{Fib}_{\vec{01}}(W_0^{(i)})$ に誘導する permutation であり、**別の local generator を再定義したものではない**。

**状態札(TB4 導出 v2.4 §8.5・(Z20-link)/(Z-norm) 凍結後に限る表現)**:

$$ \begin{array}{ll}
\text{TB1, TB3, TB4}^{\rm u}\text{, A3} & = \text{global framework assumptions}\\
\text{TB2 + root seals(}Z20\text{-link-seal/v1)} & = \text{workshop conventions}\\
\text{TB4-A20} & = \text{finite theorem (}M\mid20\text{), conditional on }Z20\text{-link}\\
\text{TB4-B} & = \text{profinite theorem, conditional on }Z\text{-norm}
\end{array} $$
...
#### 1.4.2【v1.5 新設・裁定 71・便 60 F5-6】full seal(`Z-norm-seal/v1`)への typed reference

**身分**: 本節は §1.4.1 の `Z20-link-seal/v1`(scope = level_20)を**置き換えない**。`Z-norm-seal/v1`(scope = profinite)は**別 ID・別 scope**の seal であり、既存の `Z20-link-seal/v1` と既存 finite edge ID `rule1-tb2-root-equality/v1` は**この版イベントでも無変更**(便 60 F5 payload 項 6・`docs/znorm_seal_final_v1.md` §4「両者を『一部凍結/全体凍結』の関係で呼んではならない」)。本節が追加するのは、**full seal artifact への typed reference 1 ブロックのみ**である。

```text
full_seal_reference (typed reference — 個別 hash はここに転記しない。digest は event receipt を正とする):
  seal_id              = "Z-norm-seal/v1"
  seal_path            = docs/znorm_seal_final_v1.md
  scope                = profinite
  ...
  digest_authority     = event receipt(final_seal_sha256・component digest とも本節には転記しない)

unchanged (この版イベントでも変更しない):
  z20_link_seal_id     = "Z20-link-seal/v1"   (§1.4.1・scope = level_20・4 条は逐語不変)
  finite_edge_id       = "rule1-tb2-root-equality/v1"   (§1.4.1 (3)・lhs/rhs/scope/digest 束縛は不変)
```

**なぜ digest を転記しないか**: `docs/znorm_seal_final_v1.md` §9 は「final seal 自身に、それを含む receipt の digest を要求して循環させてはならない」と明記し、component 束の digest は event receipt が束縛する。本節は **artifact の所在(path)と scope・ID を型付けして参照する**にとどめ、個別 hash の転記(=版事故のたびに二箇所を同期させる負債)を作らない。

**`root_normalization_level = profinite` の昇格条件は本節では宣言しない**: 当該 record が `Z-norm-seal/v1` の (R1)(R2)(R3)(TB4 導出 v2.4 §8.6a・便 60 F5 payload 項 4)をすべて満たさない限り、`profinite` を宣言できない。本節の typed reference は**参照のみ**であり、Rule 1 の既存 window($K^{(5)}$)の `root_normalization_level` を自動的に書き換えない。
...
### 7.4 事前の見込みと、それを仮定しない規律(**v1.4 改定 — TB4 導出 v2.4 §8.4・便 48 F10.2 の文言を採用**)

§1.2–§1.3 の規約の下では $\sigma_0 = \tau_i(X)$ が定義であり、$c_i$ が一意な intertwiner であることから、**採用済み framework、TB2-norm / comparison seal、凍結 input がすべて正しく実現されている限り $b_i=1$ は定理である。$b_i\ne1$ は新しい算術現象として受理せず integrity quarantine とし、次の順に監査する: (1) 実装(左右・向きの事故)→ (2) transport → (3) input / root-system seal → (4) 紙上 framework 前件および証明。**
...
> **旧文(v1.3・撤回)**: 「$b_i=1$ が**期待される**」。TB4 導出 v2.4 §8.4 は「$b_i\ne1$ は必ず実装事故」という v1 案も**強すぎるとして撤回**した — TB4 は A1–A3 の framework-conditional な紙上定理なので、診断候補に**紙上前件・証明の誤り**も入る(4 段目)。
...
### 8.4 bridge evaluation clause(**v1.4 改定・条文案 A = `docs/amendment_5prime_draft.md` §2 の normative-only transfer**)
...
root_equality_edge_id = "rule1-tb2-root-equality/v1"     # §1.4.1 (3) の named edge と同一値
                                                          # (lhs_object_id = root_system_tb2_id,
                                                          #  rhs_object_id = rule1_root_2M_id,
                                                          #  scope = level_20)
equality_certificate_digest = root_equality_edge_id の certificate_digest  # §1.4.1 (3) の named edge から束縛
```
...
**$b_i$ は 8.4.0 (F2) の値。これが K5 campaign の operative bridge evaluation clause である。** 従来の exact **(5′)** は $b_i=1$ の特殊化であり、**次の 2 route のいずれかで回収される**:
> **(R-a) 現行 BFC proof を採る場合**: **(TB4)+$(Z_{2M}$-link$)$** の下で回収(BFC v2.11 §8・§8.1 の**現行 proof ID**・**current source**)。**本 campaign が採るのはこちら。**
...
**★ ord5 での観測 character の扱い**: $\mathrm{ord}([v_i]_{10})=5$ なら観測 character から指数を読み取れるが、その値は**凍結済み $b_i$ との照合にのみ**使用でき(**「verified/検証済み」は Lean 証明書に予約する。この照合の状態札は実際に記録された独立経路に従い、二経路の一致が閉じた場合に限り cross-checked とする。**)、**$b_i$ の選択には使えない**(8.4.3)。
...
> **理由**: $\mathrm{ord}([v_i]_{10})=1$ の分岐では $\kappa_i(G_K)=1$ となり、右辺は $b\in\{1,3,7,9\}$ の**四つすべてで自明**になる。したがって**この分岐で空虚になるのは $b$ の同定・選択**である。
...
**数学的には $b_{\rm op,sq}=b_{\rm op,ns}$ は定理である**(**共通の $\varepsilon$ と $\bar t_{10}$ から $b_{\rm op}=(\bar t_{10}\varepsilon)^{-1}$ が共通** — BFC 補題 B-9′(a)。**前件は (TB4$^{\rm u}$) 等の共通枠組み前件であり framework-conditional である**)。**したがって §7.3 は「二つの独立 transport が共通値を実現したかを見る negative control」**であり、**不一致は新現象ではなく実装 transport の破損** ⇒ **既存 `I-d` の即時 integrity stop がそのまま正しい分類**である(§7.3 は撤回されず、役割が「規約」から「integrity 検査」へ明確化される)。
...
**★ 版の区別**: **amendment 成立後**は `bridge_result_i` を含む campaign 全判定が $\varepsilon$-free になる。**成立前は Belyi-side の限定結論だけ**が無条件である(BFC v2.11 §10.1.2 (e)・**current source**)。
...
8. **(v1.4 新設)§8.4.0 (F4) の二段コミット schema がそのまま結果 record の該当欄になる**: `b_rule_commitment`(凍結 1)・`b_value_i = b_op`・`b_semantics = "op"`(固定・裸の $b$ を禁止)・`b_cmp_value`(別欄・`root_system_tb2_id` 必須)・`root_twist_2M_value`($t_{2M}$・link $\iff t_{2M}=1$)・`root_twist_mod_M_value`($\bar t_M$・$\bar t_M=1$ は link と同値でない)・`b_value_source`・`b_observed_before_gk`。**これらは `provenance/results_k5.md` の結果 record schema(manifest v1.6 §「結果 record schema」)と digest まで一致していなければならない** — 本節はその生成元(凍結 1 側)であり、`antecedent_bundle_id` の closed enumeration・`exact_recovery_path` の conditional presence(discriminator = `antecedent_bundle_id`)の**定義そのものは manifest v1.6 §3 に属する**(Rule 1 は §8.4.1 でこの 2 語を使用するのみで、値の列挙は manifest 側が正本)。
...
| # | 項目 | 担当 | 状態 |
|---|---|---|---|
| R-1 | §8.6/§10-3 の実装版・checker ID を**値として**記入(**裁定40/便39 F3・恒久設計**: 本行・版表とも commit ID の自己申告は行わず、blob hash のみを値として記入する。commit 状態の正本は git である) | 実装(P6 後半) | **閉**(記録当時 commit `16b18a7dc05fe94ec3b48967f1adad5a8a35013c` の内容として blob hash を版表に値記入 — 司令塔。**現在の commit 状態は本表が主張しない — git 参照**) |
...
| R-5(v1.2) | 実装側: 経路 A/B/第三 checker を副枝 (N$_\infty$)(§6.1 (b) A∞-1〜A∞-4・§6.2 B-iii・構造検査 (N∞-1)–(N∞-4)・(N$_\infty$) 用 raw schema §6.3-6)へ拡張 | 実装 | **production 較正済み(便 36 F3.2/F6-1・裁定 37 条件 1・2026-07-27。裁定41/便40 F2 注で raw schema は v3 に破壊的 version bump 済み — 下記参照)**。schema を三値 branch label `N_infty`・`M`(旧 `n` を置換)・`model_digest`・`expected_model_digest` を持つ形へ更新(初出時は v2。**裁定40/便39 F2 で `u-pathA-ninf/v3`・`u-pathB-ninf/v3` へ破壊的 version bump 済み**(旧 v2 raw は `P0_type`/`a_M`/`b_Mm3` を欠いており、同じ schema 名の下でこれらを必須化するのは受理言語の破壊的変更だったため — 詳細 `certificates/k5pipeline/retracted/NOTE.md`)。以下の記述で「schema v2」とあるのは**初出時点(便36)の記録**であり、現行 raw は v3)。`search/u-extract-pathA.g` の `ExtractPathA_Ninf` に (N∞-1)–(N∞-4) の fail-closed 検査(deg A=M・deg B=M-3・$b_{M-3}=a_M\ne0$・$A^2-B^2f$=定数=1・必要級数長 $\ge 2M+4$)と `gcd(f,f')` が単元であることの exact 検査(GAP 側 `PolyGcdIsUnit`・node 側は独立実装の Euclid 互除法)を追加。**Sol 提供の exact synthetic fixture**($p=x^2+1$・$a=1+x(x^2+1)^2$・$f=x^6+2x^4+x^2+2x$・$\lambda=\mu^2$ で $\deg A=10$・$\deg B=7$・$b_7=a_{10}=2$・$\hat c=1$)で `search/u-extract-pathA-ninf-production-driver.g` + `crosscheck/u-extract-pathB-ninf-production-driver.mjs` + `crosscheck/u-compare-ninf.mjs`(第三 checker・R-7 の expected digest 束縛も同時実装)を実行し、$u^{(A)}=u^{(B)}=1/4$・`result:"ACCEPT"`・digest 一致を確認(実行ログは本裁定対応の便報告に原文記載)。旧 M=3 玩具は `search/u-extract-pathA-ninf-toy-driver.g`/`crosscheck/u-extract-pathB-ninf-toy-driver.mjs` として**同一ライブラリの unit test**に位置づけ直し(現行 schema v3・chat=1 の passing fixture)、旧 chat=2 入力での fail-closed 停止は `crosscheck/check-r5-r8-ninf-fail-closed.mjs`(裁定41時点 38/38 PASS)で確認。詳細 `docs/week4-K5_Rule1_impl_versions.md` §9.3/§9.8/§9.9 |
...
| **R-8**(v1.3) | §2.2 M0 / §6.3-6 / §9.2 **I-m**: 枝ラベルを三値 enumeration とし、**未知ラベルの既定値 fallback を禁止**(fail-closed) | 実装 | **閉(便 37 F3/裁定 38 条件 2・2026-07-27)**。便 36 時点の修理は無条件 Weierstrass fallback バグを除去したが、`branchP0` という**単一 field** に大域枝(M0 の三値 $\{{\tt W},{\tt N\_aff},{\tt N\_infty}\}$)と局所 $P_0$ の Weierstrass 性を混在させており、Sol 便37 F3 の指摘どおり型が正本と異なっていた。本便で二軸に分離: 大域 field `branch`(main-path raw では $\{{\tt W},{\tt N\_aff}\}$、$N_\infty$ raw では常に `"N_infty"`)と局所 field `P0_type`($\{{\tt Weierstrass},{\tt nonWeierstrass}\}$)。整合規則も fail-closed に実装: `branch='W'` は `P0_type='nonWeierstrass'` を要求(S5-W 補題・SS4.1「v1.2 の絞り込み」)、`branch='N_aff'` のみ両値を許す、$N_\infty$ raw の `P0_type` は与えられれば `'nonWeierstrass'` のみ許す(補題 R1-M0 3.)。schema 名(`u-pathA/v3`・`u-pathB/v3`・`u-pathA-ninf/v3`・`u-pathB-ninf/v3`。(N$_\infty$) 側は裁定40/便39 F2 で v2→v3 へ破壊的 version bump — 旧 v2 raw は必須 field `P0_type`/`a_M`/`b_Mm3` を欠いていた)と枝ラベルの突合も第三 checker に追加。`crosscheck/check-r5-r8-ninf-fail-closed.mjs` で `branch='W'`+`P0_type='Weierstrass'` の拒否・`branch='N_aff'`+`P0_type='Weierstrass'` の受理などを含め **裁定41時点 38/38 PASS**(裁定40/便39 F1.2 の chat 実値検査攻撃 3 件+裁定41/便40 F1.2 の strict rational parser 攻撃 6 件を含む)。**現行(裁定42/便41 で司令塔独自攻撃 trim 除去 4 件を追加)は 42/42 PASS** -- 現行値は `docs/week4-K5_Rule1_impl_versions.md` の ACTIVE-BLOB-TABLE 該当行、および `node crosscheck/check-r5-r8-ninf-fail-closed.mjs` の実行結果を正本とする。 |
...
**現状(2026-07-27・便 37/裁定 38 修理反映後)**: R-3(実体は閉・Sol 検分済み)・R-4(閉)・R-5(production 較正済み)・R-6(数学 PASS・文言修理済み)・**R-7(閉・bundle 束縛へ修理)**・**R-8(閉・branch/P0_type 分離へ修理)**。**R-1・R-2 も閉**(記録当時 commit 16b18a7 の内容として blob hash の値記入と最終 digest 取得 = 司令塔・便 38。**現在の commit 状態は本文書が主張せず git を正本とする — 裁定40/便39 F3**)。これで R-1〜R-8 全閉。**その後、裁定39/便38 F2・裁定40/便39 F1.2/F1.3/F2 の修理で working tree はさらに変化している(最新の blob hash・adversarial 較正結果は `docs/week4-K5_Rule1_impl_versions.md` §9.7/§9.8 参照)。**
...
## 出所対応表(v1.5・便 56 差分ゲート用・裁定 68 で機械可読 5 欄へ修理・裁定 71 で v1.5 行を追加)

**欄の定義(便 55 F5・便 56 P56-1)**: `transfer_mode` ∈ {`verbatim`(転記元の逐語・版ラベルと節番号の付替えのみ), `normative-only`(規範内容は保存するが版履歴注記・自己訂正説明を省略), `adapted`(転記元から意味的に変更・型替えを行った)}。`verbatim(…を除く)` のような複合値は**禁止** — 例外を持つ転記は行を分割して別の `transfer_mode` を割り当てる。`source_range`(転記元の文書・節)/ `target_range`(本文書の節)/ `approval_id`(`adapted` のとき REQUIRED)/ `change_summary`(`adapted` のとき REQUIRED)。

| transfer_mode | source_range | target_range | approval_id | change_summary |
|---|---|---|---|---|
| normative-only | 裁定 66 Part A(`sol/裁定_66_ben54.md`) | 冒頭版履歴(v1.4 ボックス) | — | — |
| adapted | `docs/week4-TB4導出_opus_v1.md`(TB4 導出 v2.4)§8.1 | §1.4.1 `Z20-link-seal/v1`(4 条) | 裁定 67 | 便 55 F3.2 の型指定に基づき、全 $n$ を量化する 4 条(full `(Z-norm)`)を有限 level 20 限定(scope = level_20・$n\nmid20$ 非量化)へ縮小 |
| verbatim | 同 §8.2 | §1.4.1 A3 framework seal | — | — |
| verbatim | 同 §8.9(便 49 F6.6 の Rule 1 §7.1 追記案) | §1.4.1 typed equality($U_\lambda=U_\beta$・$\gamma_0$ 同一視) | — | — |
...
| normative-only | 同 §4「結果 record schema」(A7・A8・A13・A20 の該当欄)。closed enumeration と conditional presence の定義自体は manifest v1.6 §3 が正本 | §10 記録欄 8.(結果 record schema の生成元としての位置づけ) | — | — |
| normative-only | 裁定 71・便 60 F5-6(`sol/sol_reply_60_seal_spec_v2.md` payload 項 6) | 冒頭版履歴(v1.5 ボックス) | — | — |
| adapted | `docs/znorm_seal_final_v1.md`(`Z-norm-seal/v1` final・§1・§9 component 束) | §1.4.2(新設・full seal typed reference) | 裁定 71 | seal artifact の path・ID・scope・component 束の path のみを型付けて参照する形へ縮小。個別 hash は転記せず、digest authority を event receipt とする(便 60 F4.1/F4.5 の自己参照禁止を踏襲) |

**確認**: `transfer_mode` の分類は上表が悉皆である(便 56 F6)。上表以外に本文書が独自の判断を要する言い換えを追加することはない。
