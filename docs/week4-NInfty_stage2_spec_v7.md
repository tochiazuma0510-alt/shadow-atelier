# $N_\infty$ searcher — **stage 2 述語の仕様(spec v7・自己完結版)**

2026-07-28 起草: Claude(数学者レイヤー・Opus 5)。**司令塔委嘱(裁定 78)。v6 の erratum。**
...
erratum_scope = { §4.2 witness kind (math type error), [26] rename }
...
>
> **手続き的具体化**: `mb/ninfty-verifier-contract/v2` §3.1・§3.1.2(W-2 / W-2′)。**本節と contract v2 は同じ型分けを述べる。**
...
> **設計理由**: 証拠不信 [9]–[12] → 定理強制恒等式の破れ [13]–[15] → chart/locus 未処理 [16]–[19] → 大域整合 [20]–[24] → 二経路照合 [25]–[26](**[25]/[26] の述語は §5.3.3**)。
...
#### 5.3.3 `[25]` / `[26]` の相互排他な述語【v7・E2】

> **v6 の未閉鎖(自認)**: v6 は `[26]` に名前を与えただけで**述語を持たなかった**。素朴に「searcher/checker の native 不一致」を割り当てると、**native divisor が違えば、それを同一視する equality witness も必ず不成立**になり、**同一原因が [25] と [26](finite partition まで違えば [24] も)を同時発生させる**。

**評価順序(排他性はここで担保する — contract v2 §5.1 X-1〜X-4 と同期)**:

```text
step 1  envelope-level: leak / digest / dependency checks
        -> [9]..[12] を発して停止(witness 段へ進まない)
...
step 2  native cross-check: 両 native に対する specific な数学的検査
        ...
        -> [13]..[24] のうち該当する specific code を発して停止
...
step 4  A/B result 突合
        -> 同じ certificate・同じ native inputs に対する
           A/B verifier result の不一致 -> [26] verifier-result-mismatch
```

| 段 | 述語 |
|---|---|
| **[24]** | **両 native の finite aggregate partition が異なる**(step 2) |
| **[25]** | **native の一致が step 2 で確認された下で**、A と B が**ともに** witness の欠落・不成立を確認 |
| **[26]** | **同一 certificate・同一 native inputs に対する A/B verifier result の不一致** |

> **step は上から順に評価し、reason を発した段で停止する。** ゆえに [9]–[12] / [13]–[24] / [25] / [26] は**相互排他**。**step 2 で停止した場合も witness 検証は実行し結果を記録するが、reason code は発しない** — これが二重割当の回避点。
> **searcher/checker の native の実体差は step 2 の specific code へ送る。**「native 不一致を [26] に予約する」案は**不採用**(便 65 F5)。
...
```text
predicate_spec_id     = "mb/ninfty-stage2-predicate/v7"
...
# --- 実装契約(§4.4 が要求する欄の実体)---
verifier_contract_id     = "mb/ninfty-verifier-contract/v2"
verifier_contract_digest = 1fd36b3eda0da33b2aba5d3d371a24749850b9b05a3f4c4f17ef1725ffe555bd
dependency_manifest_schema_id     = "mb/dependency-manifest/v2"
dependency_manifest_schema_digest = c485e0166da1aa4b9c34474dbb49d1ed18d4c3f1c29ccba14614dbd4dcbb56d2
...
## 8. Sol への監査依頼(v7 — **erratum の 2 点に限る**)
...
9. **【推奨】改名 `verifier-result-mismatch`** で spec と contract v2 の語彙が同期したか。
