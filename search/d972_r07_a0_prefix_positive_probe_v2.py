#!/usr/bin/env python3
"""Task433: positive-only fork of the authenticated v12 occurrence prefix."""
...
INPUT_BYTES = 461087575
INPUT_SHA = "8918df4407e91a7b4ab1a29246a23ba5b0ed1a7b6011f4abf74775cc33d82705"
...
def load_v12():
    ...
    mod = types.ModuleType("task433_v12_pinned")
    mod.__file__ = str(p)
...
def main():
    ...
    try:
        if a.mode == "FIXTURE":
            out = fixture()
            print("R07_A0_PREFIX_POSITIVE_PROBE_V2_FIXTURE_PASS " + json.dumps(out, sort_keys=True, separators=(",", ":")), flush=True)
            return 0
        ...
        print("R07_A0_PREFIX_POSITIVE_PROBE_V2 " + envelope["status"], flush=True)
        return 0
    except Exception as exc:
        print("R07_A0_PREFIX_POSITIVE_PROBE_V2 " + UNKNOWN + " " + str(exc), flush=True)
        return 0
