...
def _target(root: Path, live_parent: bool = False) -> tuple[np.ndarray, dict[str, Any]]:
    manifest, _ = _read_json(root / "manifest.json", "checker_rho2_manifest")
    require({"schema", "marker", "files", "lower_zero_coordinates", "rho2_sha256", "rho2_dense_sha256", "lower_dense_sha256"}.issubset(manifest) and
            manifest["schema"] == TARGET_PAYLOAD_SCHEMA and manifest["lower_zero_coordinates"] == 32260, "checker_rho2_shape")
    ...
    if fixture:
        ...
        require((root / "manifest.json").stat().st_size == RHO2_MANIFEST["bytes"] and
                sha((root / "manifest.json").read_bytes()) == RHO2_MANIFEST["sha256"],
    ...
    require(len(packed) == PHYSICAL_BYTES and len(dense) == PHYSICAL_WIDTH and len(lower) == 32260 and
            sha(packed) == manifest["rho2_sha256"] and sha(dense) == manifest["rho2_dense_sha256"] and sha(lower) == manifest["lower_dense_sha256"], "checker_rho2_digest")
...
def selftest() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="d972-r07-check-state-v1-") as td:
        base = Path(td); connection = _make_connection(base / "connection"); member_target = np.zeros(PHYSICAL_WIDTH, dtype=np.uint8); member_target[[100, 10, 300]] = 1; target = _make_target(base / "target", member_target); state = base / "state"; output = base / "output"; launch = _fixture_state_and_terminal(base, connection, target, state, output); report_member = check_launch(launch); require(report_member["kind"] == "ConnectionMember", "checker_member_fixture")
