...
TASK = Path("sol/luna_task_157ej_b345_lexfirst_block_target6_v2.md")
TASK_SHA = "1d6f14ef9f799a43fc344daa38606281dbcf0b2fe47d86db0b44df382762f290"
TASK_BYTES = 14667
...
V1_PRODUCER = Path("search/d972_b345_lexfirst_block_target6_v1.py")
V1_PRODUCER_SHA = "f901cffd73069e78c9cc256e1a6c18c7e7ce6adef6d4de0c4fe68970571476bb"
V1_PRODUCER_BYTES = 143075
V1_CHECKER = Path("search/check_d972_b345_lexfirst_block_target6_v1.py")
V1_CHECKER_SHA = "d0601533131008002d09a6320ab643df865a2a86245ed23f399e4c469bd93c57"
V1_CHECKER_BYTES = 128399
V1_DRIVER = Path("search/d972_b345_lexfirst_block_target6_gha_driver_v1.g")
V1_DRIVER_SHA = "e0cb01bf119ae7834fa85da7910c6dd82048c8ae756e48f834fad055a7bc4c0a"
V1_DRIVER_BYTES = 10516
V1_REPLY = Path("sol/luna_reply_157ei_b345_lexfirst_block_target6.md")
V1_REPLY_SHA = "de6c22867a7a66cb28fdbbffae2f92632e8dfc382a5f7088a097d7518cef2ad2"
V1_REPLY_BYTES = 13277
V1_TASK = Path("sol/luna_task_157ei_b345_lexfirst_block_target6.md")
V1_TASK_SHA = "cfe0c50046a750e4169c473872c5770ce76c105267353e82c9ed19de01c043f4"
V1_TASK_BYTES = 24179
...
PIN_SPECS = {
    "157ej_task": (TASK, TASK_SHA, TASK_BYTES),
    "157ei_v1_producer": (V1_PRODUCER, V1_PRODUCER_SHA, V1_PRODUCER_BYTES),
    "157ei_v1_checker": (V1_CHECKER, V1_CHECKER_SHA, V1_CHECKER_BYTES),
    "157ei_v1_driver": (V1_DRIVER, V1_DRIVER_SHA, V1_DRIVER_BYTES),
    "157ei_v1_reply": (V1_REPLY, V1_REPLY_SHA, V1_REPLY_BYTES),
    "157ei_v1_task": (V1_TASK, V1_TASK_SHA, V1_TASK_BYTES),
...
def pin_rows(q3_path: Path) -> dict[str, Any]:
    ...
    rows["157eh_run_evidence"] = {"run": "32374248796",
        ...
        "evidence_only_not_imported": True}
    rows["157ei_failed_run_evidence"] = {
        "run": "32391706973", "attempt": 1,
        "head": "8045966623b17c264567798032ca35b73c7e3ea6",
        "conclusion": "failure", "artifact_count": 0,
        "receipt_exists": False, "mathematical_result": False,
        "failure_locus": "WordExpr flat value/gradient",
        "evidence_only_not_imported": True}
    return rows
...
def exact_pinned_value_roots(target_root: int,
                             source_roots: Sequence[int],
                             node_count: int, *,
                             proposed: Sequence[int] | None = None) \
        -> list[int]:
    """Exact target-first value coverage for the six pinned source roots."""
    sources = list(source_roots)
    require(type(node_count) is int and node_count > 0 and
            len(sources) == 6 and type(target_root) is int and
            1 <= target_root <= node_count and
            all(type(root) is int and 1 <= root <= node_count
                for root in sources),
            "157ej pinned value-root node contract")
    expected = [target_root, *sources]
    roots = list(expected if proposed is None else proposed)
    require(roots == expected and
            set(roots) == ({target_root} | set(sources)),
            "157ej exact target/source value-root coverage and order")
    return roots


def _base_pinned_candidate_values(old: Any, compiled: dict[str, Any],
                                  e4: Any, static: Any, target_root: int,
                                  *, proposed_value_roots:
                                  Sequence[int] | None = None,
                                  evaluator_call: Callable[..., Any] | None =
                                  None) -> Any:
    """Production-shared call boundary; pins stay enabled and ordered."""
    roots = exact_pinned_value_roots(
        target_root, compiled["source_roots"], len(compiled["dag"].opcode),
        proposed=proposed_value_roots)
    call = old._affine_candidate_values if evaluator_call is None \
        else evaluator_call
    require(callable(call), "157ej pinned evaluator call")
    return call(compiled, e4, 0, static, pin_sources=True,
                value_roots=roots)
...
def _target6_system_core(old: Any, seed_info: dict[str, Any], e4: Any,
    ...
    base_eval = _base_pinned_candidate_values(
        old, base_compiled, e4, static, root)
...
def self_test() -> None:
    ...
    coverage_sources = [11, 12, 13, 14, 15, 16]
    coverage_expected = [7, *coverage_sources]
    coverage_compiled = {"dag": CoverageDag(),
                         "source_roots": list(coverage_sources)}
    coverage_calls: list[tuple[tuple[Any, ...], dict[str, Any]]] = []

    def coverage_recorder(*args: Any, **kwargs: Any) -> str:
        coverage_calls.append((args, kwargs)); return "coverage-ok"

    require(_base_pinned_candidate_values(
                None, coverage_compiled, "toy-e4", "toy-static", 7,
                proposed_value_roots=coverage_expected,
                evaluator_call=coverage_recorder) == "coverage-ok" and
            len(coverage_calls) == 1 and
            coverage_calls[0][0] == (coverage_compiled, "toy-e4", 0,
                                      "toy-static") and
            coverage_calls[0][1] == {
                "pin_sources": True, "value_roots": coverage_expected},
            "157ej production wrapper exact coverage keywords")
    omission_rejections = 0
    for index in range(6):
        omitted = coverage_expected[:index+1]+coverage_expected[index+2:]
        _expect_failure(lambda roots=omitted: exact_pinned_value_roots(
            7, coverage_sources, 20, proposed=roots),
            f"source omission {index+1}")
        omission_rejections += 1
    _expect_failure(lambda: exact_pinned_value_roots(
        7, coverage_sources, 20, proposed=[7]),
        "v1 target-only value roots omit all six sources")
    _expect_failure(lambda: exact_pinned_value_roots(
        7, coverage_sources, 20, proposed=coverage_sources),
        "target omission")
    reordered = list(coverage_expected)
    reordered[1], reordered[2] = reordered[2], reordered[1]
    _expect_failure(lambda: exact_pinned_value_roots(
        7, coverage_sources, 20, proposed=reordered),
        "source reordering")
    _expect_failure(lambda: exact_pinned_value_roots(
        7, coverage_sources, 20, proposed=coverage_expected+[17]),
        "extra value root")
    _expect_failure(lambda: exact_pinned_value_roots(
        7, [11, 12, 13, 14, 15, 0], 20),
        "malformed non-node source root")
    require(omission_rejections == 6,
            "157ej every source omission rejected")
    inherited = load_eh()
    inherited.self_test()
    ...
    print("D972_B345_LEXBLOCK_TARGET6_V2_PRODUCER_SELFTEST_PASS "
          ...
          "value_root_union=1 source_omission_rejected=1 "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("D972_B345_LEXBLOCK_TARGET6_V2_PRODUCER_PASS "
