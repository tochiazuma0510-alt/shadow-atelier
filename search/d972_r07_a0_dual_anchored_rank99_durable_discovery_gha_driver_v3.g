#############################################################################
## R07 durable discovery v3 driver. ASCII only.
#############################################################################
if not IsBound(D972_R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_RUN) or
   D972_R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_RUN<>true then
 Error("task482 external run preamble required"); fi;
if not IsBound(D972_R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_INPUT) then
 D972_R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_INPUT:=
  "search/certs/d972_r07_a0_dual_anchored_rank99_candidate_v1.json";; fi;
D482Input:=D972_R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_INPUT;;
D482Producer:="search/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.py";;
D482Checker:="crosscheck/check_d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.py";;
...
D482ProducerBytes:=100066;; D482ProducerSHA:="90bd58dce838eb518da7b32d8eaec210223efdee6a35d5f98d404e57517615a1";;
D482CheckerBytes:=66854;; D482CheckerSHA:="70540c60f0685539d21ca5a23c10cdacb840c4317b93b88fa57fb89fc7398c35";;
...
D482Receipt:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.json";;
D482Checkpoint:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.checkpoint";;
D482Verdict:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.checker.json";;
D482PLog:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.producer.log";;
D482CLog:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.checker.log";;
D482Script:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.sh";;
D482OK:="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.ok";;
...
if not D482Safe(D482Input) then Error("task482 unsafe resume input"); fi;
if IsExistingFile(D482Receipt) or IsExistingFile(D482Checkpoint) or IsExistingFile(D482Verdict) or
 ...
 Error("task482 stale output"); fi;
Exec("mkdir -p ci/out");;
D482S:=OutputTextFile(D482Script,false);; if D482S=fail then Error("task482 script open"); fi;
...
PrintTo(D482S,"test \"$(grep -Ec '^R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3 status=(COMMON_CANDIDATE|UNKNOWN_RESOURCE)$' ",D482PLog,")\" = 1\n");;
PrintTo(D482S,"if grep -Eq '^R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3 status=UNKNOWN_RESOURCE$' ",D482PLog,"; then\n");;
PrintTo(D482S," test -s ",D482Receipt," && test -s ",D482Checkpoint,"\n");;
PrintTo(D482S," grep -Eq '\"candidate_marker\":\"R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_RESOURCE_CANDIDATE\"' ",D482Receipt,"\n");;
...
PrintTo(D482S," printf '%s\\n' 'R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_RESOURCE_CANDIDATE' > ",D482OK,"\n");;
PrintTo(D482S," exit 0\nfi\n");;
PrintTo(D482S,"test \"$(grep -Ec '^R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3 status=COMMON_CANDIDATE$' ",D482PLog,")\" = 1\n");;
...
PrintTo(D482S,"test \"$(grep -Ec '^R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_CHECKER_PASS terminal=COMMON_CANDIDATE$' ",D482CLog,")\" = 1\n");;
PrintTo(D482S,"test -s ",D482Verdict,"\n");;
PrintTo(D482S,"printf '%s\\n' 'R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_COMPLETE' > ",D482OK,"\n");;
CloseStream(D482S);; Exec(Concatenation("bash ",D482Script));
if not IsExistingFile(D482OK) then Error("task482 missing success marker"); fi;
Print("R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3_COMPLETE\n");
