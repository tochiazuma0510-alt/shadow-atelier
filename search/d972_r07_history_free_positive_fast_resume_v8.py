#!/usr/bin/env python3
"""R07 v8 audited history-free positive resume.
...
candidate for the helper-nonshared v8 checker, never a self-authorized claim.
...
SCHEMA = "d972-r07-history-free-positive-fast-resume/v8"
CHECKPOINT_SCHEMA = SCHEMA + "/checkpoint"
COMMON = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V8_COMMON_WORD"
SELFTEST_TERMINAL = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V8_SELFTEST_PASS"
UNKNOWN_INPUT = "UNKNOWN_INPUT"
UNKNOWN_RESOURCE = "UNKNOWN_RESOURCE"
PRODUCER_PREFIX = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V8_PRODUCER_TERMINAL"
...
SOURCE_PINS: dict[str, tuple[str, int, str]] = {
    ...
    "fixture": ("search/certs/d972_r07_history_free_positive_fast_resume_selftest_v8_20260829.json", 2784,
...
class Meter:
    """Fresh v8 counters.  Historical checkpoint counters are never seeded."""
    ...
    def public(self) -> dict[str, Any]:
        return {"phase": self.phase,
                ...
                "limits": self.limits, "fresh_v8_counters": self.counters,
...
class FormalReducer:
    ...
    def expand(self, node: int) -> dict[str, int]:
        return self.ancestry.expand(node)

    def add_actual(self, source: dict[bytes, int], symbol: str) -> tuple[bytes, dict[str, int]]:
        row = dict(source); node = self.ancestry.literal({symbol: 1})
        for pivot in self.order:
            value = row.get(pivot, 0)
            if value:
                self.live.add_scaled(row, self.rows[pivot], -value)
        ...
        expression = self.ancestry.expand(node)
        # ``node`` already carries the row normalization.  Expanding it a
        # second time by ``inverse`` would double-scale every pivot-2 row.
        self.inject(pivot, row, expression, expression_node=node)
...
class Search:
    def __init__(self, runtime: dict[str, Any], registry: SourceRegistry,
                 ...
                 workers: int, selftest: dict[str, Any],
    ...
    def checkpoint_body(self, phase: str) -> dict[str, Any]:
        ...
        return {"schema": CHECKPOINT_SCHEMA, "phase": phase,
            ...
            "formal_solution": [[key, value] for key, value in sorted(
                self.reducer.expand(solution_node).items())],
    ...
    def write_checkpoint(self, phase: str, terminal_checkpoint: bool) -> dict[str, Any]:
        ...
        if self.checkpoint_sequence == 1:
            ...
            checkpoint_path = self.output.with_name(
                self.output.name + f".checkpoint.{self.checkpoint_sequence}.{phase}.json")
        size, digest = atomic_json(checkpoint_path, checkpoint)
    ...
    def bind_section_identity(self, section: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
        ...
        result.update({"q0_state_hex": bytes(qstate).hex(),
            ...
            "gamma_state_hex": p176.packed_joint_blob(
                gamma_state, "v8 Gamma provenance").hex(),
            "gamma_state_sha256": sha_bytes(p176.packed_joint_blob(
                gamma_state, "v8 Gamma provenance")),
...
def restore_checkpoint(search: Search, path: Path) -> None:
    ...
    require(prior_monitor.get("limits") == search.meter.limits and
            type(prior_monitor.get("fresh_v8_counters")) is dict,
            "v7 checkpoint monitor")
    for name, counter in prior_monitor["fresh_v8_counters"].items():
    ...
    require(formal.get("new_expressions") == [record["pivot_expression"]
            ...
            formal.get("dag_nodes") == [list(node) for node in search.reducer.ancestry.nodes] and
    ...
    require(live.public_sparse(remainder) == value.get("remainder") and
            [[key, coefficient] for key, coefficient in sorted(
                search.reducer.expand(solution_node).items())] ==
    ...
    if remainder:
        ...
        require(derived_remainder == remainder and derived_solution_node == solution_node and
                ...
                "v8 freshly derived resume dual")
        ...
        require(progress_dual in (None, live.sha_obj(value.get("current_dual"))),
                "v8 correction cursor dual binding")
...
def run_real_owner_selftest(runtime: dict[str, Any], old_value: dict[str, Any],
    ...
    require(fixture.get("schema") ==
            "d972-r07-history-free-positive-fast-resume/selftest-input/v8" and
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--mode", choices=("SELFTEST", "PRODUCTION"), default="PRODUCTION")
    value.add_argument("--source", type=Path, required=True)
    value.add_argument("--manifest", type=Path, required=True)
    value.add_argument("--output", type=Path, required=True)
    value.add_argument("--resume", type=Path)
    value.add_argument("--resume-manifest", type=Path)
    value.add_argument("--selftest-receipt", type=Path)
...
def require_selftest_identity(path: Path, registry: SourceRegistry) -> dict[str, Any]:
    """Production accepts only an already sealed, exact v8 SELFTEST receipt."""
    flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
    try: fd = os.open(path, flags)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    resume_manifest_path = None if args.resume_manifest is None else (
        args.resume_manifest if args.resume_manifest.is_absolute() else
        ROOT / args.resume_manifest)
    if resume_path is not None:
        require(resume_manifest_path is not None,
                "resume_manifest_required")
        resume_manifest, _manifest_physical = read_bounded_json(
            resume_manifest_path, 16 * 1024 * 1024)
        _resume_probe, resume_physical = read_bounded_json(
            resume_path, MAX_CHECKPOINT_BYTES)
        require(resume_manifest.get("schema") == CHECKPOINT_SCHEMA + "/portable-manifest" and
                resume_manifest.get("code_schema") == SCHEMA and
                resume_manifest.get("bytes") == resume_physical["bytes"] and
                resume_manifest.get("sha256") == resume_physical["sha256"],
                "resume_manifest_binding")
    if output.exists():
        raise ProtocolStop("stale output")
    ...
    try:
        ...
        if args.mode == "SELFTEST":
            ...
            receipt = seal({"schema": SCHEMA + "/selftest", "status": "PASS",
                "terminal": SELFTEST_TERMINAL,
                "fixture_sha256": SOURCE_PINS["fixture"][2],
                "selftest": selftest, "claims": dict(FALSE_CLAIMS),
                "execution": "UNEXECUTED_ARTIFACT_ONLY"})
        else:
            require(args.selftest_receipt is not None,
                    "production_requires_accepted_selftest_identity")
            selftest = require_selftest_identity(
                args.selftest_receipt if args.selftest_receipt.is_absolute() else
                ROOT / args.selftest_receipt, registry)
            search = Search(runtime, registry, source_public, old_value, reducer, p_rows,
                            triangular, meter, output, args.workers, None,
                            defer_owner_start=resume_path is not None)
            if resume_path is not None:
...
if __name__ == "__main__":
    try:
        ...
        print("R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V8_PRODUCER_STOP " +
