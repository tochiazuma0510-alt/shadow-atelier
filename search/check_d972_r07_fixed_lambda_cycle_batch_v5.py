...
PARENT_ROLES = (*V4_PARENT_ROLES, "batch-parent-v4")
PRODUCER_FILE = "search/d972_r07_fixed_lambda_cycle_batch_v5.py"
CHECKER_FILE = "search/check_d972_r07_fixed_lambda_cycle_batch_v5.py"
CHECKER_WORKFLOW = ".github/workflows/d972-r07-fixed-lambda-cycle-batch-v5.yml"
...
def check_acceptance_header(value: Any) -> None:
    require(type(value) is dict and set(value) == {"schema", "parents", "anchor", "batch_anchor", "next_batch_anchor", "code", "runtime", "registration"} and
            value["schema"] == SCHEMA + ".acceptance", "acceptance_exact_eight_plain_keys")
...
class AcceptedInputs:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        check_next_parent_roles(value["parents"])
...
def root_records(inputs: AcceptedInputs, anchor: ThinAnchor, old: dict[str, Any], bundle: C.FixedBundle) -> RootRecords:
    ...
    layout = document("parent-layout", {"portable_acceptance_sha256": inputs.portable_sha256,
        **{key: copy.deepcopy(inputs.portable[key]) for key in ("parents", "anchor", "batch_anchor", "next_batch_anchor", "code", "runtime", "registration")}})
    ...
    start = document("start", {"owner_sha256": sha(canonical(owner)), "source_sha256": sha(canonical(source)),
        ...
        "previous_parent_target_derivations": len(inputs.intermediate_parents),
...
def batch_observation(inputs: AcceptedInputs, records: RootRecords, files: CandidateFiles,
    ...
    if not admitted:
        sequence = -1
    batch = inputs.value["next_batch_anchor"]
    parents = {item["role"]: item["artifact"] for item in inputs.portable["parents"]}
    accepted = inputs.next_batch_parent["records"]
    old = {"parent_role": "batch-parent", "parent_artifact": copy.deepcopy(parents["batch-parent"]),
    ...
    current = {"parent_role": "batch-parent-v4", "parent_artifact": copy.deepcopy(parents["batch-parent-v4"]),
    ...
    if admitted:
        ...
        require(pairing["rows"] == NEXT_BATCH_RANK and pairing["lambda_pivots"] == 0 and
...
def check_actual(args: argparse.Namespace, report: dict[str, Any]) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        anchor = stack.enter_context(promote_next_batch_parent(inputs, intermediate, old))
...
def k128_registration_canary(root: Path) -> dict[str, Any]:
    ...
    acceptance = {"schema": SCHEMA + ".acceptance", "parents": [], "anchor": {}, "batch_anchor": {}, "next_batch_anchor": {}, "code": {},
    ...
    for side in ("producer", "checker"):
        ...
        bad[side]["file"] = bad[side]["file"].replace("_v5.py", "_v4.py")
...
def selftest(root: Path) -> dict[str, Any]:
    ...
    require([len(item["rejected_cases"]) for item in (first, second, third, fourth)] == [28, 9, 6, 7],
            "registered_C_four_group_rejection_counts")
    return document("selftest", {"status": "PASS", "tests": [first, second, third, fourth],
        "fixture_scope": "new-k128-registration-and-portable-metadata;synthetic-full-54433-rosters-and-physical-dependent-continuation;real-reduction-phase-and-candidate-publication-reader;synthetic-parent1578-header-projection-target-hash;synthetic-parent1706-two-layer-metadata;inherited-dependent-negative-stops-at-expected-file-size-hash-before-semantic-outcome-comparison;all-fixtures-retained;no-accepted-parent-or-earlier-five-E-phase-arithmetic",
    ...
    try:
        if args.selftest:
            ...
            require(all(getattr(args, role.replace("-", "_") + "_root") is not None for role in PARENT_ROLES if not role.startswith("block-")) and
                    ...
                    "all_seventeen_roots_and_exact_acceptance")
