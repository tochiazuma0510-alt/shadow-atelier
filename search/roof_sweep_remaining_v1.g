...
ScanRoofHexagonCapped := function(qrec, charmingSet, capSeconds)
  ...
  if thetaHom = fail or tauHom = fail then
    return rec(candidate_total := 0, shadow_total := 0, shadows := [],
               ...
               theta_tau_not_well_defined := true, derived_subgroup_too_large := false,
               elapsed_seconds := 0.0);;
  fi;
  D := DerivedSubgroup(G);
  # self-caught (roof_sweep_v1.g, 裁定1059/1070中に発見・K(81) x 1152[a] roofで実際に
  # クラッシュ発生: "reached the pre-set memory limit" at Elements(D)): |D| を Size() で
  # 先に測ってから Elements() を呼ぶ(Size()はSchreier-Simsで比較的軽く、Elements()は
  # 全元をメモリ上に展開するため巨大な D で OOM する)。閾値超過ならこのroofをUNKNOWNとして
  # fail-closedに次へ進む(GAPのbreak loopに落ちて全体が止まるのを防ぐ)。
  DERIVED_SUBGROUP_SIZE_CAP := 300000;;
  if Size(D) > DERIVED_SUBGROUP_SIZE_CAP then
    return rec(candidate_total := 0, shadow_total := 0, shadows := [],
               derived_order := Size(D), capped := false,
               theta_tau_not_well_defined := false, derived_subgroup_too_large := true,
               elapsed_seconds := 0.0);;
  fi;
  Delts := Elements(D);
  ...
  return rec(candidate_total := candidateTotal, shadow_total := Length(shadows), shadows := shadows,
             ...
             theta_tau_not_well_defined := false, derived_subgroup_too_large := false,
...
RunRoof := function(leftW, rightW, capSeconds)
  ...
  if resM.derived_subgroup_too_large then
    out.status := "UNKNOWN_DERIVED_SUBGROUP_TOO_LARGE";;
    return out;;
  fi;
  if resM.capped then
  ...
  # self-caught (裁定1059/1070中に発見): composition-table構築(shadow_total^2 のオーダー、
  # Ehom構成 shadow_total 回)にはScan段のcapが及ばない -- K(15)x1152[a](shadow_total=2880)は
  # scan自体は速かったがcomposition段だけで約960秒かかり、合計1032秒とcap 600秒を大幅に
  # 超過した。ここでshadow_total自体に閾値を設け、composition構築前にfail-closedで判定。
  SHADOW_TOTAL_COMPOSITION_CAP := 2000;;
  if resM.shadow_total > SHADOW_TOTAL_COMPOSITION_CAP then
    out.status := "UNKNOWN_SHADOW_TOTAL_TOO_LARGE_FOR_COMPOSITION";;
    return out;;
  fi;
