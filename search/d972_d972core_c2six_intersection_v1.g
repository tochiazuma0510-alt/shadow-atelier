#############################################################################
## d972_d972core_c2six_intersection_v1.g
##
## 157bd: joint E^4/G9^4 image for the four pinned PB4 strand deletions.
## The producer constructs the joint image and asks GAP for lossless source
## words for the 24 basis elements of the E^4 kernel of the G9^4 projection.
...
D972BDOutput := Filename(DirectoryTemporary(),
  "d972_d972core_c2six_intersection_v1.json");;
D972BDMode := "full";;
...
if D972BDMode<>"selftest" then
  D972BDJointGens:=List([1..6],j->D972BDJoint(D972BDTupleRows[j],
    D972BDTupleG9Rows[j]));;
  D972BDJointGroup:=Group(D972BDJointGens);;
  D972BDFree:=FreeGroup(6,"d972_joint");;
  D972BDFreeGens:=GeneratorsOfGroup(D972BDFree);;
  D972BDJointEpi:=GroupHomomorphismByImages(D972BDFree,
    D972BDJointGroup,D972BDFreeGens,D972BDJointGens);;
  if D972BDJointEpi=fail then Error("157bd: joint free epimorphism failed"); fi;;
  for D972BDCoord in [1..4] do
    for D972BDBasisIndex in [1..6] do
      D972BDEVals:=[];;
      for D972BDJ in [1..4] do
        if D972BDJ=D972BDCoord then
          Add(D972BDEVals,D972BDModule[D972BDBasisIndex]);
        else
          Add(D972BDEVals,One(D972BDE));
        fi;
      od;;
      D972BDPVars:=List([1..4],j->One(D972BDP));;
      D972BDGVals:=List([1..4],j->One(D972BDG9));;
      D972BDTarget:=D972BDJoint(D972BDEVals,D972BDGVals);;
      D972BDWord:=PreImagesRepresentative(D972BDJointEpi,D972BDTarget);;
      if D972BDWord=fail or
         Image(D972BDJointEpi,D972BDWord)<>D972BDTarget then
        Error("157bd: W preimage failed at coordinate ",D972BDCoord,
...
if D972BDMode="selftest" then
  Print("D972_CORE_INTERSECTION_GAP_SELFTEST_PASS maps=4 tuples=6 g9=2916 rank=24\n");
else
  D972BDGEOrder:=Size(D972BDE)^4;;
  D972BDGPOrder:=Size(D972BDP)^4;;
  D972BDReceipt:=rec(
    schema:="d972-d972core-c2six-intersection/v1",
    final_marker:="D972_CORE_INTERSECTION_FINAL",
    status:="CORE_INTERSECTION_COMPUTED",
    ...
    proof_level:="GAP joint-image preimages plus independent Python replay; rank equality uses Goursat",
  ...
  Print("D972_CORE_INTERSECTION_FINAL status=CORE_INTERSECTION_COMPUTED output=",
