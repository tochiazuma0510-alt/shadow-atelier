#!/usr/bin/env python3
"""Task1191 draft: independent fixed-lambda, seven-layer parent2346 checker.
...
SCHEMA = "d972.r07.fixed-lambda-cycle-batch.v10"
...
PARENT_ROLES = (*V9_PARENT_ROLES, "batch-parent-v9")
PRODUCER_FILE = "search/d972_r07_fixed_lambda_cycle_batch_v10.py"
CHECKER_FILE = "search/check_d972_r07_fixed_lambda_cycle_batch_v10.py"
CHECKER_WORKFLOW = ".github/workflows/d972-r07-fixed-lambda-cycle-batch-v10.yml"
...
CURRENT_PRODUCER_REGISTRATION: dict[str, Any] | None = {'file': 'search/d972_r07_fixed_lambda_cycle_batch_v10.py', 'bytes': 1034265, 'sha256': '33c4bbb97313bc1ea2017b6ac6ad2cc0932ae15d1affc8bcd6a8aaedd0c76085'}
...
class SavedParentJsonBytes:
    ...
    def __init__(self, ordinal: int):
        ...
        self.rows = {(role, kind): {key: 0 for key in self.metrics}
            for role in (BATCH_PARENT_ROLE, NEXT_BATCH_ROLE, THIRD_BATCH_ROLE, FOURTH_BATCH_ROLE, FIFTH_BATCH_ROLE, SIXTH_BATCH_ROLE, SEVENTH_BATCH_ROLE)
    ...
    def emit(self, status: str) -> None:
        ...
        emit_auxiliary_diagnostic({"schema": "d972.r07.saved-parent-json-parse-bytes.v1", "side": "C",
            ...
            "measurement_scope": "same-PinnedTree.json-read-to-json_value;seven-saved-roles;candidate6/reduction-and-physical-literal;successful-canonical-parse-not-later-seal-or-schema;unique-first-successful-role-path",
...
def check_acceptance_header(value: Any) -> None:
    require(type(value) is dict and len(value) == V10_CURRENT_ACCEPTANCE_KEY_COUNT and set(value) == {"schema", "parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "batch_anchor_v8", "batch_anchor_v9", "code", "runtime", "registration"} and
            value["schema"] == SCHEMA + ".acceptance", "acceptance_exact_thirteen_plain_keys")
...
class AcceptedInputs:
    ...
    def __init__(self, args: argparse.Namespace, parse_bytes: SavedParentJsonBytes):
        ...
        check_seventh_parent_roles(value["parents"])
        require(type(SEVENTH_BATCH_INVENTORY_REGISTRATION) is dict and type(CURRENT_PRODUCER_REGISTRATION) is dict,
...
def fifth_batch_json(tree: PinnedTree, name: str, suffix: str) -> dict[str, Any]:
    value = tree.json(name, True)
    check_document(value)
    require(value["schema"] == FIFTH_BATCH_SCHEMA + "." + suffix, "fifth_batch_native_v7_schema:" + name)
    return value


def fifth_batch_object(kind: str, fields: dict[str, Any]) -> dict[str, Any]:
    body = {"schema": FIFTH_BATCH_SCHEMA + "." + kind, **fields}
    return {**body, "sha256": sha(canonical(body))}
...
def check_fifth_parent_header(head: dict[str, Any]) -> None:
    require(head.get("schema") == FIFTH_BATCH_SCHEMA + ".head" and "completed_steps" not in head,
...
def check_fifth_selection_lambda_contract(value: Any, registered_file: Any, expected: str) -> None:
    require(registered_file == "output/selection/start.json" and type(registered_file) is str and
            type(value) is dict and value.get("schema") == FIFTH_BATCH_SCHEMA + ".selection-start" and
            "selection_lambda_sha256" in value,
...
def check_v7_acceptance_header(value: Any) -> None:
    require(type(value) is dict and set(value) == {"schema", "parents", "anchor", "batch_anchor",
            "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "code", "runtime", "registration"} and
            value["schema"] == FIFTH_BATCH_SCHEMA + ".acceptance", "native_v7_acceptance_exact_ten_plain_keys")
...
def authenticate_fifth_batch_parent_metadata(inputs: AcceptedInputs) -> dict[str, Any]:
    ...
    same_json(old_oracle, {"failed_count": 36000, "first_failed_index": 110, "first_failed_edge": 212},
              "accepted_parent_old_oracle_is_observed_not_predicted")
    expected_anchor = {"accepted_schema": FIFTH_BATCH_SCHEMA, **{key: copy.deepcopy(tree.by_name[name]) for key, name in paths.items()},
        "invocations": ordinary_invocations, "checkpoints": ordinary_checkpoints, "upstream_completed_steps": 64,
    ...
    source = records["source"]
    expected_source = {"schema": FIFTH_BATCH_SCHEMA + ".source", "sha256": source["sha256"],
        "producer": code["producer"], "retained_producer_dependencies": code["producer_dependencies"],
        "checker": code["checker"], "retained_checker_dependencies": code["checker_dependencies"], "data": code["data"],
        "runtime": inputs.value["runtime"], "formula_id": FORMULA, "retained_TCB_independence_reproved": False}
...
def check_v7_intake_header(value: Any) -> None:
    ...
    require(type(value) is dict and set(value) == {
        "accepted_batch_anchor_sha256", "accepted_batch_checker_sha256", "accepted_batch_head_sha256", "accepted_batch_result_sha256",
        "accepted_batch_v5_anchor_sha256", "accepted_batch_v5_checker_sha256", "accepted_batch_v5_head_sha256", "accepted_batch_v5_parent_intake_sha256",
        "accepted_batch_v5_result_sha256", "accepted_batch_v6_anchor_sha256", "accepted_batch_v6_checker_sha256", "accepted_batch_v6_head_sha256",
        "accepted_batch_v6_parent_intake_sha256", "accepted_batch_v6_result_sha256", "accepted_next_batch_anchor_sha256", "accepted_next_batch_checker_sha256",
        "accepted_next_batch_head_sha256", "accepted_next_batch_parent_intake_sha256", "accepted_next_batch_result_sha256", "accepted_parent_batch_rows",
        "candidate_manifests_checked", "candidate_phase_manifests_checked", "checkpoints_checked", "direct_pairing",
        "generation", "intermediate_generation", "intermediate_rank", "intermediate_target_derivation_parents",
        "invocations_checked", "lambda_sha256", "native_pairing_rows_rechecked", "old_anchor_head_sha256",
        "old_batch_numeric_replays", "old_loader_regions", "old_rank", "old_snapshot_numeric_replays",
        "old_target_derivation_parents", "original_rho2_directly_read", "parent_layers", "portable_acceptance_sha256",
        "previous_parent_batch_rows", "previous_target_remainder_sha256", "rank", "row_manifests_checked",
        "schema", "second_intermediate_generation", "second_intermediate_rank", "second_intermediate_target_derivation_parents",
        "sha256", "state_head", "target_derivation_parents", "target_remainder_sha256",
        "third_intermediate_generation", "third_intermediate_rank", "third_intermediate_target_derivation_parents", "total_parent_batch_rows",
        "upstream_completed_steps",
        } and value.get("schema") == FIFTH_BATCH_SCHEMA + ".parent-intake",
...
def check_fifth_native_keysets(records: dict[str, Any]) -> None:
    # Saved v7 fieldsets; current v8 extensions never enter these historical objects.
    keysets = {
        "start": ('accepted_batch_anchor_sha256', 'accepted_batch_checker_sha256', 'accepted_batch_head_sha256', 'accepted_batch_result_sha256', 'accepted_batch_v5_anchor_sha256', 'accepted_batch_v5_checker_sha256', 'accepted_batch_v5_head_sha256', 'accepted_batch_v5_parent_intake_sha256', 'accepted_batch_v5_result_sha256', 'accepted_batch_v6_anchor_sha256', 'accepted_batch_v6_checker_sha256', 'accepted_batch_v6_head_sha256', 'accepted_batch_v6_parent_intake_sha256', 'accepted_batch_v6_result_sha256', 'accepted_next_batch_anchor_sha256', 'accepted_next_batch_checker_sha256', 'accepted_next_batch_head_sha256', 'accepted_next_batch_parent_intake_sha256', 'accepted_next_batch_result_sha256', 'accepted_parent_batch_rows', 'accepted_parent_target_derivations', 'accepted_target_derivation_parents', 'anchor_checker_sha256', 'anchor_completed_steps', 'anchor_head_sha256', 'anchor_pairing', 'anchor_pairing_rows', 'anchor_result_sha256', 'external_e_attached', 'generation', 'kind', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'original_rho2_packed_sha256', 'owner_sha256', 'parent_intake_sha256', 'parent_layout_sha256', 'previous_parent_batch_rows', 'previous_parent_target_derivations', 'previous_target_remainder_sha256', 'rank', 'registration', 'schema', 'selection_lambda_sha256', 'sha256', 'source_sha256', 'state_head', 'target_remainder_sha256', 'total_parent_batch_rows'),
        "parent_intake": ('accepted_batch_anchor_sha256', 'accepted_batch_checker_sha256', 'accepted_batch_head_sha256', 'accepted_batch_result_sha256', 'accepted_batch_v5_anchor_sha256', 'accepted_batch_v5_checker_sha256', 'accepted_batch_v5_head_sha256', 'accepted_batch_v5_parent_intake_sha256', 'accepted_batch_v5_result_sha256', 'accepted_batch_v6_anchor_sha256', 'accepted_batch_v6_checker_sha256', 'accepted_batch_v6_head_sha256', 'accepted_batch_v6_parent_intake_sha256', 'accepted_batch_v6_result_sha256', 'accepted_next_batch_anchor_sha256', 'accepted_next_batch_checker_sha256', 'accepted_next_batch_head_sha256', 'accepted_next_batch_parent_intake_sha256', 'accepted_next_batch_result_sha256', 'accepted_parent_batch_rows', 'candidate_manifests_checked', 'candidate_phase_manifests_checked', 'checkpoints_checked', 'direct_pairing', 'generation', 'intermediate_generation', 'intermediate_rank', 'intermediate_target_derivation_parents', 'invocations_checked', 'lambda_sha256', 'native_pairing_rows_rechecked', 'old_anchor_head_sha256', 'old_batch_numeric_replays', 'old_loader_regions', 'old_rank', 'old_snapshot_numeric_replays', 'old_target_derivation_parents', 'original_rho2_directly_read', 'parent_layers', 'portable_acceptance_sha256', 'previous_parent_batch_rows', 'previous_target_remainder_sha256', 'rank', 'row_manifests_checked', 'schema', 'second_intermediate_generation', 'second_intermediate_rank', 'second_intermediate_target_derivation_parents', 'sha256', 'state_head', 'target_derivation_parents', 'target_remainder_sha256', 'third_intermediate_generation', 'third_intermediate_rank', 'third_intermediate_target_derivation_parents', 'total_parent_batch_rows', 'upstream_completed_steps'),
        "parent_layout": ('anchor', 'batch_anchor', 'batch_anchor_v5', 'batch_anchor_v6', 'code', 'next_batch_anchor', 'parents', 'portable_acceptance_sha256', 'registration', 'runtime', 'schema', 'sha256'),
        "head": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'dependent_candidates', 'final_manifest_sha256', 'generation', 'kind', 'lambda_sha256', 'new_lambda_oracle', 'owner_sha256', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_sha256', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256', 'terminal'),
        "result": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_observation', 'candidate', 'candidates', 'cross_checked', 'dependent_candidates', 'elapsed_seconds', 'final_lambda_characters', 'final_manifest_sha256', 'final_telemetry', 'full_A0', 'generation', 'grade2_member', 'grade2_nonmember', 'head_sha256', 'input_preservation', 'invocation_sha256', 'invocations', 'kind', 'lambda_sha256', 'new_lambda_oracle', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'old_success_suites', 'owner_sha256', 'parent_layout_sha256', 'positive_readout', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_readout', 'selection_sha256', 'selection_start_sha256', 'selection_telemetry', 'sha256', 'skipped_after_linear', 'source_sha256', 'start_sha256', 'state_head', 'status', 'target_remainder_sha256', 'terminal', 'verified'),
        "checker": ('accepted_new_rows', 'accepted_rows_compared', 'all_completed_payloads_and_json_compared', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_observation', 'candidate', 'candidate_decisions_compared', 'candidate_phases_compared', 'checker_old_loader_regions', 'checker_source', 'cross_checked', 'dependent_candidates', 'durable_tail', 'elapsed_seconds', 'final_manifest_sha256', 'full_A0', 'generation', 'grade2_member', 'grade2_nonmember', 'input_preservation', 'lambda_sha256', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'old_success_suites', 'owner_sha256', 'parent_intake_sha256', 'partial', 'processed_candidates', 'producer_result_sha256', 'progress_head_sha256', 'public_final_compared', 'public_head_sha256', 'rank', 'runtime', 'schema', 'selected_count', 'selection_phases_compared', 'selection_sha256', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'status', 'target_remainder_sha256', 'terminal', 'verified'),
        "final_manifest": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'dependent_candidates', 'eof', 'files', 'generation', 'kind', 'lambda_sha256', 'last_candidate_manifest_sha256', 'last_row_manifest_sha256', 'owner_sha256', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_sha256', 'selection_start_sha256', 'sha256', 'skipped_after_linear', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256', 'terminal'),
        "separator": ('anchor_pairing_rows', 'direct_pairing', 'final_pairing_rows', 'kind', 'lambda_rho2', 'lambda_sha256', 'new_lambda_oracle', 'physical_lower_zero', 'schema', 'selection_lambda_sha256', 'sha256', 'source_lower_zero'),
        "selection_start": ('anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_size', 'fixed_manifest_sha256', 'generation', 'max_batches', 'owner_sha256', 'previous_target_remainder_sha256', 'rank', 'schema', 'selection_lambda_sha256', 'selection_policy', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256'),
        "selection": ('aux_values', 'auxiliary_tests', 'basis_chords', 'basis_tau', 'batch_size', 'chords_checked', 'eof', 'failed_count', 'failed_edges', 'failed_indices', 'first_failed_edge', 'first_failed_index', 'fit', 'fixed_manifest_sha256', 'max_batches', 'owner_sha256', 'phase_manifests', 'refill', 'schema', 'selected', 'selected_count', 'selection_policy', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'terminal'),
        "source": ('checker', 'data', 'formula_id', 'producer', 'retained_TCB_independence_reproved', 'retained_checker_dependencies', 'retained_producer_dependencies', 'runtime', 'schema', 'sha256'),
        "owner": ('formula_id', 'parent_layout_sha256', 'portable_acceptance_sha256', 'registration', 'schema', 'scope', 'sha256', 'source_sha256'),
        "fixed": ('accepted_fixed_manifest', 'accepted_geometry_stage_sha256', 'files', 'fixed_values_independent_of_lambda', 'owner_sha256', 'schema', 'sha256', 'source_sha256', 'start_sha256'),
        "progress_head": ('accepted_new_rows', 'checkpoint_sha256', 'current_lambda_sha256', 'dependent_candidates', 'generation', 'kind', 'owner_sha256', 'processed_candidates', 'rank', 'reduction_state_head', 'schema', 'sequence', 'sha256', 'source_sha256', 'start_sha256', 'target_remainder_sha256'),
    }
    for name, keys in keysets.items():
        require(type(records[name]) is dict and set(records[name]) == set(keys),
                "fifth_native_v7_exact_keys:" + name)
...
def sixth_batch_json(tree: PinnedTree, name: str, suffix: str) -> dict[str, Any]:
    value = tree.json(name, True)
    check_document(value)
    require(value["schema"] == SIXTH_BATCH_SCHEMA + "." + suffix, "sixth_batch_native_v8_schema:" + name)
    return value


def sixth_batch_object(kind: str, fields: dict[str, Any]) -> dict[str, Any]:
    body = {"schema": SIXTH_BATCH_SCHEMA + "." + kind, **fields}
    return {**body, "sha256": sha(canonical(body))}
...
def check_sixth_parent_header(head: dict[str, Any]) -> None:
    require(head.get("schema") == SIXTH_BATCH_SCHEMA + ".head" and "completed_steps" not in head,
...
def check_sixth_selection_lambda_contract(value: Any, registered_file: Any, expected: str) -> None:
    require(registered_file == "output/selection/start.json" and type(registered_file) is str and
            type(value) is dict and value.get("schema") == SIXTH_BATCH_SCHEMA + ".selection-start" and
            "selection_lambda_sha256" in value,
...
def check_v8_acceptance_header(value: Any) -> None:
    require(type(value) is dict and set(value) == {"schema", "parents", "anchor", "batch_anchor",
            "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "code", "runtime", "registration"} and
            value["schema"] == SIXTH_BATCH_SCHEMA + ".acceptance", "native_v8_acceptance_exact_eleven_plain_keys")
...
def authenticate_sixth_batch_parent_metadata(inputs: AcceptedInputs) -> dict[str, Any]:
    ...
    same_json(old_oracle, {"failed_count": 36107, "first_failed_index": 304, "first_failed_edge": 603},
              "accepted_parent_old_oracle_is_observed_not_predicted")
    expected_anchor = {"accepted_schema": SIXTH_BATCH_SCHEMA, **{key: copy.deepcopy(tree.by_name[name]) for key, name in paths.items()},
        "invocations": ordinary_invocations, "checkpoints": ordinary_checkpoints, "upstream_completed_steps": 64,
    ...
    source = records["source"]
    expected_source = {"schema": SIXTH_BATCH_SCHEMA + ".source", "sha256": source["sha256"],
        "producer": code["producer"], "retained_producer_dependencies": code["producer_dependencies"],
        "checker": code["checker"], "retained_checker_dependencies": code["checker_dependencies"], "data": code["data"],
        "runtime": inputs.value["runtime"], "formula_id": FORMULA, "retained_TCB_independence_reproved": False}
...
def check_v8_intake_header(value: Any) -> None:
    ...
    require(type(value) is dict and set(value) == {
        ...
        "accepted_batch_v5_anchor_sha256", "accepted_batch_v5_checker_sha256", "accepted_batch_v5_head_sha256", "accepted_batch_v5_parent_intake_sha256",
        "accepted_batch_v5_result_sha256", "accepted_batch_v6_anchor_sha256", "accepted_batch_v6_checker_sha256", "accepted_batch_v6_head_sha256",
        "accepted_batch_v6_parent_intake_sha256", "accepted_batch_v6_result_sha256", "accepted_batch_v7_anchor_sha256", "accepted_batch_v7_checker_sha256",
        "accepted_batch_v7_head_sha256", "accepted_batch_v7_parent_intake_sha256", "accepted_batch_v7_result_sha256", "accepted_next_batch_anchor_sha256",
        "accepted_next_batch_checker_sha256", "accepted_next_batch_head_sha256", "accepted_next_batch_parent_intake_sha256", "accepted_next_batch_result_sha256",
        "accepted_parent_batch_rows", "candidate_manifests_checked", "candidate_phase_manifests_checked", "checkpoints_checked",
        "direct_pairing", "fourth_intermediate_generation", "fourth_intermediate_rank", "fourth_intermediate_target_derivation_parents",
        "generation", "intermediate_generation", "intermediate_rank", "intermediate_target_derivation_parents",
        "invocations_checked", "lambda_sha256", "native_pairing_rows_rechecked", "old_anchor_head_sha256",
        "old_batch_numeric_replays", "old_loader_regions", "old_rank", "old_snapshot_numeric_replays",
        "old_target_derivation_parents", "original_rho2_directly_read", "parent_layers", "portable_acceptance_sha256",
        ...
        "third_intermediate_generation", "third_intermediate_rank", "third_intermediate_target_derivation_parents", "total_parent_batch_rows",
        "upstream_completed_steps",
        } and value.get("schema") == SIXTH_BATCH_SCHEMA + ".parent-intake",
...
def check_sixth_native_keysets(records: dict[str, Any]) -> None:
    # Actual saved v8 fieldsets; current v9 extensions stay outside these objects.
    keysets = {
        "checker": ('accepted_new_rows', 'accepted_rows_compared', 'all_completed_payloads_and_json_compared', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_observation', 'candidate', 'candidate_decisions_compared', 'candidate_phases_compared', 'checker_old_loader_regions', 'checker_source', 'cross_checked', 'dependent_candidates', 'durable_tail', 'elapsed_seconds', 'final_manifest_sha256', 'full_A0', 'generation', 'grade2_member', 'grade2_nonmember', 'input_preservation', 'lambda_sha256', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'old_success_suites', 'owner_sha256', 'parent_intake_sha256', 'partial', 'processed_candidates', 'producer_result_sha256', 'progress_head_sha256', 'public_final_compared', 'public_head_sha256', 'rank', 'runtime', 'schema', 'selected_count', 'selection_phases_compared', 'selection_sha256', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'status', 'target_remainder_sha256', 'terminal', 'verified'),
        "final_manifest": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'dependent_candidates', 'eof', 'files', 'generation', 'kind', 'lambda_sha256', 'last_candidate_manifest_sha256', 'last_row_manifest_sha256', 'owner_sha256', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_sha256', 'selection_start_sha256', 'sha256', 'skipped_after_linear', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256', 'terminal'),
        "fixed": ('accepted_fixed_manifest', 'accepted_geometry_stage_sha256', 'files', 'fixed_values_independent_of_lambda', 'owner_sha256', 'schema', 'sha256', 'source_sha256', 'start_sha256'),
        "head": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'dependent_candidates', 'final_manifest_sha256', 'generation', 'kind', 'lambda_sha256', 'new_lambda_oracle', 'owner_sha256', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_sha256', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256', 'terminal'),
        "owner": ('formula_id', 'parent_layout_sha256', 'portable_acceptance_sha256', 'registration', 'schema', 'scope', 'sha256', 'source_sha256'),
        "parent_intake": ('accepted_batch_anchor_sha256', 'accepted_batch_checker_sha256', 'accepted_batch_head_sha256', 'accepted_batch_result_sha256', 'accepted_batch_v5_anchor_sha256', 'accepted_batch_v5_checker_sha256', 'accepted_batch_v5_head_sha256', 'accepted_batch_v5_parent_intake_sha256', 'accepted_batch_v5_result_sha256', 'accepted_batch_v6_anchor_sha256', 'accepted_batch_v6_checker_sha256', 'accepted_batch_v6_head_sha256', 'accepted_batch_v6_parent_intake_sha256', 'accepted_batch_v6_result_sha256', 'accepted_batch_v7_anchor_sha256', 'accepted_batch_v7_checker_sha256', 'accepted_batch_v7_head_sha256', 'accepted_batch_v7_parent_intake_sha256', 'accepted_batch_v7_result_sha256', 'accepted_next_batch_anchor_sha256', 'accepted_next_batch_checker_sha256', 'accepted_next_batch_head_sha256', 'accepted_next_batch_parent_intake_sha256', 'accepted_next_batch_result_sha256', 'accepted_parent_batch_rows', 'candidate_manifests_checked', 'candidate_phase_manifests_checked', 'checkpoints_checked', 'direct_pairing', 'fourth_intermediate_generation', 'fourth_intermediate_rank', 'fourth_intermediate_target_derivation_parents', 'generation', 'intermediate_generation', 'intermediate_rank', 'intermediate_target_derivation_parents', 'invocations_checked', 'lambda_sha256', 'native_pairing_rows_rechecked', 'old_anchor_head_sha256', 'old_batch_numeric_replays', 'old_loader_regions', 'old_rank', 'old_snapshot_numeric_replays', 'old_target_derivation_parents', 'original_rho2_directly_read', 'parent_layers', 'portable_acceptance_sha256', 'previous_parent_batch_rows', 'previous_target_remainder_sha256', 'rank', 'row_manifests_checked', 'schema', 'second_intermediate_generation', 'second_intermediate_rank', 'second_intermediate_target_derivation_parents', 'sha256', 'state_head', 'target_derivation_parents', 'target_remainder_sha256', 'third_intermediate_generation', 'third_intermediate_rank', 'third_intermediate_target_derivation_parents', 'total_parent_batch_rows', 'upstream_completed_steps'),
        "parent_layout": ('anchor', 'batch_anchor', 'batch_anchor_v5', 'batch_anchor_v6', 'batch_anchor_v7', 'code', 'next_batch_anchor', 'parents', 'portable_acceptance_sha256', 'registration', 'runtime', 'schema', 'sha256'),
        "progress_head": ('accepted_new_rows', 'checkpoint_sha256', 'current_lambda_sha256', 'dependent_candidates', 'generation', 'kind', 'owner_sha256', 'processed_candidates', 'rank', 'reduction_state_head', 'schema', 'sequence', 'sha256', 'source_sha256', 'start_sha256', 'target_remainder_sha256'),
        "result": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_observation', 'candidate', 'candidates', 'cross_checked', 'dependent_candidates', 'elapsed_seconds', 'final_lambda_characters', 'final_manifest_sha256', 'final_telemetry', 'full_A0', 'generation', 'grade2_member', 'grade2_nonmember', 'head_sha256', 'input_preservation', 'invocation_sha256', 'invocations', 'kind', 'lambda_sha256', 'new_lambda_oracle', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'old_success_suites', 'owner_sha256', 'parent_layout_sha256', 'positive_readout', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_readout', 'selection_sha256', 'selection_start_sha256', 'selection_telemetry', 'sha256', 'skipped_after_linear', 'source_sha256', 'start_sha256', 'state_head', 'status', 'target_remainder_sha256', 'terminal', 'verified'),
        "selection": ('aux_values', 'auxiliary_tests', 'basis_chords', 'basis_tau', 'batch_size', 'chords_checked', 'eof', 'failed_count', 'failed_edges', 'failed_indices', 'first_failed_edge', 'first_failed_index', 'fit', 'fixed_manifest_sha256', 'max_batches', 'owner_sha256', 'phase_manifests', 'refill', 'schema', 'selected', 'selected_count', 'selection_policy', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'terminal'),
        "selection_start": ('anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_size', 'fixed_manifest_sha256', 'generation', 'max_batches', 'owner_sha256', 'previous_target_remainder_sha256', 'rank', 'schema', 'selection_lambda_sha256', 'selection_policy', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256'),
    ...
    for name, keys in keysets.items():
        require(type(records[name]) is dict and set(records[name]) == set(keys),
                "sixth_native_v8_exact_keys:" + name)
...
def seventh_batch_json(tree: PinnedTree, name: str, suffix: str) -> dict[str, Any]:
    value = tree.json(name, True)
    check_document(value)
    require(value["schema"] == SEVENTH_BATCH_SCHEMA + "." + suffix, "seventh_batch_native_v9_schema:" + name)
    return value


def seventh_batch_object(kind: str, fields: dict[str, Any]) -> dict[str, Any]:
    body = {"schema": SEVENTH_BATCH_SCHEMA + "." + kind, **fields}
    return {**body, "sha256": sha(canonical(body))}
...
def check_seventh_parent_header(head: dict[str, Any]) -> None:
    require(head.get("schema") == SEVENTH_BATCH_SCHEMA + ".head" and "completed_steps" not in head,
...
def check_seventh_selection_lambda_contract(value: Any, registered_file: Any, expected: str) -> None:
    require(registered_file == "output/selection/start.json" and type(registered_file) is str and
            type(value) is dict and value.get("schema") == SEVENTH_BATCH_SCHEMA + ".selection-start" and
            "selection_lambda_sha256" in value,
...
def check_v9_acceptance_header(value: Any) -> None:
    require(type(value) is dict and set(value) == {"schema", "parents", "anchor", "batch_anchor",
            "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "batch_anchor_v8", "code", "runtime", "registration"} and
...
def authenticate_seventh_batch_parent_metadata(inputs: AcceptedInputs) -> dict[str, Any]:
    ...
    same_json(old_oracle, {"failed_count": 35647, "first_failed_index": 242, "first_failed_edge": 489},
              "accepted_parent_old_oracle_is_observed_not_predicted")
    expected_anchor = {"accepted_schema": SEVENTH_BATCH_SCHEMA, **{key: copy.deepcopy(tree.by_name[name]) for key, name in paths.items()},
        "invocations": ordinary_invocations, "checkpoints": ordinary_checkpoints, "upstream_completed_steps": 64,
    ...
    check_seventh_native_parent_projection(inputs.portable["parents"], layout["parents"])
    ...
    source = records["source"]
    expected_source = {"schema": SEVENTH_BATCH_SCHEMA + ".source", "sha256": source["sha256"],
        "producer": code["producer"], "retained_producer_dependencies": code["producer_dependencies"],
        "checker": code["checker"], "retained_checker_dependencies": code["checker_dependencies"], "data": code["data"],
        "runtime": inputs.value["runtime"], "formula_id": FORMULA, "retained_TCB_independence_reproved": False}
...
def check_v9_intake_header(value: Any) -> None:
    # Saved native header only; the ordinary full reconstruction follows later.
    require(type(value) is dict and set(value) == {
        "accepted_batch_anchor_sha256", "accepted_batch_checker_sha256", "accepted_batch_head_sha256", "accepted_batch_result_sha256",
        "accepted_batch_v5_anchor_sha256", "accepted_batch_v5_checker_sha256", "accepted_batch_v5_head_sha256", "accepted_batch_v5_parent_intake_sha256",
        "accepted_batch_v5_result_sha256", "accepted_batch_v6_anchor_sha256", "accepted_batch_v6_checker_sha256", "accepted_batch_v6_head_sha256",
        "accepted_batch_v6_parent_intake_sha256", "accepted_batch_v6_result_sha256", "accepted_batch_v7_anchor_sha256", "accepted_batch_v7_checker_sha256",
        "accepted_batch_v7_head_sha256", "accepted_batch_v7_parent_intake_sha256", "accepted_batch_v7_result_sha256", "accepted_batch_v8_anchor_sha256",
        "accepted_batch_v8_checker_sha256", "accepted_batch_v8_head_sha256", "accepted_batch_v8_parent_intake_sha256", "accepted_batch_v8_result_sha256",
        "accepted_next_batch_anchor_sha256", "accepted_next_batch_checker_sha256", "accepted_next_batch_head_sha256", "accepted_next_batch_parent_intake_sha256",
        "accepted_next_batch_result_sha256", "accepted_parent_batch_rows", "candidate_manifests_checked", "candidate_phase_manifests_checked",
        "checkpoints_checked", "direct_pairing", "fifth_intermediate_generation", "fifth_intermediate_rank",
        "fifth_intermediate_target_derivation_parents", "fourth_intermediate_generation", "fourth_intermediate_rank", "fourth_intermediate_target_derivation_parents",
        "generation", "intermediate_generation", "intermediate_rank", "intermediate_target_derivation_parents",
        "invocations_checked", "lambda_sha256", "native_pairing_rows_rechecked", "old_anchor_head_sha256",
        "old_batch_numeric_replays", "old_loader_regions", "old_rank", "old_snapshot_numeric_replays",
        "old_target_derivation_parents", "original_rho2_directly_read", "parent_layers", "portable_acceptance_sha256",
        ...
        "third_intermediate_generation", "third_intermediate_rank", "third_intermediate_target_derivation_parents", "total_parent_batch_rows",
        "upstream_completed_steps",
        } and value.get("schema") == SEVENTH_BATCH_SCHEMA + ".parent-intake",
...
def check_seventh_native_keysets(records: dict[str, Any]) -> None:
    # Actual saved V9 fieldsets; current V10 extensions stay outside these objects.
    keysets = {
        "checker": ('accepted_new_rows', 'accepted_rows_compared', 'all_completed_payloads_and_json_compared', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_observation', 'candidate', 'candidate_decisions_compared', 'candidate_phases_compared', 'checker_old_loader_regions', 'checker_source', 'cross_checked', 'dependent_candidates', 'durable_tail', 'elapsed_seconds', 'final_manifest_sha256', 'full_A0', 'generation', 'grade2_member', 'grade2_nonmember', 'input_preservation', 'lambda_sha256', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'old_success_suites', 'owner_sha256', 'parent_intake_sha256', 'partial', 'processed_candidates', 'producer_result_sha256', 'progress_head_sha256', 'public_final_compared', 'public_head_sha256', 'rank', 'runtime', 'schema', 'selected_count', 'selection_phases_compared', 'selection_sha256', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'status', 'target_remainder_sha256', 'terminal', 'verified'),
        "final_manifest": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'dependent_candidates', 'eof', 'files', 'generation', 'kind', 'lambda_sha256', 'last_candidate_manifest_sha256', 'last_row_manifest_sha256', 'owner_sha256', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_sha256', 'selection_start_sha256', 'sha256', 'skipped_after_linear', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256', 'terminal'),
        "fixed": ('accepted_fixed_manifest', 'accepted_geometry_stage_sha256', 'files', 'fixed_values_independent_of_lambda', 'owner_sha256', 'schema', 'sha256', 'source_sha256', 'start_sha256'),
        "head": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'dependent_candidates', 'final_manifest_sha256', 'generation', 'kind', 'lambda_sha256', 'new_lambda_oracle', 'owner_sha256', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_sha256', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256', 'terminal'),
        "owner": ('formula_id', 'parent_layout_sha256', 'portable_acceptance_sha256', 'registration', 'schema', 'scope', 'sha256', 'source_sha256'),
        "parent_intake": ('accepted_batch_anchor_sha256', 'accepted_batch_checker_sha256', 'accepted_batch_head_sha256', 'accepted_batch_result_sha256', 'accepted_batch_v5_anchor_sha256', 'accepted_batch_v5_checker_sha256', 'accepted_batch_v5_head_sha256', 'accepted_batch_v5_parent_intake_sha256', 'accepted_batch_v5_result_sha256', 'accepted_batch_v6_anchor_sha256', 'accepted_batch_v6_checker_sha256', 'accepted_batch_v6_head_sha256', 'accepted_batch_v6_parent_intake_sha256', 'accepted_batch_v6_result_sha256', 'accepted_batch_v7_anchor_sha256', 'accepted_batch_v7_checker_sha256', 'accepted_batch_v7_head_sha256', 'accepted_batch_v7_parent_intake_sha256', 'accepted_batch_v7_result_sha256', 'accepted_batch_v8_anchor_sha256', 'accepted_batch_v8_checker_sha256', 'accepted_batch_v8_head_sha256', 'accepted_batch_v8_parent_intake_sha256', 'accepted_batch_v8_result_sha256', 'accepted_next_batch_anchor_sha256', 'accepted_next_batch_checker_sha256', 'accepted_next_batch_head_sha256', 'accepted_next_batch_parent_intake_sha256', 'accepted_next_batch_result_sha256', 'accepted_parent_batch_rows', 'candidate_manifests_checked', 'candidate_phase_manifests_checked', 'checkpoints_checked', 'direct_pairing', 'fifth_intermediate_generation', 'fifth_intermediate_rank', 'fifth_intermediate_target_derivation_parents', 'fourth_intermediate_generation', 'fourth_intermediate_rank', 'fourth_intermediate_target_derivation_parents', 'generation', 'intermediate_generation', 'intermediate_rank', 'intermediate_target_derivation_parents', 'invocations_checked', 'lambda_sha256', 'native_pairing_rows_rechecked', 'old_anchor_head_sha256', 'old_batch_numeric_replays', 'old_loader_regions', 'old_rank', 'old_snapshot_numeric_replays', 'old_target_derivation_parents', 'original_rho2_directly_read', 'parent_layers', 'portable_acceptance_sha256', 'previous_parent_batch_rows', 'previous_target_remainder_sha256', 'rank', 'row_manifests_checked', 'schema', 'second_intermediate_generation', 'second_intermediate_rank', 'second_intermediate_target_derivation_parents', 'sha256', 'state_head', 'target_derivation_parents', 'target_remainder_sha256', 'third_intermediate_generation', 'third_intermediate_rank', 'third_intermediate_target_derivation_parents', 'total_parent_batch_rows', 'upstream_completed_steps'),
        "parent_layout": ('anchor', 'batch_anchor', 'batch_anchor_v5', 'batch_anchor_v6', 'batch_anchor_v7', 'batch_anchor_v8', 'code', 'next_batch_anchor', 'parents', 'portable_acceptance_sha256', 'registration', 'runtime', 'schema', 'sha256'),
        "progress_head": ('accepted_new_rows', 'checkpoint_sha256', 'current_lambda_sha256', 'dependent_candidates', 'generation', 'kind', 'owner_sha256', 'processed_candidates', 'rank', 'reduction_state_head', 'schema', 'sequence', 'sha256', 'source_sha256', 'start_sha256', 'target_remainder_sha256'),
        "result": ('accepted_new_rows', 'anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_observation', 'candidate', 'candidates', 'cross_checked', 'dependent_candidates', 'elapsed_seconds', 'final_lambda_characters', 'final_manifest_sha256', 'final_telemetry', 'full_A0', 'generation', 'grade2_member', 'grade2_nonmember', 'head_sha256', 'input_preservation', 'invocation_sha256', 'invocations', 'kind', 'lambda_sha256', 'new_lambda_oracle', 'old_insert_numeric_replays', 'old_snapshot_numeric_replays', 'old_success_suites', 'owner_sha256', 'parent_layout_sha256', 'positive_readout', 'processed_candidates', 'rank', 'schema', 'selected_count', 'selection_readout', 'selection_sha256', 'selection_start_sha256', 'selection_telemetry', 'sha256', 'skipped_after_linear', 'source_sha256', 'start_sha256', 'state_head', 'status', 'target_remainder_sha256', 'terminal', 'verified'),
        "selection": ('aux_values', 'auxiliary_tests', 'basis_chords', 'basis_tau', 'batch_size', 'chords_checked', 'eof', 'failed_count', 'failed_edges', 'failed_indices', 'first_failed_edge', 'first_failed_index', 'fit', 'fixed_manifest_sha256', 'max_batches', 'owner_sha256', 'phase_manifests', 'refill', 'schema', 'selected', 'selected_count', 'selection_policy', 'selection_start_sha256', 'sha256', 'source_sha256', 'start_sha256', 'terminal'),
        "selection_start": ('anchor_accepted_parent_batch_rows', 'anchor_completed_steps', 'anchor_previous_parent_batch_rows', 'anchor_total_parent_batch_rows', 'batch_size', 'fixed_manifest_sha256', 'generation', 'max_batches', 'owner_sha256', 'previous_target_remainder_sha256', 'rank', 'schema', 'selection_lambda_sha256', 'selection_policy', 'sha256', 'source_sha256', 'start_sha256', 'state_head', 'target_remainder_sha256'),
    ...
    for name, keys in keysets.items():
        require(type(records[name]) is dict and set(records[name]) == set(keys),
                "seventh_native_v9_exact_keys:" + name)
...
def root_records(inputs: AcceptedInputs, anchor: ThinAnchor, old: dict[str, Any], bundle: C.FixedBundle) -> RootRecords:
    ...
    layout = document("parent-layout", {"portable_acceptance_sha256": inputs.portable_sha256,
        **{key: copy.deepcopy(inputs.portable[key]) for key in ("parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "batch_anchor_v8", "batch_anchor_v9", "code", "runtime", "registration")}})
    ...
    emit_parent_timing("direct-pairing", SEVENTH_BATCH_ROLE, 7, pairing_started, rows=pairing["rows"], records=len(anchor.parents))
    ...
    start = document("start", {"owner_sha256": sha(canonical(owner)), "source_sha256": sha(canonical(source)),
        ...
        "previous_parent_target_derivations": len(inputs.sixth_intermediate_parents),
    ...
    require(len(layout) == V10_CURRENT_LAYOUT_KEY_COUNT and len(start) == V10_CURRENT_START_KEY_COUNT and
            len(intake) == V10_CURRENT_INTAKE_KEY_COUNT, "c10_current_root_key_counts_from_independent_sums")
...
def batch_observation(inputs: AcceptedInputs, records: RootRecords, files: CandidateFiles,
    ...
    if not admitted:
        sequence = -1
    batch = inputs.value["batch_anchor_v9"]
    parents = {item["role"]: item["artifact"] for item in inputs.portable["parents"]}
    accepted = inputs.seventh_batch_parent["records"]
    old = {"parent_role": "batch-parent-v8", "parent_artifact": copy.deepcopy(parents["batch-parent-v8"]),
    ...
    current = {"parent_role": "batch-parent-v9", "parent_artifact": copy.deepcopy(parents["batch-parent-v9"]),
    ...
    if admitted:
        ...
        require(pairing["rows"] == SEVENTH_BATCH_RANK and pairing["lambda_pivots"] == 0 and
    ...
    check_seventh_unobserved_oracle(current, sequence)
...
def compare_producer_result(files: CandidateFiles, inputs: AcceptedInputs, records: RootRecords,
    ...
    check_v10_current_anchor_batch_counts(actual)
...
def check_actual(args: argparse.Namespace, report: dict[str, Any], parse_bytes: SavedParentJsonBytes) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        restore_started = time.monotonic()
        anchor = stack.enter_context(promote_seventh_batch_parent(inputs, sixth_intermediate, old))
        emit_parent_timing("state-restore", SEVENTH_BATCH_ROLE, 7, restore_started, rows=anchor.rank, records=len(anchor.parents))
        ...
        if "selection/start.json" in files.expected:
            check_v10_current_anchor_batch_counts(records.selection_start)
        ...
        if "start.json" in files.expected and "parent-intake.json" in files.expected:
            check_v10_current_anchor_batch_counts(report)
        ...
        if "final/manifest.json" in files.before:
            ...
            check_v10_current_anchor_batch_counts(final.manifest)
            if final.head is not None:
                check_v10_current_anchor_batch_counts(final.head)
...
def k128_registration_canary(root: Path) -> dict[str, Any]:
    ...
    acceptance = {"schema": SCHEMA + ".acceptance", "parents": [], "anchor": {}, "batch_anchor": {}, "next_batch_anchor": {}, "batch_anchor_v5": {}, "batch_anchor_v6": {}, "batch_anchor_v7": {}, "batch_anchor_v8": {}, "batch_anchor_v9": {}, "code": {},
...
def seventh_batch_parent_admission_canary(root: Path) -> dict[str, Any]:
    ...
    pair("current-schema-in-native73-intake", native_intake,
         fixture_reseal(native_intake, schema=SCHEMA + ".parent-intake"),
    ...
    try:
        if args.selftest:
            ...
            require(all(getattr(args, role.replace("-", "_") + "_root") is not None for role in PARENT_ROLES if not role.startswith("block-")) and
                    ...
                    "all_twenty_two_roots_and_exact_acceptance")
