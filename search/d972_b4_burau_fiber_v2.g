...
D972BFDefect:=function(parts)
 ...
 D972BFSelfTest:=function()
  ...
  if s[1]*s[2]*s[1]<>s[2]*s[1]*s[2] or
     s[2]*s[3]*s[2]<>s[3]*s[2]*s[3] or s[1]*s[3]<>s[3]*s[1] then
    Error("B4 Artin relation selftest failed"); fi;;
  if ForAny(s,m->DeterminantMat(m)=Zero(GF(D972BFQ))) then
    Error("Burau determinant selftest failed"); fi;;
...
if D972BFS[1]*D972BFS[2]*D972BFS[1]<>D972BFS[2]*D972BFS[1]*D972BFS[2] or
   D972BFS[2]*D972BFS[3]*D972BFS[2]<>D972BFS[3]*D972BFS[2]*D972BFS[3] or
   D972BFS[1]*D972BFS[3]<>D972BFS[3]*D972BFS[1] then Error("B4 Artin relation gate failed"); fi;;
