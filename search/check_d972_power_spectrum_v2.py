"""Independent checker for the exact D972 shadow power spectrum v2.
...
def search_authenticated_outside() -> str:
    for path in ROOT.joinpath("search", "certs").rglob("*.json"):
        ...
        if isinstance(obj, dict) and obj.get("semantic_roof") == \
            ...
            return "AUTHENTICATED_LABEL_FOUND_BUT_NOT_CONSUMED_BY_V2"
...
def check(receipt_path: Path) -> dict[str, Any]:
    ...
    require(receipt.get("final_marker") == "D972_POWER_SPECTRUM_V2_GAP_FINAL" and
            receipt.get("status") == "POWER_SPECTRUM_V2_COMPLETE" and
    ...
    return {"status": "D972_POWER_SPECTRUM_V2_CHECKER_PASS", "row_count": 972,
            "exponent": exponent, "outside_label_status": receipt["outside_label_status"]}
...
def self_test() -> None:
    ...
    print("D972_POWER_SPECTRUM_V2_CHECKER_SELFTEST_PASS")


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--receipt", type=Path, default=Path("ci/out/d972_power_spectrum_v2.json"))
    ...
    print("D972_POWER_SPECTRUM_V2_CHECKER_PASS")
    return 0
