// i24p1-u-h3.mjs — 予言 I24-P1 の測定: week4-u-k3.mjs を複製し λ の定義のみ差し替え
// plane_model: F(x,t) = t² + (x−1)²(4x−1)t + 4x⁶  (LMFDB 6T9-6_6_2.2.1.1-a、week4-u-k3.mjs と同一)
...
//////////////////// 5. ★ u の抽出(予言 I24-P1: λ₃ := t/(t+1)) ////////////////////
// 我々の λ 割当: (0,1,∞) 上で (6, 2²1², 6)。⇒ λ(t=0)=0, λ(t=−1)=1, λ(t=∞)=∞ ⇒ 元は **λ₂ = −t**
// 予言 I24-P1(docs/notes/i24_transport_design_v1.md §7.4): μ(w)=w/(w−1), λ₂=−t のとき
//   λ₃ := μ(λ₂) = (−t)/(−t−1) = t/(t+1)  ★ここが差し替えの 1 行(λ の定義のみ)★
// 同一 cusp P0=(0,0)・同一 uniformizer s=x で測定する。t(x) の冪級数計算はロジック変更なし。
{
  // 冪級数で t(x) を解く(ℚ[[x]])
  ...
  // λ₃ = t/(t+1)(予言 I24-P1 の λ)。冪級数演算は上と同じ sAdd/sMul/sInv/sub を再利用(ロジック変更なし)。
