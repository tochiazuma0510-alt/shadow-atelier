...
D363Producer:="search/d972_r07_pre_a0_single_target_a3_v5.py";;
D363Checker:="crosscheck/check_d972_r07_pre_a0_single_target_a3_v5.py";;
D363Receipt:="ci/out/d972_r07_pre_a0_single_target_a3_v5.json";;
D363Verdict:="ci/out/d972_r07_pre_a0_single_target_a3_v5.verdict.json";;
D363ProducerLog:="ci/out/d972_r07_pre_a0_single_target_a3_v5.producer.log";;
D363CheckerLog:="ci/out/d972_r07_pre_a0_single_target_a3_v5.checker.log";;
D363Shell:="ci/out/d972_r07_pre_a0_single_target_a3_v5.driver.sh";;
D363Sentinel:="ci/out/d972_r07_pre_a0_single_target_a3_v5.driver.accepted";;
...
D363Pins:=[
  ...
  [D363Producer,"4fbbd5792a1d1cc7bb1c3d534bdc0966291751cc9d3cea99d1ed20ca7d70fecb",104446],
  [D363Checker,"90838f12061783c77651c656f7bd1a572ca4a687339b5b70747342d18d32028a",116872]
];;
...
if not D363Hex64(D363P0Self) then
  Error("task384: malformed P0 self seal"); fi;
if not IsExistingFile(D363Bash) or not IsExecutableFile(D363Bash) then
  Error("task384: exact /usr/bin/bash is not executable"); fi;

D363Read:=function(path,label)
  local raw;
  raw:=StringFile(path);;
  if raw=fail or Length(raw)=0 then Error("task384: missing ",label); fi;
  return raw;
end;;
...
for D363Path in [D363Receipt,D363Verdict,D363ProducerLog,D363CheckerLog,
  ...
  if IsExistingFile(D363Path) then
    Error("task384: stale output ",D363Path); fi;
od;
if not IsDirectoryPath("ci/out") then
  if CreateDir("ci/out")=fail then Error("task384: cannot create ci/out"); fi;
fi;
if not IsDirectoryPath("ci/out") then
  Error("task384: ci/out is not a directory"); fi;

D363Stream:=OutputTextFile(D363Shell,false);;
if D363Stream=fail then Error("task384: cannot open command script"); fi;
...
PrintTo(D363Stream,"compgen -G 'ci/out/.d972_r07_pre_a0_single_target_a3_v5.json.tmp.*' >/dev/null && fail stale-receipt-temp\n");
PrintTo(D363Stream,"compgen -G 'ci/out/.d972_r07_pre_a0_single_target_a3_v5.verdict.json.tmp.*' >/dev/null && fail stale-verdict-temp\n");
...
PrintTo(D363Stream,"need(verdict.get('schema')=='d972-r07-pre-a0-single-target-a3/v5/verdict/v1' and verdict.get('status')==terminal and verdict.get('terminal')==terminal and verdict.get('accepted') is True and verdict.get('independent') is True and verdict.get('reason') is None,'verdict acceptance')\n");
...
PrintTo(D363Stream,"rootfd=cifd=dfd=fd=-1\n");
...
PrintTo(D363Stream,"    raw=b'D363_V5_ACCEPTED'; offset=0\n");
...
PrintTo(D363Stream,"    retain(original,'publication',exc)\n");
...
if not IsInt(D363Status) then
  Error("task384: noninteger shell process status"); fi;
if D363Status<>0 then
  Error("task384: nonzero shell process status ",D363Status); fi;
D363Observed:=StringFile(D363Sentinel);;
if D363Observed<>"D363_V5_ACCEPTED" then
  Error("task384: serial accepting-only driver did not accept"); fi;
