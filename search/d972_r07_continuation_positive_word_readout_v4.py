#!/usr/bin/env python3
"""Task1016: v4 bounded disk node indices and measured resource limits.
...
def integer_decimal_capacity(error: ValueError) -> bool:
    getter = getattr(sys, "get_int_max_str_digits", None)
    if sys.implementation.name != "cpython" or not callable(getter):
        return False
    limit = getter()
    return type(error) is ValueError and plain_int(limit, 1) and error.args == (
        "Exceeds the limit (" + str(limit) + " digits) for integer string conversion; "
...
def canonical(value: Any) -> bytes:
    try:
        ...
        if integer_decimal_capacity(error):
            raise ResourceStop("canonical_integer_decimal_capacity") from error
        raise
...
# Task1016 private storage: this layout is not a checker interchange format.
RESOURCE_SCHEMA = SCHEMA + ".resource-v4"
CACHE_LIMIT = 67108864
LINE_LIMIT = 67108864
SCRATCH_LIMIT = 17179869184
FREE_FLOOR = 1073741824
PAGE_ROWS = 4096
NODE_STRUCT = struct.Struct("<32sBBQ")
PAGE_BYTES = PAGE_ROWS * NODE_STRUCT.size
CACHE_PAGES = (CACHE_LIMIT - 2 * PAGE_BYTES) // PAGE_BYTES
UINT64_MAX = (1 << 64) - 1
REFERENCE_P3 = ("search/d972_r07_continuation_positive_word_readout_v3.py", 200658,
                "bc51546ee1b2e73cff3a115947c817164199179b25699f90a7cae3283872e16e")
RESOURCE: Any = None
PROCESS_SCRATCH_EXTENTS: dict[Path, int] = {}


def resource_seal(kind: str, body: dict[str, Any]) -> dict[str, Any]:
    unsigned = {"schema": RESOURCE_SCHEMA + "." + kind, **body}
    require("schema" not in body and "sha256" not in body, "resource_reserved_fields")
    return {**unsigned, "sha256": sha(canonical(unsigned))}


def path_without_symlink(path: Path) -> Path:
    absolute = Path(os.path.abspath(str(path)))
    for item in (absolute, *absolute.parents):
        require(not item.is_symlink(), "resource_path_symlink")
    return absolute.resolve()


def disjoint_path(left: Path, right: Path) -> bool:
    return left != right and left not in right.parents and right not in left.parents


def resource_path_gate(scratch: Path, output: Path, parents: Iterable[Path],
                       protected: Iterable[Path]) -> tuple[Path, Path]:
    scratch, output = path_without_symlink(scratch), path_without_symlink(output)
    require(not scratch.exists() and not output.exists(), "resource_fresh_scratch_and_output")
    require(disjoint_path(scratch, output), "resource_scratch_output_disjoint")
    for parent in parents:
        parent = path_without_symlink(parent)
        require(parent.is_dir() and disjoint_path(scratch, parent) and disjoint_path(output, parent),
                "resource_readonly_parent_disjoint")
    for item in protected:
        item = path_without_symlink(item)
        require(item.is_file() and disjoint_path(scratch, item) and disjoint_path(output, item),
                "resource_protected_file_disjoint")
    return scratch, output


def process_memory() -> dict[str, Any]:
    value: dict[str, Any] = {"ru_maxrss": None, "ru_maxrss_unit": None, "peak_rss_bytes": None,
        "VmRSS_bytes": None, "VmHWM_bytes": None, "VmSize_bytes": None,
        "rlimit_as_soft": None, "rlimit_as_hard": None}
    if sys.platform != "win32":
        import resource
        peak = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
        value.update(ru_maxrss=peak, ru_maxrss_unit="bytes" if sys.platform == "darwin" else "KiB",
                     peak_rss_bytes=peak if sys.platform == "darwin" else peak * 1024)
        soft, hard = resource.getrlimit(resource.RLIMIT_AS)
        value.update(rlimit_as_soft=soft, rlimit_as_hard=hard)
    if sys.platform.startswith("linux"):
        try:
            with open("/proc/self/status", "r", encoding="ascii") as stream:
                for line in stream:
                    fields = line.split()
                    if fields and fields[0] in ("VmRSS:", "VmHWM:", "VmSize:"):
                        if len(fields) == 3 and fields[2] == "kB" and fields[1].isdigit():
                            value[fields[0][:-1] + "_bytes"] = int(fields[1]) * 1024
        except OSError:
            pass  # Missing OS telemetry is null, never a fabricated zero.
    return value


class PagePool:
    def __init__(self, session: Any, rows: int, pages: int):
        require(plain_int(rows, 1) and plain_int(pages, 1), "private_cache_shape")
        self.session, self.rows, self.pages = session, rows, pages
        self.page_bytes = rows * NODE_STRUCT.size
        require((pages + 2) * self.page_bytes <= CACHE_LIMIT, "aggregate_index_cache_ceiling")
        self.cache: OrderedDict[tuple[int, int], list[Any]] = OrderedDict()
        self.hits = self.misses = self.evictions = self.flushes = 0
        self.max_resident_pages = 0

    def get(self, store: Any, page: int) -> list[Any]:
        key = store.number, page
        if key in self.cache:
            self.hits += 1
            self.cache.move_to_end(key)
            return self.cache[key]
        self.misses += 1
        if len(self.cache) >= self.pages:
            old_key = next(iter(self.cache))
            self.flush_one(old_key)
            del self.cache[old_key]
            self.evictions += 1
        data = bytearray(self.page_bytes)
        expected = min(self.rows, max(0, store.count - page * self.rows)) * NODE_STRUCT.size
        if expected:
            store.stream.seek(store.header_size + page * self.page_bytes)
            raw = store.stream.read(expected)
            self.session.io("index_read", len(raw))
            require(len(raw) == expected, "private_index_positioned_EOF")
            data[:expected] = raw
        value = [data, False]
        self.cache[key] = value
        self.max_resident_pages = max(self.max_resident_pages, len(self.cache))
        return value

    def flush_one(self, key: tuple[int, int]) -> None:
        data, dirty = self.cache[key]
        if not dirty:
            return
        store = self.session.stores[key[0]]
        used = min(self.rows, store.count - key[1] * self.rows) * NODE_STRUCT.size
        require(used > 0, "private_dirty_page_has_rows")
        offset = store.header_size + key[1] * self.page_bytes
        self.session.before_write(store.path, offset + used)
        store.stream.seek(offset)
        require(store.stream.write(memoryview(data)[:used]) == used, "private_index_full_write")
        self.session.io("index_write", used)
        self.cache[key][1] = False
        self.flushes += 1

    def flush_store(self, store: Any, *, discard: bool = False) -> None:
        for key in list(self.cache):
            if key[0] == store.number:
                self.flush_one(key)
                if discard:
                    del self.cache[key]

    def statistics(self) -> dict[str, Any]:
        return {"page_rows": self.rows, "page_bytes": self.page_bytes, "cache_pages_limit": self.pages,
            "io_buffer_reserve_bytes": 2 * self.page_bytes,
            "configured_cache_and_io_bytes": (self.pages + 2) * self.page_bytes,
            "resident_pages": len(self.cache), "resident_payload_bytes": len(self.cache) * self.page_bytes,
            "max_resident_pages": self.max_resident_pages, "hits": self.hits, "misses": self.misses,
            "evictions": self.evictions, "flushes": self.flushes}


class NodeView:
    def __init__(self, store: Any, field: str):
    ...
    def __getitem__(self, index: int) -> Any:
        if type(index) is not int:
            raise TypeError("node_view_integer_position")
        if index < 0:
            index += self.store.count
        if not 0 <= index < self.store.count:
            raise IndexError("node_view_position")
        digest, pair, _ = self.store.read(index)
        return digest if self.field == "hash" else pair


class NodeStore:
    def __init__(self, session: Any, number: int, purpose: str):
        self.session, self.number, self.purpose = session, number, purpose
        self.count = self.durable_count = 0
        self.finished = self.closed = False
        self.receipt: Any = None
        self.binding = sha(canonical({"session_sha256": session.start_sha256, "number": number,
                                     "purpose": purpose, "stride": NODE_STRUCT.size}))
        self.header = b"R07PND4\0" + struct.pack("<II", 4, NODE_STRUCT.size) + bytes.fromhex(self.binding)
        self.header_size = len(self.header)
        self.path = session.root / "indices" / (f"{number:06d}-" + purpose + ".bin")
        session.before_write(self.path, self.header_size)
        self.stream = self.path.open("x+b", buffering=0)
        require(self.stream.write(self.header) == self.header_size, "private_index_header_write")
        session.io("index_write", self.header_size)
        self.hashes, self.pairs = NodeView(self, "hash"), NodeView(self, "pair")

    def read(self, node: int) -> tuple[str, tuple[int, int], int]:
        require(not self.closed and plain_int(node, 0, self.count - 1), "private_existing_node_ID")
        page, row = divmod(node, self.session.pool.rows)
        data = self.session.pool.get(self, page)[0]
        digest, x, y, uses = NODE_STRUCT.unpack_from(data, row * NODE_STRUCT.size)
        require(x < 54 and y < 54, "private_residue54_range")
        return digest.hex(), (x, y), uses

    def append(self, digest: str, pair: tuple[int, int]) -> int:
        require(not self.closed and not self.finished and digest_string(digest) and
                isinstance(pair, tuple) and len(pair) == 2 and
                all(plain_int(value, 0, 53) for value in pair), "private_append_fields")
        if self.count >= UINT64_MAX:
            raise ResourceStop("private_node_count_capacity")
        node = self.count
        new_end = self.header_size + (node + 1) * NODE_STRUCT.size
        if new_end > UINT64_MAX:
            raise ResourceStop("private_node_offset_capacity")
        self.session.reserve(self.path, new_end)
        page, row = divmod(node, self.session.pool.rows)
        slot = self.session.pool.get(self, page)
        NODE_STRUCT.pack_into(slot[0], row * NODE_STRUCT.size, bytes.fromhex(digest), pair[0], pair[1], 0)
        slot[1] = True
        self.count += 1
        return node

    def add_use(self, node: int) -> None:
        digest, pair, uses = self.read(node)
        if uses == UINT64_MAX:
            raise ResourceStop("private_edge_count_capacity")
        page, row = divmod(node, self.session.pool.rows)
        slot = self.session.pool.get(self, page)
        NODE_STRUCT.pack_into(slot[0], row * NODE_STRUCT.size, bytes.fromhex(digest),
                              pair[0], pair[1], uses + 1)
        slot[1] = True

    def check_reachable(self, root: int) -> None:
        require(plain_int(root, 0) and root == self.count - 1, "private_single_last_root")
        for node in range(self.count):
            uses = self.read(node)[2]
            require((uses == 0) if node == root else (uses > 0), "private_all_nodes_reach_root")
            if node % 4096 == 0:
                check_resources("word-reachability")

    def flush(self) -> None:
        require(not self.closed, "private_open_index")
        self.session.pool.flush_store(self)
        self.stream.flush()
        os.fsync(self.stream.fileno())
        self.session.fsyncs += 1
        self.durable_count = self.count
        self.check_eof()

    def check_eof(self) -> None:
        require(not self.closed and not self.path.is_symlink(), "private_regular_open_index")
        self.stream.seek(0)
        header = self.stream.read(self.header_size)
        self.session.io("index_read", len(header))
        require(header == self.header, "private_header_binding_stride")
        self.stream.seek(0, os.SEEK_END)
        require(self.stream.tell() == self.header_size + self.count * NODE_STRUCT.size, "private_exact_index_EOF")

    def finish(self, root: int, *, reachable: bool = True) -> dict[str, Any]:
        require(not self.finished, "private_index_not_reused")
        if reachable:
        ...
        self.stream.seek(0)
        while True:
            raw = self.stream.read(self.session.pool.page_bytes)
            if not raw:
                break
            digest.update(raw)
            total += len(raw)
            self.session.io("index_pin_read", len(raw))
            check_resources("private-index-EOF")
        self.receipt = resource_seal("index", {"number": self.number, "purpose": self.purpose,
            "binding_sha256": self.binding, "file": self.path.relative_to(self.session.root).as_posix(),
            "bytes": total, "sha256_payload": digest.hexdigest(), "header_bytes": self.header_size,
            "stride": NODE_STRUCT.size, "encoding": "sha256-raw32,residue54-u8,residue54-u8,incoming-u64LE",
            "count": self.count, "root_id": root, "root_sha256": self.hashes[root],
            "all_nodes_reachable": reachable, "eof": True})
        self.session.write_json("index-receipts/" + f"{self.number:06d}.json", self.receipt)
        self.finished = True
        return self.receipt
    ...
    def close(self, *, flush: bool = True) -> None:
        if self.closed:
            return
        try:
            if flush and not self.finished:
                self.flush()
            self.session.pool.flush_store(self, discard=True) if flush else None
        finally:
            for key in list(self.session.pool.cache):
                if key[0] == self.number:
                    del self.session.pool.cache[key]
            self.stream.close()
            self.closed = True

    def statistics(self) -> dict[str, Any]:
        return {"number": self.number, "purpose": self.purpose, "rows": self.count,
                "durable_rows": self.durable_count, "logical_bytes": self.header_size + self.count * NODE_STRUCT.size,
                "actual_bytes": self.path.stat().st_size, "closed": self.closed, "finished": self.finished}


class ResourceSession:
    def __init__(self, root: Path, binding: dict[str, Any], *, fixture: bool = False,
                 page_rows: int = PAGE_ROWS, cache_pages: int = CACHE_PAGES,
                 line_limit: int = LINE_LIMIT, scratch_limit: int = SCRATCH_LIMIT,
                 free_floor: int = FREE_FLOOR):
        require(type(fixture) is bool and all(plain_int(x, 1) for x in (line_limit, scratch_limit)) and
                plain_int(free_floor, 0), "resource_limits_typed")
        if not fixture:
            require((page_rows, cache_pages, line_limit, scratch_limit, free_floor) ==
                    (PAGE_ROWS, CACHE_PAGES, LINE_LIMIT, SCRATCH_LIMIT, FREE_FLOOR), "registered_resource_limits")
        require(not root.exists() and not root.is_symlink(), "new_resource_session")
        self.root, self.fixture = root, fixture
        self.line_limit, self.scratch_limit, self.free_floor = line_limit, scratch_limit, free_floor
        self.extents: dict[Path, int] = {}
        self.stores: dict[int, NodeStore] = {}
        self.io_bytes: dict[str, int] = {}
        self.io_calls: dict[str, int] = {}
        self.fsyncs = 0
        self.last_sample = 0.0
        self.last_sample_node = 0
        self.sample_count = 0
        self.sampling = False
        self.stage: dict[str, Any] = {}
        self.word_counts: dict[str, dict[str, int]] = {}
        self.max_fan_in = self.max_line_bytes = self.max_frames = None
        self.frames = self.symbols = self.active_symbols = None
        self.completed_result: Any = None
        self.pool = PagePool(self, page_rows, cache_pages)
        self.start = resource_seal("start", {"invocation": uuid.uuid4().hex, "binding": binding,
            "format": "private-P4-node42-v1", "cache": self.pool.statistics(),
            "limits": {"line_bytes": line_limit, "scratch_bytes": scratch_limit, "free_floor_bytes": free_floor,
                       "process_memory_bytes": MEMORY_LIMIT_BYTES, "process_max_seconds": None if DEADLINE is None
                       else DEADLINE - STARTED}, "fixture_only": fixture, "resume": False})
        self.start_sha256 = sha(canonical(self.start))
        root.mkdir(parents=True, exist_ok=False)
        (root / "indices").mkdir()
        (root / "index-receipts").mkdir()
        self.write_json("start.json", self.start)
        self.sample("resource-session-created", force=True)

    def reserve(self, path: Path, size: int) -> None:
        require(plain_int(size, 0) and not path.is_symlink(), "resource_write_extent")
        old = self.extents.get(path, 0)
        if size > old:
            new_total = sum(value for name, value in self.extents.items() if self.root in name.parents)
            if self.root in path.parents:
                if new_total + size - old > self.scratch_limit:
                    raise ResourceStop("scratch_byte_limit")
                process_old = PROCESS_SCRATCH_EXTENTS.get(path, 0)
                if sum(PROCESS_SCRATCH_EXTENTS.values()) + size - process_old > SCRATCH_LIMIT:
                    raise ResourceStop("process_scratch_byte_limit")
                PROCESS_SCRATCH_EXTENTS[path] = size
            self.extents[path] = size

    def before_write(self, path: Path, size: int) -> None:
        self.reserve(path, size)
        if size > UINT64_MAX:
            raise ResourceStop("write_offset_capacity")
        parent = path.parent
        while not parent.exists():
            parent = parent.parent
        free = shutil.disk_usage(parent).free
        previous = path.stat().st_size if path.exists() else 0
        if free - max(0, size - previous) < self.free_floor:
            raise ResourceStop("disk_free_floor")

    def io(self, category: str, count: int) -> None:
        self.io_bytes[category] = self.io_bytes.get(category, 0) + count
        self.io_calls[category] = self.io_calls.get(category, 0) + 1

    def write_json(self, name: str, value: Any) -> None:
        path = self.root / name
        require(not path.exists() and not path.is_symlink(), "fresh_resource_receipt")
        raw = canonical(value)
        self.before_write(path, len(raw))
        with path.open("xb") as stream:
            require(stream.write(raw) == len(raw), "resource_receipt_full_write")
            stream.flush()
            os.fsync(stream.fileno())
            self.fsyncs += 1
        self.io("resource_receipt_write", len(raw))

    def new_store(self, purpose: str) -> NodeStore:
        require(purpose in ("build", "reread", "fixture"), "private_index_purpose")
        number = len(self.stores)
        store = NodeStore(self, number, purpose)
        self.stores[number] = store
        return store

    def observe_node(self, purpose: str, args: Any, op: str, length: int) -> None:
        counters = self.word_counts.setdefault(purpose, {"nodes": 0, "edges": 0, "zero_power_edges": 0, "refs": 0})
        fan_in = len(child_links(op, args))
        counters["nodes"] += 1
        counters["edges"] += fan_in
        counters["zero_power_edges"] += int(op == "IntegerPower" and args["exponent"] == 0)
        counters["refs"] += int(op == "Ref")
        self.max_fan_in, self.max_line_bytes = max(self.max_fan_in or 0, fan_in), max(self.max_line_bytes or 0, length)
        self.sample(PHASE)

    def sample(self, phase: str, *, force: bool = False) -> None:
        if self.sampling:
            return
        now = time.monotonic()
        nodes = sum(item["nodes"] for item in self.word_counts.values())
        if not force and nodes - self.last_sample_node < 4096 and now - self.last_sample < 5:
            return
        self.sampling = True
        try:
            value = resource_seal("sample", {"sample": self.sample_count, "phase": phase,
                "elapsed_seconds": now - STARTED, "memory": process_memory(), "cache": self.pool.statistics(),
                "indices": [store.statistics() for store in self.stores.values()], "words": self.word_counts,
                "io_bytes": self.io_bytes, "io_calls": self.io_calls, "fsyncs": self.fsyncs,
                "session_scratch_reserved_bytes": sum(size for name, size in self.extents.items()
                                                     if self.root in name.parents),
                "process_scratch_reserved_bytes": sum(PROCESS_SCRATCH_EXTENTS.values()),
                "max_fan_in": self.max_fan_in, "max_line_bytes": self.max_line_bytes,
                "frames": self.frames, "max_frames": self.max_frames,
                "symbols": self.symbols, "active_symbols": self.active_symbols,
                "semantic_live_nodes": None, "parent_object_overhead_bytes": None,
                "snapshot": self.stage, "last_sample_is_failure_peak": False})
            raw = canonical(value)
            path = self.root / "telemetry.jsonl"
            current = path.stat().st_size if path.exists() else 0
            self.before_write(path, current + len(raw))
            with path.open("ab") as stream:
                require(stream.write(raw) == len(raw), "resource_sample_full_write")
                stream.flush()
                os.fsync(stream.fileno())
                self.fsyncs += 1
            self.io("telemetry_write", len(raw))
            self.sample_count += 1
            self.last_sample, self.last_sample_node = now, nodes
        finally:
            self.sampling = False
    ...
    def finish(self, status: str, result: Any = None) -> None:
        if self.completed_result is not None:
            return
        if status == "PASS" and not self.fixture:
            require(len(self.stores) == 2 and all(store.finished and store.closed for store in self.stores.values()),
                    "both_full_word_indices_completed")
        self.sample("resource-session-" + status, force=True)
        value = resource_seal("result", {"status": status, "session_sha256": self.start_sha256,
            "word_result_sha256": None if result is None else sha(canonical(result)),
            "root_id": None if result is None else result.get("root_id"),
            "root_sha256": None if result is None else result.get("root_sha256"),
            "word_manifest_sha256": None if result is None else result.get("word_manifest_sha256"),
            "indices": [store.receipt for store in self.stores.values() if store.finished],
            "index_states": [store.statistics() for store in self.stores.values()],
            "cache": self.pool.statistics(), "samples": self.sample_count,
            "eof": status == "PASS", "successful_word_bundle": status == "PASS" and not self.fixture,
            "fixture_only": self.fixture, "candidate": False, "cross_checked": False, "verified": False})
        self.write_json("result.json", value)
        self.completed_result = value
    ...
    def abort_close(self) -> None:
        for store in self.stores.values():
            if not store.closed:
                try:
                    store.close()
                except (OSError, ValueError, ResourceStop, MemoryError):
                    store.close(flush=False)


def limited_lines(stream: BinaryIO, category: str = "jsonl_scan") -> Iterable[bytes]:
    limit = LINE_LIMIT if RESOURCE is None else RESOURCE.line_limit
    while True:
        raw = stream.readline(limit + 1)
        if RESOURCE is not None:
            RESOURCE.io(category, len(raw))
            RESOURCE.max_line_bytes = max(RESOURCE.max_line_bytes or 0, len(raw))
        if len(raw) > limit:
            if RESOURCE is not None:
                RESOURCE.stage["oversized_line"] = {"observed_prefix_bytes": len(raw), "complete_line": False}
            raise ResourceStop("jsonl_line_limit_including_LF")
        if not raw:
            return
        yield raw


def resource_before_write(path: Path, size: int) -> None:
    if RESOURCE is not None:
        RESOURCE.before_write(path, size)


def resource_io(category: str, count: int) -> None:
    if RESOURCE is not None:
        RESOURCE.io(category, count)
...
def progress(phase: str, **fields: Any) -> None:
    ...
    if RESOURCE is not None:
        RESOURCE.sample(phase, force=True)
...
def file_pin(path: Path) -> dict[str, Any]:
    ...
    with path.open("rb") as stream:
        while True:
            ...
            digest.update(chunk)
            length += len(chunk)
            resource_io("whole_file_pin_read", len(chunk))
...
def read_json_file(path: Path, *, canonical_required: bool = True) -> Any:
    raw = path.read_bytes()
    resource_io("parent_json_read", len(raw))
    value = json.loads(raw.decode("utf-8"))
    resource_io("parent_json_parse_bytes", len(raw))
...
class WordDAG:
    ...
    store: Any = None
    hashes: Any = field(init=False)
    pairs: Any = field(init=False)
    symbols: dict[tuple[str, str, str], int] = field(default_factory=dict)
    length: int = 0
    file_digest: Any = field(default_factory=hashlib.sha256)

    def __post_init__(self) -> None:
        require(RESOURCE is not None, "explicit_resource_session_for_WordDAG")
        if self.store is None:
            self.store = RESOURCE.new_store("fixture")
        self.hashes, self.pairs = self.store.hashes, self.store.pairs
    ...
    def add(self, op: str, args: dict[str, Any], refs: Iterable[int] = ()) -> int:
        ...
        RESOURCE.max_line_bytes = max(RESOURCE.max_line_bytes or 0, len(raw))
        if len(raw) > RESOURCE.line_limit:
            raise ResourceStop("writer_canonical_line_limit_including_LF")
        resource_before_write(Path(self.stream.name), self.length + len(raw))
        require(self.stream.write(raw) == len(raw), "word_stream_full_write")
        resource_io("word_write", len(raw))
        RESOURCE.observe_node(self.store.purpose, args, op, len(raw))
        pair = node_residue(op, args, self.pairs)
        require(self.store.append(record["node_sha256"], pair) == index, "private_append_original_ID")
        for child in child_links(op, args):
            self.store.add_use(child["node"])
        self.length += len(raw)
...
def admit_parents(args: argparse.Namespace) -> Parents:
    ...
    wanted_sources = {"producer": "search/d972_r07_continuation_positive_word_readout_v4.py",
                      "checker": "search/check_d972_r07_continuation_same_word_eleven_slots_v4.py"}
...
class Ancestors:
    ...
    def add(self, namespace: str, role: str, name: str, *, offset: int | None = None,
        ...
        if pointer is not None:
            ...
            resource_io("positioned_ancestor_read", len(raw))
...
def read_positioned(parents: Parents, role: str, name: str, offset: int, length: int, digest: str) -> Any:
    require(plain_int(offset, 0) and plain_int(length, 1), "positioned_JSONL_extent")
    limit = LINE_LIMIT if RESOURCE is None else RESOURCE.line_limit
    if length > limit:
        raise ResourceStop("positioned_JSONL_line_limit")
    with parents.path(role, name).open("rb") as stream:
        stream.seek(offset)
        raw = stream.read(length)
    resource_io("positioned_parent_read", len(raw))
    require(len(raw) == length and sha(raw) == digest and raw.endswith(b"\n"), "positioned_record_bytes")
    value = json.loads(raw.decode("ascii"))
    resource_io("positioned_parent_parse_bytes", len(raw))
...
def validate_original_start_header(start: Any) -> None:
    """Authenticate the original start; JSON bool/float are not append counts."""
    require(isinstance(start, dict), "original_start_object")
    for key, wanted in (("rank", 1386), ("generation", 8091),
                        ("completed_steps", 0), ("external_e_attached", 1)):
        require(type(start.get(key)) is int and start[key] == wanted,
                "original_start_exact_integer:" + key)
    require(start.get("external_e_numerically_replayed") is False,
            "original_start_external_E_not_replayed")
...
class TargetHistory:
    ...
    def add_row(self, role: str, name: str, offset: int, digest: str | None, lead: int) -> int:
        ...
        resource_io("physical_row_read", len(raw))
    ...
    def output(self) -> dict[str, Any]:
        ...
        resource_io("target_payload_read", len(residual))
...
def read_base_target(history: TargetHistory) -> None:
    ...
    with parents.path("state", "state/instructions.jsonl").open("rb") as stream:
        for offer, raw in enumerate(limited_lines(stream, "base_instruction_scan")):
            require(raw.endswith(b"\n"), "base_instruction_final_LF")
            record = json.loads(raw.decode("ascii"))
            resource_io("base_instruction_parse_bytes", len(raw))
...
REFINEMENT_SCHEMA = "d972.r07.full-origin-refinement.v1"
REFINEMENT_PRODUCER_SHA = "d7e32aad9a9667c6af54ed7514d0417e48b3e363c60652ab585ce4633f2aedfa"
REFINEMENT_HEAD_PIN = (921, "6bf3b4fce6a3f159563c13a9aa50f6478827fbad1af13d820b70359b3b2f5cba")
REFINEMENT_BINDING_FIELDS = ("owner_sha256", "producer_sha256", "source_sha256", "start_sha256",
    "canonical_index_sha256", "packet_manifest_sha256")
REFINEMENT_INSTRUCTION_END_FIELDS = ("schema", "step", "predecessor", "offer", "generation", "rank", "lead",
    "sigma", "physical_offset", "packet_manifest_sha256", "scan_manifest_sha256", "materialization_sha256",
    "canonical_index_sha256", "physical_sha256", "target_scalar", "target_remainder_sha256", "rolling_sha256")
REFINEMENT_RESULT_END_FIELDS = ("schema", "step", "kind", "owner_sha256", "packet_manifest_sha256",
    "parent_state_head", "state_head", "rank_before", "rank_after", "generation_before", "generation_after",
    "scan_manifest_sha256", "materialization_sha256", "target")


def validate_refinement_head(head: Any, bindings: Any) -> None:
    """Read the actual old HEAD type; it has no target hash field."""
    fields = {"schema", "sha256", *REFINEMENT_BINDING_FIELDS, "completed_steps", "step_manifest_sha256",
              "current_scan_manifest_sha256", "rank", "generation", "state_head", "kind"}
    require(isinstance(head, dict) and set(head) == fields, "refinement_HEAD_exact_fifteen_fields")
    check_seal(head, REFINEMENT_SCHEMA + ".head")
    require(isinstance(bindings, dict) and set(bindings) == set(REFINEMENT_BINDING_FIELDS),
            "refinement_actual_binding_fields")
    for key in REFINEMENT_BINDING_FIELDS:
        require(digest_string(bindings[key]) and head[key] == bindings[key], "refinement_HEAD_binding:" + key)
    require(bindings["producer_sha256"] == REFINEMENT_PRODUCER_SHA, "refinement_frozen_producer")
    for key, expected in (("completed_steps", 26), ("rank", 1385), ("generation", 8090)):
        require(plain_int(head[key], expected, expected), "refinement_HEAD_ordinary_integer:" + key)
    require(head["kind"] == "Separator" and head["current_scan_manifest_sha256"] is None and
            digest_string(head["state_head"]) and digest_string(head["step_manifest_sha256"]),
            "refinement_saved_terminal_cursor")


def validate_refinement_terminal(head: Any, bindings: Any, replay: Any, last: Any) -> None:
    """Join the pinned HEAD to metadata projected only after full step authentication."""
    validate_refinement_head(head, bindings)
    require(isinstance(replay, dict) and set(replay) == {"completed_steps", "rank", "generation", "state_head",
            "target_remainder_sha256", "last_step_manifest_sha256", "last_parent_target_sha256"},
            "refinement_replayed_end_fields")
    for key in ("completed_steps", "rank", "generation"):
        require(plain_int(replay[key], head[key], head[key]), "refinement_replayed_end_integer:" + key)
    require(replay["state_head"] == head["state_head"] and
            replay["last_step_manifest_sha256"] == head["step_manifest_sha256"] and
            digest_string(replay["target_remainder_sha256"]) and digest_string(replay["last_parent_target_sha256"]),
            "refinement_replayed_end_hashes")
    require(isinstance(last, dict) and set(last) == {"manifest", "instruction", "result", "payloads"},
            "refinement_last_projection_fields")
    manifest, instruction, result = last["manifest"], last["instruction"], last["result"]
    check_seal(manifest, REFINEMENT_SCHEMA + ".step-manifest")
    require(set(manifest) == {"schema", "sha256", "files", "step", "owner_sha256", "packet_manifest_sha256",
            "parent_state_head", "predecessor_step_manifest_sha256", "scan_manifest_sha256",
            "state_head", "rank", "generation", "kind"}, "refinement_last_manifest_exact_fields")
    require(sha(canonical(manifest)) == head["step_manifest_sha256"], "refinement_last_manifest_HEAD_hash")
    require(isinstance(instruction, dict) and set(instruction) == set(REFINEMENT_INSTRUCTION_END_FIELDS) and
            instruction["schema"] == REFINEMENT_SCHEMA + ".instruction", "refinement_instruction_projection")
    require(isinstance(result, dict) and set(result) == set(REFINEMENT_RESULT_END_FIELDS) and
            result["schema"] == REFINEMENT_SCHEMA + ".step-result", "refinement_result_projection")
    for value in (manifest, instruction, result):
        require(plain_int(value["step"], 26, 26), "refinement_last_step_ordinary_integer")
        require(value["packet_manifest_sha256"] == bindings["packet_manifest_sha256"],
                "refinement_last_packet_binding")
    for value in (manifest, result):
        require(value["owner_sha256"] == bindings["owner_sha256"] and value["kind"] == head["kind"] and
                value["state_head"] == head["state_head"], "refinement_last_owner_kind_state")
    for value in (manifest, instruction):
        require(plain_int(value["rank"], head["rank"], head["rank"]) and
                plain_int(value["generation"], head["generation"], head["generation"]),
                "refinement_last_rank_generation")
    require(plain_int(result["rank_before"], 1384, 1384) and plain_int(result["rank_after"], 1385, 1385) and
            plain_int(result["generation_before"], 8089, 8089) and
            plain_int(result["generation_after"], 8090, 8090), "refinement_last_result_rank_generation")
    require(instruction["rolling_sha256"] == head["state_head"] and
            instruction["predecessor"] == manifest["parent_state_head"] == result["parent_state_head"] and
            instruction["canonical_index_sha256"] == bindings["canonical_index_sha256"] and
            instruction["scan_manifest_sha256"] == manifest["scan_manifest_sha256"] == result["scan_manifest_sha256"] and
            instruction["materialization_sha256"] == result["materialization_sha256"],
            "refinement_last_instruction_chain")
    require(plain_int(instruction["offer"], 8089, 8089) and
            plain_int(instruction["physical_offset"], 1384 * PHYSICAL_BYTES, 1384 * PHYSICAL_BYTES) and
            plain_int(instruction["lead"], 0, PHYSICAL_TRITS - 1) and plain_int(instruction["sigma"], 1, 2),
            "refinement_last_instruction_position")
    target = result["target"]
    require(isinstance(target, dict) and set(target) ==
            {"parent_remainder_sha256", "remainder_sha256", "scalar"} and
            plain_int(target["scalar"], 0, 2) and plain_int(instruction["target_scalar"], 0, 2) and
            target["scalar"] == instruction["target_scalar"], "refinement_last_plain_target_type")
    require(target["parent_remainder_sha256"] == replay["last_parent_target_sha256"] and
            target["remainder_sha256"] == instruction["target_remainder_sha256"] ==
            replay["target_remainder_sha256"], "refinement_last_target_not_in_HEAD")
    require(isinstance(manifest["files"], list), "refinement_last_payload_roster")
    names = []
    for entry in manifest["files"]:
        pin_entry(entry)
        names.append(entry["file"])
    require(names == sorted(set(names)), "refinement_last_unique_payloads")
    files = {entry["file"]: entry for entry in manifest["files"]}
    require(isinstance(last["payloads"], dict) and set(last["payloads"]) ==
            {"instruction.json", "result.json", "target-remainder.bin"}, "refinement_last_actual_payloads")
    for name, entry in last["payloads"].items():
        pin_entry(entry)
        require(entry["file"] == name and entry == files[name], "refinement_last_actual_file:" + name)
    require(last["payloads"]["target-remainder.bin"]["bytes"] == PHYSICAL_BYTES and
            last["payloads"]["target-remainder.bin"]["sha256"] == target["remainder_sha256"] and
            files["materialization.json"]["sha256"] == instruction["materialization_sha256"] and
            files["physical-normalized.bin"]["sha256"] == instruction["physical_sha256"],
            "refinement_last_target_and_normalized_payloads")


def read_refinement_parent(history: TargetHistory) -> tuple[Any, Any, Any]:
    parents = history.parents
    head = parents.read("refinement", "output/HEAD")
    source, start, owner = (parents.read("refinement", "output/" + name + ".json")
                            for name in ("source", "start", "owner"))
    for name, value in (("source", source), ("start", start), ("owner", owner)):
        check_seal(value, REFINEMENT_SCHEMA + "." + name)
    bindings = {"owner_sha256": parents.pin("refinement", "output/owner.json")["sha256"],
        "producer_sha256": source["producer_sha256"],
        "source_sha256": parents.pin("refinement", "output/source.json")["sha256"],
        "start_sha256": parents.pin("refinement", "output/start.json")["sha256"],
        "canonical_index_sha256": parents.pin("refinement", "output/canonical-index.json")["sha256"],
        "packet_manifest_sha256": parents.pin("packet", "output/packet/manifest.json")["sha256"]}
    validate_refinement_head(head, bindings)
    size, digest = REFINEMENT_HEAD_PIN
    require(parents.pin("refinement", "output/HEAD") == {"file": "output/HEAD", "bytes": size, "sha256": digest},
            "actual_accepted_refinement_HEAD_bytes")
    require(plain_int(start["rank"], 1359, 1359) and plain_int(start["generation"], 8064, 8064) and
            start["rank"] == len(history.pivots) and start["generation"] == history.generation and
            start["state_head"] == history.current_state and start["target_remainder_sha256"] == history.current_target and
            start["accepted_target_derivation_parents"] == history.named_parents, "refinement_original_packet_start")
    check_seal(start["packet_parent_layout"], REFINEMENT_SCHEMA + ".packet-parent-layout")
    require(start["packet_parent_layout"]["state_head"] == start["state_head"] and
            start["packet_parent_layout"]["target_remainder_sha256"] == start["target_remainder_sha256"] and
            owner["accepted_packet_manifest_sha256"] == bindings["packet_manifest_sha256"] and
            owner["accepted_packet_owner_sha256"] == parents.pin("packet", "output/owner.json")["sha256"],
            "refinement_owner_packet_join")
    terminal = parents.read("refinement", "output/result.json")
    check_seal(terminal, REFINEMENT_SCHEMA + ".result")
    require(terminal["status"] == "PASS" and terminal["head_sha256"] == digest and
            terminal["owner_sha256"] == bindings["owner_sha256"] and
            terminal["packet_manifest_sha256"] == bindings["packet_manifest_sha256"] and
            terminal["state_head"] == head["state_head"] and terminal["scan_manifest_sha256"] is None,
            "refinement_saved_result_HEAD_join")
    for key in ("completed_steps", "rank", "generation"):
        require(plain_int(terminal[key], head[key], head[key]), "refinement_result_count:" + key)
    return head, bindings, terminal


def read_target_history(parents: Parents, ancestors: Ancestors) -> TargetHistory:
    progress("history-selected-continuation")
    ...
    for number in range(1, 4):
        progress("history-packet-step", step=number)
    ...
    progress("history-refinement-parent")
    ref_head, ref_bindings, ref_terminal = read_refinement_parent(history)
    previous = None
    for number in range(1, 27):
        progress("history-refinement-step", step=number)
        directory = f"output/steps/{number:06d}"
        manifest = authenticate_file_manifest(parents, "refinement", directory + "/manifest.json")
        check_seal(manifest, REFINEMENT_SCHEMA + ".step-manifest")
        require(plain_int(manifest["step"], number, number) and
                manifest["parent_state_head"] == history.current_state and
        ...
        last_parent_target = history.current_target
        instruction, ref_result = history.add_delta("refinement", directory, {"kind": "refinement",
            "parent_role": "refinement", "file": directory + "/materialization.json", "pointer": ""},
            f"refinement-step-{number}")
        check_seal(ref_result, REFINEMENT_SCHEMA + ".step-result")
        require(instruction["schema"] == REFINEMENT_SCHEMA + ".instruction" and
                plain_int(instruction["step"], number, number) and plain_int(ref_result["step"], number, number) and
                instruction["materialization_sha256"] == ref_result["materialization_sha256"] ==
                parents.pin("refinement", directory + "/materialization.json")["sha256"] and
                manifest["state_head"] == history.current_state and
                plain_int(manifest["rank"], len(history.pivots), len(history.pivots)) and
                plain_int(manifest["generation"], history.generation, history.generation) and
                manifest["kind"] == ref_result["kind"] == ref_head["kind"], "refinement_materialization_join")
        require(manifest["owner_sha256"] == ref_result["owner_sha256"] == ref_bindings["owner_sha256"] and
                manifest["packet_manifest_sha256"] == instruction["packet_manifest_sha256"] ==
                ref_result["packet_manifest_sha256"] == ref_bindings["packet_manifest_sha256"] and
                instruction["canonical_index_sha256"] == ref_bindings["canonical_index_sha256"],
                "refinement_each_step_original_bindings")
        previous = parents.pin("refinement", directory + "/manifest.json")["sha256"]
    last = {"manifest": manifest,
        "instruction": {key: instruction[key] for key in REFINEMENT_INSTRUCTION_END_FIELDS},
        "result": {key: ref_result[key] for key in REFINEMENT_RESULT_END_FIELDS},
        "payloads": {name: {**parents.pin("refinement", directory + "/" + name), "file": name}
                     for name in ("instruction.json", "result.json", "target-remainder.bin")}}
    validate_refinement_terminal(ref_head, ref_bindings, {
        "completed_steps": number, "rank": len(history.pivots), "generation": history.generation,
        "state_head": history.current_state, "target_remainder_sha256": history.current_target,
        "last_step_manifest_sha256": previous, "last_parent_target_sha256": last_parent_target}, last)
    require(ref_terminal["lambda_rho2"] == ref_result["separator"]["lambda_rho2"],
            "refinement_terminal_last_derived_receipt")
    progress("history-refinement-complete", steps=number, pivots=len(history.pivots))
    progress("history-external-E")
    ...
    for number in range(1, head["completed_steps"] + 1):
        progress("history-continuation-step", step=number)
        previous = read_loop_step(history, number, previous)
    progress("history-continuation-final")
...
def load_literal_parents(parents: Parents, ancestors: Ancestors, history: TargetHistory,
    ...
    with parents.path("p1", ip["path"]).open("rb") as stream:
        for node, raw in enumerate(limited_lines(stream, "P1_instruction_scan")):
            require(node < P1_ROWS and raw.endswith(b"\n"), "P1_full_instruction_stream")
            record = json.loads(raw.decode("ascii"))
            resource_io("P1_instruction_parse_bytes", len(raw))
...
class TargetWordCompiler:
        ...
        while True:
            ...
            try:
                ...
                if not stack:
                    if RESOURCE is not None:
                        RESOURCE.frames = 0
                        RESOURCE.active_symbols = 0
                        RESOURCE.symbols = len(self.dag.symbols)
                    return node
                sent, current = node, None
            if RESOURCE is not None:
                RESOURCE.frames = len(stack)
                RESOURCE.max_frames = max(RESOURCE.max_frames or 0, len(stack))
                RESOURCE.symbols = len(self.dag.symbols)
                RESOURCE.active_symbols = len(active)
                RESOURCE.stage = {"ancestor_receipts": len(self.ancestors.entries),
                    "recipe_ref_symbols": len(self.recipe_refs), "raw_cache_entries": len(self.raw_data),
                    "geometry_cache_entries": len(self.geometries), "symbol_order_entries": len(self.symbol_order),
                    "paused_recipe_object_bytes": None, "paused_factor_bytes": None}
    ...
    def geometry(self, recipe: Any) -> Any:
        ...
        if scope not in self.geometries:
            ...
            for name, count in (("parent.u32", 54432), ("parent-edge.u32", 54432), ("next-pos.u32", 108864)):
                ...
                resource_io("geometry_read", len(raw))
    ...
    def build_E_source(self, recipe: Any, refs: list[int]) -> Any:
        ...
        actual_alpha = self.parents.path(role, alpha_name).read_bytes()
        resource_io("alpha_payload_read", len(actual_alpha))
        require(actual_alpha == bytes(alpha), "E_actual_full_alpha_bytes")
        value = self.dag.product(factors, refs)
...
def write_output(root: Path, name: str, raw: bytes) -> dict[str, Any]:
    ...
    resource_before_write(root / name, len(raw))
    with (root / name).open("xb") as stream:
        require(stream.write(raw) == len(raw), "output_complete_write")
        resource_io("word_metadata_write", len(raw))
        stream.flush()
        os.fsync(stream.fileno())
        if RESOURCE is not None:
            RESOURCE.fsyncs += 1
...
def compile_target_word(history: TargetHistory, literal: LiteralParents, output: Path,
    ...
    require(RESOURCE is not None, "production_resource_session")
    store = RESOURCE.new_store("build")
    try:
        with (output / "ordered-word.jsonl").open("xb") as stream:
            dag = WordDAG(stream, dictionary_sha,
                ...
                lambda: len(history.ancestors.entries), store)
...
def read_normalized_pair(output: Path, word_manifest: Any, dictionary: Any, ancestor_count: int) -> dict[str, Any]:
    require(RESOURCE is not None, "reread_uses_new_empty_private_index")
    require(word_manifest["grammar"] == GRAMMAR and word_manifest["nodes_file"]["file"] == "ordered-word.jsonl",
            "C_same_word_bundle")
    store = RESOURCE.new_store("reread")
    hashes, pairs, total, count = store.hashes, store.pairs, 0, 0
    digest = hashlib.sha256()
    try:
        with (output / "ordered-word.jsonl").open("rb") as stream:
            for node_id, raw in enumerate(limited_lines(stream, "word_reread_scan")):
                require(raw.endswith(b"\n"), "C_JSONL_final_LF")
                node = json.loads(raw.decode("ascii"))
                resource_io("word_json_parse_bytes", len(raw))
                ...
                pair = node_residue(node["op"], node["args"], pairs)
                require(store.append(node["node_sha256"], pair) == node_id, "C_disk_original_ID")
                for child in child_links(node["op"], node["args"]):
                    store.add_use(child["node"])
                total += len(raw)
                count += 1
                digest.update(raw)
                RESOURCE.observe_node("reread", node["args"], node["op"], len(raw))
                if count % 512 == 0:
        ...
        store.finish(word_manifest["root_id"])
...
def run_actual(args: argparse.Namespace) -> Any:
    global OUTPUT_CREATED, RESOURCE
    protected = [Path(args.acceptance), Path(__file__).resolve(),
        SOURCE_ROOT / "search/check_d972_r07_continuation_same_word_eleven_slots_v4.py",
        *(SOURCE_ROOT / name for name in DATA_PINS)]
    roots = [Path(getattr(args, role.replace("-", "_") + "_root")) for role in PARENT_ROLES
             if not role.startswith("block-")] + [Path(value) for value in args.block_root]
    scratch, output = resource_path_gate(Path(args.scratch), Path(args.output), roots, protected)
    print(json.dumps({"schema": RESOURCE_SCHEMA + ".pre-admission", "memory": process_memory(),
                      "max_memory_mib": args.max_memory_mib, "max_seconds": args.max_seconds},
                     sort_keys=True), file=sys.stderr, flush=True)
    parents = admit_parents(args)
    RESOURCE = accepted_resource_session(args, parents)
    ancestors = Ancestors(parents)
    history = read_target_history(parents, ancestors)
    require(all(plain_int(history.head.get(key), expected, expected) for key, expected in
                (("completed_steps", 64), ("rank", 1450), ("generation", 8155))),
            "registered_positive_resource_parent64")
...
def selftest_word(root: Path) -> dict[str, Any]:
    ...
    with (root / "ordered-word.jsonl").open("xb") as stream:
        ...
        target = dag.product([dag.power(x, 18), dag.power(y, -18), second,
                              dag.power(first, 0), x, inverse, identity, dag.power(xy, 0), dag.power(yx, 0)])
...
def selftest_original_start_header() -> Any:
    """Metadata-only fixture using the five observed, pinned original-start fields."""
    header = json.loads(canonical({"rank": 1386, "generation": 8091, "completed_steps": 0,
        "external_e_attached": 1, "external_e_numerically_replayed": False}))
    validate_original_start_header(header)
    rejected: list[str] = []
    for name, value in (("attached-bool-true", True), ("attached-float-one", 1.0),
                        ("attached-string-one", "1"), ("attached-zero", 0), ("attached-two", 2)):
        mutated = json.loads(canonical({**header, "external_e_attached": value}))
        selftest_rejection(name, lambda mutated=mutated: validate_original_start_header(mutated), rejected)
    for name, key, value in (("rank-float", "rank", 1386.0),
                             ("generation-float", "generation", 8091.0),
                             ("completed-bool-false", "completed_steps", False),
                             ("replayed-integer-zero", "external_e_numerically_replayed", 0)):
        mutated = json.loads(canonical({**header, key: value}))
        selftest_rejection(name, lambda mutated=mutated: validate_original_start_header(mutated), rejected)
    size, digest = LOOP_FIXED_FILES["output/start.json"]
    return {"name": "actual-start-header-count-type", "status": "PASS", "rejected_cases": rejected,
        "accepted_header": header, "source_header_file": {"file": "output/start.json", "bytes": size, "sha256": digest},
        "actual_parent_numerically_replayed": False}



REFINEMENT_END_METADATA_FIXTURE = json.loads(r'''{
  "head": {
    "canonical_index_sha256": "452fe97a9229fa5188493256d1478ead1e684b495bbfed0db03a64f5acf4f00e",
    "completed_steps": 26,
    "current_scan_manifest_sha256": null,
    "generation": 8090,
    "kind": "Separator",
    "owner_sha256": "c4fd8b27590450d0b73e72efe9d45bf9319e111b5e21d1f3ff0b0ee23910f48c",
    "packet_manifest_sha256": "d5e3ef0c0d691131b6bd1293d066d6e994c572086dc0c89a6e5ec766a8474199",
    "producer_sha256": "d7e32aad9a9667c6af54ed7514d0417e48b3e363c60652ab585ce4633f2aedfa",
    "rank": 1385,
    "schema": "d972.r07.full-origin-refinement.v1.head",
    "sha256": "a756db9a15f9bfce5b9919cea13582164bd6095ba11de05dc393e4a1f125dc12",
    "source_sha256": "7e99018f58f3f49e371b55e6daab491b71855bb463c8c47cd872dffb57b5774f",
    "start_sha256": "1a709c2853a6d0c239bc31d50ba6e03b0fb4707d93b625d291a487e6d43dc131",
    "state_head": "8f6605a28d337cd8541a7eacf6aef78f5a70308a6bb71fd105138803ca623a61",
    "step_manifest_sha256": "1bfd33af5054a11b8210781146a872e914acb1bd7214b0b945f7e3520b31200c"
  },
  "bindings": {
    "owner_sha256": "c4fd8b27590450d0b73e72efe9d45bf9319e111b5e21d1f3ff0b0ee23910f48c",
    "producer_sha256": "d7e32aad9a9667c6af54ed7514d0417e48b3e363c60652ab585ce4633f2aedfa",
  ...
  "replay": {
    ...
    "last_step_manifest_sha256": "1bfd33af5054a11b8210781146a872e914acb1bd7214b0b945f7e3520b31200c",
    "last_parent_target_sha256": "5cb563ec85586ff7653ded61edb51dfb8748576a8e42d92323625552b5c96427"
  },
  "last": {
    "manifest": {
      "files": [
        {
          "bytes": 147304,
          "file": "instruction.json",
          "sha256": "db5327c34a6447220a4309bd4f606a9372849977221bb1c290730c53df52ddc9"
        },
        {
          "bytes": 12096,
          "file": "lambda.bin",
          "sha256": "1e720af4a30bac955ab4565366f0242b5c2d43125eb280e241df20976331cdf1"
        },
        {
          "bytes": 5892217,
          "file": "materialization.json",
          "sha256": "e98048839da8b75f4ebf434a7f8599cfd148ce656a4acf43f46668ebe566749c"
        },
        {
          "bytes": 12096,
          "file": "physical-normalized.bin",
          "sha256": "1ed0c6c5c35b5ae13ab528e6151be9d8d1c753c3cf02b6e62842cce76ee2c678"
        },
        {
          "bytes": 12096,
          "file": "physical-raw.bin",
          "sha256": "cac07ac713c35ee927d091ef382557f18f0d45bd847bc9de08602540541398c4"
        },
        {
          "bytes": 12096,
          "file": "physical-remainder.bin",
          "sha256": "468e0ffd03bab1cbb747162d1003275fb21a334cedbaa5d8d58371163da1ee56"
        },
        {
          "bytes": 151584,
          "file": "result.json",
          "sha256": "45588d8b319fe4c3497bb9ae6d7768119711aa2c8779779945bdf5fcbf78edd7"
        },
        {
          "bytes": 9072,
          "file": "source-d.bin",
          "sha256": "bc4d3a101a97d55a5db0c067dac88777b6efffdfd302d3a9f97d665b6b0a365d"
        },
        {
          "bytes": 36288,
          "file": "source-full-top.bin",
          "sha256": "d137424dafa0a4a3fc5079c8f2f90c6592930998c752f933aa69c48428d1d0f9"
        },
        {
          "bytes": 12096,
          "file": "target-remainder.bin",
          "sha256": "111d12e064b96a6bf579f39a9c9d5e35181560c0403bf0d237bffc924230c0ad"
        }
      ],
      "generation": 8090,
      "kind": "Separator",
      "owner_sha256": "c4fd8b27590450d0b73e72efe9d45bf9319e111b5e21d1f3ff0b0ee23910f48c",
      "packet_manifest_sha256": "d5e3ef0c0d691131b6bd1293d066d6e994c572086dc0c89a6e5ec766a8474199",
      "parent_state_head": "b086ddbe49eb75ca6b7380442efd81186cb669c70360e7ebdc1b98e26369846e",
      "predecessor_step_manifest_sha256": "a96afde3c0a0d38018f88d1224d7a3673492df7e45ac984ebbeea4e910a26dd1",
      "rank": 1385,
      "scan_manifest_sha256": "d0e54809d04b84e2936989fee47fb8386b538d8c264807823b57b10750916028",
      "schema": "d972.r07.full-origin-refinement.v1.step-manifest",
      "sha256": "8c34c9b3ac8d7d97955c7614034592f632c2b609690eb2addb4c115cd1a8f9fc",
      "state_head": "8f6605a28d337cd8541a7eacf6aef78f5a70308a6bb71fd105138803ca623a61",
      "step": 26
    },
    "instruction": {
      "schema": "d972.r07.full-origin-refinement.v1.instruction",
      "step": 26,
      "predecessor": "b086ddbe49eb75ca6b7380442efd81186cb669c70360e7ebdc1b98e26369846e",
      "offer": 8089,
      "generation": 8090,
      "rank": 1385,
      "lead": 1456,
      "sigma": 2,
      "physical_offset": 16740864,
      "packet_manifest_sha256": "d5e3ef0c0d691131b6bd1293d066d6e994c572086dc0c89a6e5ec766a8474199",
      "scan_manifest_sha256": "d0e54809d04b84e2936989fee47fb8386b538d8c264807823b57b10750916028",
      "materialization_sha256": "e98048839da8b75f4ebf434a7f8599cfd148ce656a4acf43f46668ebe566749c",
      "canonical_index_sha256": "452fe97a9229fa5188493256d1478ead1e684b495bbfed0db03a64f5acf4f00e",
      "physical_sha256": "1ed0c6c5c35b5ae13ab528e6151be9d8d1c753c3cf02b6e62842cce76ee2c678",
      "target_scalar": 1,
      "target_remainder_sha256": "111d12e064b96a6bf579f39a9c9d5e35181560c0403bf0d237bffc924230c0ad",
      "rolling_sha256": "8f6605a28d337cd8541a7eacf6aef78f5a70308a6bb71fd105138803ca623a61"
    },
    "result": {
      "schema": "d972.r07.full-origin-refinement.v1.step-result",
      "step": 26,
      "kind": "Separator",
      "owner_sha256": "c4fd8b27590450d0b73e72efe9d45bf9319e111b5e21d1f3ff0b0ee23910f48c",
      "packet_manifest_sha256": "d5e3ef0c0d691131b6bd1293d066d6e994c572086dc0c89a6e5ec766a8474199",
      ...
      "rank_before": 1384,
      "rank_after": 1385,
      "generation_before": 8089,
      "generation_after": 8090,
      "scan_manifest_sha256": "d0e54809d04b84e2936989fee47fb8386b538d8c264807823b57b10750916028",
      "materialization_sha256": "e98048839da8b75f4ebf434a7f8599cfd148ce656a4acf43f46668ebe566749c",
      "target": {
        "parent_remainder_sha256": "5cb563ec85586ff7653ded61edb51dfb8748576a8e42d92323625552b5c96427",
        "remainder_sha256": "111d12e064b96a6bf579f39a9c9d5e35181560c0403bf0d237bffc924230c0ad",
        "scalar": 1
      }
    },
    "payloads": {
...
def selftest_refinement_head() -> Any:
    """Actual small metadata; full parent arithmetic and large step arrays are not replayed."""
    fixture = json.loads(canonical(REFINEMENT_END_METADATA_FIXTURE))
    original = canonical(fixture)
    require(sha(canonical(fixture["head"])) == REFINEMENT_HEAD_PIN[1] and
            len(canonical(fixture["head"])) == REFINEMENT_HEAD_PIN[0], "actual_old_HEAD_fixture_pin")
    validate_refinement_terminal(**fixture)
    rejected: list[str] = []

    def probe(name: str, change: Callable[[Any], None], *, reseal_head: bool = False) -> None:
        value = json.loads(original)
        change(value)
        if reseal_head:
            value["head"]["sha256"] = sha(canonical({key: item for key, item in value["head"].items()
                                                     if key != "sha256"}))
        selftest_rejection(name, lambda: validate_refinement_terminal(**value), rejected)

    for name, key, value in (
            ("HEAD-wrong-schema", "schema", REFINEMENT_SCHEMA + ".result"),
            ("HEAD-count-bool", "completed_steps", True), ("HEAD-count-float", "completed_steps", 26.0),
            ("HEAD-wrong-count", "completed_steps", 25), ("HEAD-wrong-rank", "rank", 1384),
            ("HEAD-generation-float", "generation", 8090.0), ("HEAD-wrong-generation", "generation", 8089),
            ("HEAD-wrong-state", "state_head", "1" * 64), ("HEAD-wrong-last-manifest", "step_manifest_sha256", "2" * 64),
            ("HEAD-wrong-kind", "kind", "Member"), ("HEAD-unexpected-current-scan", "current_scan_manifest_sha256", "3" * 64),
            ("HEAD-forbidden-new-target-field", "target_remainder_sha256", fixture["replay"]["target_remainder_sha256"])):
        probe(name, lambda obj, key=key, value=value: obj["head"].__setitem__(key, value), reseal_head=True)
    for key in REFINEMENT_BINDING_FIELDS:
        probe("HEAD-other-" + key, lambda obj, key=key: obj["head"].__setitem__(key, "4" * 64), reseal_head=True)
    probe("HEAD-bad-inner-seal", lambda obj: obj["head"].__setitem__("sha256", "5" * 64))
    probe("replay-target-hash", lambda obj: obj["replay"].__setitem__("target_remainder_sha256", "6" * 64))
    probe("replay-parent-target-hash", lambda obj: obj["replay"].__setitem__("last_parent_target_sha256", "6" * 64))
    probe("last-instruction-target-hash", lambda obj:
          obj["last"]["instruction"].__setitem__("target_remainder_sha256", "7" * 64))
    probe("last-result-target-hash", lambda obj:
          obj["last"]["result"]["target"].__setitem__("remainder_sha256", "8" * 64))
    probe("last-target-scalar-bool", lambda obj: obj["last"]["result"]["target"].__setitem__("scalar", True))
    probe("last-instruction-scalar-bool", lambda obj: obj["last"]["instruction"].__setitem__("target_scalar", True))
    probe("last-target-payload-hash", lambda obj:
          obj["last"]["payloads"]["target-remainder.bin"].__setitem__("sha256", "9" * 64))
    probe("last-target-payload-size", lambda obj:
          obj["last"]["payloads"]["target-remainder.bin"].__setitem__("bytes", PHYSICAL_BYTES - 1))
    probe("last-instruction-file-hash", lambda obj:
          obj["last"]["payloads"]["instruction.json"].__setitem__("sha256", "a" * 64))
    probe("last-rolling-state", lambda obj: obj["last"]["instruction"].__setitem__("rolling_sha256", "b" * 64))
    probe("last-result-rank-float", lambda obj: obj["last"]["result"].__setitem__("rank_after", 1385.0))
    probe("last-step-count-bool", lambda obj: obj["last"]["instruction"].__setitem__("step", True))
    require(canonical(fixture) == original, "actual_refinement_metadata_fixture_unchanged")
    validate_refinement_terminal(**fixture)
    return {"name": "actual-refinement-head-last-target", "status": "PASS", "rejected_cases": rejected,
        "source_header_file": {"file": "output/HEAD", "bytes": REFINEMENT_HEAD_PIN[0], "sha256": REFINEMENT_HEAD_PIN[1]},
        "source_last_step_files": [fixture["last"]["payloads"][name] for name in sorted(fixture["last"]["payloads"])],
        "source_last_manifest_sha256": fixture["replay"]["last_step_manifest_sha256"],
        "actual_target_remainder_sha256": fixture["replay"]["target_remainder_sha256"],
        "head_fields": 15, "target_field_in_HEAD": False, "same_production_helper": "validate_refinement_terminal",
        "actual_parent_numerically_replayed": False}


def legacy_selftest() -> Any:
    ...
    with tempfile.TemporaryDirectory(prefix="r07-positive-word-selftest-") as temporary:
        ...
        tests = [selftest_word(root / "word"), selftest_history(root / "history"), selftest_raw(root / "raw"),
                 selftest_original_start_header(), selftest_refinement_head()]
    require(len(tests) == 5 and all(test["status"] == "PASS" and test["rejected_cases"] for test in tests),
            "five_new_interface_groups")
    return seal("selftest", {"status": "PASS", "tests": tests,
        ...
        "production_interfaces_used": ["WordDAG", "read_normalized_pair", "TargetHistory.add_row",
            ...
            "validate_original_start_header", "validate_refinement_head", "validate_refinement_terminal"],
        "old_success_suites_rerun": 0, "candidate": False, "cross_checked": False, "verified": False})



@contextmanager
def fixture_resources(root: Path, **options: Any) -> Iterable[ResourceSession]:
    global RESOURCE
    previous = RESOURCE
    session = ResourceSession(root, {"mode": "resource-selftest", "producer": {
        "file": Path(__file__).name, **file_pin(Path(__file__).resolve())}}, fixture=True, **options)
    RESOURCE = session
    completed = False
    try:
        yield session
        ...
        try:
            session.abort_close()
            session.finish("PASS" if completed else "INCOMPLETE")
        finally:
            RESOURCE = previous


def reference_word_module() -> Any:
    name, size, digest = REFERENCE_P3
    path = safe_file(SOURCE_ROOT, name)
    require(file_pin(path) == {"bytes": size, "sha256": digest}, "resource_reference_source_exact_pin")
    module_name = "_r07_positive_P3_resource_anchor"
    require(module_name not in sys.modules, "resource_reference_module_new")
    spec = importlib.util.spec_from_file_location(module_name, path)
    require(spec is not None and spec.loader is not None, "resource_reference_loader")
    ...
    try:
        ...
        require(file_pin(path) == {"bytes": size, "sha256": digest}, "resource_reference_source_unchanged")
        return module
    finally:
...
def resource_expect(name: str, action: Callable[[], Any], rejected: list[str],
                    expected: type[BaseException] = ValueError) -> None:
    try:
        action()
    except expected:
        rejected.append(name)
    else:
        raise AssertionError("resource_selftest_expected:" + name)


def short_word(dag: Any, scope: str) -> int:
    identity, x, y = dag.identity(), dag.letter(1), dag.letter(2)
    rel = dag.rel("r:1", [0])
    empty = dag.product([])
    act = dag.act(x, rel, [0])
    zero = dag.power(act, 0, [0])
    negative, positive = dag.power(rel, -1), dag.power(y, 18)
    inverse = dag.inverse(x)
    alias = dag.ref("p1", "same-node", scope, act, [0])
    target = dag.product([identity, x, y, rel, empty, act, zero, negative, positive, inverse, alias, x, x], [0])
    return dag.ref("target", "resource-fixture", scope, target, [0])


def selftest_private_store(root: Path) -> dict[str, Any]:
    rejected: list[str] = []
    with fixture_resources(root, page_rows=2, cache_pages=2) as session:
        store = session.new_store("fixture")
        expected = []
        for node in range(13):
            pair = (node % 54, (-node) % 54)
            digest = sha(canonical({"anchor": node}))
            require(store.append(digest, pair) == node, "fixture_disk_original_ID")
            expected.append((digest, pair))
            if node:
                store.add_use(node - 1)
        store.add_use(0)
        require(store.read(0)[2] == 2, "fixture_repeated_edge_count")
        for node in reversed(range(len(expected))):
            require(store.read(node)[:2] == expected[node], "fixture_evicted_page_exact_values")
        require(session.pool.evictions > 0 and session.pool.max_resident_pages == 2 and
                session.pool.statistics()["configured_cache_and_io_bytes"] <= CACHE_LIMIT,
                "fixture_real_page_eviction")
        require(store.hashes[-1] == expected[-1][0] and store.pairs[-13] == expected[0][1],
                "fixture_list_position_compatibility")
        for name, action in (
                ("future-node-ID", lambda: store.read(store.count)),
                ("negative-node-ID", lambda: store.read(-1)),
                ("bool-node-ID", lambda: store.read(True)),
                ("short-hash", lambda: store.append("ab", (0, 0))),
                ("residue54-overflow", lambda: store.append("a" * 64, (54, 0))),
                ("residue54-bool", lambda: store.append("a" * 64, (False, 0)))):
            resource_expect(name, action, rejected)
        resource_expect("view-negative-outside-table", lambda: store.pairs[-14], rejected, IndexError)
        resource_expect("view-bool-is-not-position", lambda: store.pairs[True], rejected, TypeError)
        def count_capacity() -> None:
            old = store.count
            try:
                store.count = UINT64_MAX
                store.append("a" * 64, (0, 0))
            finally:
                store.count = old
        resource_expect("ordinary-count-capacity-is-resource", count_capacity, rejected, ResourceStop)
        store.flush()
        original = store.path.read_bytes()
        def mutate_file(raw: bytes) -> None:
            with store.path.open("wb") as stream:
                stream.write(raw)
            store.check_eof()
        resource_expect("partial-index-row", lambda: mutate_file(original[:-1]), rejected)
        store.path.write_bytes(original)
        altered = bytearray(original)
        altered[12] ^= 1
        resource_expect("wrong-private-stride", lambda: mutate_file(bytes(altered)), rejected)
        store.path.write_bytes(original)
        altered = bytearray(original)
        altered[16] ^= 1
        resource_expect("different-source-scope-binding", lambda: mutate_file(bytes(altered)), rejected)
        store.path.write_bytes(original)
        def bad_count() -> None:
            old = store.count
            try:
                ...
                store.count = old
        resource_expect("count-without-row", bad_count, rejected)
        receipt = store.finish(12)
        store.close()
        require(receipt["count"] == 13 and receipt["all_nodes_reachable"] is True, "fixture_full_index_receipt")
        return {"name": "disk-index-cache-and-integrity", "status": "PASS", "rejected_cases": rejected,
                "rows": 13, "cache": session.pool.statistics()}


def selftest_resource_word(root: Path, reference: Any, scratch: Path) -> dict[str, Any]:
    root.mkdir()
    rejected: list[str] = []
    dictionary = {"relators": {"r:1": [1, 2]}}
    scope = sha(canonical(dictionary))
    old_dir, new_dir = root / "old-word", root / "new-word"
    old_dir.mkdir()
    new_dir.mkdir()
    with (old_dir / "ordered-word.jsonl").open("xb") as stream:
        old_dag = reference.WordDAG(stream, scope, {"r:1": (1, 2)}, lambda: 1)
        old_root = short_word(old_dag, scope)
        old_pin = old_dag.flush()
    with fixture_resources(scratch, page_rows=2, cache_pages=2) as session:
        with (new_dir / "ordered-word.jsonl").open("xb") as stream:
            new_store = session.new_store("build")
            new_dag = WordDAG(stream, scope, {"r:1": (1, 2)}, lambda: 1, new_store)
            new_root = short_word(new_dag, scope)
            new_pin = new_dag.flush()
            require(new_root == old_root and new_pin == old_pin and
                    list(new_store.pairs[node] for node in range(new_store.count)) == old_dag.pairs and
                    list(new_store.hashes[node] for node in range(new_store.count)) == old_dag.hashes,
                    "reference_old_WordDAG_all_bytes_hashes_pairs")
            new_store.finish(new_root)
        new_store.close()
        original = (new_dir / "ordered-word.jsonl").read_bytes()
        require(original == (old_dir / "ordered-word.jsonl").read_bytes(), "reference_actual_JSONL_bytes")
        word = seal("word-manifest", {"grammar": GRAMMAR, "nodes_file": new_pin, "root_id": new_root,
            "root_sha256": old_dag.hashes[old_root], "literal_dictionary_sha256": scope})
        normalized = read_normalized_pair(new_dir, word, dictionary, 1)
        expected = reference.read_normalized_pair(old_dir, word, dictionary, 1)
        require(normalized == expected and session.stores[1].purpose == "reread" and
                session.stores[1].finished and session.stores[0].closed, "new_empty_reread_not_builder_answers")
        nodes = [json.loads(raw) for raw in original.splitlines()]
        require({node["op"] for node in nodes} == OPS, "reference_all_eight_ops")
        def mutate(index: int, action: Callable[[Any], None]) -> None:
            raw_nodes = [json.loads(raw) for raw in original.splitlines()]
            action(raw_nodes[index])
            raw_nodes[index]["node_sha256"] = sha(canonical({key: value for key, value in raw_nodes[index].items()
                                                            if key != "node_sha256"}))
            (new_dir / "ordered-word.jsonl").write_bytes(b"".join(canonical(node) for node in raw_nodes))
            read_normalized_pair(new_dir, word, dictionary, 1)
        product = next(node["id"] for node in nodes if node["op"] == "OrderedProduct" and
                       len(node["args"]["factors"]) > 3)
        power = next(node["id"] for node in nodes if node["op"] == "IntegerPower")
        resource_expect("zero-edge-removed-fixed-word-root", lambda: mutate(product,
            lambda node: node["args"]["factors"].pop(6)), rejected)
        resource_expect("repeated-edge-removed-fixed-word-root", lambda: mutate(product,
            lambda node: node["args"]["factors"].pop()), rejected)
        resource_expect("future-child-with-new-seal", lambda: mutate(product,
            lambda node: node["args"]["factors"][0].update(node=product)), rejected)
        resource_expect("power-bool-with-new-seal", lambda: mutate(power,
            lambda node: node["args"].update(exponent=False)), rejected)
        resource_expect("short-child-hash", lambda: mutate(product,
            lambda node: node["args"]["factors"][0].update(sha256="ab")), rejected)
        (new_dir / "ordered-word.jsonl").write_bytes(original)
        orphan = root / "orphan"
        orphan.mkdir()
        with (orphan / "ordered-word.jsonl").open("xb") as stream:
            orphan_dag = WordDAG(stream, scope, {"r:1": (1, 2)}, lambda: 1)
            orphan_dag.letter(1)
            last = orphan_dag.identity()
            orphan_pin = orphan_dag.flush()
            orphan_hash = orphan_dag.hashes[last]
        orphan_dag.store.close()
        orphan_word = seal("word-manifest", {"grammar": GRAMMAR, "nodes_file": orphan_pin, "root_id": last,
            "root_sha256": orphan_hash, "literal_dictionary_sha256": scope})
        resource_expect("unreachable-node-even-with-complete-seals", lambda:
            read_normalized_pair(orphan, orphan_word, dictionary, 1), rejected)
        (new_dir / "ordered-word.jsonl").write_bytes(original[:-1])
        resource_expect("incomplete-word-final-LF", lambda:
            read_normalized_pair(new_dir, word, dictionary, 1), rejected)
        (new_dir / "ordered-word.jsonl").write_bytes(original)
        require(read_normalized_pair(new_dir, word, dictionary, 1) == expected, "original_word_restored_full_EOF")
        return {"name": "old-word-bytes-and-new-reread", "status": "PASS", "rejected_cases": rejected,
            "reference_source": {"file": REFERENCE_P3[0], "bytes": REFERENCE_P3[1], "sha256": REFERENCE_P3[2]},
            "word": new_pin, "root_id": new_root, "root_sha256": old_dag.hashes[old_root],
            "normalized_pair": normalized["normalized_pair"], "ops": sorted({node["op"] for node in nodes}),
            "old_full_suites_run": 0}


def selftest_resource_paths(root: Path, scratch: Path) -> dict[str, Any]:
    root.mkdir()
    rejected: list[str] = []
    parent, protected = root / "parent", root / "protected.txt"
    parent.mkdir()
    protected.write_text("fixture", encoding="ascii")
    output = root / "output"
    require(resource_path_gate(scratch, output, [parent], [protected]) == (scratch, output),
            "fixture_valid_separate_paths")
    for name, bad_scratch, bad_output in (
            ("scratch-is-output", output, output),
            ("scratch-contains-output", root / "pair", root / "pair" / "word"),
            ("scratch-below-parent", parent / "scratch", output),
            ("output-below-parent", scratch, parent / "word"),
            ("scratch-contains-protected-source", root, output)):
        resource_expect(name, lambda a=bad_scratch, b=bad_output:
                        resource_path_gate(a, b, [parent], [protected]), rejected)
    link = root / "parent-link"
    link.symlink_to(parent, target_is_directory=True)
    try:
        resource_expect("scratch-symlink-parent", lambda:
            resource_path_gate(link / "new", output, [parent], [protected]), rejected)
    finally:
        link.unlink()
    with fixture_resources(scratch, page_rows=2, cache_pages=2) as session:
        resource_expect("existing-scratch-reuse", lambda:
            resource_path_gate(scratch, output, [parent], [protected]), rejected)
        old_limit = session.line_limit
        session.line_limit = 8
        try:
            resource_expect("oversized-line-is-resource", lambda:
                list(limited_lines(io.BytesIO(b"12345678\n"))), rejected, ResourceStop)
            require(list(limited_lines(io.BytesIO(b"1234567\n"))) == [b"1234567\n"], "line_limit_includes_LF")
        finally:
            session.line_limit = old_limit
        store = session.new_store("fixture")
        store.append("a" * 64, (0, 53))
        old_disk_limit = session.scratch_limit
        session.scratch_limit = sum(size for name, size in session.extents.items() if scratch in name.parents)
        try:
            resource_expect("scratch-byte-limit-is-resource", lambda:
                store.append("b" * 64, (1, 52)), rejected, ResourceStop)
            require(store.count == 1, "capacity-stop-before-index-append")
        finally:
            session.scratch_limit = old_disk_limit
        old_floor = session.free_floor
        session.free_floor = UINT64_MAX
        try:
            resource_expect("disk-free-floor-is-resource", lambda:
                session.before_write(scratch / "never.bin", 1), rejected, ResourceStop)
        finally:
            session.free_floor = old_floor
        require(not (scratch / "never.bin").exists(), "resource-stop-created-no-file")
        store.finish(0)
        store.close()
        getter, setter = getattr(sys, "get_int_max_str_digits", None), getattr(sys, "set_int_max_str_digits", None)
        threshold = getattr(sys.int_info, "str_digits_check_threshold", None)
        require(sys.implementation.name == "cpython" and callable(getter) and callable(setter) and
                plain_int(threshold, 1), "fixture_registered_CPython_decimal_capacity")
        original_digits = getter()
        capacity_path = root / "decimal-capacity-word.jsonl"
        with capacity_path.open("xb") as stream:
            dag = WordDAG(stream, "a" * 64, {}, lambda: 0)
            letter = dag.letter(1)
            before = dag.store.count, dag.length, stream.tell()
            try:
                setter(threshold)
                require(canonical({"exponent": -18}) == b'{"exponent":-18}\n' and
                        canonical({"exponent": 0}) == b'{"exponent":0}\n' and
                        canonical({"exponent": 18}) == b'{"exponent":18}\n',
                        "ordinary_integer_canonical_bytes_unchanged")
                for label, exponent in (("positive", 10 ** threshold), ("negative", -(10 ** threshold))):
                    resource_expect(label + "-ordinary-exponent-decimal-capacity-is-resource",
                        lambda exponent=exponent: dag.power(letter, exponent), rejected, ResourceStop)
                    require((dag.store.count, dag.length, stream.tell()) == before,
                            "decimal_capacity_stop_before_word_or_index_append")
                resource_expect("bool-exponent-remains-type-failure", lambda:
                    dag.power(letter, False), rejected)
                circular: list[Any] = []
                circular.append(circular)
                resource_expect("unrelated-circular-ValueError-remains-failure", lambda:
                    canonical(circular), rejected)
                require(not integer_decimal_capacity(ValueError("unrelated-json-error")),
                        "no_general_ValueError_resource_fallback")
            finally:
                setter(original_digits)
            require(getter() == original_digits, "original_decimal_capacity_restored")
            negative, zero, positive = dag.power(letter, -18), dag.power(letter, 0), dag.power(letter, 18)
            root_id = dag.product([letter, negative, zero, positive])
            dag.flush()
            dag.store.finish(root_id)
        dag.store.close()
        return {"name": "scratch-line-and-resource-boundaries", "status": "PASS", "rejected_cases": rejected,
                "fixture_resource_limits_only": True}


def resource_selftest(args: argparse.Namespace) -> dict[str, Any]:
    if args.scratch:
        scratch = path_without_symlink(Path(args.scratch))
        temp_roots = [Path(tempfile.gettempdir()).resolve()]
        if os.environ.get("RUNNER_TEMP"):
            temp_roots.append(path_without_symlink(Path(os.environ["RUNNER_TEMP"])))
        require(any(root in scratch.parents for root in temp_roots), "resource_selftest_in_temporary_root")
        base = scratch.parent
    else:
        base = Path(tempfile.mkdtemp(prefix="r07-positive-resource-v4-"))
        scratch = base / "scratch"
    fixtures = base / "fixtures"
    protected = [Path(__file__).resolve(), SOURCE_ROOT / REFERENCE_P3[0],
                 *(SOURCE_ROOT / name for name in DATA_PINS)]
    resource_path_gate(scratch, fixtures, [], protected)
    paths_path = base / "resource-selftest-paths.json"
    require(not paths_path.exists(), "resource_selftest_paths_receipt_fresh")
    base.mkdir(parents=True, exist_ok=True)
    scratch.mkdir()
    fixtures.mkdir()
    paths = resource_seal("selftest-paths", {"scratch": str(scratch), "fixtures": str(fixtures),
        "temporary_roots_only": True, "delete_on_exit": False, "explicit_scratch": bool(args.scratch),
        "producer": {"file": Path(__file__).name, **file_pin(Path(__file__).resolve())}})
    paths_path.write_bytes(canonical(paths))
    reference = reference_word_module()
    tests = [selftest_private_store(scratch / "store"),
             selftest_resource_word(fixtures / "word", reference, scratch / "word"),
             selftest_resource_paths(fixtures / "paths", scratch / "paths")]
    require([test["name"] for test in tests] == ["disk-index-cache-and-integrity", "old-word-bytes-and-new-reread",
            "scratch-line-and-resource-boundaries"] and all(test["status"] == "PASS" and test["rejected_cases"]
            for test in tests), "three_new_resource_groups")
    return seal("resource-selftest", {"status": "PASS", "tests": tests,
        "fixture_scope": "small-private-storage-and-frozen-P3-word-helper-only;not-an-accepted-numerical-run",
        "production_interfaces_used": ["NodeStore", "PagePool", "WordDAG", "read_normalized_pair",
            "limited_lines", "resource_path_gate", "ResourceSession.before_write", "canonical"],
        "paths": paths, "paths_receipt": {"file": str(paths_path), **file_pin(paths_path)},
        "reference_source": {"file": REFERENCE_P3[0], "bytes": REFERENCE_P3[1], "sha256": REFERENCE_P3[2]},
        "old_full_suites_run": 0, "candidate": False, "cross_checked": False, "verified": False})


def selftest() -> Any:
    with tempfile.TemporaryDirectory(prefix="r07-legacy-resource-wrapper-") as temporary:
        with fixture_resources(Path(temporary) / "scratch"):
            return legacy_selftest()


def accepted_resource_session(args: argparse.Namespace, parents: Parents) -> ResourceSession:
    own_path = Path(__file__).resolve()
    source = {"file": own_path.relative_to(SOURCE_ROOT).as_posix(), **file_pin(own_path)}
    return ResourceSession(Path(args.scratch).resolve(), {
        "producer": source, "acceptance": {"file": str(Path(args.acceptance).resolve()),
            "sha256": parents.acceptance_sha256, "bytes": len(canonical(parents.acceptance))},
        "parents": parents.acceptance["parents"], "consumer_sources": parents.acceptance["consumer_sources"],
        "raw_sources": [{"file": name, "bytes": value[0], "sha256": value[1]} for name, value in DATA_PINS.items()],
        "runtime": parents.acceptance["runtime"], "accepted_owner": parents.acceptance["selected"]["owner"],
        "accepted_head": parents.acceptance["selected"]["head"],
        "scratch_path": str(Path(args.scratch).resolve()), "output_path": str(Path(args.output).resolve()),
        "ordered_word_identity_unchanged_by_private_storage": True})


def finish_resource_safely(status: str, result: Any = None) -> None:
    if RESOURCE is None:
        return
    try:
        RESOURCE.abort_close()
        RESOURCE.finish(status, result)
    except (OSError, ResourceStop, MemoryError, ValueError) as error:
        # Preserve the original failure and prior periodic samples; this is not PASS.
        print("RESOURCE_RECEIPT_INCOMPLETE:" + type(error).__name__ + ":" + str(error), file=sys.stderr)
...
def diagnostic(args: argparse.Namespace, status: str, error: BaseException) -> Any:
    ...
    if OUTPUT_CREATED is not None:
        ...
        try:
            raw = canonical(value)
            resource_before_write(OUTPUT_CREATED / name, len(raw))
            with (OUTPUT_CREATED / name).open("xb") as stream:
                require(stream.write(raw) == len(raw), "diagnostic_full_write")
                stream.flush()
                os.fsync(stream.fileno())
        except (ResourceStop, MemoryError):
            print("DIAGNOSTIC_FILE_INCOMPLETE:resource-limit;stdout-and-prior-samples-remain", file=sys.stderr)
        except OSError as secondary:
            if secondary.errno not in (errno.ENOSPC, errno.EFBIG):
                raise
            print("DIAGNOSTIC_FILE_INCOMPLETE:storage-capacity;stdout-and-prior-samples-remain", file=sys.stderr)
    return value
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resource-selftest", action="store_true")
    ...
    if args.selftest or args.resource_selftest:
        if args.selftest and args.resource_selftest:
            parser.error("choose only one selftest mode")
        if args.selftest and args.scratch:
            parser.error("legacy selftest owns temporary roots; use --resource-selftest for explicit scratch")
        if args.max_seconds is None:
            args.max_seconds = 300
        if args.max_memory_mib is None:
            args.max_memory_mib = 7168 if args.resource_selftest else 1024
        ...
        missing += ["--" + name.replace("_", "-") for name in ("acceptance", "output", "scratch", "max_seconds", "max_memory_mib")
...
def main() -> int:
    ...
    try:
        ...
        value = resource_selftest(args) if args.resource_selftest else (selftest() if args.selftest else run_actual(args))
        if RESOURCE is not None:
            RESOURCE.finish("PASS", value)
        status = 0
    except (ResourceStop, MemoryError, KeyboardInterrupt) as error:
        traceback.print_exception(type(error), error, error.__traceback__, file=sys.stderr)
        finish_resource_safely("UNKNOWN_RESOURCE")
        value, status = diagnostic(args, "UNKNOWN_RESOURCE", error), 3
    except OSError as error:
        traceback.print_exception(type(error), error, error.__traceback__, file=sys.stderr)
        resource_failure = error.errno in (errno.ENOSPC, errno.EFBIG)
        status_name = "UNKNOWN_RESOURCE" if resource_failure else "FAIL"
        finish_resource_safely(status_name)
        value, status = diagnostic(args, status_name, error), 3 if resource_failure else 1
    except Exception as error:
        traceback.print_exception(type(error), error, error.__traceback__, file=sys.stderr)
        finish_resource_safely("FAIL")
