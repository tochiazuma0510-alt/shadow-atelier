#!/usr/bin/env python3
"""v12b SELFTEST_BOOTSTRAP producer with no production/resume surface.
...
PRODUCER_PREFIX = "V12B_PRODUCER_TERMINAL"
...
SOURCE_PINS: dict[str, tuple[str, int, str]] = {
    ...
    "fixture": ("search/certs/d972_r07_history_free_positive_fast_resume_selftest_v12b_20260829.json", 23679,
...
RESOURCE_MODEL = {
    ...
    "conservative_live_peak_formula": "2604166234 payload/output bytes + 2295833766 CPython/COW/mutation reserve = 4900000000",
    "conservative_live_peak_bytes": 4_900_000_000,
    "rss_hard_cap_bytes": 5_700_000_000,
    "rss_margin_bytes": 800_000_000,
        ...
        for letter in range(2):
            ...
            for coordinate, width in enumerate(widths):
                ...
                if pc_raw is None:
                    ...
                    meter.bump("pc_cache_insertions", 1, "Q0_pc_cache")
...
def build_heavy(runtime: dict[str, Any], registry: SourceRegistry,
    ...
    projected_packed = [b"".join(p176.packed_joint_blob(
        value, "v12b projected Gamma state") for value in row)
...
def atomic_json(path: Path, value: dict[str, Any], *, allow_replace: bool = False,
    ...
    reserved = estimated_json_size(value) + 1
    if reserved > maximum:
        raise ResourceStop("checkpoint_serialization", "checkpoint_bytes",
                           reserved, maximum)
    if meter is not None:
        meter.reserve("checkpoint_bytes", reserved, "receipt_publication_reserve")
...
def triangular_mutation_selftest(old_value: dict[str, Any], live: Any,
    ...
    results = []
    before_digest = sha_bytes(canonical(baseline) + b"\n")
    original_prefix = json.loads(canonical(baseline["columns"][:3]).decode("ascii"))
    with tempfile.TemporaryDirectory(prefix="r07-v7-triangular-") as directory:
        root = Path(directory)
        for ordinal, name in enumerate(names):
            for index in range(3):
                baseline["columns"][index] = json.loads(
                    canonical(original_prefix[index]).decode("ascii"))
            ...
            before, after, reason = _physical_mutation_trace(
                root / f"{ordinal:02d}.json", value, validate_mutant,
                baseline_sha256=before_digest, trace_runtime=trace_runtime)
            ...
            results.append({"id": name, "owner_path": expected["owner_path"],
                ...
                "first_reason": reason})
    for index in range(3):
        baseline["columns"][index] = original_prefix[index]
    return results
...
def producer_bind_section_identity(runtime: dict[str, Any],
    ...
    selected_coordinate_mark = [p176.packed_joint_blob(
        value, "v12b selected coordinate mark") for value in coordinate_mark]
    ...
    for gamma_id, projected_row in enumerate(runtime["projected"], 1):
        packed = b"".join(p176.packed_joint_blob(
            value, "v12b Gamma projected coordinate") for value in projected_row)
...
def producer_selected_correction_mutations(runtime: dict[str, Any],
    ...
    with tempfile.TemporaryDirectory(prefix="r07-v12b-producer-selected-") as directory:
        root = Path(directory)
...
def process_selftest(runtime: dict[str, Any], reducer: FormalReducer,
    ...
    # Each fault semantic is exercised once, sequentially, with the smallest
    # preregistered W2 owner.  Repeating the same fault under W4 adds no
    # load-bearing property and formerly hid work behind fresh meters.
    fault_workers = 2
    ...
    blocked = blocked_send_selftest(context)
...
def producer_boundary_mutations(runtime: dict[str, Any], processes: dict[str, Any],
    ...
    before = sha_bytes(canonical(baseline) + b"\n"); ledger = []
    with tempfile.TemporaryDirectory(prefix="r07-v12b-producer-boundary-") as directory:
        root = Path(directory)
...
def producer_positive_mutations(runtime: dict[str, Any], selected_frame: dict[str, Any],
    ...
    before = sha_bytes(canonical(baseline) + b"\n"); ledger = []
    with tempfile.TemporaryDirectory(prefix="r07-v12b-producer-positive-") as directory:
        root = Path(directory)
...
def producer_physical_mutations_v297(runtime: dict[str, Any],
    ...
    ledger = []
    with tempfile.TemporaryDirectory(prefix="r07-v12b-producer-physical-v297-") as directory:
        root = Path(directory)
...
def phase_gate_selftest(runtime: dict[str, Any], fixture: dict[str, Any],
    ...
    baseline_revalidated = True
    rows = []
    with tempfile.TemporaryDirectory(prefix="r07-v12b-producer-phase-") as directory:
        root = Path(directory)
...
def run_real_owner_selftest(runtime: dict[str, Any], old_value: dict[str, Any],
    ...
    observed_groups = {"triangular": triangular, "boundary": boundary_ledger,
        "selected_correction": selected_ledger, "positive": positive_ledger,
        "physical": physical_ledger, "phase": phases}
    normalized = {group: [{key: row[key] for key in (
            "id", "owner_path", "validator", "stage", "first_reason", "reseal")}
        for row in rows] for group, rows in observed_groups.items()}
    ...
    require(normalized == expected_normalized,
            "producer normalized ordinary mutation ledger")
    ...
    return {"fixture_sha256": SOURCE_PINS["fixture"][2],
            ...
            "normalized_mutation_ledger": normalized,
...
def validate_p0_manifest(value: dict[str, Any]) -> None:
    ...
    require(value.get("schema") == P0_SCHEMA, "v12b P0 schema")
    require(value.get("mode") == "SELFTEST_BOOTSTRAP" and
            ...
            "v12b P0 bootstrap envelope")
    require(value.get("manifest_path") ==
            "ci/in/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12b.manifest.v1.json",
            "v12b P0 path")
    require(value.get("platform_contract", {}).get("gha_runner") ==
            ...
            value.get("resource_contract", {}).get(
                "q0_ten_store_payload") ==
                "1469664*(5*40+5*154)=1425574080",
            "v12b P0 platform/resource/deadline contract")
    prospective = value.get("prospective_outputs")
    require(type(prospective) is dict and
            prospective.get("R", {}).get("sha256") ==
            "TO_BE_GENERATED_BY_AUDITED_V12B_SELFTEST" and
            prospective.get("V", {}).get("sha256") ==
            "TO_BE_GENERATED_BY_AUDITED_V12B_SELFTEST",
            "v12b prospective identity gate")
    sources = value.get("sources")
    require(type(sources) is dict and set(sources) == {
            "producer_v12b", "checker_v12b", "fixture_v12b"} and
            all(type(sources[key]) is dict for key in sources),
            "v12b P0 source graph without driver cycle")
    expected_paths = {
    ...
    require(type(frozen) is dict and set(frozen) == set(expected_frozen),
            "v12b P0 frozen authority roster")
    for key, (path, size, digest) in expected_frozen.items():
        ...
        require(type(row) is dict and row.get("path") == path and
                ...
                "v12b P0 frozen authority row:" + key)
...
def validate_owned_paths(output: Path, source: Path) -> None:
    ...
    require(output.absolute() == expected_output,
            "v12b receipt path is not preregistered")
    require(source.absolute() == expected_source,
            "v12b raw source path is not preregistered")
    output_dir = ROOT / "ci/out"
    require(output_dir.is_dir() and not output_dir.is_symlink(),
            "v12b output directory identity")
    try:
        source_stat = os.lstat(source)
    except OSError as exc:
        raise InputStop("v12b raw source path unavailable") from exc
    require(stat.S_ISREG(source_stat.st_mode) and source_stat.st_nlink == 1,
            "v12b raw source must be a unique regular owner")
    if os.path.lexists(str(output)):
        ...
        require(stat.S_ISREG(output_stat.st_mode) and
                ...
                "v12b receipt output identity")
    for relative in OUTPUT_SIBLINGS:
        target = ROOT / relative
        require(not os.path.lexists(str(target)), "stale v12b transport:" + relative)


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description="v12b SELFTEST_BOOTSTRAP producer")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    require(args.seconds == WALL_SECONDS,
            "v12b producer envelope must be 9600 seconds")
    ...
    try:
        ...
        raise InputStop("v12b_triangular_gate") from exc
    meter.check("v12b_light_basis_complete")
    ...
    processes = process_selftest(runtime, reducer, registry.json("fixture"), meter)
    ...
    exclusive_json(output, seal(receipt_body), meter)
...
if __name__ == "__main__":
    try:
        ...
        temporary = candidate.with_name(candidate.name + f".tmp.{os.getpid()}")
        for owned in (temporary, candidate):
            try:
                owned.unlink()
            except FileNotFoundError:
                pass
