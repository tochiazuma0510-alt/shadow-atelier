            ...
            class PackedJointGroup(joint.JointGroup):
                def blob(self, value):
                    return p176.packed_joint_blob(
                        value, "v4 task198 evaluator Gamma element")
...
def main(argv=None):
    ...
    parser.add_argument("--output",
                        default="ci/out/d972_r07_pre_a0_single_target_a3_v4.json")
    args = parser.parse_args(argv)
    meter = Meter()
    try:
        install_linux_hard_limits(meter)
        with WallDeadline(meter, "complete v4 producer route"):
    ...
    try:
        ...
        with WallDeadline(meter, "complete v4 producer emergency publication"):
            target = prepare_output(args.output)
