#!/usr/bin/env python3
"""Exact Task496 v5-carrier provenance successor of Task193 compiler-v5."""
...
PATCHES = (
    (
        ...
        b'"""Task496 v5-rank99 carrier-fed exact Task193 mathematical compiler."""',
        1,
    ),
    ...
    (b'R07_SECOND_FRATTINI_AFFINE_PREFIX_COMPILER_V5',
     b'R07_SECOND_FRATTINI_AFFINE_PREFIX_COMPILER_V6', 1),
    (
    ...
    (b'd972-r07-task451-task193-carrier/v1',
     b'd972-r07-rank99-v5-task193-carrier/v1', 4),
    (b'R07_TASK451_TASK193_CARRIER_V1',
     b'R07_RANK99_V5_TASK193_CARRIER_V1', 4),
    (
        b'("search/d972_r07_a0_dual_anchored_active_batch_v1.py",13834,"ca7fb15e06dd04881146c38d63d93015a9e630fbc334cf15098cbd8a32f22f9b")',
        b'("search/d972_r07_a0_dual_anchored_rank99_durable_discovery_v5.py",104031,"25c308ec11b9f36cc9779dfec46058a4956068969d664ee582a26f9cb0db7c09")',
        2,
    ),
    (
        b'("crosscheck/check_d972_r07_a0_dual_anchored_active_batch_v1.py",13725,"5c2f76b825bd920245d0200f29ff860ba93a32663ef5db9567bc499a86f7ff8a")',
        b'("crosscheck/check_d972_r07_a0_dual_anchored_rank99_durable_discovery_v5.py",71589,"970ffe3a78687f3a27a222e089ae3d5e928bbfa048b9aef9f51fcf4c0b5d578d")',
        2,
    ),
    (
        ...
        b'("search/d972_r07_a0_dual_anchored_rank99_durable_discovery_gha_driver_v5.g",9425,"bed9105b36fef5e59120d954029ec507b16f393ab2859a7599867a19156b1b5d")',
        2,
    ),
    (b'3316809e483223ec571ca7d6976dc1317c892441',
     b'dd6d90b64e2bfba73d7f131f4da876235746f314', 2),
    (b'task451_result', b'v5_result', 2),
    (b'task451_checkpoint', b'v5_checkpoint', 2),
    (b'task451_checker_log', b'v5_checker_log', 2),
    (b'"task454_v4_core"', b'"task496_v5_core"', 1),
    (b'd972-r07-second-frattini-affine-prefix-compiler-checkpoint/v5',
     b'd972-r07-second-frattini-affine-prefix-compiler-checkpoint/v6', 1),
    (b'v5 starts from rank zero; foreign checkpoint rejected',
     b'v6 starts from rank zero; foreign checkpoint rejected', 1),
    (b'"corrected_word_source":"Task452 carrier"',
     b'"corrected_word_source":"Task496 v5 rank99 carrier"', 1),
    (b'actual_task451_positive', b'actual_common', 1),
)
GENERATED_BYTES = 12282
GENERATED_SHA256 = "2d26dd83c930165f740ec53b621b03d1b57b1232a7b2cd1905730033f4e20341"
...
def _generate() -> tuple[bytes, list[dict[str, object]]]:
    ...
    for old, new, expected in PATCHES:
        ...
        if old_before != expected or new_before != 0:
            raise RuntimeError("producer source-patch cardinality drift:" + old.decode("ascii"))
        raw = raw.replace(old, new)
        old_after = raw.count(old)
        new_after = raw.count(new)
        if old_after != 0 or new_after != expected:
            raise RuntimeError("producer source-patch postcondition drift:" + new.decode("ascii"))
