#!/usr/bin/env python3
"""Independent checker for the fixed-candidate relative-Frattini v5 lane.
...
def evaluate_proof_dag(block: dict[str, Any], expected_names: Sequence[str],
    ...
    for index in range(n):
        ...
        if (node_id & 4095) == 0 or now-last_progress >= \
            ...
            print("D972_B345_RELFRAT3_FIXED_CANDIDATE_V5_CHECKER_PROGRESS "
...
def validate_fixed_receipt(data: dict[str, Any], q3: dict[str, Any],
    ...
    require(data["source_hashes"] == {
                "producer_sha256": digest_file(
                    repo/"search/d972_b345_relfrat3_fixed_candidate_v5.py"),
                "checker_sha256": digest_file(Path(__file__)),
                "driver_sha256": digest_file(
                    repo/"search/d972_b345_relfrat3_fixed_candidate_gha_driver_v5.g")},
            "fixed source hashes")
    require(data["representation_contract"]["version"] ==
            "fixed-candidate-transactional-packed-v5" and
            data["representation_contract"]["correction_dictionary_constructed"] is False and
            data["representation_contract"]["fixed_context_cheap_DP_executed"] is False and
    ...
    if is_unknown:
        ...
        require(data["resource_accounting_at_stop"]["bounded_search_prefix"] ==
...
def self_test_fixed() -> None:
    """One bounded test of only the fixed-candidate v5 production contract."""
    class TrivialQ:
        identity = 0
...
def main() -> int:
    ...
    print(f"B345_RELFRAT3_FIXED_CANDIDATE_CHECKER_PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(f"B345_RELFRAT3_FIXED_CANDIDATE_CHECKER_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
