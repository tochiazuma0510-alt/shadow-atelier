...
def validate_launch(path: Path) -> dict[str, Any]:
    ...
    if launch["fixture_only"]:
        ...
        # The live v1 path has no independently authenticated resume artifact
...
def _read_target_parent(root: Path, live_parent: bool = False) -> tuple[np.ndarray, dict[str, Any]]:
    manifest, _ = _read_json(root / "manifest.json", "rho2_manifest")
    required = {"schema", "marker", "files", "lower_zero_coordinates",
                "rho2_sha256", "rho2_dense_sha256", "lower_dense_sha256"}
    require(set(required).issubset(manifest) and
            ...
            manifest["lower_zero_coordinates"] == 32260, "rho2_manifest_shape")
    files = {str(x["file"]): x for x in manifest["files"]}
    fixture = False
    ...
    require(len(packed) == PHYSICAL_BYTES and len(dense) == PHYSICAL_WIDTH and
            len(lower) == 32260 and sha(packed) == manifest["rho2_sha256"] and
            sha(dense) == manifest["rho2_dense_sha256"] and
            sha(lower) == manifest["lower_dense_sha256"], "rho2_receipts")
...
def _fixture_target(root: Path, target: np.ndarray) -> Path:
    ...
    manifest = {"schema": TARGET_PAYLOAD_SCHEMA,
                ...
                "files": [{"file": "rho2.bin", "bytes": len(packed), "sha256": sha(packed)},
                          {"file": "rho2-dense.bin", "bytes": len(dense), "sha256": sha(dense)},
                          {"file": "lower-dense.bin", "bytes": len(lower), "sha256": sha(lower)}],
                "lower_zero_coordinates": 32260, "rho2_sha256": sha(packed),
                "rho2_dense_sha256": sha(dense), "lower_dense_sha256": sha(lower)}
    atomic_json(root / "manifest.json", manifest)
...
def selftest() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d972-r07-state-v1-") as td:
        ...
        require((state / "manifest.json").read_bytes() == (resumed / "manifest.json").read_bytes() and
                ...
                "resume_state_bytes")
        return {"schema": "d972.r07.physical-state-separator.v1.selftest",
