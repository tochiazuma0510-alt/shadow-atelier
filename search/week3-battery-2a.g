...
# ---- U-F7: BLOCKED -- restricted presentation "D_3^(2)" has no construction formula anywhere
# in the spec projection given to me (verified by grep of manifest_spec_v1.md; even GAP-E5 flags
# this as unresolved for the commander's own copy). Reporting BLOCKED, not fabricating, not
# silently skipping. Per sec.1.2 this would normally halt the stage; proceeding with everything
# else that IS independently computable from the verbal side, per continuation instruction, and
# flagging this prominently for commander review.
Print("[BLOCKED] U-F7: restricted presentation D_3^(2) has no construction formula in the spec\n");
Print("  projection available to me -- cannot independently verify 'both presentations agree'.\n");
Print("  Proceeding with all other fixtures/enumeration (verbal side only); flagged for review.\n");

if not fixtureOK then
  Print("\n[UNKNOWN] stage 2a: fixture mismatch (excluding U-F7 which is BLOCKED, not FAILED) -- halting.\n");
  haltStage := true;
fi;
...
s := Concatenation(
  ...
  "\"uf7_status\":\"BLOCKED\",",
  "\"uf7_note\":\"restricted presentation D_3^(2) の構成式が spec 射影内に見当たらない(manifest_spec_v1.md を grep 済み・GAP-E5 も未了と明記)。verbal 側(U-F4 の関係式)のみで P2 を構成し独立検算した。両表示一致(U-F7)は本証明書では判定不能 -- 司令塔確認要\"",
  "}");;
