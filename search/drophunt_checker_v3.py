...
def build_hom_tracked_chain_general(gen_image_pairs: list[tuple], deg: int) -> list[dict]:
    """General form: gen_image_pairs = [(gen1,img1), (gen2,img2), ...]. Builds
    the chain over the group GENERATED BY [gen1,gen2,...] (which may be
    strictly larger than <JX,JY> -- e.g. A=<JX,JY,JC> when JC is not itself
    in <JX,JY>, i.e. c_in_K is False). This generalization was added after
    this pass's own 20-window dry run surfaced a receipt (node_id
    f1b8d8351bdb19b5..., c_in_K=False) where tau~(y^m f) genuinely leaves
    G=<JX,JY> (since tau~(y) involves JC, which is outside G exactly when
    c_in_K is False) -- so tau~^2 could not be sifted through a chain built
    only over G. This is a CHECKER COVERAGE fix, not a producer fix: the
    producer's own tauHom is a GAP GroupHomomorphismByImages over the full
    A, which was never scope-limited to G in the first place."""
    gens = [g for g, _ in gen_image_pairs]
    gen_images = {}
    for g, img in gen_image_pairs:
        gen_images[g] = img
        gen_images[pinv(g)] = pinv(img)
    current = [g for g in gens if g != pid(deg)]
    remaining_points = list(range(deg))
    chain = []
    while True:
...
def build_hom_tracked_chain(JX: Perm, JY: Perm, imgX: Perm, imgY: Perm, deg: int) -> list[dict]:
    """Backward-compatible 2-generator form (chain over G=<JX,JY> only) --
    kept for theta~, which only ever needs G (theta~(x)=y, theta~(y)=x, both
    already in G, so theta~ never leaves G regardless of c_in_K)."""
    return build_hom_tracked_chain_general([(JX, imgX), (JY, imgY)], deg)
...
def check_receipt(path: str) -> dict:
    ...
    # theta~(x)=y, theta~(y)=x -- both already in G=<JX,JY>, so theta~ never
    # leaves G regardless of c_in_K; the 2-generator chain suffices.
    theta_chain = build_hom_tracked_chain(JX, JY, JY, JX, deg)
    # tau~(x)=y, tau~(y)=y^-1 x^-1 c, tau~(c)=c. tau~(y) involves JC, which
    # is OUTSIDE G=<JX,JY> exactly when c_in_K is False -- in that case
    # tau~(anything involving y) can leave G, and tau~^2 then needs to sift
    # an element that may not be in G at all. Building the chain over the
    # FULL A=<JX,JY,JC> (not just G) fixes this (item1/item8 finding this
    # pass: node_id f1b8d8351bdb19b5..., c_in_K=False, tau~^2 previously
    # returned a spurious None from a chain that only covered G -- a
    # CHECKER coverage gap, not a producer bug, since the producer's own
    # tauHom is a GAP homomorphism over the full A from the start).
    tau_img_y = pmul(pmul(pinv(JY), pinv(JX)), JC)  # y^-1 x^-1 c
    tau_chain = build_hom_tracked_chain_general([(JX, JY), (JY, tau_img_y), (JC, JC)], deg)
    ...
    # 裁定1776 (MULT-COSET / final stop lattice): independently recompute
    # F1_prime, F1_double_prime, per_m, lift_m, mult_set, valid_total,
    # verdict from the SAME rows already re-derived above (not trusted from
    # the receipt), and require an EXACT match against the receipt's own
    # declared values -- including a STRICT null-vs-0 distinction in per_m
    # (null = gcd-excluded m, never evaluated; 0 = evaluated, no witness).
    m_ord = M_ord
    f1_prime_recomputed = K_ord // m_ord
    m_cands = [m_ord * t for t in range(f1_prime_recomputed)]
    per_m_recomputed = []
    lift_m_recomputed = 0
    mult_set_recomputed = []
    valid_total_recomputed = 0
    for m in m_cands:
        if math.gcd(2 * m + 1, K_ord) != 1:
            per_m_recomputed.append(None)
        else:
            cnt = sum(1 for row in rows if row["m"] == m and row["verdict"] is True)
            per_m_recomputed.append(cnt)
            valid_total_recomputed += cnt
            if cnt >= 1:
                lift_m_recomputed += 1
                if cnt not in mult_set_recomputed:
                    mult_set_recomputed.append(cnt)
    mult_set_recomputed_sorted = sorted(mult_set_recomputed)
    f1_double_prime_recomputed = sum(1 for m in m_cands if math.gcd(2 * m + 1, K_ord) == 1)
    if lift_m_recomputed > f1_double_prime_recomputed or len(mult_set_recomputed_sorted) > 1:
        verdict_class_recomputed = "BUG"
    elif valid_total_recomputed == 0:
        verdict_class_recomputed = "DROP"
    elif lift_m_recomputed < f1_double_prime_recomputed:
        verdict_class_recomputed = "ANOMALY"
    else:
        verdict_class_recomputed = "PASS"

    require("F1_prime" in receipt, "MULT_F1_PRIME_MISSING", "")
    require("F1_double_prime" in receipt, "MULT_F1_DOUBLE_PRIME_MISSING", "")
    f1_double_prime_declared = receipt.get("F1_double_prime")
    per_m_declared = receipt.get("per_m")
    lift_m_declared = receipt.get("lift_m")
    mult_set_declared = receipt.get("mult_set")
    valid_total_declared = receipt.get("valid_total")
    verdict_declared = receipt.get("verdict")
    require(window.get("F1_m_factor") == f1_prime_recomputed, "MULT_F1_PRIME_MISMATCH",
            f"recomputed={f1_prime_recomputed} declared_window_F1_m_factor={window.get('F1_m_factor')}")
    require(f1_double_prime_declared == f1_double_prime_recomputed, "MULT_F1_DOUBLE_PRIME_MISMATCH",
            f"recomputed={f1_double_prime_recomputed} declared={f1_double_prime_declared}")
    require(isinstance(per_m_declared, list) and len(per_m_declared) == len(per_m_recomputed),
            "MULT_PER_M_SHAPE_MISMATCH", f"declared={per_m_declared} recomputed={per_m_recomputed}")
    if isinstance(per_m_declared, list) and len(per_m_declared) == len(per_m_recomputed):
        for i, (declared_v, recomputed_v) in enumerate(zip(per_m_declared, per_m_recomputed)):
            # STRICT null/0 distinction: Python's "None == 0" is False, so a
            # plain == already refuses to conflate them; the explicit
            # isinstance checks below make that refusal an intentional,
            # documented property of this check rather than an accident of
            # Python semantics (0 = evaluated/no-lift; None = gcd-excluded/
            # never evaluated -- they must never be treated as equal).
            entry_ok = (declared_v is None and recomputed_v is None) or \
                       (isinstance(declared_v, int) and isinstance(recomputed_v, int) and declared_v == recomputed_v)
            require(entry_ok, "MULT_PER_M_ENTRY_MISMATCH", f"m_index={i}: declared={declared_v!r} recomputed={recomputed_v!r}")
    require(lift_m_declared == lift_m_recomputed, "MULT_LIFT_M_MISMATCH",
            f"recomputed={lift_m_recomputed} declared={lift_m_declared}")
    require(isinstance(mult_set_declared, list) and sorted(mult_set_declared) == mult_set_recomputed_sorted,
            "MULT_SET_MISMATCH", f"recomputed={mult_set_recomputed_sorted} declared={mult_set_declared}")
    require(valid_total_declared == valid_total_recomputed, "MULT_VALID_TOTAL_MISMATCH",
            f"recomputed={valid_total_recomputed} declared={valid_total_declared}")
    require(verdict_declared == verdict_class_recomputed, "MULT_VERDICT_MISMATCH",
            f"recomputed={verdict_class_recomputed} declared={verdict_declared}")
