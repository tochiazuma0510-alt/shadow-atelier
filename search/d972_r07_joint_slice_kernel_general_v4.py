...
PASS="R07_JOINT_SLICE_KERNEL_GENERAL_V4_PRODUCER_SELFTEST_PASS"
...
def parse_fixture(path):
    value=json.loads(path.read_text(encoding="utf-8"));require(value.get("schema")==SELF and value.get("fixture_seal"),"fixture seal/schema");require(value.get("modulus")==3 and len(value.get("cases",[]))==5,"fixture cases")
...
def mutate(case,owner):
    ...
    c["mutation_fixture_seal"]=digest(c)
...
def selftest(fixture):
    ...
    controls=[]
    for owner in OWNERS:controls.append({"owner":owner,"accepted":mutate(f["cases"][1],owner) is not None})
    require(not any(x["accepted"] for x in controls),"mutation accepted")
    body={"schema":SELF,"status":"COMPLETE","terminal":PASS,"cases":results,"mutation_controls":controls,"mutation_attempted":len(controls),"mutation_rejected":sum(x["accepted"] is False for x in controls),"production_input":False}
...
def main(argv=None):
    ...
    a.output.write_bytes(canon(value)+b"\n");print(PASS if a.mode=="SELFTEST" else "R07_JOINT_SLICE_KERNEL_GENERAL_V4_PRODUCER_TERMINAL "+value["terminal"])
