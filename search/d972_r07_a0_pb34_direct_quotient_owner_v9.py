#!/usr/bin/env python3
"""Task427 v9: standalone quotient loop with durable candidate telemetry."""
from __future__ import annotations
import argparse,gzip,hashlib,json,marshal,os,shutil,sys,tempfile,time,types
from collections import deque
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];SCHEMA="d972-r07-a0-pb34-direct-quotient-owner/v9";RSS_CAP=4800000000;UNKNOWN="UNKNOWN";RESOURCE="UNKNOWN_RESOURCE"
...
v3=load(V3,"task427_v3_pinned");LAST_DURABLE=None
class Echelon:
...
def actor_v9(runtime,model,owner,p176,q,row,letter):
 out={};groups={};cache={}
 for k,c in row.items():
  if k[:1]==b"N":
   x=(out.get(k,0)+int(c))%3
...
def seed_v9(model,old,owner,p176,q,word):
 grouped={}
...
def cp_write(path,state):
 ...
 n,h=whole(tmp)
 with tempfile.NamedTemporaryFile(dir=p.parent,delete=False) as f:
  out=Path(f.name);f.write(("D972-A0V9-CP1 "+h+" "+str(n)+"\n").encode());
...
def cp_read(path):
 ...
 with p.open("rb") as f:
  head=f.readline().decode().rstrip("\n").split(" ");need(len(head)==3 and head[0]=="D972-A0V9-CP1","checkpoint_header");h=hashlib.sha256();n=0
...
def run(a):
 global LAST_DURABLE
 start=time.monotonic();t413=v3.load(v3.T413,"task427_task413");base=t413["bound_module"](t413["BASE"],"task427_base");receipt=t413["load_json"](base,t413["JOINT"]);q3=t413["load_json"](base,t413["Q3"]);pres=base["compact"](receipt,q3);core=base["load_task198_core"]();roof=t413["load_json"](base,base["ROOF"]);acceptance=t413["load_json"](base,base["ACCEPTANCE"]);need(base["acceptance_ok"](acceptance),"acceptance_v2_contract");authority=types.SimpleNamespace(receipt=roof);layout=base["load_bound_module"](base["TASK379"],"task427_layout")["validate_layout"];ledger=layout(core,authority);runtime=core.Runtime(authority,core.Meter(dict(core.CAPS)));owner,g760,model=base["direct_physical_owner"](runtime);p176=base["load_bound_module"](base["TASK176"],"task427_p176");q=Quotient(owner,p176,runtime.e3,runtime.e4);target=q.transform(t413["target_row"](base,owner,runtime.old,runtime.e3,runtime.e4,g760,model));occ=Echelon();phys=Echelon();queue=deque();seed_cursor=parent_cursor=action_cursor=seq=0;last_save=0.;durable=None;binding=hashlib.sha256((SCHEMA+V3[2]).encode()).hexdigest();print("phase=preflight owner_v9=BOUND rss_cap=%d"%RSS_CAP,flush=True);print("phase=runtime_bootstrap owner_v9=READY",flush=True)
 ...
 if a.resume:
  ...
  for i,w in enumerate(pres["relators"],1):
   guard("seeds");ss=runtime.states_direct(list(w));need(all(x.a==x.q.identity for x in ss),"seed_identity");r=seed_v9(model,runtime.old,owner,p176,q,list(w));rise,p=occ.add(r,{"family":"LEAF","seed":i});seed_cursor=i
 ...
 if a.resume and seed_cursor<44:
  for i in range(seed_cursor+1,45):
   w=pres["relators"][i-1];guard("seeds");ss=runtime.states_direct(list(w));need(all(x.a==x.q.identity for x in ss),"seed_identity");r=seed_v9(model,runtime.old,owner,p176,q,list(w));rise,p=occ.add(r,{"family":"LEAF","seed":i});seed_cursor=i
   if rise:queue.append(p);phys.add(aggregate(occ.rows[p]),{"family":"PHYSICAL","occurrence_pivot":p.hex(),"seed":i})
   guard("seeds")
 ...
 while queue:
  ...
  for letter in (1,-1,2,-2):
   r=actor_v9(runtime,model,owner,p176,q,occ.rows[p],letter);rise,np=occ.add(r,{"family":"CONJUGATE","letter":letter,"parent":p.hex()});action_cursor+=1
...
def main():
 ...
 print("R07_A0_PB34_DIRECT_QUOTIENT_OWNER_V9 "+status,flush=True);return 0
