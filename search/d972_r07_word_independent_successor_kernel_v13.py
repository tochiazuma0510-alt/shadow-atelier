#!/usr/bin/env python3
"""A4 v13 owner: frozen v12 arithmetic with early completed-row checkpoints.

The mathematical oracle is unchanged.  This wrapper restores the exact v12
generated source, then applies only checkpoint-prefix and observability
repairs.  In particular, it does not batch boundary columns.
"""
...
OWNER = Path(__file__).with_name("d972_r07_word_independent_successor_kernel_v12.py")
OWNER_BYTES = 7209
OWNER_SHA256 = "816bae92d86ac4bf3a6feb05297f505680072c2ce793db97135154cef928e9c5"
...
PATCHES = (
    ...
    (
        b'    meter.reserve("correlation_pairs", expected_pairs, "full_D_correlation")',
...
def restore_frozen() -> bytes:
    ...
    if len(owner_raw) != OWNER_BYTES or hashlib.sha256(owner_raw).hexdigest() != OWNER_SHA256:
        raise SystemExit("v13 producer: frozen v12 owner drift")
    owner_ns: dict[str, Any] = {
        "__name__": "_r07_a4_v12_owner",
    ...
    if len(raw) != owner_ns["SOURCE_BYTES"] or hashlib.sha256(raw).hexdigest() != owner_ns["SOURCE_SHA256"]:
        raise SystemExit("v13 producer: frozen v6 source drift")
    for old, new in owner_ns["PATCHES"]:
        if raw.count(old) != 1:
            raise SystemExit("v13 producer: v12 restoration cardinality")
        raw = raw.replace(old, new)
    for old, new in PATCHES:
        if raw.count(old) != 1:
            raise SystemExit("v13 producer: audited site is not unique")
        raw = raw.replace(old, new)
    return raw
