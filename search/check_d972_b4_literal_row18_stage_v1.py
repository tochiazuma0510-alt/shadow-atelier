...
def validate_report(report: dict) -> None:
    ...
    for images in ARTIN:
        ...
        for word in words:
            require(word_value(word,pgens)==onep4 and word_value(word,ggens)==oneg4,
                    "action left kernel")
            rows.append(mask24(word_value(word,egens),masks))
    ...
    root_action=[mask24(word_value(substitute(w,root_source),egens),masks) for w in basis_words]
    ...
    for exponent,pword,row_index,key in power_inputs:
        ...
        bhe=hexagons(pword,0,core["x"],core["y"])
        bhp=hexagons(pword,0,core["px"],core["py"])
        bhg=hexagons(pword,0,core["gx"],core["gy"])
        base_hex_masks=None
        ...
        pe=pent(pword,egens); pp=pent(pword,pgens); pg=pent(pword,ggens)
        ...
        for bits in range(64):
            ...
            he=hexagons(candidate,0,core["x"],core["y"])
            hp=hexagons(candidate,0,core["px"],core["py"])
            hg=hexagons(candidate,0,core["gx"],core["gy"])
            ce=pent(candidate,egens); cp=pent(candidate,pgens); cg=pent(candidate,ggens)
            ...
            roof_ok=(word_value(candidate,(core["px"],core["py"]))==word_value(pword,(core["px"],core["py"])) and
                     word_value(candidate,(core["gx"],core["gy"]))==word_value(pword,(core["gx"],core["gy"])))
            ...
            if preliminary:
                fe=word_value(candidate,(core["x"],core["y"])); fg=word_value(candidate,(core["gx"],core["gy"]))
    ...
    if selected is not None and not global_missing:
        ...
        sa=[mask24(word_value(substitute(w,sword),egens),masks) for w in basis_words]
        local_orders_ok=True
        for c in range(4):
            local_orders_ok &= len(closure(tuple(blocks(x,DEG_E)[c] for x in se)))==32256
            local_orders_ok &= len(closure(tuple(blocks(x,DEG_P)[c] for x in sp4)))==504
            local_orders_ok &= len(closure(tuple(blocks(x,DEG_G9)[c] for x in sg4)))==2916
        relation_preserved=all(span.solve(apply_matrix(r["vector"],sa)) is not None for r in relation)
        quotient_image=Span(24)
        for r in relation: quotient_image.insert(r["vector"],1)
        for row in sa: quotient_image.insert(row,1)
        quotient_bijective=relation_preserved and quotient_image.rank==24
        if row_rank(sa,24)!=24 or not local_orders_ok or not quotient_bijective:
            ...
            settlement={"source_words":sword,"source_images_E":[list(x) for x in se],
                        ...
                        "literal_quotient_bijective":True,"settled":True}
