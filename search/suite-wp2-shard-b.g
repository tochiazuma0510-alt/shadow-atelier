# suite-wp2-shard-b.g -- WP2a GAP explorer, shard B (n=13..16 + N5 control)
#
# ** SUPERSEDED (2026-07-19): 実測 897.2 秒(GAP 内部 877922 ms)で 10 分 cap を超過した。
#    n=13(shadows=312, compTable 最大約 97344 組)の JSON 文字列結合コストが支配的と推定
#    (観察のみ・原因判定は司令塔)。司令塔指示によりさらに分割: suite-wp2-shard-b1.g
#    (n=13,14) / suite-wp2-shard-b2.g (n=15,16+N5) に置き換える。本ファイルは証明書
#    K13..K16,N5 を正しく(決定的に)生成しているため証拠として無変更のまま残す
#    (再実行結果は照合済み: hashes 17/17 一致)。以後の正本は shard-b1/b2。**
#
# 実行: .\gap.ps1 search\suite-wp2-shard-b.g
#
# 背景: search/suite-wp2-explorer.g (n=3..16+N5 を一括生成) が 687 秒で 10 分 cap を
# 14% 超過したため、決定的 shard 分割で再実行する(司令塔指示 2026-07-19)。分割は
# 対象集合の分割のみであり、各対象の計算内容・出力形式は無変更(元スクリプトの該当
# コードをそのままコピー -- BuildCertJsonDihedral/BuildCertJsonN5 内の "generated_by".script
# 文字列リテラルも含め無変更。これは証明書 JSON のバイト内容を保つため -- 変更すると
# provenance/cert-hashes-wp2.txt の記録済みハッシュと不一致になる)。
#
# 宇宙分担(事前登録宇宙 {3..16,18,36,N5} の厳密な分割):
#   shard A (suite-wp2-shard-a.g): n = 3..12 の dihedral
#   shard B (本ファイル): n = 13..16 の dihedral + N5 control
#   q1836 (suite-wp2-explorer-q1836.g, 既存・無変更): n = 18, 36
#   3 本の対象集合の和 = {3..16,18,36,N5} = 事前登録宇宙と exact 一致。
#   注記: 元スクリプトの reduction branch suite 3 対((8,4),(12,4),(9,3))はすべて
#   n in {3,4,8,9,12} 上で完結するため shard A 側にあり、本 shard は reduction 計算
#   を行わない(K13..K16 の "reduction" フィールドは元スクリプトでも空配列)。
#
...
# ================= 基本ヘルパー (suite-wp2-explorer.g より無変更で流用) =================
MakeDn := function(n)
  local r, s;
...
# ================= reduction (q -> n, K^(q) <= K^(n)) -- 本 shard では未使用 (n=13..16 は
# reduction branch suite に含まれないため。ヘルパーのみ元スクリプトから複製維持) =================
...
# ================= メインドライバ: shard B -- n=13..16 + N5 =================
universe := [13..16];;
results := [];;    # results[n] := ProcessDihedral(n)
Print("宇宙 shard B (事前登録宇宙 {3..16,18,36,N5} のうち n=13..16 + N5): ", universe, " + N5\n");
...
Print("\n累計 elapsed ms (n=13..16): ", Runtime()-startTime, "\n");

# ---- reduction: 本 shard の対象 (13..16) は reduction branch suite に含まれない
# (元スクリプトでも reductionsByQ[13..16] は未設定 = 空配列) ----
reductionsByQ := List([1..40], i -> []);;
Print("(reduction branch suite 5 対はすべて shard A / q1836 側で計算済み。n=13..16 には reduction なし)\n");
