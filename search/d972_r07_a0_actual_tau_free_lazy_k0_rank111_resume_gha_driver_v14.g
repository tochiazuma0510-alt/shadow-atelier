#############################################################################
## Task529 exact-schema rank111 lazy K=0 successor driver v14. External preamble required.
#############################################################################
if not IsBound(D972_R07_A0_LAZY_K0_RANK111_RESUME_V14_RUN) or
   D972_R07_A0_LAZY_K0_RANK111_RESUME_V14_RUN<>true then
 Error("task529 external preamble required"); fi;
D527Producer:="search/d972_r07_a0_actual_tau_free_lazy_k0_seed_v6.py";;
D527Checker:="crosscheck/check_d972_r07_a0_actual_tau_free_lazy_k0_seed_v10.py";;
...
D527Zip:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_rank111_resume_v14.zip";;
D527Extract:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_rank111_resume_v14_archive";;
D527ResumeMember:="d972_r07_a0_actual_tau_free_rank_ladder_v10_output.checkpoint";;
D527Input:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_rank111_resume_v14_input.checkpoint";;
D527Artifact:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_seed_v6.json";;
D527Checkpoint:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_seed_v6_output.checkpoint";;
D527PL:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_seed_v6_producer.log";;
D527CL:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_seed_v6_checker.log";;
D527DL:="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_rank111_resume_v14_preflight.log";;
D527ZipBytes:=37586;;
D527ZipSHA:="8b740dbbc81f5d2e659371a81453ded56c6711ce8ace35a4af5255303e0095de";;
D527ProducerBytes:=42434;;
D527ProducerSHA:="43f5dac842fd4025d714b99a1e16f63ecf7cc2a35c53d8f106748a4d06a13b1c";;
D527CheckerBytes:=33455;;
D527CheckerSHA:="36db2a4e5eafb9a2c6a23d0ec9d280f22503b033fcf098f1c2ee19f32db5dd78";;
...
D527Quote:=function(s)
 if PositionSublist(s,"'")<>fail or PositionSublist(s,"\n")<>fail or
    PositionSublist(s,"\r")<>fail then Error("task529 unsafe shell text"); fi;
 return Concatenation("'",s,"'");
end;;
if Length(D527MemberManifest)<>8 then Error("task529 eight-member manifest"); fi;
if D527Run<>"33564845217" or D527Job<>"100045550767" or
 ...
 Error("task529 immutable source binding drift"); fi;
if IsExistingFile(D527Zip) or IsDirectoryPath(D527Extract) or
 ...
 Error("task529 stale output"); fi;
if not IsDirectoryPath("ci/out") then
 if CreateDir("ci/out")=fail then Error("task529 ci/out"); fi; fi;
D527Cmd:=Concatenation(
 "set -euo pipefail; umask 077; ",
 "test ! -e \"",D527DL,"\"; test ! -L \"",D527DL,"\"; ",
 "exec 9>\"",D527DL,"\"; ",
 "printf \"TASK529_R07_A0_LAZY_K0_RANK111_PREFLIGHT_BEGIN\\n\" >&9; ",
 "task529_preflight_fail() { rc=$?; printf \"TASK529_R07_A0_LAZY_K0_RANK111_PREFLIGHT_FAIL rc=%s cmd=%s\\n\" \"$rc\" \"$BASH_COMMAND\" >&9; return \"$rc\"; }; ",
 "trap task529_preflight_fail ERR; ",
...
D527Cmd:=Concatenation(D527Cmd,
 ...
 "test \"$(grep -Ec \"^R07_A0_ACTUAL_TAU_FREE_LAZY_K0_SEED_V6 status=(UNKNOWN_RESOURCE|UNKNOWN|COMMON_CANDIDATE)$\" \"",D527PL,"\")\" = \"1\"; ",
 "timeout --foreground --signal=TERM --kill-after=60s 3600s python3 -u -B ",D527Checker,
 " \"",D527Artifact,"\" 2>&1 | tee \"",D527CL,"\"; ",
 "test \"$(grep -Fc \"R07_A0_ACTUAL_TAU_FREE_LAZY_K0_SEED_V10_CHECKER_PASS\" \"",D527CL,"\")\" = \"1\"; ",
...
if not IsExistingFile(D527DL) or Length(StringFile(D527DL))=0 then
 Error("task529 preflight diagnostic missing"); fi;
if not IsExistingFile(D527Artifact) or not IsExistingFile(D527Checkpoint) or
   not IsExistingFile(D527CL) then Error("task529 result/checker missing"); fi;
if Length(StringFile(D527Artifact))=0 or Length(StringFile(D527Checkpoint))=0 then
 Error("task529 result/checkpoint empty"); fi;
if PositionSublist(StringFile(D527PL),"R07_A0_ACTUAL_TAU_FREE_LAZY_K0_SEED_V6 status=")=fail then
 Error("task529 producer marker"); fi;
if StringFile(D527CL)<>"R07_A0_ACTUAL_TAU_FREE_LAZY_K0_SEED_V10_CHECKER_PASS\n" then
 Error("task529 checker marker"); fi;
Print("R07_A0_LAZY_K0_RANK111_RESUME_V14_DRIVER_PASS\n");
