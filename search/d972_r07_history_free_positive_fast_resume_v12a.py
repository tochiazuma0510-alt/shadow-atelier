#!/usr/bin/env python3
"""v12a SELFTEST_BOOTSTRAP producer with no production/resume surface.
...
P0_PATH = ROOT / "ci/in/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12a.manifest.v1.json"
SCHEMA = "d972-r07-history-free-positive-fast-resume/v12a"
P0_SCHEMA = "d972-r07-history-free-positive-fast-resume/selftest-bootstrap-manifest/v1"
FIXTURE_PATH = "search/certs/d972_r07_history_free_positive_fast_resume_selftest_v12a_20260829.json"
R_OUTPUT_PATH = "ci/out/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12a.receipt.json"
V_OUTPUT_PATH = "ci/out/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12a.verdict.json"
RAW_SOURCE_PATH = "ci/resume/d972_r07_history_free_positive_fast_resume_selftest_v12a.raw.json"
OUTPUT_SIBLINGS = (
...
SELFTEST_TERMINAL = "V12A_SELFTEST_BOOTSTRAP_ARTIFACT"
PRODUCER_PREFIX = "V12A_PRODUCER_TERMINAL"
...
SOURCE_PINS: dict[str, tuple[str, int, str]] = {
    ...
    "fixture": ("search/certs/d972_r07_history_free_positive_fast_resume_selftest_v12a_20260829.json", 22094,
                "6a87bf608bf0a392ff77d3aacbe813a0cc01f54d67bd5d346fb75ee1e7000ffc"),
...
class AncestryDAG:
    ...
    def expand(self, node: int, max_nodes: int = MAX_FORMAL_ENTRIES,
               max_entries: int = MAX_FORMAL_ENTRIES) -> dict[str, int]:
        require(0 <= int(node) < len(self.nodes), "DAG expansion node")
        memo: dict[int, dict[str, int]] = {}
        self.last_expanded_node_count = 0
        # Explicit post-order stack: recursion depth is not an implicit
        # resource and every reachable node/expanded entry is bounded.
        stack: list[tuple[int, bool]] = [(int(node), False)]
        while stack:
            current, finished = stack.pop()
            if current in memo:
                continue
            require(len(memo) < int(max_nodes), "DAG expansion node cap")
            require(0 <= current < len(self.nodes), "DAG expansion reference")
            item = self.nodes[current]
            if not finished:
                stack.append((current, True))
                if item[0] == "add":
                    stack.append((int(item[2]), False))
                    stack.append((int(item[1]), False))
                continue
            if item[0] == "zero":
                answer: dict[str, int] = {}
            elif item[0] == "literal":
                ...
                answer = dict(memo[int(item[1])])
                for key, value in memo[int(item[2])].items():
        ...
        self.last_expanded_node_count = len(memo)
        return dict(memo[int(node)])
...
def _stable_k0_hash(coarse: bytes) -> int:
    ...
    return int.from_bytes(hashlib.sha256(
        b"v12a-k0-coarse-key\0" + coarse).digest()[:8], "little")
...
class StableProducerK0Index:
    ...
    def build(self) -> None:
        if self.build_count:
            return
        state_digest = hashlib.sha256(b"v12a-k0-state-stream/v1\0")
        ...
        self.build_count = 1
        slot_digest = hashlib.sha256(b"v12a-k0-slot-stream/v1\0")
        ...
        self._public_digest = sha_obj({
            "schema": "v12a-k0-public-cache/v1",
    ...
    def public(self) -> dict[str, Any]:
        return {"state_count": self.state_count, "width": self.width,
                ...
                "hash": "sha256(v12a-k0-coarse-key\\0)[:8]:little",
    ...
    def _coarse_index(self, coordinate: int) -> StableProducerK0Index:
        ...
        if index is None:
            ...
            index = StableProducerK0Index(coordinate,
                self.rt["stores"][coordinate], width, degree)
...
def prove_l_shared(old: Any, name: str, membership: bytearray, count: int,
    ...
    for state_id in range(1_469_664):
        if _producer_bit_get(membership, state_id) and state_id not in subgroup:
            ...
            require(all(_producer_bit_get(membership, value) for value in subgroup),
                    name + " closure subset")
            if len(subgroup) == count:
                break
    require(len(subgroup) == count and
            all((state_id in subgroup) == _producer_bit_get(membership, state_id)
                for state_id in range(1_469_664)), name + " generated bitset")
...
def build_heavy(runtime: dict[str, Any], registry: SourceRegistry,
    ...
    projected_packed = [b"".join(p176.packed_joint_blob(
        value, "v12a projected Gamma state") for value in row)
    ...
    full_diagnostics = [producer_full_gamma_diagnostic(runtime, state)
                        for state in gamma.states]
    require(sum(projected_widths) == 970 and
            sum(full_diagnostic_widths) == 4814 and
            all(len(row) == 4814 for row in full_diagnostics) and
    ...
    heavy_public = {
        ...
        "full_diagnostic_state_sha256": sha_obj(
            [row.hex() for row in full_diagnostics]),
...
def atomic_json(path: Path, value: dict[str, Any], *, allow_replace: bool = False,
    ...
    if trace_runtime is not None:
        _producer_validator_event(trace_runtime, "exclusive_json", "stale",
                                  "candidate.path")
    # Final publication is a no-replace operation.  ``path.exists()`` is only
    # an early stale check; it cannot protect the check/publish interval.
    # Scratch mutation files use their own open/replace paths and never call
    # this publication API with replacement enabled.
    if allow_replace:
    ...
    if not stat.S_ISDIR(parent_before.st_mode) or stat.S_ISLNK(parent_before.st_mode):
        raise ProtocolStop("publication parent is not a regular directory")
    ...
    if estimated_json_size(value) > maximum:
        raise ResourceStop("checkpoint_serialization", "checkpoint_bytes",
                           estimated_json_size(value), maximum)
    ...
    try:
        ...
        try:
            ...
            os.close(fd)
        # A hard-link publish fails atomically if a target appeared after the
        # precheck; unlike os.replace it can never overwrite that owner.
        try:
        ...
        try:
            ...
            raise ProtocolStop("publication parent disappeared") from exc
        if ((parent_after.st_dev, parent_after.st_ino,
             parent_after.st_mode) !=
                (parent_before.st_dev, parent_before.st_ino,
                 parent_before.st_mode)):
            raise ProtocolStop("publication parent identity changed")
        try:
            ...
            raise ProtocolStop("publication directory fsync") from exc
    finally:
        try:
            temporary.unlink()
        ...
        try:
            ...
            if trace_runtime is not None:
                _producer_capture_physical_projection(
                    ...
                    path_after=path_after, raw=bytes(raw), reason=None)
...
def _triangular_subset_frame(old_value: dict[str, Any], live: Any) -> dict[str, Any]:
    # The self-test frame is the authenticated physical checkpoint roster,
    # not a toy prefix.  Every one of the 2,896 columns is replayed.
    records = json.loads(canonical(old_value["columns"]).decode("ascii"))
...
def _physical_mutation_trace(path: Path, value: dict[str, Any],
                             validator: Any,
                             baseline_value: dict[str, Any] | None = None,
                             trace_runtime: dict[str, Any] | None = None
                             ) -> tuple[str, str, str]:
    """Write/read one real mutation and retain its first ordinary reason."""
    raw = canonical(value) + b"\n"
    before = sha_bytes(canonical(baseline_value if baseline_value is not None
                                 else value) + b"\n")
    if trace_runtime is not None:
    ...
    try:
        ...
        try:
            candidate, physical = read_bounded_json(
                path, len(raw), None, trace_runtime=trace_runtime)
        except (ProtocolStop, InputStop) as exc:
            if trace_runtime is not None:
        ...
        raise ProtocolStop("physical mutation accepted:" + path.name)
    finally:
        try:
            path.unlink()
        except FileNotFoundError:
    ...
    with tempfile.TemporaryDirectory(prefix="r07-v7-triangular-") as directory:
        ...
        for ordinal, name in enumerate(names):
            ...
            if name == "future_ancestry_index":
                ...
                # Keep the mutation a valid physical encoding; the ordinary
                # P-equation validator then rejects the wrong pivot owner.
            ...
            expected = (contract[ordinal] if contract is not None else
                        {"owner_path": "old.columns", "validator":
                         "_validate_triangular_subset", "stage":
                         "full_P_equation", "reseal": True,
                         "first_reason": "triangular selftest ancestry"})
            trace_runtime: dict[str, Any] = {}
            def validate_mutant(candidate: dict[str, Any]) -> None:
                _validate_triangular_subset(candidate, live, trace_runtime)
            before, after, reason = _physical_mutation_trace(
                root / f"{ordinal:02d}.json", value, validate_mutant, baseline,
                trace_runtime)
            ...
            results.append({"id": name, "owner_path": expected["owner_path"],
...
def producer_bind_section_identity(runtime: dict[str, Any],
    ...
    selected_coordinate_mark = [p176.packed_joint_blob(
        value, "v12a selected coordinate mark") for value in coordinate_mark]
    ...
    for gamma_id, projected_row in enumerate(runtime["projected"], 1):
        packed = b"".join(p176.packed_joint_blob(
            value, "v12a Gamma projected coordinate") for value in projected_row)
    ...
    result.update({"q0_state_hex": bytes(qstate).hex(),
        ...
        "q0_parent_letter_digest": sha_obj([
            list(runtime["parents"]), list(runtime["letters"])]),
...
def producer_validate_correction(runtime: dict[str, Any], record: dict[str, Any],
    ...
    gamma_full = producer_full_gamma_diagnostic(
        runtime, runtime["gamma"].states[gid - 1])
    _producer_validator_event(runtime, "validate_correction_provenance",
    ...
    require(section.get("q0_parent_letter_digest") == sha_obj(
        [list(runtime["parents"]), list(runtime["letters"])]),
...
def _producer_correction_selftest_frame(runtime: dict[str, Any],
    ...
    """Select one actual physical correction owner for mutation selftests."""
    ...
    for coordinate in range(10):
        ...
        for gamma_id in range(1, 244):
            ...
            for roster_index, roster_row in enumerate(runtime["roster"], 1):
                ...
                rkeys = [key for key in sorted(row) if key[:1] == b"R"]
                if not rkeys:
                    continue
                key = rkeys[0]; coefficient = int(row[key]) % 3
                dual = {key: coefficient}
...
def producer_selected_statement(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"search_key": [int(provenance["roster_index"]),
        ...
        "dual": {"canonical_active_dual": record["active_dual"],
            ...
            "pivot_epoch_owner": {"pivot_hex": record.get("pivot_hex"),
                "rank_before": record.get("rank_before"),
                "rank_after": record.get("rank_after")}},
...
def producer_final_heavy_identity(runtime: dict[str, Any],
    ...
    public = {"schema": "r07-a0-final-heavy-carrier/v2",
        ...
        "algorithms": {"selected_k0":
            "v12a-coarse-open-address-retained-full-state-first-gid-bfs",
...
def producer_build_owner_preselection(runtime: dict[str, Any],
    ...
    owner = {"schema": "r07-a0-checker-local-preselection/v1",
        ...
        "algorithms": {"task176_decoder":
            ...
            "selected_k0": "v12a-coarse-open-address-retained-full-state-first-gid-bfs",
            "canonical_json": "json.dumps(sort_keys=True,separators=(',',':'),ensure_ascii=True)",
...
def _producer_mutation_trace(path: Path, value: dict[str, Any],
                             validator: Any,
                             mutation_hook: Any | None = None,
                             baseline_value: dict[str, Any] | None = None,
                             trace_runtime: dict[str, Any] | None = None
    ...
    try:
        try:
            candidate, physical = read_bounded_json(path, len(raw), None,
                                                    mutation_hook,
                                                    trace_runtime)
            ...
            if trace_runtime is not None:
                trace_runtime["_terminal_count"] = int(
                    trace_runtime.get("_terminal_count", 0)) + 1
            if trace_runtime is not None:
                _producer_physical_projection_reason(trace_runtime,
                                                     "read_bounded_json:" + str(exc))
            return before, sha_bytes(raw), "read_bounded_json:" + str(exc)
        try:
            validator(candidate)
        except ProtocolStop as exc:
            if trace_runtime is not None:
            ...
            if trace_runtime is not None:
                _producer_physical_projection_reason(trace_runtime, str(exc))
            return before, physical["sha256"], str(exc)
        raise ProtocolStop("producer mutation accepted:" + path.name)
    finally:
        try:
            path.unlink()
        except FileNotFoundError:
...
def producer_selected_correction_mutations(runtime: dict[str, Any],
    ...
    with tempfile.TemporaryDirectory(prefix="r07-v12a-producer-selected-") as directory:
        root = Path(directory)
        for ordinal, name in enumerate(fixture["selected_correction_mutations"]):
            ...
            _before, after, reason = _producer_mutation_trace(
                ...
                lambda candidate: _producer_selected_validator(
                    runtime, candidate, recovery), baseline_value=baseline,
...
def blocked_send_selftest(context: Any, deadline_seconds: float = 0.05) -> dict[str, Any]:
    ...
    def no_drain(sock: socket.socket, deadline: float) -> None:
        # Hold the peer open with one bounded kernel wait.  A polling spin
        # would turn this transport fault probe into unmetered CPU work.
...
def _deterministic_boundary_accounting(accounting: dict[str, Any],
    ...
    require(all(type(accounting.get(key)) is int and accounting[key] >= 0
            ...
            accounting.get("stop_frames_sent") == workers and
...
def process_selftest(runtime: dict[str, Any], reducer: FormalReducer,
    ...
    return {"first_dual": live.public_sparse(dual),
            ...
            "cumulative_accounting": {
                ...
                "fault_owner_count": 3 * len(fixture["worker_counts"]),
                "composition": "v290-additive/max; host telemetry excluded"},
...
def producer_boundary_validate(runtime: dict[str, Any], frame: dict[str, Any]) -> None:
    ...
    require(frame.get("transport") == {"fault": None, "partial": False,
                "dead": False, "blocked": False, "cleanup_complete": True,
                "survivors": []}, "boundary selftest owner frame")
    _producer_validator_event(runtime, "validate_boundary_frame",
...
def _producer_boundary_mutators() -> dict[str, Any]:
    return {
        ...
        "blocked_send": lambda value: value["transport"].__setitem__("blocked", True),
        "partial_worker": lambda value: value["transport"].__setitem__("partial", True),
        "dead_worker": lambda value: value["transport"].__setitem__("dead", True),
        "surviving_process": lambda value: value["transport"].__setitem__(
            "survivors", [1]),
...
def producer_boundary_mutations(runtime: dict[str, Any], processes: dict[str, Any],
    ...
    workers = 4; meter = Meter(WALL_SECONDS)
    owner = PersistentBoundaryOwner(runtime, meter, workers); owner.start()
    try:
        outcome = owner.run_epoch(dual)
    finally:
        owner.close()
    # Freeze the ordinary owner result once.  All following mutation cases
    # route through producer_boundary_validate and compare this exact value.
    ...
    mutators = _producer_boundary_mutators()
    require(list(contract) == fixture["boundary_mutations"] and
            set(mutators) == set(contract), "producer boundary mutation contract")
    before = sha_bytes(canonical(baseline) + b"\n"); ledger = []
    with tempfile.TemporaryDirectory(prefix="r07-v12a-producer-boundary-") as directory:
        root = Path(directory)
        for ordinal, name in enumerate(fixture["boundary_mutations"]):
            value = json.loads(canonical(baseline).decode("ascii")); mutators[name](value)
            value = seal(value)
            _before, after, reason = _producer_mutation_trace(
...
def producer_validate_positive(runtime: dict[str, Any], value: dict[str, Any],
    ...
    producer_validate_correction(runtime, selected[0], recovery)
...
def producer_positive_mutations(runtime: dict[str, Any], selected_frame: dict[str, Any],
    ...
    before = sha_bytes(canonical(baseline) + b"\n"); ledger = []
    with tempfile.TemporaryDirectory(prefix="r07-v12a-producer-positive-") as directory:
        root = Path(directory)
        for ordinal, name in enumerate(fixture["positive_mutations"]):
            if name == "wrong_coefficient_two_word":
                ...
                value = seal({"schema": SCHEMA + "/coefficient-two-selftest-frame",
                    ...
                    "correction_word": runtime["live"].inverse_word(prov["conjugate_word"]) + [1]})
            ...
            require(before != after, "producer positive identity unchanged:" + name)
            events = runtime.get("_active_validator_events", [])
...
def producer_physical_mutations_v297(runtime: dict[str, Any],
                                     ...
                                     fixture: dict[str, Any],
                                     registry: SourceRegistry) -> list[dict[str, Any]]:
    """Record measured path identities and first ordinary rejection events."""
    recovery = _producer_recovery_public(registry)
    baseline = seal(positive)
    baseline_revalidated = False
    producer_validate_positive(runtime, baseline, recovery)
    baseline_revalidated = True
    ...
    ledger = []
    with tempfile.TemporaryDirectory(prefix="r07-v12a-producer-physical-v297-") as directory:
        root = Path(directory)
        base_raw = canonical(baseline) + b"\n"
        for ordinal, name in enumerate(fixture["physical_mutations"]):
            # Each experiment owns its own baseline path.  In particular a
            # hard-link case cannot leave a raised link count for a later row.
            base = root / f"base-{ordinal:02d}.json"
            with base.open("xb") as stream:
                stream.write(base_raw); stream.flush(); os.fsync(stream.fileno())
            expected = contract[name]; logical_label = name
            # The baseline and mutant use the same logical label; comparison
            # must never be driven by a fixture-name/path-label difference.
            base_identity = _producer_path_identity(
                base, logical_label, sha_bytes(base_raw))
            ...
            runtime["_physical_logical_label"] = logical_label
            runtime["_physical_logical_link_target"] = "physical/baseline"
            runtime["_physical_baseline_sha256"] = sha_bytes(base_raw)
            runtime.pop("_physical_projection", None)
            first_identity = base_identity
            after_identity: dict[str, Any]; before_sha = sha_bytes(base_raw)
            after_sha: str | None = None
            if name in ("symlink_candidate", "hardlink_candidate"):
                try:
                    if name == "symlink_candidate": os.symlink(base, path)
                    else: os.link(base, path)
                except OSError as exc:
                ...
                if not isinstance(after_identity, dict):
                    after_identity = _producer_path_identity(path, logical_label)
                require(_producer_identity_changed(first_identity, after_identity),
                        "physical mutation identity unchanged:" + name)
                try:
                    read_bounded_json(path, MAX_CHECKPOINT_BYTES, None,
                                      trace_runtime=runtime)
                except (InputStop, ProtocolStop) as exc:
                ...
                after_sha = sha_bytes(base_raw)
                after_identity = runtime.get("_physical_projection", after_identity)
                ...
                with substitute.open("xb") as stream:
                    stream.write(base_raw); stream.flush(); os.fsync(stream.fileno())
                first_identity = _producer_path_identity(path, logical_label,
                                                         sha_bytes(base_raw))
                before_sha = sha_bytes(base_raw)
                try:
                    read_bounded_json(path, MAX_CHECKPOINT_BYTES, None,
                ...
                require(isinstance(after_identity, dict),
                        "producer TOCTOU projection missing")
                after_sha = sha_bytes(base_raw)
                require(_producer_identity_changed(first_identity, after_identity),
                        "physical mutation identity unchanged:" + name)
            elif name == "stale_output":
                # The unmutated output owner is absent; the first create is
                # the after identity and the second create is the rejection.
                first_identity = _producer_path_identity(path, logical_label)
                atomic_json(path, baseline, trace_runtime=runtime)
                after_identity = _producer_path_identity(
                    path, logical_label, sha_bytes(canonical(baseline) + b"\n"))
                require(_producer_identity_changed(first_identity, after_identity),
                        "physical mutation identity unchanged:" + name)
                try:
                    atomic_json(path, baseline, trace_runtime=runtime)
                    ...
                    raise ProtocolStop("MUTATION_ACCEPTED:" + name)
                after_sha = sha_bytes(canonical(baseline) + b"\n")
                runtime["_physical_projection"] = {
                    "logical_case_path": logical_label, "owner_kind": "regular",
                    "byte_length": len(canonical(baseline) + b"\n"),
                    "content_sha256": after_sha, "link_count_before": None,
                    "link_count_after": 1, "symlink_or_reparse": False,
                    "logical_link_target": None, "single_open_handle": False,
                    "opened_handle_stable": False,
                    "pathname_matches_opened_handle": True,
                    "substitution_detected": False,
                    "canonical_before_sha256": "UNREADABLE_AT_REGISTERED_STAGE",
                    ...
                    "event_trace_digest": sha_obj(runtime.get(
                        "_active_validator_events", [])),
                    "first_typed_rejection": reason}
                _producer_physical_projection_reason(runtime, reason)
            else:
                value = json.loads(canonical(baseline).decode("ascii"))
                validator: Any = lambda candidate: producer_validate_positive(
                    runtime, candidate, recovery)
                ...
                before_sha, after_sha, reason = _producer_mutation_trace(
                    path, value, validator, baseline_value=baseline,
                    trace_runtime=runtime)
                after_identity = _producer_path_identity(path, logical_label,
                                                          after_sha)
                require(before_sha != after_sha,
                        "physical regular bytes unchanged:" + name)
                projected = runtime.get("_physical_projection")
                require(isinstance(projected, dict),
                        "producer semantic projection missing")
                after_identity = projected
            # Dispose the case owner before publishing its evidence.  In
            # particular, a hard link must not raise the baseline link count
            # for any later case.  Revalidate the unchanged baseline through
            # the ordinary positive validator after the physical cleanup.
            try:
                path.unlink()
            except FileNotFoundError:
                pass
            runtime["_last_owner_disposed"] = True
            # Preserve the mutation trace before the ordinary baseline
            # revalidation emits its own successful validator event.
            ...
            baseline_check = dict(runtime)
            baseline_check.pop("_physical_logical_label", None)
            baseline_check.pop("_physical_logical_link_target", None)
            baseline_check.pop("_physical_baseline_sha256", None)
            baseline_check.pop("_physical_projection", None)
            baseline_check["_active_validator_events"] = []
            case_baseline_revalidated = False
            producer_validate_positive(baseline_check, baseline, recovery)
            require(_producer_path_identity(base, logical_label, sha_bytes(base_raw)) ==
                    base_identity, "producer physical baseline restoration:" + name)
            case_baseline_revalidated = True
            events = mutation_events
            require(events and all(type(event) is dict for event in events),
                    "producer physical validator event trace")
            entered = [event["validator"] for event in events]
            require(reason == expected["first_reason"],
                    "producer physical first reason:" + name + ":" + reason)
            rejection_event = events[-1]
            first_validator = rejection_event["validator"]
            ledger.append({"id": name, "owner_path": expected["owner_path"],
                "identity_kind": ("path" if name in (
                    "symlink_candidate", "hardlink_candidate", "toctou_substitution",
                    "stale_output") else "regular_file"),
                "before_identity": first_identity,
                "after_identity": after_identity,
                "event_trace_digest": sha_obj(events),
                "entered_validators": entered,
                "first_rejection": {"validator": first_validator,
                    "stage": rejection_event["stage"], "narrow_reason": reason},
                ...
                "reached_validator": first_validator,
                "first_reason": reason})
            require(mutation_terminal_count == 1 and mutation_owner_disposed,
                    "producer physical measured terminal/owner:" + name)
            try:
                base.unlink()
            except FileNotFoundError:
                pass
            require(not base.exists(), "producer physical baseline disposal:" + name)
    return ledger


def phase_gate_selftest(runtime: dict[str, Any],
                        fixture: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    """Exercise the real phase owner and its physical transition frames."""
    ...
    baseline = seal({"schema": SCHEMA + "/phase-owner-frame", "phase": "preselection",
        "light_input_sha256": light_hash, "heavy_input_sha256": None,
        "owner_preselection_sha256": owner_pre_sha,
        "heavy_owner_sha256": None,
        "heavy_complete": False, "correction_started": False,
    ...
    def validate(value: dict[str, Any]) -> None:
        _producer_validator_event(runtime, "phase_owner.validate",
                                  "preselection", "phase-owner-frame")
        validate_seal(value)
        require(set(value) == set(baseline) and
                value.get("schema") == SCHEMA + "/phase-owner-frame" and
                value.get("phase") == "preselection" and
                value.get("light_input_sha256") == light_hash and
                value.get("heavy_input_sha256") is None and
                value.get("owner_preselection_sha256") == owner_pre_sha and
                value.get("heavy_owner_sha256") is None and
                value.get("heavy_complete") is False and
                value.get("correction_started") is False and
                value.get("correction_progress") == baseline[
                    "correction_progress"] and
                value.get("zero_is_negative") is False and
                value.get("claims") == FALSE_CLAIMS,
                "phase gate")

    actions = {
        "heavy_call_before_heavy_digest": lambda value: value.__setitem__(
    ...
    baseline_revalidated = True
    rows = []
    with tempfile.TemporaryDirectory(prefix="r07-v12a-producer-phase-") as directory:
        root = Path(directory)
        for ordinal, name in enumerate(fixture["phase_mutations"]):
            value = json.loads(canonical(baseline).decode("ascii")); actions[name](value)
            value = seal(value); expected = contract[name]
            before, after, reason = _producer_mutation_trace(
                root / f"phase-{ordinal:02d}.json", value, validate,
                baseline_value=baseline, trace_runtime=runtime)
            require(reason == expected["first_reason"],
            ...
            rows.append({"id": name, "owner_path": expected["owner_path"],
                ...
                "first_reason": reason})
        gates = {
            "light_resource_checkpoint": {"schema": SCHEMA + "/phase-owner-frame",
                "phase": "light_resource_checkpoint", "light_input_sha256": light_hash,
                "heavy_input_sha256": None,
                "owner_preselection_sha256": owner_pre_sha,
                "heavy_owner_sha256": None, "heavy_complete": False,
                "runtime_light_public_sha256": sha_obj(runtime["light_public"])},
            "heavy_transition": {"schema": SCHEMA + "/phase-owner-frame",
                "phase": "heavy_transition", "light_input_sha256": light_hash,
                "heavy_input_sha256": heavy_hash,
                "owner_preselection_sha256": owner_pre_sha,
                "heavy_owner_sha256": heavy_owner_sha,
                "heavy_complete": True}}
        ...
        def validate_positive_gate(name: str, value: dict[str, Any]) -> None:
            _producer_validator_event(runtime, "phase_owner." + name,
                                      "light_checkpoint" if name ==
                                      "light_resource_checkpoint" else
                                      "heavy_transition", "phase-owner-frame")
            if name == "light_resource_checkpoint":
                require(value["schema"] == SCHEMA + "/phase-owner-frame" and
                        value["phase"] == name and
                        value["light_input_sha256"] == light_hash and
                        value["heavy_input_sha256"] is None and
                        value["owner_preselection_sha256"] == owner_pre_sha and
                        value["heavy_owner_sha256"] is None and
                        value["heavy_complete"] is False and
                        value["runtime_light_public_sha256"] ==
                            sha_obj(runtime["light_public"]),
                        "phase owner light checkpoint")
            else:
                require(value["schema"] == SCHEMA + "/phase-owner-frame" and
                        value["phase"] == name and
                        value["light_input_sha256"] == light_hash and
                        value["heavy_input_sha256"] == heavy_hash and
                        value["owner_preselection_sha256"] == owner_pre_sha and
                        value["heavy_owner_sha256"] == heavy_owner_sha and
                        value["heavy_complete"] is True,
                        "phase owner heavy transition")
        positive_gates = []
        for name in fixture["phase_positive_gates"]:
            expected = gate_contract[name]; value = gates[name]
            runtime["_active_validator_events"] = []
            ordinary_pass = False
            validate_positive_gate(name, value)
            ordinary_pass = True
            digest = sha_bytes(canonical(value) + b"\n")
            events = runtime.get("_active_validator_events", [])
            require(events and value == gates[name],
...
def run_real_owner_selftest(runtime: dict[str, Any], old_value: dict[str, Any],
    ...
    require(fixture.get("schema") ==
            "d972-r07-history-free-positive-fast-resume/selftest-input/v12a" and
    ...
    selected_frame = _producer_correction_selftest_frame(runtime, registry)
    ...
    positive_baseline = {"schema": SCHEMA + "/positive-selftest-frame",
        "selected": [selected_frame["record"]], "coefficient": 1,
        "target": selected_frame["record"]["sparse_row"],
        "boundary_preimage": [], "producer_sparse_equality": True,
        "heavy_input_sha256": runtime["heavy_input_sha256"],
    ...
    physical_ledger = producer_physical_mutations_v297(
        runtime, positive_baseline, fixture, registry)
    boundary_ledger = producer_boundary_mutations(runtime, processes, fixture)
    phase_result = phase_gate_selftest(runtime, fixture)
    ...
    return {"fixture_sha256": SOURCE_PINS["fixture"][2],
            ...
            "triangular_owner_frame": triangular_owner_frame,
            ...
            "physical_mutation_ledger": physical_ledger,
...
def validate_p0_manifest(value: dict[str, Any]) -> None:
    ...
    require(value.get("mode") == "SELFTEST_BOOTSTRAP" and
            ...
            value.get("requires_v12b_physical_pin") is True,
            "v12a P0 bootstrap envelope")
    require(value.get("manifest_path") ==
            "ci/in/d972_r07_history_free_positive_fast_resume_selftest_bootstrap_v12a.manifest.v1.json",
            "v12a P0 path")
    prospective = value.get("prospective_outputs")
    require(type(prospective) is dict and
            prospective.get("R", {}).get("sha256") ==
            "TO_BE_GENERATED_BY_AUDITED_V12A_SELFTEST" and
            prospective.get("V", {}).get("sha256") ==
            "TO_BE_GENERATED_BY_AUDITED_V12A_SELFTEST",
            "v12a prospective identity gate")
    sources = value.get("sources")
    require(type(sources) is dict and set(sources) == {
            "producer_v12a", "checker_v12a", "fixture_v12a"} and
            all(type(sources[key]) is dict for key in sources),
            "v12a P0 source graph without driver cycle")
    expected_paths = {
        "producer_v12a": "search/d972_r07_history_free_positive_fast_resume_v12a.py",
        "checker_v12a": "crosscheck/check_d972_r07_history_free_positive_fast_resume_v12a.py",
        "fixture_v12a": FIXTURE_PATH,
    }
    ...
    require(type(frozen) is dict and set(frozen) == set(expected_frozen),
            "v12a P0 frozen authority roster")
    for key, (path, size, digest) in expected_frozen.items():
        ...
        require(type(row) is dict and row.get("path") == path and
                ...
                "v12a P0 frozen authority row:" + key)
...
def validate_p0_source_graph(value: dict[str, Any], meter: Meter) -> None:
    """Bind every P0 source row to the final regular file before heavy load."""
    for key, row in value["sources"].items():
        path = ROOT / str(row["path"])
        raw, physical = read_physical_once(path, int(row["bytes"]),
                                            str(row["sha256"]), meter)
        require(physical["sha256"] == row["sha256"] and
                len(raw) == row["bytes"], "v12a P0 physical source:" + key)
    for key, row in value["frozen_authorities"].items():
        ...
        require(physical["sha256"] == row["sha256"] and
                len(raw) == row["bytes"],
                "v12a P0 physical frozen authority:" + key)
...
def validate_owned_paths(output: Path, source: Path) -> None:
    ...
    require(output.absolute() == expected_output,
            "v12a receipt path is not preregistered")
    require(source.absolute() == expected_source,
            "v12a raw source path is not preregistered")
    output_dir = ROOT / "ci/out"
    require(output_dir.is_dir() and not output_dir.is_symlink(),
            "v12a output directory identity")
    try:
        source_stat = os.lstat(source)
    except OSError as exc:
        raise InputStop("v12a raw source path unavailable") from exc
    require(stat.S_ISREG(source_stat.st_mode) and source_stat.st_nlink == 1,
            "v12a raw source must be a unique regular owner")
    if os.path.lexists(str(output)):
        ...
        require(stat.S_ISREG(output_stat.st_mode) and
                ...
                "v12a receipt output identity")
    for relative in OUTPUT_SIBLINGS:
        target = ROOT / relative
        require(not os.path.lexists(str(target)), "stale v12a transport:" + relative)


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description="v12a SELFTEST_BOOTSTRAP producer")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    require(manifest_path.resolve() == P0_PATH.resolve(),
            "v12a P0 manifest required")
    validate_owned_paths(output, source_path)
    require(args.seconds == WALL_SECONDS,
            "v12a producer envelope must be 10800 seconds")
    meter = Meter(args.seconds)
    p0, p0_physical = read_bounded_json(P0_PATH, 16 * 1024 * 1024, meter)
    validate_p0_manifest(p0)
    validate_p0_source_graph(p0, meter)
    ...
    try:
        ...
        raise InputStop("v12a_triangular_gate") from exc
    meter.check("v12a_light_basis_complete")
    # All W2/W4 and fault owners are forked while the light image is small;
    # no boundary child is created after the heavy ten-coordinate stores exist.
    processes = process_selftest(runtime, reducer, registry.json("fixture"))
    ...
    receipt_body = {
        ...
        "terminal": SELFTEST_TERMINAL,
        "mode": "SELFTEST_BOOTSTRAP",
        "candidate_only": True,
        "production_authorized": False,
        "requires_v12b_physical_pin": True,
        "execution": "SELFTEST_BOOTSTRAP_COMPLETE_CANDIDATE",
        "p0": {"path": str(P0_PATH.relative_to(ROOT)).replace("\\", "/"),
                "semantic_schema": P0_SCHEMA,
                "self_digest_sha256": p0["self_digest_sha256"],
                "physical_sha256": p0_physical["sha256"]},
        "source": source_public,
        "source_snapshots": registry.public(),
        "p0_sources": p0["sources"],
        "frozen_authorities": p0["frozen_authorities"],
        "triangular_certificate": triangular,
        "triangular_owner_frame": selftest["triangular_owner_frame"],
        "light_input_sha256": runtime["light_input_sha256"],
        "heavy_input_sha256": runtime["heavy_input_sha256"],
        "heavy_public": runtime["heavy_public"],
