#!/usr/bin/env python3
"""Task367 producer: authenticated, finite rows 1--7 authority trace.
...
# Filled by the immutable v4 fixture after it is sealed.
FIXTURE_BYTES = 8_457
...
def canon_meter(value: Any, meter: Meter, bound: int, retained_owner: str) -> bytes:
    if type(bound) is not int or bound < 0 or type(retained_owner) is not str or not retained_owner: raise InputStop("producer:canonical:bound_or_owner")
    meter.reserve("canonical_bytes", bound); reserved_before = meter.reserved["canonical_bytes"]; peak_active = False; retained_active = False; completed = False
    try:
        meter.reserve_peak(bound); peak_active = True; raw = canonical(value)
        if len(raw) > bound: raise InputStop("producer:canonical:overflow")
        meter.charge("canonical_bytes", len(raw))
        meter.release_peak(bound); peak_active = False; meter.retain_peak(retained_owner, len(raw)); retained_active = True
        completed = True; return raw
    finally:
        residual = meter.reserved["canonical_bytes"] - (reserved_before - bound)
        if residual > 0: meter.release("canonical_bytes", residual)
        if peak_active: meter.release_peak(bound)
        if retained_active and not completed: meter.release_retained(retained_owner)
...
def digest_object(value: Any, meter: Meter, bound: int) -> str:
    owner = "transient:digest:" + str(id(value)); raw = canon_meter(value, meter, bound, owner)
    try: return digest_bytes(raw)
    finally: del raw; meter.release_retained(owner)
    ...
    def revalidate_all(self, expected: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        for key, fd in self.handles.items():
            ...
            try:
                ...
                self.meter.revalidated_bytes += read
...
def parse_object(raw: bytes | bytearray, label: str, meter: Meter, exact: bool, retained_owner: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if exact:
            canonical_owner = "transient:parse:" + label; canonical_raw = canon_meter(value, meter, size, canonical_owner)
            try:
                if canonical_raw != raw: raise InputStop("producer:json:noncanonical:" + label)
            finally: del canonical_raw; meter.release_retained(canonical_owner)
        return value
...
def _stream_rows(rows: list[Any], chunks: list[Any], meter: Meter) -> tuple[str, bool]:
    ...
    for pos, row in enumerate(rows, 1):
        _typed_row(row, pos); owner = "transient:row:" + str(pos); piece = canon_meter(row, meter, 1_000_000, owner)
        try:
            if pos != 1: whole.update(b",")
            whole.update(piece)
            if in_chunk != 0: chunk_hash.update(b",")
            chunk_hash.update(piece); in_chunk += 1
            if row.get("word") == []: has_empty_word = True
        finally:
            del piece; meter.release_retained(owner)
...
def load_fixture(store: PhysicalStore, meter: Meter, argument: str) -> dict[str, Any]:
    ...
    if type(seal) is not str or seal != FIXTURE_SELF or seal != digest_object(body, meter, 1_000_000) or fixture.get("schema") != SCHEMA + "/authority-fixture/v4": raise InputStop("producer:fixture:self_seal")
...
def execute(fixture: dict[str, Any], meter: Meter, store: PhysicalStore) -> dict[str, Any]:
    ...
    if any(meter.counts[key] != value for key, value in expected_counts.items()) or meter.revalidated_bytes != INTENDED_REVALIDATED_BYTES or any(meter.reserved.values()) or meter.live_peak != BASELINE_PEAK_BYTES or meter.peak_seen != LARGEST_INTENDED_PEAK: raise InputStop("producer:meter:final_account")
...
def _output_seal(value: dict[str, Any], meter: Meter, owner: str) -> tuple[bytes, str]:
    body = dict(value); body.pop("self_digest_sha256", None); body_raw = canon_meter(body, meter, 35_000_000, owner + ":body"); seal = digest_bytes(body_raw); del body_raw; meter.release_retained(owner + ":body"); body["self_digest_sha256"] = seal; raw = canon_meter(body, meter, 35_000_000, owner); return raw, digest_bytes(raw)
...
def write_output(path: Path, value: dict[str, Any], meter: Meter) -> None:
    ...
    try:
        parent_fd, leaf = _nofollow_parent(target); meter.charge("opens", 1); parent_open_reserved = False
    except Exception:
        if parent_open_reserved: meter.release("opens", 1)
        raise
    stage_name: str | None = None; stage_created: str | None = None; stage_fd = -1; file_fd = -1; published = False; target_was_absent = False; raw: bytes | None = None; view: memoryview | None = None; owner = "output:final_raw"
    try:
        parent_before = os.fstat(parent_fd); parent_path_before = _nofollow_lstat(target.parent); parent_identity = lambda st: (st.st_dev, st.st_ino, st.st_mode, st.st_nlink)
        if not stat.S_ISDIR(parent_before.st_mode) or parent_identity(parent_before) != parent_identity(parent_path_before): raise InputStop("producer:output:parent_identity")
        ...
        for _ in range(64):
            candidate = next(tempfile._get_candidate_names())
            try: os.mkdir(candidate, 0o700, dir_fd=parent_fd); stage_name = candidate; stage_created = candidate; break
            except FileExistsError: continue
        ...
        try:
            ...
            raise
        meter.reserve("opens", 1); file_open_reserved = True
        try:
            file_fd = os.open("staged.json", os.O_RDWR | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o600, dir_fd=stage_fd); meter.charge("opens", 1); file_open_reserved = False
        except Exception:
            if file_open_reserved: meter.release("opens", 1)
            raise
        view = memoryview(raw); offset = 0
        while offset < len(raw):
            written = os.write(file_fd, view[offset:offset + 1_048_576])
            if written <= 0: raise InputStop("producer:output:short_write")
            offset += written
        os.fsync(file_fd); staged = os.fstat(file_fd)
        if not stat.S_ISREG(staged.st_mode) or staged.st_nlink != 1 or staged.st_size != len(raw): raise InputStop("producer:output:stage_identity")
        if os.lseek(file_fd, 0, os.SEEK_SET) != 0: raise InputStop("producer:output:rewind")
        h = hashlib.sha256(); remaining = len(raw)
        while remaining:
            part = os.read(file_fd, min(1_048_576, remaining))
            if not part: raise InputStop("producer:output:short_read")
            h.update(part); remaining -= len(part)
        if h.hexdigest() != raw_sha or not _stats_equal(staged, os.fstat(file_fd)): raise InputStop("producer:output:stage_sha256")
        ...
        os.fsync(stage_fd); opened_final = os.fstat(file_fd); pathname_final = os.stat(leaf, dir_fd=parent_fd, follow_symlinks=False)
        if not stat.S_ISREG(opened_final.st_mode) or opened_final.st_nlink != 1 or opened_final.st_size != len(raw) or not _stats_equal(opened_final, pathname_final): raise InputStop("producer:output:final_identity")
        os.close(file_fd); file_fd = -1; os.close(stage_fd); stage_fd = -1; os.rmdir(stage_name, dir_fd=parent_fd)
        try: os.stat(stage_name, dir_fd=parent_fd, follow_symlinks=False); raise InputStop("producer:output:stage_present")
        except FileNotFoundError: pass
        stage_name = None; os.fsync(parent_fd)
        parent_after = os.fstat(parent_fd); parent_path_after = _nofollow_lstat(target.parent)
        if parent_identity(parent_before) != parent_identity(parent_after) or parent_identity(parent_after) != parent_identity(parent_path_after): raise InputStop("producer:output:parent_identity")
    except Exception as exc:
        cleanup_errors: list[BaseException] = []
        if published:
            try:
        ...
        if stage_fd >= 0:
            ...
            try: os.stat("staged.json", dir_fd=stage_fd, follow_symlinks=False); cleanup_errors.append(InputStop("producer:output:staging_link_present"))
            except FileNotFoundError: pass
            try: os.fsync(stage_fd)
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if file_fd >= 0:
            try: os.close(file_fd); file_fd = -1
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if stage_fd >= 0:
            try: os.close(stage_fd); stage_fd = -1
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if stage_name is not None:
            try: os.rmdir(stage_name, dir_fd=parent_fd); stage_name = None
            except FileNotFoundError: stage_name = None
            except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        try: os.fsync(parent_fd)
        except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if target_was_absent and not published:
            try: os.stat(leaf, dir_fd=parent_fd, follow_symlinks=False); cleanup_errors.append(InputStop("producer:output:rollback_present"))
            except FileNotFoundError: pass
        if stage_created is not None:
            try: os.stat(stage_created, dir_fd=parent_fd, follow_symlinks=False); cleanup_errors.append(InputStop("producer:output:stage_present"))
            except FileNotFoundError: pass
        try:
            path_now = _nofollow_lstat(target.parent)
            if parent_identity(os.fstat(parent_fd)) != parent_identity(path_now): cleanup_errors.append(InputStop("producer:output:parent_substitution"))
        except BaseException as cleanup_exc: cleanup_errors.append(cleanup_exc)
        if cleanup_errors: raise InputStop("producer:output:rollback_failed") from cleanup_errors[0]
        if isinstance(exc, InputStop): raise
