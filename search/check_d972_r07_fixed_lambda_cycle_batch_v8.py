#!/usr/bin/env python3
"""Task1172 draft: independent fixed-lambda, five-layer parent2090 checker.
...
PARENT_ROLES = (*V7_PARENT_ROLES, "batch-parent-v7")
PRODUCER_FILE = "search/d972_r07_fixed_lambda_cycle_batch_v8.py"
CHECKER_FILE = "search/check_d972_r07_fixed_lambda_cycle_batch_v8.py"
CHECKER_WORKFLOW = ".github/workflows/d972-r07-fixed-lambda-cycle-batch-v8.yml"
...
# Current P8 stays guard-closed below until root supplies its actual final opaque pin.
...
CURRENT_PRODUCER_REGISTRATION: dict[str, Any] | None = {"bytes":655812,"file":"search/d972_r07_fixed_lambda_cycle_batch_v8.py","sha256":"97f2523d29070bafb7e4295bfca7a975dad1be94daa17eb341f7a80e0bfd4e13"}
...
# Independent C sums: the public P/driver registry is an external static cross-check only.
...
class SavedParentJsonBytes:
    ...
    def __init__(self, ordinal: int):
        ...
        self.rows = {(role, kind): {key: 0 for key in self.metrics}
            for role in (BATCH_PARENT_ROLE, NEXT_BATCH_ROLE, THIRD_BATCH_ROLE, FOURTH_BATCH_ROLE, FIFTH_BATCH_ROLE)
    ...
    def emit(self, status: str) -> None:
        ...
        emit_auxiliary_diagnostic({"schema": "d972.r07.saved-parent-json-parse-bytes.v1", "side": "C",
            ...
            "measurement_scope": "same-PinnedTree.json-read-to-json_value;five-saved-roles;candidate6/reduction-and-physical-literal;successful-canonical-parse-not-later-seal-or-schema;unique-first-successful-role-path",
...
def check_acceptance_header(value: Any) -> None:
    require(type(value) is dict and len(value) == CURRENT_ACCEPTANCE_KEY_COUNT and set(value) == {"schema", "parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "code", "runtime", "registration"} and
            value["schema"] == SCHEMA + ".acceptance", "acceptance_exact_eleven_plain_keys")
...
class AcceptedInputs:
    ...
    def __init__(self, args: argparse.Namespace, parse_bytes: SavedParentJsonBytes):
        ...
        check_fifth_parent_roles(value["parents"])
        require(type(FIFTH_BATCH_INVENTORY_REGISTRATION) is dict and type(CURRENT_PRODUCER_REGISTRATION) is dict,
                "v8_formal_inventory_and_final_source_binding_pending")
        ...
        same_json(code["producer"], CURRENT_PRODUCER_REGISTRATION, "v8_registered_final_P8_opaque_full_pin")
...
def authenticate_fifth_batch_parent_metadata(inputs: AcceptedInputs) -> dict[str, Any]:
    ...
    check_fifth_native_parent_projection(inputs.portable["parents"], layout["parents"])
...
def root_records(inputs: AcceptedInputs, anchor: ThinAnchor, old: dict[str, Any], bundle: C.FixedBundle) -> RootRecords:
    ...
    layout = document("parent-layout", {"portable_acceptance_sha256": inputs.portable_sha256,
        **{key: copy.deepcopy(inputs.portable[key]) for key in ("parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "code", "runtime", "registration")}})
    ...
    emit_parent_timing("direct-pairing", FIFTH_BATCH_ROLE, 5, pairing_started, rows=pairing["rows"], records=len(anchor.parents))
    ...
    start = document("start", {"owner_sha256": sha(canonical(owner)), "source_sha256": sha(canonical(source)),
        ...
        "previous_parent_target_derivations": len(inputs.fourth_intermediate_parents),
    ...
    require(len(layout) == CURRENT_LAYOUT_KEY_COUNT and len(start) == CURRENT_START_KEY_COUNT and
            len(intake) == CURRENT_INTAKE_KEY_COUNT, "c8_current_root_key_counts_from_independent_sums")
...
def batch_observation(inputs: AcceptedInputs, records: RootRecords, files: CandidateFiles,
    ...
    if not admitted:
        sequence = -1
    batch = inputs.value["batch_anchor_v7"]
    parents = {item["role"]: item["artifact"] for item in inputs.portable["parents"]}
    accepted = inputs.fifth_batch_parent["records"]
    old = {"parent_role": "batch-parent-v6", "parent_artifact": copy.deepcopy(parents["batch-parent-v6"]),
    ...
    current = {"parent_role": "batch-parent-v7", "parent_artifact": copy.deepcopy(parents["batch-parent-v7"]),
    ...
    if admitted:
        ...
        require(pairing["rows"] == FIFTH_BATCH_RANK and pairing["lambda_pivots"] == 0 and
    ...
    check_fifth_unobserved_oracle(current, sequence)
...
def compare_producer_result(files: CandidateFiles, inputs: AcceptedInputs, records: RootRecords,
    ...
    check_current_anchor_batch_counts(actual)
...
def check_actual(args: argparse.Namespace, report: dict[str, Any], parse_bytes: SavedParentJsonBytes) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        restore_started = time.monotonic()
        anchor = stack.enter_context(promote_fifth_batch_parent(inputs, fourth_intermediate, old))
        emit_parent_timing("state-restore", FIFTH_BATCH_ROLE, 5, restore_started, rows=anchor.rank, records=len(anchor.parents))
        ...
        if "selection/start.json" in files.expected:
            check_current_anchor_batch_counts(records.selection_start)
        ...
        if "start.json" in files.expected and "parent-intake.json" in files.expected:
            check_current_anchor_batch_counts(report)
        ...
        if "final/manifest.json" in files.before:
            ...
            check_current_anchor_batch_counts(final.manifest)
            if final.head is not None:
                check_current_anchor_batch_counts(final.head)
...
def k128_registration_canary(root: Path) -> dict[str, Any]:
    ...
    acceptance = {"schema": SCHEMA + ".acceptance", "parents": [], "anchor": {}, "batch_anchor": {}, "next_batch_anchor": {}, "batch_anchor_v5": {}, "batch_anchor_v6": {}, "batch_anchor_v7": {}, "code": {},
...
def selftest(root: Path) -> dict[str, Any]:
    ...
    return document("selftest", {"status": "PASS", "tests": [first, second, third, fourth, fifth, sixth, seventh],
        "fixture_scope": "plus-synthetic-fifth-layer-metadata-fourteen-exact-purpose-rejections-native57-header-only;" + "plus-synthetic-fourth-layer-metadata-ten-exact-purpose-rejections-native49-header-only;" + "plus-synthetic-third-layer-metadata-with-exact-cycle_batch-prefix;" + "new-k128-registration-and-portable-metadata;synthetic-full-54433-rosters-and-physical-dependent-continuation;real-reduction-phase-and-candidate-publication-reader;synthetic-parent1578-header-projection-target-hash;synthetic-parent1706-two-layer-metadata;inherited-dependent-negative-stops-at-expected-file-size-hash-before-semantic-outcome-comparison;all-fixtures-retained;no-accepted-parent-or-earlier-five-E-phase-arithmetic",
        "production_interfaces_used": ["check_fifth_native_parent_projection", "check_fifth_row_namespace",
    ...
    try:
        if args.selftest:
            ...
            require(all(getattr(args, role.replace("-", "_") + "_root") is not None for role in PARENT_ROLES if not role.startswith("block-")) and
                    ...
                    "all_twenty_roots_and_exact_acceptance")
