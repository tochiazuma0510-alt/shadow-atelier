#!/usr/bin/env python3
"""Task977: immutable-snapshot repair of the independent Task972 checker.
...
class PhysicalState:
    ...
    def derived(self) -> dict[str, Any] | None:
        # Export owned metadata: later append or nested edits cannot change a sealed snapshot.
        if self.kind != "Separator":
            return None
        return {"mode": "derived", "value": 1, "original_rho2_directly_read": False,
            "original_rho2_packed_sha256": self.original_rho2, "accepted_target_derivation_parents": copy.deepcopy(self.parents),
            "identity_convention": {
    ...
    def summary(self) -> dict[str, Any]:
        return {"kind": self.kind, "rank": self.rank, "generation": self.generation, "state_head": self.head,
            ...
            "accepted_target_derivation_parents": copy.deepcopy(self.parents), "lambda_rho2": self.derived(), "direct_pairing": copy.deepcopy(self.direct_pairing)}
...
def check_actual(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        return document("checker-result", {"status": "PASS", "terminal": result["terminal"], "kind": state.kind,
            ...
            "lambda_rho2": state.derived(), "direct_pairing": copy.deepcopy(state.direct_pairing),
...
def snapshot_isolation_selftest() -> dict[str, Any]:
    """Exercise state exports and actual root/snapshot/HEAD serializers only."""
    boundary("snapshot_isolation_fixture")
    target = np.zeros(PHYSICAL, dtype=np.uint8); target[4] = 1
    base = np.zeros(PHYSICAL, dtype=np.uint8); base[0] = 1
    initial = {"kind": "Separator", "rank": 1, "generation": 23, "state_head": "1" * 64,
        "target_remainder_sha256": sha(pack(target)), "lambda_sha256": sha(pack(target)),
        "accepted_target_derivation_parents": [{"role": "fixture-base", "manifest_sha256": "2" * 64,
            "result_sha256": "3" * 64, "target_sha256": "4" * 64}],
        "lambda_rho2": {"original_rho2_packed_sha256": "5" * 64},
        "direct_pairing": {"rows": 1, "row_pairings_sha256": sha(b"\0"), "lambda_pivots": 0,
            "lambda_parent_remainder": 1, "lambda_new_remainder": 1}}
    # Only metadata shapes are fixtures; no accepted-parent admission is claimed.
    accepted_e = {"entries": {"output/HEAD": copy.deepcopy(initial),
        "output/result.json": {"terminal": "PIVOT_CANDIDATE"}},
        "hashes": {"instruction.json": "6" * 64},
        "objects": {"start.json": {"accepted_refinement_layout": {"fixture": "metadata-only"}}}}
    oracle = {"entries": {"output/start.json": copy.deepcopy(initial),
        "output/result.json": {"terminal": "VIOLATION_CANDIDATE"},
        "output/owner.json": {key: {"fixture": key} for key in
            ("p1_parent", "task554_parent", "task712_parent", "task712_manifest_sha256",
             "word_dictionary_sha256", "relator_dictionary_sha256")}}, "witness_sha256": "7" * 64}
    source = document("source", {"fixture": "immutable-metadata-boundary"})
    fixed_sha = "8" * 64
    with tempfile.TemporaryDirectory(prefix="r07-cegar-snapshot-isolation-") as temporary:
        root = Path(temporary)
        (root / "state").mkdir(); (root / "state/physical.bin").write_bytes(pack(base))
        accepted = {"pivots": [{"offer": 0, "lead": 0, "physical_offset": 0,
            "coefficient_offset": None, "rolling_sha256": "1" * 64}], "saved_rows": [],
            "start_target": target, "start_lambda": target.copy()}
        with PhysicalState(argparse.Namespace(state_root=root), accepted, initial, base_count=1) as state:
            start, owner = root_start_owner(state, accepted_e, oracle)
            start_sha = sha(canonical(start))
            snapshot0 = snapshot_record(state, sha(canonical(owner)), sha(canonical(source)), start_sha, fixed_sha)
            exports = {"start": start, "snapshot0": snapshot0, "derived": state.derived(),
                "summary": state.summary(), "measure": state.measure()}
            before = {name: canonical(value) for name, value in exports.items()}
            # Deliberately retain the old mutable escape as an adverse control.
            control = document("snapshot", {"accepted_target_derivation_parents": state.parents,
                "lambda_rho2": {"accepted_target_derivation_parents": state.parents},
                "direct_pairing": state.direct_pairing})
            control_before = canonical(control)
            control_seal = control["sha256"]
            raw = np.zeros(PHYSICAL, dtype=np.uint8); raw[2] = raw[4] = 1
            result = E.one_physical_row(raw, state.target, state.functional, state.pivots, state.row, state.generation, 1)
            instruction = {"schema": E.SCHEMA + ".instruction", "predecessor": state.head,
                "offer": state.generation, "rank": state.rank + 1, "generation": state.generation + 1,
                "physical_offset": state.rank * ROW_BYTES, "lead": result["lead"],
                "physical_sha256": sha(pack(result["normalized"])),
                "target_remainder_sha256": sha(pack(result["target"])), "target_scalar": result["target_scalar"]}
            instruction["rolling_sha256"] = sha(bytes.fromhex(state.head) + canonical(instruction))
            record = {"parent_state_head": state.head, "state_head": instruction["rolling_sha256"],
                "rank_after": state.rank + 1, "generation_after": state.generation + 1,
                "kind": result["kind"], "separator": result["separator"],
                "target": {"parent_remainder_sha256": sha(pack(state.target)),
                    "remainder_sha256": sha(pack(result["target"])), "scalar": result["target_scalar"]}}
            payloads = {"physical-normalized.bin": pack(result["normalized"]),
                "target-remainder.bin": pack(result["target"]), "lambda.bin": pack(result["lambda"])}
            state.attach(payloads, instruction, record, "9" * 64, "loop-e-000001", new_step=True)
            require(state.completed_steps == 1 and state.rank == 2 and state.generation == 24 and
                state.head == instruction["rolling_sha256"] and result["target_scalar"] == 0,
                "isolation_actual_state_advanced")
            require(all(canonical(exports[name]) == raw_before for name, raw_before in before.items()),
                "isolation_all_exports_survive_actual_attach")
            require(canonical(control) != control_before and control["sha256"] == control_seal and
                sha(canonical(unsigned(control))) != control_seal, "isolation_legacy_alias_control_detected")
            rejected(lambda: sealed_bytes(canonical(control), SCHEMA + ".snapshot"), "isolation_stale_control_seal")
            snapshot1 = snapshot_record(state, sha(canonical(owner)), sha(canonical(source)), start_sha, fixed_sha)
            snapshot1_before = canonical(snapshot1)
            # This is the production ordering: attach, then hash the prior snapshot receipt.
            receipt0 = {"snapshot_sha256": sha(canonical(snapshot0))}
            require(receipt0["snapshot_sha256"] == sha(before["snapshot0"]) and
                len(snapshot0["accepted_target_derivation_parents"]) == 1 and
                len(snapshot1["accepted_target_derivation_parents"]) == 2 and snapshot1["step"] == 1,
                "isolation_post_attach_receipt_and_current_snapshot")
            current_measure = state.measure()
            current_measure_before = canonical(current_measure)
            state.parents[0]["manifest_sha256"] = "a" * 64
            state.parents[-1]["result_sha256"] = "b" * 64
            state.direct_pairing["rows"] = 999
            require(all(canonical(exports[name]) == raw_before for name, raw_before in before.items()) and
                canonical(snapshot1) == snapshot1_before and canonical(current_measure) == current_measure_before,
                "isolation_deep_parent_and_pairing_mutations")
            state.measure()
            current = state.summary()
            require(current["accepted_target_derivation_parents"][0]["manifest_sha256"] == "a" * 64 and
                current["lambda_rho2"]["accepted_target_derivation_parents"][-1]["result_sha256"] == "b" * 64,
                "isolation_current_exports_reflect_current_state")
            # Mutating an exported record must also leave the owner state unchanged.
            current["accepted_target_derivation_parents"][0]["manifest_sha256"] = "c" * 64
            current["lambda_rho2"]["accepted_target_derivation_parents"][-1]["result_sha256"] = "d" * 64
            current["direct_pairing"]["rows"] = 998
            require(state.parents[0]["manifest_sha256"] == "a" * 64 and
                state.parents[-1]["result_sha256"] == "b" * 64 and state.direct_pairing["rows"] == 2,
                "isolation_export_mutations_do_not_reenter_state")
            for name, schema in (("start", "start"), ("snapshot0", "snapshot")):
                sealed_bytes(canonical(exports[name]), SCHEMA + "." + schema)
            sealed_bytes(canonical(snapshot1), SCHEMA + ".snapshot")
            head = head_record(state, start, owner, source, fixed_sha, "9" * 64, None, None)
            require(head["start_sha256"] == start_sha and head["completed_steps"] == 1 and
                head["state_head"] == state.head, "isolation_actual_final_HEAD_keeps_original_start")
            (root / "HEAD").write_bytes(canonical(head))
            same(sealed_bytes((root / "HEAD").read_bytes(), SCHEMA + ".head"), head, "isolation_HEAD_roundtrip")
    boundary("snapshot_isolation_eof")
    return document("snapshot-isolation-selftest", {"status": "PASS", "tests": [
        {"name": "actual-attach-root-start-and-post-attach-snapshot-receipt", "status": "PASS"},
        {"name": "deep-parent-and-pairing-mutation-isolation-both-directions", "status": "PASS"},
        {"name": "legacy-alias-control-and-unchanged-final-HEAD-gate", "status": "PASS"}],
        "actual_physical_state": True, "actual_root_snapshot_HEAD_serializers": True,
        "legacy_alias_control_detected": True, "old_success_suites": 0,
        "accepted_parent_arithmetic_replayed": False, "fixture_scope": "one-base-row-and-one-actual-attach",
...
def main() -> int:
    ...
    modes.add_argument("--snapshot-isolation-selftest", action="store_true")
    ...
    exit_code = 0
    try:
        if args.snapshot_isolation_selftest:
            result = snapshot_isolation_selftest()
        elif args.selftest:
            result = selftest()
        else:
