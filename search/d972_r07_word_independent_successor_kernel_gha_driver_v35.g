#############################################################################
## A4 v35 RESUME driver for the v19 producer and v25 checker.
## ASCII only.  The exact legacy checkpoint is installed only as the base;
## the delta HEAD is deliberately absent until the first completed row seals it.
#############################################################################
if not IsBound(D386Mode) then Error("task410 MODE required"); fi;
if D386Mode<>"RESUME" then
 Error("task410 v35 requires RESUME");
fi;
...
D410Raw:=D410ReplaceAll(D410Raw,"v30","v35");;
D410Pairs:=[
 ...
 ["D345PCheckpoint:=\\\\\\\"ci/out/d972_r07_word_independent_successor_kernel_v35.producer.checkpoint.json\\\\\\\";;",
  "D345PCheckpoint:=\\\\\\\"ci/out/d972_r07_word_independent_successor_kernel_v35.producer.head.checkpoint.json\\\\\\\";;"],
 ["D386InstallSeed(\"ci/out/d972_r07_word_independent_successor_kernel_v35.producer.checkpoint.json\"",
  "D386InstallSeed(\"ci/out/d972_r07_word_independent_successor_kernel_v35.producer.base.checkpoint.json\""],
];;
