#!/usr/bin/env python3
"""Independent checker for the bounded target-6 full-D2 column generator."""
...
TASK_BYTES = 43_511
PRODUCER = Path("search/d972_b345_target6_dual_colgen_v1.py")
PRODUCER_SHA = "8a3dd09811ec790b90f5f3d16890e9ef534a9fc0399597a0daa892605c58c8fc"
PRODUCER_BYTES = 410_757
...
ALGORITHM_PUBLIC = {"name": "bounded-full-D2-dual-column-generation",
    "version": 1, "max_batches": 8, "max_translations_per_batch": 1024,
...
def solve_remainders(v2: Any, old: Any, e4: Any,
    ...
    target = {"generation": generation, "variables": 108,
        ...
        "row_space_sha256": system.digest(),
        "remainders_sha256": sha_obj([sorted(row.items())
                                       for row in remainders]),
...
def validate_stable_public_schema(data: dict[str, Any]) -> None:
    ...
    if support:
        require(set(support) == DIRECTED_SUPPORT_KEYS and
                support["occurrence_count"] == len(support["occurrences"]) == 76 and
                support["ordered_sha256"] == sha_obj(support["occurrences"]) ==
                    BASE_OCCURRENCE_SHA and
    ...
    if prefix:
        require(set(prefix) == PREFIX_B0_KEYS and
                ...
                prefix["dependent_event_count"] ==
                    len(prefix["dependent_events"]) == 16 and
                ...
                prefix["complete_block_registry"]["translation_count"] == 32975 and
    ...
    if surgery:
        require(prefix and set(surgery) == DIRECTED_SURGERY_KEYS and
                surgery["round_count"] == len(surgery["rounds"]) == 32 and
                surgery["translation_count"] ==
                    len(surgery["translations"]) == 207 and
    ...
    if data["initial_target"]:
        ...
        require(set(initial) == INITIAL_TARGET_KEYS and
                ...
                _sha256_text(initial["raw_gradients_sha256"]),
...
def check_receipt(q3_path: Path, receipt_path: Path, *,
    ...
    require(current_target["row_space_sha256"] == B1["target_row_space_sha256"]
            and current_target["remainders_sha256"] ==
...
def self_test() -> None:
    ...
    print("D972_B345_TARGET6_DUAL_COLGEN_V1_CHECKER_SELFTEST_PASS "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("D972_B345_TARGET6_DUAL_COLGEN_V1_CHECKER_PASS "
