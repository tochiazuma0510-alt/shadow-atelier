#############################################################################
## lins-census-2000-v1.g -- LINS-2000 (裁定803/804/806/807/808, FIB-SWEEP撤回後の
## FIB-MULT(2) + P-Q8FAM + WIN-CNOTN 相乗り走行). Extends the index bound of
## lins-twin-census-v1.g (裁定548 W-1, index<=1000, already committed as
## search/certs/lins_twin_census_v1_20260806.json) to index<=2000, per
## docs/notes/hunting_chapter_v1_addendum_a.md §4.3 ("lins の指数上限を
## 1000->2000 に上げる1本に統合"). Runs on GHA per 裁定807/808 (venue: GHA,
## not local -- 裁定633 訂正2's own note "指数2000=GHA内"). Otherwise IDENTICAL
## apparatus/universe/discipline to lins-twin-census-v1.g (LID-1: single GAP
## process, single LowIndexNormalSubgroupsSearch call; same row/pairing logic
## verbatim) -- only CENSUS_INDEX_HI and the output path differ.
##
## lins-twin-census-v1.g -- 裁定 548 W-1 【TWIN-CENSUS】(original header, retained)
##
...
##
## Output: search/certs/lins_census_2000_v1_20260811.json
#############################################################################
...
CENSUS_INDEX_HI := 2000;;  # LINS-2000 (裁定803/804/806/807/808) -- raised from
                            # the original 1000 (lins-twin-census-v1.g) per
                            # docs/notes/hunting_chapter_v1_addendum_a.md §4.3.
                            # Run on GHA (裁定807/808), not local.
...
for nd in nodes do
  ...
  if GAPLIB_CheckCap(9000.0, "lins-census-2000-v1 per-node pass") then
...
outParts := [];;
Add(outParts, "{\n");
Add(outParts, "  \"generated_by\": \"search/lins-census-2000-v1.g\",\n");
Add(outParts, "  \"ruling\": \"裁定803/804/806/807/808 (LINS-2000, index bound raised 1000->2000 per docs/notes/hunting_chapter_v1_addendum_a.md SS4.3; supersedes/extends 裁定548 W-1's lins-twin-census-v1.g)\",\n");
Add(outParts, "  \"note\": \"INVENTORY ONLY -- AS-GAP-6 hunting-ground catalog of twin pairs (N,K), N<>K normal in B3, same index, isomorphic B3/N quotient. No claim of GTSh(K,N) non-emptiness, no isolated/settled verdict, no non-isolated-witness claim. Distinct apparatus from Week 3 twincell-enum.g/check-twincell.mjs (dihedral SmallGroup(32) adjacency search). Single GAP process, single LowIndexNormalSubgroupsSearch call (LID-1 discipline): same enumeration => exhaustive over the bound within this run. verbal_status is UNKNOWN for every row in this pass by design. Sealed 3 quantities / 705,894-pair universe / kill theorems: non-contact. Feeds FIB-MULT(2)/P-Q8FAM/WIN-CNOTN aggregation as follow-on post-processing (not performed in this script).\",\n");
Add(outParts, "  \"operational_deviation\": \"executed on GHA ubuntu runner instead of local gap.ps1/-o 2g machine (8GB total RAM), per 裁定633 訂正2 ('指数2000=GHA内') and 裁定807/808 (venue triage: heavy new computation goes directly to GHA, no further local trial). Local timing tests at bound<=1000 (2026-08-06) and a local bound=2000 attempt (this session) informed the timeout/shard parameters but the REAL run executes on GHA. Mathematical universe (B3 presentation, PB3/c definitions, LID-1 single-call discipline, row/pairing logic) is UNCHANGED from lins-twin-census-v1.g.\",\n");
...
WriteFile("search/certs/lins_census_2000_v1_20260811.json", Concatenation(outParts));
Print("Wrote search/certs/lins_census_2000_v1_20260811.json\n");
Print("ALL_DONE\n");
