## shard 2 of A14 (D4, ell=9) generator search -- independent GAP process, parallel to shard 1
n := 14;;
...
for trial in [1..3000000] do
  ...
  if trial mod 50000 = 0 then
    Print("[shard2] progress trial=", trial, " elapsed_ms=", Runtime()-t0, "\n");
  fi;
od;
Print("[shard2] trials=", trials, " time=", Runtime()-t0, " found=", found<>fail, "\n");
if found <> fail then
  Print("[shard2] a1 := ", found.a1, ";;\n");
  Print("[shard2] b1 := ", found.b1, ";;\n");
  Print("[shard2] u type check: ", SortedList(CycleLengths(found.u,[1..n])), "\n");
fi;
Print("[shard2] DONE\n");
