...
# ================================================================================
# CORRECTED 2026-07-26 (independent audits: docs/notes/検証_q式導出.md + sol/sol2_reply_01_q.md,
# both judged the original Cs/QTheta/QN "(b) 誤り" -- FIX NOT YET COMPLETE, see below).
#
# P-1 (section convention, FIXED): the canonical section is s(a)=w^{a_w}p^{a_p}q^{a_q}... (Hall
# order w<p<q<..., ASCENDING, per v3 sec.1.3), not the descending order I had assumed. With
# [u,v]:=u^-1v^-1uv, uv=vu[u,v], t5=[w,p], t6=[w,q], the correct section cocycle is
#   c_s(a,b) = (-a_p*b_w, -a_q*b_w)     (sol2 eq.1, hand-verified: c_s(w,p)=wp(wp)^-1=0,
#                                         c_s(p,w)=pw(wp)^-1=-t5, matching this formula)
# This replaces the WRONG c_s(a,b)=(a_w*b_p, a_w*b_q) I used before (that was the cocycle for a
# DIFFERENT section with p,q ordered before w).
#
# P-2 (STILL MISSING, NOT FIXED -- this is why item 3 stays ON HOLD): q_theta(f), q_N(f) are NOT
# simply c_s(theta_bar f, f) / the 3-fold c_s sum. The correct additive decomposition (both audits,
# independently) is:
#   q_theta(a) = d_theta(a) + c_s(Ta, a)                                    [sol2 eq.2]
#   q_N(a) = eps_m + d_{sigma^2}(a) + d_sigma(a)
#            + c_s(e,S^2a) + c_s(e+S^2a,Sa) + c_s(e+S^2a+Sa,a)              [sol2 eq.3]
# where d_theta(a):=theta(s(a))*s(bar_theta a)^-1, d_sigma(a):=sigma(s(a))*s(bar_sigma a)^-1,
# eps_m := E_m*s(bar_E_m)^-1  -- i.e. the ACTUAL "how far theta/sigma/E_m are from being exactly
# the canonical lift of their Abar-level images" defect terms. These require genuine Hall
# collection on the FULL 12-dim group A=gamma_2/gamma_6 (not just Abar+C separately), which is
# NOT constructible from beta/theta|_C/sigma|_C alone (both audits confirm: "指定された作用spec
# だけではd_theta,d_sigma,eps_mは決まらない"). This data is not yet in any document I have been
# given permission to read. QTheta/QN below are therefore STILL INCOMPLETE and MUST NOT be used
# for a live 384-system judgement until d_theta/d_sigma/eps_m are supplied and independently
# verified via the group-product recheck (v3 sec.3.3(C)) -- reporting this precisely rather than
# silently re-deriving the missing terms myself.
# ================================================================================
...
# ================================================================================
# QThetaPartial / QNPartial (RENAMED from QTheta/QN 2026-07-26 to make the incompleteness
# impossible to miss at a call site): these compute ONLY the c_s-cocycle terms, i.e. NOT
# q_theta/q_N themselves but the "コードの値" part identified by both audits. The missing
# d_theta(f) / [eps_m + d_{sigma^2}(f) + d_sigma(f)] additive terms are NOT included because
# they are not yet constructible from any document I have permission to read (see header note
# above). DO NOT use these as q_theta/q_N for any solvability judgement. Retained only so the
# corrected Cs (P-1) is recorded and so the two audits' cocycle-bookkeeping can be spot-checked
# once d_theta/d_sigma/eps_m are supplied.
# ================================================================================
QThetaPartial := function(f)
  local thf;
  thf := ThetaP(f);
  return Cs(thf, f);   # = c_s(Ta,a) in sol2's eq.2 -- MISSING + d_theta(f)
end;;

QNPartial := function(f, m)
  ...
  return term3 + term1 + term2;   # = the three c_s terms in sol2's eq.3 -- MISSING
                                   # + eps_m + d_{sigma^2}(f) + d_sigma(f)
end;;

Print("\n[HOLD] QThetaPartial/QNPartial are cocycle-only partial sums (P-1 section-order fix\n");
Print("  applied); the required d_theta/d_sigma/eps_m action-defect terms are NOT constructible\n");
Print("  from any spec-projection document available to me. Per both independent audits\n");
Print("  (docs/notes/検証_q式導出.md, sol/sol2_reply_01_q.md), the 384-system sweep (item3)\n");
Print("  remains ON HOLD until those terms are supplied and the v3 sec.3.3(C) group-product\n");
Print("  independent_recheck (theta(s(a))s(a), E_m sigma^2(s(a))sigma(s(a))s(a) recomputed via\n");
Print("  an ACTUAL group A=gamma_2/gamma_6, not just Abar+C) passes.\n");

# Xi (would-be q_theta/q_N combined obstruction vector) is NOT defined -- see HOLD notice above.
# Left undefined deliberately so any future accidental call fails loudly instead of silently
# using the incomplete QThetaPartial/QNPartial as if they were correct.
