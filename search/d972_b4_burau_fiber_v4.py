...
FINAL = "D972_B4_BURAU_FIBER_V4_FINAL"
...
def complete_hprime(x: TupleElt, y: TupleElt, q: int,
    ...
    while True:
        ...
        print(f"D972_B4_BURAU_V4_PROGRESS phase=normal-closure round={rounds} "
...
def run_full(q: int, a: int, output: Path, cal_q3: Path, cal_q4: Path) -> int:
    ...
    print(f"D972_B4_BURAU_V4_PROGRESS phase=schreier-complete projected={len(sec)} "
    ...
    for i, (m, key, word) in enumerate(rows, 1):
        ...
        if i % 81 == 0:
            print(f"D972_B4_BURAU_V4_PROGRESS phase=rows completed={i}", flush=True)
    ...
    output.write_text(json.dumps(receipt, sort_keys=True, separators=(",", ":")) + "\n",
                      encoding="utf-8")
    print(f"D972_B4_BURAU_V4_DONE q={q} a={a} h={h_order} "
...
def self_test() -> None:
    ...
    print("D972_B4_BURAU_V4_SOURCE_WORD_ROOF_ONLY_NEGATIVE_PASS "
          "row=2 exponent=(-4,-8) nonzero=956")
    print("D972_B4_BURAU_FIBER_V4_NEGATIVE_FIXTURES_PASS")
    TQ = 5
    print("D972_B4_BURAU_FIBER_V4_SELFTEST_PASS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("ci/out/d972_b4_burau_fiber_v4.json"))
    ap.add_argument("--calibration-q3", type=Path,
                    default=Path("ci/out/d972_b4_burau_fiber_v4_q3.json"))
    ap.add_argument("--calibration-q4", type=Path,
                    default=Path("ci/out/d972_b4_burau_fiber_v4_q4.json"))
    ...
    try:
        ...
        print(f"D972_B4_BURAU_V4_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
