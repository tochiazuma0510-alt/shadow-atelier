#!/usr/bin/env python3
"""Task1027: independent fixed-lambda, k64 one-batch cycle checker.
...
PRODUCER_FILE = "search/d972_r07_fixed_lambda_cycle_batch_v2.py"
CHECKER_FILE = "search/check_d972_r07_fixed_lambda_cycle_batch_v2.py"
CHECKER_WORKFLOW = ".github/workflows/d972-r07-fixed-lambda-cycle-batch-v2.yml"
...
def select_all_residuals(chord_edges: np.ndarray, tau: np.ndarray, values: np.ndarray,
    ...
    """Only the registered first 64 failures are offered; the whole array is read."""
...
def check_registration(value: Any, expected: dict[str, Any]) -> None:
    ...
    require(integer(value["batch_size"], "registered_k64", 64, 64) == BATCH_SIZE and
            ...
            value["refill"] is False, "registered_k64_policy_and_no_refill")
...
def fixture_write(root: Path, name: str, raw: bytes) -> None:
    relative_name(name, "k64_fixture_relative_name")
    target = root / name
    require(target.resolve().is_relative_to(root.resolve()), "k64_fixture_write_containment")
...
def fixture_directory(parent: Path, name: str, payloads: dict[str, bytes]) -> Path:
    root = parent / relative_name(name, "k64_fixture_case")
    root.mkdir()
...
def k64_fixture_records(registration: dict[str, Any]) -> RootRecords:
...
class K64MetadataAnchor:
    ...
    def row(self, index: int) -> bytes:
        raise ValueError("k64_metadata_fixture_has_no_old_rows:" + str(index))


def k64_registration_canary(root: Path) -> dict[str, Any]:
    root.mkdir()
    ...
    records = k64_fixture_records(registration)
    ...
    registration_case("registered-64-one-no-refill", registration, False)
    for suffix, bad in (("32", 32), ("33", 33), ("63", 63), ("65", 65), ("128", 128),
    ...
    registration_case("old-policy32", {**registration, "selection_policy": "CHORD_FIRST_ROSTER_32_THEN_FIRST_AUX"}, True)
    acceptance = {"schema": SCHEMA + ".acceptance", "parents": [], "anchor": {}, "code": {},
    ...
    saved = fixture_directory(root, "old-acceptance-schema", {"acceptance.json": canonical({**acceptance,
        "schema": "d972.r07.fixed-lambda-cycle-batch.v1.acceptance"})})
    ...
    for side in ("producer", "checker"):
        ...
        bad[side]["file"] = bad[side]["file"].replace("_v2.py", "_v1.py")
    ...
    for name, owner in (("old-owner-schema", fixture_reseal(records.owner,
                            schema="d972.r07.fixed-lambda-cycle-batch.v1.owner")),
                        ("old-owner-scope32", fixture_reseal(records.owner, scope={**SCOPE, "batch_size": 32}))):
    ...
    require(canonical(records.owner) == initial_owner, "k64_owner_portable_and_unchanged")

    state = BatchReductionState(K64MetadataAnchor(), 0)
    row = bytes(ROW_BYTES)
    state.rows = [row] * BATCH_SIZE
    state.row_manifests = ["2" * 64] * BATCH_SIZE
    last_row = row_source(state, 63)
    require(last_row["local_row_offset"] == 63, "k64_last_local_row_63")
    fixture_directory(root, "row63", {last_row["file"]: row, "row-source.json": canonical(last_row),
        "scope.json": canonical({"fixture_only": True, "old_rows": 0, "private_rows": 64,
            "row_bytes": ROW_BYTES, "selected_offset": 63, "no_primal_or_reduction_arithmetic": True})})
    state.rows.append(row); state.row_manifests.append("2" * 64)
    negative("row-offset-64", lambda: row_source(state, 64))
    require(phase_at_sequence(387) == ("candidates/000063/reduction", "reduction", 63), "k64_last_phase_387")
    negative("phase-sequence-388", lambda: phase_at_sequence(388))
    prefix = "candidates/000063/reduction"
    phase_data, _ = fixture_phase(records, prefix, "reduction", {}, None, 63)
    saved = fixture_directory(root, "ordinal63", phase_data)
    compare_phase(CandidateFiles(saved), prefix, "reduction", {}, records, None, ordinal=63)
    negative("candidate-ordinal64", lambda: compare_phase(CandidateFiles(saved), prefix, "reduction", {}, records, None, ordinal=64))
    registered = registered_basenames(64)
    require("rows/000063" in registered and "rows/000064" not in registered and "candidates/000063" in registered,
            "k64_normal_row_and_candidate_scope")
    ...
    require(audit.committed["processed_candidates"] == 0 and audit.tail["published_by_checker"] is False,
            "k64_one_phase_tail_does_not_advance_HEAD")
    ...
    def bootstrap(name: str, count: Any, bad: bool) -> None:
        ...
        if bad:
            ...
            require(len(descriptors) == len(values) == 1 and not progress.checkpoints and progress.head is None,
                    "k64_one_bootstrap_zero_counts_without_formed_HEAD")
            files.unchanged()
    ...
    fixture_write(root, "case-ledger.json", canonical({"fixture_only": True, "rejected_cases": rejected_names,
        "positive_boundaries": {"candidate_ordinal": 63, "row_offset": 63, "phase_sequence": 387,
    ...
    return {"name": "k64-version-registration-and-types", "status": "PASS", "rejected_cases": rejected_names}


def k64_roster_canary(root: Path) -> dict[str, Any]:
    root.mkdir()
    registration = registered_limits(argparse.Namespace(producer_max_seconds=5400, producer_max_memory_mib=7168,
                                                         max_seconds=10800, max_memory_mib=7168))
    records = k64_fixture_records(registration)
    ...
    def restore(saved: Path, arithmetic: SelectionArithmetic, witnesses: list[dict[str, Any]],
        ...
        require(tree_sha == selected["phase_manifests"]["tree"], "k64_saved_tree_manifest_identity")
        _, complete = compare_selection_publication(files, records, selected, witnesses, True)
        require(complete, "k64_all_selected_witnesses_and_views_restored")
        compare_candidate_roster(files, len(witnesses))
        files.unchanged()

    selected_64 = None
    for name, count, expected_count, aux in (("failed32", 32, 32, [2, 1]), ("failed33", 33, 33, [2, 1]),
            ("failed63", 63, 63, [2, 1]), ("failed64", 64, 64, [2, 1]), ("failed65", 65, 64, [2, 1]),
        ...
        require(plan["failed_indices"].tolist() == failed and plan["failed_edges"].tolist() == [int(edges[i]) for i in failed] and
                len(plan["candidates"]) == expected_count, "k64_full_failure_roster_and_registered_cutoff:" + name)
        if count:
            require(failed[-1] == CHORDS - 1 and plan["mode"] == "CHORD_BATCH" and
                    ...
                    "k64_end_of_full_array_and_chords_precede_nonzero_aux")
            for item in plan["candidates"]:
                ...
                require(witness["basis_coefficients"] == coefficient_rows[item["roster_index"]] and
                        ...
                        witness["tau"] == [0] * 5, "k64_nontrivial_five_coefficients_six_cycles_keep_zero_factors")
        elif expected_count:
            require(plan["mode"] == "AUXILIARY" and plan["candidates"][0]["witness"]["coordinate"] == (0 if aux[0] else 1),
                    "k64_first_nonzero_aux_only_after_all_chords_zero")
        else:
            require(plan["mode"] == "COMPLETE_ZERO_CANDIDATE", "k64_complete_current_lambda_zero")
        ...
        if count == 64:
            selected_64 = arithmetic, witnesses, selected, data
        if count == 65:
            ...
            extra = fixture_reseal(witnesses[-1], ordinal=64)
            corrupt("over-limit-65-witnesses", [*witnesses, extra])
            corrupt("old-first32-cutoff", witnesses[:32])
    ...
    require(selected_64 is not None, "k64_tail_fixture_available")
    arithmetic, witnesses, selected, data = selected_64
    require(witnesses[63]["roster_index"] == CHORDS - 1, "k64_sixtyfourth_is_actual_last_chord")
    mutations = []
    altered = copy.deepcopy(witnesses)
    altered[63] = fixture_reseal(altered[63], roster_index=CHORDS - 2)
    mutations.append(("sixtyfourth-roster-index-changed", altered))
    altered = copy.deepcopy(witnesses)
    coefficients = list(altered[63]["basis_coefficients"])
    coefficients[0] = (coefficients[0] + 1) % 3
    cycles = copy.deepcopy(altered[63]["cycles"])
    cycles[1]["coefficient"] = -coefficients[0] % 3
    altered[63] = fixture_reseal(altered[63], basis_coefficients=coefficients, cycles=cycles)
    mutations.append(("sixtyfourth-coefficient-and-cycle-changed", altered))
    altered = copy.deepcopy(witnesses)
    altered[62], altered[63] = fixture_reseal(altered[63], ordinal=62), fixture_reseal(altered[62], ordinal=63)
    ...
    return {"name": "k64-full-roster-cutoff-and-restoration", "status": "PASS", "rejected_cases": rejected_names}


def selftest(root: Path) -> dict[str, Any]:
    first = k64_registration_canary(root / "registration")
    boundary("k64_registration_group_complete")
    second = k64_roster_canary(root / "roster")
    boundary("k64_roster_group_complete")
    return document("selftest", {"status": "PASS", "tests": [first, second],
        "fixture_scope": "new-k64-registration-and-portable-metadata;synthetic-full-54433-rosters;real-saved-payload-reader;all-fixtures-retained;no-accepted-parent-arithmetic",
        "production_interfaces_used": ["check_registration", "check_acceptance_header", "check_executable_paths", "compare_root_records",
            ...
            "compare_candidate_roster", "CandidateFiles", "prepare_selftest_root"],
...
def main() -> int:
    ...
    try:
        if args.selftest:
            ...
            require(args.max_seconds == 300 and args.max_memory_mib == 7168, "registered_k64_selftest_resource_limits")
