#############################################################################
## Researcher-authorized R07 A0/v13 fresh production driver.
## ASCII only.  The driver has no recursive self pin.
#############################################################################
D342Producer:="search/d972_r07_history_free_positive_fast_resume_v13.py";;
D342Checker:="crosscheck/check_d972_r07_history_free_positive_fast_resume_v13.py";;
D342Fixture:="search/certs/d972_r07_history_free_positive_fast_resume_selftest_v10_20260829.json";;
D342Task176Receipt:="ci/in/d972_r07_all_seven_extension_section_census_v1.json";;
D342Task176Manifest:="ci/in/d972_r07_all_seven_extension_section_census_v1.manifest.json";;
D342Task176Crosscheck:="ci/in/d972_r07_all_seven_extension_section_census_crosscheck_v1.json";;
D342Task176Recovery:="ci/in/d972_r07_all_seven_extension_section_census_crosscheck_v1.recovery.manifest.json";;
D342Task176RecoveryV2:="ci/in/d972_r07_all_seven_extension_section_census_crosscheck_v1.recovery.manifest.v2.json";;
...
D342Raw:="ci/resume/d972_r07_history_free_positive_fast_resume_v13.raw.json";;
D342Base:="ci/out/d972_r07_history_free_positive_fast_resume_v13_production";;
...
D342Common:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V10_COMMON_WORD";;
D342ProducerPrefix:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V10_PRODUCER_TERMINAL";;
D342CheckerPrefix:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V10_CHECKER_TERMINAL";;
D342Sentinel:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V13_DRIVER_PASS";;

D342Pins:=[
 [D342Producer,147409,"4d1be83fefbb1a1c0b23010825c0013b80650439b714dce7e35a6e0f53a2ff2a"],
 [D342Checker,131946,"42e8f6df8d85169bf4039bc4195a0e47c284ad475a177414308ba28f99377b64"],
 [D342Fixture,3785,"de6273d681238b1aa560353c70a245cc28823326908e31be464fa2c399917203"],
 [D342Task176Receipt,13649089,"715441d8ecb1b4bb39a51cf3df15f04d6179ee6adeafa5b925485dbbe91f7f41"],
 [D342Task176Manifest,349,"de62e5e55a2e348a3cce297764f7ff4bfedc10ebe2545f22cbc1551f15e1adc1"],
 [D342Task176Crosscheck,757,"e6a45a34353ce1fb54c99b4f9cbc8b106f34bfc751dd50044f2a79da72cad5e5"],
 [D342Task176Recovery,2035,"41d2cb72614ce7e2d5b2d7a9000e861414da1c749876b3d51f1ccf2ca63390a8"],
 [D342Task176RecoveryV2,2690,"67dd555f6e0f943d0161ef2f2c8124b4cc31c9167846b45b43fd2001f5fbba3f"],
...
D342Read:=function(path)
  local raw;
  raw:=StringFile(path);;
  if raw=fail or Length(raw)=0 then Error("task350 missing input ",path); fi;
  return raw;
end;;
...
if D342Contents=fail then Error("task350 cannot inspect ci/out"); fi;
for D342Name in D342Contents do
  if PositionSublist(D342Name,
      "d972_r07_history_free_positive_fast_resume_v13_")=1 then
    Error("task350 stale v9 owned output ",D342Name);
  fi;
od;
...
if Length(D342Outputs)<>Length(Set(D342Outputs)) then
  Error("task350 duplicate output path");
fi;
for D342Path in D342Outputs do
  if IsExistingFile(D342Path) then Error("task350 stale output ",D342Path); fi;
od;

D342Stream:=OutputTextFile(D342Shell,false);;
if D342Stream=fail then Error("task350 shell open"); fi;
SetPrintFormattingStatus(D342Stream,false);;
PrintTo(D342Stream,"#!/usr/bin/env bash\nset -euo pipefail\n");
PrintTo(D342Stream,"for command in python3 timeout tee grep sed cmp wc tr nproc; do command -v \"$command\" >/dev/null; done\n");
...
PrintTo(D342Stream,"            raise SystemExit('task350 frozen sole ZIP member gate')\n");
...
PrintTo(D342Stream,"            raise SystemExit('task350 frozen raw member gate')\n");
...
PrintTo(D342Stream,"set +e\n");
PrintTo(D342Stream,"timeout --foreground 11100s python3 -u -B ",D342Producer,
 " --mode PRODUCTION --source ",D342Raw," --manifest ",D342Manifest,
 " --output ",D342Receipt," --seconds 10800 --workers \"$workers\" 2>&1 | tee ",
 D342ProducerLog,"\n");
PrintTo(D342Stream,"producer_pipeline_status=(\"${PIPESTATUS[@]}\")\n");
PrintTo(D342Stream,"set -e\n");
PrintTo(D342Stream,"if [ \"${producer_pipeline_status[0]}\" -ne 0 ]; then exit \"${producer_pipeline_status[0]}\"; fi\n");
PrintTo(D342Stream,"if [ \"${producer_pipeline_status[1]}\" -ne 0 ]; then exit \"${producer_pipeline_status[1]}\"; fi\n");
...
PrintTo(D342Stream,"set +e\n");
PrintTo(D342Stream,"timeout --foreground 7500s python3 -u -B ",D342Checker,
 " --receipt ",D342Receipt," --verdict ",D342Verdict," 2>&1 | tee ",
 D342CheckerLog,"\n");
PrintTo(D342Stream,"checker_pipeline_status=(\"${PIPESTATUS[@]}\")\n");
PrintTo(D342Stream,"set -e\n");
PrintTo(D342Stream,"if [ \"${checker_pipeline_status[0]}\" -ne 0 ]; then exit \"${checker_pipeline_status[0]}\"; fi\n");
PrintTo(D342Stream,"if [ \"${checker_pipeline_status[1]}\" -ne 0 ]; then exit \"${checker_pipeline_status[1]}\"; fi\n");
...
if D342Observed<>D342Sentinel then Error("task350 sentinel mismatch"); fi;
Print(D342Sentinel,"\n");
