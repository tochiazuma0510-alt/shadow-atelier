...
PRODUCER_PATH = Path("search/d972_b4_burau_joint_v2.py")
...
SCHEMA = "d972-b4-burau-joint/v2-synchronized"
FINAL = "D972_B4_BURAU_JOINT_V2_FINAL"
...
def enum_kernel(gens: Sequence[JointElt], specs: Sequence[Spec], degree: int) -> list[JointElt]:
...
def complete(x: JointElt, y: JointElt, specs: Sequence[Spec]) -> tuple[dict[Perm, JointElt], list[JointElt], list[JointElt]]:
    ...
    while True:
        ...
        ke = enum_kernel(kernel_rels(gens, sec, specs), specs, len(x[0]))
        ks = set(ke)
        add = []
        for g in tuple(gens):
            for h in actors:
...
def check_receipt(path: Path) -> dict[str, Any]:
    ...
    require(r.get("words_sha256") == WORDS_SHA and
            ...
            len(r["rows"]) == 972, "source row binding drift")
    roof = build_roof()
    ...
    sec, kernel, hpgens = complete(x, y, specs)
    ks = set(kernel)
    ...
    require(isinstance(evidence, dict) and evidence.get("seed") ==
            ...
            evidence.get("m_residue_gate") == M_RESIDUE_GATE,
    ...
    require(len(seen) == 972, "incomplete roof key set")
    require((r["status"] == "CANDIDATE_B4_A_BURAU_JOINT_ZERO_FIBER" and any_full_zero) or
            (r["status"] == "UNKNOWN_BURAU_JOINT_ALLPASS" and not any_full_zero),
            "status/count semantics drift")
    return {"status": ("B4_A_BURAU_JOINT_ZERO_FIBER_CROSSCHECKED"
                        if any_full_zero else r["status"]),
            "config": config, "rows": 972, "h_order": len(qreps) * len(sec) * len(kernel),
            "hprime_order": len(sec) * len(kernel), "kernel_order": len(kernel),
            "pentagon_zero_fibers": sum(1 for z in r["rows"]
                                         if z["simultaneous_identity_defect_count"] == 0),
            "full_GT_zero_fibers": sum(1 for z in r["rows"]
...
def selftest() -> None:
    ...
    print("D972_B4_BURAU_JOINT_V2_CORRUPT_RECEIPT_NEGATIVE_PASS")
    print("D972_B4_BURAU_JOINT_V2_Q2Q7_ADMISSIBILITY_AUDIT_PASS q2=[1] q7=[1,2,3,4,5,6]")
    print("D972_B4_BURAU_JOINT_V2_SOURCE_WORD_ROOF_ONLY_NEGATIVE_PASS row=2 exponent=(-4,-8) nonzero=956")
    print("D972_B4_BURAU_JOINT_V2_CHECKER_NEGATIVE_FIXTURES_PASS")
    print("D972_B4_BURAU_JOINT_V2_CHECKER_SELFTEST_PASS")
    print("D972_B4_BURAU_JOINT_V2_CHECKER_FINAL_MARKER status=PASS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("D972_B4_BURAU_JOINT_V2_CHECK_PASS", json.dumps(result, sort_keys=True))
    return 0
