#############################################################################
## search/a18-kernel-structure.g -- kernel-structure capsule for W-D-A18-13a
## (裁定200 工程1)
##
## Same pattern as search/a16-kernel-structure.g / search/a20-kernel-structure.g,
## extended with Q_action_on_Kab (the KE-o field: Q's generators' action
## matrix on K^ab = K/[K,K]), per docs/notes/a20_prediction_v1.md S A18-4's
## 9+1-field measurement spec. Field names below follow that spec's
## terminology exactly (group_order, ker_size, chi_image_order, Q_struct,
## K_struct, Kprime_order, Gprime_order, Gprime_struct, ZK_order,
## ZK_equals_Gprime, KmodGprime_struct, derived_length_G,
## GmodGprime_invariant_factors, Q_action_on_Kab).
##
## Output: search/certs/a18_kernel_structure_20260729.json
#############################################################################
...
Print("=== reconstructing GTSh(N,N) for W-D-A18-13a ===\n");
...
if not gi.closed then
  Error("a18-kernel-structure.g: (3.53) did not close -- refusing to report ",
...
outParts := [];;
Add(outParts, "{\n");
Add(outParts, "  \"generated_by\": \"search/a18-kernel-structure.g\",\n");
Add(outParts, "  \"note\": \"kernel-structure capsule for GTSh(W-D-A18-13a), field names per docs/notes/a20_prediction_v1.md S A18-4; NOT a ledger claim, no cross-check performed (single implementation)\",\n");
...
WriteFile("search/certs/a18_kernel_structure_20260729.json", Concatenation(outParts));
Print("\nWrote search/certs/a18_kernel_structure_20260729.json\n");
Print("A18_KERNEL_STRUCTURE_DONE\n");
