#!/usr/bin/env python3
"""Full registered-4096 typed WordExpr/Fox lane at relative Frattini-3.
...
SCHEMA = "d972-b345-relative-frattini3-wordexpr/v8"
...
OUTPUT_PATH = Path("ci/out/d972_b345_relfrat3_wordexpr_v8.json")
...
class ResourceMonitor:
    ...
    def _progress(self, force: bool = False) -> None:
        ...
        print("D972_B345_RELFRAT3_WORDEXPR_V8_PROGRESS "
...
class SectionExpressionDAG:
    ...
    def _append(self, key: tuple[Any, ...], kind: int, signed: int,
        ...
        if len(self.kind) >= CAPS["directed_section_expr_nodes"]:
            raise ResourceStop("directed_section_expr_nodes")
        if self.edge_count + edges > CAPS["directed_section_expr_edges"]:
            raise ResourceStop("directed_section_expr_edges")
        node = len(self.kind)
        self.keys[key] = node
...
class SparseSectionOracle:
    ...
    def register_directed(self, value: EKey, root: int) \
        ...
        if len(self.directed_blobs) >= CAPS["directed_unique_translations"]:
            raise ResourceStop("directed_unique_translations")
...
class WordExprDAG:
    ...
    def _add(self, opcode: int, rank: int, word: tuple[int, ...],
        ...
        if len(self.opcode) >= CAPS["wordexpr_nodes_per_candidate"]:
            raise ResourceStop("wordexpr_nodes_per_candidate")
        if self.edge_count + len(children) > CAPS["wordexpr_edges_per_candidate"]:
            raise ResourceStop("wordexpr_edges_per_candidate")
        ...
        if opcode == self.FLAT_WORD:
            ...
            if self.flat_leaf_count > CAPS["wordexpr_flat_leaves_per_candidate"]:
                raise ResourceStop("wordexpr_flat_leaves_per_candidate")
        return identifier
...
class WordExprEvaluator:
    """Independent-of-pool value and exact left-Fox evaluator."""

    def __init__(self, dag: WordExprDAG, quotient: MatchedQuotient) -> None:
        self.dag = dag
        self.quotient = quotient
        self.values: list[EKey] = []
        self.live_gradient_peak = 0

    def evaluate_values(self) -> list[EKey]:
        values: list[EKey] = []
        q = self.quotient
    ...
    def evaluate_gradients(self, roots: Sequence[int]) \
        ...
        for node in sorted(reached):
            ...
            if opcode == WordExprDAG.IDENTITY:
                ...
                gradient, value, _ = fox_gradient(self.dag.word[node-1], q)
                require(value == self.values[node-1], "WordExpr flat value/gradient")
            elif opcode == WordExprDAG.PRODUCT:
                left, right = self.dag.children[node-1]
                ...
                gradient = _raw_gradient_translate(
            ...
            if self.live_gradient_peak > CAPS["candidate_live_gradient_entries_total"]:
                raise ResourceStop("candidate_live_gradient_entries_total")
            for child in self.dag.dependencies(node):
                references[child] -= 1
                require(references[child] >= 0, "WordExpr reference accounting")
                if references[child] == 0 and child not in requested:
                    live -= len(gradients.pop(child))
...
def build_wordexpr_candidate(correction_index: int,
    ...
    if any(dag.expanded_count[root-1] >
        ...
        raise ResourceStop("wordexpr_expanded_letter_count_per_target")
    return {
...
def make_base_receipt(q3_path: Path, output_path: Path, q3_data: dict[str, Any],
    ...
    return {
        ...
        "representation_contract": {
            "version": "typed-wordexpr-chain-rule-v8",
...
def finish_wordexpr_scan(
    ...
    def candidate1_flat_bridge(compiled: dict[str, Any],
                               evaluator: WordExprEvaluator) -> dict[str, Any]:
        ...
        bindings = []
        for (name, kind, root), (_, _, flat_word) in zip(expr_rows, flat_rows):
            ...
            flat_gradient, flat_value, _ = fox_gradient(flat_word, e4)
    ...
    for correction_index, correction in enumerate(dictionary["words"], 1):
        ...
        evaluator = WordExprEvaluator(expr, e4)
        ...
        if correction_index == 1:
            bridge = candidate1_flat_bridge(compiled, evaluator)
            receipt["candidate1_flat_bridge"] = bridge
        ...
        if failed_name is not None:
            ...
            missing_ordinal = 0
            total_entries = 0
            for ordinal, (name, kind, root) in enumerate(compiled["acceptance"], 1):
            ...
            if missing_key is not None:
                ...
                records.append({
                    ...
                    "wordexpr_max_expanded_letters":
                        expr_accounting["max_expanded_letter_count"],
                    "wordexpr_live_gradient_peak": evaluator.live_gradient_peak,
                    "pool_suffix_removed": removed,
                })
            else:
                ...
                replay_evaluator = WordExprEvaluator(replay_expr, e4)
        ...
        receipt["_wordexpr_scan_current"] = None
        now = time.monotonic()
        if correction_index % 256 == 0 or now-last_progress >= 30 or selected:
    ...
    receipt["selected_pair"] = {
        ...
        "fixed_inverse_words_sha256": digest_obj(finite_inverse_words),
...
def run(q3_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    def record_phase(label: str) -> None:
        ...
        print(f"D972_B345_RELFRAT3_WORDEXPR_V8_PHASE {label} "
    ...
    def append_trace(row: dict[str, Any]) -> None:
        if len(checkpoint_trace) >= CAPS["transaction_trace_records"]:
            raise ResourceStop("transaction_trace_records")
    ...
    if pin_errors:
        source_hashes = {
            ...
            "checker_sha256": digest_file(
                repo/"search/check_d972_b345_relfrat3_wordexpr_v8.py"),
            "driver_sha256": digest_file(
                repo/"search/d972_b345_relfrat3_wordexpr_gha_driver_v8.g"),
        }
    ...
    source_hashes = {
        "producer_sha256": digest_file(Path(__file__)),
        "checker_sha256": digest_file(repo/"search/check_d972_b345_relfrat3_wordexpr_v8.py"),
        "driver_sha256": digest_file(repo/"search/d972_b345_relfrat3_wordexpr_gha_driver_v8.g"),
    }
    receipt = make_base_receipt(q3_path, output_path, q3_data, source_hashes,
                                "B345_RELFRAT3_WORDEXPR_UNKNOWN_RESOURCE",
                ...
                try:
                    ...
                    if missing is not None:
                        if correction_index not in blockers and \
                            ...
                            raise ResourceStop("blocker_table")
                    ...
                    for translation_blob, translation, occurrence in candidates:
                        ...
                        for relator_index in range(1, 12):
                            if len(directed_column_rows) >= CAPS["directed_columns"]:
                                raise ResourceStop("directed_columns")
...
def self_test_wordexpr_v8() -> None:
    ...
    evaluator = WordExprEvaluator(expr, q)  # type: ignore[arg-type]
    evaluator.evaluate_values()
    ...
    long_eval = WordExprEvaluator(long_expr, q)  # type: ignore[arg-type]
    ...
    print("D972_B345_RELFRAT3_WORDEXPR_V8_PRODUCER_SELFTEST_PASS "
          "product=1 inverse=1 substitution=1 negative_prefix=1 "
          "long_unflattened=262144 scan=4096 terminals=4")
...
if __name__ == "__main__":
    try:
        ...
        print(f"B345_RELFRAT3_WORDEXPR_V8_PRODUCER_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
