#############################################################################
## A0 v27 two-phase low-memory resume driver. ASCII only; fail closed.
#############################################################################
...
D407Resume:="ci/in/prior/d972_r07_history_free_positive_fast_resume_v24_checkpoint.json";;
D407Base:="ci/out/d972_r07_history_free_positive_fast_resume_v27_production";;
...
D407Sentinel:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V27_DRIVER_PASS";;
...
for D407Path in [D407Receipt,D407Checkpoint,D407Verdict,D407ProducerLog,
                 ...
                 D407Shell,D407OK] do
  if IsExistingFile(D407Path) then Error("task407 stale output ",D407Path); fi;
od;
if not IsExistingFile(D407Zip) or not IsExistingFile(D407Manifest) or
   not IsExistingFile(D407Resume) then Error("task407 missing input"); fi;
D407Stream:=OutputTextFile(D407Shell,false);;
if D407Stream=fail then Error("task407 shell open"); fi;
...
if D407Observed<>D407Sentinel then Error("task407 sentinel mismatch"); fi;
Print(D407Sentinel,"\n");;
