# suite-wp2-shard-b2.g -- WP2a GAP explorer, shard B2 (n=15,16 + N5 control)
#
# 実行: .\gap.ps1 search\suite-wp2-shard-b2.g
#
...
# コストが支配的と推定 -- 原因判定は司令塔)、さらに決定的分割する(司令塔指示 2026-07-19)。
# 分割は対象集合の分割のみであり、各対象の計算内容・出力形式は無変更(元スクリプトの
# 該当コードをそのままコピー -- BuildCertJsonDihedral/BuildCertJsonN5 内の
# "generated_by".script 文字列リテラルも含め無変更。証明書 JSON のバイト内容を保つため
# -- 変更すると provenance/cert-hashes-wp2.txt の記録済みハッシュと不一致になる)。
#
# 宇宙分担(事前登録宇宙 {3..16,18,36,N5} の厳密な分割):
#   shard A (suite-wp2-shard-a.g): n = 3..12
#   shard B1 (suite-wp2-shard-b1.g): n = 13,14
#   shard B2 (本ファイル): n = 15,16 + N5
...
#   n=15,16 は reduction branch suite に含まれない(元スクリプトでも reduction は空配列)。
#
...
# ================= メインドライバ: shard B2 -- n=15,16 + N5 =================
universe := [15,16];;
results := [];;
Print("宇宙 shard B2 (事前登録宇宙 {3..16,18,36,N5} のうち n=15,16 + N5): ", universe, " + N5\n");
...
Print("\n累計 elapsed ms (n=15,16 ProcessDihedral): ", Runtime()-startTime, "\n");

reductionsByQ := List([1..40], i -> []);;
Print("(n=15,16 は reduction branch suite に含まれない -- 空配列のまま)\n");
...
Print("\nN5: shadows=", Length(n5res.shadows), " (expect m in {0,1,3,4})  tcOk=", n5res.tcOk,
      "  process_ms=", Runtime()-t0, "\n");
t0 := Runtime();
WriteFile("certificates/N5.v1.json", BuildCertJsonN5(n5res));
Print("wrote certificates/N5.v1.json  (write_ms=", Runtime()-t0, ")\n");
