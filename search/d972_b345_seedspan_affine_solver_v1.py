...
###############################################################################
# 157eb: exact 104-seed raw-Fox affine lane.
###############################################################################
...
def affine_monitor_receipt(monitor: ResourceMonitor, hit: bool) -> dict[str, Any]:
    ...
    row["terminal_on_hit"] = "B345_SEEDSPAN_AFFINE_UNKNOWN_RESOURCE"
...
def affine_seed_words(q3_data: dict[str, Any], e3: MatchedQuotient) \
        -> dict[str, Any]:
    """Rebuild exactly the 26 cube/104 commutator prefix, never the BFS."""
    cubes: list[list[int]] = []
    seen_cubes: set[tuple[int, ...]] = set()
    for row in q3_data["correction_fibre"]["records"]:
        word = list(row["word"])
        if not word:
            continue
        cube = reduce_word(word + word + word)
        key = tuple(cube)
        if key not in seen_cubes:
            seen_cubes.add(key)
            cubes.append(cube)
    ...
    return {
        ...
        "seed_words": seeds, "seed_count": len(seeds),
...
def _affine_delta_matrix_rank(delta_rows: dict[Any, dict[int, int]],
    ...
    """Rank of the current target's 104-column delta map over F3."""
...
def _affine_target_row(system: AffineSystem, base_remainder: dict[tuple[int, str], int],
                       ...
                       target_ordinal: int) -> dict[str, Any]:
...
def _affine_target_row_transposed(
        ...
        monitor: ResourceMonitor | None = None) -> dict[str, Any]:
...
def _affine_source_preflight(seeds: Sequence[Sequence[int]], e4: MatchedQuotient,
    ...
    require(len(contexts) <= CAPS["cheap_contexts"] and
    ...
    require(len(records) == 104, "affine source preflight count")
    return {"supported": True, "seed_count": 104,
...
def affine_make_receipt(q3_data: dict[str, Any], status: str, reason: str,
    ...
    claims = ({"claim_classification": "positive_exact_seedspan_certificate",
               ...
               "negative_claimed": False, "B4_A_claimed": False,
               "B4_B_claimed": False}
              if status == "B345_SEEDSPAN_AFFINE_POSITIVE" else no_claims)
    return {
        ...
        "registered_universe": {"kind": "ordered_104_seed_affine_span",
                                 "seed_count": 104, "cube_count": 26,
...
def affine_run(q3_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    pinned = [(repo/Q3_PRODUCER, Q3_PRODUCER_SHA, "q3 producer"),
              (repo/Q3_CHECKER, Q3_CHECKER_SHA, "q3 checker"),
              (repo/Q3_DRIVER, Q3_DRIVER_SHA, "q3 driver"),
    ...
    source_hashes = {
        ...
        "checker_sha256": _affine_digest_or_missing(
            repo/"search/check_d972_b345_seedspan_affine_solver_v1.py"),
    ...
    receipt = affine_make_receipt(
        q3_data,
        "B345_SEEDSPAN_AFFINE_UNKNOWN_INPUT" if input_error else
        "B345_SEEDSPAN_AFFINE_UNKNOWN_RESOURCE",
    ...
    try:
        ...
        # Target-major: only the current target's 104 sparse remainders live.
        for ordinal, (name, kind, base_root) in enumerate(base_candidate["acceptance"], 1):
            ...
            if ordinal <= 5:
                # The five charming-error rows are the shared IDENTITY node
                # by construction.  Gate its opcode/topology once and record
                # the exact 104 zero split columns; invoking 208 full
                # evaluators here would add no mathematics and would obscure
                # the first informative target.
                ...
                current_seed_index = 104
                live_remainder_entries = 0
                target_row = _affine_target_row(
                    system, {}, [{} for _ in range(104)], ordinal)
                ...
                receipt["affine_system"] = {
                    "variables": 104, "rank": system.rank(),
                    "nullity": system.nullity(),
                    "consistent": system.consistent,
                    "row_space_sha256": system.digest()}
                print("D972_B345_SEEDSPAN_AFFINE_PROGRESS " +
            ...
            for seed_index, seed in enumerate(seed_info["seed_words"], 1):
                ...
                # all-target, all-104 load-bearing affine certificate.
                ...
                require(typed_value == e4.identity and typed_raw == seed_raw,
                        f"typed/flat raw chain split {name}/{seed_index}")
                split_ledger.append({
            ...
            target_row = _affine_target_row_transposed(
                ...
                live_remainder_entries, monitor)
            target_row.update({"name": name, "kind": kind,
                               "diagnostics_excluded": True,
            ...
            current_seed_index = 104
            receipt["targets"] = target_rows
            receipt["affine_system"] = {"variables": 104, "rank": system.rank(),
                                         "nullity": system.nullity(),
                                         "consistent": system.consistent,
                                         "row_space_sha256": system.digest()}
            print("D972_B345_SEEDSPAN_AFFINE_PROGRESS " +
...
def affine_self_test() -> None:
    ...
    transactional_system = AffineSystem(2)
    ...
    transactional_rows = {
        (0, f"coordinate-{index}"): 0 for index in range(1024)}
    try:
    ...
    source_boundary = {"phase": "affine_source_preflight", "seed": 104}
    ...
    for token in AFFINE_TERMINALS:
        require(token.startswith("B345_SEEDSPAN_AFFINE_"),
    ...
    print("D972_B345_SEEDSPAN_AFFINE_PRODUCER_SELFTEST_PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(f"B345_SEEDSPAN_AFFINE_PRODUCER_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
