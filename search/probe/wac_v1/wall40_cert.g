#############################################################################
## search/probe/wac_v1/wall40_cert.g
##  n=40 の非可解窓(ell=37, t=3, C37 x S3, |C(w0)|=222)の正式 cert 化 probe。
...
##  HIT レコード(ell=37, t=3, n=40)-- 実データは
##  scratchpad/mine_dl/tmax-scan-37-41-20260801/mine-run-tmax-scan-37-41-20260801-ell37/
##  mine/out/tmax-scan-37-41-20260801/ell37/tmax_scan_37_ell37_20260731.json
##  (sha256=96e9a6d16ce2970973fb4272c61b3a09bd3aae5eefa47f1143c549207641c67f、
##  report 記載値と一致確認済み)の records[0].t_records[t=3]から逐語。
...
Sha256OfString := function(s)
  ...
  tmp := "search/.tmp_wall40_cert_sha.txt";
  out := "search/.tmp_wall40_cert_sha.out";
  ...
  if line = fail or Length(line) < 64 then
    Error("wall40_cert.g: Sha256OfString: sha256sum did not return a hash line");
  fi;
  return line{[1 .. 64]};
end;;
...
ComputeSha256File := function(relpath)
  local tmp, f, line;
  tmp := "search/.tmp_wall40_cert_selfsha.txt";
...
ValidateJsonFile := function(path)
  ...
  if not ok then
    Error("wall40_cert.g: ValidateJsonFile: python json.load failed to parse ", path,
          " -- got: ", line);
  fi;
  return true;
end;;
...
#############################################################################
## ---- witness (n=40, ell=37, t=3; tmax_scan_37_ell37_20260731.json
##      (scratchpad/mine_dl, sha256 一致確認済み)の該当 HIT レコードから逐語) ----
#############################################################################
n := 40;;
a1 := (1,36)(2,13)(3,19)(4,30)(5,22)(6,8)(7,39)(9,21)(10,11)(12,20)(14,35)(15,16)(17,34)(18,31)(23,29)(24,26)(25,40)(27,28)(32,33)(37,38);;
b1 := (1,35,13)(2,12,19)(3,18,30)(4,29,22)(5,21,8)(6,7,39)(9,20,11)(14,34,16)(17,33,31)(23,28,26)(24,25,40)(36,37,38);;
Snn := SymmetricGroup(n);;  Ann := AlternatingGroup(n);;

Print("=== P-WALL-40 窓 assert (n=", n, ") ===\n");
Print("a1 = ", a1, "\nb1 = ", b1, "\n");
...
#############################################################################
lid1Str := Concatenation("LID1/v1|id=P-WALL-40|n=", String(n),
             "|a1=", String(a1), "|b1=", String(b1),
             "|S1=", String(s1), "|S2=", String(s2));;
lid1Sha := Sha256OfString(lid1Str);;
Print("\n  LID-1 = ", lid1Str, "\n  LID-1 sha256 = ", lid1Sha, "\n");

selfSha := ComputeSha256File("search/probe/wac_v1/wall40_cert.g");;

#############################################################################
## ---- JSON 出力 ----
#############################################################################
if isSmoke then
  outName := "search/certs/wall40_cert_20260801_smoke.json";;
else
  outName := "search/certs/wall40_cert_20260801.json";;
fi;
...
cert := Concatenation(
  ...
  "  \"window_label\":\"P-WALL-40\",\n",
  "  \"f_orientation\":\"probe11_handwritten_hexagon\",\n",
  "  \"source_witness\":\"tmax_scan_37_ell37_20260731.json (mine/reports/tmax-scan-37-41-20260801_report.md 参照; ell=37, t=3, n=40, stage=HIT レコード; sha256=96e9a6d16ce2970973fb4272c61b3a09bd3aae5eefa47f1143c549207641c67f), 逐語\",\n",
