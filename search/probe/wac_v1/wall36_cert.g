#############################################################################
## search/probe/wac_v1/wall36_cert.g
##  n=36 の非可解窓(ell=31, t=5, C31 x S5, |C(w0)|=3720)の正式 cert 化 probe。
...
##  の records[ell=31].t_records[t=5](n=36, stage="HIT")から逐語。検算ロジックは
...
Sha256OfString := function(s)
  ...
  tmp := "search/.tmp_wall36_cert_sha.txt";
  out := "search/.tmp_wall36_cert_sha.out";
  ...
  if line = fail or Length(line) < 64 then
    Error("wall36_cert.g: Sha256OfString: sha256sum did not return a hash line");
  fi;
  return line{[1 .. 64]};
end;;
...
ComputeSha256File := function(relpath)
  local tmp, f, line;
  tmp := "search/.tmp_wall36_cert_selfsha.txt";
...
ValidateJsonFile := function(path)
  ...
  if not ok then
    Error("wall36_cert.g: ValidateJsonFile: python json.load failed to parse ", path,
          " -- got: ", line);
  fi;
  return true;
end;;
...
#############################################################################
## ---- witness (n=36, ell=31, t=5; search/certs/scans/tmax_scan_29-31_
##      ell29-31_20260731.json の該当 HIT レコードから逐語) ----
#############################################################################
n := 36;;
a1 := (1,3)(2,34)(4,29)(5,25)(6,16)(7,9)(8,35)(10,15)(11,12)(13,14)(17,24)(18,20)(19,32)(21,23)(22,33)(26,28)(27,36)(30,31);;
b1 := (1,2,34)(3,31,29)(4,28,25)(5,24,16)(6,15,9)(7,8,35)(10,14,12)(17,23,20)(18,19,32)(21,22,33)(26,27,36);;
Snn := SymmetricGroup(n);;  Ann := AlternatingGroup(n);;

Print("=== P-WALL-36 窓 assert (n=", n, ") ===\n");
Print("a1 = ", a1, "\nb1 = ", b1, "\n");
...
#############################################################################
lid1Str := Concatenation("LID1/v1|id=P-WALL-36|n=", String(n),
             "|a1=", String(a1), "|b1=", String(b1),
             "|S1=", String(s1), "|S2=", String(s2));;
lid1Sha := Sha256OfString(lid1Str);;
Print("\n  LID-1 = ", lid1Str, "\n  LID-1 sha256 = ", lid1Sha, "\n");

selfSha := ComputeSha256File("search/probe/wac_v1/wall36_cert.g");;

#############################################################################
## ---- JSON 出力 ----
#############################################################################
if isSmoke then
  outName := "search/certs/wall36_cert_20260731_smoke.json";;
else
  outName := "search/certs/wall36_cert_20260731.json";;
fi;
...
cert := Concatenation(
  ...
  "  \"source_witness\":\"search/certs/scans/tmax_scan_29-31_ell29-31_20260731.json (ell=31, t=5, n=36, stage=HIT レコード), 逐語\",\n",
