...
import os; exec(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'u_meas_caseb_search2.py')).read().split('if __name__')[0])
...
def scan(p, log):
    ...
    out = []; t0 = time.time()
    for a in range(1, p):
        b = (a*a*quarter) % p; c = (-27*a*a*a*pow(8,p-2,p)) % p
        f6 = [c, 0, b, 0, a, 0, 1]
