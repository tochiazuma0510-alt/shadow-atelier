#############################################################################
## search/probe/wac_v1/wall2_cert.g
##  裁定241 工程1(便89監査資料): P-WALL-2(n=24)の再走可能・独立 cert 生成 probe。
##  witness (a1,b1) は ops/express/20260731_数学者Opus_壁到達と窓2枚.md 窓A
##  (壁P4)から逐語。検算ロジックは search/probe/wac_v1/sat_l1_probe11.g を
...
Sha256OfString := function(s)
  ...
  tmp := "search/.tmp_wall2_cert_sha.txt";
  out := "search/.tmp_wall2_cert_sha.out";
  ...
  if line = fail or Length(line) < 64 then
    Error("wall2_cert.g: Sha256OfString: sha256sum did not return a hash line");
  fi;
  return line{[1 .. 64]};
end;;
...
ComputeSha256File := function(relpath)
  local tmp, f, line;
  tmp := "search/.tmp_wall2_cert_selfsha.txt";
...
#############################################################################
## ---- witness (窓 A / P-WALL-2, n=24; 速達文書の窓Aから逐語) ----
#############################################################################
n := 24;;
...
## 具体的な witness で評価すると、probe11/probe13(数学者 Opus 5 検算済み・
...
#############################################################################
lid1Str := Concatenation("LID1/v1|id=P-WALL-2|n=", String(n),
             "|a1=", String(a1), "|b1=", String(b1),
             "|S1=", String(s1), "|S2=", String(s2));;
lid1Sha := Sha256OfString(lid1Str);;
Print("\n  LID-1 = ", lid1Str, "\n  LID-1 sha256 = ", lid1Sha, "\n");

selfSha := ComputeSha256File("search/probe/wac_v1/wall2_cert.g");;
...
cert := Concatenation(
  "{\n",
  "  \"schema\":\"wac_v1-wall2-cert/v1\",\n",
  "  \"generated_by\":\"search/probe/wac_v1/wall2_cert.g\",\n",
  "  \"window_label\":\"P-WALL-2\",\n",
  "  \"source_witness\":\"ops/express/20260731_数学者Opus_壁到達と窓2枚.md 窓A(壁P4), 逐語\",\n",
...
WriteFile("search/certs/wall2_cert_20260731.json", cert);;
Print("\nWrote search/certs/wall2_cert_20260731.json\n");
Print("\nWALL2_CERT_DONE\n");
QUIT;
