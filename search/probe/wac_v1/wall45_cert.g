#############################################################################
## search/probe/wac_v1/wall45_cert.g
##  n=45 の非可解窓(ell=41, t=4, C41 x S4, |C(w0)|=984)の正式 cert 化 probe。
##  wall36_cert.g(P-WALL-36, n=36)と同型(逐語複製・n/witness/出力名のみ差替)。
##  witness (a1, b1) は mine/reports/tmax-scan-37-41-20260801_report.md 参照の
##  HIT レコード(ell=41, t=4, n=45)-- 実データは
##  scratchpad/mine_dl/tmax-scan-37-41-20260801/mine-run-tmax-scan-37-41-20260801-ell41/
##  mine/out/tmax-scan-37-41-20260801/ell41/tmax_scan_41_ell41_20260731.json
##  (sha256=acef861defde5a452dc2540b2aa9568e00a5dd4617b3213763c0bfb449525232、
##  report 記載値と一致確認済み)の records[0].t_records[t=4]から逐語。
##  この cert は collector の schema 未対応で search/certs/scans/ へ未収蔵の
##  ため、mine 実行時の scratchpad 保存物から転記した(司令塔指示は
##  「search/certs/scans/」を挙げていたが実体はここに未収蔵 -- sha256 一致で
##  確認済みなので実装は続行し、この逸脱を報告する)。検算ロジックは
##  wall36_cert.g をそのまま踏襲する(hexagon 判定は search/kerchi-judge.g の
...
Sha256OfString := function(s)
  ...
  tmp := "search/.tmp_wall45_cert_sha.txt";
  out := "search/.tmp_wall45_cert_sha.out";
  ...
  if line = fail or Length(line) < 64 then
    Error("wall45_cert.g: Sha256OfString: sha256sum did not return a hash line");
  fi;
  return line{[1 .. 64]};
end;;
...
ComputeSha256File := function(relpath)
  local tmp, f, line;
  tmp := "search/.tmp_wall45_cert_selfsha.txt";
...
ValidateJsonFile := function(path)
  ...
  if not ok then
    Error("wall45_cert.g: ValidateJsonFile: python json.load failed to parse ", path,
          " -- got: ", line);
  fi;
  return true;
end;;
...
#############################################################################
## ---- witness (n=45, ell=41, t=4; tmax_scan_41_ell41_20260731.json
##      (scratchpad/mine_dl, sha256 一致確認済み)の該当 HIT レコードから逐語) ----
#############################################################################
n := 45;;
a1 := (1,27)(2,3)(4,26)(5,23)(6,13)(7,9)(8,42)(10,12)(11,43)(14,22)(15,20)(16,35)(17,31)(18,40)(19,36)(24,25)(28,29)(30,41)(32,34)(33,44)(37,39)(38,45);;
b1 := (1,26,3)(4,25,23)(5,22,13)(6,12,9)(7,8,42)(10,11,43)(14,21,20)(15,19,35)(16,34,31)(17,30,40)(18,39,36)(27,41,29)(32,33,44)(37,38,45);;
Snn := SymmetricGroup(n);;  Ann := AlternatingGroup(n);;

Print("=== P-WALL-45 窓 assert (n=", n, ") ===\n");
Print("a1 = ", a1, "\nb1 = ", b1, "\n");
...
#############################################################################
lid1Str := Concatenation("LID1/v1|id=P-WALL-45|n=", String(n),
             "|a1=", String(a1), "|b1=", String(b1),
             "|S1=", String(s1), "|S2=", String(s2));;
lid1Sha := Sha256OfString(lid1Str);;
Print("\n  LID-1 = ", lid1Str, "\n  LID-1 sha256 = ", lid1Sha, "\n");

selfSha := ComputeSha256File("search/probe/wac_v1/wall45_cert.g");;

#############################################################################
## ---- JSON 出力 ----
#############################################################################
if isSmoke then
  outName := "search/certs/wall45_cert_20260801_smoke.json";;
else
  outName := "search/certs/wall45_cert_20260801.json";;
fi;
...
cert := Concatenation(
  ...
  "  \"window_label\":\"P-WALL-45\",\n",
  "  \"f_orientation\":\"probe11_handwritten_hexagon\",\n",
  "  \"source_witness\":\"tmax_scan_41_ell41_20260731.json (mine/reports/tmax-scan-37-41-20260801_report.md 参照; ell=41, t=4, n=45, stage=HIT レコード; sha256=acef861defde5a452dc2540b2aa9568e00a5dd4617b3213763c0bfb449525232), 逐語\",\n",
