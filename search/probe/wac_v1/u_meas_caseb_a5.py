...
CERT = "search/certs/u_meas_caseb_a5_20260731.json"
rep = {"schema":"u-meas-caseb-a5/v1","sympy_version":sp.__version__,"prime":7,"model":"a=-2/5, b=1/25, c=-512/421875 (candidate from CB-27 sqrt(-5) descent)","u_touched":False,"uniqueness_claimed":False,"checkpoints":[]}

A = sp.Rational(-2,5); CVAL = sp.Rational(-512, 421875)
log("a=%s b=%s c=%s" % (A, A**2/4, CVAL))
x = sp.symbols('x'); c3, c5, c7, c9 = sp.symbols('c3 c5 c7 c9'); h4, h2, h0 = sp.symbols('h4 h2 h0')
q = x**3 + A/2*x; f6 = sp.expand(q**2 + CVAL)
...
SEEDS=[list(t) for t in _pr(range(7),repeat=4) if t[3]!=0 and all(ev(d,list(t),7)==0 for d in POLYS)]
log("F_7 points: %s"%SEEDS)
seed=None
for _s in SEEDS:
    for _cb in combinations(range(len(POLYS)),4):
        _J=[[dev(POLYS[i],_s,7,j) for j in range(4)] for i in _cb]
        if detmod(_J,7)%7!=0: seed=_s; break
    if seed: break
log("chosen seed (Jacobian invertible): %s"%seed)
