#############################################################################
## search/probe/wac_v1/wall2_cert_judge.g
##  Sol 便89 §9 是正(sol/裁定_246_便89検収と定理CENT.md 差戻し5「証明書は
##  judge 規約での versioned 再発行が必要」): P-WALL-2(n=24)の cert を、
##  search/probe/wac_v1/wall2_cert.g の手書き規約(mathematician_handwritten)
##  ではなく、**judge 規約**(左共役 -- docs/notes/hexagon_orientation_ruling_v1.md
##  §1 が正本: f_judge = f_hand^{-1})で再発行する。
##
##  witness (a1,b1) は wall2_cert.g と同一(ops/express/20260731_数学者Opus_
##  壁到達と窓2枚.md 窓A、逐語)。窓構成 (aE,bE,s1,s2) も同一。
##
##  差はただ一点: hexagon 判定式。wall2_cert.g は probe11/13 の検算済み
##  手書き式 (s1*f^-1*s2*f=f^-1*s1*s2 かつ f^-1*s2*f*s1=s2*s1*f) を
##  f=(a1^z)*a1 に適用した。本 script は **judge の実物条件**
##  (AbstractProd/TH/RtOf -- search/kerchi-judge.g 146-165 行を sed で
##  機械抽出した search/probe/wac_v1/_judge_core_extract.g をそのまま
##  再利用、改変ゼロ)を **fj := a1*(a1^z)**(= 訂正版 定理 SURV(judge 規約)、
##  hexagon_orientation_ruling_v1.md §3)に適用する。
##  これは search/probe/wac_v1/sat_l1_probe17.g(数学者 Opus 5・裁定用確定
##  実験)が既に実証した式そのものであり、本 script はそれを cert 化する
##  だけで判定ロジック自体は変更しない(不可侵条項の遵守)。
##
...
Sha256OfString := function(s)
  ...
  tmp := "search/.tmp_wall2j_cert_sha.txt";
  out := "search/.tmp_wall2j_cert_sha.out";
  ...
  if line = fail or Length(line) < 64 then
    Error("wall2_cert_judge.g: Sha256OfString: sha256sum did not return a hash line");
  fi;
  return line{[1 .. 64]};
end;;
...
ComputeSha256File := function(relpath)
  local tmp, f, line;
  tmp := "search/.tmp_wall2j_cert_selfsha.txt";
...
#############################################################################
## ---- witness (窓 A / P-WALL-2, n=24; wall2_cert.g と同一・逐語) ----
#############################################################################
n := 24;;
...
Print("=== P-WALL-2 窓 assert (n=", n, ", judge 規約) ===\n");
Print("a1 = ", a1, "\nb1 = ", b1, "\n");
...
## _judge_core_extract.g's MakeWindow -- sed-verbatim from kerchi-judge.g,
## byte-for-byte the judge's own construction (not a re-derivation).
W := MakeWindow(s1, s2);;
one := Identity(W.Bq);;
cIsOne := (W.c = one);;
pEqAn := (W.PN = Ann);;
...
#############################################################################
## ---- (iii) SURV 構成 judge 規約 fj=a1*(a1^z) の全数検算 ----
## hexagon_orientation_ruling_v1.md §3「訂正版 定理 SURV(judge 規約)」:
##   fj := a1*(a1^z)  (= wall2_cert.g の f=(a1^z)*a1 の逆元、両者 a1 が
##   対合なので fj = f^-1)
## judge の実物条件をそのまま fj に適用する(AbstractProd/TH/RtOf は
## _judge_core_extract.g -- kerchi-judge.g からの sed 逐語抽出・改変ゼロ):
##   AbstractProd([fj, TH(W,fj)]) = one  かつ  RtOf(W,0,fj) = W.c^0
## 生成条件(judge 規約, sat_l1_probe17.g の okGenI 分岐と同一式):
##   Group(W.x, AbstractProd([fj^-1, W.y, fj])) = W.PN
#############################################################################
...
#############################################################################
lid1Str := Concatenation("LID1/v1|id=P-WALL-2|orient=judge|n=", String(n),
             "|a1=", String(a1), "|b1=", String(b1),
             "|S1=", String(s1), "|S2=", String(s2));;
lid1Sha := Sha256OfString(lid1Str);;
Print("\n  LID-1 = ", lid1Str, "\n  LID-1 sha256 = ", lid1Sha, "\n");

selfSha := ComputeSha256File("search/probe/wac_v1/wall2_cert_judge.g");;
...
WriteFile("search/certs/wall2_cert_judge_20260731.json", cert);;
Print("\nWrote search/certs/wall2_cert_judge_20260731.json\n");
Print("\nWALL2_CERT_JUDGE_DONE\n");
QUIT;
