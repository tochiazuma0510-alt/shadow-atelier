...
BuildConvP := function(Pgroup, xbar, ybar, jx, jy, epi)
  local D, pcgsD, n, FXY, Fx, Fy, epiFree, ws, Ghat, i, w, gateOk, wImg,
        Qgrp, convHom, convImage, convIso;
  ...
  ## Physical map gate for CONV-INJ.  Checking six connection equations is
  ## not enough if a common image collapse were introduced.  Construct the
  ## homomorphism D -> Q from the six images, identify its generated image,
  ## then retype the codomain to that image and demand a genuine bijection.
  Qgrp := Range(epi);
  convHom := GroupHomomorphismByImages(D, Qgrp, List(pcgsD, q -> q), Ghat);
  if convHom = fail then
    Error("CONV_P_STOP: six Ghat images do not define a homomorphism D -> Q");
  fi;
  convImage := Subgroup(Qgrp, Ghat);
  if Image(convHom) <> convImage then
    Error("CONV_P_STOP: computed map image differs from subgroup generated by Ghat");
  fi;
  convIso := GroupHomomorphismByImages(D, convImage, List(pcgsD, q -> q), Ghat);
  if convIso = fail or not IsBijective(convIso) then
    Error("CONV_P_STOP: D -> <Ghat_i> failed explicit bijectivity gate (CONV-INJ)");
  fi;
  Print("CONV-P map gate: Image=<Ghat_i> and D -> Image is bijective. PASS.\n");

  return rec(P := Pgroup, D := D, pcgsD := pcgsD, ws := ws, Ghat := Ghat,
             n := n, map := convHom, image := convImage,
             image_generated := true, image_bijective := true);
end;;
