...
## Lane S judgement predicate library for HS main-run (prereg v2).
## NOT a rewrite: the block between BEGIN/END VERBATIM below is a literal,
## byte-for-byte copy of lines 1-69 of the calibration driver
## search/probe/hsp7_cond4_laneS/driver_step3_eval_candidates.g (the same
## file cited by prereg v1/v2 SS1.3 table row "Lane S"). This file supplies
## only group construction (P, x, y) + the judgement predicates
## (theta, tau, Hex310, Hex311, Hex310r, Hex311r) as reusable bindings; it
## does not loop over candidates and does not call QUIT (unlike the
## calibration driver, which is a standalone script). The main-run lane
## wrapper (lane_wrapper_S.g) Read()s this file and then drives the
## judgement predicate over a shard of the frozen candidate universe.
##
## Byte-identity check: scratchpad verification (implementer, 2026-08-05)
## extracted lines 1-69 of the calibration driver via `sed -n '1,69p'` and
## diffed byte-for-byte against the VERBATIM block below; diff exit code 0
## (zero differences). See prereg v2 SS2/appendix-digest table for the
## recorded SHA-256 of this extracted region.
##
## ===== BEGIN VERBATIM (driver_step3_eval_candidates.g lines 1-69) =====
