# 便100修文 補題 DUM-HEX の検算(独立・整数演算のみ)-- v2
#
# ★ v2 の差分 = 冒頭コメント (3) の式のみ(便101 W101-1.2 の一行 erratum)。
#   計算コード(import 以下)は v1 と同一。数値結果は v1 と一致する(14 検査・FAILS=0)。
#   v1 = search/probe/hsp7_v1/hs_prop7_dumhex_check.py
#        sha256 = 7b1c7e067a13a24de2200b60d226a8c58fe7ada44567753365827629fad8df5f
#        (★ v1 は記録として不改変。本 v2 が effective source -- CV-10)
#
...
#   (3) ★ 一般形(精確化 PREC-1・便101 F101-1.4 で PASS):
#         (1+tau_*+tau_*^2)(alpha v1 + beta v2 + gamma v3)
#             == (2alpha - beta + 2gamma)(v1+v2+v3)      <- 一般形(像は階数 1)
#       ★ 旧 v1 のコメントは右辺を (4alpha-beta) と書いていたが、これは
#         (3.10)-locus すなわち alpha=gamma 上での還元形であって一般形ではない。
#         2alpha-beta+2gamma |_{alpha=gamma} = 4alpha-beta なので locus 上では一致し、
#         h4=(1,4,1) は locus 上にあるため両式とも 0 を与える
#         ==> v1 の数値結果・検査 11/12/13 は無傷(文書 defect のみ・W101-1.2)。
#       ==> hexagon の深さ 4 解空間 = {alpha=gamma} ∩ {2alpha-beta+2gamma=0} = Z(1,4,1)
#           すなわち h4 = (1,4,1) で 0(定理 D4-POWER (a) の結論は不変)
