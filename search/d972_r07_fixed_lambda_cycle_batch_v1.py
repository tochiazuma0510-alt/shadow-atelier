#!/usr/bin/env python3
"""Task994: one fixed-lambda cycle batch with a private durable reduction prefix.
...
C_FILE = "check_d972_r07_fixed_lambda_cycle_batch_v1.py"
...
def canary_selection(l: Any, e: Any, oracle: Any) -> Any:
    rejected, binding = [], canary_binding()
    fixed, store, cochain, selected = canary_selection_fixture(l, e, oracle, binding,
        list(range(5, 40)) + [CHORDS - 1], [2, 1])
    decoded = saved_selection_values(oracle, fixed, store, cochain)
    require(selected["failed_count"] == 36 and len(decoded["witnesses"]) == 32 and
            int(selected["failed_indices"][-1]) == CHORDS - 1 and
            decoded["witnesses"][0]["roster_index"] == 5 and decoded["witnesses"][-1]["roster_index"] == 36 and
            all(item["kind"] == "chord" and len(item["cycles"]) == 6 for item in decoded["witnesses"]) and
            any(item["coefficient"] == 0 for item in decoded["witnesses"][0]["cycles"]),
            "canary_full_roster_before32_and_all_six_factors_including_zero")
    _, _, _, late = canary_selection_fixture(l, e, oracle, binding, [CHORDS - 1], [0, 0])
    require(late["first_failed_index"] == CHORDS - 1 and len(late["witnesses"]) == 1, "canary_last_chord_is_not_skipped")
    aux_fixed, aux_store, aux_cochain, aux = canary_selection_fixture(l, e, oracle, binding, [], [1, 2])
    require(aux["witnesses"][0]["coordinate"] == 0 and len(aux["witnesses"]) == 1 and
            saved_selection_values(oracle, aux_fixed, aux_store, aux_cochain)["selection"]["failed_count"] == 0,
            "canary_first_aux_only_after_all_chords_zero")
    zero_fixed, zero_store, zero_cochain, zero = canary_selection_fixture(l, e, oracle, binding, [], [0, 0])
    require(zero["terminal"] == "COMPLETE_ZERO_CANDIDATE" and
            saved_selection_values(oracle, zero_fixed, zero_store, zero_cochain)["witnesses"] == [],
            "canary_complete_zero_includes_both_aux")
    for name, key, value in (("resealed-other-selection-start", "selection_start_sha256", "f" * 64),
                             ("resealed-bool-ordinal", "ordinal", False),
                             ("resealed-bool-cycle-coefficient", "cycle", True)):
        bad = copy.copy(store)
        bad.values = copy.deepcopy(store.values)
        witness = bad.values["tree"]["witness-roster.json"]["witnesses"][0]
        body = {key: value for key, value in witness.items() if key not in ("schema", "sha256")}
        if key == "cycle":
            body["cycles"][0]["coefficient"] = value
        else:
            body[key] = value
        bad.values["tree"]["witness-roster.json"]["witnesses"][0] = seal("witness", body)
        roster = bad.values["tree"]["witness-roster.json"]
        bad.values["tree"]["witness-roster.json"] = seal("witness-roster",
            {key: value for key, value in roster.items() if key not in ("schema", "sha256")})
        canary_reject(name, lambda: saved_selection_values(oracle, fixed, bad, cochain), rejected)
    for name, key, value in (("resealed-bool-aux-coordinate", "coordinate", False),
                             ("resealed-bool-aux-eta", "eta", [True, False])):
        bad = copy.copy(aux_store)
        bad.values = copy.deepcopy(aux_store.values)
        roster = bad.values["tree"]["witness-roster.json"]
        witness = roster["witnesses"][0]
        body = {field: item for field, item in witness.items() if field not in ("schema", "sha256")}
        body[key] = value
        roster["witnesses"][0] = seal("witness", body)
        bad.values["tree"]["witness-roster.json"] = seal("witness-roster",
            {field: item for field, item in roster.items() if field not in ("schema", "sha256")})
        canary_reject(name, lambda: saved_selection_values(oracle, aux_fixed, bad, aux_cochain), rejected)
    tau, values = store.values["tree"]["chord-tau.u8"], store.values["tree"]["chord-values.u8"]
    canary_reject("early-EOF", lambda: classify_batch(oracle, fixed.geometry["chords"], tau, values,
        values, list(range(5)), np.zeros(5, dtype=np.uint8), cochain["b_aux"], eof=False), rejected)
    canary_reject("truncated-full-roster", lambda: classify_batch(oracle, fixed.geometry["chords"][:-1], tau,
        values, values, list(range(5)), np.zeros(5, dtype=np.uint8), cochain["b_aux"]), rejected)
    return {"name": "fixed-selection-full-roster-and-aux", "status": "PASS", "rejected_cases": rejected}


def canary_physical_fixture() -> Any:
    row = np.zeros(PHYSICAL, dtype=np.uint8)
    row[0] = 1
    raw = encode_array(row, "packed3")[0]
    target, functional = np.zeros(PHYSICAL, dtype=np.uint8), np.zeros(PHYSICAL, dtype=np.uint8)
    target[1:4], functional[1] = [2, 1, 1], 2
    anchor = {"kind": "Separator", "rank": 1, "generation": 7, "head": "1" * 64,
        "records": [{"offer": 6, "rank": 1, "lead": 0, "physical_offset": 0, "rolling_sha256": "1" * 64}],
        "rows": [raw], "leads": [0], "target_raw": encode_array(target, "packed3")[0],
        "lambda": functional, "lambda_raw": encode_array(functional, "packed3")[0],
        "target_parents": [{"synthetic_parent": index} for index in range(97)]}
    sources = [{"kind": "parent-row", "role": "state", "file": "synthetic-physical.bin",
        "file_bytes": len(raw), "file_sha256": sha(raw), "offset": 0, "length": len(raw), "row_sha256": sha(raw)}]
    return anchor, make_reduction_state(anchor, sources)


def canary_reduction_store(l: Any, e: Any, root: Path, binding: Any, state: Any, numeric: Any,
                           corrected: Any, ordinal: int) -> BatchPhaseStore:
    store = BatchPhaseStore(l, e, root, binding, selection_sha="a" * 64, ordinal=ordinal,
                            witness_sha="b" * 64, initial_previous="c" * 64)
    store.hashes = {phase: sha(("synthetic-" + phase).encode()) for phase in CANDIDATE_PHASES[:-1]}
    store.ensure("reduction", lambda: reduction_payloads(binding, "a" * 64, ordinal, "b" * 64,
                                                        state, corrected, numeric, 1))
    return store


def canary_reduction(l: Any, e: Any, oracle: Any, m: Any, directory: Path) -> Any:
    rejected, binding = [], canary_binding()
    anchor, state = canary_physical_fixture()
    corrected = {"record": {"synthetic": "same-source-correction"}, "roots": {"synthetic": "same-P1-roots"}}
    raws = ([0, 2, 2, 0], [0, 2, 2, 0], [0, 2, 1, 0], [0, 2, 2, 1])
    states = []
    for ordinal, leading in enumerate(raws):
        dense = np.zeros(PHYSICAL, dtype=np.uint8)
        dense[:4] = leading
        raw = encode_array(dense, "packed3")[0]
        numeric = reduce_candidate_numeric(oracle, m, anchor["lambda"], state, raw, 1)
        store = canary_reduction_store(l, e, directory / "candidates" / f"{ordinal:06d}",
                                       binding, state, numeric, corrected, ordinal)
        restored = restore_reduction(oracle, state, anchor["lambda"], {"raw": raw}, corrected, store, 1)
        literal = store.values["reduction"]["physical-literal.json"]
        require(len(literal["physical_factors"]) == state["rank"] and
                all(factor["exponent"] == -signrep(factor["coefficient"])
                    for factor in literal["physical_factors"]), "canary_all_ordered_factors_and_minus_sign")
        before = state
        state, candidate, row = publish_candidate_decision(directory, store, before, restored)
        require(before["processed_candidates"] == ordinal and state["processed_candidates"] == ordinal + 1 and
                before["target_parents"] is not state["target_parents"], "canary_no_mutating_alias")
        if ordinal == 0:
            require(numeric["sigma"] == 2 and numeric["target_scalar"] == 2 and
                    literal["outer_exponent"] == -1 and row["target_literal_factor"]["exponent"] == -1,
                    "canary_sigma2_and_theta2_both_public_signs")
            expected_target = (decode_array(before["target_raw"], "packed3", [PHYSICAL]).astype(np.int16) -
                2 * decode_array(numeric["normalized"], "packed3", [PHYSICAL]).astype(np.int16)) % 3
            require(np.array_equal(decode_array(state["target_raw"], "packed3", [PHYSICAL]), expected_target),
                    "canary_actual_target_minus_theta")
            bad = copy.copy(store)
            bad.values = copy.deepcopy(store.values)
            record = bad.values["reduction"]["reduction.json"]
            body = {key: value for key, value in record.items() if key not in ("schema", "sha256")}
            body["target_scalar"] = True
            bad.values["reduction"]["reduction.json"] = seal("reduction", body)
            canary_reject("resealed-bool-target-scalar", lambda: restore_reduction(
                oracle, before, anchor["lambda"], {"raw": raw}, corrected, bad, 1), rejected)
        elif ordinal == 1:
            require(candidate["outcome"] == "DEPENDENT" and row is None and
                    state["rank"] == before["rank"] and state["head"] == before["head"] and
                    state["target_raw"] == before["target_raw"] and literal["outer_exponent"] is None,
                    "canary_dependent_has_no_append_or_word_identity_claim")
        elif ordinal == 2:
            require(numeric["outcome"] == "INDEPENDENT" and numeric["remainder_pairing"] == 0 and
                    numeric["normalized_pairing"] == 0, "canary_independent_row_with_zero_selection_pairing")
            separator = final_separator_numeric(oracle, m, state, anchor["target_raw"])
            require(separator["kind"] == "Separator" and separator["lambda"] is not None,
                    "canary_single_final_reverse_separator")
        else:
            require(not any(state["target_raw"]) and
                    final_separator_numeric(oracle, m, state, anchor["target_raw"])["kind"] == "LinearMembershipCandidate",
                    "canary_actual_Linear_target_zero")
        states.append((before, state, store, restored))
    canary_reject("packed-short-EOF", lambda: decode_array(b"\0" * (PHYSICAL_BYTES - 1), "packed3", [PHYSICAL]), rejected)
    canary_reject("packed-long-EOF", lambda: decode_array(b"\0" * (PHYSICAL_BYTES + 1), "packed3", [PHYSICAL]), rejected)
    canary_reject("packed-two-bit-byte", lambda: decode_array(bytes([81]), "packed3", [4]), rejected)
    canary_reject("packed-nonzero-padding", lambda: decode_array(bytes([3]), "packed3", [1]), rejected)
    canary_reject("bool-is-not-residue", lambda: signrep(True), rejected)
    require(decode_array(bytes([80]), "packed3", [4]).tolist() == [2, 2, 2, 2] and
            encode_array(np.asarray([4294967295], dtype=np.uint32), "u32le")[0] == b"\xff" * 4,
            "canary_registered_base3_and_unsigned_sentinel_layout")
    return {"name": "dependent-independent-target-signs-and-packed", "status": "PASS", "rejected_cases": rejected}, states


def canary_invocation_history(output: Path, prefix: Any, admission: Any, rejected: list[str]) -> None:
    """Exercise normal intake with synthetic, explicitly unadmitted parent metadata."""
    portable = {"schema": SCHEMA + ".acceptance", "parents": [{"role": role, "artifact": {},
        "files": [], "directories": []} for role in ROLES], "anchor": {}, "code": {},
        "runtime": copy.deepcopy(REGISTERED_RUNTIME), "registration": registered_policy()}
    admission.update({"portable": portable, "portable_sha256": sha(canonical(portable)), "acceptance": portable})
    host_paths = {"parents": {role: str((output / "synthetic-input" / role).resolve()) for role in ROLES},
                  "acceptance": str((output / "synthetic-acceptance.json").resolve()), "output": str(output.resolve())}
    acceptance = copy.deepcopy(portable)
    for parent in acceptance["parents"]:
        parent["path"] = host_paths["parents"][parent["role"]]
    directory = output / "invocations"
    directory.mkdir()
    pending = directory / ("." + "6" * 32 + ".json.pending-" + "7" * 32)
    pending.write_bytes(b"synthetic interrupted invocation")
    authenticate_output_roster(output, prefix["files"], 0)
    require(invocation_files(output, admission, prefix, None) == ([], {}), "canary_unformed_nonce_has_zero_invocations")
    def receipt(identity: str) -> Any:
        return seal("invocation", {"id": identity, "portable_acceptance_sha256": admission["portable_sha256"],
            "acceptance_sha256": sha(canonical(acceptance)), **prefix["binding"],
            "registration": registered_policy(), "resume": True, "batch_size": 32, "max_batches": 1,
            ...
            "started_utc": "2000-01-01T00:00:00Z", "launch": {"run": 1, "attempt": 1, "head": "a" * 40,
            "workflow": WORKFLOW}, "host_paths": host_paths})
    originals = {}
    for identity in ("1" * 32, "2" * 32):
        value = receipt(identity)
        name = "invocations/" + identity + ".json"
        originals[name] = canonical(value)
        write_once(output, name, originals[name])
        entries, values = invocation_files(output, admission, prefix, None)
        require(len(entries) == len(originals) and all(value["resume"] is True for value in values.values()),
                "canary_zero_fresh_and_one_or_multiple_bootstrap_receipts")
        prefix["files"].add(name)
    for name, raw in originals.items():
        body = {key: value for key, value in json_bytes(raw).items() if key not in ("schema", "sha256")}
        body["resume"] = False
        atomic_write(output, name, canonical(seal("invocation", body)), replace=True)
    canary_reject("two-regular-fresh-invocations",
        lambda: invocation_files(output, admission, prefix, None), rejected)
    for name, raw in originals.items():
        atomic_write(output, name, raw, replace=True)
    first_name = next(iter(originals))
    first = json_bytes(originals[first_name])
    body = {key: value for key, value in first.items() if key not in ("schema", "sha256")}
    body["processed_candidates_before"] = False
    atomic_write(output, first_name, canonical(seal("invocation", body)), replace=True)
    canary_reject("bootstrap-bool-zero-count",
        lambda: invocation_files(output, admission, prefix, None), rejected)
    atomic_write(output, first_name, originals[first_name], replace=True)
    body = {key: value for key, value in first.items() if key not in ("schema", "sha256")}
    body["acceptance_sha256"] = "f" * 64
    atomic_write(output, first_name, canonical(seal("invocation", body)), replace=True)
    canary_reject("bootstrap-host-acceptance-hash-mismatch",
        lambda: invocation_files(output, admission, prefix, None), rejected)
    atomic_write(output, first_name, originals[first_name], replace=True)


def canary_diagnostics(output: Path, admission: Any, prefix: Any, rejected: list[str]) -> None:
    entries, invocations = invocation_files(output, admission, prefix, None)
    prefix["files"].update(entry["file"] for entry in entries)
    checkpoint = prefix["checkpoints"][0]
    common = {"phase": "synthetic-stopped-prefix", "reason": "interface fixture", "partial": True,
        **{key: prefix["binding"][key] for key in ("owner_sha256", "source_sha256", "start_sha256", "selection_start_sha256")},
        "selection_sha256": None, "invocation_sha256": entries[0]["sha256"],
        "progress_head_sha256": sha(canonical(private_head(checkpoint))), "checkpoint_sha256": sha(canonical(checkpoint)),
        "public_head_sha256": None, "final_manifest_sha256": None,
        **{key: checkpoint[key] for key in ("processed_candidates", "dependent_candidates", "accepted_new_rows", "rank", "generation")},
        "max_seconds": 5400, "max_memory_mib": 7168, "elapsed_seconds": 0.0, **FALSE_ASSURANCE}
    resource = seal("resource-stop", {"status": "UNKNOWN_RESOURCE", "terminal": "UNKNOWN_RESOURCE", **common})
    rejected_value = seal("rejected", {"status": "FAIL", "terminal": "REJECTED", **common})
    write_once(output, "resource-stop.json", canonical(resource))
    write_once(output, "rejected.json", canonical(rejected_value))
    names = admit_diagnostics(output, admission, prefix, invocations, None)
    require(names == {"resource-stop.json", "rejected.json"}, "canary_both_stop_histories_retained")
    prefix["files"].update(names)
    atomic_write(output, "resource-stop.json", canonical(rejected_value), replace=True)
    canary_reject("diagnostic-name-schema-swapped",
        lambda: admit_diagnostics(output, admission, prefix, invocations, None), rejected)
    atomic_write(output, "resource-stop.json", canonical(resource), replace=True)
    for name, field, changed in (("diagnostic-other-owner", "owner_sha256", "f" * 64),
                                  ("diagnostic-unknown-invocation", "invocation_sha256", "e" * 64),
                                  ("diagnostic-uncommitted-checkpoint", "progress_head_sha256", "d" * 64),
                                  ("diagnostic-bool-count", "rank", True)):
        body = {key: value for key, value in resource.items() if key not in ("schema", "sha256")}
        body[field] = changed
        atomic_write(output, "resource-stop.json", canonical(seal("resource-stop", body)), replace=True)
        canary_reject(name, lambda: admit_diagnostics(output, admission, prefix, invocations, None), rejected)
        atomic_write(output, "resource-stop.json", canonical(resource), replace=True)


def canary_publication(l: Any, e: Any, oracle: Any, refinement: Any, m: Any, base: Any,
                       directory: Path, physical_states: Any) -> Any:
    global STOP_REQUESTED, COMPLETED_READONLY, OUTPUT_CREATED
    from types import SimpleNamespace
    rejected = []
    path_args = SimpleNamespace(block_root=[directory / "parents" / f"block-{index}" for index in range(4)],
        acceptance=directory / "receipts" / "acceptance.json", output_root=directory / "safe-new-output")
    for role in ROLES:
        if not role.startswith("block-"):
            setattr(path_args, role + "_root", directory / "parents" / role)
    require(output_path_gate(path_args) == path_args.output_root.resolve() and not path_args.output_root.exists(),
            "canary_path_admission_does_not_create_output")
    for name, destination in (
            ("output-equals-readonly-parent", path_args.state_root),
            ("output-child-of-readonly-parent", path_args.state_root / "uncreated-child"),
            ("output-contains-readonly-parent", directory / "parents"),
            ("output-contains-acceptance", directory / "receipts"),
            ("output-contains-registered-source", SEARCH)):
        path_args.output_root = destination
        canary_reject(name, lambda: output_path_gate(path_args), rejected)
    require(not (path_args.state_root / "uncreated-child").exists(),
            "canary_disjoint_rejection_precedes_every_output_creation")
    output = directory / "bootstrap"
    output.mkdir()
    anchor, state = canary_physical_fixture()
    metadata = {name: seal(kind, {"synthetic_publication_fixture": True}) for name, kind in
        (("parent-layout.json", "parent-layout"), ("source.json", "source"), ("owner.json", "owner"),
         ("start.json", "start"), ("fixed/manifest.json", "fixed-manifest"),
         ("selection/start.json", "selection-start"))}
    intake = {"state": anchor, "fixed": SimpleNamespace(digest="0" * 64), "row_sources": state["row_sources"]}
    admission: dict[str, Any] = {}
    def load() -> Any:
        return load_private_prefix(l, e, oracle, refinement, m, base, admission, intake, metadata, output)
    prefix = load()
    authenticate_output_roster(output, prefix["files"], 0)
    canary_invocation_history(output, prefix, admission, rejected)
    recover_private_metadata(output, prefix, metadata)
    require(not (output / "HEAD").exists() and not (output / "result.json").exists(),
            "canary_private_bootstrap_does_not_publish_physical_HEAD")
    before = inventory(output)
    restored = load()
    same_json(before, inventory(output), "canary_readonly_private_load_preserves_all_bytes")
    canary_diagnostics(output, admission, restored, rejected)
    head_raw = safe_file(output, "progress/HEAD").read_bytes()
    for name, field, value in (("resealed-private-count-bool", "processed_candidates", False),
                               ("resealed-other-owner", "owner_sha256", "f" * 64),
                               ("resealed-private-sequence-bool", "sequence", False)):
        head = json_bytes(head_raw)
        body = {key: item for key, item in head.items() if key not in ("schema", "sha256")}
        body[field] = value
        atomic_write(output, "progress/HEAD", canonical(seal("progress-head", body)), replace=True)
        canary_reject(name, load, rejected)
        atomic_write(output, "progress/HEAD", head_raw, replace=True)
    extra = output / "progress/checkpoints" / ("f" * 64 + ".json")
    extra.write_bytes(canonical(restored["checkpoints"][0]))
    canary_reject("checkpoint-hole-or-unreferenced-normal-extra",
        lambda: authenticate_output_roster(output, restored["files"], 0), rejected)
    extra.unlink()
    canary_reject("two-durable-phases-ahead", lambda: sequence_scope(0, 2), rejected)
    canary_reject("durable-prefix-behind-HEAD", lambda: sequence_scope(2, 1), rejected)
    sequence_scope(0, 1)
    args = SimpleNamespace(batch_size=32, max_seconds=5400, max_memory_mib=7168)
    changed_policy = registered_policy()
    changed_policy["batch_size"] = 1
    canary_reject("resume-cap-reset", lambda: authenticate_registration(changed_policy, args), rejected)
    # An interrupted new invocation has no regular .json receipt yet. Preserve
    # its exact atomic name while counting no additional ordinary invocation.
    nonce = "8" * 32
    pending = output / "invocations" / ("." + nonce + ".json.pending-" + "9" * 32)
    pending.parent.mkdir(exist_ok=True)
    pending.write_bytes(b"unfinished synthetic invocation\n")
    authenticate_output_roster(output, restored["files"], 0)
    require(len(invocation_files(output, admission, restored, None)[0]) == 2, "canary_atomic_invocation_not_counted")
    bad_pending = pending.parent / (".not-a-nonce.json.pending-" + "9" * 32)
    bad_pending.write_bytes(b"bad")
    canary_reject("unregistered-atomic-invocation-name",
        lambda: authenticate_output_roster(output, restored["files"], 0), rejected)
    bad_pending.unlink()
    phase_pending = output / "selection" / (".pending-section-" + "7" * 32)
    phase_pending.mkdir()
    (phase_pending / "partial.bin").write_bytes(b"diagnostic")
    authenticate_output_roster(output, restored["files"], 0)
    link = phase_pending / "forbidden-symlink"
    link.symlink_to(output / "owner.json")
    canary_reject("symlink-inside-named-pending",
        lambda: authenticate_output_roster(output, restored["files"], 0), rejected)
    link.unlink()
    # Complete saved reduction: replay only the durable receipt/publication,
    # never its numeric builder and never increment an already restored state.
    before_state, after_state, store, numeric = physical_states[0]
    builder_calls = []
    require(store.ensure("reduction", lambda: builder_calls.append("unexpected")) is False and not builder_calls,
            "canary_completed_phase_never_calls_builder")
    physical_output = store.root.parent.parent
    original = inventory(physical_output)
    repeated, candidate, row = publish_candidate_decision(physical_output, store, before_state, numeric)
    require(repeated["accepted_new_rows"] == after_state["accepted_new_rows"] == 1 and
            before_state["accepted_new_rows"] == 0, "canary_saved_decision_is_not_double_applied")
    same_json(original, inventory(physical_output), "canary_existing_row_and_decision_recovery_byte_preservation")
    copied = copy.deepcopy(after_state)
    copied["target_parents"][0]["synthetic_parent"] = -1
    require(after_state["target_parents"][0]["synthetic_parent"] == 0 and
            before_state["target_parents"][0]["synthetic_parent"] == 0, "canary_deep_parent_history_isolation")
    # The final serializer is exercised with an actual synthetic nonzero target
    # and reverse separator. Its anchor fixture is not an accepted rank1450 run.
    final_output = directory / "final-publication"
    final_output.mkdir()
    final_state = physical_states[2][1]
    final_binding = canary_binding()
    final_metadata = {"start.json": {"accepted_target_derivation_parents": copy.deepcopy(anchor["target_parents"]),
        "original_rho2_packed_sha256": "0" * 64, "anchor_completed_steps": 64}}
    final_prefix = {"complete_private": True, "binding": final_binding, "state": final_state,
        "selection": {"terminal": "VIOLATION_CANDIDATE", "selected_count": 3},
        "head_sequence": 21, "durable_sequence": 21, "files": set()}
    final = prepare_final(final_output, oracle, m, final_metadata, {"state": anchor}, final_prefix)
    require(not (final_output / "HEAD").exists(), "canary_final_payload_precedes_public_HEAD")
    old_stop = STOP_REQUESTED
    try:
        STOP_REQUESTED = True
        write_once(final_output, "HEAD", canonical(public_head_value(final_binding, final["manifest"])))
        try:
            check_deadline("canary_stop_after_atomic_publication")
        except ResourceStop:
            rejected.append("cooperative-stop-observed-after-publication")
        else:
            raise ValueError("canary_stop_flag_not_observed")
    finally:
        STOP_REQUESTED = old_stop
    loaded_final = read_final(final_output, oracle, m, final_metadata, {"state": anchor}, final_prefix)
    same_json(loaded_final["manifest"], final["manifest"], "canary_saved_final_reuses_separator_without_solve")
    canary_reject("HEAD-without-result-is-not-completed-readonly-resume",
        lambda: authenticate_completed_result(final_output, {}, {}, final_prefix, final, [], {}), rejected)
    altered = copy.deepcopy(final["manifest"])
    body = {key: value for key, value in altered.items() if key not in ("schema", "sha256")}
    body["accepted_new_rows"] = False
    atomic_write(final_output, "final/manifest.json", canonical(seal("final-manifest", body)), replace=True)
    canary_reject("fully-resealed-final-count-change",
        lambda: read_final(final_output, oracle, m, final_metadata, {"state": anchor}, final_prefix), rejected)
    canary_reject("zero-regular-invocations-after-progress",
        lambda: validate_invocation_history([], output, restored), rejected)
    saved_flags = (OUTPUT_CREATED, COMPLETED_READONLY, STOP_REQUESTED)
    readonly_before = inventory(output)
    try:
        OUTPUT_CREATED, COMPLETED_READONLY, STOP_REQUESTED = output, True, True
        completion_log("synthetic-authenticated-readonly-reception")
        probe_args = SimpleNamespace(max_seconds=5400, max_memory_mib=7168)
        first_stop = diagnostic(probe_args, "UNKNOWN_RESOURCE", "synthetic post-authentication stop")
        second_stop = diagnostic(probe_args, "FAIL", "synthetic stdout failure")
        require(first_stop["candidate"] is False and second_stop["candidate"] is False,
                "canary_readonly_failure_is_only_a_non_candidate_external_receipt")
    finally:
        OUTPUT_CREATED, COMPLETED_READONLY, STOP_REQUESTED = saved_flags
    same_json(readonly_before, inventory(output), "canary_completed_readonly_failure_never_writes_packet_diagnostics")
    return {"name": "private-prefix-publication-resume-and-isolation", "status": "PASS", "rejected_cases": rejected}


def selftest(args: Any) -> Any:
    import tempfile
    require(args.max_seconds == 300 and args.max_memory_mib == 7168 and args.batch_size == BATCH_SIZE and
            not args.resume, "new_interface_selftest_registered_limits")
    l, e, oracle, refinement, p2, m, base, descriptors = own_dependencies()
    first = canary_selection(l, e, oracle)
    with tempfile.TemporaryDirectory(prefix="r07-fixed-lambda-batch-new-canary-") as temporary:
        root = Path(temporary)
        second, states = canary_reduction(l, e, oracle, m, root / "physical")
        third = canary_publication(l, e, oracle, refinement, m, base, root, states)
    return seal("selftest", {"status": "PASS", "tests": [first, second, third],
        "fixture_scope": "synthetic full-length cochain roster, small physical span and metadata publication; no actual Omega or rank1450 arithmetic",
        "production_interfaces_used": ["classify_batch", "saved_selection_values", "reduce_candidate_numeric",
            "reduction_payloads", "restore_reduction", "BatchPhaseStore.ensure", "publish_candidate_decision",
            "load_private_prefix", "recover_private_metadata", "authenticate_output_roster", "invocation_files",
            "final_separator_numeric", "prepare_final", "read_final", "public_head_value",
            "authenticate_completed_result", "authenticate_registration", "validate_invocation_history",
            "admit_diagnostics", "completion_log", "diagnostic", "output_path_gate"],
...
def cli() -> Any:
    ...
    if args.selftest:
        require(args.output_root is None and args.acceptance is None and not args.block_root and
        ...
        require(args.output_root is not None and args.acceptance is not None and len(args.block_root) == 4 and
