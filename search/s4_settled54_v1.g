...
# ker(T_{m,f}) cap PB3 = N_S4 cap PB3 at the PB3/N level. N_S4 <= PB3 for this "quotient_ok"/c_in_N
# window per PU-F7 below.
# [versioned 2026-08-12, 裁定974 -- comment-only fix, no code/cert change, per docs/notes/
#  iso_s4_paper_side_v1.md 検分 §2: the ORIGINAL wording here said "the B3/PB3=S3 quotient
#  component of T_{m,f} is untouched by m,f" and cited that as the reason kernel equality at the
#  full B3 level reduces to triviality of Kernel(psi). That reasoning was INSUFFICIENT as stated:
#  "independent of (m,f)" does NOT imply "identity map" does NOT imply "injective" (a fixed-but-
#  nontrivial S3 self-map, e.g. one with kernel A3, would also be "independent of (m,f)" without
#  giving ker(T_{m,f}) subseteq PB3). The CORRECT argument is Prop S3-ID (iso_s4_paper_side_v1.md
#  §2.3, candidate/proof-carrying): u:=2m+1 is odd by definition (Prop 3.2); f in [F2,F2] <= F2 <=
#  PB3 so f maps to 1 under B3->S3, hence the induced S3-self-map sends each transposition s_i to
#  s_i^u; since s_i has order 2 and u is odd, s_i^u = s_i; S3=<s1,s2> is generated by the
#  transpositions, so the induced self-map is the IDENTITY on S3 (not merely "some fixed map"),
#  hence injective, giving ker(T_{m,f}) subseteq PB3 -- this is what actually licenses the
#  kernel-equality reduction used below (and separately licenses the definition of psi itself,
#  cor. S3-ID(b)). This load-bearing step was flagged for mathematician review at implementation
#  time (see the cert's own "b_kernel_equality" caveat in the s4-settled54/v2 output); the review
#  confirmed the CONCLUSION was correct but the stated REASON was not strong enough, hence this
#  versioned comment fix. No code, no computed value, no cert byte changes as a result of this fix.]
kernelEqDetailJson := [];;
