#!/usr/bin/env python3
"""Fixed-candidate sparse-DAG positive lane at relative Frattini-3.
...
SCHEMA = "d972-b345-relative-frattini3-fixed-candidate/v5"
...
OUTPUT_PATH = Path("ci/out/d972_b345_relfrat3_fixed_candidate_v5.json")
...
class ResourceMonitor:
    ...
    def _progress(self, force: bool = False) -> None:
        ...
        print("D972_B345_RELFRAT3_FIXED_CANDIDATE_V5_PROGRESS "
    ...
    def receipt(self, hit: bool) -> dict[str, Any]:
        return {
            ...
            "terminal_on_hit": "B345_RELFRAT3_FIXED_CANDIDATE_UNKNOWN_RESOURCE",
...
def make_base_receipt(q3_path: Path, output_path: Path, q3_data: dict[str, Any],
    ...
    return {
        ...
        "representation_contract": {
            "version": "fixed-candidate-transactional-packed-v5",
        ...
        "theorem_boundary": {
            "proved_if_PASS": "the one registered empty-correction literal outside pair survives every isolated elementary-F3 chief refinement L with Phi3(H4)<=L<=H4",
        ...
        "prohibited_work": {
            ...
            "omitted_corrections_evaluated": 0,
...
def run(q3_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    def record_phase(label: str) -> None:
        ...
        print(f"D972_B345_RELFRAT3_FIXED_V5_PHASE {label} "
    ...
    source_hashes = {
        "producer_sha256": digest_file(Path(__file__)),
        "checker_sha256": digest_file(repo/"search/check_d972_b345_relfrat3_fixed_candidate_v5.py"),
        "driver_sha256": digest_file(repo/"search/d972_b345_relfrat3_fixed_candidate_gha_driver_v5.g"),
    }
    receipt = make_base_receipt(q3_path, output_path, q3_data, source_hashes,
                                "B345_RELFRAT3_FIXED_CANDIDATE_UNKNOWN_RESOURCE",
                                "initializing fixed candidate")
    ...
    try:
        ...
        record_phase("matched_quotients_packed_pool")

        monitor.check("base_replay_fixed_candidate")
        receipt["base_q3_replay"] = replay_base_q3(q3_data, e3, e4)
        record_phase("base_replay_fixed_candidate")
        ...
        require(target_names[5] == "hexagon_1_coface_0" and
        ...
        search_accounting = {
            "method": "shared incremental sparse basis plus candidate-local transactional target streaming",
            "translation_order": "BFS shortlex steps +1..+6,-1..-6",
            ...
            "blocker_history_sha256": digest_obj(blocker_history),
            "pivot_introductions": basis.pivot_introductions,
            "checkpoint_trace": checkpoint_trace,
            "checkpoint_trace_sha256": digest_obj(checkpoint_trace),
        ...
        if selected_candidate is None:
            receipt["status"] = receipt["terminal_token"] = \
                "B345_RELFRAT3_FIXED_CANDIDATE_INCOMPLETE"
            receipt["reason"] = "all registered translations exhausted for correction index 1 without a proof root; blockers are runtime skips only"
            receipt["direct_lane"] = {
                "literal_pair_found": False, "PB5_branch_constructed": False,
                "PB5_reason": "fixed-candidate sparse membership remained incomplete",
            }
            receipt["claim_classification"] = "unknown_not_obstruction"
            receipt["claim_scope"] = "fixed_candidate_only"
        ...
        receipt["resource_stop"] = {
            "cap": exc.reason,
            "candidate_local": search_prefix["current"]["correction_index"] is not None,
...
def self_test_fixed() -> None:
    ...
    class TrivialE4:
        ...
        def eval(word: Sequence[int],
                 images: Sequence[int] | None = None) -> int:
            return 0
    ...
    print("D972_B345_RELFRAT3_FIXED_CANDIDATE_V5_PRODUCER_SELFTEST_PASS "
          "universe=1 dictionary_DP_calls=0 targets=50 blocker_ordinal=6 "
          "skip_retry_replace=1 transaction_rollback_ID_reuse=1 "
          "PASS_regeneration=1 terminals=3 structural_UNKNOWN=1")
...
if __name__ == "__main__":
    try:
        ...
        print(f"B345_RELFRAT3_FIXED_CANDIDATE_PRODUCER_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
