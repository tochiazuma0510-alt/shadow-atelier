#!/usr/bin/env python3
"""Task482 durable-discovery rank-99 continuation (v424/v426/v427)."""
...
MARKER = "R07_A0_DUAL_ANCHORED_RANK99_DURABLE_DISCOVERY_V3"
...
def segment_chain_gate(state: dict[str, Any], c99: dict[str, Any]) -> None:
    ...
    for index, segment in enumerate(segments):
        ...
        need(segment.get("prior_ledger_digest") == ledger and
             ...
             segment.get("prior_state_seal") == expected, "segment:prior_state_seal")
...
def load_resume(path: str, c99: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], bool]:
    ...
    need(value.get("phase") in {"READY", "CLOSED"}, "resume:phase")
...
def replay_prefix(v3: Any, P: dict[str, Any], m: Any, p179: Any, prefix: list[dict[str, Any]],
...
def replay_batch_rows(v3: Any, P: dict[str, Any], m: Any, p179: Any, batch: dict[str, Any],
...
def replay_all(v3: Any, P: dict[str, Any], m: Any, p179: Any, c99: dict[str, Any], state_value: dict[str, Any],
               args: argparse.Namespace, bmod: Any) -> tuple[Any, Any]:
    state, sf = replay_prefix(v3, P, m, p179, c99["accepted_sources"][:8], args, bmod)
    for batch in c99["batches"]:
        state, sf = replay_batch_rows(v3, P, m, p179, batch, state, sf, args, bmod)
...
def delayed_retain(phys: Any, candidate: Any, scalar: int, replay_atom: Any,
    ...
    """The v424 ABI order.  The first reduce is non-mutating."""
    if int(scalar) % 3 == 0:
        return None
...
def update_state_after_batch(previous: dict[str, Any], batch: dict[str, Any], post_state: Any,
    ...
    if active is None:
        ...
        active["prior_state_seal"] = active["prior_core_digest"]
...
def commit_batch(P: dict[str, Any], m: Any, bmod: Any, previous: dict[str, Any],
                 ...
                 writer: Any = write_checkpoint) -> tuple[dict[str, Any], Any, dict[str, Any], dict[str, Any]]:
    ...
    post_profile = bmod.profile(P) if post_state[0] is not None else None
...
def flush_rows(P: dict[str, Any], m: Any, bmod: Any, previous: dict[str, Any],
               ...
               writer: Any = write_checkpoint) -> tuple[dict[str, Any], Any, dict[str, Any] | None,
    ...
    try:
        next_state, post_state, next_active, next_durable = commit_batch(
            ...
            identity, active, args, started, writer)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    state, input_id, _ = load_resume(args.resume, c99)
    # The only durable object before construction is this exact closed span.
    state = sealed(dict(state, phase="BOOTSTRAP", input_checkpoint=input_id, open_batch=False))
    ...
    try:
        ...
        physical_state, sf = replay_all(v3, P, m, p179, c99, state, args, v3.b)
        boundary(args, started, "physical_replay")
        state = sealed(dict(state, phase="READY", input_checkpoint=input_id, open_batch=False))
        durable = write_checkpoint(args.checkpoint, state)
        ready_written = True
        ...
        while True:
                    ...
                    for formula, seed_word in zip(formulas, P["pres"].relators):
                        for coordinate, target in sorted(formula["merged"], key=lambda x: (x[0], x[1])):
                            ...
                            for ordinal, eta in enumerate(sf.ensure_kernel_prefix(coordinate, 9)):
                                ...
                                candidate = sf.kernel_candidate(fibre, eta)
                                scalar = v3.b.formula_scalar(model, formula, candidate["coordinate_blobs"])
                                if not scalar:
                                    continue
                                delta = list(candidate["source_word"])
                                row = v12.aggregate(v12.replay_atom(formula["seed_index"], delta, P["runtime"],
                                                                      P["model"], P["pres"], P["owner"], P["p176"], P["q"]))
                                # v424: exactly this non-mutating ABI precedes all literal work.
                                remainder, _ = P["phys"].reduce(row)
                                if not remainder:
                                    continue
                                predicted = min(remainder)
                                conjugate = p179.reduce_word(delta + list(seed_word) + p179.inverse_word(delta))
                                fresh = v12.aggregate(v12.seed_v12(P["model"], P["runtime"].old, P["owner"],
                                                                   P["p176"], P["q"], conjugate))
                                need(row == fresh, "correction:seed_v12_equality")
                                ex, ey = v12.v3.exp_pair(conjugate)
                                need(ex % 18 == 0 and ey % 18 == 0, "correction:exponent")
                                need(all(key[:1] != b"E" for key in row), "correction:forbidden_E")
                                direct = tuple(p179.coordinate_blobs(sf.rt, delta))
                                need(direct[coordinate] == target and
                                     v3.b.formula_scalar(model, formula, direct) == scalar and
                                     pair(dual, row, v3.b) == scalar, "correction:scalar_gates")
                                pre = len(P["phys"].order)
                                source = {"family": "DIRECT_CORRECTION", "seed_index": formula["seed_index"],
                                          "delta_word": delta, "source_digest": v12.row_digest(row)}
                                rise, actual = P["phys"].add(row, source)
                                need(rise and actual == predicted, "correction:predicted_pivot")
                                rows.append({"kind": "correction", "seed_index": formula["seed_index"],
                                             "delta_word": delta, "exact_exponent_pair": [ex, ey],
                                             "adjoint_digest": adj["adjoint_digest"],
            ...
            if soft_reason is not None:
                ...
                state, state_runtime, active, durable, committed, close_reason = flush_rows(
                    P, m, v3.b, state, state_runtime, batch_no, rows, round_no,
...
def fixture_legacy() -> dict[str, Any]:
    # Exercise the exact producer ABI with a tiny instrumented physical object.
    calls: list[str] = []
    ...
    def literal_checks(candidate: Any, row: Any, scalar: int) -> bool:
        ...
        return scalar in (1, 2) and candidate.get("literal") == "ok"
    def add(row: Any, candidate: Any) -> tuple[bool, int]:
        calls.append("add")
        return True, 2

    dependent = delayed_retain(Phys({}), {"exact_exponent_pair": [72, 0]}, 1,
                               replay_atom, aggregate, seed_v12, exponent_pair, literal_checks, add)
    need(dependent is not None and dependent["dependent"] and
         calls == ["replay_atom", "aggregate", "reduce"], "fixture:dependent_order")
    calls.clear()
    independent_candidate = {"exact_exponent_pair": [72, 0], "literal": "ok"}
    independent = delayed_retain(Phys({2: 1, 4: 1}), independent_candidate, 1,
                                 replay_atom, aggregate, seed_v12, exponent_pair, literal_checks, add)
    need(independent is not None and not independent["dependent"] and
         calls == ["replay_atom", "aggregate", "reduce", "seed_v12", "aggregate",
                   "exponent", "literal_checks", "add"] and
         independent["predicted_pivot"] == independent["actual_pivot"] == 2,
         "fixture:independent_order")
    semantic_changed = dict(independent_candidate, literal="changed")
    try:
        delayed_retain(Phys({2: 1, 4: 1}), semantic_changed, 1, replay_atom, aggregate,
                       seed_v12, exponent_pair, literal_checks, add)
    except RuntimeError:
        semantic_mutation_rejected = True
    else:
        semantic_mutation_rejected = False
    need(semantic_mutation_rejected, "fixture:semantic_mutation")
    c99 = authenticate_c99()
    base_id = {"path": C99[0], "bytes": C99[1], "sha256": C99[2]}
    base = state_from_c99(c99, base_id)
    loaded_base, loaded_id, is_base = load_resume(C99[0], c99)
    need(is_base and loaded_id == base_id and loaded_base == base, "fixture:base_resume")
    validate_closed_state(base, c99, base_id)
    append_template = {"kind": "correction", "seed_index": 1, "delta_word": [1],
                       ...
                       "predicted_pivot": "aa", "pivot": "aa"}
    append_rows = []
    for index in range(BATCH_CAP):
        row = copy.deepcopy(append_template)
        row.update({"pre_rank": 99 + index, "post_rank": 100 + index})
        append_rows.append(row)
    append_batch = {"batch": 4, "anchor_rank": 99, "anchor_dual_digest": base["current_dual_profile"]["dual_digest"],
                    "anchor_remainder_digest": base["current_dual_profile"]["remainder_digest"],
                    "rows": append_rows, "row_count": BATCH_CAP, "post_rank": 99 + BATCH_CAP,
                    "post_remainder_digest": base["current_dual_profile"]["remainder_digest"],
                    "post_dual_digest": base["current_dual_profile"]["dual_digest"],
                    "round": 13, "closed": True}
    ready_base = sealed(dict(base, phase="READY", input_checkpoint=base_id, open_batch=False))
    own = copy.deepcopy(base)
    own.update({"appended_batches": [append_batch], "batches": base["base_batches"] + [append_batch],
                "accepted_sources": base["accepted_sources"] + append_rows, "accepted_count": 56 + BATCH_CAP,
                "batch_count": 4, "rank": 99 + BATCH_CAP, "round": 13, "phase": "CLOSED",
                "segments": [{"input_checkpoint": base_id, "input_checkpoint_bytes": C99[1],
    ...
    own = sealed(own)
    own["input_checkpoint"] = base_id
    own = sealed(own)
    validate_closed_state(own, c99, base_id)
    wide_mutation = copy.deepcopy(own)
    wide_mutation["segments"][0]["new_rises"] = BATCH_CAP - 1
    wide_mutation = sealed(wide_mutation)
    try:
        validate_closed_state(wide_mutation, c99, base_id)
    except RuntimeError:
        wide_segment_mutation_rejected = True
    else:
        wide_segment_mutation_rejected = False
    need(wide_segment_mutation_rejected, "fixture:wide_segment_mutation")
    identity_mutation = copy.deepcopy(own)
    identity_mutation["segments"][0]["input_checkpoint"]["sha256"] = "c" * 64
    identity_mutation["segments"][0]["input_checkpoint_sha256"] = "c" * 64
    identity_mutation = sealed(identity_mutation)
    try:
        validate_closed_state(identity_mutation, c99, base_id)
    except RuntimeError:
        segment_identity_mutation_rejected = True
    else:
        segment_identity_mutation_rejected = False
    prior_seal_mutation = copy.deepcopy(own)
    prior_seal_mutation["segments"][0]["prior_state_seal"] = "c" * 64
    prior_seal_mutation = sealed(prior_seal_mutation)
    try:
        validate_closed_state(prior_seal_mutation, c99, base_id)
    except RuntimeError:
    ...
    need(segment_identity_mutation_rejected and segment_prior_seal_mutation_rejected,
         "fixture:segment_chain_mutations")
    safe_ok = canonical_input_path(C99[0]).as_posix() == C99[0]
    rejected: list[str] = []
    for label, bad in (("prefix", lambda x: x.update(base_prefix=[{}] + x["base_prefix"][1:])),
                       ("batch", lambda x: x["base_batches"][0].update(anchor_rank=52)),
                       ("segment", lambda x: x.update(segments=[{"closed": False}])),
                       ("open", lambda x: x.update(open_batch=True))):
        mutated = copy.deepcopy(base)
        bad(mutated)
        try:
            validate_closed_state(mutated, c99, base_id)
        except RuntimeError:
            rejected.append(label)
    cap_results = []
    for rises in (63, 64, 65):
        try:
            cap_gate(UNKNOWN_RESOURCE + ":max_rises", rises)
            cap_results.append((rises, "accept"))
        except RuntimeError:
            cap_results.append((rises, "reject"))
    for label, value in (("traversal", "search/certs/../x.json"),
                         ("dots", "search/certs/a..json"),
                         ("suffix", "search/certs/a.txt"),
                         ("control", "search/certs/a\n.json")):
        try:
            canonical_input_path(value)
        except RuntimeError:
            rejected.append(label)
    need(rejected == ["prefix", "batch", "segment", "open", "traversal", "dots", "suffix", "control"],
         "fixture:mutation_rejections")
    bootstrap = sealed(dict(base, phase="BOOTSTRAP"))
    need(bootstrap["phase"] == "BOOTSTRAP" and bootstrap["open_batch"] is False,
         "fixture:bootstrap_resource")
    try:
        terminal_result(UNKNOWN_RESOURCE, UNKNOWN_RESOURCE + ":module_load", base, base_id, None,
                        time.monotonic(), argparse.Namespace(), False, False, False,
                        None, None, True, (56, 99, 3, 12))
    except RuntimeError:
        missing_fallback_rejected = True
    else:
        missing_fallback_rejected = False
    need(missing_fallback_rejected, "fixture:missing_fallback")
    update_calls: list[str] = []
    fake_v12 = types.SimpleNamespace(row_digest=lambda value: "c" * 64)
    fake_phys = types.SimpleNamespace(order=[None] * (99 + BATCH_CAP))
    fake_p = {"phys": fake_phys, "v12": fake_v12}
    fake_bmod = types.SimpleNamespace(update=lambda p, m: (update_calls.append("update") or
                                                            (None, {}, {})))
    fake_anchor = ({b"dual": 1}, {b"rem": 1}, {})
    fake_receipt, _ = close_batch(fake_p, None, fake_bmod, 4, fake_anchor,
                                  [{"pre_rank": 99}], 13)
    need(len(update_calls) == 1 and fake_receipt["closed"] is True,
         "fixture:single_post_batch_update")
    return {"status": "PASS", "dependent_replay_reduce_once": True,
            "dependent_no_seed_exponent_add": True, "independent_seed_once_add_once": True,
            "predicted_actual_pivot_equal": True, "exact_base_authenticated": safe_ok,
            "appendable_own_schema_validator": True, "sixteen_row_batch_validator": True,
            "wide_segment_mutation_rejected": wide_segment_mutation_rejected,
            ...
            "prefix_batch_segment_mutations": rejected[:4],
            "cap_63_reject": cap_results[0][1] == "reject", "cap_64_accept": cap_results[1][1] == "accept",
            "cap_65_reject": cap_results[2][1] == "reject", "open_batch_reject": True,
            "missing_physical_fallback_rejected": missing_fallback_rejected,
            "single_post_batch_update": len(update_calls) == 1,
            "early_resource_bootstrap": True, "path_mutations_rejected": rejected[4:],
            "structural_seal_semantic_mutation_rejected": semantic_mutation_rejected}
...
def fixture() -> dict[str, Any]:
    ...
    class Phys:
        def __init__(self, remainder: dict[int, int], rank: int = 99):
            self.remainder = remainder
    ...
    def literal_checks(candidate: Any, row: Any, scalar: int) -> bool:
        calls.append("literal_checks")
        return candidate.get("literal") == "ok" and scalar in (1, 2)
    ...
    dependent = delayed_retain(Phys({}), {"exact_exponent_pair": [72, 0]}, 1,
                               replay_atom, aggregate, seed_v12, exponent_pair,
                               literal_checks, add)
    need(dependent is not None and dependent["dependent"] and
         calls == ["replay_atom", "aggregate", "reduce"], "fixture:dependent_order")
    calls.clear()
    candidate = {"exact_exponent_pair": [72, 0], "literal": "ok"}
    independent = delayed_retain(Phys({2: 1}), candidate, 1, replay_atom, aggregate,
                                  seed_v12, exponent_pair, literal_checks, add)
    need(independent is not None and not independent["dependent"] and
         calls == ["replay_atom", "aggregate", "reduce", "seed_v12", "aggregate",
                   "exponent", "literal_checks", "add"] and
         ...
         "fixture:independent_order")
    try:
        delayed_retain(Phys({2: 1}), dict(candidate, literal="changed"), 1, replay_atom,
                       aggregate, seed_v12, exponent_pair, literal_checks, add)
    except RuntimeError:
        semantic_mutation_rejected = True
    else:
        semantic_mutation_rejected = False
    ...
    def flush_fixture(previous: dict[str, Any], count: int, identity: dict[str, Any] = base_id,
                      ...
                      hard_writer: Any = None) -> tuple[dict[str, Any], dict[str, Any] | None,
                                                         bool, str | None, int]:
        rank = previous["rank"]
        ...
        profile = {"physical_rank": rank + count, "dual_digest": "d" * 64,
    ...
    one, one_durable, one_committed, one_reason, one_writes = flush_fixture(base, 1)
    ...
    validate_closed_state(one, c99, base_id)
    ...
    hard_result = flush_rows({}, None, types.SimpleNamespace(), base, object(), 4,
                             rows_at(99, 1), 13, base_id, None, one_durable, hard_args,
                             time.monotonic())
    need(hard_result[0] is base and hard_result[3] is one_durable and
    ...
    validate_closed_state(second, c99, second_id)
    ...
    try:
        validate_closed_state(row_mutation, c99, second_id)
    except RuntimeError:
    ...
    identity_mutation = sealed(identity_mutation)
    try:
        validate_closed_state(identity_mutation, c99, second_id)
    except RuntimeError:
    ...
    try:
        validate_closed_state(prior_mutation, c99, second_id)
    except RuntimeError:
    ...
    need(own_resume_ok, "fixture:own_resume")
    ...
    try:
        ...
        globals()["literal_row"] = lambda P, m, p179, sf, model, formulas, record, dual, adj, bmod: (
            {"row": 1}, 1, {})
        replay_state, _ = replay_prefix(fake_v3, fake_p, fake_m, None, [replay_record],
    ...
    need(len(replay_calls) == 1 and len(replay_calls[0]) == 3 and
         ...
         "fixture:production_replay_v3_abi")
    ...
    return {"status": "PASS", "dependent_replay_reduce_once": True,
            ...
            "production_replay_v3_abi": True, "flush_one": True, "flush_fifteen": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", default="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.json")
    parser.add_argument("--checkpoint", default="ci/out/d972_r07_a0_dual_anchored_rank99_durable_discovery_v3.checkpoint")
