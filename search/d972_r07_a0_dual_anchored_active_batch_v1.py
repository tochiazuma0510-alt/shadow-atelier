#!/usr/bin/env python3
"""Task451 dual-anchored ACTIVE batch continuation from frozen rank 51."""
from __future__ import annotations
import argparse,hashlib,importlib.util,json,os,tempfile,time
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];SCHEMA="d972-r07-a0-dual-anchored-active-batch/v1";CP_SCHEMA=SCHEMA+"/checkpoint";MARKER="R07_A0_DUAL_ANCHORED_ACTIVE_BATCH_V1"
V3=("search/d972_r07_a0_actual_tau_free_rank_ladder_v3.py",12215,"0140447110cfa568a77300bd7d43d51c622597704b7b91ed5209c63168c9ef37");FROZEN=("search/certs/d972_r07_a0_actual_tau_free_rank51_checkpoint_v1.json",10934,"a83959e4c9fcfa79093c712e82164d47c31b78c9fc00b512f7adac9413c481f4");BINDING=hashlib.sha256((SCHEMA+V3[2]+FROZEN[2]).encode()).hexdigest()
def need(x,m):
...
v3=load(V3,"task451_v3")
def frozen():
    s=json.loads(raw(FROZEN));h=s.pop("state_sha256",None);need(h=="22dcfdfb396524ea5853488aa2ad52d28b4f7d10164123bc83f121e59dd83159" and h==sha(canonical(s)),"frozen seal");s["state_sha256"]=h;need(s["rank"]==51 and s["accepted_count"]==8 and s["round"]==9,"frozen fields");return s
...
def terminal(status,reason,P,prefix,batches,round_no,profile,seal,started,positive=None):
    accepted=list(prefix)+[r for x in batches for r in x["rows"]];return {"schema":SCHEMA,"status":status,"terminal":status,"reason":reason,"frozen_prefix_count":8,"accepted_sources":accepted,"accepted_count":len(accepted),"batches":batches,"batch_count":len(batches),"round":round_no,"physical_rank":len(P["phys"].order),"current_dual_profile":profile,"terminal_replay":positive,"durable_state":seal,"claims":{"A0":status=="COMMON_CANDIDATE","COMMON":False,"NONMEMBER":False,"fake":False,"Ihara":False},"elapsed_seconds":time.monotonic()-started}
def run(a):
    started=time.monotonic();base=frozen();v1=load(v3.b.V1,"task451_v1");v4=v1.load(v1.V4,"task451_v4");m=v4.load_v1();m.RUN_STARTED=started;m.MARKER=MARKER;v12=m.load(m.V12,"task451_v12");p435=m.load(m.P435,"task451_p435");p179=m.load(m.P179,"task451_p179");P=v4.adapt(m,m.prefix(v12,p435,a));P["started"]=started;prefix=list(base["accepted_sources"]);state=v3.b.update(P,m);state=v3.replay(P,m,p179,prefix,state);need(len(P["phys"].order)==51,"rank51 replay");batches=[];round_no=base["round"]
    if a.resume:s=cp_read(a.resume);need(s["accepted_sources"][:8]==prefix,"resume prefix");batches=list(s["batches"]);round_no=s["round"];state=replay_batches(P,m,batches,state)
    ...
    try:
        while True:
            ...
            receipt,state=close_batch(P,m,batch_no,state,rows);batches.append(receipt);new_rises+=len(rows);dual2,rem2,_=state;current=v3.b.profile(P) if dual2 is not None else None;seal=cp_write(a.checkpoint,checkpoint_state(prefix,batches,len(P["phys"].order),round_no,"batch_closed",current));closed_round=round_no;closed_profile=current;rss=getattr(v12.v3,"rss",lambda:0)() or 0;print(f"{MARKER} progress batch={batch_no} rank={len(P['phys'].order)} accepted_count={8+sum(x['row_count'] for x in batches)} elapsed_seconds={time.monotonic()-started:.3f} rss_bytes={rss}",flush=True)
...
def fixture():
    ...
    existing=[{"row_count":7},{"row_count":9}];resumed_rises=sum(x["row_count"] for x in existing);need(rank==53 and len(rows)==2 and resumed_rises==16,"toy batch");return {"status":"PASS","anchor_reused":True,"candidate_count":3,"rise_count":2,"single_post_batch_update":True,"open_batch_not_durable":True,"resumed_cumulative_rises":resumed_rises}
def main(argv=None):
    ap=argparse.ArgumentParser();ap.add_argument("--mode",choices=("PRODUCTION","FIXTURE"),default="PRODUCTION");ap.add_argument("--output",default="ci/out/d972_r07_a0_dual_anchored_active_batch_v1.json");ap.add_argument("--checkpoint",default="ci/out/d972_r07_a0_dual_anchored_active_batch_v1_output.checkpoint");ap.add_argument("--resume");ap.add_argument("--seconds",type=float,default=7200);ap.add_argument("--rss-bytes",type=int,default=4_800_000_000);ap.add_argument("--max-rises",type=int,default=64);ap.add_argument("--batch-cap",type=int,default=16);a=ap.parse_args(argv)
