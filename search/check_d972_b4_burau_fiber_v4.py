...
V4_SCHEMA = "d972-b4-burau-fiber/v4-lowmem"
V4_FINAL = "D972_B4_BURAU_FIBER_V4_FINAL"
V4_PRODUCER = Path("search/d972_b4_burau_fiber_v4.py")
...
V4_ALGORITHM = "exact tuple Reidemeister-Schreier normal closure"
...
def v4_check_receipt(path: Path, cal_q3: Path | None = None, cal_q4: Path | None = None) -> dict[str, Any]:
    ...
    require(r.get("status") in {"CANDIDATE_B4_A_BURAU_FINITE_ZERO_FIBER", "UNKNOWN_BURAU_SPECIALIZATION_ALLPASS", "UNKNOWN_RESOURCE"}, "status")
    ...
    elif r.get("status") == "CANDIDATE_B4_A_BURAU_FINITE_ZERO_FIBER":
        require(any_zero, "candidate without zero fiber")
    elif r.get("status") == "UNKNOWN_BURAU_SPECIALIZATION_ALLPASS":
        require(not any_zero, "allpass with zero fiber")
    if q == 5:
        gate = r.get("calibration_gate"); require(isinstance(gate, dict) and gate.get("required_for_q5") is True, "q5 calibration gate")
        require(cal_q3 is not None and cal_q4 is not None and v4_check_receipt(cal_q3)["q"] == 3 and v4_check_receipt(cal_q4)["q"] == 4, "q5 checked calibrations")
        require(gate.get("accepted_receipt_sha256") == {"q3_sha256": file_sha(cal_q3), "q4_sha256": file_sha(cal_q4)}, "q5 calibration binding")
...
def v4_selftest() -> None:
    ...
    print("D972_B4_BURAU_FIBER_V4_SOURCE_WORD_ROOF_ONLY_NEGATIVE_PASS row=2 exponent=(-4,-8) nonzero=956")
    print("D972_B4_BURAU_FIBER_V4_CHECKER_SELFTEST_PASS")
    print("D972_B4_BURAU_FIBER_V4_CHECKER_FINAL_MARKER status=PASS")
...
def main_v4() -> None:
    ...
    if not a.receipt:
        ap.error("receipt path or --self-test required")
    print("D972_B4_BURAU_FIBER_V4_CHECK_PASS", json.dumps(v4_check_receipt(Path(a.receipt), a.calibration_q3, a.calibration_q4), sort_keys=True))
