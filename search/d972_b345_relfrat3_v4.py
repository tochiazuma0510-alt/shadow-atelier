#!/usr/bin/env python3
"""Transactional sparse-DAG positive-semidecision at relative Frattini-3.
...
SCHEMA = "d972-b345-relative-frattini3/v4"
...
OUTPUT_PATH = Path("ci/out/d972_b345_relfrat3_v4.json")
...
class ResourceMonitor:
    ...
    def _progress(self, force: bool = False) -> None:
        ...
        print("D972_B345_RELFRAT3_V4_PROGRESS "
...
def run(q3_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    search_prefix: dict[str, Any] = {
        "cheap": {"evaluated": 0, "completed": False,
                  "survivor_count": 0,
    ...
    source_hashes = {
        "producer_sha256": digest_file(Path(__file__)),
        "checker_sha256": digest_file(repo/"search/check_d972_b345_relfrat3_v4.py"),
    ...
    try:
        ...
        cheap_public, dp_failures = fixed_context_cheap_dp(
            dictionary, e4, monitor, search_prefix)
        receipt["fixed_context_cheap_DP"] = cheap_public
        cheap_survivor_indices = list(cheap_public["survivor_indices"])
        require(len(dp_failures) == len(dictionary["words"]) and
                1 in cheap_survivor_indices and not dp_failures[0],
                "empty correction exact cheap DP gate")
        phase_start = phase("fixed_context_cheap_DP", phase_start)
            ...
            for correction_index in scheduled:
                ...
                try:
                    ...
                    direct_row = {
                        "checkpoint": used,
                        "candidate_index": correction_index,
                        "DP_failed_gates": dp_failures[correction_index-1],
                        "direct_failed_gates": temporary["quotient_bad"],
                        "equal": temporary["quotient_bad"] ==
                                 dp_failures[correction_index-1],
                    }
                    require(direct_row["equal"], "direct-versus-DP cheap gate drift")
                    ...
                    if temporary["quotient_bad"]:
                        ...
                        unusable_candidates.add(correction_index)
                        transaction["full_quotient_rejections"] += 1
                        ...
                        append_trace({"checkpoint": used,
                                      "candidate_index": correction_index,
                                      "event": "full_quotient_reject",
                                      "pool_suffix_removed": removed,
                                      "target_gradients_generated": 0,
                                      "transient_sparse_entries": 0,
                                      "peak_transient_sparse_entries": 0})
        ...
        search_accounting = {
            ...
            "fixed_context_cheap_DP": cheap_public,
            "candidate_membership_tests": tests,
            "same_basis_reused_for_all_candidates": True,
            "candidate_order": "checkpoint-major; registered correction order within each checkpoint",
            "cheap_candidates_evaluated": cheap_public["evaluated"],
            "cheap_survivor_indices": cheap_survivor_indices,
            "direct_word_replay_count": direct_replay_count,
            "direct_vs_DP_comparisons": direct_comparisons,
            "direct_vs_DP_all_equal": all(row["equal"] for row in direct_comparisons),
        ...
        if selected_candidate is None:
            receipt["status"] = receipt["terminal_token"] = \
                "B345_RELFRAT3_SEARCH_INCOMPLETE"
            receipt["reason"] = "registered translated-relator and correction caps exhausted without a proof root; blockers are runtime skips only and no obstruction or nonexistence is claimed"
            receipt["direct_lane"] = {
        ...
        receipt["direct_vs_DP_comparisons"] = direct_comparisons
