#!/usr/bin/env python3
"""Task1053: a new lambda1578 k128 fixed-lambda batch with a private durable reduction prefix.
...
C_FILE = "check_d972_r07_fixed_lambda_cycle_batch_v4.py"
...
def authenticate_acceptance(args: Any) -> Any:
    ...
    exact_keys(value, ("schema", "parents", "anchor", "batch_anchor", "code", "runtime", "registration"), "acceptance_seven_plain_keys")
    ...
    require(not args.output_root.is_symlink() and isinstance(value["parents"], list) and
            len(value["parents"]) == len(ROLES), "sixteen_registered_roots")
    inventories, by_role = [], {}
...
def finish_inputs(output: Path, admission: Any, args: Any) -> Any:
    ...
    same_json(observed, admission["parent_inventories"], "all_sixteen_parent_file_and_directory_invariance")
...
def validate_host_paths(value: Any) -> None:
    ...
    exact_keys(value["parents"], ROLES, "host_paths_exact_sixteen_roles")
...
def current_derived_rho2(start: Any, state: Any) -> Any:
    ...
    require(count == start["accepted_parent_target_derivations"] == 225 and
            ...
            "all225_accepted_parents_and_only_new_accepted_target_rows")
...
def final_manifest_value(binding: Any, state: Any, selection: Any, terminal: str, kind: str,
    ...
    return seal("final-manifest", {**{key: binding[key] for key in
        ...
        "anchor_completed_steps": 64, "anchor_accepted_parent_batch_rows": 128, "selected_count": selection["selected_count"],
...
def public_head_value(binding: Any, final: Any) -> Any:
    ...
    return seal("head", {**{key: binding[key] for key in
        ...
        **{key: final[key] for key in ("terminal", "kind", "anchor_completed_steps", "anchor_accepted_parent_batch_rows", "selected_count",
...
def batch_observation(output: Path | None, context: Any | None, checkpoint: Any | None) -> Any:
    old = {"parent_role": "continuation", "parent_artifact": copy.deepcopy(ANCHOR_ARTIFACT),
        "state_head": "076c4b9df33957b090a7bf698e4dd1100e85350cfff86f88eb9ab12a39fdb667",
        "lambda_sha256": "7c0dbe475f9b162f4648f5468ee1098eb86b43dbee247d71f811076559ab88fe",
        "selection_sha256": BATCH_ENTRY_FILES["output/selection/selection.json"]["sha256"],
        "failed_count": 36274, "first_failed_index": 70, "first_failed_edge": 125}
    current = {"parent_role": "batch-parent", "parent_artifact": copy.deepcopy(BATCH_PARENT_ARTIFACT),
    ...
    accepted = admission["batch_objects"]["objects"]
    ...
    require(direct["rows"] == 1578 and direct["lambda_pivots"] == 0 and
            direct["lambda_parent_remainder"] == direct["lambda_new_remainder"] == 1,
...
def result_value(output: Path, admission: Any, intake: Any, prefix: Any, final: Any,
    ...
    return seal("result", {"status": "PASS", **{key: manifest[key] for key in
        ("terminal", "kind", "anchor_completed_steps", "anchor_accepted_parent_batch_rows", "selected_count", "processed_candidates",
...
def k128_registration_canary(root: Path) -> Any:
    ...
    old_acceptance = {"schema": "d972.r07.fixed-lambda-cycle-batch.v2.acceptance", "parents": [],
                      "anchor": {}, "batch_anchor": {}, "code": {}, "runtime": {}, "registration": registration}
    ...
    portable = {"schema": SCHEMA + ".acceptance",
        ...
        "anchor": {}, "batch_anchor": {}, "code": {}, "runtime": copy.deepcopy(REGISTERED_RUNTIME), "registration": registration}
    current_acceptances = []
...
def selftest(args: Any) -> Any:
    ...
    try:
        ...
        require([len(item["rejected_cases"]) for item in (first, second, third)] == [30, 10, 6], "selftest_exact_three_group_counts")
        return seal("selftest", {"status": "PASS", "tests": [first, second, third],
            "fixture_scope": "preserved synthetic k128 registration/full-roster and nonzero dependent-then-independent physical reduction/publication plus synthetic batch1578 metadata/header/projection/hash; no actual parent, Omega or E source/primal/P1 arithmetic",
...
def cli() -> Any:
    ...
    if args.selftest:
        ...
        require(args.selftest_root is None and args.output_root is not None and args.acceptance is not None and len(args.block_root) == 4 and
                ...
                "production_requires_exact_sixteen_roots_acceptance_and_output")
    return args
