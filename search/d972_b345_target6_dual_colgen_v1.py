#!/usr/bin/env python3
"""Bounded target-6 full-D2 dual column generation (157em).
...
TASK = Path("sol/luna_task_157em_b345_target6_dual_colgen.md")
TASK_SHA = "60df04261bfd9f30928ed51b26bd501518c05eae43b0bb8ca08507e3b6c4ca99"
TASK_BYTES = 43_511
SCHEMA = "d972-b345-target6-dual-colgen/v1"
OUTPUT = Path("ci/out/d972_b345_target6_dual_colgen_v1.json")
...
class PackedBlockLedger:
    ...
    def add_record(self, generation: int, ordinal: int, relator: int,
        ...
        require(1 <= generation <= 8 and 1 <= ordinal <= 1024 and
...
def solve_from_remainders(ei: Any, old: Any, e4: Any,
    ...
    return system, {"generation": generation, "target_row": target_row,
        ...
        "remainders_sha256": sha_obj([sorted(row.items()) for row in remainders]),
    ...
    if initial_target is not None:
        ...
        require(target["generation"] == 1 and
                ...
                target["remainders_sha256"] ==
                    initial_target["target6"]["fresh_remainder_sha256"],
...
def validate_generation_ledger(rows: Any, token: str,
                               ...
                               fixed_block: dict[str, Any] | None = None,
                               fixed_anchor: dict[str, Any] | None = None,
                               initial_target: dict[str, Any] | None = None) -> None:
...
def validate_resource_stage_projection(receipt: dict[str, Any],
    ...
    if not require_external_prefix:
        pass
...
def validate_initial_target_payload(ei: Any, block: dict[str, Any],
    ...
    require(isinstance(row, dict) and set(row) == INITIAL_TARGET_KEYS and
            ...
            _sha256_text(row["raw_gradients_sha256"]),
    ...
    try:
        ...
        while True:
            ...
            if not correlation["active"]:
                ...
                break
            if remaining == 0:
                raise LaneResource("total_new_translation_blocks", 4096,
                    4097, "gt", "correlation_pass1",
            ...
            if batches == CAPS["column_generation_batches"]:
                raise LaneResource("column_generation_batches", 8, 9, "gt",
                    "correlation_pass1", {"generation": generation,
...
def self_test() -> None:
    ...
    # The pinned predecessor self-test supplies the real block reducer,
    # target/source-root closure, packed proof, monitor, module-lifecycle, and
    # checked-write production fixtures.  It cannot enter its full main path.
    ei = load_ei(); ei.self_test()
    ...
    old = ed.load_pinned_module(ed.OLD_PRODUCER, ed.OLD_PRODUCER_SHA,
        "_d972_157em_selftest_old_producer")
    ...
    target1 = {"generation": 1, "variables": 108, "equations": 1,
        ...
        "remainders_sha256": update_public["pre_update_remainder_sha256"],
    ...
    chain_initial = {"affine_system": {"variables": 108, "equations": 1,
        ...
        "target6": {"fresh_remainder_sha256":
            update_public["pre_update_remainder_sha256"]}}
    ...
    def fixture_receipt(token: str) -> dict[str, Any]:
        ...
        fixture_basis = {**state_accounting(toy_prefix, pool_digest=False),
                         "recovery": toy_recovery.public()}
        ...
        def fixture_target(consistent: bool) -> dict[str, Any]:
            return {"generation": 1, "variables": 108,
                "equations": 0 if consistent else 1, "rank": 0,
                "nullity": 108, "consistent": consistent,
                "row_space_sha256": "22"*32,
                "remainders_sha256": "33"*32,
    ...
    generation_resource["generation_ledger"] = [{
        "generation": 1,
        "basis": copy.deepcopy(
            obstruction_fixture["generation_ledger"][0]["basis"]),
        "target": {"generation": 1, "variables": 108, "equations": 1,
            ...
            "stopped_at_first_contradiction": False,
            "dual": copy.deepcopy(obstruction_fixture[
    ...
    print("D972_B345_TARGET6_DUAL_COLGEN_V1_PRODUCER_SELFTEST_PASS "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("D972_B345_TARGET6_DUAL_COLGEN_V1_PRODUCER_PASS " +
