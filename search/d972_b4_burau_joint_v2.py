...
def complete_hprime(x: JointElt, y: JointElt, specs: Sequence[Spec],
    ...
    while True:
        ...
        print(f"D972_B4_BURAU_JOINT_V2_PROGRESS phase=normal-closure "
...
def run_full(config: str, output: Path) -> int:
    ...
    print(f"D972_B4_BURAU_JOINT_V2_PROGRESS phase=schreier-complete "
    ...
    for i, (m, key, word) in enumerate(rows, 1):
        ...
        if i % 81 == 0:
            print(f"D972_B4_BURAU_JOINT_V2_PROGRESS phase=rows config={config} "
    ...
    output.write_text(json.dumps(receipt, sort_keys=True, separators=(",", ":")) + "\n",
                      encoding="utf-8")
    print(f"D972_B4_BURAU_JOINT_V2_DONE config={config} h={h_order} "
...
def self_test() -> None:
    ...
    print("D972_B4_BURAU_JOINT_V2_Q2Q7_ADMISSIBILITY_AUDIT_PASS "
          "q2=[1] q7=[1,2,3,4,5,6]")
    print("D972_B4_BURAU_JOINT_V2_SOURCE_WORD_ROOF_ONLY_NEGATIVE_PASS "
          "row=2 exponent=(-4,-8) nonzero=956")
    print("D972_B4_BURAU_JOINT_V2_NEGATIVE_FIXTURES_PASS")
    print("D972_B4_BURAU_JOINT_V2_SELFTEST_PASS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("ci/out/d972_b4_burau_joint_v2_q3a2_q4a2.json"))
    ...
    try:
        ...
        print(f"D972_B4_BURAU_JOINT_V2_ERROR {type(exc).__name__}: {exc}",
              file=sys.stderr)
        return 2
