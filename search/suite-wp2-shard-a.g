# suite-wp2-shard-a.g -- WP2a GAP explorer, shard A (n=3..12)
#
# 実行: .\gap.ps1 search\suite-wp2-shard-a.g
#
...
# コードをそのままコピー -- BuildCertJsonDihedral 内の "generated_by".script 文字列
# リテラルも含め無変更。これは証明書 JSON のバイト内容を保つため -- 変更すると
# provenance/cert-hashes-wp2.txt の記録済みハッシュと不一致になる)。
#
# 宇宙分担(事前登録宇宙 {3..16,18,36,N5} の厳密な分割):
#   shard A (本ファイル): n = 3..12 の dihedral
#   shard B (suite-wp2-shard-b.g): n = 13..16 の dihedral + N5 control
...
# ================= メインドライバ: shard A -- n=3..12 =================
universe := [3..12];;
results := [];;    # results[n] := ProcessDihedral(n)
Print("宇宙 shard A (事前登録宇宙 {3..16,18,36,N5} のうち n=3..12): ", universe, "\n");
...
Print("\n累計 elapsed ms (n=3..12): ", Runtime()-startTime, "\n");

# ---- reduction: 本 shard 範囲内で計算可能な branch suite 3 対 (元スクリプトと同一) ----
...
Print("(reduction (36->12), (18->3) は search/suite-wp2-explorer-q1836.g で計算する; K13..K16+N5 は search/suite-wp2-shard-b.g)\n");
