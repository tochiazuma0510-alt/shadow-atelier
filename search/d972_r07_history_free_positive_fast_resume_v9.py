...
PRODUCER_PREFIX = "R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V9_PRODUCER_TERMINAL"
...
SOURCE_PINS: dict[str, tuple[str, int, str]] = {
    ...
    "fixture": ("search/certs/d972_r07_history_free_positive_fast_resume_selftest_v9_20260829.json", 2784,
...
class Meter:
    ...
    def public(self) -> dict[str, Any]:
        return {"phase": self.phase,
                ...
                "limits": self.limits, "fresh_v9_counters": self.counters,
...
class Search:
    ...
    def bind_section_identity(self, section: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
        ...
        result.update({"q0_state_hex": bytes(qstate).hex(),
            ...
            "gamma_full_state_hex": p176.packed_joint_blob(
                gamma_state, "v9 Gamma provenance").hex(),
            "gamma_full_state_sha256": sha_bytes(p176.packed_joint_blob(
                gamma_state, "v9 Gamma provenance")),
...
def restore_checkpoint(search: Search, path: Path) -> None:
    ...
    require(type(formal) is dict and value.get("formal_ancestry_sha256") == sha_obj(formal) and
            ...
            type(formal.get("pivot_expr_ids")) is list, "v9 formal DAG owner")
    ...
    require(prior_monitor.get("limits") == search.meter.limits and
            type(prior_monitor.get("fresh_v9_counters")) is dict,
            "v7 checkpoint monitor")
    for name, counter in prior_monitor["fresh_v9_counters"].items():
    ...
    if remainder:
        ...
        require(progress_dual in (None, live.sha_obj(value.get("current_dual"))),
                "v9 correction cursor dual binding")
...
def run_real_owner_selftest(runtime: dict[str, Any], old_value: dict[str, Any],
    ...
    require(fixture.get("schema") ==
            "d972-r07-history-free-positive-fast-resume/selftest-input/v9" and
...
if __name__ == "__main__":
    try:
        ...
        print("R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V9_PRODUCER_STOP " +
