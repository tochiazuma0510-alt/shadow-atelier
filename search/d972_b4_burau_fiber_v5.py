...
CAL_KERNEL_ORDER = 8
SCHEMA = "d972-b4-burau-fiber/v5-lowmem"
FINAL = "D972_B4_BURAU_FIBER_V5_FINAL"
...
def complete_hprime(x: TupleElt, y: TupleElt, q: int,
    ...
    while True:
        ...
        print(f"D972_B4_BURAU_V5_PROGRESS phase=normal-closure round={rounds} "
...
def run_full(q: int, a: int, output: Path, cal_q3: Path, cal_q4: Path) -> int:
    ...
    print(f"D972_B4_BURAU_V5_PROGRESS phase=schreier-complete projected={len(sec)} "
    ...
    for i, (m, key, word) in enumerate(rows, 1):
        ...
        if i % 81 == 0:
            print(f"D972_B4_BURAU_V5_PROGRESS phase=rows completed={i}", flush=True)
    ...
    receipt = {
        ...
        "calibration_gate": {"required_for_q5_or_q7": True,
    ...
    output.write_text(json.dumps(receipt, sort_keys=True, separators=(",", ":")) + "\n",
                      encoding="utf-8")
    print(f"D972_B4_BURAU_V5_DONE q={q} a={a} h={h_order} "
...
def self_test() -> None:
    ...
    print("D972_B4_BURAU_V5_SOURCE_WORD_ROOF_ONLY_NEGATIVE_PASS "
          "row=2 exponent=(-4,-8) nonzero=956")
    print("D972_B4_BURAU_FIBER_V5_NEGATIVE_FIXTURES_PASS")
    TQ = 5
    print("D972_B4_BURAU_FIBER_V5_SELFTEST_PASS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("ci/out/d972_b4_burau_fiber_v5.json"))
    ap.add_argument("--calibration-q3", type=Path,
                    default=Path("ci/out/d972_b4_burau_fiber_v5_q3.json"))
    ap.add_argument("--calibration-q4", type=Path,
                    default=Path("ci/out/d972_b4_burau_fiber_v5_q4.json"))
    ...
    try:
        ...
        print(f"D972_B4_BURAU_V5_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
