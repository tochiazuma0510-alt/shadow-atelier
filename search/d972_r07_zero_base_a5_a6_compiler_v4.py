...
frozen v12 runtime; task193-v4 supplies the actual pointed target.  No
...
SCHEMA = "d972-r07-zero-base-a5-a6-compiler/v4"
PRODUCER_LINE = "R07_ZERO_BASE_A5_A6_COMPILER_V4_PRODUCER_TERMINAL"
...
def load_task198() -> types.ModuleType:
    ...
    module = types.ModuleType("r07_task198_v12_runtime_for_a5_v4")
...
class Budget:
    ...
    def progress(self, phase: str, cursor: int, pre: int, joint: int,
        ...
        print("A5_V4_PROGRESS phase=" + phase + " cursor=" + str(cursor) +
    ...
    def run(self) -> dict[str, Any]:
        ...
        for relator in range(1, ROWS + 1):
            ...
            self.budget.progress("RELATORS", relator, len(pre.pivots), len(joint.pivots),
                                 self.total_labels())
            if relator % 32 == 0: self.authority.meter.check("A5_V4_RELATOR_" + str(relator))
        solution, final_dual = self._target_oracle(joint)
        ...
        while queue_cursor < len(pre.pivots):
            ...
            self.authority.meter.check("A5_V4_ACTION_" + str(queue_cursor))
        solution, final_dual = self._target_oracle(joint)
