#!/usr/bin/env python3
"""Task387/v8 producer: authenticated, finite rows 1--7 authority trace.
...
SCHEMA = "d972-r07-a4-actual-owner-trace/v8"
...
def _path_identity(path: Path, kind: str) -> dict[str, Any]:
    ...
    try: st = _nofollow_lstat(lexical)
        ...
        return {"identity_kind": kind, "path": str(lexical), "exists": False, "type": "missing", "logical_link_target": None, "mode": None, "bytes": None, "sha256": None, "device": None, "inode": None, "mtime_ns": None, "nlink": None, "single_open_handle": False, "opened_handle_stable": False, "pathname_matches_opened_handle": False, "substitution_detected": False}
...
def admit_path(path: Path, role: str, workspace: Path | None, events: EventSink) -> Path:
    ...
    lexical = Path(os.path.abspath(path))
    registered = Path(os.path.abspath(ROOT / (RECEIPT_REL if role == "receipt" else MANIFEST_REL)))
...
def load_fixture(store: PhysicalStore, meter: Meter, argument: str) -> dict[str, Any]:
    ...
    if type(seal) is not str or seal != FIXTURE_SELF or seal != digest_object(body, meter, 1_000_000, "fixture_body") or fixture.get("schema") != FIXTURE_SCHEMA: raise InputStop("producer:fixture:self_seal")
...
def _project(identity: dict[str, Any], logical: str, before: str, after: str) -> dict[str, Any]:
    ...
    owner_type = identity.get("type")
    readable = exists is True and owner_type == "regular"
    content_readable = readable and identity.get("sha256") is not None
    symlink_or_reparse = owner_type in ("symlink", "reparse")
    link_target = identity.get("logical_link_target") if symlink_or_reparse else None
    return {"logical_case_path": logical, "owner_kind": identity.get("identity_kind"), "exists": exists, "type": owner_type, "readable": readable, "byte_length": identity.get("bytes") if content_readable else "UNREADABLE_AT_REGISTERED_STAGE", "content_sha256": identity.get("sha256") if content_readable else "UNREADABLE_AT_REGISTERED_STAGE", "link_count": identity.get("nlink") if identity.get("nlink") is not None else "UNREADABLE_AT_REGISTERED_STAGE", "symlink_or_reparse": symlink_or_reparse, "logical_link_target": link_target, "single_open_handle": identity.get("single_open_handle") is True, "opened_handle_stable": identity.get("opened_handle_stable") is True, "pathname_matches_opened_handle": identity.get("pathname_matches_opened_handle") is True, "substitution_detected": identity.get("substitution_detected") is True, "canonical_before_sha256": before, "canonical_after_sha256": after}
...
def run_mutation(name: str, baseline: dict[str, Any], fixture: dict[str, Any], meter: Meter, store: PhysicalStore, workspace: Path, pre_transcript: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            rejection_entries = [(event["stage"], event["owner"]) for event in events.events if event["validator"] == rejection.validator]
            final_event = events.events[-1] if events.events else None
            if name == "resolved_path_traversal":
