#!/usr/bin/env python3
"""Production successor: replayable checkpoints with a pre-pool-only patch."""
...
if (len(_patched) != _BASE_BYTES or
    ...
    raise SystemExit("v18 frozen v13 owner drift")
for _old, _new in _REPLACEMENTS:
    if _patched.count(_old) != 1:
        raise SystemExit("v18 substitution cardinality")
    _patched = _patched.replace(_old, _new)
for _old, _new in _REPLACEMENTS:
    if _patched.count(_old) != 0:
        raise SystemExit("v18 substitution failed")
