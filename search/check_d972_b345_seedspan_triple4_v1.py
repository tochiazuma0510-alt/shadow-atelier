...
def checker_validate_positive_replay(data: dict[str, Any],
    ...
    if correction_word is not None:
        require(checker_literal_selected_replay(coefficients, seeds) ==
                correction_word,
                "checker positive literal selected replay")
...
###############################################################################
# 157ec independent checker: 108-seed affine receipt.
###############################################################################
...
AFFINE_ALLOWED_TOP_LEVEL = {
    ...
    "partial", "core_validation", "dual_witness", "dual_provenance",
}
...
def checker_affine_schema_target6_started(data: dict[str, Any]) -> bool:
...
def checker_affine_expected_top_keys(data: dict[str, Any]) -> set[str]:
    """Independent closed production envelope by terminal and progress."""
    token = data.get("terminal_token")
    reason = data.get("reason")
    keys = set(AFFINE_COMMON_TOP_LEVEL)
    if token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT":
        errors = data.get("input_errors") or {}
        started = errors.get("mathematical_scan_started") is True
        phase = str((data.get("performance") or {}).get(
    ...
    if token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE":
        ...
        if evaluated > 0 or phase.startswith("positive_typed_replay"):
            keys |= set(AFFINE_TARGET_TOP_LEVEL)
        if checker_affine_schema_target6_started(data):
            keys |= set(AFFINE_TARGET6_TOP_LEVEL)
        return keys
    ...
    raise Reject("checker affine production terminal/stage schema")


def checker_raw_split_gate(direct_gradient: dict[Any, int],
                           direct_value: Any,
    ...
    require(direct_value == identity and typed_value == identity and
            ...
            f"{label}: checker raw direct/typed split")


def checker_literal_selected_replay(coefficients: Sequence[int],
...
def checker_split_ledger_gate(ledger: list[dict[str, Any]], *,
                              old_shortcut_count: int) -> None:
    require(len(ledger) == AFFINE_CAPS["seed_count"],
            "checker split ledger count")
    for index, row in enumerate(ledger, 1):
        require(isinstance(row, dict) and row.get("seed_index") == index and
                ...
                "checker split ledger order/identity")
        if index <= old_shortcut_count:
            require(set(row) == {"seed_index", "gradient_sha256",
                    ...
                    "checker old split shortcut ledger")
        else:
            require(set(row) == {"seed_index", "gradient_sha256",
                    ...
                    "checker new split direct/typed ledger")


def checker_target_row_schema_gate(row: dict[str, Any], *,
                                   old_shortcut_count: int) -> None:
    require(set(row) == {
...
def checker_rebuild_occurrence_preflight(
    ...
    require(len(contexts) == 31,
            "checker production 31-context registry")
...
def affine_checker_seed_words(q3: dict[str, Any], e3: Quotient) -> dict[str, Any]:
    """Independently rebuild the frozen 104 prefix plus four triple cubes."""
    records = q3["correction_fibre"]["records"]
    require(len(records) == 27 and not records[0]["word"],
            "checker triple4 correction fibre shape")
    cubes: list[list[int]] = []
    ...
    for record_index, row in enumerate(records, 1):
        ...
        require(tuple(cube) not in seen_cubes, "checker duplicate cube")
        seen_cubes.add(tuple(cube)); record_to_cube[record_index] = len(cubes)+1
        cubes.append(cube)
    require(len(cubes) == 26 and digest_obj(cubes) == TRIPLE4_CUBES_SHA,
            "checker affine cube count/digest")
    seeds: list[list[int]] = []
    ...
    for manifest in TRIPLE4_MANIFEST:
        ...
        require(record_tuple in TRIPLE4_RECORD_TUPLES and
                cube_tuple in TRIPLE4_CUBE_TUPLES and
                tuple(record_to_cube[index] for index in record_tuple) ==
                cube_tuple, "checker triple4 tuple binding")
        ...
        require(len(word) == manifest["length"] and
                ...
                "checker triple4 word manifest")
        require(exponent_sums(word, 2) == [0, 0] and
                e3.eval(embed_f2(word)) == e3.identity,
                "checker triple4 roof typing")
        require(tuple(word) not in seen, "checker triple4 duplicate seed")
        seen.add(tuple(word)); new_seeds.append(word)
        new_provenance.append({
            "family": "positive_triple_cube", "global_index": 104+len(new_seeds),
            "record_tuple": list(record_tuple), "cube_tuple": list(cube_tuple),
            "ordered_product": "cube_a * cube_b * cube_c",
            "repeated_indices_literal": len(set(cube_tuple)) != 3,
            "reduced_length": len(word), "reduced_sha256": digest_obj(word),
            "exponent_sums": exponent_sums(word, 2), "E3_identity": True,
        })
    all_seeds = seeds + new_seeds
    require(len(all_seeds) == 108 and len({tuple(x) for x in all_seeds}) == 108,
            "checker 108 seed universe")
    ...
    return {"cube_words": cubes, "seed_words": all_seeds,
            "old_seed_words": seeds, "new_seed_words": new_seeds,
            "cube_count": 26, "seed_count": 108,
            "old_seed_count": 104, "new_seed_count": 4,
...
class CheckerAffineSystem:
    def __init__(self, variables: int,
        ...
        require(len(coordinate_widths) == 2 and
                ...
                "checker affine coordinate widths")
        ...
        self.equation_history: list[tuple[dict[int, int], int, Any]] = []
    ...
    def _account_provenance(self, candidate_entries: int = 0) -> None:
        ...
        require(live >= 0, "checker provenance live accounting")
        ...
        while row:
            ...
            if old is None:
                ...
                require(len(self.provenance[pivot]) <= len(self.rows),
                        "checker pivot provenance rank invariant")
                return True
            ...
            for key, term in old_provenance.items():
                ...
                if result: provenance[key] = result
                else: provenance.pop(key, None)
        if value:
            self._account_provenance(len(provenance))
            self.consistent = False
            factor = 1 if value == 1 else 2
            witness = {key: (factor*term) % 3
                       for key, term in provenance.items() if (factor*term) % 3}
            require(witness, "checker nonempty dual witness")
    ...
    def dual_public(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "equations": equations, "support_count": len(equations),
    ...
    def verify_dual(self, public: dict[str, Any] | None) -> None:
        require(public == self.dual_public(), "checker dual replay binding")
        if public is None:
            require(self.consistent, "checker missing inconsistent dual")
            return
        coefficients = {json.dumps(row["label"], sort_keys=True,
                                   separators=(",", ":")): row["coefficient"]
                        for row in public["equations"]}
        aggregate: dict[int, int] = {}
        rhs = 0
        for equation, equation_rhs, label in self.equation_history:
            key = self._label_key(label)
            multiplier = coefficients.get(key, 0)
            for column, value in equation.items():
                aggregate[column] = (aggregate.get(column, 0) +
                                     multiplier*value) % 3
            rhs = (rhs + multiplier*equation_rhs) % 3
        require(not any(aggregate.values()) and rhs == 1 and
                public["yTz_mod3"] == 2 and
                public["all_108_annihilation_dimension"] == self.variables and
                public["all_108_annihilation_sha256"] ==
                    digest_obj([0] * self.variables) and
                public["live_provenance_entries"] == sum(
                    len(item) for item in self.provenance.values()) and
                public["witness_provenance_entries"] ==
                    len(self.dual_witness) and
                public["peak_live_provenance_entries"] ==
                    self.peak_live_provenance_entries and
                public["support_count"] <= AFFINE_CAPS["dual_support"],
                "checker public dual equation witness")
        for equation in public["equations"]:
            label = equation["label"]
            require(isinstance(label, list) and len(label) == 4 and
                    isinstance(label[2], int) and 1 <= label[2] <= 6 and
                    isinstance(label[3], str) and
                    len(label[3]) == self.blob_hex_length and
                    all(char in "0123456789abcdefABCDEF" for char in label[3]) and
                    len(bytes.fromhex(label[3])) == self.blob_width,
                    "checker public dual label encoding")
...
def checker_delta_matrix_rank(delta_rows: dict[Any, dict[int, int]],
    ...
    """Independent rank of the current target's 108-column delta map."""
...
def checker_target_row_transposed(
    ...
    for row_index, coordinate in enumerate(coordinates, 1):
        ...
        component, blob = coordinate
        label = [int(target_ordinal), str(target_name), int(component), blob]
        system.add(coefficients, -base_remainder.get(coordinate, 0), label)
    return {"ordinal": target_ordinal,
            ...
            "nullity": AFFINE_CAPS["affine_variables"]-system.rank(),
...
def checker_affine_common_envelope(data: dict[str, Any]) \
    ...
    no_claims = {"claim_classification": "unknown_not_obstruction",
                 "claim_scope":
                     "registered_old104_plus_four_triple_cube_affine_span_against_fixed_D2_prefix",
    ...
    require(data.get("claim_boundary") ==
            (positive_claims if token ==
             "B345_SEEDSPAN_TRIPLE4_POSITIVE" else no_claims),
    ...
    partial = data.get("partial") if token == \
        "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE" else None
    if token != "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE":
    ...
    if partial is not None:
        require(set(partial) == {
                ...
                0 <= partial["current_seed_index"] <= AFFINE_CAPS["seed_count"] and
...
def checker_affine_injected_core(data: dict[str, Any],
    ...
    """
    if data.get("terminal_token") == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT":
    ...
    if partial is not None:
        ...
        return
    require(data["terminal_token"] == "B345_SEEDSPAN_TRIPLE4_POSITIVE" and
...
def checker_affine_metadata_gate(data: dict[str, Any], q3: dict[str, Any],
    ...
    expected_source_hashes = {
        "producer_sha256": digest_file(
            repo/"search/d972_b345_seedspan_triple4_v1.py"),
    ...
    if has_rebuild:
        ...
        early = (partial is not None and partial["phase"] in {
            ...
            "fresh_immutable_prefix"}) or (
                data["terminal_token"] ==
                "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE" and
                data["reason"] == "affine_seed_preflight_unsupported")
        require(early, "affine missing prefix metadata stage")


def checker_affine_production_schema_partition(data: dict[str, Any]) -> None:
    """Reject injected core/partial/input ledgers on the real receipt path."""
    require("core_validation" not in data,
            "affine production core_validation injection")
    expected = checker_affine_expected_top_keys(data)
    require(set(data) == expected,
            "affine production exact terminal/stage top-level schema")
    token = data.get("terminal_token")
    if token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT":
        require("input_errors" in data and "partial" not in data,
                "affine UNKNOWN_INPUT schema partition")
    else:
        require("input_errors" not in data,
                "affine non-input input_errors injection")
    if token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE":
        require("partial" in data and data.get("partial") is not None,
                "affine UNKNOWN_RESOURCE partial schema partition")
        require(all(key not in data for key in AFFINE_POSITIVE_TOP_LEVEL),
                "affine UNKNOWN_RESOURCE positive-only schema partition")
    else:
        require("partial" not in data,
                "affine non-resource partial injection")
    performance = data.get("performance")
    require(isinstance(performance, dict),
            "affine production terminal performance")
    if token == "B345_SEEDSPAN_TRIPLE4_POSITIVE":
        require(data.get("reason") ==
                "all_33_typed_acceptance_proofs_replayed" and
                performance.get("phase_complete") ==
                "positive_typed_replay_complete",
                "affine positive terminal values")
        positive_replay = data.get("positive_replay")
        require(isinstance(positive_replay, dict) and
                data.get("diagnostics") ==
                positive_replay.get("diagnostics"),
                "affine positive diagnostics binding")
    elif token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT":
        errors = data.get("input_errors")
        require(isinstance(errors, dict) and
                data.get("reason") ==
                errors.get("authenticated_external_input"),
                "affine UNKNOWN_INPUT reason binding")
        ...
        require(isinstance(partial, dict) and
                ...
                "affine UNKNOWN_RESOURCE phase binding")
    elif token == "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE":
        if data.get("reason") == "affine_seed_preflight_unsupported":
            require(performance.get("phase_complete") == "source_preflight",
                    "affine preflight phase binding")
        elif data.get("reason") == "affine_system_inconsistent":
            rows = data.get("targets")
            require(isinstance(rows, list) and rows and
                    all(isinstance(row, dict) for row in rows),
                    "affine inconsistent target ledger")
            last_ordinal = max(int(row["ordinal"]) for row in rows)
            require(performance.get("phase_complete") ==
                    f"affine_target_{last_ordinal}",
                    "affine inconsistent phase binding")
        else:
            raise Reject("affine incomplete reason registry")
    require(isinstance(data.get("performance"), dict) and
            set(data["performance"]) ==
            {"runtime_seconds", "phase_complete"},
            "affine production performance nested schema")
    require(isinstance(data.get("resource_guards"), dict) and
            set(data["resource_guards"]) ==
            set(AFFINE_RESOURCE_GUARD_KEYS),
            "affine production resource nested schema")
    if "affine_system" in data:
        system_keys = set(AFFINE_SYSTEM_KEYS)
        if data["affine_system"].get("canonical_solution_sha256") \
                is not None:
            system_keys.add("canonical_solution_sha256")
        require(set(data["affine_system"]) == system_keys,
                "affine production affine-system nested schema")
    if "strong_canary" in data:
        require(set(data["strong_canary"]) == {
            "word", "cofaces", "target6", "raw_Fox_zero",
            "replayed_not_imported"},
            "affine production strong-canary nested schema")
    if "typed_positive_candidate" in data:
        require(set(data["typed_positive_candidate"]) ==
                set(AFFINE_TYPED_CANDIDATE_KEYS),
                "affine production typed-candidate nested schema")
    if "positive_replay" in data:
        require(set(data["positive_replay"]) ==
                set(AFFINE_POSITIVE_REPLAY_KEYS),
                "affine production positive-replay nested schema")
...
def checker_affine_unknown_input_gate(data: dict[str, Any],
    ...
    require(data.get("schema") == AFFINE_SCHEMA and
            data.get("status") == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT" and
    ...
    if errors["mathematical_scan_started"] is False:
        for key in {
                "affine_system", "targets", "diagnostics", "seed_family",
                "normalized_inverse_fibre", "occurrence_preflight",
                "prefix_accounting", "strong_canary",
                "typed_positive_candidate", "partial"}:
    ...
    performance_phase = data["performance"].get("phase_complete")
    if errors["mathematical_scan_started"] is False:
        require(performance_phase in {
                    "authenticated_input", "q3_schema_authentication"},
                "affine UNKNOWN_INPUT pre-scan performance stage")
    else:
        require(performance_phase == "affine_source_preflight" or
                performance_phase == "fresh_immutable_prefix" or
                performance_phase.startswith("affine_target_") or
                performance_phase.startswith("positive_typed_replay"),
                "affine UNKNOWN_INPUT started performance stage")
...
def checker_affine_validate(data: dict[str, Any], q3: dict[str, Any],
    ...
    if injected_provider is not None:
        ...
        return
    checker_affine_production_schema_partition(data)
    require(data.get("schema") == AFFINE_SCHEMA and
            data.get("terminal_token") in AFFINE_TERMINALS and
            data.get("status") == data.get("terminal_token"),
    ...
    require(claims == (positive_claims if
                       token == "B345_SEEDSPAN_TRIPLE4_POSITIVE" else
    ...
    if resource_partial is not None:
        require(set(resource_partial) == {
                ...
                0 <= resource_partial["current_seed_index"] <= AFFINE_CAPS["seed_count"] and
    ...
    require(data["strong_canary"] == strong,
    ...
    system = CheckerAffineSystem(
        ...
        coordinate_widths=(e4.degree, e4.collector.n)); rows: list[dict[str, Any]] = []
    for ordinal, (name, kind, root) in enumerate(base_compiled["acceptance"], 1):
        if resource_partial is not None and ordinal > \
                resource_partial["evaluated_targets"]:
            break
        # The identity topology is a shortcut only for the old 104 columns;
        # the four appended columns are independently direct/typed replayed.
        if ordinal <= 5:
            ...
            require(base_gradient == {},
                    f"checker charming base identity gradient {name}")
            delta_rows: dict[tuple[int, str], dict[int, int]] = {}
            live_remainder_entries = 0
            for index, seed in enumerate(
                    seed_info["new_seed_words"],
                    AFFINE_CAPS["old_seed_count"]+1):
                checker_deadline("checker new triple4 target seed", force=True)
                ...
                require(evaluator.values[seed_root-1] == e4.identity,
                        f"checker new seed target identity {name}")
                ...
                if compiled["dag"].counts[seed_root-1] <= \
                    ...
                    require(direct == seed_gradient and
                            ...
                            f"checker new seed direct gradient {name}")
                typed_one = checker_make_typed_positive(
                    [0 if position != index-1 else 1
                     for position in range(len(seed_info["seed_words"]))],
                    seed_info["seed_words"])
                typed_one_targets = checker_build_typed_targets(
                ...
                typed_raw = typed_one_eval.gradients(
                    [typed_one_root])[typed_one_root]
                checker_raw_split_gate(
                    seed_gradient, evaluator.values[seed_root-1],
                    typed_raw, typed_one_eval.values[typed_one_root-1],
                    e4.identity, f"checker new seed {name}/{index}")
                split_ledger.append({
                    "seed_index": index,
                    "gradient_sha256": digest_obj(check_gradient_binding(
                        name, kind, typed_raw,
            ...
            target_row.update({"name": name, "kind": kind,
                          ...
                          "old_shortcut_count":
                              AFFINE_CAPS["old_seed_count"],
                          "new_direct_count": AFFINE_CAPS["new_seed_count"],
            ...
            checker_target_row_schema_gate(
        ...
        for index, seed in enumerate(seed_info["seed_words"], 1):
            ...
            checker_raw_split_gate(
                seed_gradient, e4.identity, typed_raw,
                typed_one_eval.values[typed_one_root-1], e4.identity,
                f"checker typed/flat raw chain {name}/{index}")
            split_ledger.append({
                "seed_index": index,
                "gradient_sha256": digest_obj(check_gradient_binding(
                    name, kind, typed_raw,
            ...
            for coordinate, coefficient in delta_remainder.items():
                delta_rows.setdefault(coordinate, {})[index-1] = coefficient
        checker_split_ledger_gate(split_ledger, old_shortcut_count=0)
        row = checker_target_row_transposed(
            system, base_remainder, delta_rows, ordinal,
            live_remainder_entries, name)
        row.update({"name": name, "kind": kind,
               "diagnostics_excluded": True, "seed_count": len(seed_info["seed_words"]),
               "typed_split_count": len(split_ledger),
               "typed_split_old104_count": AFFINE_CAPS["old_seed_count"],
               "typed_split_new4_count": AFFINE_CAPS["new_seed_count"],
               "typed_split_order": "old104_then_new4",
        ...
        checker_target_row_schema_gate(row, old_shortcut_count=0)
        rows.append(row)
    ...
    if resource_partial is not None:
        ...
        if positive_phase.startswith("positive_typed_replay"):
            ...
            require(isinstance(partial_system, dict) and
                    partial_system.get("variables") == AFFINE_CAPS["affine_variables"] and
                    partial_system.get("rank") == system.rank() and
                    partial_system.get("nullity") == AFFINE_CAPS["affine_variables"]-system.rank() and
                    partial_system.get("consistent") is True and
                    partial_system.get("row_space_sha256") == system.digest() and
                    partial_system.get("dual_witness") is None and
        ...
        if not rows:
            ...
            require(isinstance(partial_system, dict) and
                    ...
                    partial_system.get("row_space_sha256") == system.digest() and
                    partial_system.get("dual_witness") == system.dual_public(),
                    "checker partial affine system")
        return
    expected_system = data["affine_system"]
    require(expected_system["variables"] == AFFINE_CAPS["affine_variables"] and
            expected_system["rank"] == system.rank() and
            expected_system["nullity"] == AFFINE_CAPS["affine_variables"]-system.rank() and
            expected_system["consistent"] is system.consistent and
            expected_system["row_space_sha256"] == system.digest() and
            expected_system.get("dual_witness") == system.dual_public(),
            "checker affine system binding")
    require(data.get("dual_witness") == system.dual_public() and
            data.get("dual_provenance") == system.dual_public(),
            "checker top-level dual witness binding")
    system.verify_dual(data.get("dual_witness"))
    if data["terminal_token"] == "B345_SEEDSPAN_TRIPLE4_POSITIVE":
        candidate = data.get("typed_positive_candidate")
        require(isinstance(candidate, dict) and
                set(candidate) == set(AFFINE_TYPED_CANDIDATE_KEYS),
                "checker positive typed-candidate nested schema")
        require(isinstance(data.get("positive_replay"), dict) and
                set(data["positive_replay"]) ==
                set(AFFINE_POSITIVE_REPLAY_KEYS),
                "checker positive replay nested schema")
        require(data.get("diagnostics") ==
                data["positive_replay"].get("diagnostics"),
                "checker positive diagnostics binding")
        require(isinstance(candidate, dict) and
        ...
        checker_validate_positive_replay(
            ...
            pool, basis)
    elif data["terminal_token"] == "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE":
    ...
    elif data["terminal_token"] == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE":
...
def affine_checker_self_test() -> None:
    ...
    dual_fixture = CheckerAffineSystem(AFFINE_CAPS["affine_variables"])
    dual_fixture.add({0: 1, 1: 1}, 0, [6, "target6", 1, "00"])
    require(not dual_fixture.add({0: 1, 1: 1}, 1,
                                 [6, "target6", 2, "00"]),
            "checker dual contradiction")
    dual_public = dual_fixture.dual_public()
    require(isinstance(dual_public, dict) and
            ...
            dual_public["coordinate_encoding"] == {
                ...
                "endianness": "byte-string order; no integer reinterpretation",
                "pivot_order": "component then exact E4 bytes"},
            "checker normalized dual fixture")
    dual_fixture.verify_dual(dual_public)
    forged_dual = dict(dual_public)
    forged_dual["support_sha256"] = "0"*64
    expect_reject(lambda: dual_fixture.verify_dual(forged_dual),
                  "checker dual digest mutation")
    forged_width = dict(dual_public)
    forged_width["coordinate_encoding"] = dict(
        dual_public["coordinate_encoding"], blob_hex_length=4)
    expect_reject(lambda: dual_fixture.verify_dual(forged_width),
                  "checker dual width mutation")
    expect_reject(lambda: dual_fixture.add(
        {0: 1}, 0, [6, "target6", 1, "0000"]),
                  "checker dual label hex width mutation")
    for bad_component, bad_blob, label in [
            (0, "00", "zero component"),
            (7, "00", "seven component"),
            (1, "000", "odd blob length"),
            (1, "zz", "nonhex blob")]:
        expect_reject(lambda c=bad_component, b=bad_blob:
                      dual_fixture.add({0: 1}, 0,
                                       [6, "target6", c, b]),
                      f"checker dual encoding mutation: {label}")
    ...
    metadata_source_hashes = {
        "producer_sha256": digest_file(
            selftest_repo/"search/d972_b345_seedspan_triple4_v1.py"),
        "checker_sha256": digest_file(Path(__file__).resolve()),
        "driver_sha256": digest_file(
            selftest_repo/"search/d972_b345_seedspan_triple4_gha_driver_v1.g"),
        "strong_prefix_sha256": digest_file(
            selftest_repo/AFFINE_STRONG_SOURCE),
    }
    base = {
        "schema": AFFINE_SCHEMA,
        "status": "B345_SEEDSPAN_TRIPLE4_POSITIVE",
        "terminal_token": "B345_SEEDSPAN_TRIPLE4_POSITIVE",
    ...
    invoke(control)
    def production_shape(token: str, reason: str, *, phase: str = "",
                         evaluated: int = 0, target: int = 0,
                         seed: int = 0) -> dict[str, Any]:
        shape = {key: base.get(key) for key in AFFINE_COMMON_TOP_LEVEL}
        resource_hit = token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE"
        shape["resource_guards"] = {
            "seconds": 18000, "minutes": 300, "rss_bytes": 4831838208,
            "rss_gib": 4.5, "external_job_limit_minutes": 330,
            "safety_margin_minutes": 30, "clock": "time.monotonic",
            "rss_primary": "/proc/self/status VmRSS",
            "rss_portable_fallback": "getrusage peak or injected selftest reader",
            "hit": resource_hit,
            "hit_reason": reason if resource_hit else None,
            "last_checked_phase": phase or "schema_fixture",
            "check_count": 0, "current_rss_bytes": 0,
            "peak_rss_bytes": 0,
            "terminal_on_hit": "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE",
            "consulted_in_selftest": False, "schema": AFFINE_SCHEMA,
        }
        shape.update({"status": token, "terminal_token": token,
                      "reason": reason, "claim_boundary":
                      positive_claims if token ==
                      "B345_SEEDSPAN_TRIPLE4_POSITIVE" else no_claims,
                      "performance": {"runtime_seconds": 0.0,
                          "phase_complete": phase or "schema_fixture"}})
        if token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT":
            shape["input_errors"] = {
            ...
            cap = checker_resource_cap_limit(reason)
            relation = "ge" if reason == "element_pool" else "gt"
            shape["partial"] = {
                "phase": phase or "authenticated_input",
                ...
                "cap_key": reason, "cap_limit": cap,
                "observed_count": cap+1 if relation == "gt" else cap,
                "trigger_relation": relation,
                "affine_equations": 0,
            }
        elif token == "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE":
            if reason == "affine_seed_preflight_unsupported":
                shape.update({key: base.get(key) for key in
                              AFFINE_Q3_COMPLETE_TOP_LEVEL})
            else:
                shape.update({key: base.get(key) for key in
                              AFFINE_Q3_COMPLETE_TOP_LEVEL |
                              AFFINE_PREFIX_TOP_LEVEL |
                              AFFINE_TARGET_TOP_LEVEL |
                              AFFINE_TARGET6_TOP_LEVEL |
                              AFFINE_DUAL_TOP_LEVEL})
                shape["targets"] = [{"ordinal": 6}]
            ...
            shape.update({key: base.get(key) for key in
                          ...
                          AFFINE_TARGET_TOP_LEVEL |
                          AFFINE_TARGET6_TOP_LEVEL |
                          AFFINE_POSITIVE_TOP_LEVEL})
            shape["targets"] = [{"ordinal": 6}]
        expected = checker_affine_expected_top_keys(shape)
        for key in expected:
            shape.setdefault(key, base.get(key))
        if "affine_system" in expected:
            system_shape = {
                "variables": 108, "rank": 0, "nullity": 108,
                "consistent": True, "row_space_sha256": "0"*64,
                "dual_witness": None,
            }
            if token == "B345_SEEDSPAN_TRIPLE4_POSITIVE" or \
                    (token == "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE" and
                     phase.startswith("positive_typed_replay")):
                system_shape["canonical_solution_sha256"] = "0"*64
            shape["affine_system"] = system_shape
        if "strong_canary" in expected:
            shape["strong_canary"] = {
                "word": [], "cofaces": [], "target6": {},
                "raw_Fox_zero": True, "replayed_not_imported": True,
            }
        if "typed_positive_candidate" in expected:
            shape["typed_positive_candidate"] = {
                key: None for key in AFFINE_TYPED_CANDIDATE_KEYS}
        if "positive_replay" in expected:
            shape["positive_replay"] = {
                key: None for key in AFFINE_POSITIVE_REPLAY_KEYS}
            shape["positive_replay"]["diagnostics"] = []
        if "diagnostics" in expected:
            shape["diagnostics"] = []
        return shape

    production_control = production_shape(
        "B345_SEEDSPAN_TRIPLE4_POSITIVE",
        "all_33_typed_acceptance_proofs_replayed",
        phase="positive_typed_replay_complete")
    checker_affine_production_schema_partition(production_control)
    require(production_control["diagnostics"] ==
            production_control["positive_replay"]["diagnostics"],
            "checker production success diagnostics canary")
    bad_success_diagnostics = copy.deepcopy(production_control)
    bad_success_diagnostics["diagnostics"] = [{"forged": True}]
    expect_reject(
        lambda: checker_affine_production_schema_partition(
            bad_success_diagnostics),
        "checker production success diagnostics mutation")
    bad_success_reason = copy.deepcopy(production_control)
    bad_success_reason["reason"] = "schema fixture"
    expect_reject(
        lambda: checker_affine_production_schema_partition(
            bad_success_reason),
        "checker production success reason mutation")
    bad_schema = copy.deepcopy(production_control)
    bad_schema["core_validation"] = {}
    expect_reject(lambda: checker_affine_production_schema_partition(bad_schema),
                  "checker production core_validation partition")
    bad_schema = copy.deepcopy(production_control)
    bad_schema["input_errors"] = {"injected": True}
    expect_reject(lambda: checker_affine_production_schema_partition(bad_schema),
                  "checker production input_errors partition")
    inconsistent_shape = production_shape(
        "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE",
        "affine_system_inconsistent", phase="affine_target_6")
    bad_schema = copy.deepcopy(inconsistent_shape)
    bad_schema["typed_positive_candidate"] = {}
    expect_reject(lambda: checker_affine_production_schema_partition(bad_schema),
                  "checker nonpositive positive-only schema partition")
    bad_schema = copy.deepcopy(production_control)
    bad_schema["partial"] = {}
    expect_reject(lambda: checker_affine_production_schema_partition(bad_schema),
                  "checker positive resource-only schema partition")
    early_shape = production_shape(
        "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE",
        "affine_seed_preflight_unsupported", phase="source_preflight")
    bad_schema = copy.deepcopy(early_shape)
    bad_schema["positive_replay"] = {}
    expect_reject(lambda: checker_affine_production_schema_partition(bad_schema),
                  "checker early incomplete late-proof schema partition")
    ...
    reject_receipt("terminal status", lambda x: x.update(
        {"status": "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE"}))
    ...
    reject_receipt("terminal incomplete", lambda x: x.update(
        {"status": "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE",
         "terminal_token": "B345_SEEDSPAN_TRIPLE4_SEARCH_INCOMPLETE"}))
    reject_receipt("terminal unknown input", lambda x: x.update(
        {"status": "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT",
         "terminal_token": "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT"}))

    unknown = production_shape(
        "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT",
    ...
    invoke(unknown)
    checker_affine_production_schema_partition(unknown)
    bad_unknown_schema = copy.deepcopy(unknown)
    bad_unknown_schema.pop("input_errors")
    expect_reject(
        lambda: checker_affine_production_schema_partition(bad_unknown_schema),
        "checker UNKNOWN_INPUT missing input_errors partition")
    ...
    fresh_unknown = production_shape(
        "B345_SEEDSPAN_TRIPLE4_UNKNOWN_INPUT",
        "selftest fresh prefix TOCTOU drift",
        phase="fresh_immutable_prefix")
    fresh_unknown["input_errors"]["mathematical_scan_started"] = True
    for key in AFFINE_Q3_COMPLETE_TOP_LEVEL:
        fresh_unknown[key] = {}
    checker_affine_production_schema_partition(fresh_unknown)
    fresh_unknown_bad = copy.deepcopy(fresh_unknown)
    fresh_unknown_bad.pop("occurrence_preflight")
    expect_reject(
        lambda: checker_affine_production_schema_partition(fresh_unknown_bad),
        "checker UNKNOWN_INPUT fresh-prefix schema mutation")
    ...
    def resource_receipt(phase: str, evaluated: int,
        ...
        return production_shape(
            "B345_SEEDSPAN_TRIPLE4_UNKNOWN_RESOURCE", reason,
            phase=phase, evaluated=evaluated, target=target, seed=seed)

    resource_control = resource_receipt("authenticated_input", 0, 0, 0)
    checker_affine_production_schema_partition(resource_control)
    bad_resource_positive = copy.deepcopy(resource_control)
    bad_resource_positive["diagnostics"] = []
    expect_reject(
        lambda: checker_affine_production_schema_partition(
            bad_resource_positive),
        "checker resource positive-only diagnostics mutation")
    invoke(resource_control)
    bad_resource_schema = copy.deepcopy(resource_control)
    bad_resource_schema["partial"] = None
    expect_reject(
        lambda: checker_affine_production_schema_partition(bad_resource_schema),
        "checker UNKNOWN_RESOURCE partial partition")
    ...
    checker_split_ledger_gate(split_fixture, old_shortcut_count=104)
    bad_split = copy.deepcopy(split_fixture)
    bad_split[104]["identity_shortcut"] = True
    bad_split[104].pop("direct_replay")
    expect_reject(lambda: checker_split_ledger_gate(
        bad_split, old_shortcut_count=104),
        "checker new-column shortcut mutation")
    checker_raw_split_gate({0: 1}, toy_quotient.identity, {0: 1},
                           toy_quotient.identity, toy_quotient.identity,
                           "checker old/new raw formula")
    expect_reject(lambda: checker_raw_split_gate(
        {0: 1}, toy_quotient.identity, {0: 2}, toy_quotient.identity,
        toy_quotient.identity, "checker raw drift"),
        "checker raw direct/typed drift")
    selected = checker_literal_selected_replay(
        [2, 2, 0], [[1], [2], [3]])
    require(selected == reduce_word([1, 1, 2, 2]) and
            selected != reduce_word([1, 2]),
            "checker consistent literal selected replay")
    nonzero_base = {0: 1}
    toy_columns = [{index: 1} for index in range(AFFINE_CAPS["seed_count"])]
    require(nonzero_base and len(toy_columns) == 108,
            "checker nonzero base/108 columns")
    wide = CheckerAffineSystem(AFFINE_CAPS["affine_variables"])
    ...
    require(not wide.consistent and wide.dual_public()["support_count"] > 1 and
            ...
            "checker 108-column support-two inconsistency")
    row_system = CheckerAffineSystem(AFFINE_CAPS["affine_variables"])
    toy_row = checker_target_row_transposed(
        row_system, {}, {}, 1, 0, "toy_split_target")
    toy_row.update({
    ...
    checker_target_row_schema_gate(toy_row, old_shortcut_count=0)
    bad_row = copy.deepcopy(toy_row)
    bad_row["new_direct_count"] = 104
    expect_reject(lambda: checker_target_row_schema_gate(
        bad_row, old_shortcut_count=0),
        "checker target-row split mutation")
    print("D972_B345_SEEDSPAN_TRIPLE4_CHECKER_SELFTEST_PASS "
          ...
          "affine_consistent=1 affine_inconsistent=1 selected_proof=1 "
           ...
           "prefix_counts=1 deadline=1 split_ledger=1 row_schema=1 "
...
def main() -> int:
    ...
    print(f"D972_B345_SEEDSPAN_TRIPLE4_CHECK_PASS terminal={data['terminal_token']} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"B345_SEEDSPAN_TRIPLE4_CHECK_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
