...
def mutation_fixture(rows, p0, task198_meta, g760, interface, consumer,
    ...
    return {
        ...
        "interface": copy.deepcopy(interface), "consumer": copy.deepcopy(consumer),
...
def cheap_validate(value, reference, p0):
    ...
    for actual, expected in zip(value["ledger"], local_ledger()):
...
def run_mutations(rows, p0, task198_meta, g760, interface, consumer,
    ...
    require(cheap_validate(reference, reference, p0) is True,
            "untouched cheap baseline")
    records = []
    for name in roster:
        mutant = copy.deepcopy(reference)
        if name == "task198_raw_manifest_binding":
            mutant["authority"]["manifest_contract"]["producer"]["member"]["sha256"] = "0" * 64
            owner = "authority"
            ...
            mutant["central"]["blocks"]["H1"]["signed_rows"][0][0] = 3
            owner = "central"
        elif name == "H2_central_row":
            mutant["central"]["blocks"]["H2"]["signed_rows"][0][0] = 6
            owner = "central"
        elif name == "P_central_row":
            mutant["central"]["blocks"]["P"]["signed_rows"][0][3] = 3
            owner = "central"
        elif name == "projected_area_target":
            mutant["areas"]["rows"][0]["bar_epsilon_1"]["H1"] = []
            owner = "areas"
        elif name == "ABI_seal_target":
            mutant["consumer"]["bar_epsilon_1"]["H1"] = []
            owner = "consumer"
...
def load_engine(rel, size, sha, tag_name, meter):
    ...
    module_name = "_d363_producer_" + tag_name
...
def production(meter):
    ...
    rows, evaluator_contract, task198_meta = authenticate_task198(
        p0, pins, raw_by_path)
    raw_by_path.clear()
    del raw_by_path
    meter.sample_rss("task198 raw released before result construction")
    ...
    del package, full_abi, evaluator_contract
...
def main(argv=None):
    ...
    parser.add_argument("--output",
                        default="ci/out/d972_r07_pre_a0_single_target_a3_v2.json")
    args = parser.parse_args(argv)
    meter = Meter()
    try:
        target = prepare_output(args.output)
        terminal, receipt = production(meter)
