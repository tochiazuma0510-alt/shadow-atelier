#!/usr/bin/env python3
"""Task791: reached-seed direct canary successor for the endpoint consumer.
...
PREFIX_TABLE_SCHEMA = 'r07.v512.actual-g-dependent-prefix-table.v1'
V484_PROOF_SHA256 = '25e292c8d996000c5dd442619f9afa269d83193ce5f58e4f3536c55b61f77492'
V500_PROOF_SHA256 = 'f0efc3d4292e512bfc8ff920c1c54ce31257310566c5e89b2981d287372a3318'
RUNTIME_PROFILE = {
    'profile': 'endpoint-minimal-reached-seed-canary-v1',
    'contexts': 31,
    ...
    'v500_proof_sha256': V500_PROOF_SHA256,
    'direct_replay': DIRECT_REPLAY_LABEL,
...
PAPER_PINS['sol/proof_r07_direct_all_seven_signature_bucket_replay_v500.md'] = V500_PROOF_SHA256
...
def build_endpoint_minimal(module, registry, meter, words):
    ...
    # The empty word is the mathematical identity, not a producer replay.
    # Keep this canary on the quotient identities and the authenticated
    # deletion map so the production preamble performs no generic endpoint
    # evaluation.
    ...
    require(runtime['delete'](e4.identity) == e3.identity,
            'endpoint_zero_deletion_identity')
    typed_identity = identity_signature(producer)
    require(tuple(raw for _kind, raw in typed_identity) ==
            (identity_e3,) * 6 + (identity_e4,) * 5,
            'endpoint_zero_identity_signature')
    endpoint_checkpoint(meter, 'endpoint_minimal_step_7_identity_signature_canary')
...
def identity_signature(model):
    """Build the eleven-slot identity from quotient identities only.

    MatchedQuotient.eval(()) is its stored identity and the authenticated
    deletion map sends the E4 identity to the E3 identity.  This helper keeps
    that finite fact explicit without calling ProducerAllSeven.coordinates.
    """
    runtime = getattr(model, 'rt', None)
    require(type(runtime) is dict, 'identity runtime')
    p176, e3, e4 = runtime.get('p176'), runtime.get('e3'), runtime.get('e4')
    delete = runtime.get('delete')
    require(p176 is not None and e3 is not None and e4 is not None and
            callable(delete), 'identity runtime components')
    e3_identity, e4_identity = e3.identity, e4.identity
    require(delete(e4_identity) == e3_identity,
            'identity deletion map')
    e3_raw = p176.packed_joint_blob(e3_identity,
                                    'endpoint direct E3 identity')
    e4_raw = p176.packed_joint_blob(e4_identity,
                                    'endpoint direct E4 identity')
    return tuple(('E3' if index < 6 else 'E4',
                  e3_raw if index < 6 else e4_raw)
                 for index in range(len(TEN)))
...
def build_atom_cache(model, meter=None):
    """Evaluate each actor atom exactly once and retain its typed signature."""
    cache = {}
    for slot, letter in enumerate(ATOM_LETTERS, 1):
        if meter is not None:
            endpoint_checkpoint(
                meter, 'endpoint_atom_' + str(slot) + '_' + str(letter) + '_start')
        require(letter not in cache, 'endpoint_atom_duplicate')
        cache[letter] = signature((letter,), model)
        if meter is not None:
            endpoint_checkpoint(
                meter, 'endpoint_atom_' + str(slot) + '_' + str(letter) + '_complete')
    require(tuple(cache) == ATOM_LETTERS and
            all(len(value) == len(TEN) for value in cache.values()),
            'endpoint_atom_cache_shape')
    return cache


def extend_signature(parent, letter, atom_cache, model, v12f):
    require(type(atom_cache) is dict and tuple(atom_cache) == ATOM_LETTERS,
            'endpoint_atom_cache_binding')
    atom=atom_cache.get(int(letter))
    require(atom is not None, 'endpoint_atom_missing')
    require(len(parent) == len(atom) == len(TEN),
            'endpoint_signature_width')
    answer=[]
    for index,((parent_kind,left_raw),(atom_kind,right_raw)) in enumerate(zip(parent,atom)):
        expected_kind = 'E3' if index < 6 else 'E4'
        require(parent_kind == expected_kind,
                'endpoint_parent_signature_type')
        require(atom_kind == expected_kind,
                'endpoint_atom_signature_type')
        ...
        answer.append((expected_kind,v12f.producer_element_blob(model.rt,quotient.mul(left,right))))
    return tuple(answer)


def fold_signature(word, atom_cache, model, v12f, initial=None):
    answer = identity_signature(model) if initial is None else initial
    for letter in word:
        answer = extend_signature(answer, int(letter), atom_cache, model, v12f)
    return answer


def full_signature_gate(value):
    require(type(value) is tuple and len(value) == len(TEN),
            'full_signature_width')
    for index, row in enumerate(value):
        require(type(row) is tuple and len(row) == 2 and
                row[0] == ('E3' if index < 6 else 'E4') and
                type(row[1]) is bytes, 'full_signature_type')
    return value


def canonical_signature_buckets(complete, signatures):
    """The existing last-path representative convention on full signatures."""
    buckets = {}
    for seed, word, coefficient in complete:
        path = tuple(word)
        require(path in signatures, 'signature_path_missing')
        key = (seed, full_signature_gate(signatures[path]))
        old = buckets.get(key, (0, path))
        buckets[key] = ((old[0] + coefficient) % 3, path)
    return {key: value for key, value in buckets.items() if value[0]}


def validate_signature_buckets(value, complete, signatures):
    require(value == canonical_signature_buckets(complete, signatures),
            'signature_bucket_canonical')
    return value


def validate_direct_completion(done, total):
    require(type(done) is int and type(total) is int and
            done == total and done >= 0, 'endpoint_direct_bucket_incomplete')


OCCURRENCE_LAYOUT = (
    (1, 'H1_fxy', 1, 1, 0), (2, 'H1_fxz', 1, -1, 1),
    (3, 'H1_fyz', 1, 1, 2), (4, 'H2_fux', 2, -1, 3),
    (5, 'H2_fxy', 2, -1, 0), (6, 'H2_fuy', 2, 1, 4),
    (7, 'P_b1', 3, 1, 5), (8, 'P_b2', 3, 1, 6),
    (9, 'P_b3', 3, 1, 7), (10, 'P_b5_inverse', 3, -1, 8),
    (11, 'P_b4_inverse', 3, -1, 9),
)


def public_typed_signature(value):
    full_signature_gate(value)
    return [[kind, raw.hex()] for kind, raw in value]


def occurrence_prefix_contract(model, v12f):
    require(len(model.specs) == len(OCCURRENCE_LAYOUT),
            'endpoint_prefix_spec_count')
    rows = []
    for spec, expected in zip(model.specs, OCCURRENCE_LAYOUT):
        ordinal, label, block, factor_sign, coordinate = expected
        require((len(rows) + 1, spec['label'], int(spec['block']),
                 int(spec['sign']), int(spec['coordinate'])) == expected,
                'endpoint_prefix_layout')
        raw = v12f.producer_element_blob(model.rt, spec['occurrence_prefix'])
        rows.append({'ordinal': ordinal, 'label': label,
                     'block': int(spec['block']), 'coordinate': coordinate,
                     'sign': factor_sign, 'prefix_hex': raw.hex()})
    return {'schema': PREFIX_TABLE_SCHEMA, 'rows': rows}


def inverse_atom_equalities(atom_cache, model, v12f):
    equalities = []
    for negative, positive in ((-2, 2), (-1, 1)):
        expected = []
        for index, (kind, raw) in enumerate(atom_cache[positive]):
            block = 1 if index < 6 else 3
            quotient = model.rt['e3'] if index < 6 else model.rt['e4']
            value = v12f.producer_unpack_element(model.rt, raw, block)
            expected.append((kind, v12f.producer_element_blob(
                model.rt, quotient.inverse(value))))
        require(atom_cache[negative] == tuple(expected),
                'endpoint_atom_inverse')
        equalities.append({'negative': negative, 'positive': positive,
                           'equal': True})
    return equalities


def atom_order_anchor(atom_cache, model, v12f):
    parent_letter, atom_letter = 1, 2
    calculated = fold_signature((parent_letter, atom_letter), atom_cache,
                                model, v12f)
    direct = signature((parent_letter, atom_letter), model)
    require(calculated == direct, 'endpoint_order_anchor_direct')
    reverse = extend_signature(atom_cache[atom_letter], parent_letter,
                               atom_cache, model, v12f)
    differing = [index + 1 for index, (left, right) in enumerate(
        zip(calculated, reverse)) if left != right]
    require(differing, 'endpoint_order_anchor_commuting')
    return {'parent_letter': parent_letter, 'atom_letter': atom_letter,
            'word': [parent_letter, atom_letter],
            'reverse_word': [atom_letter, parent_letter],
            'order': 'parent*atom',
            'calculated_signature': public_typed_signature(calculated),
            'direct_signature': public_typed_signature(direct),
            'reverse_signature': public_typed_signature(reverse),
            'noncommuting_slots': differing}


def canonical_sparse_row(value):
    require(type(value) is dict, 'endpoint_base_row_type')
    rows = []
    for key, coefficient in value.items():
        require(type(key) is bytes and type(coefficient) is int and
                coefficient in (1, 2), 'endpoint_base_row_entry')
        rows.append([key.hex(), int(coefficient)])
    rows.sort(key=lambda row: bytes.fromhex(row[0]))
    require(len({row[0] for row in rows}) == len(rows),
            'endpoint_base_row_duplicate')
    return rows


def rolling_base_rows(rows):
    rolling = bytes(32)
    for row in rows:
        rolling = hashlib.sha256(rolling + canon(row)).digest()
    return rolling.hex()


def replay_reached_seed_base(reached_seeds, relators, direct_column,
                             progress, guard_call):
    """Run one complete generic direct canary per raw reached seed."""
    seeds = [int(seed) for seed in reached_seeds]
    require(seeds == sorted(set(seeds)) and
            all(1 <= seed <= len(relators) for seed in seeds),
            'endpoint_reached_seed_roster')
    total = len(seeds)
    done = 0
    rows = []
    for seed in seeds:
        row, replay = direct_column(
            (), tuple(int(x) for x in relators[seed - 1]))
        require(isinstance(replay, dict) and
                replay.get('eleven_occurrence_replay') is True and
                replay.get('direct_all_seven_replay') is True,
                'endpoint_base_row_replay_scope')
        public = canonical_sparse_row(row)
        rows.append({'seed': seed, 'nnz': len(public),
                     'canonical_sparse_row_sha256': sha(canon(public))})
        done += 1
        guard_call()
        progress(done, total)
    validate_direct_completion(done, total)
    receipt = {'reached_seeds': seeds, 'base_rows': rows,
               'completed_count': done,
               'rolling_sha256': rolling_base_rows(rows), 'eof': True}
    validate_base_receipt(receipt, seeds)
    return receipt


def validate_base_receipt(value, reached_seeds):
    seeds = [int(seed) for seed in reached_seeds]
    require(type(value) is dict and set(value) ==
            {'reached_seeds', 'base_rows', 'completed_count',
             'rolling_sha256', 'eof'}, 'endpoint_base_canary_shape')
    require(value['reached_seeds'] == seeds and
            value['completed_count'] == len(seeds) and value['eof'] is True,
            'endpoint_base_canary_eof')
    rows = value['base_rows']
    require(type(rows) is list and len(rows) == len(seeds) and
            [row.get('seed') for row in rows] == seeds and
            all(type(row) is dict and set(row) ==
                {'seed', 'nnz', 'canonical_sparse_row_sha256'} and
                type(row['nnz']) is int and row['nnz'] >= 0 and
                isinstance(row['canonical_sparse_row_sha256'], str) and
                len(row['canonical_sparse_row_sha256']) == 64
                for row in rows), 'endpoint_base_canary_rows')
    require(value['rolling_sha256'] == rolling_base_rows(rows),
            'endpoint_base_canary_rolling')
    return value


def validate_direct_canary(value, reached_seeds, atom_cache, model, v12f,
                           prefix_table, order_anchor, inverse_equalities,
                           base_receipt=None):
    require(type(value) is dict and
            set(value) == {'schema', 'label', 'reached_seeds', 'base_rows',
                           'completed_count', 'atom_signatures',
                           'inverse_equalities', 'order_anchor',
                           'prefix_table', 'generic_direct_scope',
                           'rolling_sha256', 'eof'},
            'endpoint_canary_shape')
    require(value['schema'] == DIRECT_CANARY_SCHEMA and
            value['label'] == DIRECT_REPLAY_LABEL and
            value['generic_direct_scope'] ==
            '31-context joint direct_column at empty path per reached seed; '
            'all-path propagation by conjugacy' and
            value['reached_seeds'] == list(reached_seeds) and
            value['completed_count'] == len(reached_seeds) and
            value['eof'] is True,
            'endpoint_canary_header')
    base_value = {key: value[key] for key in (
        'reached_seeds', 'base_rows', 'completed_count',
        'rolling_sha256', 'eof')}
    validate_base_receipt(base_value, reached_seeds)
    if base_receipt is not None:
        require(base_value == base_receipt, 'endpoint_canary_base_rows')
    expected_atoms = [{'letter': letter,
                       'signature': public_typed_signature(atom_cache[letter])}
                      for letter in ATOM_LETTERS]
    require(value['atom_signatures'] == expected_atoms,
            'endpoint_canary_atoms')
    require(value['inverse_equalities'] == inverse_equalities,
            'endpoint_canary_inverse')
    require(value['order_anchor'] == order_anchor,
            'endpoint_canary_order')
    require(value['prefix_table'] == prefix_table,
            'endpoint_canary_prefix')
    return value
...
def evaluate(roots,replaced,loaded,task601,candidate,out):
    ...
    atom_cache=build_atom_cache(all_seven, seven_runtime['meter'])
    ...
    identity=identity_signature(all_seven)
    trie={():identity}
    for path in paths:
        for n in range(1,len(path)+1):
            prefix=path[:n]
            if prefix not in trie:
                trie[prefix]=extend_signature(
                    trie[prefix[:-1]],prefix[-1],atom_cache,all_seven,_v12f)
    ...
    for seed in reached_seeds:
        endpoint_checkpoint(seven_runtime['meter'],
                            'endpoint_reached_seed_' + str(seed) + '_start')
        # Seed endpoint identity is a separate gate from trie recurrence and
        # remains one direct evaluation per reached seed (at most 44).
        endpoints=all_seven.coordinates(
            tuple(int(x) for x in words['relators'][seed-1]))
        identities=(all_seven.rt['e3'].identity,
                    all_seven.rt['e4'].identity)
        for j,tag in enumerate(TEN):
            expected=all_seven.rt['p176'].packed_joint_blob(
                identities[j>=6], 'endpoint reached-seed identity')
            if endpoints[tag] != expected: fail('endpoint_gate')
        endpoint_checkpoint(seven_runtime['meter'],
                            'endpoint_reached_seed_' + str(seed) + '_complete')
    buckets = canonical_signature_buckets(complete, trie)
    ...
    print('A0_PROGRESS side=producer phase=endpoint_direct_bucket_counts L=' +
          str(len(complete)) + ' U=' + str(len(trie)) +
          ' G=' + str(len(buckets)), flush=True)
    inverse_equalities = inverse_atom_equalities(atom_cache, all_seven, _v12f)
    prefix_table = occurrence_prefix_contract(all_seven, _v12f)
    order_anchor = atom_order_anchor(atom_cache, all_seven, _v12f)
    endpoint_checkpoint(seven_runtime['meter'],
                        'endpoint_reached_seed_base_canary_start')
    base_receipt = replay_reached_seed_base(
        reached_seeds, words['relators'], all_seven.direct_column,
        lambda done, total: seven_runtime['meter'].check(
            'endpoint_reached_seed_base_canary_done=' + str(done) + '/' +
            str(total)), lambda: guard(started))
    direct_canary = dict(base_receipt)
    direct_canary.update({
        'schema': DIRECT_CANARY_SCHEMA, 'label': DIRECT_REPLAY_LABEL,
        'atom_signatures': [
            {'letter': letter,
             'signature': public_typed_signature(atom_cache[letter])}
            for letter in ATOM_LETTERS],
        'inverse_equalities': inverse_equalities,
        'order_anchor': order_anchor, 'prefix_table': prefix_table,
        'generic_direct_scope':
            '31-context joint direct_column at empty path per reached seed; '
            'all-path propagation by conjugacy',
    })
    validate_direct_canary(direct_canary, reached_seeds, atom_cache,
                           all_seven, _v12f, prefix_table, order_anchor,
                           inverse_equalities, base_receipt)
    endpoint_checkpoint(seven_runtime['meter'],
                        'endpoint_reached_seed_base_canary_complete')
    ...
    aggregation_total = len(buckets)
    endpoint_checkpoint(seven_runtime['meter'],
                        'endpoint_precision2_aggregation_start')
    aggregation_done = 0
    for index,((seed,_),(coefficient,path)) in enumerate(buckets.items(),1):
        ...
        aggregation_done += 1
        guard(started)
        seven_runtime['meter'].check(
            'endpoint_precision2_aggregation_done=' + str(index) + '/' +
            str(aggregation_total))
    require(aggregation_done == aggregation_total,
            'endpoint_precision2_aggregation_incomplete')
    endpoint_checkpoint(seven_runtime['meter'],
    ...
    manifest={'schema':'d972.r07.a0.fresh-precision2-endpoint-signature.v9','marker':MARKER,'runtime_profile':RUNTIME_PROFILE,'parent':parent,'root':'Compose(C_<1,C_T)','source_ancestry_sha256':EXPECTED_FILES['source_ancestry'][2],'roots_sha256':sha(loaded['roots']),'occurrence':{'count':11,'types':['E3']*6+['E4']*5,'coordinates':list(TEN),'signs':list(SIGNS),'base_checks':base_checks,'max_base_checks':484,'all_seven_canary':True,'first_six_typed_restriction':True},'direct_canary':direct_canary,'compression':{'L':len(complete),'U':len(trie),'G':len(buckets),'G_le_L':len(buckets)<=len(complete),'seed_cache_count':len(cache),'precision2_bucket_actions':aggregation_done,'precision2_complete':aggregation_done==len(buckets)},'dimensions':{'lower':LOWER,'top':TOP,'packed_rho2':PACKED},'rho2':{'support':len(sparse),'sparse_sha256':sha(canon(sparse)),'dense_sha256':sha(top_dense),'packed_sha256':sha(packed),'packing_roundtrip':True},'files':receipts,'degree1_task625_physical_replay':True,'degree1_task595_member_equation_zero':True,'member_coefficient_count':len(candidate_body['member_coefficients']),'lower_all_zero':True,'direct_occurrence_replay':False,'next_degree2_residual':None,'grade2_MEMBER':False,'grade2_NONMEMBER':False,'A0':False,'ORDER_54432':False,'full_Q0':False,'COMMON':False,'cofinal_lift':False,'FAKE':False,'IHARA':False,'cross_checked':False,'verified':False}
...
def _tiny_signature_fixtures():
    """Bounded proof that identity and cached right recurrence are equivalent."""
    class TinyQ:
        def __init__(self, degree, generators):
            self.identity = tuple(range(degree))
            self.generators = list(generators)
    ...
    class TinyP176:
        @staticmethod
        def packed_joint_blob(value, _label):
            return bytes(value)

    class TinyV12F:
        @staticmethod
        def producer_unpack_element(_runtime, raw, _block):
            return tuple(raw)

        @staticmethod
        def producer_element_blob(_runtime, value):
            return bytes(value)
    ...
    x3, y3 = cycle(3), swap(3)
    x4, y4 = cycle(4), swap(4)
    e3 = TinyQ(3, [x3, x3, y3])
    e4 = TinyQ(4, [x4, y4, x4, y4, x4, y4])
    contexts = [(x4, y4) for _ in range(31)]
    old, p176 = TinyOld(), TinyP176()

    def delete(value):
        return e3.identity if value == e4.identity else value

    class TinyModel:
        def __init__(self):
            self.rt = {'p176': p176, 'e3': e3, 'e4': e4, 'delete': delete}
            self.calls = []
            self.specs = []
            for _ordinal, _label, _block, _sign, _coordinate in OCCURRENCE_LAYOUT:
                self.specs.append({
                    'label': _label, 'block': _block, 'sign': _sign,
                    'coordinate': _coordinate,
                    'occurrence_prefix': (bytes(e3.identity)
                                          if _block < 3 else
                                          bytes(e4.identity))})

        def coordinates(self, word):
            word = tuple(int(letter) for letter in word)
            self.calls.append(word)
            left = bytes(e3.eval(old.embed_f2_pb3(word)))
            right = tuple(bytes(e4.eval(word, context)) for context in contexts)
            return (left,) * 5 + right[:5]

    model, v12f = TinyModel(), TinyV12F()
    direct_empty = model.coordinates(())
    identity = identity_signature(model)
    identity_raw = tuple(raw for _kind, raw in identity)
    require(direct_empty == (bytes(e3.identity),) * 5 +
            (bytes(e4.identity),) * 5,
            'fixture_direct_empty_identity')
    require(tuple(direct_empty[tag] for tag in TEN) == identity_raw,
            'fixture_identity_signature_projection')

    atom_cache = build_atom_cache(model)
    require(model.calls[1:] == [(-2,), (-1,), (1,), (2,)],
            'fixture_atom_cache_once')
    inverse_equalities = inverse_atom_equalities(atom_cache, model, v12f)
    prefix_table = occurrence_prefix_contract(model, v12f)
    order_anchor = atom_order_anchor(atom_cache, model, v12f)
    def tiny_canary_direct(path, relator):
        require(tuple(path) == () and tuple(relator) == (1,),
                'fixture_canary_direct_arguments')
        return {b'R\x01\x01\x00\x01x': 1}, {
            'eleven_occurrence_replay': True,
            'direct_all_seven_replay': True}
    base_receipt = replay_reached_seed_base(
        [1], [[1]], tiny_canary_direct, lambda _done, _total: None,
        lambda: None)
    canary = dict(base_receipt)
    canary.update({
        'schema': DIRECT_CANARY_SCHEMA, 'label': DIRECT_REPLAY_LABEL,
        'atom_signatures': [
            {'letter': letter,
    ...
    validate_direct_canary(canary, [1], atom_cache, model, v12f,
                           prefix_table, order_anchor, inverse_equalities,
                           base_receipt)

    typed_mutation_rejections = 0
    def reject_typed(call, reason):
        nonlocal typed_mutation_rejections
        try:
            call()
        except RuntimeError as exc:
            require(str(exc) == reason, 'fixture_typed_mutation_reason')
            typed_mutation_rejections += 1
        else:
            fail('fixture_typed_mutation_accepted')

    none_cache = dict(atom_cache)
    none_cache[1] = None
    reject_typed(
        lambda: extend_signature(identity, 1, none_cache, model, v12f),
        'endpoint_atom_missing')
    bad_parent = list(identity)
    bad_parent[0] = ('E4', bad_parent[0][1])
    reject_typed(
        lambda: extend_signature(tuple(bad_parent), 1, atom_cache, model, v12f),
        'endpoint_parent_signature_type')
    bad_atom_cache = dict(atom_cache)
    bad_atom = list(bad_atom_cache[1])
    bad_atom[6] = ('E3', bad_atom[6][1])
    bad_atom_cache[1] = tuple(bad_atom)
    reject_typed(
        lambda: extend_signature(identity, 1, bad_atom_cache, model, v12f),
        'endpoint_atom_signature_type')

    order_words = ((1, 2), (1, -2), (-1, 2), (-1, -2))
    words = [(), (1,), (-1,), *order_words]
    recurrence_cases = 0
    noncommuting_cases = 0
    signed_noncommuting_cases = 0
    for word in words:
        before = len(model.calls)
        recurrence = fold_signature(word, atom_cache, model, v12f, identity)
        require(len(model.calls) == before,
                'fixture_recurrence_re_evaluation')
        direct = model.coordinates(word)
        require(tuple(direct[tag] for tag in TEN) ==
                tuple(raw for _kind, raw in recurrence),
                'fixture_recurrence_direct_equality')
        if word in order_words:
            parent = fold_signature(
                word[:-1], atom_cache, model, v12f, identity)
            reverse_cache = dict(atom_cache)
            reverse_cache[word[-1]] = parent
            reversed_order = extend_signature(
                atom_cache[word[-1]], word[-1], reverse_cache, model, v12f)
            require(reversed_order[0] != recurrence[0],
                    'fixture_E3_noncommuting_order')
            require(reversed_order[6] != recurrence[6],
                    'fixture_E4_noncommuting_order')
            noncommuting_cases += 1
            if word[-1] < 0:
                signed_noncommuting_cases += 1
        recurrence_cases += 1
    require(model.calls[1:5] == [(-2,), (-1,), (1,), (2,)],
            'fixture_atom_cache_order')
    return {'atom_evaluations': 4, 'recurrence_cases': recurrence_cases,
            'identity_cases': 2,
            'typed_mutation_rejections': typed_mutation_rejections,
            'noncommuting_cases': noncommuting_cases,
            'signed_noncommuting_cases': signed_noncommuting_cases}


def _tiny_bucket_fixtures():
    full = tuple([('E3', b'a')] * 6 + [('E4', b'b')] * 5)
    mutated = list(full); mutated[6] = ('E4', b'c'); mutated = tuple(mutated)
    signatures = {(1,): full, (2,): full}
    equal_terms = [[1, [1], 1], [1, [2], 1]]
    grouped = canonical_signature_buckets(equal_terms, signatures)
    require(len(grouped) == 1 and next(iter(grouped.values())) == (2, (2,)),
            'fixture_bucket_representative')
    calls = []; progress = []
    def tiny_direct(path, relator):
        require(tuple(path) == (), 'fixture_base_path_not_empty')
        calls.append((tuple(path), tuple(relator)))
        return {b'R\x01\x01\x00\x01x': 1}, {
            'eleven_occurrence_replay': True,
            'direct_all_seven_replay': True}
    base = replay_reached_seed_base(
        [1, 2], [[1], [2]], tiny_direct,
        lambda done, total: progress.append((done, total)), lambda: None)
    require(calls == [((), (1,)), ((), (2,))] and
            progress == [(1, 2), (2, 2)] and
            base['completed_count'] == 2 and base['eof'] is True,
            'fixture_reached_seed_base_schedule')

    split = canonical_signature_buckets(
        equal_terms, {(1,): full, (2,): mutated})
    require(len(split) == 2, 'fixture_E4_slot_split')
    rejected = 0
    def reject(call):
        nonlocal rejected
        before = len(calls)
        try:
            ...
            require(len(calls) == before, 'fixture_reject_before_direct')
            rejected += 1
        else:
            fail('fixture_bucket_mutation_accepted')

    bad_digest = dict(base)
    bad_digest['base_rows'] = [dict(row) for row in base['base_rows']]
    bad_digest['base_rows'][0]['canonical_sparse_row_sha256'] = '0' * 64
    reject(lambda: validate_base_receipt(bad_digest, [1, 2]))
    bad_count = dict(base)
    bad_count['completed_count'] = 1
    reject(lambda: validate_base_receipt(bad_count, [1, 2]))
    bad_eof = dict(base)
    bad_eof['eof'] = False
    reject(lambda: validate_base_receipt(bad_eof, [1, 2]))

    wrong_representative = dict(grouped)
    only_key, (coefficient, _path) = next(iter(wrong_representative.items()))
    wrong_representative[only_key] = (coefficient, (1,))
    reject(lambda: validate_signature_buckets(
        wrong_representative, equal_terms, signatures))
    first_six = {(1, full[:6]): (2, (2,))}
    reject(lambda: validate_signature_buckets(first_six, equal_terms, signatures))
    reject(lambda: validate_direct_completion(0, 1))
    return {'bucket_mutation_rejections': rejected,
            'base_canary_direct_calls': len(calls),
            'base_canary_completion': base['completed_count'],
            'E4_split_buckets': len(split)}


def selftest():
    fixture = _tiny_joint_fixtures()
    signature_fixture = _tiny_signature_fixtures()
    bucket_fixture = _tiny_bucket_fixtures()
    ...
    print(json.dumps({'fixture':'PASS','actor_multiplication':'PASS','inverse_action':'PASS','coefficient_2':'PASS','occurrence_components':11,'endpoint_ceiling':484,'empty_endpoint_generic_evaluations':0,'actor_atom_generic_evaluations':signature_fixture['atom_evaluations'],'full_prefix_generic_comparisons':0,'direct_schedule':'S','direct_replay_label':DIRECT_REPLAY_LABEL,'bucket_mutation_rejections':bucket_fixture['bucket_mutation_rejections'],'base_canary_direct_calls':bucket_fixture['base_canary_direct_calls'],'base_canary_completion':bucket_fixture['base_canary_completion'],'E4_split_buckets':bucket_fixture['E4_split_buckets'],'leaf_live_mutations':rejected,'first_six_shift_mutations':shift_mutations,'endpoint_installer':'PASS','endpoint_fine_source_order':59049,'wrong_fine_order_rejected':wrong_fine_order,'build_heavy_trap_called':False,'generic_builders_called':False,'joint_equality_cases':fixture['joint_equality_cases'],'joint_mutation_rejections':fixture['joint_mutation_rejections'],'identity_fixture_cases':signature_fixture['identity_cases'],'recurrence_cases':signature_fixture['recurrence_cases'],'typed_signature_mutation_rejections':signature_fixture['typed_mutation_rejections'],'noncommuting_recurrence_cases':signature_fixture['noncommuting_cases'],'signed_noncommuting_recurrence_cases':signature_fixture['signed_noncommuting_cases'],'g760_length':760,'context_order':31,'q0_marked_rows':2,'runtime_profile_mutations':profile_mutations,'forbidden_runtime_rejections':forbidden_runtime_rejections,'seed_cache_bytes':10644832,'rho2_bytes':PACKED},sort_keys=True))
def main():
    global started
