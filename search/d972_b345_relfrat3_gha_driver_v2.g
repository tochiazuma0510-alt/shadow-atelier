#############################################################################
## d972_b345_relfrat3_gha_driver_v2.g
##
...
D972RFProducerPath := "search/d972_b345_relfrat3_v2.py";;
D972RFProducerSHA :=
  "fad364043926dbdc03e56accf089f454d625e0b315c98a7647bc891677313cc8";;
D972RFCheckerPath := "search/check_d972_b345_relfrat3_v2.py";;
D972RFCheckerSHA :=
  "3c8967bea6946b42cef08cd097eab4e9071aae203ee27ac38038c4d5adb83f07";;
D972RFV1ProducerPath := "search/d972_b345_relfrat3_v1.py";;
D972RFV1ProducerSHA :=
  "4b73fbfe19bb33a9decdec5fda437f58f61a3ecb1989090bd08151f60ce6609e";;
D972RFV1CheckerPath := "search/check_d972_b345_relfrat3_v1.py";;
D972RFV1CheckerSHA :=
  "3d86240237229b250943c4795c24c32ac75af9229534c73d16bd838f6d6d0101";;
D972RFV1DriverPath := "search/d972_b345_relfrat3_gha_driver_v1.g";;
D972RFV1DriverSHA :=
...
D972RFArtifact := "ci/out/d972_b345_relfrat3_v2.json";;
D972RFProducerLog := "ci/out/d972_b345_relfrat3_v2_producer.log";;
D972RFProducerOk := "ci/out/d972_b345_relfrat3_v2_producer.ok";;
D972RFCheckerLog := "ci/out/d972_b345_relfrat3_v2_checker.log";;
D972RFCheckerOk := "ci/out/d972_b345_relfrat3_v2_checker.ok";;
D972RFSelfLog := "ci/out/d972_b345_relfrat3_v2_selftest.log";;
D972RFSelfOk := "ci/out/d972_b345_relfrat3_v2_selftest.ok";;
...
D972RFRequireSHA := function(path,expected)
  local raw,got;
  raw:=StringFile(path);;
  if raw=fail then Error("157dn v2 driver: missing source ",path); fi;
  got:=HexSHA256(raw);;
  if got<>expected then
    Error("157dn v2 driver: source SHA drift ",path," got=",got);
  fi;
  return true;
end;;
...
D972RFCount := function(text,needle)
  ...
  if not IsString(text) or not IsString(needle) or Length(needle)=0 then
    Error("157dn v2 driver: occurrence input");
  fi;
...
D972RFReadRequired := function(path,label)
  local raw;
  raw:=StringFile(path);;
  if raw=fail or Length(raw)=0 then Error("157dn v2 driver: missing ",label); fi;
  return raw;
end;;
...
D972RFRequireSHA(D972RFV1ProducerPath,D972RFV1ProducerSHA);;
D972RFRequireSHA(D972RFV1CheckerPath,D972RFV1CheckerSHA);;
D972RFRequireSHA(D972RFV1DriverPath,D972RFV1DriverSHA);;
...
if D972RFSelf=D972RFFull then
  Error("157dn v2 driver: select exactly one of SELFTEST and RUN");
fi;

if D972RFSelf then
  Exec("mkdir -p 'ci/out' && rm -f 'ci/out/d972_b345_relfrat3_v2_selftest.log' 'ci/out/d972_b345_relfrat3_v2_selftest.ok' && python3 -B search/d972_b345_relfrat3_v2.py --self-test > 'ci/out/d972_b345_relfrat3_v2_selftest.log' 2>&1 && python3 -B search/check_d972_b345_relfrat3_v2.py --self-test >> 'ci/out/d972_b345_relfrat3_v2_selftest.log' 2>&1 && printf '%s' 'D972_B345_RELFRAT3_SELFTEST_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_v2_selftest.ok'");;
  D972RFSelfRaw:=D972RFReadRequired(D972RFSelfLog,"selftest log");;
  if StringFile(D972RFSelfOk)<>D972RFSelfSentinel then
    Error("157dn v2 driver: selftests did not exit zero");
  fi;
  if D972RFCount(D972RFSelfRaw,
       "D972_B345_RELFRAT3_V2_PRODUCER_SELFTEST_PASS")<>1 or
     D972RFCount(D972RFSelfRaw,
       "D972_B345_RELFRAT3_V2_CHECKER_SELFTEST_PASS")<>1 then
    Error("157dn v2 driver: selftest marker count");
  fi;
  Print(D972RFSelfRaw,"\n");;
  Print("B345_RELFRAT3_V2_GHA_DRIVER_PASS mode=selftest\n");;
else
  if not IsBound(D972_B345_RELFRAT3_OUTPUT) or
     D972_B345_RELFRAT3_OUTPUT<>D972RFArtifact then
    Error("157dn v2 driver: full output must be fixed ci/out artifact");
  fi;
  ...
  Exec("mkdir -p 'ci/out' && rm -f 'ci/out/d972_b345_q3_chief_v1.json' 'ci/out/d972_b345_q3_checker_full.log' 'ci/out/d972_b345_q3_checker_full.ok' 'ci/out/d972_b345_relfrat3_v2.json' 'ci/out/d972_b345_relfrat3_v2_producer.log' 'ci/out/d972_b345_relfrat3_v2_producer.ok' 'ci/out/d972_b345_relfrat3_v2_checker.log' 'ci/out/d972_b345_relfrat3_v2_checker.ok'");;
  ...
  if HexSHA256(D972RFQ3Raw)<>D972RFQ3ArtifactSHA then
    Error("157dn v2 driver: regenerated q3 artifact SHA drift");
  fi;
  ...
  if StringFile("ci/out/d972_b345_q3_checker_full.ok")<>
    ...
    Error("157dn v2 driver: q3 checker gate");
  fi;
  Exec("python3 -B search/d972_b345_relfrat3_v2.py ci/out/d972_b345_q3_chief_v1.json ci/out/d972_b345_relfrat3_v2.json > 'ci/out/d972_b345_relfrat3_v2_producer.log' 2>&1 && printf '%s' 'D972_B345_RELFRAT3_PRODUCER_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_v2_producer.ok'");;
  ...
  if StringFile(D972RFProducerOk)<>D972RFProducerSentinel then
    Error("157dn v2 driver: producer did not exit zero");
  fi;
  ...
  if D972RFTerminalCount<>1 then
    Error("157dn v2 driver: producer terminal marker count");
  fi;
  D972RFArtifactRaw:=D972RFReadRequired(D972RFArtifact,"relative artifact");;
  Exec("python3 -B search/check_d972_b345_relfrat3_v2.py ci/out/d972_b345_q3_chief_v1.json ci/out/d972_b345_relfrat3_v2.json > 'ci/out/d972_b345_relfrat3_v2_checker.log' 2>&1 && printf '%s' 'D972_B345_RELFRAT3_CHECKER_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_v2_checker.ok'");;
  ...
  if StringFile(D972RFCheckerOk)<>D972RFCheckerSentinel or
    ...
    Error("157dn v2 driver: checker gate");
  fi;
  Print(D972RFProducerRaw,"\n",D972RFCheckerRaw,"\n");;
  Print("B345_RELFRAT3_V2_GHA_DRIVER_PASS mode=full artifact_sha256=",
