#############################################################################
## Task342 R07 history-free positive fast resume v7 production driver.
## ASCII only.  The driver has no recursive self pin.
#############################################################################
D342Producer:="search/d972_r07_history_free_positive_fast_resume_v7.py";;
D342Checker:="crosscheck/check_d972_r07_history_free_positive_fast_resume_v7.py";;
D342Fixture:="search/certs/d972_r07_history_free_positive_fast_resume_selftest_v7_20260829.json";;
...
D342Raw:="ci/resume/d972_r07_history_free_positive_fast_resume_v7.raw.json";;
D342Base:="ci/out/d972_r07_history_free_positive_fast_resume_v7_production";;
...
D342Common:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V7_COMMON_WORD";;
D342ProducerPrefix:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V7_PRODUCER_TERMINAL";;
D342CheckerPrefix:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V7_CHECKER_TERMINAL";;
D342Sentinel:="R07_HISTORY_FREE_POSITIVE_FAST_RESUME_V7_DRIVER_PASS";;

D342Pins:=[
 [D342Producer,122132,"cddef2abe41aaad9b2c0dd81d6fcc67b603bbf70359888fdfd34bf1d9f069ef9"],
 [D342Checker,96334,"8afcf5653eefedfb1639ea9a0d30e6d7ba5fc3f337406295b4522c31ae61423c"],
 [D342Fixture,2783,"5f6628d8413ebcc78a87196fa999b4f73122d4bca2c22efc0a9f637e59f483c6"],
...
D342Read:=function(path)
  local raw;
  raw:=StringFile(path);;
  if raw=fail or Length(raw)=0 then Error("task342 missing input ",path); fi;
  return raw;
end;;
...
if D342Contents=fail then Error("task342 cannot inspect ci/out"); fi;
for D342Name in D342Contents do
  if PositionSublist(D342Name,
      "d972_r07_history_free_positive_fast_resume_v7_")=1 then
    Error("task342 stale v7 owned output ",D342Name);
  fi;
od;
...
if Length(D342Outputs)<>Length(Set(D342Outputs)) then
  Error("task342 duplicate output path");
fi;
for D342Path in D342Outputs do
  if IsExistingFile(D342Path) then Error("task342 stale output ",D342Path); fi;
od;

D342Stream:=OutputTextFile(D342Shell,false);;
if D342Stream=fail then Error("task342 shell open"); fi;
...
PrintTo(D342Stream,"            raise SystemExit('task342 frozen sole ZIP member gate')\n");
PrintTo(D342Stream,"        digest = hashlib.sha256(); size = 0\n");
PrintTo(D342Stream,"        with archive.open(infos[0], 'r') as incoming, open(output, 'xb') as outgoing:\n");
...
PrintTo(D342Stream,"            raise SystemExit('task342 frozen raw member gate')\n");
PrintTo(D342Stream,"except BaseException:\n");
PrintTo(D342Stream,"    try: os.unlink(output)\n");
...
PrintTo(D342Stream,"  UNKNOWN_RESOURCE:phase=*) [[ \"$terminal\" =~ ^UNKNOWN_RESOURCE:phase=[A-Za-z0-9_]+:cap=[A-Za-z0-9_]+:value=[0-9]+([.][0-9]+)?:limit=[0-9]+([.][0-9]+)?$ ]] && test -s ",D342Checkpoint," ;;\n");
...
if D342Observed<>D342Sentinel then Error("task342 sentinel mismatch"); fi;
Print(D342Sentinel,"\n");
