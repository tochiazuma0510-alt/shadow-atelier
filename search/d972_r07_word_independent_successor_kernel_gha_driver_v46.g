...
D514S:=Concatenation(D514S,
 ...
 " test -s ",D514Output,"; test \"$(grep -Fxc 'R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_TERMINAL R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PASS' ",D514PLog," || true)\" = 1; python3 -c 'import json; x=json.load(open(\"",D514Output,"\")); assert x.get(\"complete\") is True; assert all(x.get(k) is False for k in (\"A0\",\"COMMON\",\"NONMEMBER\") if k in x); assert x.get(\"forbidden_downstream\")=={\"lift\":False,\"fake\":False,\"Ihara\":False,\"base_pairs\":False,\"ambient_E3_E4_enumeration\":False}'\n",
