#!/usr/bin/env python3
"""R07 A4/v6 producer: frozen core plus fail-closed physical-owner gate.

The v6 physical-owner layer is intentionally not claimed complete until all
48 live/physical routes have authenticated expected first-rejection records.
The ordinary v5 core is retained only as the versioned audit baseline.
...
SELFTEST_SCHEMA = SCHEMA + "/selftest-fixture/v6"
...
class Runtime:
    ...
    def __init__(self, authority: AuthorityAdapter, meter: Meter):
        self.meter = meter; self.old = load_pinned_module(E4_SOURCE, "r07_v6_frozen_e4", meter)
...
def write_atomic(path: Path, raw: bytes) -> None:
    ...
    fd, temporary = tempfile.mkstemp(prefix=".v6-", dir=str(path.parent))
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.selftest:
            ...
            print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_SELFTEST_PASS", flush=True); return 0
        ...
        if output: write_sealed(output, result, meter)
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_TERMINAL " + PASS, flush=True); return 0
        ...
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_TERMINAL " + UNKNOWN_RESOURCE, flush=True); return 0
        ...
        if output: write_sealed(output, terminal_certificate(UNKNOWN_INPUT, str(exc), meter, checkpoint_arg,
                                                             authority_identity), meter, terminal_transport=True)
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_TERMINAL " + UNKNOWN_INPUT, flush=True); return 0
    except Exception as exc:
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_STOP " +
              type(exc).__name__ + ":" + str(exc), file=sys.stderr, flush=True)
        print("R07_WORD_INDEPENDENT_SUCCESSOR_KERNEL_V6_PRODUCER_STOP " + type(exc).__name__, flush=True); return 2
