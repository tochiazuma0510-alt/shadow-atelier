#!/usr/bin/env python3
# bundle-selfaudit-v3.py -- N_infty stage2 freeze bundle self-audit (13 checks)
...
# v3 (裁定 85 / 内部前哨ゲート第 2 巡):
#   - check #12 cross-doc clause-ID 同期(contract の operative 本文が参照する
#     manifest 系 ID が manifest の clause+check registry に実在するか・両方向)
#   - check #13 authblock label<->digest 対応(live_authority_refs[] の artifact_id の
#     版と digest が §0 header / §6 pin と一致するか)
#   - [sweep-def] block 型に対応し、historical_upper_bound = current-1 を自動照合。
#   - check registry に R-<digit>(closure 生成規則)を追加。
# usage: python search/bundle-selfaudit-v2.py   (exit 0 = ALL PASS)
import os, re, sys, hashlib
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
D = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
F = {"spec":     "docs/week4-NInfty_stage2_spec_v12.md",
     "contract": "docs/mb_ninfty_verifier_contract_v7.md",
     "manifest": "docs/mb_dependency_manifest_v7.md"}
CUR = {"spec": "v12", "contract": "v7", "manifest": "v7"}
...
for name, txt in T.items():
    ...
    for i, l in enumerate(txt.split("\n"), 1):
        ...
        for series, pat in PAT:
            # 【chg vN …】 masking の設計限界と、その封じ込め(裁定 85 項目 10):
            #   行頭 allowlist 化は table セル内の正当な changelog を壊すので採らない。
            #   代わりに masking を二条件で束縛する:
            #     (a) 名指す版が historical_upper_bound 以下(= 現行版・将来版は隠せない)
            #     (b) span 内に artifact slug(mb/...)を含まない(= 完全な artifact ID は隠せない)
            #   条件を満たさない span は mask しない → その中の token は sweep される。
            def _mask(mm):
                span = mm.group(0)
...
CHK_RE = r"(?<![A-Za-z0-9])(D-[0-9]′?|R-[0-9]|U-[0-9]|P-[0-9]\.[0-9]|W-[0-9]|W-2′|S[123]|C1)(?![A-Za-z0-9])"
ok3 = True; det3 = []
for name in ("contract", "manifest"):
    txt = T[name]
...
report(9, "lifecycle tense", ok9 and gate, " ; ".join(rows9) + " ; spec gate(%s 束)=%s" % (CUR["spec"], gate))
...
# ---------- 12. cross-doc clause-ID 同期 (第 2 巡 重大 1) ----------
MREG = set(m.group(1) for m in re.finditer(REG_RE, T["manifest"], re.M))
MCHK = set(m.group(1) for m in re.finditer(CHK_RE, T["manifest"]))
MALL = MREG | MCHK
CREG = set(m.group(1) for m in re.finditer(REG_RE, T["contract"], re.M))
CCHK = set(m.group(1) for m in re.finditer(CHK_RE, T["contract"]))
CALL = CREG | CCHK
XREF = r"(?<![A-Za-z0-9])((?:D-R|D-|I-0c|I-0|I-|SB-|E-|R-|H-|M-|FA-|BA-|Y-|U-|T-|CR-|LA-)[A-Za-z0-9\-\.]*[′″‴⁗]?)(?![A-Za-z0-9])"
cls_c = classify(T["contract"])
sec7 = []
inside = False
for i, l in enumerate(T["contract"].split(chr(10)), 1):
    if l.startswith("## 7."): inside = True
    elif l.startswith("## ") and inside: inside = False
    if inside and cls_c.get(i) in ("table", "prose", "heading"):
        sec7.append(l)
refs = set()
for l in sec7:
    for m in re.finditer(XREF, l):
        ...
        refs.add(t)
own = {x for x in refs if x in CALL and x not in MALL}
missing = sorted(x for x in refs if x not in MALL and x not in own)
ok12 = not missing
report(12, "cross-doc clause-ID 同期 (contract §7 -> manifest registry)", ok12,
       "§7 が参照する manifest 系 ID=%d / manifest registry=%d(clause %d + check %d) / 実在しない ID=%s"
       % (len(refs), len(MALL), len(MREG), len(MCHK), missing if missing else "なし"))

# ---------- 13. authblock label<->digest 対応 (第 2 巡 重大 2) ----------
SLUG = {"mb/ninfty-stage2-predicate": "spec", "mb/ninfty-verifier-contract": "contract",
        "mb/dependency-manifest": "manifest"}
ok13 = True; rows13 = []
for name in F:
    txt = T[name]
    blk = re.search(r"live_authority_refs\[\] = \[(.*?)\n\]", txt, re.S)
    if not blk:
        ok13 = False; rows13.append("%s: live_authority_refs[] なし" % name); continue
    for e in re.finditer(r'artifact_id: "([^"]+)/(v\d+)",\s*\n\s*digest_or_receipt_slot: "([^"]+)"', blk.group(1)):
        slug, ver, dg = e.group(1), e.group(2), e.group(3)
        tgt = SLUG.get(slug)
        if tgt is None: ok13 = False; rows13.append("%s: 未知 slug %s" % (name, slug)); continue
        vok = (ver == CUR[tgt])
        ORD = {"manifest": 0, "contract": 1, "spec": 2}
        if dg.startswith("receipt:"):
            # hash 順序 manifest->contract->spec: 自分自身と下流(後続)は pin できないので
            # receipt slot で参照するのが正しい
            dok = (tgt == name) or (ORD[tgt] > ORD[name])
        else:
            dok = (dg == DG[tgt])
        if not (vok and dok):
            ok13 = False
            rows13.append("%s -> %s: label=%s(現行 %s) digest_ok=%s" % (name, tgt, ver, CUR[tgt], dok))
    # sweep-def の historical_upper_bound = current - 1
    sd = re.search(r'current_version\s*=\s*"v(\d+)".*?historical_upper_bound\s*=\s*"v(\d+)"', txt, re.S)
    if sd:
        cu, ub = int(sd.group(1)), int(sd.group(2))
        if not (ub == cu - 1 and "v%d" % cu == CUR[name]):
            ok13 = False; rows13.append("%s: sweep-def current=v%d upper=v%d (期待 %s / v%d)" % (name, cu, ub, CUR[name], int(CUR[name][1:]) - 1))
    else:
        ok13 = False; rows13.append("%s: sweep_definition block なし" % name)
report(13, "authblock label<->digest 対応 + sweep-def 上限", ok13,
       "NG=%d %s" % (len(rows13), rows13 if rows13 else ""))
