#!/usr/bin/env python3
"""Task1078 draft: independent fixed-lambda, two-layer parent1706 checker.
...
PARENT_ROLES = (*V5_PARENT_ROLES, "batch-parent-v5")
PRODUCER_FILE = "search/d972_r07_fixed_lambda_cycle_batch_v6.py"
CHECKER_FILE = "search/check_d972_r07_fixed_lambda_cycle_batch_v6.py"
CHECKER_WORKFLOW = ".github/workflows/d972-r07-fixed-lambda-cycle-batch-v6.yml"
...
def emit_parent_timing(stage: str, role: str, ordinal: int, started: float,
    ...
    event = {"schema": "d972.r07.parent-timing.v1", "side": "C", "stage": stage,
        ...
        "files": files, "file_bytes": None, "rows": rows, "records": records}
    try:
...
def check_acceptance_header(value: Any) -> None:
    require(type(value) is dict and set(value) == {"schema", "parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "code", "runtime", "registration"} and
            value["schema"] == SCHEMA + ".acceptance", "acceptance_exact_nine_plain_keys")
...
class AcceptedInputs:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        check_third_parent_roles(value["parents"])
        require(type(THIRD_BATCH_INVENTORY_REGISTRATION) is dict and type(CURRENT_PRODUCER_REGISTRATION) is dict,
                "v6_formal_inventory_and_final_source_binding_pending")
        ...
        for parent_ordinal, (item, portable_item) in enumerate(zip(value["parents"], portable["parents"])):
            ...
            emit_parent_timing("parent-inventory", role, parent_ordinal, parent_started, files=len(self.trees[role].files))
        ...
        same_json(code["producer"], CURRENT_PRODUCER_REGISTRATION, "v6_registered_final_P6_opaque_full_pin")
...
def authenticate_saved_next_batch_rows(inputs: AcceptedInputs, anchor: ThinAnchor,
    ...
    for ordinal in range(NEXT_BATCH_ROW_COUNT):
        ...
        for phase in CANDIDATE_PHASES:
            ...
            phases[phase] = predecessor
        reduction_prefix = prefix + "/reduction"
        reduction = next_batch_json(tree, reduction_prefix + "/reduction.json", "reduction")
        ...
        ordered = [{"row_id": index, "source": sources[index], "lead": pivot["lead"], "coefficient": coefficients[index]}
                   for index, pivot in enumerate(pivots)]
        same_json(reduction["ordered_reductions"], ordered, "next_batch_all_parent_and_prior_batch_row_sources")
        factors = [{"row_id": item["row_id"], "source": item["source"], "coefficient": item["coefficient"],
                    "exponent": -E.signed(item["coefficient"])} for item in ordered]
...
def promote_next_batch_parent(inputs: AcceptedInputs, anchor: ThinAnchor, legacy: dict[str, Any]) -> ThinAnchor:
    ...
    chain = authenticate_saved_next_batch_rows(inputs, anchor, legacy)
...
def authenticate_third_batch_parent_metadata(inputs: AcceptedInputs) -> dict[str, Any]:
    ...
    check_third_native_parent_projection(inputs.portable["parents"], layout["parents"])
...
def authenticate_saved_third_batch_rows(inputs: AcceptedInputs, anchor: ThinAnchor,
    ...
    for ordinal in range(THIRD_BATCH_ROW_COUNT):
        ...
        for phase in CANDIDATE_PHASES:
            ...
            phases[phase] = predecessor
        reduction_prefix = prefix + "/reduction"
        reduction = third_batch_json(tree, reduction_prefix + "/reduction.json", "reduction")
        ...
        ordered = [{"row_id": index, "source": sources[index], "lead": pivot["lead"], "coefficient": coefficients[index]}
                   for index, pivot in enumerate(pivots)]
        same_json(reduction["ordered_reductions"], ordered, "third_batch_all_parent_and_prior_batch_row_sources")
        factors = [{"row_id": item["row_id"], "source": item["source"], "coefficient": item["coefficient"],
                    "exponent": -E.signed(item["coefficient"])} for item in ordered]
...
def promote_third_batch_parent(inputs: AcceptedInputs, anchor: ThinAnchor, legacy: dict[str, Any]) -> ThinAnchor:
    ...
    chain = authenticate_saved_third_batch_rows(inputs, anchor, legacy)
...
def root_records(inputs: AcceptedInputs, anchor: ThinAnchor, old: dict[str, Any], bundle: C.FixedBundle) -> RootRecords:
    ...
    layout = document("parent-layout", {"portable_acceptance_sha256": inputs.portable_sha256,
        **{key: copy.deepcopy(inputs.portable[key]) for key in ("parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "code", "runtime", "registration")}})
    ...
    emit_parent_timing("direct-pairing", THIRD_BATCH_ROLE, 3, pairing_started, rows=pairing["rows"], records=len(anchor.parents))
    ...
    start = document("start", {"owner_sha256": sha(canonical(owner)), "source_sha256": sha(canonical(source)),
        ...
        "previous_parent_target_derivations": len(inputs.second_intermediate_parents),
...
def batch_observation(inputs: AcceptedInputs, records: RootRecords, files: CandidateFiles,
    ...
    if not admitted:
        sequence = -1
    batch = inputs.value["batch_anchor_v5"]
    parents = {item["role"]: item["artifact"] for item in inputs.portable["parents"]}
    accepted = inputs.third_batch_parent["records"]
    old = {"parent_role": "batch-parent-v4", "parent_artifact": copy.deepcopy(parents["batch-parent-v4"]),
    ...
    current = {"parent_role": "batch-parent-v5", "parent_artifact": copy.deepcopy(parents["batch-parent-v5"]),
    ...
    if admitted:
        ...
        require(pairing["rows"] == THIRD_BATCH_RANK and pairing["lambda_pivots"] == 0 and
    ...
    check_third_unobserved_oracle(current, sequence)
...
def check_actual(args: argparse.Namespace, report: dict[str, Any]) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        restore_started = time.monotonic()
        anchor = stack.enter_context(promote_third_batch_parent(inputs, second_intermediate, old))
        emit_parent_timing("state-restore", THIRD_BATCH_ROLE, 3, restore_started, rows=anchor.rank, records=len(anchor.parents))
...
def k128_registration_canary(root: Path) -> dict[str, Any]:
    ...
    acceptance = {"schema": SCHEMA + ".acceptance", "parents": [], "anchor": {}, "batch_anchor": {}, "next_batch_anchor": {}, "batch_anchor_v5": {}, "code": {},
    ...
    for side in ("producer", "checker"):
        ...
        bad[side]["file"] = bad[side]["file"].replace("_v6.py", "_v5.py")
...
def third_batch_parent_admission_canary(root: Path) -> dict[str, Any]:
    ...
    same_json(list(public_artifacts), list(PARENT_ROLES), "c6_parent1834_public_artifact_role_order")
...
def selftest(root: Path) -> dict[str, Any]:
    ...
    return document("selftest", {"status": "PASS", "tests": [first, second, third, fourth, fifth],
        "fixture_scope": "plus-synthetic-third-layer-metadata-with-exact-cycle_batch-prefix;" + "new-k128-registration-and-portable-metadata;synthetic-full-54433-rosters-and-physical-dependent-continuation;real-reduction-phase-and-candidate-publication-reader;synthetic-parent1578-header-projection-target-hash;synthetic-parent1706-two-layer-metadata;inherited-dependent-negative-stops-at-expected-file-size-hash-before-semantic-outcome-comparison;all-fixtures-retained;no-accepted-parent-or-earlier-five-E-phase-arithmetic",
        "production_interfaces_used": ["artifact_identity", "check_third_native_parent_projection", "check_third_row_namespace",
    ...
    try:
        if args.selftest:
            ...
            require(all(getattr(args, role.replace("-", "_") + "_root") is not None for role in PARENT_ROLES if not role.startswith("block-")) and
                    ...
                    "all_eighteen_roots_and_exact_acceptance")
