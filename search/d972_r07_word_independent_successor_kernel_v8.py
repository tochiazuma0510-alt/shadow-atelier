#!/usr/bin/env python3
"""Validation-only v8 owner for the frozen v6 A4 producer."""
...
def main() -> None:
    ...
    if len(raw) != SOURCE_BYTES or hashlib.sha256(raw).hexdigest() != SOURCE_SHA256:
        raise SystemExit("v8 producer: frozen v6 source drift")
    for old, new in PATCHES:
        if raw.count(old) != 1:
            raise SystemExit("v8 producer: audited correction site is not unique")
