#############################################################################
## Exact D972 shadow power/order spectrum v2.
##
## v2 keeps the v1 construction but binds every GAP source read by the
...
DPS2Write := function(s)
  local f;
  f := OutputTextFile(DPS2Output, false);
  SetPrintFormattingStatus(f, false);
  PrintTo(f, Concatenation(s, "\n"));
  CloseStream(f);
end;;

DPS2PermLine := function(p, n)
  return List([1..n], i -> i^p);
end;;

DPS2IntListText := function(xs)
  return Concatenation("[", D972Join(List(xs, String), ","), "]");
end;;

DPS2Compact := function(B, p)
  return D972DirectSumPerm(
    D972BlockRestrict(p, 0, 27), 27,
    D972BlockRestrict(p, B.component9_degree, 9), 9);
end;;

DPS2Key := function(m, p)
  return D972NFTargetKey(m,
    D972BlockRestrict(p, 0, 27),
    D972BlockRestrict(p, 27, 9));
end;;

DPS2DigestLines := function(lines)
  local z;
...
DPS2SelfTest := function()
  local p, q, id, prod;
  p := PermList([2,3,1]); q := PermList([1,3,2]);
  id := PermList([1,2,3]);
  prod := AbstractProd([p,q]);
  if prod <> q*p or AbstractProd([id,p]) <> p then
    Error("power-spectrum v2 orientation canary failed");
  fi;
  if 2*(2*1*2+1+2)+1 <> (2*1+1)*(2*2+1) mod 18 then
    Error("power-spectrum v2 cyclotomic canary failed");
  fi;
  Print("D972_POWER_SPECTRUM_V2_GAP_SELFTEST_PASS\n");
  DPS2Write("{\"schema\":\"d972-power-spectrum/v2\",\"mode\":\"selftest\",\"all_pass\":true,\"final_marker\":\"D972_POWER_SPECTRUM_V2_GAP_SELFTEST_PASS\"}");
end;;

DPS2Run := function()
  local B, S, raw, keys, targetDigest, keyMap, rows, row, i, j, m1, m2,
        u2, f1, f2, hom, sub, g, key, pos, products, orders, inverses,
        identity, power, n, hist, histRows, rowLines, productLines,
        rowDigest, productDigest, identityKey, inv, canary, jsonRows,
        jsonProducts, jsonOrders, jsonInverses, jsonHist, x, y, C,
        targetSet, squareMap, cubeMap, exponent;
  ...
  if Length(raw) <> 972 or Length(targetSet) <> 972 or
    ...
    Error("D972 power spectrum v2: frozen settled 972-row roof gate failed");
  fi;
  ...
  for row in raw do
    ...
    rows[pos] := rec(m:=row.m, key:=row.key,
      f:=DPS2Compact(B, row.f), settled:=row.settled);
  od;
  ...
  for i in [1..972] do
    ...
    for j in [1..972] do
      ...
      if pos = fail then Error("shadow product left frozen v2 972-set"); fi;
      products[i][j] := pos-1;
    od;
  od;
  ...
  rowLines := List(rows, r -> Concatenation(String(r.m), "|", r.key, "|",
    DPS2IntListText(DPS2PermLine(r.f,36))));
  productLines := List(products, r -> D972Join(List(r,String), ","));
  rowDigest := DPS2DigestLines(rowLines);
  productDigest := DPS2DigestLines(productLines);
  identityKey := rows[identity].key;
  jsonRows := D972Join(List([1..972], i -> Concatenation(
    "{\"row_index\":",String(i-1),",\"m\":",String(rows[i].m),
    ",\"target_key\":",DPS2JsonString(rows[i].key),
    ",\"f\":",String(DPS2PermLine(rows[i].f,36)),
  ...
  DPS2Write(Concatenation(
    "{\"schema\":\"d972-power-spectrum/v2\",\"final_marker\":\"D972_POWER_SPECTRUM_V2_GAP_FINAL\"",
    ",\"status\":\"POWER_SPECTRUM_V2_COMPLETE\",\"row_count\":972",
    ",\"target_key_digest\":",DPS2JsonString(targetDigest),
    ",\"target_key_order\":",DPS2JsonString("lexicographic frozen set"),
    ...
    ",\"runtime_source_sha256\":",DPS2RuntimeManifest,
    ...
    ",\"identity_index\":",String(identity-1),",\"identity_key\":",DPS2JsonString(identityKey),
    ...
    ",\"row_digest_sha256\":",DPS2JsonString(rowDigest),
    ",\"product_table_sha256\":",DPS2JsonString(productDigest),
  ...
  Print("D972_POWER_SPECTRUM_V2_GAP_FINAL status=POWER_SPECTRUM_V2_COMPLETE output=",DPS2Output,"\n");
end;;

if DPS2Mode = "selftest" then DPS2SelfTest(); else DPS2Run(); fi;;
