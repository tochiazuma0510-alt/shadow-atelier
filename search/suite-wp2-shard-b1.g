# suite-wp2-shard-b1.g -- WP2a GAP explorer, shard B1 (n=13,14)
#
# 実行: .\gap.ps1 search\suite-wp2-shard-b1.g
#
# 背景: search/suite-wp2-shard-b.g (n=13..16+N5 まとめ) が実測 897.2 秒で 10 分 cap を
# 超過したため(観察: n=13 は shadows=312, compTable 最大約 97344 組で JSON 文字列結合
# コストが支配的と推定)、決定的分割(shard-b1/b2)へさらに分割(司令塔指示 2026-07-19)。
# しかし shard-b1(n=13,14)単独でも 612.0 秒で cap をなお超過(実測: n=13 の JSON 直列化
# 単独で 437.5 秒)。n=13 は単一対象のためこれ以上「対象集合の分割」では解消不能と判断し
# 報告 → **司令塔裁定(2026-07-19): 選択肢(a)採用 — 「出力形式を1バイトも変えない」の
# 保護対象は証明書のバイト列そのもの(ハッシュ照合が判定器)であり、内部アルゴリズムの
# 効率化は許可される**。よって本ファイルのみ JoinC を O(合計文字数) の線形実装に置換した
# (Concatenation の逐次呼び出し = O(list長^2) だったものを、部品リストへ Add してから
# 単一の Concatenation(list) 呼び出し1回にまとめる実装に変更 -- GAP の Concatenation は
# 単一のリスト引数を「リストのリスト」として1回で連結するため線形時間。生成される文字列は
# 数学的に同一であり、実際に K13/K14 の SHA-256 が変更前と完全一致することで確認する)。
# shard-a/shard-b2/q1836 は cap 内のため今回は無変更(JoinC の統一は将来の別便)。
#
# 分割は対象集合の分割のみであり、各対象の計算内容・出力形式は無変更(元スクリプトの
# 該当コードをそのままコピー -- BuildCertJsonDihedral 内の "generated_by".script 文字列
# リテラルも含め無変更。証明書 JSON のバイト内容を保つため -- 変更すると
# provenance/cert-hashes-wp2.txt の記録済みハッシュと不一致になる)。
#
# 宇宙分担(事前登録宇宙 {3..16,18,36,N5} の厳密な分割):
#   shard A (suite-wp2-shard-a.g): n = 3..12
#   shard B1 (本ファイル): n = 13,14
#   shard B2 (suite-wp2-shard-b2.g): n = 15,16 + N5
#   q1836 (suite-wp2-explorer-q1836.g, 既存・無変更): n = 18, 36
#   5 本の対象集合の和 = {3..16,18,36,N5} = 事前登録宇宙と exact 一致。
#   注記: reduction branch suite 3 対((8,4),(12,4),(9,3))はすべて shard A 側で完結。
#   n=13,14 は reduction branch suite に含まれない(元スクリプトでも reduction は空配列)。
#
...
#
# 規約確認(元スクリプト search/suite-wp2-explorer.g からコピー. 無変更):
...
# 線形時間版(司令塔裁定 2026-07-19 選択肢(a)): 部品を list に集めてから Concatenation(list)
# を 1 回だけ呼ぶ(逐次 Concatenation(r,sep,strs[i]) の O(n^2) 再割り当てを回避)。
# 生成される文字列は旧実装と数学的に同一(区切り文字の挿入位置は不変)。
...
# ================= メインドライバ: shard B1 -- n=13,14 =================
universe := [13,14];;
results := [];;
Print("宇宙 shard B1 (事前登録宇宙 {3..16,18,36,N5} のうち n=13,14): ", universe, "\n");
...
Print("\n累計 elapsed ms (n=13,14 ProcessDihedral): ", Runtime()-startTime, "\n");

reductionsByQ := List([1..40], i -> []);;
Print("(n=13,14 は reduction branch suite に含まれない -- 空配列のまま)\n");
...
for n in universe do
  ...
  Print("wrote certificates/K", n, ".v1.json  (json_build_ms so far=", Runtime()-t0, ")\n");
od;
