...
SELF_PATH = Path("search/d972_r07_760_l3_target6_delta_resume_v4.py")
...
SCHEMA = "d972-r07-760-l3-target6-delta-resume/v4"
DELTA_SCHEMA = "d972-r07-760-l3-target6-delta-checkpoint-stream/v4"
J_CHECKPOINT_SCHEMA = (
    "d972-r07-760-l3-target6-completed-j-checkpoint/v4")
...
STATE_DOMAIN = "d972-r07-760-l3-target6/v4/cumulative-pivot-state/v1"
ORDER_DOMAIN = "d972-r07-760-l3-target6/v4/cumulative-insertion-order/v1"
PREFLIGHT_STATE = "R07_760_L3_TARGET6_DELTA_RESUME_V4_PREFLIGHT_READY"
DELTA_STATE = "R07_760_L3_TARGET6_DELTA_RESUME_V4_DELTA_READY"
J_CHECKPOINT_STATE = "R07_760_L3_TARGET6_DELTA_RESUME_V4_J_READY"
FINAL_MARKER = "R07_760_L3_TARGET6_DELTA_RESUME_V4_PRODUCER_PASS"
DELTA_MARKER = "R07_760_L3_TARGET6_DELTA_RESUME_V4_DELTA_CHECKPOINT"
J_MARKER = "R07_760_L3_TARGET6_DELTA_RESUME_V4_J_CHECKPOINT"

DEFAULT_PREFLIGHT = Path(
    "search/certs/d972_r07_760_l3_target6_delta_resume_"
    "preflight_v4_20260826.json")
DEFAULT_FULL = Path("ci/out/d972_r07_760_l3_target6_delta_resume_v4.json")
...
V4_PIN_SPECS: dict[str, tuple[Path, int, str]] = {
...
def v4_pin_inputs() -> dict[str, Any]:
    rows = {}
    for label, (path, size, digest) in V4_PIN_SPECS.items():
        full = ROOT / path
        if not full.is_file() or full.stat().st_size != size or \
                digest_file(full) != digest:
            raise InputStop("v4 pin drift: " + path.as_posix())
...
def producer_source_record() -> dict[str, Any]:
    full = ROOT / SELF_PATH
    require(full.is_file(), "v4 producer source missing")
...
def load_v3() -> Any:
    ...
    try:
        ...
        raise
    require(digest_file(ROOT / V3_PATH) == V4_PIN_SPECS["v3_producer"][2],
...
def fixed_bindings(summary: dict[str, Any], prior: dict[str, Any],
    ...
    result = {
        ...
        "v4_pin_manifest": meta["v4_pin_manifest"],
        "v4_pin_manifest_sha256": meta["v4_pin_manifest_sha256"],
...
def delta_filename(j: int, relator: int) -> str:
    ...
    return ("d972_r07_760_l3_target6_delta_resume_v4_"
            f"j{j:02d}_r{relator:02d}.delta.jsonl.gz")


def j_filename(j: int) -> str:
    require(j in FRESH_J_ORDER, "j checkpoint index")
    return f"d972_r07_760_l3_target6_delta_resume_v4_j{j:02d}.json"
...
def validate_public_j_row(v3: Any, row: dict[str, Any],
                          summary: dict[str, Any]) -> None:
    v3.validate_public_j_row(row, summary)
    append = row.get("v4_append_only_delta")
...
def build_j_checkpoint(
        v3: Any, summary: dict[str, Any], bindings: dict[str, Any],
        progression: Sequence[dict[str, Any]],
        prior_j_record: dict[str, Any] | None,
    ...
    validate_record_shape(terminal_delta_record, kind="delta", j=j,
                          relator=11)
    data = {
        "schema": J_CHECKPOINT_SCHEMA, "version": 4,
...
def write_j_checkpoint(path: Path, data: dict[str, Any], v3: Any,
                       summary: dict[str, Any],
                       bindings: dict[str, Any]) -> dict[str, Any]:
    validate_j_checkpoint(v3, data, summary, bindings)
...
def load_j_checkpoint_chain(
    ...
    previous_data, records = load_j_checkpoint_chain(
        previous_path, directory, v3, None, None, summary, None, bindings)
...
def validate_terminal_header(header: dict[str, Any], row: dict[str, Any],
    ...
    require(header["j"] == row["j"] and
            header["completed_j_progression"] == list(preceding) and
            header["completed_closure_receipts"] ==
                row["per_relator_closure_receipts"] and
            header["rank_after"] == row["rank_D2bar_alone"] and
    ...
    return {path.name for path in directory.iterdir()
            ...
            (path.name.endswith(".delta.jsonl.gz") or
             (path.name.startswith(
                 "d972_r07_760_l3_target6_delta_resume_v4_j") and
...
def toy_delta_replay_receipt(v3: Any, v2: Any, v1: Any) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d972-r07-v4-replay-") as tmp:
...
def delta_mutation_tests(v3: Any, v2: Any, v1: Any) -> int:
    labels = []
    old_default = globals()["DEFAULT_CHECKPOINT_DIR"]
    with tempfile.TemporaryDirectory(prefix="d972-r07-v4-delta-") as tmp:
        directory = Path(tmp).resolve()
        globals()["DEFAULT_CHECKPOINT_DIR"] = directory
        try:
            toy = toy_delta_chain(v3, v2, v1, directory)
...
def j2_delta_accounting(v3: Any, v1: Any,
    ...
    for ordinal, raw in enumerate(private["relator_pc"], 1):
        ...
        require(reference_receipt == fast_receipt and
                ...
                "j2 legacy/v4 append-only closure equality")
...
def storage_bounds(v3: Any, summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in v3.checkpoint_size_estimates(summary):
        rows.append({
            ...
            "v4_cumulative_delta_pivot_bytes_upper":
            ...
            "v3_to_v4_pivot_payload_upper_ratio": 11,
...
def validate_output(data: dict[str, Any]) -> None:
    verify_self_digest(data, "v4 output")
...
def self_test() -> None:
    ...
    print(
        "R07_760_L3_TARGET6_DELTA_RESUME_V4_PRODUCER_SELFTEST_PASS "
