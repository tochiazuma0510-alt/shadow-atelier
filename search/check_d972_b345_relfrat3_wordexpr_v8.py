#!/usr/bin/env python3
"""Independent checker for the full registered-4096 WordExpr/Fox v8 lane.
...
def checker_deadline(phase: str, force: bool = False) -> None:
    """Independent 7,200-second checker soft wall."""
    global CHECKER_CHECKS
    if CHECKER_STARTED is None:
        return
    CHECKER_CHECKS += 1
    if force or (CHECKER_CHECKS & 255) == 0:
        require(time.monotonic()-CHECKER_STARTED < 7200,
...
class ReplayPool:
    ...
    def rollback(self, checkpoint: int) -> int:
        ...
        for blob in self.values[checkpoint:]:
            require(self.ids.pop(blob, None) is not None,
                    "checker replay pool rollback binding")
        del self.values[checkpoint:]
        self.product.clear()
        self.inverses.clear()
        require(len(self.ids) == len(self.values),
                "checker replay pool rollback uniqueness")
        return removed
...
class CheckWordExprEvaluator:
    ...
    def evaluate_values(self) -> list[Element]:
        values: list[Element] = []
...
def validate_wordexpr_scan(block: dict[str, Any], dictionary: dict[str, Any],
    ...
    for index in range(1, n+1):
        ...
        if index == 1:
            require(candidate1_bridge == bridge_validator(
                        compiled, evaluator, e4, normalized),
                    "checker candidate1 mandatory flat bridge")
        ...
        if failure_name is not None:
            ...
            for ordinal, (name, kind, root) in enumerate(
                ...
                if missing is not None:
                    ...
                    break
            max_live = max(max_live, evaluator.live_peak)
            if missing is not None:
                ...
                record = {
                    ...
                    "wordexpr_max_expanded_letters":
                        accounting["max_expanded_letter_count"],
                    "wordexpr_live_gradient_peak": evaluator.live_peak,
                    "pool_suffix_removed": removed,
                }
            else:
    ...
    require(block["record_bindings"] == public and
            block["record_bindings_sha256"] == digest_obj(expected_records) and
            block["failure_distribution"] == distribution,
            "checker scan record bindings/distribution")
...
def validate_v8_pins(data: dict[str, Any], q3_path: Path, repo: Path,
    ...
    require(set(pins) == {"q3_producer", "q3_checker", "q3_driver",
                          ...
                          "semantic_reference_v7"}, "v8 pin key set")
...
def validate_v8_receipt(data: dict[str, Any], q3: dict[str, Any],
    ...
    require(source == {
                "producer_sha256": digest_file(repo/V8_PRODUCER),
                "checker_sha256": digest_file(repo/V8_CHECKER),
                "driver_sha256": digest_file(repo/V8_DRIVER)},
    ...
    pool, basis = replay_pivot_surgery(data, e4, targets)
    if (token == "B345_RELFRAT3_WORDEXPR_UNKNOWN_RESOURCE" and
...
def validate_fixed_receipt(data: dict[str, Any], q3: dict[str, Any],
    ...
    validate_cap_utilization(
        ...
        expression_edges=search["lazy_sections"]
            ["expressions"]["peak_edges"])
    replay_pivot_surgery(data, e4, targets)
...
def self_test_fixed() -> None:
    ...
    for label, mutate, expected_entry in mutation_paths:
        ...
        if expected_entry in {"preflight", "selected", "proof"}:
        ...
        if expected_entry in {"selected", "proof"}:
            require(trace["scan_validator_entries"] >
                    ...
                    f"v8 mutation selected core entry: {label}")
    ...
    print("D972_B345_RELFRAT3_WORDEXPR_V8_CHECKER_SELFTEST_PASS "
          ...
          "acceptance=33 diagnostics=17 mutations=11 terminal_fixtures=5")
...
def main() -> int:
    ...
    CHECKER_CHECKS = 0
    validate_v8_receipt(data, q3, q3_path, repo)
    checker_deadline("checker completion", force=True)
    print(f"B345_RELFRAT3_WORDEXPR_V8_CHECKER_PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(f"B345_RELFRAT3_WORDEXPR_V8_CHECKER_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
