#############################################################################
## d972_b345_relfrat3_pivot_surgery_gha_driver_v7.g
##
## Fail-closed pivot-surgery-v7 bootstrap.  The frozen q3 producer/checker
...
D972RF3ProducerPath := "search/d972_b345_relfrat3_pivot_surgery_v7.py";;
D972RF3ProducerSHA :=
  "a19c3353c5cfc6da8ad0b7d941ba94bde043c80e69e33c889c5710c897d7a757";;
D972RF3CheckerPath := "search/check_d972_b345_relfrat3_pivot_surgery_v7.py";;
D972RF3CheckerSHA :=
...
D972RF3Artifact := "ci/out/d972_b345_relfrat3_pivot_surgery_v7.json";;
D972RF3ArtifactSHAFile := "ci/out/d972_b345_relfrat3_pivot_surgery_v7.sha256";;
D972RF3Q3Child := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.g";;
D972RF3Q3ChildLog := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.log";;
D972RF3Q3ChildOk := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.ok";;
D972RF3ProducerLog := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_producer.log";;
D972RF3ProducerOk := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_producer.ok";;
D972RF3CheckerLog := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_checker.log";;
D972RF3CheckerOk := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_checker.ok";;
D972RF3SelfLog := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.log";;
D972RF3SelfOk := "ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.ok";;
D972RF3Q3ChildSentinel := "D972_B345_RELFRAT3_PIVOT_SURGERY_V7_Q3_CHILD_EXIT_ZERO";;
D972RF3ProducerSentinel := "D972_B345_RELFRAT3_PIVOT_SURGERY_V7_PRODUCER_EXIT_ZERO";;
D972RF3CheckerSentinel := "D972_B345_RELFRAT3_PIVOT_SURGERY_V7_CHECKER_EXIT_ZERO";;
D972RF3SelfSentinel := "D972_B345_RELFRAT3_PIVOT_SURGERY_V7_SELFTEST_EXIT_ZERO";;

D972RF3RequireSHA := function(path,expected)
  local raw,got;
  raw:=StringFile(path);;
  if raw=fail then Error("157dv v7 driver: missing source ",path); fi;
  got:=HexSHA256(raw);;
  if got<>expected then
    Error("157dv v7 driver: source SHA drift ",path," got=",got);
  fi;
  return true;
end;;
...
D972RF3Count := function(text,needle)
  ...
  if not IsString(text) or not IsString(needle) or Length(needle)=0 then
    Error("157dv v7 driver: occurrence input");
  fi;
...
D972RF3ReadRequired := function(path,label)
  local raw;
  raw:=StringFile(path);;
  if raw=fail or Length(raw)=0 then Error("157dv v7 driver: missing ",label); fi;
  return raw;
end;;
...
D972RF3CheckedWrite := function(path,text)
  ...
  if stream=fail then Error("157dv v7 driver: child script open"); fi;
  SetPrintFormattingStatus(stream,false);;
  PrintTo(stream,text);;
  CloseStream(stream);;
  got:=StringFile(path);;
  if got=fail or got<>text then Error("157dv v7 driver: child script readback"); fi;
  return true;
end;;
...
D972RF3Self:=IsBound(D972_B345_RELFRAT3_PIVOT_SURGERY_V7_SELFTEST) and
  D972_B345_RELFRAT3_PIVOT_SURGERY_V7_SELFTEST=true;;
D972RF3Full:=IsBound(D972_B345_RELFRAT3_PIVOT_SURGERY_V7_RUN) and
  D972_B345_RELFRAT3_PIVOT_SURGERY_V7_RUN=true;;
if D972RF3Self=D972RF3Full then
  Error("157dv v7 driver: select exactly one of SELFTEST and RUN");
fi;

if D972RF3Self then
  Exec("mkdir -p 'ci/out' && rm -f 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.log' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.ok' && python3 -u -B search/d972_b345_relfrat3_pivot_surgery_v7.py --self-test > 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.log' 2>&1 && python3 -u -B search/check_d972_b345_relfrat3_pivot_surgery_v7.py --self-test >> 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.log' 2>&1 && printf '%s' 'D972_B345_RELFRAT3_PIVOT_SURGERY_V7_SELFTEST_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_selftest.ok'");;
  D972RF3SelfRaw:=D972RF3ReadRequired(D972RF3SelfLog,"selftest log");;
  if StringFile(D972RF3SelfOk)<>D972RF3SelfSentinel then
    Error("157dv v7 driver: selftests did not exit zero");
  fi;
  if D972RF3Count(D972RF3SelfRaw,
       "D972_B345_RELFRAT3_PIVOT_SURGERY_V7_PRODUCER_SELFTEST_PASS")<>1 or
     D972RF3Count(D972RF3SelfRaw,
       "D972_B345_RELFRAT3_PIVOT_SURGERY_V7_CHECKER_SELFTEST_PASS")<>1 then
    Error("157dv v7 driver: selftest marker count");
  fi;
  Print(D972RF3SelfRaw,"\n");;
  Print("B345_RELFRAT3_PIVOT_SURGERY_V7_GHA_DRIVER_PASS mode=selftest\n");;
else
  if not IsBound(D972_B345_RELFRAT3_PIVOT_SURGERY_V7_OUTPUT) or
     D972_B345_RELFRAT3_PIVOT_SURGERY_V7_OUTPUT<>D972RF3Artifact then
    Error("157dv v7 driver: full output must be fixed ci/out artifact");
  fi;
  Exec("mkdir -p 'ci/out' && rm -f 'ci/out/d972_b345_q3_chief_v1.json' 'ci/out/d972_b345_q3_checker_full.log' 'ci/out/d972_b345_q3_checker_full.ok' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.g' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.log' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.ok' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7.json' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7.sha256' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_producer.log' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_producer.ok' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_checker.log' 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_checker.ok'");;
  ...
  Exec("bash -o pipefail -c 'gap -l \";/usr/share/gap;/usr/lib/gap;${GAP_P2_PACKAGE_ROOT:?}\" --quitonbreak -q -o 12g ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.g 2>&1 | tee ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.log' && printf '%s' 'D972_B345_RELFRAT3_PIVOT_SURGERY_V7_Q3_CHILD_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_q3_child.ok'");;
  ...
  if StringFile("ci/out/d972_b345_q3_checker_full.ok")<>
    ...
    Error("157dv v7 driver: q3 checker gate");
  fi;
  Exec("bash -o pipefail -c 'python3 -u -B search/d972_b345_relfrat3_pivot_surgery_v7.py ci/out/d972_b345_q3_chief_v1.json ci/out/d972_b345_relfrat3_pivot_surgery_v7.json 2>&1 | tee ci/out/d972_b345_relfrat3_pivot_surgery_v7_producer.log' && printf '%s' 'D972_B345_RELFRAT3_PIVOT_SURGERY_V7_PRODUCER_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_producer.ok'");;
  ...
  if StringFile(D972RF3ProducerOk)<>D972RF3ProducerSentinel then
    Error("157dv v7 driver: producer did not exit zero");
  fi;
  ...
  for D972RF3Token in [
      ...
      "B345_RELFRAT3_PIVOT_SURGERY_UNKNOWN_INPUT"] do
    D972RF3TerminalCount:=D972RF3TerminalCount+
      D972RF3Count(D972RF3ProducerRaw,D972RF3Token);;
  od;
  if D972RF3TerminalCount<>1 then
    Error("157dv v7 driver: producer terminal marker count");
  fi;
  Exec("bash -o pipefail -c 'python3 -u -B search/check_d972_b345_relfrat3_pivot_surgery_v7.py ci/out/d972_b345_q3_chief_v1.json ci/out/d972_b345_relfrat3_pivot_surgery_v7.json 2>&1 | tee ci/out/d972_b345_relfrat3_pivot_surgery_v7_checker.log' && printf '%s' 'D972_B345_RELFRAT3_PIVOT_SURGERY_V7_CHECKER_EXIT_ZERO' > 'ci/out/d972_b345_relfrat3_pivot_surgery_v7_checker.ok'");;
  ...
  if StringFile(D972RF3CheckerOk)<>D972RF3CheckerSentinel or
     D972RF3Count(D972RF3CheckerRaw,
       "B345_RELFRAT3_PIVOT_SURGERY_V7_CHECKER_PASS")<>1 then
    Error("157dv v7 driver: checker gate");
  fi;
  Exec("sha256sum 'ci/out/d972_b345_relfrat3_pivot_surgery_v7.json' | awk '{printf \"%s\",$1}' > 'ci/out/d972_b345_relfrat3_pivot_surgery_v7.sha256'");;
  ...
  if Length(D972RF3ArtifactSHAValue)<>64 then
    Error("157dv v7 driver: relative artifact SHA length");
  fi;
  Print("B345_RELFRAT3_PIVOT_SURGERY_V7_GHA_DRIVER_PASS mode=full artifact_sha256=",
