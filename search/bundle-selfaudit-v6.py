#!/usr/bin/env python3
# bundle-selfaudit-v6.py -- N_infty stage2 freeze bundle self-audit (15 checks + M70/M71 fail-closed 変異試験)
#
...
F = {"spec":     "docs/week4-NInfty_stage2_spec_v16.md",
     "contract": "docs/mb_ninfty_verifier_contract_v11.md",
     "manifest": "docs/mb_dependency_manifest_v11.md"}
CUR = {"spec": "v16", "contract": "v11", "manifest": "v11"}
...
if "--mutate" in sys.argv:
    ...
    H2 = "b" * 64
    NEG = [
      ("present=non-boolean",            {"build_record_present": "yes", "source_artifact_digests": ["s"],
                                          "toolchain_digest": H2, "build_step_digests": [H2]}, "[12]"),
      ("false+toolchain=null",           {"build_record_present": False, "source_artifact_digests": ["s"],
                                          "toolchain_digest": None, "build_step_digests": [H2]}, "[12]"),
      ("false+toolchain=non-64hex",      {"build_record_present": False, "source_artifact_digests": ["s"],
                                          "toolchain_digest": "deadbeef", "build_step_digests": [H2]}, "[12]"),
      ("false+build_steps=non-list",     {"build_record_present": False, "source_artifact_digests": ["s"],
                                          "toolchain_digest": H2, "build_step_digests": H2}, "[12]"),
      ("true+pinned=non-list",           {"build_record_present": True, "source_artifact_digests": ["s"],
      ...
      ("true+pinned=[]",                 {"build_record_present": True, "source_artifact_digests": ["s"],
                                          "toolchain_digest": H2, "build_step_digests": [H2],
    ...
    # ---- M71-3 BC_USE_MAP causal-use gate(F12.1)----
    FIELD_LINE = {"recompute": "recompute", "required_keys": "required_keys", "forbidden_keys": "forbidden_keys",
                  "closure_policy": "closure_policy"}
    CONS_FN = {"D-R2" + chr(0x2057): consumer_DR2, "I-0c" + chr(0x2033): consumer_I0c,
    ...
    if BC_USE is not None:
        for cname, fn in CONS_FN.items():
            ...
            if deps is None: okmap = False; det.append("%s が BC_USE_MAP に無い" % cname); continue
            det.append("%s<-%s" % (cname, ",".join(deps)))
        # 宣言と実装の整合: recompute を持つ consumer だけが true/false で D-3 検査を変える
        rec_users = [c for c, d in BC_USE.items() if "recompute" in d]
        okmap &= ("D-R2" + chr(0x2057) in rec_users) and ("I-0c" + chr(0x2033) in rec_users)
        okmap &= ("build_artifact_set" in BC_USE and "recompute" not in BC_USE["build_artifact_set"])
    mreport("M71-3", okmap, "BC_USE_MAP parsed=%s / %s" % (BC_USE is not None, " ; ".join(det)))
