...
# q2/q7 use every nonzero prime-field parameter.  a=0 is excluded explicitly:
# the Burau generator determinant then vanishes, so it is not a GL_4 field
# specialization.  Each registered pair is rechecked by audit_specialization.
...
def audit_specialization(q: int, a: int) -> dict[str, Any]:
    """Recheck field-unit, invertibility, and defining braid gates."""
...
def kernel_from_section(gens: Sequence[JointElt], sec: dict[Perm, JointElt],
    ...
    for r, lift in sec.items():
        for g in signed_gens(gens, specs):
            ...
            if z != one:
                rels.add(z)
    return sorted(rels, key=tuple_key)


def enumerate_kernel_with_witness(
        gens: Sequence[JointElt], specs: Sequence[Spec], degree: int
) -> tuple[list[JointElt], list[JointElt]]:
    """Exact joint-kernel BFS plus discovery-edge generating witnesses."""
...
def enumerate_kernel(gens: Sequence[JointElt], specs: Sequence[Spec],
    ...
    return enumerate_kernel_with_witness(gens, specs, degree)[0]
...
def complete_hprime(x: JointElt, y: JointElt, specs: Sequence[Spec],
                   expected: int) -> tuple[dict[Perm, JointElt], list[JointElt],
                                                list[JointElt], int, list[JointElt]]:
    ...
    while True:
        ...
        kelts, witness = enumerate_kernel_with_witness(rels, specs, len(x[0]))
        kset = set(kelts)
        ...
        print(f"D972_B4_BURAU_JOINT_ACCEL_PROGRESS phase=normal-closure "
...
def run_full(config: str, output: Path) -> int:
    specs = specs_for(config)
    registered_audit = specialization_audit(specs)
    rows = load_words()
    ...
    print(f"D972_B4_BURAU_JOINT_ACCEL_PROGRESS phase=schreier-complete "
    ...
    for i, (m, key, word) in enumerate(rows, 1):
        ...
        if full_count == 0:
            ...
            break
        if i % 81 == 0:
            print(f"D972_B4_BURAU_JOINT_ACCEL_PROGRESS phase=rows config={config} "
                  f"completed={i}", flush=True)
    status = ("CANDIDATE_B4_A_BURAU_JOINT_ZERO_FIBER" if any_full_zero
              else "UNKNOWN_BURAU_JOINT_ALLPASS")
    require(row_scan_contract(status, len(out_rows), row_scan_complete,
                              len(out_rows), terminal_zero_row_index),
            "invalid candidate/all-pass scan metadata")
    receipt = {
        ...
        "algorithm": ALGORITHM,
        "admissibility_audit": ADMISSIBILITY_AUDIT,
        "registered_specialization_audit": registered_audit,
    ...
    output.write_text(json.dumps(receipt, sort_keys=True, separators=(",", ":")) + "\n",
                      encoding="utf-8")
    print(f"D972_B4_BURAU_JOINT_ACCEL_DONE config={config} h={h_order} "
          f"hprime={hprime_order} kernel={len(kernel)} rows={len(out_rows)}")
...
def self_test() -> None:
    ...
    require("UNKNOWN_RESOURCE" not in ADMISSIBLE_STATUSES,
            "corrupt receipt status fixture accepted")
    ...
    one = joint_identity(q3_specs)
    witness_blocks = list(one[1]); witness_blocks[0] = burau_generators(3, -1)[0]
    toy_generator = (one[0], tuple(witness_blocks))
    toy_kernel, toy_witness = enumerate_kernel_with_witness(
        [toy_generator], q3_specs, 36)
    require(toy_witness and
            set(enumerate_kernel(toy_witness, q3_specs, 36)) ==
    ...
    print("D972_B4_BURAU_JOINT_ACCEL_Q2Q7_ADMISSIBILITY_AUDIT_PASS "
          "q2=[1] q7=[1,2,3,4,5,6]")
    print("D972_B4_BURAU_JOINT_ACCEL_SOURCE_WORD_ROOF_ONLY_NEGATIVE_PASS "
          "row=2 exponent=(-4,-8) nonzero=956")
    print("D972_B4_BURAU_JOINT_ACCEL_NEGATIVE_FIXTURES_PASS")
    print("D972_B4_BURAU_JOINT_ACCEL_SELFTEST_PASS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("ci/out/d972_b4_burau_joint_accel_v1_q3a2_q4a2.json"))
    ...
    try:
        ...
        print(f"D972_B4_BURAU_JOINT_ACCEL_ERROR {type(exc).__name__}: {exc}",
              file=sys.stderr)
        return 2
