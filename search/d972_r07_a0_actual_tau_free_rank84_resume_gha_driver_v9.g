#############################################################################
## Task488 rank-84 continuation runtime-envelope repair v9. External preamble required.
...
#############################################################################
if not IsBound(D972_R07_A0_RANK84_CHECKPOINT_RESUME_V9_RUN) or
   D972_R07_A0_RANK84_CHECKPOINT_RESUME_V9_RUN<>true then
 Error("task488 external preamble required"); fi;
...
D484Zip:="ci/out/d972_r07_a0_actual_tau_free_rank84_resume_v9.zip";;
D484Extract:="ci/out/d972_r07_a0_actual_tau_free_rank84_resume_v9_archive";;
D484ResumeMember:="d972_r07_a0_actual_tau_free_rank_ladder_v7_output.checkpoint";;
D484Input:="ci/out/d972_r07_a0_actual_tau_free_rank84_resume_v9_input.checkpoint";;
D484Artifact:="ci/out/d972_r07_a0_actual_tau_free_rank_ladder_v9.json";;
D484Checkpoint:="ci/out/d972_r07_a0_actual_tau_free_rank_ladder_v9_output.checkpoint";;
D484PL:="ci/out/d972_r07_a0_actual_tau_free_rank_ladder_v9_producer.log";;
D484CL:="ci/out/d972_r07_a0_actual_tau_free_rank_ladder_v9_checker.log";;
D484DL:="ci/out/d972_r07_a0_actual_tau_free_rank84_resume_v9_preflight.log";;
D484ZipBytes:=23004;;
...
D484ResumeSHA:="eb1a6d69a855b88d2a934dbf8e58c0f539a7a4d6802cc4a5c7f544b0880da24f";;
D484MemberManifest:=[
...
D484Quote:=function(s)
 if PositionSublist(s,"'")<>fail or PositionSublist(s,"\n")<>fail or
    PositionSublist(s,"\r")<>fail then Error("task488 unsafe shell text"); fi;
 return Concatenation("'",s,"'");
end;;
if Length(D484MemberManifest)<>7 then Error("task488 seven-member manifest"); fi;
if D484Run<>"33524681526" or D484Job<>"99912387760" or
 ...
 Error("task488 immutable source binding drift"); fi;
if IsExistingFile(D484Zip) or IsDirectoryPath(D484Extract) or
   ...
   IsExistingFile(D484CL) or IsExistingFile(D484DL) then
 Error("task488 stale output"); fi;
if not IsDirectoryPath("ci/out") then
 if CreateDir("ci/out")=fail then Error("task488 ci/out"); fi; fi;
D484Cmd:=Concatenation(
 "set -euo pipefail; umask 077; ",
 "test ! -e \"",D484DL,"\"; test ! -L \"",D484DL,"\"; ",
 "exec 9>\"",D484DL,"\"; ",
 "printf \"TASK488_R07_RANK84_PREFLIGHT_BEGIN\\n\" >&9; ",
 "task488_preflight_fail() { rc=$?; printf \"TASK488_R07_RANK84_PREFLIGHT_FAIL rc=%s cmd=%s\\n\" \"$rc\" \"$BASH_COMMAND\" >&9; return \"$rc\"; }; ",
 "trap task488_preflight_fail ERR; ",
...
Exec(Concatenation("bash -o pipefail -c ",D484Quote(D484Cmd)));
 if not IsExistingFile(D484DL) or Length(StringFile(D484DL))=0 then
  Error("task488 preflight diagnostic missing"); fi;
 if not IsExistingFile(D484Artifact) or not IsExistingFile(D484Checkpoint) or
    not IsExistingFile(D484CL) then Error("task488 result/checker missing"); fi;
if Length(StringFile(D484Artifact))=0 or Length(StringFile(D484Checkpoint))=0 then
 Error("task488 result/checkpoint empty"); fi;
if PositionSublist(StringFile(D484PL),"R07_A0_ACTUAL_TAU_FREE_RANK_LADDER_V3 status=")=fail then
 Error("task488 producer marker"); fi;
if StringFile(D484CL)<>"R07_A0_ACTUAL_TAU_FREE_RANK_LADDER_V7_CHECKER_PASS\n" then
 Error("task488 checker marker"); fi;
 Print("R07_A0_RANK84_CHECKPOINT_RESUME_V9_DRIVER_PASS\n");
