...
D380Producer:="search/d972_r07_history_free_positive_fast_resume_v12e.py";;
...
D380Pins:=[
 ...
 [D380Producer,342849,"5d023748d2a840ca0d95109dc77d5410eebd47cb47a7d6ce8afe7910ceaf3c58"],
...
PrintTo(D380Stream,"for command in python3 timeout grep sed cmp uname wc tail; do command -v \"$command\" >/dev/null; done\n");
...
PrintTo(D380Stream,"}\n");
PrintTo(D380Stream,"exit_handler() {\n");
PrintTo(D380Stream,"status=$?\n");
PrintTo(D380Stream,"if [ \"$status\" -eq 0 ]; then return 0; fi\n");
PrintTo(D380Stream,"set +e\n");
PrintTo(D380Stream,"for log in \"$producer_tmp\" \"$checker_tmp\"; do\n");
PrintTo(D380Stream,"  if [ -f \"$log\" ]; then\n");
PrintTo(D380Stream,"    printf '%s\\n' \"task380 bounded failure log: $log\" >&2\n");
PrintTo(D380Stream,"    tail -c 65536 -- \"$log\" >&2\n");
PrintTo(D380Stream,"    printf '\\n%s\\n' \"task380 end bounded failure log: $log\" >&2\n");
PrintTo(D380Stream,"  fi\n");
PrintTo(D380Stream,"done\n");
PrintTo(D380Stream,"cleanup_owned\n");
PrintTo(D380Stream,"cleanup_status=$?\n");
PrintTo(D380Stream,"if [ \"$cleanup_status\" -ne 0 ]; then printf '%s\\n' \"task380 cleanup failed with status $cleanup_status\" >&2; fi\n");
PrintTo(D380Stream,"return \"$status\"\n");
PrintTo(D380Stream,"}\n");
PrintTo(D380Stream,"trap exit_handler EXIT\n");
