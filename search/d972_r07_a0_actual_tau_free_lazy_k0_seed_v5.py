#!/usr/bin/env python3
"""Task527: actual task445 rank111 lazy K=0 single-row successor."""
...
MARKER = "R07_A0_ACTUAL_TAU_FREE_LAZY_K0_SEED_V5"
...
base = load(BASE, "task527_task445_v3")
PIN_BINDING = {
    "schema": SCHEMA,
...
def validate_counters(value: Any) -> dict[str, int]:
    ...
    need(all(isinstance(value[key], int) and value[key] >= 0 for key in COUNTER_KEYS),
         "counter type")
    return value
...
def validate_new_source(source: dict[str, Any]) -> dict[str, Any]:
    ...
    need(source.get("record_version") == 5, "new record version")
    ...
    if source["kind"] == "action":
        ...
        need(source.get("selector_mode") == "K0_SUPPORT" and source.get("K") == 0,
             "K0-only correction")
        ...
        need(source.get("formula_scalar") == source.get("scalar"), "formula scalar")
        need(isinstance(source.get("N_coefficients"), list) and
             len(source["N_coefficients"]) == 2 and
             all(x in (0, 1, 2) for x in source["N_coefficients"]), "N coefficients")
        need(isinstance(source.get("normalized_exponent_quotients"), list) and
             len(source["normalized_exponent_quotients"]) == 2 and
             source["exact_exponent_pair"] ==
             [18 * x for x in source["normalized_exponent_quotients"]], "epsilon/18 typing")
...
def setup(args: argparse.Namespace, started: float):
    v1 = base.load(base.b.V1, "task527_state_v1")
    v4 = v1.load(v1.V4, "task527_state_v4")
    m = v4.load_v1()
    m.RUN_STARTED = started
    m.MARKER = MARKER
    v12 = m.load(m.V12, "task527_v12")
    p435 = m.load(m.P435, "task527_p435")
    p179 = m.load(m.P179, "task527_p179")
...
def direct_correction(P: dict[str, Any], p179: Any, sf: Any,
    ...
    pivot = min(reduced)
    extra = {"record_version": 5, "selector_mode": "K0_SUPPORT",
...
def select_one(P: dict[str, Any], m: Any, p179: Any, state: tuple[Any, Any, Any],
    ...
    for candidate, source0 in P["q"].action_support_hits(
        ...
        extra = {"record_version": 5, "selector_mode": "ACTION",
...
def replay_new(P: dict[str, Any], m: Any, p179: Any,
    ...
    for record in accepted[LEGACY_ACCEPTED:]:
        ...
        generated, state = commit(P, m, [], record["round"], state, selected)
        inv(generated == record, "new resume record")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    progress: dict[str, Any] = {"phase": "migrated_legacy" if legacy else "resumed_v5"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", default="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_seed_v5.json")
    parser.add_argument("--checkpoint", default="ci/out/d972_r07_a0_actual_tau_free_lazy_k0_seed_v5_output.checkpoint")
