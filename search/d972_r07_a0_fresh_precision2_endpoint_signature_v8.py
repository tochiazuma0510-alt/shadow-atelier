#!/usr/bin/env python3
"""Task759: finite release repair for the precision-two endpoint consumer.
...
MARKER = 'R07_FRESH_PRECISION2_ENDPOINT_SIGNATURE_V5_CANDIDATE'
...
RUNTIME_PROFILE = {
    'profile': 'endpoint-minimal-full-signature-bucket-v1',
    'contexts': 31,
    ...
    'direct_replay': 'full-11-slot-nonzero-buckets',
...
def replay_bucket_direct(buckets, relators, direct_column, progress, guard_call):
    """Call the direct all-seven canary exactly once per nonzero bucket."""
    total = len(buckets)
    done = 0
    for (seed, _signature), (coefficient, path) in buckets.items():
        require(coefficient in (1, 2), 'endpoint_direct_zero_bucket')
        require(type(seed) is int and 1 <= seed <= len(relators),
                'endpoint_direct_seed')
        direct_column(path, tuple(int(x) for x in relators[seed - 1]))
        done += 1
        guard_call()
        progress(done, total)
...
def evaluate(roots,replaced,loaded,task601,candidate,out):
    ...
    endpoint_checkpoint(seven_runtime['meter'], 'endpoint_direct_column_start')
    replay_bucket_direct(
        buckets, words['relators'], all_seven.direct_column,
        lambda done, total: seven_runtime['meter'].check(
            'endpoint_direct_column_done=' + str(done) + '/' + str(total)),
        lambda: guard(started))
    endpoint_checkpoint(seven_runtime['meter'], 'endpoint_direct_column_complete')
...
def _tiny_bucket_fixtures():
    ...
    calls = []; progress = []
    replay_bucket_direct(
        grouped, [[1]], lambda path, relator: calls.append((path, relator)),
        lambda done, total: progress.append((done, total)), lambda: None)
    require(calls == [((2,), (1,))] and progress == [(1, 1)],
            'fixture_equal_signature_one_direct')

    zero = canonical_signature_buckets(
        [[1, [1], 1], [1, [2], 2]], signatures)
    zero_calls = []
    replay_bucket_direct(
        zero, [[1]], lambda path, relator: zero_calls.append((path, relator)),
        lambda done, total: None, lambda: None)
    require(not zero and not zero_calls, 'fixture_zero_bucket_no_direct')
    ...
    malicious_zero = {(1, full): (0, (2,))}
    reject(lambda: replay_bucket_direct(
        malicious_zero, [[1]],
        lambda path, relator: calls.append((path, relator)),
        lambda done, total: None, lambda: None))
    reject(lambda: validate_direct_completion(0, 1))
    return {'bucket_mutation_rejections': rejected,
            'equal_signature_direct_calls': len(calls),
            'zero_bucket_direct_calls': len(zero_calls),
