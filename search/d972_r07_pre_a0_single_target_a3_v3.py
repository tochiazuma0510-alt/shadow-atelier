...
def run_raw_authority_mutation(p0, pins, raw_by_path, meter):
    """Mutate the authenticated manifest and re-enter the full validator."""
    ...
    observed = None
    try:
        authenticate_task198(mutant_p0, pins, mutant_raws)
            ...
            class PackedJointGroup(joint.JointGroup):
                def blob(self, value):
                    return p176.packed_joint_blob(
                        value, "v3 task198 evaluator Gamma element")
...
def run_mutations(rows, p0, g760, interface, consumer,
    ...
    for name in roster[1:]:
        ...
        if name == "task198_ledger_sign":
            ...
            owner = "central"
            mutant[owner] = mutant_p0["central_rows"]
            ...
            seal_fresh(mutant["consumer"], "mutated task227 consumer ABI")
            owner = "consumer"
...
def main(argv=None):
    ...
    parser.add_argument("--output",
                        default="ci/out/d972_r07_pre_a0_single_target_a3_v3.json")
    args = parser.parse_args(argv)
    meter = Meter()
    try:
        target = prepare_output(args.output)
        terminal, receipt = production(meter)
        with WallDeadline(meter, "receipt sealing and publication"):
