#!/usr/bin/env python3
"""Task1185 draft: independent fixed-lambda, six-layer parent2218 checker.
...
PARENT_ROLES = (*V8_PARENT_ROLES, "batch-parent-v8")
PRODUCER_FILE = "search/d972_r07_fixed_lambda_cycle_batch_v9_repair_v2.py"
CHECKER_FILE = "search/check_d972_r07_fixed_lambda_cycle_batch_v9_repair_v2.py"
CHECKER_WORKFLOW = ".github/workflows/d972-r07-fixed-lambda-cycle-batch-v9-repair-v2.yml"
...
CURRENT_PRODUCER_REGISTRATION: dict[str, Any] | None = {"bytes":760214,"file":"search/d972_r07_fixed_lambda_cycle_batch_v9_repair_v2.py","sha256":"99cefc6c3eff5f0b192f9b2b1ee4e17e277b41b849e2392625041b86f0046db1"}
...
class SavedParentJsonBytes:
    ...
    def __init__(self, ordinal: int):
        ...
        self.rows = {(role, kind): {key: 0 for key in self.metrics}
            for role in (BATCH_PARENT_ROLE, NEXT_BATCH_ROLE, THIRD_BATCH_ROLE, FOURTH_BATCH_ROLE, FIFTH_BATCH_ROLE, SIXTH_BATCH_ROLE)
    ...
    def emit(self, status: str) -> None:
        ...
        emit_auxiliary_diagnostic({"schema": "d972.r07.saved-parent-json-parse-bytes.v1", "side": "C",
            ...
            "measurement_scope": "same-PinnedTree.json-read-to-json_value;six-saved-roles;candidate6/reduction-and-physical-literal;successful-canonical-parse-not-later-seal-or-schema;unique-first-successful-role-path",
...
def check_acceptance_header(value: Any) -> None:
    require(type(value) is dict and len(value) == V9_CURRENT_ACCEPTANCE_KEY_COUNT and set(value) == {"schema", "parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "batch_anchor_v8", "code", "runtime", "registration"} and
            value["schema"] == SCHEMA + ".acceptance", "acceptance_exact_twelve_plain_keys")
...
class AcceptedInputs:
    ...
    def __init__(self, args: argparse.Namespace, parse_bytes: SavedParentJsonBytes):
        ...
        check_sixth_parent_roles(value["parents"])
        require(type(SIXTH_BATCH_INVENTORY_REGISTRATION) is dict and type(CURRENT_PRODUCER_REGISTRATION) is dict,
                "v9_formal_inventory_and_final_source_binding_pending")
        ...
        same_json(code["producer"], CURRENT_PRODUCER_REGISTRATION, "v9_registered_final_P9_opaque_full_pin")
...
def authenticate_sixth_batch_parent_metadata(inputs: AcceptedInputs) -> dict[str, Any]:
    ...
    check_sixth_native_parent_projection(inputs.portable["parents"], layout["parents"])
...
def root_records(inputs: AcceptedInputs, anchor: ThinAnchor, old: dict[str, Any], bundle: C.FixedBundle) -> RootRecords:
    ...
    layout = document("parent-layout", {"portable_acceptance_sha256": inputs.portable_sha256,
        **{key: copy.deepcopy(inputs.portable[key]) for key in ("parents", "anchor", "batch_anchor", "next_batch_anchor", "batch_anchor_v5", "batch_anchor_v6", "batch_anchor_v7", "batch_anchor_v8", "code", "runtime", "registration")}})
    ...
    emit_parent_timing("direct-pairing", SIXTH_BATCH_ROLE, 6, pairing_started, rows=pairing["rows"], records=len(anchor.parents))
    ...
    start = document("start", {"owner_sha256": sha(canonical(owner)), "source_sha256": sha(canonical(source)),
        ...
        "previous_parent_target_derivations": len(inputs.fifth_intermediate_parents),
    ...
    require(len(layout) == V9_CURRENT_LAYOUT_KEY_COUNT and len(start) == V9_CURRENT_START_KEY_COUNT and
            len(intake) == V9_CURRENT_INTAKE_KEY_COUNT, "c9_current_root_key_counts_from_independent_sums")
...
def batch_observation(inputs: AcceptedInputs, records: RootRecords, files: CandidateFiles,
    ...
    if not admitted:
        sequence = -1
    batch = inputs.value["batch_anchor_v8"]
    parents = {item["role"]: item["artifact"] for item in inputs.portable["parents"]}
    accepted = inputs.sixth_batch_parent["records"]
    old = {"parent_role": "batch-parent-v7", "parent_artifact": copy.deepcopy(parents["batch-parent-v7"]),
    ...
    current = {"parent_role": "batch-parent-v8", "parent_artifact": copy.deepcopy(parents["batch-parent-v8"]),
    ...
    if admitted:
        ...
        require(pairing["rows"] == SIXTH_BATCH_RANK and pairing["lambda_pivots"] == 0 and
    ...
    check_sixth_unobserved_oracle(current, sequence)
...
def compare_producer_result(files: CandidateFiles, inputs: AcceptedInputs, records: RootRecords,
    ...
    check_v9_current_anchor_batch_counts(actual)
...
def check_actual(args: argparse.Namespace, report: dict[str, Any], parse_bytes: SavedParentJsonBytes) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        restore_started = time.monotonic()
        anchor = stack.enter_context(promote_sixth_batch_parent(inputs, fifth_intermediate, old))
        emit_parent_timing("state-restore", SIXTH_BATCH_ROLE, 6, restore_started, rows=anchor.rank, records=len(anchor.parents))
        ...
        if "selection/start.json" in files.expected:
            check_v9_current_anchor_batch_counts(records.selection_start)
        ...
        if "start.json" in files.expected and "parent-intake.json" in files.expected:
            check_v9_current_anchor_batch_counts(report)
        ...
        if "final/manifest.json" in files.before:
            ...
            check_v9_current_anchor_batch_counts(final.manifest)
            if final.head is not None:
                check_v9_current_anchor_batch_counts(final.head)
...
def k128_registration_canary(root: Path) -> dict[str, Any]:
    ...
    acceptance = {"schema": SCHEMA + ".acceptance", "parents": [], "anchor": {}, "batch_anchor": {}, "next_batch_anchor": {}, "batch_anchor_v5": {}, "batch_anchor_v6": {}, "batch_anchor_v7": {}, "batch_anchor_v8": {}, "code": {},
    ...
    try:
        if args.selftest:
            ...
            require(all(getattr(args, role.replace("-", "_") + "_root") is not None for role in PARENT_ROLES if not role.startswith("block-")) and
                    ...
                    "all_twenty_one_roots_and_exact_acceptance")
