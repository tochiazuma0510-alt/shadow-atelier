...
class SoftDeadline:
    """Monotonic producer deadline; never constructed by selftest."""

    def __init__(self, start: float, seconds: int) -> None:
    ...
    def check(self, phase_name: str) -> None:
        self.last_phase = phase_name
        self.check_count += 1
        if time.monotonic() - self.start >= self.seconds:
            raise ResourceStop("producer_soft_timeout")
...
class SparseBoundaryBasis:
    def __init__(self, quotient: MatchedQuotient,
                 relator_columns: list[SparseVector], dag: ProvenanceDAG,
...
def serialize_proof_dag(dag: ProvenanceDAG, roots: dict[str, int],
    ...
    require(bool(roots) and len(set(roots.values())) <= len(roots),
            "proof DAG roots")
    reached = dag.reachable(list(roots.values()))
    ordered = sorted(reached)
    renumber = {old: new for new, old in enumerate(ordered, 1)}
    ...
    for old_id in ordered:
        ...
        if basis.deadline is not None and new_id % 1024 == 0:
            basis.deadline.check("proof_DAG_serialization")
        source = dag.nodes[old_id-1]
        if source["kind"] == "translated_relator_leaf":
            translation = source["translation"]
            require(translation in basis.translation_sections,
                    "proof DAG leaf translation section")
            translation_id = registry.add(
                4, translation, basis.translation_sections[translation])
            row = {
                "id": new_id,
                "kind": "translated_relator_leaf",
            ...
            require(source["kind"] == "linear_combination", "proof DAG node kind")
            terms = []
            for parent, coefficient in source["terms"]:
                require(parent in renumber and renumber[parent] < new_id and
                        coefficient in (1, 2), "proof DAG topological term")
                terms.append({"node_id": renumber[parent],
                              "coefficient": coefficient})
            row = {"id": new_id, "kind": "linear_combination", "terms": terms}
            edge_count += len(terms)
        nodes.append(row)
    root_rows = [{"name": name, "node_id": renumber[node_id]}
    ...
    payload = {
        ...
        "combination_node_count": len(nodes)-leaf_count,
        "all_serialized_nodes_reachable_from_roots": True,
        "unreachable_search_nodes_pruned": len(dag.nodes)-len(nodes),
        "expanded_boundary_ledgers_serialized": False,
    }
    return {**payload, "dag_sha256": digest_obj(payload)}, renumber
...
def self_test() -> None:
    ...
    leaf_a = toy_dag.leaf(1, ((0,), (0,)))
    leaf_b = toy_dag.leaf(2, ((1,), (1,)))
    ...
    require(root == 4 and toy_dag.nodes[root-1]["terms"] ==
            [(leaf_a, 1), (leaf_b, 2)] and toy_dag.reachable([root]) ==
            {leaf_a, leaf_b, root}, "shared provenance DAG/rollback canary")
    print("D972_B345_RELFRAT3_V2_PRODUCER_SELFTEST_PASS "
          f"relevant_formula_sha256={digest_obj(relevant_formula())} "
          "normalized_inverse_cache_hit_canaries=1 provenance_DAG_canaries=1 "
          "soft_timeout_consulted=false")
