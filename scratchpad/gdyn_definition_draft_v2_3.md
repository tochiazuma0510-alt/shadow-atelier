# $G_{\rm dyn}$ 定義起草 **v2.3** — 定理 DYN-NOGO・二窓テスト・$T/O$ の明示化
...
**状態札**: `§1 = ★補題 VE-NOGO + 系 DYN-NOGO(paper-proof・falsifier 独立照合 PASS)/ §2 = 撤回 2 件 / §3 = 実装仕様 v2(candidate)/ §4 = T/O の分岐データ(**v2.3 で機械裏づけ済** — sympy の恒等式 + Riemann–Hurwitz `gate:`・§6-2 補題 O-RH)/ §5 = prereg 文(**v2.3 で Z-1/Z-3 を追加**)/ §7 = falsifier 照合記録 / §8 = ★鋭形 VE-NOGO⁺ + 系 VE-COF + VE-TEST(v2.1 新設・paper-proof・未監査)/ novelty は [12][14] の UNVERIFIED 債務に依存`
...
> **(b) が等号(切断力ゼロ)のときの解釈は二義であり、実験では分離できない**:
> 1. **力学が弱い** — $T$ の $\mathrm{IMG}$ が算術像を切るだけの情報を持たない;
> 2. **塔が届いていない** — $\bigcap_kN_k=\ker(F_2\twoheadrightarrow\mathrm{IMG}(T))\ne1$ なら、二窓テストの分解能はこの核で頭打ちになり、$\mathrm{DYN}_T$ は核の中を見分けられない(§8.2)。**$T$ について $\ker=1$ かは UNKNOWN。**
>
> ⟹ **cert には必ず両方の解釈を書くこと。**「$T$ でも足りない ⟹ 写像の選定基準を再設計」と**単一解釈で結論してはならない**(⚠ v2/v2.1/v2.2 のこの行はその単一解釈で書かれていた — **本ブロックで置換**)。
> **分離するには**別途 $\ker(F_2\to\mathrm{IMG}(T))$ の非自明性(または自明性)の独立判定が要る — **未着手・UNKNOWN**。
...
> **一般形**: 定義から $\displaystyle\bigcap_k N_k=\ker\bigl(F_2\twoheadrightarrow\mathrm{IMG}(\varphi)\bigr)$。**これが $1$ かどうかは $T$・$O$ について UNKNOWN**(自前知識では、非例外的 PCF 写像の IMG は縮小的で自由群ではないと予想されるが、**これは推測であって証明ではない** — 格 = `heuristic`)。
