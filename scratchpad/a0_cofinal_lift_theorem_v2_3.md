# A0 と「cofinal な両立 lift」— 二つの塔の分離・有限厳密性定理・König 境界(候補 paper note v2.3)

Author: Fable(数学者・Claude 側)/ 2026-09-05
Status: candidate paper note.  **v2.3 = v2.2(sha16 `5953a6c2e1d8d022`, 不変・保存)に工房 falsifier 前哨(`scratchpad/fal_a0cl_v2_2_preflight_v1.md`, sha16 `7fd1c0e71b3a5428`)の
...
本版(v2.2・v2.3 とも)は **定理 A・定理 B・命題 C とその表題・証明を一切変更していない**(Sol の保持判定と前哨の指摘に従う).  v2.2 の変更は §5.2/§5.3/§0/§8/§10 の主張の射程と
...
機械計算 = **v2.2・v2.3 では新規計算なし**(§9 の出力・sha16 は v2.1 のまま).  **再実行の帰属(v2.3 で明確化・D19)**: `fal_a0cl_e3check_v1.g` の再実行は **v2**, `fal_a0cl_nu_pent_check_v1.g` の再実行は **v2.1**,
...
決める」型は, **全 f 一様な形**については DLL の有限反例(35 窓中 24 で不成立)により**その商族の射程内で**反証されるが(DLL の商族と R07 の登録相対 Frattini 商族との翻訳は未証明 = §8 Q8), **点固有の形は排除されていない**(副有限版 = Furusho Q14 は未解決のままで, 有限反例からは偽と結論できない).  本稿はいずれも証明しない.  v2.2 はこの三分割と「(c) が唯一」の撤回, 「全段非空 ⟺ 証人」の lane 限定を行った版である.
...
すなわち**現行作業は `2,016 → 54,432` の段であって `54,432 → Q₀`(= rung 4, 未着手)ではない**(Sol 162 §0 1. の前提訂正; **地図は 9/4 delta `docs/地図.md` L27 で訂正済** — 9/3 delta は元から同じ target 名規約, D20).
...
**内部にも同型の例がある(v2.3 で条件形に修正・D16 — 独立の第二論証ではなく, Sol の反例と同型の instance)**: **仮に** R07 の欠陥加群上で return involution θ の同変性が成り立つとすると,
...
**R07 の完成水準での θ 同変性は未確立**であり, 本稿もそれを主張しない.  またこの reductio は **Sol の反例(`H1 − H2 = 0`)を R07 の記号で書いた同型の instance**であって(λ の量化に `E_P` 係数 0 を
...
UNIFORM LIFT THEOREM / U3 EXISTENCE:                                                              NOT PROVED
  (b) UNIFORM FORM (H-forcing syzygy, all admissible f):  REFUTED IN THE DLL FINITE-QUOTIENT SCOPE (stock (2)); TRANSLATION TO THE REGISTERED K_{r,n} UNPROVED  (N1a)
...
- **D5(必須・撤回・§5.2 (i′)/§0/要約/§10)**: 「**形式的 P–H syzygy は存在しない**」を **撤回**.  syzygy であること自体からは pentagon ⟹ hexagon は出ない(Sol の反例: 関係 `H1 − H2 = 0` は
  `P = 0` かつ `H1 = H2 ≠ 0` を許す).  内部の同型例(**v2.3 の D16 で条件形に修正**): (S1) の θ 同変性を**仮定すれば**同じ reductio が立つ — ただし独立の第二論証ではなく Sol の反例と同型の instance.
...
- **D16(要修正 F2・§5.2 (i′)/§10 D5)**: D5 の「内部反証」の前件を **条件形**に書き直した.  引用先はいずれも仮定形である — v169 §5「**Suppose** … θ acts continuously on (3.3)」, v399「The actual R07
  ...
  **仮定すれば**」と書く.  あわせて, この reductio は (S1) が `E_P` を含まない以上 **Sol の反例 `H1 − H2 = 0` と同型の instance**であって独立の第二論証ではないと明記し, 逆に **(N1a) の「H-forcing」
  限定がなぜ要るかの実例**として位置づけ直した.  claim boundary は維持((N1a) の根拠は在庫 (2) の有限反例であって, この instance ではない).
- **D17(軽微 F3・§5.4)**: (d1)/(d2) の「未証明の前件」欄に **語つき source section σ** と **all-edge specialization**(関手的 schema; v526 §6 / v537 §4)を追加し, Sol 162 §3 の 4 本列挙
...
§9 の機械出力そのもの, および v2.2 の narrowing 判断((N1a)(N1b)(N1c)・D6 の UNKNOWN 格下げ・D7 の読み替え・D8 の撤回・D10 の lane 限定)— 前哨は「Sol 162 §2 の 3 要求を過不足なく吸収」と判定した.

`R07_A0_COFINAL_LIFT_TWO_TOWERS_FABLE_V2_3`
