...
## 追記(972専任数学者 初回裁定 972-01 §3.2 準拠・ERRATUM-972-M-01 と同時執行)

**(iv)-m は掃引なしで閉じる。**

972-01 §3.2 の指摘: Prop 2.3(well-defined性)+ N_ord=18 の実測(本セッションで確定した「粗商 normalized-orbit の周期9」ではなく、**細対象 e4 自身・5 coface での ord=18**)を組み合わせると、それ自体が **走査宇宙の完全性証明書**として機能する——④で述べた「(iv): 走査宇宙の完全性証明書なし」という工房側ドラフトの結論は、**B(m掃引)という枠組み自体が誤っていたための誤読**であり、正しい対象系(A型計器 v3、972-01 §4/§10 仕様)では掃引を尽くさずとも(iv)が閉じる、という主張として記録する。

この追記は ERRATUM-972-M-01(`search/certs/d972_atype_gtpair_sweep_v1_20260822.json` / `_v2_O1O5_` 両cert)と同一裁定に基づく。旧ドラフト①〜⑤の記述(T-56原文・Def 3.12逐語・discharge表)自体は撤回しない(一次資料としてなお有効)——撤回されるのは「B(m掃引)というレンズを通した読み」のみ。**「GT-pairはm=0のみ」という句は本仕様書内でも使用禁止**(ERRATUM-972-M-01 に準拠)。
