...
EOF
echo "=== RETRY3 window (${ORDER},${ID}) ===" | tee -a "$LOGFILE"
timeout 120 powershell.exe -NoProfile -File ./gap.ps1 "$PRELUDE" < /dev/null >> "$LOGFILE" 2>&1
RC=$?
if [ "$RC" -eq 124 ]; then
  echo "TIMEOUT_SKIPPED ${ORDER} ${ID} wall_cap_120s_exceeded_retry3" | tee -a "$STATUS_FILE"
  taskkill //F //IM gap.exe > /dev/null 2>&1
elif [ "$RC" -ne 0 ]; then
  echo "COMPUTE_FAILED ${ORDER} ${ID} exit_code_${RC}_retry3" | tee -a "$STATUS_FILE"
else
  echo "COMPLETED ${ORDER} ${ID}_retry3" | tee -a "$STATUS_FILE"
fi
rm -f "$PRELUDE"
