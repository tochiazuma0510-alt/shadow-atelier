# MEAS プログラム 草案 v1.3 — 井原の証人問題の測度論的再定式化

> ### ★ 本文書の規約(2 本・全文書共通)
> **(R-1) 訂正は同ターンで本文に打つ。** 訂正表だけでは足りない — **被訂正文は除去するか、その行に ⚠ マーカを打つ**。追記方式は複製文書で機能しない(正本は git 追跡下・被訂正文の残置は `git diff` で即検出)。
> **(R-2) 機械出力の接頭辞を分離する。** `gate:` は**述語を実際に評価した**場合のみ。文書内の文字列存在検査は `doc-keyword:`。

> ### v1.3 の変更(falsifier 第 2 便・**すべて本文に直接打った**)
> | # | 箇所 | 内容 |
> |---|---|---|
> | **Y-1** | **§7-4** | 「(P-ISO) は UNKNOWN・MEAS 全体がこの前件に載る」を**置換** ⟹ (P-ISO) は定理(Prop 3.14 直後 + Prop 3.15 工房証明[falsifier 条件付き PASS])。MEAS が載るのは **(C-2)** の方で、それも COFIN-1/2 で閉じた |
> | **Y-2** | **§6 末・§7 債務** | 「優先度は $K_3$ より (1) の証明が上」「最優先: MEAS-N2 (1)(2) とも UNKNOWN」を**置換** ⟹ (1) は閉じた・残るは (2) のみ・**落下側で $K_3$ に特権なし**(DROP-FREE) |
> | **Y-3** | **§2** | $M\le N\le K$ の $M$ が最細で §3 以降の $M$(972 窓・最粗)と**逆向き二役** ⟹ **$K''\le K'\le K$** へ改名。$M$ は 972 窓に予約 |
> | **Y-4** | **§6 台帳 D-5** | $q_1=1$ の誤付与を**撤回**($q_j$ は M6 で**入れ子 cofinal 梯子上にのみ**定義・非入れ子 5 窓には付かない) |
> | **Y-5** | **§6 台帳 D-11** | **48 の二重意味を機械で決着**(`gate:` MATCH)⟹ ⚠ を「raw 対空間の繊維(探索空間と一致)」へ**緩和**。「密度と読むな」は不変 |
> **v1.2(sha16 `8d6805bdf7aad11d`)・v1.1(`89073d266188e063`)・v1(`22cd90de6efd82df`)は不変のまま並置。**
...
> ⚠ **v1.3 改名(記号衝突の解消)**: v1.2 はここで $M\le N\le K$ と書き **$M$ を最細の窓**に使っていたが、**§3 以降の $M$ は 972 窓(最粗)**であり、同じ文字が逆向きの二役を担っていた。**以後この節では $K''\le K'\le K$**($K''$ が最細、$K$ が最粗)を使う。**$M$ は 972 窓専用に予約する。**

- (2)〔DICHOT (2)〕(**source-first**・$K''\le K'\le K$)各非空繊維は $\ker R_{K',K}$ の剰余類 ⟹ $F_{K''}\to F_{K'}$ の繊維は $\ker R_{K'',K'}$ の剰余類で**サイズ一定**($R_{K'',K}=R_{K',K}\circ R_{K'',K'}$ より $\ker R_{K'',K'}\subseteq\ker R_{K'',K}$)。
- (S)〔段全射〕$F_{K''}\to F_{K'}$ が全射。$R_{K'',K'}$ 全射ならこれは従う。
...
| **D-5** | `OBS-UNIF-1` 5 窓(**非入れ子**) | 繊維 2,2,2,3,9 | — | **$q$ は付かない**(⚠ v1/v1.1/v1.2 の「$q_1=1$」は**誤付与・撤回** — $q_j$ は M6 で**入れ子 cofinal 梯子上にのみ**定義され、非入れ子 5 窓には定義されない) | M3 前件 (S)(段全射性の観測) | — | — | **cross-checked(観測のみ・昇格禁止)** | `sol_reply_159_iv.md` §15.1 |
...
> ⚠ **D-11 の $2/48$ を密度と読むのは誤り**(D-6 と同型の罠)。**測度側の数は fibre $=2$**(valid lift の個数)。$2/48=1/24$ は探索効率であって $\nu_g$ ではない。
> ⚠ **v1.3 緩和(機械決着)**: v1.1/v1.2 は「48 は**実装の**探索空間」と書いたが、これは**過小**だった。`cofin_cert_draft_v1_2.md` §9.2 の `gate:` により
> $$\#\mathrm{fib}(K_2)=\frac{K_{2,\rm ord}}{M_{\rm ord}}\cdot[M_{F_2}:K_{2,F_2}]=\frac{36}{18}\cdot(8\cdot3)=2\cdot24=\mathbf{48}\quad(\text{observed }48\ \Rightarrow\ \text{MATCH})$$
> ⟹ **48 は窓データから決まる正準な raw 候補繊維**であり、実装の探索空間と**一致している**(実装の恣意ではない)。⟹ 表現を「**raw 対空間の繊維(探索空間と一致)**」へ緩和。**「密度と読むな」は不変。**
...
> **★ 三位一体との接続(重要)**: `RUNG-UNIF` の閉形式は「一様生存定理」の材料だが、**非 cofinal な塔の上ではそれが成立しても何も証明しない**(三位一体)。
> ### ⚠ v1.3 訂正 — 優先度指定は撤回
> v1/v1.1/v1.2 のこの位置には「**現在のボトルネックは (1) の cofinality 証明書である。優先度は $K_3$ の測定より (1) の証明が上**」と書いてあった。**(1) は `cofin_cert_draft_v1_2.md` COFIN-1/COFIN-2 で閉じたので、この優先度指定は撤回する。**
> **現在のボトルネックは (2) 一様生存定理のみ。** また **落下側では $K_3$ に特権はない**(`cofin_cert` §6.2 DROP-FREE)— $K_3$ の残る価値は $\prod q_i$ の簿記(証人側)である。⚠ さらに **MONO は包含鎖にしか効かない**(`cofin_cert` §6.3 W-2)ので、「深い窓ほど落ちやすい」は**比較可能な窓の間でのみ**成り立つ。**資源配分は決定しない**(トレードオフ表は `cofin_cert` §9.2・決定は研究者との選定会議)。
...
4. **(P-ISO) は定理**(⚠ v1/v1.1/v1.2 §7-4 の「UNKNOWN・MEAS 全体がこの前件に載る」は**誤り — 本行で置換**)。典拠は正典 **Prop 3.14 直後の段落**(cofinality)+ **Prop 3.15**(有向性 = 有限交叉閉性;原文に証明が無いため**工房証明**を `cofin_cert_draft_v1_2.md` §4.5 に収録・**falsifier 第 2 便で条件付き PASS**、Sol 監査は未了)。⟹ **Haar(M1–M3)は無条件に立つ。MEAS が載っているのは (P-ISO) ではなく (C-2)(具体的な鎖の cofinality)の方**で、それも `cofin_cert` COFIN-1/COFIN-2 で閉じた。**残る前件は (C-3)(一様生存)のみ。**
...
### 未決・債務(推測で埋めていない)
- ★ **最優先**: 【MEAS-N2】**(1) cofinality 証明書は閉じた**(`cofin_cert_draft_v1_2.md` COFIN-1/COFIN-2)⟹ **残るは (2) 一様生存定理のみ — UNKNOWN**。(⚠ v1/v1.1/v1.2 のこの行は「(1)(2) とも UNKNOWN」と書いていた — **本行で置換**。)正枝への唯一の経路であることは不変で、段数を積んでも代替できない。**(P-ISO) はここに含まれない**(定理・§7-4)。なお $K_2^\diamond=K_2$ は 1 事例の測定であって全称ではない — それは COFIN-1 の (F2)(各段 isolated)を**実装が満たしているかの検査事項**であって、前件の未証明ではない。
...
| §2 前件 (2)(S) | $\ker R_{K,N}$ / $R_{N,M}$ 全射 | $\ker R_{K',K}$ / $R_{K'',K'}$ 全射($K''\le K'\le K$・**v1.3 の Y-3 で $M$ から改名**) | 添字順 |
