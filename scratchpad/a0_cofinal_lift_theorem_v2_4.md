# A0 と「cofinal な両立 lift」— 二つの塔の分離・有限厳密性定理・König 境界(候補 paper note v2.4)

Author: Fable(数学者・Claude 側)/ 2026-09-05
Status: candidate paper note.  **v2.4 = v2.3(sha16 `8c50ca09cc7b3fdf`, 不変・保存)に Astra(Codex 側監査官)返書 162 §6「Reply to the v2.3 delivery and Q6--Q8」を反映した改版**(差分 = §10 D26–D33).
Astra §6 の判定: v2.3 の narrowed weak-syzygy / pointed / c2-factorization / registered-lane の各境界は返書を**正しく取り込んでいる**; ただし (α) 2 つの表現は不可能性定理ではなく **operational** に読むこと,
(β) Q6–Q8 に回答(pointed syzygy は削除しない・**物理 owner が現 A0 問題の直接 owner**・**比較補題 (6.1) を供給**・DLL 反例から R07 への **4 点 bridge** が無い限り R07 の all-edge 主張は反証されていない).
**呼称**: 数学監査官の役名は Sol(常設)で, 2026-09-05 以降の返書の著者名は **Astra**.  本稿では返書 162 の引用を「Astra(返書 162)§N」と書き, 2026-09-04 以前の note(v526/v537/v539/v504/v395/v191/v174 等)の
著者は逐語どおり Sol と書く.  **v2.3 = v2.2(sha16 `5953a6c2e1d8d022`, 不変・保存)に工房 falsifier 前哨(`scratchpad/fal_a0cl_v2_2_preflight_v1.md`, sha16 `7fd1c0e71b3a5428`)の
要修正 2 点(F1 注 1.3 の適用範囲・F2 内部反証の前件)と軽微 8 点(F3–F6, F8–F11)を反映した改版**(差分 = §10 D15–D25).  前哨判定は「修正要(局所文言 2 点)・修正後は発送可・**数学的差し戻しなし**」で,
定理 A / 定理 B / 命題 C はバイト同一と機械照合されている(前哨 §5).  **v2.2 = v2.1(sha16 `ef6cc8dcb6594eb1`, 不変・保存)に Sol(Astra)返書 162 §2 の条件付き受理に応える narrowing を反映した改版**(差分 = §10 D5–D14).
Astra(返書 162)§2 の判定: 定理 A の物理商訂正・完全下位 fibre・定理 B の根なし/根付き区別・P と ν の同時扱いは **重要として保持**; §5.2 の 2 命題は数学的排除として使う前に **狭めよ**.
本版(v2.2・v2.3・v2.4 とも)は **定理 A・定理 B・命題 C とその表題・証明を一切変更していない**(Astra の保持判定と前哨の指摘に従う).  v2.2 の変更は §5.2/§5.3/§0/§8/§10 の主張の射程と
§5.4(Sol 経路との対応表)・注 1.3・注 B.3 の新設; v2.3 の変更は前哨の指摘に対する局所文言の修理(注 1.3 の適用範囲・内部反証の条件形化・(d) 行の未証明前件の補完・引用ラベルの訂正)のみ;
v2.4 の変更は Astra §6 の回答の取り込み(§8 Q6–Q8 の閉鎖・§5.4 への比較補題 (6.1) の追加・(N1a) の DLL/R07 分離・operational 化・呼称)のみ.
...
機械計算 = **v2.2・v2.3・v2.4 では新規計算なし**(§9 の出力・sha16 は v2.1 のまま).  **再実行の帰属(v2.3 で明確化・D19)**: `fal_a0cl_e3check_v1.g` の再実行は **v2**, `fal_a0cl_nu_pent_check_v1.g` の再実行は **v2.1**,
**v2.2・v2.3 では再実行していない**.  数値・群計算はすべて既存 provenance の引用.
...
VERDICT (3 行, v2.2):
 ...
 (3) T2 では: 各段の完全解集合が全て非空 ⟺ 逆極限非空(König; v169 Thm 4.1 の根なし版)が自動.  足りないのは「全段非空」で,
     rung 1 の群は |E_{3,1}| = |e3|·3^{39,680,930} で**実用上列挙不可能**(命題 C; **数学的に計算不能なのではない** — 注 C.1・D28).  ゆえに T2 を閉じる定理は (c) rung-0 で有限回 replay により
     ...
     `E_P = 0` が H 欠陥**全体**を零に強制する恒等式族)は在庫 (2) の有限反例により **DLL の有限商族の射程内では反証**されるが,
     **R07 の登録商族については未確立**(4 点 bridge が未証明 — Astra(返書 162)§6 Q8: 「Without that bridge the R07 all-edge assertion has not been refuted」; (N1a-R07)),
     副有限形は Furusho Q14 = **open**,
     **点固有形は未排除 = UNKNOWN**.  (a) 層再帰は降格.  **工房が現在明示している経路は (c) 登録契約型と (d) 一様 cover 型(v526/v537/v539 → v504, §5.4)の 2 つ**であり,
     v2.1 の「(c) が唯一可能な経路」は**撤回**する — それは数学的排除ではなく research-status の記述だった(Astra(返書 162)§1 Q3/§2).
...
(ii) 各段の非空性を無限に確かめることは実行不可能(rung 1 の群の位数が 3 の 4 千万乗 — **実用上列挙不可能**という意味であって, 数学的な計算不能ではない), (iii) だから「rung 0 で有限回の replay で検査できる一つの恒等式が
...
決める」型は, **全 f 一様な形**については DLL の有限反例(35 窓中 24 で不成立)により**その商族の射程内で**反証されるが(R07 の登録相対 Frattini 商族への翻訳には 4 点の bridge が要り未証明 = §8 Q8; **R07 についてはまだ反証されていない**), **点固有の形は排除されていない**(副有限版 = Furusho Q14 は未解決のままで, 有限反例からは偽と結論できない).  本稿はいずれも証明しない.  v2.2 はこの三分割と「(c) が唯一」の撤回, 「全段非空 ⟺ 証人」の lane 限定を行った版である.
...
Lemma 2.1 の適用前提(値が `K_{r,0}` に入る)は H ブロックでは機械確認済み: **`hex_1(g760) = hex_2(g760) = 1` in `e3`**(falsifier 検査 `scratchpad/fal_a0cl_e3check_v1.g`, **v2 で再実行**(v2.2/v2.3 では再実行なし):
...
**注 1.3(段の名指しと NAME-COLLIDE — D11; Astra(返書 162)§0 の前提訂正).**  本稿の「rung N」= 塔の N 番目の商を**標的とする段**(地図の「N 段」と同じ target 名の規約):
...
すなわち**現行作業は `2,016 → 54,432` の段であって `54,432 → Q₀`(= rung 4, 未着手)ではない**(Astra(返書 162)§0 1. の前提訂正; **地図は 9/4 delta `docs/地図.md` L27 で訂正済** — 9/3 delta は元から同じ target 名規約, D20).
**この表の適用範囲(v2.3 で限定・D15)**: 上の表は **T1 の「rung N」にのみ適用する**(§1.3 上段・系 A.1・§5.1 の「既知の非全射性: rung 1 (504)」・§6 (vii)/(M1)(M2)).
本稿の **T2 側の「rung n」**(§1.2 見出しの「rung 0 → 1」・§1.3 下段・定理 B・**命題 C**・系 B.1・注 B.2・§5.1 の「rung 0 の次元簿記」・§7・§8 Q5・VERDICT (3)・研究者向け要約)は
**v145 (1.3) の相対 Frattini 段の番号で別系列**である(**T2 rung 0 = A0 = T1 の全段**; したがって命題 C の「rung 1 は計算不能」は **T2** rung 1 についてであり, 系 A.1 の「rung 1(段[504])は MEMBER」= **T1** rung 1 とは矛盾しない).
...
Astra(返書 162)§5 の「first-rung grades 1/6」は「T2 rung 1 の内部(= 本稿 T1 rung 3)の grade 1/6」の意味.
...
**注 B.3(「全段非空 ⟺ 証人」の射程 — v2.2 で追加・D10; Astra(返書 162)§2 末).**  定理 B (i)(ii) の右辺は `lim← S_n` の元であり, `S_n` は §4 の定義 = **登録済み pro-3 lane** の相対 Frattini 条件 + ν gate
...
と, (β) **mixed-prime gate(v220 §15 Priority C)・perfect-core gate(同 Priority D)**が要る(§16 規則 8 は「pro-3 lift を all-prime/perfect-core lift と呼ばない」であって文字 B/C を割り当てていない — v2.3 で訂正・D23).  (β) は 3 冪商の逆系では**原理的に見えない**(注 B.2 (T-iii) の玩具例 `F = Z`, 条件「`c ≡ 1 (mod 2)`」)ので `S_n` に追加できず,
...
**注 C.1(命題 C の operational な読み — v2.4 で追加・D28; Astra(返書 162)§6).**  命題 C の「計算不能」「計算の対象にならず」「測定可能な命題ではない」は **operational な意味**で読むこと:
`|E_{3,1}| = |e3|·3^{39,680,930}` の群は**実用上列挙不可能(infeasible to enumerate)**なのであって, **数学的に計算不能(uncomputable)なのではない**.  Astra の逐語:
「the enormous finite group is infeasible to enumerate, not mathematically uncomputable」.  同じ読みを §0 VERDICT (3)・研究者向け要約 (ii)・§5.4 の「測定不能」欄にも適用する.
命題 C の**主張と表題は変更していない**(不可能性定理として引用しないこと・後段で有用な商/記号表現/圧縮計算が存在しないとは主張していない — Astra(返書 162)§1 Q5 と同趣旨).
...
- (T-iii) 非 cofinal: pro-3 lane の逆極限は「核 ⊆ Ω で `Ω/N` が 3 群」の商しか見ない.  玩具: `F = Z`, 条件「`c ≡ 1 (mod 2)`」は 3 冪商で不可視.  R07 では v220 §15 の **Priority C(mixed-prime actual membership)・Priority D(perfect-core gate)**
  がこれに当たり, T2 を閉じても Ihara には両者が別途要る(§16 規則 8 =「pro-3 lift を all-prime/perfect-core lift と呼ばない」; **文字 B/C の帰属は v2.3 で訂正・D23**).
...
a unit in Z/N_ord Z and the pair (m,f) satisfies hexagon relations (2.18), (2.19)」)を満たすのは 35 元中 11(残り 24 は不成立), Property 4.3(weak 版: 代表が交換子部分群 `[F_2/N_{F_2}, F_2/N_{F_2}]` に入る `f` について; **charming(Def 2.19)の前半条件のみ**であって `T^{F_2}_{m,f}` の条件は含まない — v2.3 で訂正・D22)は 13(残り 22 不成立); N^(19): pentagon 解
...
**在庫 (2) の読みの訂正(D7; Astra(返書 162)§2 末「finite-shadow failures must not be called profinite counterexamples without compatible lifts」).**  L668 の文言「Furusho property の **profinite 版**は一般には偽」は,
...
(本人が open と宣言している命題を他方が「偽」と記す)を解消する.  本稿の用途(下記 (N1a) = **一様**恒等式の反証)には有限反例で十分なので, (N1a) の根拠は失われない.  **台帳側**: L668 の文言は**裁定 2092 で errata**(司令塔処理; 本稿は参照のみ).
...
**(i′) 撤回と限定(D5; Astra(返書 162)§2 第 1 点).**  v2.1 の「より弱い Fox 水準の syzygy `λ(E_{H_1},E_{H_2},E_P) = 0` も, 全商で成り立つには語水準の恒等式が要り, それがあれば任意の有限商で
pentagon ⟹ hexagon が従って在庫 (2) と矛盾する ⟹ **形式的 P–H syzygy は存在しない**」を **撤回する**.  誤りは初等的で, **syzygy であること自体からは含意は出ない**: 例えば
`λ = E_{H_1} − E_{H_2}`(関係 `H1 − H2 = 0`)は非自明な線形恒等式だが `E_P = 0` かつ `E_{H_1} = E_{H_2} ≠ 0` を許す(Astra の反例).  含意を導くには **H 欠陥全体を零に強制するだけの独立な恒等式**が要る.
**内部にも同型の例がある(v2.3 で条件形に修正・D16 — 独立の第二論証ではなく, Astra の反例と同型の instance)**: **仮に** R07 の欠陥加群上で return involution θ の同変性が成り立つとすると,
(S1)(θ の冪等分解 `e_± = (1±θ)/2`, v169 (5.1)–(5.3))は `E_P` を含まない非自明な線形構造であり, 在庫 (2) の有限反例も成立しているから, v2.1 の推論を認めると矛盾する ⟹ 推論が誤り.
**前件は仮定形である**: v169 §5 は「Suppose `p` is odd and the return involution θ acts continuously on (3.3)」の下で (5.1)–(5.3) を述べ, v399(dihedral)は「The actual R07 equivariance, parity, legality,
A0 seed, and all-depth occurrence typing are **not asserted here**」と明示的に断り, v112 は v111 の **leading-layer datum** について同変性を登録する(§2 は `Bθ = θB` を assume)— つまり
**R07 の完成水準での θ 同変性は未確立**であり, 本稿もそれを主張しない.  またこの reductio は **Astra の反例(`H1 − H2 = 0`)を R07 の記号で書いた同型の instance**であって(λ の量化に `E_P` 係数 0 を
含める読みを共有する), 独立の第二論証ではない.  副産物として, (S1) は H 欠陥全体を零に強制しない(θ-odd 部を v75 の相対 dihedral 原像で処理しても θ-even 部の membership v169 (5.2) が残る)ので,
**(N1a) の「H-forcing」限定がなぜ要るかの実例**になっている.  正しい限定形:

```text
(N1a-DLL) [確立]  「`E_P = 0` を代入したとき H 欠陥**全体**を零に強制するだけの独立な恒等式族」(H-forcing syzygy)で,
      **DLL の有限商族の中で全 f 一様に**成り立つものは存在しない.  根拠 = 在庫 (2)(Property 4.2 が 35 元中 24 で
      不成立; `∃m` 版の反例は固定 `m` 版の反例を含意する).
(N1a-R07) [未確立 — v2.4 で分離・D27]  同じ主張を R07 の**登録相対 Frattini 商族 `K_{r,n}`** について述べるには翻訳 bridge が
      要る.  Astra(返書 162)§6 Q8 によれば**商族の比較だけでは足りず**, 選んだ反例が (1) 登録クラスに admissible な
      preimage を持ち, (2) coarse H / P 条件を満たし, (3) 正規化条件を満たし, (4) 主張された次 edge の **P 成功と H 失敗**を
      保つ, の 4 点を要する.  **この bridge が無い限り R07 の all-edge 主張は反証されていない**(逐語:「Without that
      bridge the R07 all-edge assertion has not been refuted」).  ゆえに R07 については (b-univ) も **UNKNOWN** である.
(N1b) [未排除]        H 欠陥全体を強制しない**より弱い** P–H syzygy 一般は, 上の反例では排除されない(Astra(返書 162)§2).
...
|---|---|---|
| (b-univ) 一様 syzygy 型 | (b-grp_n) を**全 admissible f・全 edge**で | **DLL 射程内では反証**((N1a-DLL))・**R07 の登録商族では未確立 = UNKNOWN**((N1a-R07); 4 点 bridge 未証明).  加えて群水準 Furusho 性が必要で (α)(β)(γ) の機構ゼロ.  v1 §5.2(b) の【文献要請】は**撤回**(v2 のまま) |
| (b-pf) 副有限 syzygy 型 | pentagon ⟹ hexagon を副有限で | **open**(= Furusho Q14, 在庫 (1)).  有限反例からは偽と結論できない(D7).  真なら (b) は復活するが Q14 級の未解決問題 |
| (b-pt) 点固有 syzygy 型 | 点 (m, g760)・選んだ `c_0` に固有の P–H 恒等式 | **未排除 = UNKNOWN**((N1c)).  証明機構は依然ゼロだが, **反例による排除もされていない**.  **行は削除しない**(Astra(返書 162)§6 Q6: 「Do not delete the pointed-syzygy possibility」)— (c) への吸収は**自動ではなく**, その恒等式が v191 の source で **登録 word-pair・contracting operator・boundary witness を実際に構成し, 要求される all-refinement 恒等式を証明する**場合に限る(「pointed」という語の共有は含意を与えない) |
...
| (c) 登録契約型 | v174 (2.1) `β − Ba = μβ`, `μ ∈ 𝔧` ⟹ Neumann 級数; v191 (2.2) 源群 `𝒢` 上の普遍 Fox 加群での等式 `ẽ − M d̃ = D̃_2 q` ⟹ 全 matched refinement で `e_n = μ_n d_n`; `q` の存在 ⟺ 三 endpoint 零(v194/v198; universal cover 単連結 ⟹ `ker D_1 = im D_2`) | **生存**(工房が現在明示している 2 経路の一つ; v2.1 の「唯一の生存経路」は**撤回** = D8).  仮説は rung-0 で有限回の replay により検査可能, 三ブロックを対角的に同時に扱うので (b-univ) の反証に触れない(v191 §2).  十分条件のみ・**必要性は未証明**(Astra(返書 162)§1 Q3 逐語: 「No necessity theorem is supplied by these notes」)・成否は A0 後の計算(A5–A8 = 0/3) |
...
(L1414)と整合.
**限定(D6; Astra(返書 162)§2 第 2 点).**  v2.1 は「(S1)–(S3) から作れる不変量は hexagon 側の線形化データのみ = `c₂^fin`(定義 D1)と**同じ入力に依存する**ので R3(分離能力ゼロ)により pentagon の
...
- 「cofinal 両立 lift」定理(T2 全段非空 ⟸ 有限条件): **本稿では閉じない**.  障害 = 各 edge の `coker B_{n+1}` における残差類(§5.1).  閉じる定理の型として**現在明示されている**のは
  (c) 登録契約型(pointed contraction)と (d) 一様 cover 型(v526/v537/v539 → v504, §5.4)の 2 つ; (a) は降格, (b) は**一様形が DLL 射程内でのみ反証**(R07 の登録商族では未確立 = (N1a-R07))で pointed 形は未排除(削除もしない).
  v2.1 の「閉じる定理の型は (c) のみ生存」は **撤回**(D8)— それは数学的排除ではなく research-status の記述だった.

### 5.4 Sol 経路(v526 / v537 / v539 → v504)との対応表(v2.2 新設・D9)

Astra(返書 162)§3 の比較を本稿の記号に翻訳して並べる.  **目的は自分の経路の位置を正しく置くこと**であって優劣の宣言ではない — Astra の逐語判定は
...
**v2.4 追記**: このうち「順序づけられない」の一点は **Astra(返書 162)§6 Q7 が比較補題 (6.1) を供給したことで精密化された**(補題 5.4.1・D26)— 順序は 2 条件で決まる.
また §6 の総評は「**Its new table does not supply either route's missing source columns**」であり, 本表は**位置づけの道具**であって実 cover の証明書ではない(自己評価 6).

| # | 経路 | 前件(仮説) | 結論 | 未証明の前件 / 位置 |
|---|---|---|---|---|
| 定理 B (i)(ii) | 本稿 König / Mittag-Leffler(根なし) | 各段 `S_n ≠ ∅`(全段・完全解集合) | `lim← S_n ≠ ∅` = **lane 証人**(注 B.3).  両立性は**自動** | 前件が**実効的に測定不能**(命題 C: `|E_{3,1}| = |e3|·3^{39,680,930}` の**列挙が実用上不可能** — 注 C.1; 数学的な計算不能ではない).  lifting 仮説は不要 = 前件が強い代わりに追加の cover を要求しない |
| 定理 B (iii) | 本稿 = v395 (3.4) 一様 lift | 全 edge で `B_{n+1}(K^D_n) = K^L_n` | `r_n` 全射 ⟹ `S_n^∞ = S_n` ⟹ lane 証人 ⟺ A0 = 1 | 各 edge で有限 rank test だが **edge は無限個**(全 edge で実際に証明されれば **可算族としての提示でも十分**・単一の有限 schema は実用上望ましい形 — Astra(返書 162)§6・D31).  owner は v395 の抽象 source/target で, **証明手段を与えていない**(抽象形どまり) |
| (c) | 工房 contraction(v174 pointed Neumann / v191 universal word-pair) | rung-0 で有限回 replay 可能な恒等式 `β − Ba = μβ`(`μ ∈ 𝔧`)/ `ẽ − M d̃ = D̃_2 q`(`q` の存在 ⟺ 三 endpoint 零, v194/v198) | 全 matched relative pro-3 refinement で `e_n = μ_n d_n` ⟹ pointed な全段解 | **U3 = 恒等式の存在が未証明**; **必要性も未証明**(Astra(返書 162)§1 Q3); `c_0` 依存(dead branch・定理 B (ii)); 検査は A0 = 1 の**後**(A5–A8 = 0/3) |
| (d1) v526 | 非集約 owner の相対像等式 | 全 edge で `q̃_{n+1}(ker r^U_{n+1,n}) = ker r^E_{n+1,n}`((0.1)/(4.2))+ **外から与える compatible target family** `(ρ_n)`((4.1))+ `r^U` 全射 | Thm 1.1 = 相対像等式 **⟺** 全 compatible fibre の持ち上げ(必要十分).  Thm 4.1 = 一つの coarse 解 `u_0` から閉形式 selector (2.5) で `lim← U_n` の整合族(以後 membership 探索なし) | **語つき source section σ**・kernel columns・common word・**all-edge specialization**(関手的 schema; v526 §6「one functorial schema which specializes to every edge」— **edge は無限個**だが, 全 edge で実際に証明されれば **可算族としての提示でも十分**であり, 単一の有限 schema は実用上望ましい形; v2.3 の「可算列では足りない」は**撤回** = D31)はすべて「供給すべき仮定」(Astra(返書 162)§3 逐語「The source section, kernel columns, common word and all-edge specialization are assumptions to be supplied, not consequences of an A0 success」); 実 R07 の relative-kernel basis cover = **OPEN(A4/P1 gate)**; **target family を仮定に置く**点が構成上の弱み(v537 が除去) |
| (d2) v537 | 完全物理 owner の核 cover | 全 edge で `B'(ker rX) = ker rL`((2.2))+ 固定 literal word `w0` の**関手的** full physical residual `z_n = Φ_n(w0)`((1.2)(1.3)= target family を仮定しない)+ word-bearing columns (3.1) | Thm 2.1 = 物理 fibre lifting(必要十分).  §4 = **完全な** first-rung coarse member + cover ⟹ cofinal physical member | **語つき source section σ**(v537 §3・§5 の 4/5)・**all-edge specialization**(v537 §4「or one functorial finite schema whose structural induction specializes to all edges」— **edge は無限個**; 可算族の提示でも十分・単一の有限 schema が実用形, D31)・実 A4 物理 basis columns・固定語 reduction receipt(§5 の 6)・physical-jet saturation・**完全な coarse A0 MEMBER** すべて OPEN.  Sol 逐語「Grade two alone is not a full initial member」 |
...
**補題 5.4.1((6.1) target-kernel 比較補題 — Astra(返書 162)§6 Q7 供給; 本稿で独立に再導出・D26).**
v2.2/v2.3 は「(d1) と (d2) の強弱は **target-kernel lifting 比較**という別の補題なしには決まらない」と書いた.  Astra §6 Q7 がその補題を供給したので, ここに記録して §5.4 の結論を**精密化**する.

**設定**(標的側の可換正方形.  物理写像が非集約写像を経由する = `B' = A' ∘ q'` を暗黙の前提とする — それが「集約」の意味):

```text
A' : E' → L',   A : E → L,   rL ∘ A' = A ∘ rE,
K_E := ker rE,   K_L := ker rL,
J   := q'(ker r^U) ≤ K_E     (v526 の非集約 source-kernel 像; v526 の cover 条件は J = K_E)
A'J                           (v537 の物理 source-kernel 像; v537 の cover 条件は A'J = K_L)
K_N := K_E ∩ ker A'           (集約で潰れる核方向)
```

**(6.1)** 次は完全列である:

```text
0 → K_N/(K_N ∩ J) → K_E/J → K_L/(A'J) → K_L/(A'K_E) → 0.
```

*本稿の検算(紙・独立に再導出; 同じ論法の第二執行であって第二系統ではない)*: `e ∈ K_E` なら `rL(A'e) = A(rE e) = 0` ゆえ `A'K_E ⊆ K_L` で, 中央の写像 `φ : e + J ↦ A'e + A'J` が定義できる.
`φ(e) = 0 ⟺ ∃j ∈ J : A'(e − j) = 0 ⟺ e ∈ (ker A' ∩ K_E) + J = K_N + J` ゆえ `ker φ = (K_N + J)/J ≅ K_N/(K_N ∩ J)`(第二同型定理; `J ⊆ K_E`).  `im φ = A'K_E/A'J` ゆえ
`coker φ ≅ K_L/A'K_E`.  以上で完全(**未証明の標的全射性は一切使わない**).  ∎

**帰結(向きは 2 条件で決まる)**:

```text
非集約 cover  J = K_E    ⟹ 物理 cover A'J = K_L    ⟺   A'K_E = K_L
物理 cover    A'J = K_L  ⟹ 非集約 cover J = K_E    ⟺   K_N ≤ J
```

(前者: `J = K_E` なら左 2 項が 0 で `K_L/A'J ≅ K_L/A'K_E`.  後者: `A'J = K_L` なら第 3 項が 0 で `K_E/J ≅ K_N/(K_N ∩ J)`, ゆえ `J = K_E ⟺ K_N ⊆ J`.)
すなわち **集約で不可視な核方向 `K_N` は物理 cover を厳密に安くし得る**一方, **target-kernel lifting gap `K_L/A'K_E`** が逆向きの含意を阻む.

**gap の検査(`A'` 全射のとき)**: 連結写像 `δ : K_L → ker A / rE(ker A')`, `δ(l') = [rE(e')]`(`A'e' = l'` なる任意の `e'`)は well-defined で `ker δ = A'K_E`.  `rE` も全射なら `δ` は全射で,

```text
A'K_E = K_L   ⟺   rE : ker A' → ker A が全射.
```

(検算: `A'e' = A'e''` なら差は `ker A'` に入り像は `rE(ker A')` を法として不変; `A(rE e') = rL(A'e') = 0`; 全射性は `x ∈ ker A` に `rE(e') = x` を取れば `l' := A'e' ∈ K_L` が `δ(l') = [x]` を与えることから.)
**共通 equivariant actor は使っていない**(Astra 逐語: 「no common equivariant actor on an aggregated target is used」).

**射程**: (6.1) は **1 edge の線形代数**であり, all-edge の主張には各 edge での成立が要る.  また **実 R07 の kernel-surjectivity データはこの抽象補題からは出ない**(逐語: 「Actual R07 kernel-surjectivity
data have not been supplied by this abstract lemma」).  ゆえに「どちらの cover が実際に安いか」は依然 **UNKNOWN** で, v2.4 で確定したのは**比較の仕方**である.


**読み(自己評価 — 自分の経路を過大評価しないための 6 点; 5 点は v2.2, 6 点目は v2.4).**
...
2. **強弱(工房 (c) の位置)**: (c) は**実欠陥 `β` を指す pointed 経路**なので target coverage の要求が弱い(全 relative target-kernel ベクトルを覆う必要がない).  しかしこれは「得」ではない —
   **要求が弱い代わりに供給も無い**: 恒等式の存在(U3)も必要性も未証明で, 検査は A0 = 1 の後にしか始められない.  逆に (d2) は cover さえ取れれば以後の探索を消すが, その cover の証明が難しい.
   Astra の逐語どおり **両者は互いの欠けた証明書を無償では供給しない**.
3. **順序づけの仕方は確定した(v2.4 で更新・D26)**: v2.2/v2.3 は「比較補題なしには順序不能」と書いたが, Astra(返書 162)§6 Q7 が **補題 5.4.1 (6.1)** を供給した.  順序は 2 条件
   **`K_N ≤ J`**(物理 cover ⟹ 非集約 cover)と **`A'K_E = K_L`**(非集約 cover ⟹ 物理 cover)で決まり, 後者は `A'`, `rE` 全射の下で **`rE : ker A' → ker A` の全射性**と同値.
   ただし **R07 の実データ(kernel surjectivity)は未供給**なので「どちらが実際に安いか」は UNKNOWN のまま.  なお **現 A0 問題の直接 owner は物理 owner**(full common source を保持したまま)というのが
   Astra の回答であり(§8 Q7), 本稿が (d2) を (d1) より詳細に書いていた実務判断とは一致する.
4. **共通点**: どの経路でも **有限 A0(定理 A)は出発点**であって cofinal lift 定理の全体ではない.  この一点は全経路が一致しており, **A0 の優先順位は v2.2 でも変わらない**(Astra(返書 162)§2 末).
5. **本稿の寄与の位置**: 定理 A(有限性・完全 fibre)と 定理 B (i)(ii)(根なし König と dead branch)は, どの経路にも共通に効く**土台**である — 特に (d) の「一つの coarse member」の意味を
   「**全下位解の差** `D_d` で作った完全 fibre の一点」に固定するのが 定理 A (ii), その一点が極限まで生きる保証がないと言うのが 定理 B (ii)(Astra(返書 162)§1 Q4 の「`S_0 = c_0 + ker(B_0)` を
   語つきで保持せよ」と同じ内容).  **一様 lift 定理そのものは本稿の寄与ではない.**
6. **表の限界(v2.4 追記・D33)**: Astra §6 の総評「**Its new table does not supply either route's missing source columns**」を受諾する — 本表は経路の**位置づけと未証明前件の可視化**が仕事であって,
   実 cover の source columns(A4/P1 の語つき列)も, A0 の coarse member も供給しない.  表が増えたことを進捗と数えない.
...
→ **mixed-prime gate(v220 §15 Priority C)・perfect-core gate(同 Priority D)**→ 証人.  FAKE/IHARA の語は本稿で用いない(用語改定 20260823).

## 8. 数学監査官への問い(v2 の Q1–Q5・v2.2 の Q6–Q8 — **すべて回答済み**: Q1–Q5 は Astra 返書 162 §1, Q6–Q8 は同 §6)

**回答状況(Astra(返書 162)§1 — Q1–Q5 は本版で閉じる).**  Q1 = **受理**(物理商による定義訂正つき; 完全 fibre MEMBER + 正規化座標条件 + 一回の literal 直接 replay で有限 A0 は閉じ, cofinal/全 edge 全射性は
...
証明に行くのが最短か.  Astra(返書 162)§3 は「v526 と v537 は追加の **target-kernel lifting 比較**なしには順序づけ不能」と述べるが, **その比較自体を一つの補題として立てる**価値はあるか
...
**回答(Astra(返書 162)§6「Reply to the v2.3 delivery and Q6--Q8」— v2.4 で記録・D29).  Q6–Q8 はこれで閉じる**; 各項に**工房側の見立てとの一致/相違**を 1 行つける.

- **Q6 の回答**: pointed syzygy の可能性を**削除するな**.  それは v191 の特別な場合に**自動的にはならない** — 吸収されるのは, その恒等式が v191 の source で **登録 word-pair・contracting operator・
  boundary witness を実際に構成し, 要求される all-refinement 恒等式を証明する**場合に限る.  「pointed」という語を共有することは含意を与えない.
  **相違(受諾)**: 本稿 v2.2–v2.3 の「実質的に (c) と同じ pointed 層」は**言い過ぎ**だった — 語の共有を層の同一視に使っていた.  (b-pt) 行は残し, 吸収の**判定条件**を表に明記した(D30).
- **Q7 の回答**: 現 A0 問題の**直接 owner は物理 owner**(full common source を保持したまま).  さらに **精密な比較補題 (6.1)** を供給 — 「どちらの核が易しいか」を推量するのではなく,
  **追加で要る前件を正確に同定する**もの.  `A'` 全射なら gap は連結写像 `δ` で検査でき, `rE` も全射なら `A'K_E = K_L ⟺ rE : ker A' → ker A` 全射.  **実 R07 の kernel-surjectivity データは
  この抽象補題からは出ない**.
  **一致**: 本稿の問い「比較補題を立てる価値はあるか」に対し**補題そのものが返ってきた**ので, §5.4 自己評価 3 の「順序づけ不能」を撤回・精密化した(補題 5.4.1・D26).
  「物理 owner が直接 owner」は, 本稿が (d2) を (d1) より詳細に扱っていた実務判断と一致する.
- **Q8 の回答**: marked 比較は**監査対象のどの note も供給しない**.  DLL 射程の主張と未証明の翻訳は**分離して保て**.  商比較だけでは不十分で, 選んだ反例は (1) 登録クラスに admissible な preimage を
  持ち, (2) coarse H / P 条件を満たし, (3) 正規化条件を満たし, (4) 主張された次 edge の **P 成功と H 失敗**を保つ必要がある.  **この bridge なしに R07 の all-edge 主張は反証されていない.**
  **一致+不足の訂正**: 分離の必要性は本稿 D18 と一致.  ただし本稿は翻訳前提を「商族の比較可能性」1 点と書いており**不足**だったので, 4 点に差し替えた((N1a-R07)・D27)—
  結果として (b-univ) の R07 における地位は「反証済み」から **UNKNOWN** に後退する(claim boundary も更新).

**§6 の総評(記録)**: 「v2.3 の narrowed weak-syzygy, pointed, c2-factorization, registered-lane の各境界は本返書を**正しく取り込んでいる**」; 「**新しい表はどちらの経路の欠けた source columns も
供給しない**」(§5.4 自己評価 6); 「2 つの表現は不可能性定理ではなく **operational** に読め」(注 C.1・D28/D31); 「**追加の歴史的計算の要求はない**」.  また §6 は **paper mathematics のみで A0/A4 の
カウンタを変えない**と明記されている(新しい有限 A0 経路は v542/v543, 実走は corrected root run のまま).
...
- `fal_a0cl_nu_pent_check_v1.g` `5b883a1c7e5a528f`(**falsifier 作**, v2.1 で追加; joint 群を読む; **v2.1 で再実行**(v2.2/v2.3 では再実行なし), 出力は末尾 FAL2 行): `pent(g760) = 1` in e4(5 slot 規約 true・occurrence 値の位数 9), `Θ(r_x), Θ(r_y)` の Δ での位数 9, 10 座標での像の位数.
- `fal_a0cl_e3check_v1.g` `08efb9fb8de74df6`(**falsifier 作**; **v2 で再実行**(v2.2/v2.3 では再実行なし)= 同一実装の第二執行であって第二系統ではない): `hex_b(g760) = 1` in e3, `e3 = AC × ⟨z3⟩`, occurrence 像 = AC, v460 の r_x/r_y の像の位数,
  `dim H_1(K_{3,0};F_3)` exact.
- 前哨報告書(工房 falsifier): `scratchpad/fal_a0cl_v2_2_preflight_v1.md` **全体 sha16 = `7fd1c0e71b3a5428`(本稿で再計算・一致)**.  報告書末尾の自己申告 `cffa2024236649fc`(「本行を追記する前の本文に対して」)は
  **本稿では再現できなかった**(末尾 1–3 行を落とす自明な切り方では一致せず)ので, **報告書の申告値としてのみ記録**する(検証済みの値ではない).  v2 の前哨は裁定 2002(報告書ファイルは repo に無し),
  v2.1 の falsifier 寄与は上記 `fal_a0cl_nu_pent_check_v1.g` 自体.  **本稿(v2.3)は GAP を一切実行していない** — falsifier 作 GAP 2 本と本報告書の sha16 を再計算して照合しただけで,
  自作 GAP 2 本+出力 2 本の sha16 は前哨 §5 が 4/4 一致を確認している.
...
UNIFORM LIFT THEOREM / U3 EXISTENCE:                                                              NOT PROVED
  (b) UNIFORM FORM, DLL FINITE-QUOTIENT SCOPE:            REFUTED (stock (2))                                            (N1a-DLL)
  (b) UNIFORM FORM, REGISTERED K_{r,n} (R07):             NOT REFUTED / UNKNOWN; 4-PART BRIDGE UNPROVED (Astra 162 §6 Q8)  (N1a-R07)
...
CURRENT PRECISION-TWO WORK = RUNG 3 = EDGE 2,016 -> 54,432 (NOT 54,432 -> Q_0):                    注 1.3 (D11; Astra(返書 162)§0)
TARGET-KERNEL COMPARISON (6.1), ONE EDGE:                                                         PAPER-CLOSED (Astra 162 §6 Q7; re-derived here) (補題 5.4.1)
ACTUAL R07 KERNEL-SURJECTIVITY DATA FOR (6.1):                                                    NOT SUPPLIED
PROP C ENUMERATION:                                                                               INFEASIBLE, NOT MATHEMATICALLY UNCOMPUTABLE (注 C.1)
ALL-EDGE COVER PRESENTED AS A COUNTABLE FAMILY:                                                   SUFFICIENT IF ACTUALLY PROVED; single finite schema = desired practical form (D31)
POINTED SYZYGY ROW (b-pt):                                                                        RETAINED; absorption into (c) only under the v191 construction test (Astra 162 §6 Q6)
...
- **D5(必須・撤回・§5.2 (i′)/§0/要約/§10)**: 「**形式的 P–H syzygy は存在しない**」を **撤回**.  syzygy であること自体からは pentagon ⟹ hexagon は出ない(Astra の反例: 関係 `H1 − H2 = 0` は
  `P = 0` かつ `H1 = H2 ≠ 0` を許す).  内部の同型例(**v2.3 の D16 で条件形に修正**): (S1) の θ 同変性を**仮定すれば**同じ reductio が立つ — ただし独立の第二論証ではなく Astra の反例と同型の instance.
...
- **D8(必須・撤回・§5.2 表/§5.3/§0/§10)**: 「(c) が唯一の生存経路」を **撤回**(research-status の記述であって数学的排除ではない; Astra(返書 162)§1 Q3「No necessity theorem is supplied」).
...
- **D10(必須・限定・§4 注 B.3/§0/§5.3/§10)**: 「全段非空 ⟺ 証人」を **lane 限定**に.  定理 B が与えるのは `lim← S_n` の元 = **lane 証人**(登録 pro-3 lane + ν gate のみ)であって無条件の証人ではない.
  side gate(marking/formation/onto/settlement)は各段で restriction と可換なら `S_n` に追加でき König はそのまま通る(未確認・射程外); mixed-prime gate(v220 §15 Priority C)/perfect-core gate(同 Priority D)は 3 冪逆系では**原理的に見えない**((T-iii)).
- **D11(軽微・§1.3 注 1.3/系 A.1/§10)**: 段の名指しの辞書を追加 — **現行 precision-two 作業 = rung 3 = edge `2,016 → 54,432`**(`54,432 → Q₀` = rung 4 は未着手; Astra(返書 162)§0 1. の前提訂正).
...
- **D14(軽微・§8)**: Q1–Q5 に Astra(返書 162)§1 の回答を記録して閉じ, 新規 **Q6**(pointed (b) と (c) の層の関係 — v191 が (b-pt) を吸収するか)・**Q7**(owner の選択 = v395 (3.4)/v526 (4.2)/v537 (2.2) の
  どれを主に行くか; 非集約核と物理核の比較補題を立てる価値)・**Q8**((N1a) の翻訳前提を厳密にする note の在庫)を追加.

**v2.2 が変えなかったもの(Astra の保持判定).**  定理 A(物理商 `Ẑ` 上の有限厳密性・完全下位 fibre `D_d`)/ 注 A.3(境界込み定式の訂正)/ 系 A.1 の 19 grade 勘定と注(f)/ 注 A.2(NONMEMBER の非対称)/
定理 B (i)–(iv) とその証明 / 注 B.0(根なし vs 根付き)/ 注 B.2(3 つの失敗型)/ 命題 C / §6 の有限検査条件 (i)–(xi)(特に (vi) = **ν は P ブロックと同時**)/ §7 / §9 の機械出力と sha16.

**v2.2 → v2.3 の変更点(工房 falsifier 前哨 `fal_a0cl_v2_2_preflight_v1.md` の要修正 2 点 + 軽微 8 点; 判定は「修正後は発送可・数学的差し戻しなし」).  定理 A・定理 B・命題 C とその表題・証明は不変; 新規計算なし.**
- **D15(要修正 F1・§1.3 注 1.3)**: 「本稿の他の箇所の『rung N』はこの表で読む」は **偽**だったので撤回し, **適用範囲を T1 に限定**する一文へ置換.  前哨の指摘どおり, note は「rung」を T2(相対 Frattini)の
  意味でも 12 箇所以上で使っており, 表で読むと **命題 C「rung 1 は計算不能」と 系 A.1「rung 1(段[504])= MEMBER」が正面衝突**し, §1.2 見出しの「rung 0 → 1」(T1 に rung 0 は無い)も破綻していた.
  v2.3 では T1 側(§1.3 上段・系 A.1・§5.1 の「rung 1 (504)」・§6 (vii)/(M1)(M2))と T2 側(§1.2 見出し・§1.3 下段・定理 B・命題 C・系 B.1・注 B.2・§5.1 の「rung 0」・§7・Q5・VERDICT (3)・要約)を
  明示的に列挙して**別系列**と宣言(T2 rung 0 = A0 = T1 の全段).  **命題 C の表題は変更していない**(不変条件を守るため, 衝突は注 1.3 側で解消).
- **D16(要修正 F2・§5.2 (i′)/§10 D5)**: D5 の「内部反証」の前件を **条件形**に書き直した.  引用先はいずれも仮定形である — v169 §5「**Suppose** … θ acts continuously on (3.3)」, v399「The actual R07
  equivariance … **are not asserted here**」, v112 は v111 の **leading-layer datum** についての登録(§2 は `Bθ = θB` を assume).  よって「R07 には**実際に** (S1) が在り」は言えず, 「(S1) の θ 同変性を
  **仮定すれば**」と書く.  あわせて, この reductio は (S1) が `E_P` を含まない以上 **Astra の反例 `H1 − H2 = 0` と同型の instance**であって独立の第二論証ではないと明記し, 逆に **(N1a) の「H-forcing」
  限定がなぜ要るかの実例**として位置づけ直した.  claim boundary は維持((N1a) の根拠は在庫 (2) の有限反例であって, この instance ではない).
- **D17(軽微 F3・§5.4)**: (d1)/(d2) の「未証明の前件」欄に **語つき source section σ** と **all-edge specialization**(関手的 schema; v526 §6 / v537 §4)を追加し, Astra(返書 162)§3 の 4 本列挙
  (source section, kernel columns, common word, all-edge specialization)に揃えた.  また「**edge は無限個**」の限定を自分の 定理 B (iii) 行だけでなく **(d) 行にも対称に**書いた(前哨の指摘:
  自分の経路にだけ厳しい非対称は D9 の趣旨に反する).
- **D18(軽微 F4 + 前哨 §1.4・研究者向け要約/claim boundary)**: 要約に (N1a) の**翻訳前提**(DLL の商族と R07 の登録 `K_{r,n}` の翻訳は未証明 = §8 Q8)を残し, claim boundary の `REFUTED WITHIN SCOPE` を
  **`REFUTED IN THE DLL FINITE-QUOTIENT SCOPE (stock (2)); TRANSLATION TO THE REGISTERED K_{r,n} UNPROVED`** に置換(行内から scope が読めるように).
- **D19(軽微 F5・Status/§1.2/§9)**: 「再実行もしていない」と「本稿で再実行」の字面衝突を解消.  provenance 確認の結果, `fal_a0cl_e3check_v1.g` の再実行は **v2**(v2 Status/§9 の記述), `fal_a0cl_nu_pent_check_v1.g` の
  再実行は **v2.1**(D1), **v2.2・v2.3 では再実行なし**.  3 箇所を版名で統一した.
- **D20(軽微 F6・注 1.3)**: 「地図 9/3 追記も同じ訂正済」→ **段名の訂正行は 9/4 delta(`docs/地図.md` L27)**; 9/3 delta は元から同じ target 名規約(訂正されたのではない).
- **D21(軽微 F8・§9)**: falsifier 作 GAP 2 本の sha16(`fal_a0cl_nu_pent_check_v1.g` = `5b883a1c7e5a528f`, `fal_a0cl_e3check_v1.g` = `08efb9fb8de74df6`)と前哨報告書の sha16
  (`fal_a0cl_v2_2_preflight_v1.md` = `7fd1c0e71b3a5428`; 報告書自身の申告値 `cffa2024236649fc` は末尾行追記前の本文に対する値)を §9 に追加 — 出所管理の一貫性(自作 2 本だけに sha16 を付す非対称の解消).
  **4 本とも本稿で再計算して照合**(GAP の実行はしていない).
- **D22(軽微 F9・§5.2 在庫 (2))**: DLL Property 4.3 を「weak: charming `f`」と要約していたのを訂正 — 原文 Property 4.3 の条件は「代表が `[F_2/N_{F_2}, F_2/N_{F_2}]` に入る」であり, **charming(Def 2.19)の
  前半条件のみ**(charming は加えて `T^{F_2}_{m,f}` の条件を課す).  数え(13/35・残り 22)は不変.
- **D23(軽微 F11・注 B.3/注 B.2/§7/§10)**: 「B(mixed-prime)・C(perfect-core)(v220 §16 規則 8)」は**誤ったラベル**.  v220 §15 の実物は **Priority C = mixed-prime actual membership / Priority D = perfect-core gate**
  (A = universal word-pair, B = single-seed pre-gate)で, §16 規則 8 は「pro-3 lift を all-prime/perfect-core lift と呼ばない」= **文字を割り当てていない**.  4 箇所を正しい引用に訂正した(うち 注 B.2・§7 は
  v2.1 から継承した既存箇所の修理 = D5–D14 の外).
- **D24(完全性 F10)**: v2.2 で行った **系 B.1 の見出しへの併読指示追加**(「v2.2 の限定は §5.2/§5.4 と注 B.3 を併読」)が D5–D14 に列挙されていなかったので, ここに記録する(内容の変更ではなく見出しの
  ポインタ追加).
- **D25(参照 F7)**: 台帳 `provenance/LEDGER.md` L668 の文言(「profinite 版は一般には偽」)は **裁定 2092 で errata**(司令塔処理).  本稿 §5.2 の D7 注はその参照を持つのみで, 台帳の修正は行わない.

**v2.3 が変えなかったもの.**  定理 A / 定理 B / 命題 C の本文・表題・証明(前哨 §5 でバイト同一を機械照合済), 注 A.2 / 注 A.3 / 注 B.0 / 系 A.1 の 19 grade 勘定, §6 の有限検査条件 (i)–(xi), §7 の依存鎖,
§9 の機械出力そのもの, および v2.2 の narrowing 判断((N1a)(N1b)(N1c)・D6 の UNKNOWN 格下げ・D7 の読み替え・D8 の撤回・D10 の lane 限定)— 前哨は「Astra(返書 162)§2 の 3 要求を過不足なく吸収」と判定した.

**v2.3 → v2.4 の変更点(Astra(返書 162)§6「Reply to the v2.3 delivery and Q6--Q8」の反映).  定理 A・定理 B・命題 C とその表題・証明は不変; 新規計算なし.**
- **D26(§5.4・補題 5.4.1 新設)**: Astra §6 Q7 の **(6.1) target-kernel 比較補題**(`0 → K_N/(K_N∩J) → K_E/J → K_L/(A'J) → K_L/(A'K_E) → 0`)を記録し, **本稿で独立に再導出**(紙; 同じ論法の第二執行であって
  第二系統ではない).  帰結: **非集約 cover ⟹ 物理 cover ⟺ `A'K_E = K_L`** / **物理 cover ⟹ 非集約 cover ⟺ `K_N ≤ J`**; `A'` 全射なら連結写像 `δ` で gap を検査でき, `rE` も全射なら
  `A'K_E = K_L ⟺ rE : ker A' → ker A` 全射.  これにより v2.2/v2.3 の「**追加の比較なしには順序づけ不能**」(自己評価 3)を**撤回・精密化**した — 順序は 2 条件で決まる.
  ただし **R07 の実 kernel-surjectivity データは未供給**なので, どちらの cover が実際に安いかは UNKNOWN のまま(1 edge の補題であり all-edge には各 edge での成立が要る).
- **D27(§5.2 (N1a) の分離)**: (N1a) を **(N1a-DLL)[確立]** と **(N1a-R07)[未確立]** に分けた.  翻訳前提は「商族の比較可能性」1 点ではなく **4 点 bridge**(登録クラスに admissible な preimage /
  coarse H・P 条件 / 正規化条件 / 次 edge の P 成功と H 失敗の保存)であり(Astra §6 Q8), **この bridge が無い限り R07 の all-edge 主張は反証されていない**.  (b-univ) の R07 における地位は
  「反証済み」から **UNKNOWN** に後退し, claim boundary も 2 行に分けた.
- **D28(注 C.1 新設・operational 化)**: 命題 C の「計算不能」等は **実用上列挙不可能(infeasible)であって数学的に計算不能(uncomputable)ではない**と読む — 注 C.1 を新設し, §0 VERDICT (3)・
  研究者向け要約 (ii)・§5.4 の「測定不能」欄にも同じ読みを適用.  **命題 C の主張・表題・証明は変更していない**(不可能性定理として引用しない, という運用規約の追加).
- **D29(§8)**: Q6–Q8 を Astra §6 の回答で**閉じた**.  回答を要約し, 各項に工房側の見立てとの**一致/相違**を 1 行ずつ記録(Q6 = 相違・受諾, Q7 = 一致 + 補題の受領, Q8 = 一致 + 不足の訂正).
  §8 の見出しも「数学監査官への問い(すべて回答済み)」に更新.
- **D30(§5.2 表 (b-pt) 行)**: 「pointed syzygy の可能性は**削除しない**」を明記し, (c) への吸収は**自動ではなく** v191 の source で登録 word-pair・contracting operator・boundary witness を実際に構成し
  all-refinement 恒等式を証明する場合に限る, という**判定条件**を書いた.  v2.2–v2.3 の「実質的に (c) と同じ pointed 層」は言い過ぎ(語の共有を層の同一視に使っていた)として撤回.
- **D31(all-edge cover の operational 化)**: v2.3 の D17 で書いた「**個別 cover の可算列では足りない**」を**撤回**.  Astra §6 逐語「an actually proved cover for every edge is sufficient even if presented as a
  countable family, while a single finite schema is the desired practical way to prove all of them」に従い, §5.4 の (d1)/(d2)/定理 B (iii) 行を「可算族の提示でも十分・単一の有限 schema は実用上望ましい形」に修正.
- **D32(呼称)**: 数学監査官の役名 Sol(常設)と, 2026-09-05 以降の返書の著者名 **Astra** を Status に明記し, 返書 162 の引用 25 箇所+関連文(反例・保持判定・逐語判定)を **「Astra(返書 162)」**に統一.
  2026-09-04 以前の note(v526/v537/v539/v504/v395/v191/v174 等)の著者表記は逐語どおり **Sol** のまま.
- **D33(§5.4 自己評価 6 の追加)**: Astra §6 の総評「**Its new table does not supply either route's missing source columns**」を受諾して記録 — 表は位置づけと未証明前件の可視化が仕事で,
  実 cover の source columns も A0 の coarse member も供給しない.  **表が増えたことを進捗と数えない**.

**v2.4 が変えなかったもの.**  定理 A / 定理 B / 命題 C の本文・表題・証明, 注 A.2 / 注 A.3 / 注 B.0 / 注 B.2 / 注 B.3, 系 A.1 の 19 grade 勘定, §6 の有限検査条件 (i)–(xi), §7 の依存鎖, §9 の機械出力と sha16,
および v2.2–v2.3 の narrowing 判断((N1b)(N1c)・D6 の UNKNOWN 格下げ・D7 の読み替え・D8 の撤回・D10 の lane 限定・D15 の rung 系列分離・D16 の条件形化)— Astra §6 は「各境界は本返書を正しく取り込んでいる」と判定した.

`R07_A0_COFINAL_LIFT_TWO_TOWERS_FABLE_V2_4`
