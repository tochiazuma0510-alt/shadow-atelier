...
**改版**: **v1.1(2026-08-29・裁定 1764)** — 第 1 遠征(LMFDB 9T27)の結果を受け §2.2/§2.4 を差替、§1.8(対象被覆の同定)と §3 の 4 経路を追加。**ファイル名は据置**(同一文書の改版・§0′ に訂正表)。
...
- **★ 定義体**: $C$ の $(\mathbb Z/9)^\times$ 内の安定化群は $\{1,8\}=\{\pm1\}$(軌道長 3)⟹ **定義体 $=\mathbb Q(\zeta_9)^+$**(導手 9 の巡回三次体・判別式 81・LMFDB `3.3.81.1`)。**$\mathbb Q$ ではない。** `gate: step0_rationality_probe_v1.g` (sha16 `9cd4cf02f5795192`)
- **同一視の曖昧性**: $C_{S_9}(\langle X_4,Y_4\rangle)=1$ ⟹ マーク済み対を合わせる同時共役子は**一意・残存 0 ビット**。passport $[9,9,9]$ の類は $N_{S_9}$ 同時共役で **9 個**、$G$ 同時共役で 27 個。`gate: passport_probe_v2.g` (sha16 `9f7e4898704f1a14`)
...
**必要な外部入力(仕様)**: **group `9T27`($=PSL(2,8)$・次数 9・primitive)、passport $\lambda=[[9],[9],[9]]$、種数 4、定義体 $\mathbb Q(\zeta_9)^+$(導手 9 の巡回三次体・判別式 81・LMFDB `3.3.81.1`)の明示 Belyi 写像**、および**そのファイバーのモノドロミー整合ラベル**(共役類だけでは不可 — §1.3 により $\Psi$ は類関数でないため)。
...
**生存経路**: (i) **自前計算**(rigidity により一意解・定義体も判別式 81 と小さい。ただし種数 4 × 次数 9 の明示化は Magma/Sage 級環境と専門家を要し**工房外**)(ii) **Breuer の曲面作用表・van Hoeij–Vidunas への照会**(signature $(0;9,9,9)$・閉包種数 169 で引く。**未実施・1 便**)(iii) **明示 $\mathrm{Ih}_M$ モデルの文献**(旧候補 1・不変)。
...
**新しい第一歩 = 生存経路 (ii) の照会**: **Breuer の曲面作用表・van Hoeij–Vidunas** に、$PSL(2,8)$ の signature $(0;9,9,9)$ 作用(閉包種数 169)の明示モデル・定義体・既知の Belyi 写像があるかを引く。**1 便で判定可能。**
**これが空振りなら**、生存経路 (i)(自前計算の外部委託)の可否が**研究者判断の分岐点**になる。
