...
**⟹ 本版をもって札 1 は「測定済み資産(§1)+ 完全な不在記録(§2.5)+ 一意な再開手順(§2.2″/§2.4)」の三点セットで完結する。**
...
- **発注判断 = 研究者(選定会議)。** 本書は材料の提供までとし、判断はしない。
...
**最終形の第一歩(2026-08-29 確定)= 研究者の発注判断**。文献側に踏むべき手は残っていない(§2.5 が完全探索の記録)。分岐は 2 つだけ:
1. **自前計算(§2.2″)を外部委託する** — 実行すれば翻訳ビットは決まる(rigidity より解は一意)。
2. **将来の文献出現を待つ** — 本書 §2.2′ の要求仕様(passport $[[9],[9],[9]]$・種数 4・定義体 `3.3.81.1`・**マーク付きファイバー**)が、そのまま**将来の検索クエリ**として使える。
