...
**閉じるための最小要求(1 行)**: pin(または census producer)に **各 roster の定義式**(どちらの符号の $\ker(\Psi-c'\mathcal K_3)$ か、あるいは同値に「どちらが $\varepsilon=+1$ 側か」)を**文字列として**記録すること。**数値の一致では代替できない。**
⟹ **`conclusion_pending_D6` は解除しない。**
