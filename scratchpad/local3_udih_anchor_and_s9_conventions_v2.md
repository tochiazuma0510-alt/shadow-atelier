...
| **★ D-12(新設・裁定 1732)** | **$\Psi$/$\mathcal K_3$ の向き宣言** | K1–K5 評価器は **$\mathcal K_3$ = 2405 Thm 4.3 の $k\bmod3$**(第 1 $D_n$ 成分の $r$ 指数 × $2^{-1}\!\!\bmod9=5$)、**$\Psi$ = 規約 D の向き**($u_{S4}=u_0$)で実装したことを cert に宣言する | 数学者(確定) | 追加 |
| **★ D-13(新設・裁定 1732(b) 承認)** | ★ **$\kappa(m)$ 偶数不変条件(canary)** | **$\kappa(m)=m+1$($m$ 奇)/ $-m$($m$ 偶)は常に偶数**。⟹ $\psi_n(z^{\kappa(m)})=(1,1,r^{\kappa(m)})$ が成り立つのは $\kappa$ が偶数のときだけで、**奇数 $\kappa$ を入れると $\psi_n(z)$ の第 1・第 2 成分(鏡映)を拾って公式が壊れる**。実装は $\kappa(m)$ を計算したら**偶数性を assert** すること。機械実測: `math_thm43_k_v1.g` で偶 $\kappa$ 全域 TRUE / 奇 $\kappa$ 全域 FALSE | 数学者(確定・**機械裏取り済**) | **canary** |
