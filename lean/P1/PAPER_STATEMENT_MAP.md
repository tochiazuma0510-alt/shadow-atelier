...
| oddH window carrier `H_{j,?,?}` | `window.J`, `window.Code`, `window.H`, `window.encode` | `j` is an exact two-point type; `H` is a predicate on actual `Gn n`, not ambient `En n` | Foundation closed |
