/-
P1/BlockA.lean ? ???? A((6?) ???LA-1?LA-9)?

??: docs/notes/lean_p1_allocation_plan_v1.md ?3.1 ?(??? 1:1)?
??: docs/notes/oddH_full_proof_v1.md(?? A?I??? ODD-H)/
      docs/notes/s3_family_completion_v1.md(?? ?-REG??? INN??? SIXP-fam)/
      docs/notes/w2fam_v1.md ?3.5(?? K5-1)?

??: **local targeted-build candidate**??????????????? `sorry` ???
LA-2?LA-5 ? LA-6 ???????LA-6 ???????????? actual `Gn n`
???????????LA-7?LA-9 ????? OPEN ????
-/
...
/-! ### LA-1: `Gn.structure`(oddH ?? A)

$G_n=A\rtimes Q$?$|G_n|=4n^3$?$X=a_1q_1$?$X^2=a_1^2$?$\operatorname{ord}(X)=2n$?

?? 4 ?????????(???????v1.5)???????????(4n?)?
`Gn_card_equiv` ??????? `Gn_card_formula` ????????????
plain core ? `Fintype.card` ???????cardinality witness ? exact ??????? -/
...
/-- **LA-1(d)** ?? plain-core ???? -/
structure PlainEquiv (? ? : Type) where
  toFun : ? ? ?
  invFun : ? ? ?
...
/-! ### LA-6: ?? ?-REG(`Lambda.simplyTransitive`)

???????: H ? G_n ????(?? `IsSubgrp`)???[G_n:H]=2n ??
?X??H=1(oddH ?? C(2)??? G ?????? ? ODD-H ?????????
????????????????? **?????????**)?????
?X? ? H ???? ? ?????????????
...
/-- The conjugacy class ? of `H`, represented extensionally as predicates on `G_n`. -/
...
/-! ### LA-8: ?? INN(`Phi_F0_eq_inn`)

$\Phi_{0,f_k}=\operatorname{inn}(X^{-2k})$???? $f_k=(r^{2k},r^{-2k},1)$?
??: s3_family_completion_v1.md ?4(b)???(w2fam_v1.md ?3.5 ???)?

???? **??? Y ?????**(?????)???????:
$f_k^{-1} Y f_k = X^{-2k} Y X^{2k}$(???? $a_1^{1-4k}a_2a_3q_2$ ????)?
?? ? G_n ??????????????????????(K3 ? F17 ?????
????)????????**???????????????????**????
??? emul/dmul ???????????(decide ???n ?????)? -/

/-- f? = (r^{2k}, r^{-2k}, 1) = a?^{2k}a?^{-2k}(s3_family_completion ?4)?
    ???? exponent k : Fin n ? 2k ? Fin n ???(k+k)?-2k ?????? -/
def fk (k : Fin n) : En n := ((k + k, false), (-(k + k), false), done_)

/-- ??(inner automorphism): inn(c)(g) = c g c??? -/
...
/-- **LA-8(??? X ???????)**: inn(X^{-2k})(X) = X
    (X ???????? X ??????? ? X ??????????????
    s3_family_completion ?4(b)?$\Phi_{0,f_k}(X)=X$?? `inn(X^{-2k})(X)=X` ?
    ???????????? ? **??**: ??? f???Xf?=X ????
    (f? ? X ??????????)??????? X ?????????????
    ?????? inn ??????)? -/
...
`U_{j,?} ? ?(?e?)q_j?`; `e` records the nontrivial `Q`-coset. -/
...
/-- `H_{j,?,?}` as a predicate on the actual carrier `Gn n`, not on ambient `En n`. -/
...
/-- **LA-3(a)**: subgroup closure of `H_{j,?,?}`. -/
...
`XCode` representatives and `H_{j,?,?}` codes.  Identifying all `XCode`
